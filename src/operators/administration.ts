...
export async function approveRegisteredOperator(context: AdministrationContext, input: {
  ...
  requireCurrentAuthority(context);
  const distribution = await context.registry.getProtectedDistribution(input.operatorId);
  ...
  const connectionSecret = await openOperatorSecret(distribution.connectionSecretCiphertext, context.encryption,
    { purpose: 'connection', recordId: input.operatorId });
  ...
  if (manifest.id !== input.operatorId) throw new ValidationError('Operator identity does not match registration');
