...
interface BuiltInDraft extends Omit<NormalizedReasoningProfile, 'hash' | 'schemaVersion' | 'enabled' | 'builtIn' | 'aliases' | 'levels' | 'reasoningMode'> {
  reasoningMode?: NormalizedReasoningProfile['reasoningMode'];
...
function makeBuiltIn(draft: BuiltInDraft): NormalizedReasoningProfile {
  const supportedLevels = draft.reasoningMode === 'provider-default' && draft.supportedLevels.length === 0
    ? [] : validateLevels(draft.supportedLevels, `${draft.id}.supportedLevels`);
...
export const BUILT_IN_REASONING_PROFILES: readonly NormalizedReasoningProfile[] = Object.freeze([
  ...
  makeBuiltIn({
    ...
    reasoningMode: 'provider-default', ingressContract: 'ai-gateway-chat-completions', supportedLevels: [], unsupportedLevels: ALL_LEVELS,
    removePaths: COMMON_REMOVALS, levelMappings: {}, aliases: {}, offSemantics: { status: 'unsupported' },
    toolCompatibility: { status: 'verified', levels: [], evidence: 'OpenAI-compatible streaming tool call and exact replay.' },
    recognizedResponseFields: { content: ['choices[].message.content'], tools: ['choices[].message.tool_calls'], usage: ['usage.completion_tokens_details.reasoning_tokens'] },
    validatedTransports: ['compat'], classification: 'Verified',
...
export function translateReasoningRequest(payload: Record<string, unknown>, profile: NormalizedReasoningProfile, level: PiReasoningLevel): Record<string, unknown> {
  if (!profile.enabled || profile.reasoningMode === 'provider-default') throw new Error('reasoning profile has no configurable levels');
