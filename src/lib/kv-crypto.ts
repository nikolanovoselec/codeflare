 ...
 *
 * Encrypts/decrypts credential values stored in KV (llm-keys, deploy-keys).
 ...
 * Returns null if the entry doesn't exist or decryption fails.
 */
export async function getAndDecrypt<T>(
  ...
  if (!cryptoKey) {
    // No encryption — read as JSON directly
  ...
  try {
    ...
    // Corrupted or pre-encryption data — return null
    return null;
  }
}
