...
describe('REQ-MEM-001/REQ-MEM-002: root-owned memory delivery lifecycle', () => {
  ...
  it('REQ-MEM-015 AC4: exposes identical extraction items to the model and durable metadata', async () => {
    ...
    if (!memoryItem) throw new Error('missing model-visible memory request');
...
describe('REQ-VAULT-027: transactional Pi Vault extraction delivery / REQ-VAULT-028 (vault-edit isolation after launch)', () => {
  it('REQ-VAULT-027 AC2: reconstructs Vault retry state from root JSONL after reload', async () => {
    ...
    appendEntry(harness.sessionFile,
      toolCall('vault-failed-call', 'vault-extract', launch.request),
      notification('vault-failed-call', 'Error'),
    );
