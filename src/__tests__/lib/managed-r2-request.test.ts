...
describe('REQ-ENTERPRISE-028 managed R2 request classifier', () => {
  it('allows reads and listing while denying cross-bucket targets', async () => {
    ...
    expect((await classify('HEAD', 'https://user-bucket.account.r2.cloudflarestorage.com/.claude/extensions/company/index.ts')).action).toBe('allow');
  ...
  it('decodes exactly once and rejects malformed, noncanonical, backslash, duplicate-control, and empty mutations', async () => {
    expect((await classify('PUT', 'https://account.r2.cloudflarestorage.com/user-bucket/.claude/extensions/%E2%82%AC.md')).action).toBe('deny');
    expect((await classify('PUT', 'https://account.r2.cloudflarestorage.com/user-bucket/%252eclaude/extensions/x')).action).toBe('allow');
  ...
  it('denies a whole mixed multi-delete and forwards exact ordinary bytes', async () => {
    const mixed = '<Delete xmlns="http://s3.amazonaws.com/doc/2006-03-01/"><Object><Key>Vault/a.md</Key></Object><Object><Key>.claude/extensions/company/index.ts</Key></Object></Delete>';
