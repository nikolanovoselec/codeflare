...
const canonicalSkill = (relativePath: string) => readFileSync(
  fileURLToPath(new URL(`../../../preseed/agents/claude/skills/${relativePath}`, import.meta.url)),
  'utf8',
);
...
describe('REQ-AGENT-134: advanced design skill suite', () => {
  ...
  it('REQ-AGENT-134: ships upstream licenses and marks the adapted Canvas file', () => {
    ...
    expect(canvas?.content).toBe(canonicalSkill('canvas-design/SKILL.md'));
  });
