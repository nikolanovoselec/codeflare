...
describe('Container Lifecycle - Scoped R2 Tokens', () => {
  ...
  // --- Agent credential pipeline tests ---
  ...
  it('anthropicApiKey from KV flows through setBucketName body', async () => {
    mockKV._set('llm-keys:codeflare-test-example-com', {
      openaiApiKey: 'sk-openai',
      anthropicApiKey: 'sk-ant-test123',
    });

    const body = await startSessionAndGetBody();
    expect(body.anthropicApiKey).toBe('sk-ant-test123');
    expect(body.openaiApiKey).toBe('sk-openai');
  });

  it('missing anthropicApiKey does not appear in setBucketName body', async () => {
    mockKV._set('llm-keys:codeflare-test-example-com', {
      openaiApiKey: 'sk-openai',
    });
  ...
  it('consultLlmEnabled preference flows through setBucketName body', async () => {
    mockKV._set('preferences:codeflare-test-example-com', {
      consultLlmEnabled: true,
    });

    const body = await startSessionAndGetBody();
    expect(body.consultLlmEnabled).toBe(true);
  });

  it('KV_ENCRYPTION_KEY flows through as r2EncryptionKey', async () => {
    const body = await startSessionAndGetBody({ KV_ENCRYPTION_KEY: 'dGVzdC1lbmNyeXB0aW9uLWtleQ==' });
    expect(body.r2EncryptionKey).toBe('dGVzdC1lbmNyeXB0aW9uLWtleQ==');
  });
