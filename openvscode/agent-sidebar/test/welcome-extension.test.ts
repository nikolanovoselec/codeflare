...
test('REQ-IDE-053 AC3: VS Code activation reuses and focuses a surviving session-agent terminal', () => {
  vi.useFakeTimers();
...
test('REQ-IDE-053 AC3: reconnect creates exactly one managed session-agent terminal', () => {
  vi.useFakeTimers();
  process.env.CODEFLARE_SESSION_WORKSPACE = 'vscode';
...
test('REQ-IDE-053 AC4: non-VS Code workspaces never create or focus an agent terminal', () => {
  vi.useFakeTimers();
