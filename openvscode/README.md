...
The pinned private request contract distinguishes editor submissions and supplies the invoking document, selection, and whole range. Codeflare uses that host-owned location as the sole editor source, emits one validated `textEdit` group plus its completion marker, and never falls back to whichever editor later has focus. The pinned Inline controller owns native Keep/Close, session settlement, disposal, and editor navigation; Codeflare emits no review confirmation, notification action, Chat Editing command, or document reopen. Pi RPC remains the inference path and does not use VS Code Authentication or Copilot.
