...
The pinned private request contract distinguishes editor submissions and supplies the invoking document, selection, and whole range. Codeflare uses that host-owned location as the sole editor source and follows the pinned Copilot protocol: one empty `textEdit` start marker, one validated edit batch, then one completion marker for the same URI. The managed Pi profile disables automatic chat-edited-file opening, so the host retains the already-open invoking editor instead of pinning another tab. The pinned Inline controller owns native Keep/Close, session settlement, disposal, and navigation; Codeflare emits no review confirmation, notification action, Chat Editing command, or document reopen. Pi RPC remains the inference path and does not use VS Code Authentication or Copilot.
