...
Pi uses strict LF-delimited JSONL with bounded records and correlated request IDs. It has no session file. The sidebar guard replaces edit, write, and Bash with approval-aware tools, keeps same-file approval windows serialized, checks the target again before mutation, and sends only an opaque manifest ID through Pi's extension UI request. The OpenVSCode extension host reads the mode-0600 manifest, displays the preview, and owns the confirmation.
