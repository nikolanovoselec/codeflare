...
`sidebar-settings.json` keeps native ask rules for network and MCP tools. Bypass and automatic modes, Remote Control, IDE auto-install, updates, and nonessential telemetry are disabled. `pre-tool-use-permission.mjs` auto-allows the fixed read-only set plus routine local Edit, Write, NotebookEdit, Bash, and Task actions; unknown, network, MCP, and Jupyter execution actions ask. Malformed input or internal failure exits 2.
