...
// Only the unrelated system CA operations are stubbed; startup initialization and
// Pi file generation remain the production shell. No system filesystem writes.
export const systemTrustIsolation = `
cp() {
  if [ "$1" = "/etc/cloudflare/certs/cloudflare-containers-ca.crt" ] && [ "$2" = "/usr/local/share/ca-certificates/cloudflare-containers-ca.crt" ]; then return 0; fi
  command cp "$@"
}
update-ca-certificates() { :; }
`;

export function extractPiCleanup(entrypoint) {
  const start = entrypoint.indexOf('    # Managed Pi cleanup:');
  const end = entrypoint.indexOf('    # End managed Pi cleanup.', start);
  if (start < 0 || end < 0) throw new Error('Pi cleanup markers not found');
