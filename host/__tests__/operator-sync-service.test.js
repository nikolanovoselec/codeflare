...
test('REQ-OPERATOR-005: trusted config composes an exact upload with durable receipt', async t => {
  ...
  const serializedConfig = JSON.stringify({ schemaVersion: 1, activityId: 'activity-1', sessionId: 'session-1',
    policyDigest: 'b'.repeat(64), root, remotePrefix: 'Remote Reviews/activity-1/session-1/', deadline: Date.now() + 60_000 });
  const controller = createOperatorSyncService({ serializedConfig, allowedRoot, bucket: 'owner-bucket',
  ...
  const response = await controller.handle({ method: 'POST', pathname: '/internal/operator/sync/operations',
  ...
  assert.deepEqual(calls.map(([, args]) => args[1]), [
    'r2:owner-bucket/Remote Reviews/activity-1/session-1/sync-1/report.txt',
    'r2:owner-bucket/Remote Reviews/activity-1/session-1/sync-1/manifest.json',
  ]);
...
test('REQ-OPERATOR-005: absent config preserves ordinary host and malformed/escaping config fails closed', () => {
  ...
  const escaping = JSON.stringify({ schemaVersion: 1, activityId: 'activity-1', sessionId: 'session-1',
    policyDigest: 'b'.repeat(64), root: '/other/output', remotePrefix: 'safe/', deadline: Date.now() + 60_000 });
  assert.throws(() => createOperatorSyncService({ serializedConfig: escaping, allowedRoot: '/owned', bucket: 'owner',
    rcloneConfig: '/run/rclone.conf' }), /configuration/i);
});
