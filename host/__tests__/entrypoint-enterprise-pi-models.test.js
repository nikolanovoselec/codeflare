...
describe('entrypoint enterprise Pi models.json build (REQ-ENTERPRISE-005 / REQ-ENTERPRISE-031)', () => {
  ...
  it('REQ-ENTERPRISE-031 AC2: forces dynamic-route prompts into the system role supported by Workers AI models', () => {
  ...
  it('REQ-ENTERPRISE-031 AC1: does not write provider config outside Enterprise mode', () => {
    const dir = mkdtempSync(join(tmpdir(), 'non-ent-pi-models-'));
    const modelsPath = join(dir, '.pi/agent/models.json');
    const script = [
      'set -euo pipefail',
      `USER_HOME='${dir}'`,
      extractEnterpriseBlock(),
    ].join('\n');
    const res = spawnSync('bash', ['-c', script], { encoding: 'utf8' });
    assert.equal(res.status, 0, `inactive Enterprise block exited non-zero: ${res.stderr}`);
    assert.equal(existsSync(modelsPath), false, 'Pi provider config written outside Enterprise mode');
  });
