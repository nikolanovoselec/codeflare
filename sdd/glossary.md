...
| Implements Annotation | A comment in a source file naming the REQ it implements (e.g. `// Implements REQ-AUTH-001`); spec-reviewer greps for the literal REQ ID when classifying code-vs-test coverage. Recommended whenever code implements observable behavior from a REQ; enforced by spec-reviewer's source-vs-test coverage check when `enforce_tdd: true`. |
