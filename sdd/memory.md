...
3. When triggered, the main agent spawns a background sonnet agent that prefilters the recent transcript via `prefilter-transcript.sh` (drops tool_use/tool_result/synthetic-wrapper noise and chunks the remainder), accumulates per-chunk observations into a scratchpad before synthesising the final note, and writes the capture file into `/home/user/Vault/Raw/Sessions/{ISO_TS}-{SID_SHORT}.md` with timestamps in the user's local timezone (`Europe/Zurich`).
