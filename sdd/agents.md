...
7. PR-boundary review fires only for PRs targeting `main` or `master`: either a new pull request opens with that target via `gh pr create`, or a new push lands on a branch that already has an open PR with that target. PUSH_LINE detection in the Stop hook (`enforce-review-spawn.sh`) recognises both `git push` and `gh pr merge` invocations across all three tool surfaces (Bash, `mcp__*__ctx_batch_execute`, `mcp__*__ctx_execute` with `language=shell`); the `gh pr merge` surface is required because a server-side merge into `develop` advances the develop->main PR HEAD without producing a local `git push` line in the transcript, and without that recognition the review pipeline silently fails to arm after every feature->develop merge. Layer 1 lane classification is provided by a shared helper (`lib/lane-classifier.sh`) sourced by both `enforce-review-spawn.sh` and `git-push-review-reminder.sh` so the in-turn nudge and the turn-end gate agree on which agents are required for the diff (doc-only push -> only `doc-updater`; sdd/-only push -> `spec-reviewer` then `doc-updater`; any source touch -> all three). If the classifier helper is missing or fails to source, both hooks fail-closed to the legacy all-three-lanes posture (`code-reviewer spec-reviewer doc-updater`) rather than exiting silently, so a partially-deployed install never disables review enforcement. Layer 2 (authoritative `gh pr view` HEAD SHA comparison) filters any false positives. On these triggers `spec-reviewer` runs first, then `doc-updater` runs second (sequential, never parallel) on any project containing `sdd/`.
