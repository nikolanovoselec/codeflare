...
9. Each decision is its own commit (`[sdd-init] resolve TRIAGE-{NNN}` or `mark lost`). Two developers resuming triage in parallel resolve conflicts via normal git merge.
10. When the last `Status: open` item is resolved or marked `lost`, the resolving commit also clears `transition: true` from `sdd/config.yml` and appends a single line to `sdd/changes.md` recording transition completion with totals (accepted / corrected / lost). `sdd/init-triage.md` is preserved as the audit record — resolved and lost items remain visible. The next spec-reviewer run applies full SDD discipline.
