...
6. The served worker tolerates a non-array remote `fetchFileList()` result (a transient proxy error or a stray Cloudflare Access 302 HTML body) without crashing the sync loop: the graft coerces the remote list to an empty array at the full-sync-cycle consumer binding (before `getNonSyncCandidates`'s `forEach` and the remote `map`), so a non-array response is a safe no-op cycle (zero candidates, no deletion, persisted snapshot preserved) instead of a repeating `forEach is not a function` throw. The coercion is a graft-layer string transform; the verbatim bytes and the SHA-256 drift guard are unchanged. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served sync cycle no-ops on non-array remote list, iterates a real array, verbatim chain throws without the coercion, graft preserves the SHA-256 version guard) -->
