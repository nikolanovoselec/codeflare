...
1. The vault-monitor daemon polls the vault on a short fixed cadence, excluding the agent-written capture directory, the served graph-viz copy directory (re-rendered by the extractor's own final step — detecting it re-triggers the extractor on its own output), the derived graph-output directory, the vendored editor plug-bundle directory, the editor's internal config directory, and the four preseed-managed root pages. The exclusion list is identical across runtimes (the Claude daemon/contract `find` prunes mirror Pi's generated-prefix set). The four pages are codeflare-authoritative; the per-boot preseed copy must not count as a user edit, otherwise every preseed sync at boot would re-trigger extraction. <!-- @impl: entrypoint.sh::start_vault_monitor_daemon --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (Pi vault indexing shares Claude marker semantics and exclusions) -->
...
1. Browser-initiated Service Worker registration GETs for the editor's service-worker script short-circuit the auth chain and receive SilverBullet's native service worker from the Worker (vendored verbatim, AD69). Cold-boot encryption rides the native worker's own `set-encryption-key`/`get-encryption-key` handlers, fed by the bootstrap-hop page ([REQ-VAULT-024](#req-vault-024-vault-bootstrap-hop-key-arming-and-service-worker-retention) AC1). <!-- @impl: src/routes/vault-native-sw.ts::VAULT_NATIVE_SERVICE_WORKER_JS --> <!-- @impl: src/routes/vault.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault.test.ts (isServiceWorkerRegistration / REQ-VAULT-017 (native SW short-circuit selector)) --> <!-- @test: src/__tests__/routes/vault.test.ts (VAULT_NATIVE_SERVICE_WORKER_JS / REQ-VAULT-017 AC1 (native SW served, AD69)) --> <!-- @test: src/__tests__/routes/vault-auth-chain.test.ts (native SW + shell-302 suppression (REQ-VAULT-017 AC1/AC4/AC5, AD69)) --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (the graft injects the __cfRecover helper that the verbatim worker lacks) --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (SilverBullet version drift guard) -->
...
**Intent:** Once the Vault is proven ready, opening and re-opening it must be instant and must never re-litigate that proof. A green ('armed') control opens the editor directly with no per-open re-verification; a full prewarm proof persists a per-browser marker so a later same-container page load skips the bootstrap iframe entirely and arms the control without a click (a resumed session, whose container restarted, re-initializes cleanly like a fresh one); the control surfaces the 2-click on-demand flow as a breathing affordance so the user always knows what state it's in; and a one-time controlled reload self-heals an already-warmed vault that boots before its service worker takes control. Once armed, the control stays green for the rest of the session.

**Applies To:** User

**Acceptance Criteria:**

1. A green ('armed') control opens the vault directly via the bootstrap-hop, synchronously inside the click gesture, with no per-open re-verification of local readiness or key recoverability. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload-armed (green) click opens directly — no readiness/key re-verify; a green click opens directly even when local readiness reports not-ready) -->
2. The full-prewarm marker — keyed to the container start (`lastStartedAt`), recorded at prewarm completion — drives the reload-skip OPTIMISTICALLY: the control arms (green) with no click the instant the marker exists and live local readiness holds, WITHOUT waiting for the current start to poll in (so a same-container return re-greens immediately); if the start later polls in and differs from the marker (a resumed container), the optimistic green is REVOKED and the control drops to 'available' for a clean re-init. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::readVaultPrewarmMarker --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (returns the exact container start that recorded the marker) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (returns null when this browser never prewarmed the session) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports warm for the same container start that recorded the marker (a page reload)) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports NOT warm when the container start advanced, forcing a clean re-init on a resumed session) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports NOT warm when the current container start has not polled in yet, so a resumed session never skips onto a stale store) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (a start-less prewarm records no marker, so it re-prewarms next time rather than over-skipping) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (never reads warm against an empty store) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click (no iframe) when the vault is already warm on this device (reload)) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (an optimistic reload-skip green is REVOKED when the polled-in start proves a resume) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a resume revokes even a COLD-PATH armed green (prewarmed-but-never-opened), dropping to available) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a resumed session whose lastStartedAt advanced is not reload-skipped and stays available until click) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a batch-status poll tick mid-prewarm does not tear down the in-flight iframe (arms green despite session-store churn)) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (the container start polling in mid-prewarm (null -> real) does not tear down the in-flight iframe, and arms green stamping the real start) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (preserves the prewarmed marker for an ACTIVE session during an orphan sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker for an ORPHAN session during a sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker on session DELETE) -->
3. If the readiness probe does not settle or the marker is absent, the reload-skip is ineligible and the control stays 'available' for an on-demand click rather than auto-mounting onto an unbuilt index. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload with local DBs/SW but no full prewarm proof stays available until click; a reload-skip probe that never settles leaves the button available) -->
4. The control breathes through the on-demand flow: 'available' before prewarm; the first click breathes accent-colour with a focus-loss warning; completion breathes green with a confirmation auto-hiding after 5s; the second click opens instantly. Reduced-motion keeps the colours without the animation. <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (available status is openable (click 1 starts the prewarm) and carries no breathing class; preparing auto-surfaces the status region without a click and wires aria-describedby; armed auto-surfaces the ready region on the genuine preparing->armed transition, then auto-hides after 5s while the breathing class stays) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays available (no auto-prewarm) until click 1, then breathes preparing -> armed) -->
5. On a top-level vault open, when a vault SW is active but `serviceWorker.controller` is null on first paint, the page reloads once via a loop-safe `sessionStorage` one-shot; inert in the prewarm iframe, on first boot, for non-vault scopes, without SW support, or a prewarm id. <!-- @impl: src/routes/vault-html.ts::installVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::injectVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (reloads exactly once when a vault service worker is active but not controlling the page) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does NOT reload a second time once the one-shot flag is set (loop-safe)) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (is inert in the headless prewarm iframe (window.parent !== window)) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does not reload on a genuine first boot with no vault service worker registered) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does not reload when only a non-vault service worker scope is active) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does not reload and clears the one-shot flag once the SW already controls the page) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (is inert when the browser has no service worker support) -->
6. Once armed, the control stays green for the rest of the session, gated solely on the prewarm readiness proof (`pw === 'ready'`, [REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6) rather than an open/settle latch, identically on mobile, tablet, and desktop. <!-- @impl: web-ui/src/components/Layout.tsx::vaultButtonStatus --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays green (armed) after the open click and on subsequent opens — ready means green, always) -->
...
- The reload-skip arms optimistically on marker + live local readiness before the start polls in, then revokes the green if the polled-in start proves a resume ([AD96](../../documentation/decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)).
...
**Dependencies:** [REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger), [REQ-VAULT-019](#req-vault-019-vault-key-recoverable-open-diagnostic), [REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption)

**Verification:** [layout wiring test](../../web-ui/src/__tests__/components/Layout.test.tsx), [local readiness marker test](../../web-ui/src/__tests__/lib/vault-local-readiness.test.ts), [vault button states test](../../web-ui/src/__tests__/components/VaultButton.test.tsx), [cache helper test](../../web-ui/src/__tests__/lib/vault-cache.test.ts), [vault shell helper test](../../src/__tests__/routes/vault-html-direct.test.ts)

**Status:** Implemented

---

### REQ-VAULT-019: Vault key-recoverable open diagnostic

**Intent:** The encryption key must be able to serve the editor when the Vault opens. That key is armed for real at OPEN time by the bootstrap-hop + service-worker `__cfRecover` ([REQ-VAULT-024](#req-vault-024-vault-bootstrap-hop-key-arming-and-service-worker-retention)), so a green control always opens safely. The pre-open `/.vault-key` check therefore runs as a NON-BLOCKING diagnostic at the arming poll — it must not gate the green, because a false-negative on that endpoint left a fresh session breathing accent forever, never green, even though the vault opened fine. The control greens on the prewarm readiness proof alone.

**Applies To:** User

**Acceptance Criteria:**

1. The encryption key's recoverability is probed at the arming poll via the auth-gated `/.vault-key` endpoint as a NON-BLOCKING diagnostic (a non-recoverable result only logs — the control still greens and still opens, because the key is armed at open time by the bootstrap-hop). <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultKeyRecoverable --> <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (checkVaultKeyRecoverable: GET /.vault-key with credentials, true on non-empty key, false on non-2xx/empty/throw) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (the arming poll greens on the prewarm proof even when the key check FAILS (key is a non-blocking diagnostic)) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload-armed (green) click opens directly — no readiness/key re-verify) -->
2. While the on-demand prewarm is in flight (before its proof lands), the control is in a non-openable preparing state. <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (preparing status not openable + accent-breathing class) -->
3. When the prewarm readiness proof holds, the arming poll marks the control armed (green) — the key diagnostic is not required — and the next click opens it synchronously inside the click gesture. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (the arming poll greens on the prewarm proof even when the key check FAILS (key is a non-blocking diagnostic)) --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (armed status openable + green-breathing class + fires onOpen) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a cold-path armed click opens the vault tab synchronously; a reload-armed (green) click opens directly) -->
...
- The key diagnostic runs after browser prewarm and never blocks the green — the key is armed for real at open time ([REQ-VAULT-024](#req-vault-024-vault-bootstrap-hop-key-arming-and-service-worker-retention)).
...
6. The Vault open click navigates the new tab to the bootstrap-hop URL (`/api/vault/<sid>/.codeflare-bootstrap`) rather than the bare shell, so the first open arms the per-session key via the hop's `set-encryption-key` post and SW-activation wait before redirecting to the editor. <!-- @impl: web-ui/src/components/Layout.tsx::openVaultTab --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (the open click navigates the new tab to the bootstrap-hop URL, not the bare shell) -->
