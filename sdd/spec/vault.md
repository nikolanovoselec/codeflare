...
2. A full prewarm proof records a persistent per-browser marker stamped with the container's current start timestamp (`lastStartedAt`); on a later load, the bootstrap iframe is skipped and the control arms (green) directly with no click ONLY when that marker matches the current container start AND live local readiness still holds. A same-container page reload therefore arms instantly, while a resumed session — whose container restarted and whose `lastStartedAt` advanced — is not reload-skipped and re-initializes the vault cleanly like a fresh session (one on-demand click → prewarm → green). <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports warm only for the same container start that recorded the marker) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports NOT warm when the container start advanced, forcing a clean re-init on a resumed session) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click when already warm) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a resumed session whose lastStartedAt advanced is not reload-skipped and stays available until click) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (full-prewarm marker lifecycle: preserved for active, removed on orphan/delete) -->
