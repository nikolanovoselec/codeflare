...
3. For a vault that did not exist at boot, the manifest is baselined before the R2-persisted durable initialization record is written. An existing marker-less vault receives the record without baselining so its files remain extraction-eligible. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (baselines only the first durable Vault initialization and leaves a later missing manifest full-delta eligible (REQ-VAULT-026 AC3/AC4)) -->
