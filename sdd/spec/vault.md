...
3. For a newly created vault, the manifest is baselined from current content only before the R2-persisted durable initialization record exists; the record is written only after a successful first baseline. Legacy-vault migration may write the durable record without baselining so existing files remain extraction-eligible. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (baselines only the first durable Vault initialization and leaves a later missing manifest full-delta eligible (REQ-VAULT-026 AC3/AC4)) -->
