...
2. The served worker guards against destructive sync while SilverBullet is not yet ready: when a sync cycle's remote file list is empty or non-array while the local store or sync snapshot is non-empty, the cycle aborts before deleting and retries on the next interval instead of treating every local file as deleted. A genuinely empty vault stays a safe no-op, and a real non-empty list reconciles normally. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served sync cycle aborts on empty/non-array remote list while the store or snapshot is populated, no-ops on an empty vault, reconciles a real list, verbatim chain does not abort without the graft) -->
3. The graft's transform MUST NOT introduce a duplicate lexical binding into the served worker's declarator list: a duplicate declarator breaks the worker's JavaScript parse, so the browser never registers the SW and the vault never becomes ready. The served worker stays syntactically valid JavaScript. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served worker stays syntactically valid JavaScript, a duplicate-binding graft variant is rejected, graft preserves the SHA-256 version guard) -->
