...
3. Before registering the canonical worker, the user-initiated bootstrap enumerates same-origin registrations and unregisters every Vault scope except the exact canonical scope. It re-enumerates afterward and fails closed if any stale Vault worker remains; unrelated service workers are untouched. <!-- @impl: src/lib/vault-view.ts::unregisterStaleVaultServiceWorkers --> <!-- @impl: src/lib/vault-view.ts::injectVaultBootstrapHopHtml --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (unregisterStaleVaultServiceWorkers) -->
4. The ready bridge reports the canonical scope together with completed runtime, sync, index, required-file, and file-list evidence. The dashboard rejects cross-origin or noncanonical-scope proofs. <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @impl: web-ui/src/lib/vault-prewarm.ts::startVaultPrewarm --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (reports only canonical-scope runtime/content proof after the new worker completes) --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (rejects a ready proof that claims a cross-origin Vault scope) -->
...
1. The Worker delivers the key through a one-time, GET-only bootstrap-hop page (other methods fall through) that first removes stale Vault service workers, registers the canonical native worker, posts the key to its `set-encryption-key` handler, persists an enable-encryption flag, and sets a bootstrap-completed cookie before redirecting to the shell. <!-- @impl: src/lib/vault-view.ts::unregisterStaleVaultServiceWorkers --> <!-- @impl: src/lib/vault-view.ts::injectVaultBootstrapHopHtml --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (unregisterStaleVaultServiceWorkers) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (completeVaultBootstrap / REQ-VAULT-024) -->
