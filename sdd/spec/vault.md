...
8. A full prewarm proof records a durable per-browser marker for the session. On a later load the bootstrap iframe is skipped (and the control marked ready directly) only when that marker is set AND live local readiness still holds (recorded `sb_data_*`/`sb_files_*` + active service worker, not evicted); the liveness probe is bounded by a short timeout that falls back to mounting the iframe. This stops a reload of an already-initialized device from re-running service-worker registration, space sync, or indexing — and from contending with the terminal for focus — while an interrupted first-init (stores/SW present but no recorded full proof) still re-prewarms instead of opening onto an unbuilt index. The marker lives in the `vault-session-*` cache namespace, so the orphan-cache sweep treats `vault-session-<sid>-prewarmed` as belonging to `<sid>` — preserved for an active session, removed on session delete/orphan — rather than mistaking it for a bogus orphan and erasing it on every dashboard mount. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::listSessionMarkers --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (skips the bootstrap prewarm when the vault is already warm on this device; still mounts when a full prewarm never completed; mounts when the readiness probe does not settle) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (full-prewarm marker lifecycle: preserved for active, removed on orphan/delete) -->
