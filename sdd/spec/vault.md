...
2. The cutover creates fresh current database identities while leaving every historical service-worker registration and IndexedDB database untouched. <!-- @impl: src/lib/vault-view.ts::installVaultIdbRecorder --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (invalidates only readiness markers when the canonical scope changes, then records fresh DB names) -->
3. Missing, superseded, or orphaned worker-scope evidence cannot satisfy local readiness or trigger the controlled-reload path. <!-- @impl: src/lib/vault-view.ts::findExactVaultRegistration --> <!-- @impl: src/lib/vault-view.ts::installVaultControlledReload --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (binds readiness behavior to the exact current Vault scope) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (forces the unchanged prewarm lifecycle when legacy readiness has no recorded scope) -->
4. Scope cutover uses the existing white, accent-preparing, green-ready, instant-second-click lifecycle without automatic prewarm. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-018: stays available (no auto-prewarm) until click 1, then breathes preparing -> armed) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-019: a cold-path armed click opens the vault tab synchronously) -->
5. Native-worker precaching does not cause unrelated Vault activity to expire or fail under the parallel asset burst. <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault-auth-chain.test.ts (does not schedule same-key session writes for the complete SilverBullet precache shape) --> <!-- @test: src/__tests__/routes/vault-auth-chain.test.ts (still records activity for unrelated or navigation GETs with a cache-version query) -->
