...
11. After the open click the warm control settles out of the green-breathing 'armed' state to a plain neutral 'ready' status (still clickable to reopen) instead of breathing green indefinitely; the per-session settle latch is cleared when the session is left or the page reloads so a fresh visit re-arms green, and a further click on the settled control still opens the vault. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (settles to a neutral non-breathing status after the open click, and a further click still opens) -->
...
9. On every boot the vault initializer stamps the four codeflare-authoritative pages (Index, CONFIG, README, STYLES) with the immutable preseed source's modification time (`touch -r`), even when the content-equality skip left the file's bytes untouched, so the in-container SilverBullet "secondary" server reports an identical `lastModified` for these pages across every session. Without the stamp, bisync/cp give a byte-identical `Index.md` a fresh mtime each boot; on a 2nd session that mtime diverges from the persisted client sync snapshot, so the SW sync engine reports the page "changed on secondary" on every editor watch-poll, re-enqueues one index op per cycle, and the prewarm readiness gate (index queue empty for consecutive polls, [REQ-VAULT-018](#req-vault-018-vault-browser-prewarm-readiness-gating) AC6) never settles — leaving the Vault control breathing 'preparing' forever. A deterministic mtime keeps the persisted snapshot in agreement so the spurious "changed on secondary" never fires; cold start is unaffected because its snapshot is built fresh in-session. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (stamps the codeflare-authoritative pages with the deterministic preseed mtime even when the content-equality skip leaves the bytes untouched, and not the drifted boot mtime) -->
