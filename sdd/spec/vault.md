...
11. After the open click the warm control settles out of the green-breathing 'armed' state to a plain neutral 'ready' status (still clickable to reopen) instead of breathing green indefinitely, and a further click on the settled control still opens the vault. The settle latch is recorded both in memory and in a per-session `localStorage` marker (`markVaultOpened`/`hasVaultOpened`), and the control settles when EITHER the in-memory signal OR the persisted marker is set, so a standalone-PWA reload/remount that wipes the in-memory signal on return from the vault tab keeps the control settled-neutral instead of re-breathing green and re-surfacing the ready tooltip forever. <!-- @impl: web-ui/src/components/Layout.tsx::openVaultTab --> <!-- @impl: web-ui/src/components/Layout.tsx::vaultButtonStatus --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultOpened --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultOpened --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (settles to a neutral non-breathing status after the open click; settles via the persisted opened latch after a remount that wiped the in-memory signal; a further click still opens) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (opened latch: markVaultOpened/hasVaultOpened persist per session) -->
