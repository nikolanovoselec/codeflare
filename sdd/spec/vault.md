...
3. Same-tag transitions (two clones with identical directory basenames, or branch switches within the same repo) issue no removal, because the active checkout's tag is excluded from the removal set and the global add replaces the existing entry via its source-hash dedup. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::REPO_BASENAME --> <!-- @test: host/__tests__/graphify-active-repo.test.js (graphify-active-repo.sh single-active-repo maintenance / REQ-VAULT-014 (graphify active-repo invariant + lock serialisation)) -->
