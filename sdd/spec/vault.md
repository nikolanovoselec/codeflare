...
7. Opening without a fresh prewarm — the reload-skip path, where the control is already green from a prior session — rechecks the current browser's local readiness AND key recoverability before opening; if the cache or key has evaporated, the app drops back into a fresh on-demand prepare instead of opening a stale tab. On the cold path the arming poll has already verified the prewarm proof and the key, so the green control opens synchronously inside the click gesture. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload-armed click rechecks this browser cache + key before opening; re-prepares when the local cache evaporated) -->
