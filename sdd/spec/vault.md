...
7. The service worker retains its in-memory encryption key for the worker's natural lifetime, and recovers it from the Worker only when it is genuinely gone (browser idle-termination of the SW). The codeflare graft (`graftVaultKeyRecovery`) does two complementary things. (a) It NEUTERS the upstream proactive flush: the native worker's `setInterval` that wiped the key 5s after the last window client disconnected is grafted to keep its no-client log but drop the `y=void 0` wipe, so the key survives the bootstrap-hop -> shell `location.replace` transition — a brief 0-client gap whose 5s flush tick used to wipe the just-posted key and bounce cold opens to `.auth`. (b) It injects a `__cfRecover()` helper that re-fetches the key from the auth-gated `.vault-key` endpoint (a same-origin SW fetch carries the session cookie) and decodes it with the worker's own decoder, and calls it at BOTH of the worker's key-empty failure points before either gives up: the `config` message handler's `enableClientEncryption && !y` auth-gate (the path that posts an auth-error and navigates the client to `.auth` / "Authentication not enabled"), and the `get-encryption-key` reply — covering a genuine cold worker restart after idle-termination. The key stays re-derivable from the bucket via `.vault-key` and the browser still idle-terminates the worker, so retaining it in SW memory is not a meaningful forward-secrecy regression. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault.test.ts (graftVaultKeyRecovery .vault-key recovery branch + drift-throw) --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served worker retains the key on no clients — flush neutered; verbatim negative-control wipes it) -->
8. The Vault open click navigates the new tab to the bootstrap-hop URL (`/api/vault/<sid>/.codeflare-bootstrap`) rather than the bare shell, so the first open arms the per-session key through the hop's `set-encryption-key` post and SW-activation wait (AC5) before redirecting to the editor, instead of relying on the single-shot `__cfRecover` recovery (AC7). The bare-shell open carried the bootstrap cookie, skipped the hop, and -- when the native worker had flushed the per-session key after the prewarm iframe closed -- lost the recovery race, so SilverBullet top-level-navigated to `.auth` and only a close-and-reopen recovered. <!-- @impl: web-ui/src/components/Layout.tsx::openVaultTab --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (the open click navigates the new tab to the bootstrap-hop URL, not the bare shell) -->
...
- The per-session identifier must be resolvable for every proxied request to preserve parallel-session isolation (each session maps to its own container DO; cross-session reads/writes never collide). For the bucket-stable SB-serving path the session id is carried in the `cf_vault_sid` routing cookie rather than the URL segment (see [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key)); the session-keyed entry/status path keeps it in the URL. All sessions of one user now share a single bucket-stable IndexedDB store by design (REQ-VAULT-021), so isolation is enforced at the container/auth layer, not by per-session IDBs.
...
**Notes:** Encryption rides SilverBullet's native service worker (AD69): the bootstrap-hop posts the key to the native worker's `set-encryption-key` handler, and a codeflare graft (`graftVaultKeyRecovery`) adds `.vault-key` recovery at the worker's two key-empty checkpoints - the `config` auth-gate AND `get-encryption-key` (AC7). The first integration deploy (no graft) bounced to `.auth` on cold boot; a second (graft on `get-encryption-key` only) still bounced, which localized the real trigger to the `config` gate (it reads the key directly, never asking `get-encryption-key`); the graft now covers both. Verified on the integration deploy (mobile, 2026-06-01): cold boot reaches the editor with no `.auth` bounce, the console logs "Recovered encryption key from codeflare" (the graft fired because the hop's `set-encryption-key` had already been flushed), and the encrypted `sb_data_*` / `sb_files_*` stores open and decrypt. The SB store now persists across sessions via a bucket-stable vault URL and bucket-derived key (see [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key)); [AD83](../../documentation/decisions/README.md#ad83-vault-indexeddb-cannot-be-persisted-across-sessions-by-keying-the-encryption-key-to-the-r2-bucket) is superseded — its DB-id analysis (why a key-only change was insufficient) is retained there. Update (2026-06-26): the cold-boot `.auth` bounce recurred even with `__cfRecover` in place, because the upstream proactive 5s key flush could still fire during the hop->shell 0-client gap and wipe the just-posted key. The graft now NEUTERS that flush (AC7) so the key is retained across the transition; `__cfRecover` is kept only for a genuinely idle-terminated worker.
...
1. The vault initializer copies the three codeflare-authoritative config root pages (CONFIG, README, STYLES) from the preseed source into the vault root on every boot, gated so identical files are not rewritten; `Index.md` is seeded create-if-missing rather than force-overwritten (see [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC11) because the editor normalizes and autosaves it. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (stamps codeflare-authoritative config pages with the deterministic preseed mtime even when cmp-skip leaves content untouched; seeds Index.md create-if-missing but NEVER force-overwrites an existing differing Index.md) -->
2. User content lives in dedicated user-owned subdirectories (notes, references, inbox, journal, raw-pasted, raw-sessions) and is never touched by the preseed sync; only the three codeflare-authoritative config pages are force-overwritten (`Index.md` is create-if-missing, so its edits persist). <!-- @impl: entrypoint.sh::init_user_vault --> <!-- coverage-gap: no genuine behavioral test — "User content lives in dedicated user-owned subdirectories (notes, references, inbox, journ..." is shell/process/build boot behavior with no isolatable runnable unit in the node:test pool -->
...
3. The frontend runs a session-vault-cache cleanup on session delete (not on stop), which removes the session's persisted localStorage markers (the IDB-recorder entry, the session marker, and the full-prewarm marker). Per [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) the SilverBullet IndexedDB stores and the vault service worker are bucket-stable (shared across all of a user's sessions) and are therefore NOT deleted/unregistered on per-session delete — doing so would erase the next session's vault and force a full re-index. The boot recorder (populated by a shim that wraps page-context IndexedDB opens and by native-worker IDB-open messages, keyed by the real session id) still records the IDB names for readiness checks. <!-- @impl: web-ui/src/lib/vault-cache.ts::cleanupSessionVaultCache --> <!-- @impl: src/routes/vault-html.ts::injectVaultIdbRecorder --> <!-- @impl: src/routes/vault-html.ts::VAULT_IDB_RECORDER_MARKER --> <!-- @test: src/__tests__/routes/vault.test.ts (injectVaultIdbRecorder) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (does NOT delete the recorded sb_ IndexedDB stores; does NOT unregister the vault service worker; removes the vault-session-<sid> marker and leaves other sessions intact) -->
4. After an authoritative session-list fetch succeeds, the frontend sweeps persisted localStorage markers and, for any session no longer in the user's active sessions list, drops the corresponding marker entries (covers the case where the session was deleted from another device). Per [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) the sweep does NOT delete the bucket-stable IndexedDB stores or unregister the vault service worker. Dashboard mount alone must not trigger a sweep because the initial store may be empty before the fetch resolves. <!-- @impl: web-ui/src/stores/session.ts::loadSessions --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (sweepOrphanVaultCaches removes markers, preserves bucket-stable IDB) --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (loadSessions: authoritative session-list sweep only after success + stale generation ignored) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (dashboard mount does not sweep Vault IndexedDB caches) -->
...
- The cleanup/sweep helpers operate exclusively on localStorage markers and never call the browser's databases-listing API or `deleteDatabase`. After the [REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) reconciliation they do not touch IndexedDB at all, which makes the historical "live session's IDB nuked on every dashboard mount" regression structurally impossible.
...
6. The served worker GUARDS the remote `fetchFileList()` result so a not-yet-ready or transiently-unreachable in-container SilverBullet server can never drive a destructive sync: the graft wraps the `o=` initializer with an IIFE that normalizes a non-array body (a transient 5xx, an auth hiccup, or a stray Cloudflare Access 302 HTML body) to `[]`, then THROWS to abort the cycle before any per-file reconciliation when the remote list is empty while the persistent local store (`s`) or the sync snapshot (`t.files`) is non-empty. `syncSpace` rethrows and the sync `run()` loop logs a downgraded warn and retries on its next interval (~20s), so the cycle simply defers until the server is actually serving the real list; a genuinely empty vault (empty primary AND empty snapshot) stays a safe no-op. This replaces the earlier blind coerce-to-`[]`, which — once the local store is bucket-stable and persistent ([REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC8) — made every local file look "deleted on secondary" and wiped the whole vault on a 2nd-session start. The guard MUST keep `o` a single binding in the minified `let s=…,o=…,r=…` declarator list — adding a second `o=` declarator is a duplicate lexical binding that makes the whole served worker fail to parse (the browser then never registers the SW and the vault never becomes ready); the served worker therefore stays syntactically valid JavaScript. The guard is a graft-layer string transform; the verbatim bytes and the SHA-256 drift guard are unchanged. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served sync cycle aborts on empty/non-array remote list while the store or snapshot is populated, no-ops on an empty vault, reconciles a real list, verbatim chain does not abort without the graft, served worker stays syntactically valid / duplicate-let graft rejected, graft preserves the SHA-256 version guard) -->
7. The served worker NEUTERS the upstream proactive key flush: the native worker wiped its in-memory encryption key 5s after the last window client disconnected, which raced the bootstrap-hop -> editor 0-client transition and bounced cold opens to `.auth`; the graft keeps the no-client log but drops the `y=void 0` wipe so the key is retained for the worker's natural lifetime (cold-restart recovery rides `__cfRecover`, [REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption) AC7). The flush anchor is registered in the graft's drift-guard loop so a SilverBullet version bump that moves it throws rather than silently shipping the wipe; the verbatim bytes and SHA-256 drift guard are unchanged. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @impl: src/routes/vault-native-sw.ts::ANCHOR_PROACTIVE_FLUSH --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served worker retains the key on no clients — flush neutered; verbatim negative-control wipes it; served worker stays syntactically valid) -->
...
**Intent:** The Vault warms up on demand, not automatically. The control becomes clickable once SilverBullet is serving; the user's first click starts the prewarm (the button breathes the codeflare accent and warns that terminal focus may briefly drop), and a second click opens the editor once indexing is green. A reload of an already-warm device shows the control green and opens it without re-indexing. A user should never land on an unreachable editor, a slow first-click indexing screen, or — the failure this supersedes — an empty editor for up to two minutes that needs a manual reload.

**Applies To:** User

**Acceptance Criteria:**

1. The Vault control remains guarded until the per-session vault proxy probe succeeds, stays visible, and exposes click/tap feedback while unavailable. <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Header.test.tsx (guarded prewarm/timeout/error click feedback + ready click opens) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays available until click 1 then preparing->armed; timeout retries; mid-prewarm leave re-requires a click on return) -->
2. The server probe — `probeVaultReady`, a `GET /api/vault/:sid/status` check that reports ready only when SilverBullet is serving and emits no 502/timeout console noise — retries until first success, is keyed per session, and clears readiness after a later steady-probe failure. <!-- @impl: web-ui/src/lib/vault-readiness.ts::startVaultReadinessProbe --> <!-- @impl: web-ui/src/lib/vault-readiness.ts::probeVaultReady --> <!-- @test: web-ui/src/__tests__/lib/vault-readiness.test.ts (startVaultReadinessProbe: retries past 60-attempt cap, latches on first success, clears on steady-probe fail, cancel stops chain) --> <!-- @test: web-ui/src/__tests__/lib/vault-readiness.test.ts (probeVaultReady: ready only when /status reports vaultReady; not-ready on warming/non-2xx/error) -->
3. Browser prewarm starts ON DEMAND — never automatically — only after the user clicks the server-ready ('available') control; it requests best-effort persistent browser storage on that first click, and timeout/error attempts retry while the control breathes 'preparing'. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/browser-storage-persistence.ts::requestBrowserStoragePersistence --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays available with no auto-prewarm until click 1, then preparing; retries the on-demand prewarm after a timeout) --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (iframe URL/session scoping, ready/timeout/cancel cleanup) --> <!-- @test: web-ui/src/__tests__/lib/browser-storage-persistence.test.ts (requestBrowserStoragePersistence: already persisted / grant / denial / unsupported / estimate failure) -->
...
6. The bridge is inert without valid prewarm parameters and emits ready only after the SilverBullet runtime is ready, the current browser has recorded `sb_data_*`, recorded `sb_files_*`, and an active per-session service worker, SilverBullet's service worker has completed a full space sync, the current object index version is complete with an empty index queue, and the local `/.fs/` listing contains the codeflare-authoritative vault files — and that full readiness proof has held across `requiredReadyStreak` (2) consecutive poll cycles, where a single not-ready poll resets the streak, so a momentary index-queue-empty mid-sync cannot arm the control prematurely. <!-- @impl: src/routes/vault-html.ts::injectVaultPrewarmBridge --> <!-- @impl: src/routes/vault.ts::handleVaultRequest --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (checkVaultLocalReadiness: recorded sb_data/sb_files + active service worker + optional databases API proof) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (sanitized prewarm query propagation + generic shell bridge injection + space-sync/content canary guard; arms only after the readiness proof holds across multiple consecutive polls — the stable-green streak) -->
7. A green ('armed') control opens the vault directly via the bootstrap-hop, synchronously inside the click gesture, with NO per-open re-verify of local readiness or key recoverability. A green button is ready by definition — proven at the cold-path arming poll ([REQ-VAULT-019](#req-vault-019-vault-key-recoverable-open-gate)) or by the AC8 reload-skip eligibility check — and the bootstrap-hop re-arms the SW encryption key on open while the worker no longer flushes it mid-transition ([REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption) AC7), so a direct open never races a wiped key into a `.auth` bounce nor a healthy persisted store into a redundant re-index. (The earlier per-open re-verify keyed `checkVaultLocalReadiness` by `activeSessionId`, whereas the recorder/bridge key the `vault-session-<sid>-idbs` marker by the precache-frozen `boot.sessionId` = `cf_vault_sid` cookie session; that divergence false-negatived on every subsequent in-session open and dropped the green button into a ~10s re-prewarm — the reported "re-index on every click". Removing the open-time gate cures it.) KNOWN RESIDUAL: the same marker-key divergence still false-negatives the AC8 reload-skip eligibility check, so a returning session (whose `cf_vault_sid` session cookie was cleared on browser close while the bucket-stable SW/precache persist) does not auto-green and needs one on-demand click; the durable fix — bucket-keying the readiness marker — is tracked separately and deliberately out of scope here. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload-armed (green) click opens directly — no readiness/key re-verify; a green click opens directly even when local readiness reports not-ready) -->
8. A full prewarm proof records a persistent per-browser marker for the session. On a later load the bootstrap iframe is skipped and the control is marked armed (green) directly — with NO click — only when that marker is set AND live local readiness still holds (recorded `sb_data_*`/`sb_files_*` + active service worker, not evicted); the liveness probe is bounded by a short timeout. If the probe does not settle, or the marker is absent (an interrupted first-init with stores/SW present but no recorded full proof), the reload-skip is simply ineligible and the control stays 'available' for an on-demand click — it is never auto-mounted. This stops a reload of an already-initialized device from re-running service-worker registration, space sync, or indexing — and from contending with the terminal for focus — while never opening onto an unbuilt index. The marker lives in `localStorage` under the `vault-session-*` key prefix, so the orphan-cache sweep treats `vault-session-<sid>-prewarmed` as belonging to `<sid>` — preserved for an active session, removed on session delete/orphan — rather than mistaking it for a bogus orphan and erasing it on every dashboard mount. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click when already warm; a reload with DBs but no full proof stays available until click; a reload-skip probe that never settles leaves the button available) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (full-prewarm marker lifecycle: preserved for active, removed on orphan/delete) -->
9. The control exposes the on-demand 2-click flow as a breathing affordance: server-ready is 'available' (clickable, no breathing); the first click breathes the codeflare accent and auto-surfaces a focus-loss warning while indexing; completion breathes green and auto-surfaces a ready confirmation that hides after 5s while the green breathing persists; the second click opens instantly. A `prefers-reduced-motion` user gets the same state colours without the breathing animation. <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (available openable + no breathing class; preparing auto-surfaces the status region + aria-describedby; armed auto-surfaces the ready region on the genuine preparing->armed transition, then auto-hides after 5s while the breathing class stays) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays available with no auto-prewarm until click 1, then breathes preparing -> armed) -->
10. On the real top-level vault open (not the headless prewarm iframe), a one-time controlled reload self-heals an already-warmed vault that boots before its service worker controls the page: when a vault-scoped service worker is active but `navigator.serviceWorker.controller` is still null on first paint, the page reloads exactly once so SilverBullet boots against the SW-backed local space instead of rendering an empty/partial editor that previously needed a manual reload. A `sessionStorage` one-shot makes the reload unloopable; it is inert in the prewarm iframe, on a genuine first boot (no vault service worker yet), when only a non-vault service worker scope is active, and when the browser has no service-worker support, and it clears the one-shot once the worker already controls the page. The safety net is wired only when the request carries no prewarm id. <!-- @impl: src/routes/vault-html.ts::installVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::injectVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (reloads exactly once when a vault service worker is active but not controlling the page; loop-safe one-shot; inert in the prewarm iframe, on first boot, for a non-vault scope, and without service-worker support; clears the flag once the worker already controls the page) -->
11. Once the vault is ready the control is green ('armed') and stays green for the rest of the session; green is gated solely on the prewarm readiness proof (`pw === 'ready'`, AC6), never on any open/settle/opened latch, so neither the open click nor a standalone-PWA reload — which remounts the dashboard on return from the vault tab — moves it off green, and the state is deliberately identical on mobile, tablet, and desktop because green carries no reload-dependent settle state. Neutral ('available') shows before the vault is ready; the codeflare accent ('preparing') breathes only while the on-demand prewarm runs after the first click. The 'armed' ready tooltip likewise auto-surfaces only on the genuine preparing->armed transition (tracked via the button's previous status), never on a fresh already-armed mount, so a warm reload or return from the vault tab does not re-pop it. A further click on the green control still opens the vault. <!-- @impl: web-ui/src/components/Layout.tsx::vaultButtonStatus --> <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays green (armed) after the open click and on subsequent opens — ready means green, always) --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (a fresh already-armed mount (warm reload / return from the vault tab) does NOT re-pop the ready tooltip) -->
...
6. The browser readiness machinery keeps working on the bucket-stable serving path: the shell's injected boot script carries the REAL session id (so the IDB-recorder and prewarm bridge key their `vault-session-<sid>-*` localStorage markers by the value the dashboard reads), while base-href and the CSRF cookie use the bucket token. Because the SilverBullet service worker now registers under the bucket-stable `/api/vault/<token>/` scope — which the dashboard cannot name by session id — the service-worker lookup in both the in-shell prewarm bridge and the dashboard readiness check matches any `/api/vault/` registration (there is exactly one per user). <!-- @impl: src/routes/vault.ts::handleVaultRequest --> <!-- @impl: src/routes/vault-html.ts::rewriteVaultHtmlResponse --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: src/__tests__/routes/vault.test.ts (base-href uses the bucket token while the boot recorder uses the real session id) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (finds the vault service worker even when its scope segment is the bucket token, not the session id) -->
7. The bucket-stable IndexedDB stores and service worker are durable per-user resources: per-session DELETE and the authoritative orphan sweep remove only the localStorage markers and never delete the IndexedDB stores or unregister the service worker, so the persisted vault survives across session lifecycle events. <!-- @impl: web-ui/src/lib/vault-cache.ts::cleanupSessionVaultCache --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (cleanupSessionVaultCache + sweepOrphanVaultCaches remove markers but do NOT delete the bucket-stable IDB or unregister the SW) -->
8. The persistent bucket-stable local store is never blind-deleted by the sync engine while the in-container SilverBullet "secondary" server is still warming up (~1-2 min after a fresh session start) or transiently unreachable: a full sync cycle that observes an empty or unreadable remote file list while the local primary or the snapshot is non-empty is aborted before any deletion and deferred to a later cycle, so the SW only deletes files once it has reached SilverBullet and SilverBullet has confirmed the file list. Without this guard, the cross-session persistence introduced by this REQ turns SilverBullet's startup window into a full-vault wipe on the 2nd session. The guard mechanism lives in the native-SW graft ([REQ-VAULT-017](#req-vault-017-silverbullet-native-service-worker) AC6). <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (the served sync cycle aborts with no deletion when the remote list is empty/non-array while the persistent store or snapshot is populated; reconciles normally on a real list) -->
9. On every boot the vault initializer stamps the three force-overwritten codeflare-authoritative config pages (CONFIG, README, STYLES) with the immutable preseed source's modification time (`touch -r`), even when the content-equality skip left the file's bytes untouched, so the in-container SilverBullet "secondary" server reports an identical `lastModified` for these pages across every session. These pages are force-copied from preseed on every boot, so without the stamp bisync/cp gives each a fresh mtime per boot; on a 2nd session that mtime diverges from the persisted client sync snapshot, the SW sync engine reports the page "changed on secondary" on every editor watch-poll, re-enqueues one index op per cycle, and the prewarm readiness gate (index queue empty for consecutive polls, [REQ-VAULT-018](#req-vault-018-vault-browser-prewarm-readiness-gating) AC6) never settles. A deterministic secondary mtime keeps the persisted snapshot in agreement so the spurious "changed on secondary" never fires; cold start is unaffected because its snapshot is built fresh in-session. `Index.md` is exempt from the stamp -- it is no longer force-overwritten but seeded create-if-missing (AC11); the stamp only equalizes the SECONDARY mtime and cannot stop a conflict driven by the PRIMARY (client) save, so the Index.md 2nd-start conflict is fixed at source by AC11 rather than by this stamp. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (stamps codeflare-authoritative config pages with the deterministic preseed mtime even when cmp-skip leaves content untouched) -->
10. The vault initializer seeds `Notes.md` and `References.md` landing pages create-if-missing so the dashboard's bare `[[Notes]]` / `[[References]]` wikilinks resolve to real pages instead of 404ing as broken/aspiring pages; the seed never overwrites a user edit. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (seeds Notes.md/References.md create-if-missing and never clobbers a user edit; every bare page wikilink in the real preseed Index.md resolves to a seeded preseed page) -->
11. The vault initializer seeds `Index.md` create-if-missing and no longer force-overwrites it from preseed, so the SilverBullet editor's normalized re-serialization of `Index.md` on open persists via R2 to a stable no-conflict fixed point instead of being reverted to preseed bytes on the next boot. The prior force-overwrite fought the client's autosave of the editor-normalized `Index.md`, producing a perpetual "changed on both ends" sync conflict (`Index.conflicted:*.md`) that kept the prewarm index queue hot so the Vault control never settled green on a 2nd start. The seed never overwrites an existing `Index.md` that differs from preseed. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (seeds Index.md create-if-missing but NEVER force-overwrites an existing differing Index.md (prevents the 2nd-start sync conflict)) -->
...
- The serving URL and the `cf_vault_sid` cookie (name + path) are identical for every session of one user, so the vault is one logical store per user: opening a second session in another tab overwrites `cf_vault_sid` and subsequent serving requests route to the most-recently-opened session's container. No cross-tenant leak (same bucket, same R2 data); two distinct sessions cannot drive separate vault tabs simultaneously. This is intended given the bucket-stable-IndexedDB premise.

**Priority:** P1

**Dependencies:** [REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption), [REQ-VAULT-017](#req-vault-017-silverbullet-native-service-worker), [REQ-VAULT-015](#req-vault-015-vault-idb-lifecycle-and-listing-filters), [REQ-VAULT-018](#req-vault-018-vault-browser-prewarm-readiness-gating)

**Verification:** [bucket token test](../../src/__tests__/lib/vault-bucket-token.test.ts), [bucket-derived key test](../../src/__tests__/routes/vault-crypto.test.ts), [route dispatch test](../../src/__tests__/routes/vault-bucket-routing.test.ts), [auth-chain serving test](../../src/__tests__/routes/vault-auth-chain.test.ts), [HTML-rewrite split test](../../src/__tests__/routes/vault.test.ts), [readiness SW-scope test](../../web-ui/src/__tests__/lib/vault-local-readiness.test.ts), [cache reconciliation test](../../web-ui/src/__tests__/lib/vault-cache.test.ts), [boot mtime + landing-page test](../../host/__tests__/entrypoint-vault-boot.test.js)
