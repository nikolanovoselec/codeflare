...
- The per-session identifier must be resolvable for every proxied request to preserve parallel-session isolation (each session maps to its own container DO; cross-session reads/writes never collide). The bucket-stable path carries it in the `cf_vault_sid` routing cookie rather than the URL; the session-keyed path keeps it in the URL ([REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key)). All sessions of one user now share a single bucket-stable IndexedDB store by design, so isolation is enforced at the container/auth layer, not by per-session IDBs.
...
1. A green ('armed') control opens the vault directly via the bootstrap-hop, synchronously inside the click gesture, with no per-open re-verification of local readiness or key recoverability. A green button is ready by definition — proven at the cold-path arming poll ([REQ-VAULT-019](#req-vault-019-vault-key-recoverable-open-gate)) or by the AC2 reload-skip check — and the bootstrap-hop re-arms the SW encryption key on open while the worker no longer flushes it mid-transition ([REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption) AC7), so a direct open never races a wiped key into a `.auth` bounce nor forces a redundant re-index. KNOWN RESIDUAL: a returning session whose routing cookie was cleared (while the browser-side service worker and caches persist) does not auto-green via the AC2 reload-skip and needs one on-demand click; see [AD84](../../documentation/decisions/README.md#ad84-retain-the-vault-sw-encryption-key-in-memory-neuter-the-proactive-flush-and-open-a-green-vault-button-directly) for the mechanism history and the deferred durable fix. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a reload-armed (green) click opens directly — no readiness/key re-verify; a green click opens directly even when local readiness reports not-ready) -->
2. A full prewarm proof records a persistent per-browser marker. On a later load, if the marker is set and live local readiness still holds (recorded `sb_data_*`/`sb_files_*` plus an active service worker, checked against a short timeout), the bootstrap iframe is skipped and the control arms (green) directly with no click. If the probe does not settle or the marker is absent (an interrupted first-init), the reload-skip is ineligible and the control stays 'available' for an on-demand click — never auto-mounted onto an unbuilt index. This avoids re-running service-worker registration, space sync, or indexing (and contending with the terminal for focus) on an already-initialized device. The marker's `vault-session-*` localStorage prefix lets the orphan-cache sweep treat it as belonging to its session — preserved while active, removed on delete/orphan — rather than erasing it as a bogus orphan on every dashboard mount. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click when already warm; a reload with DBs but no full proof stays available until click; a reload-skip probe that never settles leaves the button available) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (full-prewarm marker lifecycle: preserved for active, removed on orphan/delete) -->
...
4. On the real top-level vault open (not the prewarm iframe), when a vault-scoped service worker is active but `navigator.serviceWorker.controller` is still null on first paint, the page reloads exactly once so SilverBullet boots against the SW-backed local space instead of an empty/partial editor. A `sessionStorage` one-shot makes the reload unloopable and clears once the worker controls the page; it stays inert in the prewarm iframe, on a genuine first boot (no vault service worker yet), for a non-vault service-worker scope, without service-worker support, or when the request carries a prewarm id. <!-- @impl: src/routes/vault-html.ts::installVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::injectVaultControlledReload --> <!-- @impl: src/routes/vault-html.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (reloads exactly once when a vault service worker is active but not controlling the page; loop-safe one-shot; inert in the prewarm iframe, on first boot, for a non-vault scope, and without service-worker support; clears the flag once the worker already controls the page) -->
5. Once ready, the control turns green ('armed') and stays green for the rest of the session — gated solely on the prewarm readiness proof (`pw === 'ready'`, [REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6), never on an open/settle/opened latch, so neither the open click nor a standalone-PWA reload moves it off green; the state is identical on mobile, tablet, and desktop. 'Available' (neutral) shows before the vault is ready; 'preparing' (codeflare accent, breathing) shows only during on-demand prewarm after the first click. The ready tooltip auto-surfaces only on the genuine preparing->armed transition (tracked via the button's previous status), never on a fresh already-armed mount, so a warm reload or return from the vault tab does not re-pop it. A further click on the green control still opens the vault. <!-- @impl: web-ui/src/components/Layout.tsx::vaultButtonStatus --> <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (stays green (armed) after the open click and on subsequent opens — ready means green, always) --> <!-- @test: web-ui/src/__tests__/components/VaultButton.test.tsx (a fresh already-armed mount (warm reload / return from the vault tab) does NOT re-pop the ready tooltip) -->
...
- The bucket-stable key relaxes per-session forward secrecy of the local cache (a later session can decrypt a prior session's leftover cache). Accepted tradeoff; supersedes the [AD83](../../documentation/decisions/README.md#ad83-vault-indexeddb-cannot-be-persisted-across-sessions-by-keying-the-encryption-key-to-the-r2-bucket) deferral.
- The bucket token carries no PII: it is a hash of the bucket name, never the bucket name or email itself.
- The serving URL and the `cf_vault_sid` cookie are identical for every session of one user, so the vault is one logical store per user: a second session in another tab overwrites the cookie and takes over serving; two sessions cannot drive separate vault tabs simultaneously. No cross-tenant leak (same bucket, same data) — intended given the bucket-stable-IndexedDB premise.
...
2. The persistent bucket-stable local store is never blind-deleted by the sync engine while the in-container SilverBullet "secondary" server is still warming up (~1-2 min after a fresh session start) or transiently unreachable: a sync cycle that observes an empty or unreadable remote file list while the local primary or snapshot is non-empty aborts before deleting and defers to a later cycle, so files are only deleted once SilverBullet has confirmed the file list. Without this guard, the cross-session persistence this REQ provides would turn SilverBullet's startup window into a full-vault wipe on the 2nd session. The guard lives in the native-SW graft ([REQ-VAULT-017](#req-vault-017-silverbullet-native-service-worker) AC6). <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (the served sync cycle aborts with no deletion when the remote list is empty/non-array while the persistent store or snapshot is populated; reconciles normally on a real list) -->
3. On every boot the vault initializer stamps the three force-overwritten codeflare-authoritative config pages (CONFIG, README, STYLES) with the immutable preseed source's mtime (`touch -r`), even when a content-equality skip left the bytes untouched, so the in-container SilverBullet "secondary" server reports an identical `lastModified` across every session. Without the stamp, each boot's fresh mtime would diverge from the persisted client sync snapshot on a 2nd session, so the SW sync engine would report "changed on secondary" every watch-poll, re-enqueue an index op per cycle, and leave the prewarm readiness gate ([REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6) never settling; cold start is unaffected since its snapshot is built fresh in-session. `Index.md` is exempt — it is seeded create-if-missing instead (AC5), since the stamp only equalizes the secondary side and cannot fix a primary (client-save) conflict. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (stamps codeflare-authoritative config pages with the deterministic preseed mtime even when cmp-skip leaves content untouched) -->
