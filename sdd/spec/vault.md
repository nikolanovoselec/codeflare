...
5. Claude and Pi read the same committed manifest format and exclusion set. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::finalizeVaultSuccess --> <!-- @test: src/__tests__/lib/vault-manifest-detection.test.ts (REQ-VAULT-027: promotes only a matching staged manifest) -->
6. Incomplete or failed Pi extraction leaves committed manifest bytes unchanged, so the same edits remain eligible after retry, runtime switch, or restart. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::finalizeVaultSuccess --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (keeps the committed manifest byte-identical on failure and emits the next reminder) -->
