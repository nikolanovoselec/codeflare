...
4. The native worker's install precache bypasses stale browser HTTP-cache entries so stale assets cannot satisfy installation. Cache-versioned precache GETs do not fan out `lastAccessedAt` writes to the same session KV key. <!-- @impl: src/routes/vault/native-sw.ts::VAULT_NATIVE_SERVICE_WORKER_JS --> <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (REQ-VAULT-017 AC4: install precaching bypasses stale browser cache entries) --> <!-- @test: src/__tests__/routes/vault-auth-chain.test.ts (does not schedule same-key session writes for SilverBullet precache GETs) -->
...
2. A full prewarm proof records a persistent per-browser marker; on a later load, if the marker is set and live local readiness still holds for the exact recorded canonical worker scope, the bootstrap iframe is skipped and the control arms (green) directly with no click. A missing or superseded scope proof uses the normal on-demand lifecycle instead of accepting an orphaned worker. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (Layout Component / REQ-AUTH-014 (session expiry handling on 401)) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (forces the unchanged prewarm lifecycle when legacy readiness has no recorded scope) -->
...
**Verification:** Automated tests ([layout wiring test](../../web-ui/src/__tests__/components/Layout.test.tsx), [vault button states test](../../web-ui/src/__tests__/components/VaultButton.test.tsx), [cache helper test](../../web-ui/src/__tests__/lib/vault-cache.test.ts), [vault shell helper test](../../src/__tests__/routes/vault-html-direct.test.ts)); sticky-hover color is verified against the protected deployment through the agent-driven browser-e2e lane.
...
**Intent:** SilverBullet is served under one current bucket-stable URL and its local cache is encrypted with a bucket-derived key, so the client `sb_data_*` (index) and SW `sb_files_*` DB names are identical across future sessions for one user. A deliberate token-version cutover creates one fresh canonical worker scope and fresh databases without unregistering or deleting historical browser state.

**Applies To:** User

**Acceptance Criteria:**

1. The SilverBullet app is served under a bucket-stable URL `/api/vault/<token>/`, where the token is a deterministic, opaque, release-versioned hash of the user's R2 bucket name. The v2 token differs from the legacy token once, then remains identical across future sessions and worker updates so the browser reuses one canonical registration and one current pair of SB databases. <!-- @impl: src/lib/vault-bucket-token.ts::getVaultBucketToken --> <!-- @test: src/__tests__/lib/vault-bucket-token.test.ts (getVaultBucketToken (REQ-VAULT-021)) -->
...
6. The shell's injected boot script carries the real session id, keying the IDB-recorder and prewarm bridge's `vault-session-<sid>-*` localStorage markers to the value the dashboard reads, while base-href and the CSRF cookie use the bucket token. The recorder stores the exact current scope with newly observed DB names; when that scope changes, it clears only stale readiness metadata and leaves every historical registration and IndexedDB untouched. <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @impl: src/lib/vault-view.ts::installVaultIdbRecorder --> <!-- @impl: src/lib/vault-view.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (installVaultIdbRecorder (REQ-VAULT-021 v2 scope cutover)) -->
7. The in-shell prewarm bridge, dashboard readiness check, and controlled reload require the active registration at the exact current or recorded `/api/vault/<token>/` scope. Registration enumeration order and orphaned legacy scopes never satisfy readiness. <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @impl: src/lib/vault-view.ts::installVaultControlledReload --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (binds readiness to the exact current Vault scope instead of enumerating orphaned workers) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (requires the active service worker at the exact scope recorded with the current databases) -->
...
- The v2 cutover does not enumerate, unregister, update, or delete historical service-worker registrations or IndexedDB databases; they remain orphaned. “One canonical worker” means one current scope, not one total browser registration after historical use.
- The serving URL and routing cookie are identical for every future session of one user, so a second session in another tab overwrites the cookie and takes over serving (same bucket, no cross-tenant leak).
...
1. Current and historical bucket-stable IndexedDB stores and service workers are durable browser resources: per-session DELETE, the authoritative orphan sweep, and the v2 scope cutover remove only per-session readiness markers and never delete an IndexedDB store or unregister a service worker. The scope marker joins the IDB and prewarm markers in ordinary session-marker cleanup without becoming a browser-state migration. <!-- @impl: web-ui/src/lib/vault-cache.ts::cleanupSessionVaultCache --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @impl: src/lib/vault-view.ts::installVaultIdbRecorder --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (cleanupSessionVaultCache (REQ-VAULT-015 AC3 / REQ-VAULT-023)) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (invalidates only readiness markers when the canonical scope changes, then records fresh DB names) -->
