...
2. The Pi root reconstructs each request's public-call attempts and pending, running, failed, successful, or GIVEUP state from durable root-session JSONL after reload; an unconsumed launch stays pending, while six failed exact calls emit the structured GIVEUP summary. <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionTranscriptFacts --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (uses actual public attempts for reminders zero through five and then latches GIVEUP) -->
