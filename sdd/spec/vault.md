...
2. A full prewarm proof records a persistent per-browser marker stamped with the container's current start timestamp (`lastStartedAt`); the reload-skip arms the control (green) with no click only when the marker matches the current container start and live local readiness still holds. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports warm only for the same container start that recorded the marker) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports NOT warm when the container start advanced, forcing a clean re-init on a resumed session) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click (no iframe) when the vault is already warm on this device (reload)) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a resumed session whose lastStartedAt advanced is not reload-skipped and stays available until click) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a batch-status poll tick mid-prewarm does not tear down the in-flight iframe (arms green despite session-store churn)) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (preserves the prewarmed marker for an ACTIVE session during an orphan sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker for an ORPHAN session during a sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker on session DELETE) -->
