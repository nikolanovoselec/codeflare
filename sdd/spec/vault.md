...
6. The bridge is inert without valid prewarm parameters and emits ready only after two consecutive proofs of SilverBullet runtime readiness, completed space sync, a current and idle object index, and every required authoritative file. The proof uses the shell's canonical scope rather than cached session metadata. <!-- @impl: src/lib/vault-view.ts::advanceVaultReadyStreak --> <!-- @impl: src/lib/vault-view.ts::buildVaultPrewarmContentProof --> <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (builds canonical-scope proof only after sync, index, queue, and required-file readiness) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (arms only after two consecutive ready polls and resets after a failed poll) -->
...
1. A GET-only bootstrap completes canonical worker key arming and browser encryption state before the editor is shown; other methods do not enter this flow. <!-- @impl: src/lib/vault-view.ts::injectVaultBootstrapHopHtml --> <!-- @impl: src/lib/vault-view.ts::completeVaultBootstrap --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (completeVaultBootstrap / REQ-VAULT-024) -->
...
6. Every Vault open completes bucket-derived key arming and worker activation before showing the editor. <!-- @impl: web-ui/src/components/Layout.tsx::openVaultTab --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-024 AC6: the open click navigates the new tab to the bootstrap-hop URL, not the bare shell) -->
