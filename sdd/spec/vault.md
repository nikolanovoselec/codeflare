...
4. Routine bash, edit, and write calls leave the active graph unchanged without starting another graph merge when the repository path and graph modification time are unchanged. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::GRAPH_MTIME --> <!-- @test: host/__tests__/graphify-active-repo.test.js (graphify-active-repo.sh / REQ-VAULT-004 (unified global graph merges vault + active repos)) -->
...
4. Leaving the active view during in-flight prewarm cancels only the mounted iframe; while authoritative session status remains running, the open intent persists and returning resumes preparation. A stopped or absent session clears its pending prewarm, intent, and retry state so a restart re-derives from scratch. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/components/Layout.tsx::clearPrewarmingVaultStatus --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-019 AC4 (vault open-intent clearing)) -->
...
6. Once armed, the control stays green for the rest of the running session, including dashboard departure/return and the mobile visibility-return dashboard bounce. Readiness and prewarm latches are pruned only when authoritative session status is no longer running, so a stopped/restarted session re-derives from scratch. The armed state remains gated solely on the prewarm readiness proof (`pw === 'ready'`, [REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6) rather than an open/settle latch, identically on mobile, tablet, and desktop, including under a sticky touch-device `:hover`. <!-- @impl: web-ui/src/components/Layout.tsx::vaultButtonStatus --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-022 AC6: preserves a running session readiness latch across dashboard departure and return) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-022 AC6: preserves armed state through the mobile visibility-return dashboard bounce) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-022 AC6: clears readiness after an authoritative stop so restart re-derives from scratch) -->
...
4. Open intent clears when the tab opens or authoritative session status is no longer running; temporary dashboard or mobile visibility-return deactivation does not clear it. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-019 AC4 (vault open-intent clearing)) -->
