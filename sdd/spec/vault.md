...
- Restart detection needs a KNOWN recorded start: a placeholder marker recorded before `lastStartedAt` first polled reads warm even across a genuine resume until the next prewarm upgrades it — an accepted narrow relaxation, still gated by live local readiness and reconciled by SB space-sync.
