...
2. The persistent bucket-stable local store is never blind-deleted by the sync engine while the in-container SilverBullet "secondary" server is still warming up (~1-2 min after a fresh session start) or transiently unreachable: a full sync cycle that observes an empty or unreadable remote file list while the local primary or the snapshot is non-empty is aborted before any deletion and deferred to a later cycle, so the SW only deletes files once it has reached SilverBullet and SilverBullet has confirmed the file list. Without this guard, the cross-session persistence this REQ provides turns SilverBullet's startup window into a full-vault wipe on the 2nd session. The guard mechanism lives in the native-SW graft ([REQ-VAULT-017](#req-vault-017-silverbullet-native-service-worker) AC6). <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (the served sync cycle aborts with no deletion when the remote list is empty/non-array while the persistent store or snapshot is populated; reconciles normally on a real list) -->
3. On every boot the vault initializer stamps the three force-overwritten codeflare-authoritative config pages (CONFIG, README, STYLES) with the immutable preseed source's modification time (`touch -r`), even when the content-equality skip left the file's bytes untouched, so the in-container SilverBullet "secondary" server reports an identical `lastModified` for these pages across every session. These pages are force-copied from preseed on every boot, so without the stamp bisync/cp gives each a fresh mtime per boot; on a 2nd session that mtime diverges from the persisted client sync snapshot, the SW sync engine reports the page "changed on secondary" on every editor watch-poll, re-enqueues one index op per cycle, and the prewarm readiness gate (index queue empty for consecutive polls, [REQ-VAULT-018](#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6) never settles. A deterministic secondary mtime keeps the persisted snapshot in agreement so the spurious "changed on secondary" never fires; cold start is unaffected because its snapshot is built fresh in-session. `Index.md` is exempt from the stamp -- it is no longer force-overwritten but seeded create-if-missing (AC5); the stamp only equalizes the SECONDARY mtime and cannot stop a conflict driven by the PRIMARY (client) save, so the Index.md 2nd-start conflict is fixed at source by AC5 rather than by this stamp. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (stamps codeflare-authoritative config pages with the deterministic preseed mtime even when cmp-skip leaves content untouched) -->
