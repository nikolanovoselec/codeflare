...
5. A top-level vault with an active worker but no first-paint controller reloads once through a loop-safe session marker. A present valid prewarm ID identifies the hidden prewarm path and remains inert; first boot, non-vault scope, and unsupported workers also remain inert. <!-- @impl: src/lib/vault-browser-scripts.ts::VAULT_CONTROLLED_RELOAD_SOURCE --> <!-- @impl: src/lib/vault-view.ts::injectVaultControlledReload --> <!-- @impl: src/lib/vault-view.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/lib/vault-browser-bundle.test.ts (performs the exact-scope one-shot reload from bundled injected bytes) --> <!-- @test: src/__tests__/lib/vault-browser-bundle.test.ts (keeps bundled controlled reload inert for prewarm, first boot, orphaned scope, and a spent one-shot) -->
