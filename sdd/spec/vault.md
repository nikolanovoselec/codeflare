...
6. When the active checkout has no graph, the global graph holds the vault entry alone: every repository entry is removed and none is added. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::KEEP_TAG --> <!-- @test: host/__tests__/graphify-active-repo.test.js (a checkout without a graph leaves the vault alone in the global graph (REQ-VAULT-004 AC6)) -->
...
5. Removals are enumerated from the global manifest rather than derived from the previously-recorded repository, so entries a crashed reconciliation or a formerly-misresolved directory left behind are removed too; the always-on `user_vault` entry is never a removal candidate. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::STALE_TAGS --> <!-- @test: host/__tests__/graphify-active-repo.test.js (sweeps stale tags no sentinel diff could name, and never removes user_vault (REQ-VAULT-014 AC5)) -->
