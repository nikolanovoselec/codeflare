...
- **Vault-extract Agent** -- The background worker that reads only detected changed files and merges their subgraph into the unified graph. Claude is pinned to Sonnet and commits its manifest; Pi uses medium reasoning/four turns and the root promotes staged manifest bytes only after a post-commit request chunk.
...
2. The active-repo hook adds the resolved repo's graph to the unified graph whenever the active repo has a graph and the manifest does not already record this repo's tag against both the current graph hash and this checkout's own graph path. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::REPO --> <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::STORED_PATH --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::planGlobalGraphReconcile --> <!-- @test: src/__tests__/lib/agent-seed-pi-memory.test.ts (REQ-VAULT-004: memory-vault.ts publishes the cumulative vault graph to the global graph via flock-guarded graphify global add) --> <!-- @test: host/__tests__/graphify-active-repo.test.js (skips republishing a graph the manifest already records (REQ-VAULT-014 AC5)) --> <!-- @test: host/__tests__/graphify-active-repo.test.js (republishes when another checkout of the same basename holds the tag (REQ-VAULT-014 AC5)) --> <!-- @test: src/__tests__/lib/pi-global-graph-reconcile.test.ts (still adds when another checkout of the same basename published an identical graph) -->
...
2. On a cold available control, the first click prepares the current session's proxied editor and arms the control; the second click opens it. An already armed/ready control opens on the first click. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-018: stays available (no auto-prewarm) until click 1, then breathes preparing -> armed) -->
...
6. The first-click preparation path requests best-effort persistent browser storage. <!-- @impl: web-ui/src/lib/browser-storage-persistence.ts::requestBrowserStoragePersistence --> <!-- @test: web-ui/src/__tests__/lib/browser-storage-persistence.test.ts (requestBrowserStoragePersistence) -->
...
4. When a successful editor HTML document rewrite did not contain the expected base-href substring (no-op rewrite), a warning is logged on root and deep SPA paths so a future editor-template change surfaces as a logged signal instead of a silent white-screen regression. <!-- @impl: src/lib/vault-view.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (warns when a successful deep SPA HTML response has no rewritable base href (REQ-VAULT-013 AC4)) -->
...
1. Every removal the reconciliation decides on, together with the current entry's addition, runs in one shared-lock transaction; any failure prevents sentinel advancement and remains retryable. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::RECONCILED --> <!-- @test: host/__tests__/graphify-active-repo.test.js (reconciles old removal and new addition under one lock before advancing the sentinel (REQ-VAULT-014 AC1/AC2)) -->
...
- The pre-spawn hash check in [REQ-VAULT-004](#req-vault-004-unified-global-graph-merges-vault-and-active-repos) AC2 uses a SHA-256 digest truncated to the graph builder's standard tag length, validated as hexadecimal and paired with the manifest's recorded source path, so neither a malformed digest nor a same-basename checkout can poison the comparison.
...
1. Browser-initiated Service Worker registration GETs for the editor's service-worker script short-circuit the auth chain and receive SilverBullet's native service worker from the Worker (vendored verbatim, AD69). <!-- @impl: src/routes/vault/native-sw.ts::VAULT_NATIVE_SERVICE_WORKER_JS --> <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault.test.ts (VAULT_NATIVE_SERVICE_WORKER_JS / REQ-VAULT-017 AC1 (native SW served, AD69)) -->
...
3. Leaving the active view during preparation pauses it while preserving intent; returning to the still-running session resumes preparation. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/components/Layout.tsx::clearPrewarmingVaultStatus --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-018 AC3: dashboard departure cancels the in-flight iframe and return resumes preparation) -->
4. Prewarm messages are accepted only from the mounted iframe, same origin, and current attempt. <!-- @impl: web-ui/src/lib/vault-prewarm.ts::startVaultPrewarm --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (REQ-MOB-014 / REQ-VAULT-020: vault browser prewarm protocol) -->
5. The control arms only when the two immediately preceding readiness checks are complete. <!-- @impl: src/lib/vault-browser-scripts.ts::VAULT_PREWARM_BRIDGE_SOURCE --> <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @test: src/__tests__/lib/vault-browser-bundle.test.ts (posts ready only after two complete bundled bridge polls) -->
6. A ready proof is accepted only when its canonical 32-hex Vault scope exactly matches the mounted iframe document; a same-origin proof for another token is rejected. <!-- @impl: web-ui/src/lib/vault-prewarm.ts::startVaultPrewarm --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (rejects a same-origin ready proof for a different Vault token) --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (marks the prewarm ready and removes the iframe after a valid ready message) -->
7. A timeout or error stays visible without background retry; the next user click starts another bounded attempt. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (retries a timed-out prewarm only after another user click) -->
...
2. Every fresh dashboard load starts the server-ready control in the white `available` state and mounts no prewarm iframe until click one. Legacy localStorage readiness markers cannot auto-arm it; the canonical service-worker scope and IndexedDB stores may still be reused by SilverBullet during the explicit prepare. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-018: stays available (no auto-prewarm) until click 1, then breathes preparing -> armed) -->
...
4. The control shows available, accent preparation with focus warning, then green readiness with five-second confirmation; the second click opens instantly. Reduced-motion presentation keeps the state colors without animation. <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/styles/header.css::prefers-reduced-motion --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (Layout Component / REQ-AUTH-014 (session expiry handling on 401)) --> <!-- @manual -->
5. A top-level vault reloads once through a loop-safe session marker if and only if its exact canonical worker is active but does not control first paint; hidden prewarm, first boot, noncanonical scopes, unsupported workers, an existing exact controller, and a spent marker do not reload. <!-- @impl: src/lib/vault-browser-scripts.ts::VAULT_CONTROLLED_RELOAD_SOURCE --> <!-- @impl: src/lib/vault-view.ts::injectVaultControlledReload --> <!-- @impl: src/lib/vault-view.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/lib/vault-browser-bundle.test.ts (performs the exact-scope one-shot reload from bundled injected bytes) --> <!-- @test: src/__tests__/lib/vault-browser-bundle.test.ts (keeps bundled controlled reload inert for prewarm, first boot, orphaned scope, and a spent one-shot) -->
...
6. Shell rewriting carries no session-specific bootstrap or readiness metadata. The base-href and CSRF cookie use the bucket token, while authenticated routing continues to use the HttpOnly session cookie; a service-worker-cached shell therefore cannot freeze a prior session identifier into readiness logic. <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @impl: src/lib/vault-view.ts::rewriteVaultHtmlResponse --> <!-- @test: src/__tests__/routes/vault.test.ts (REQ-VAULT-021: rewrites every shell to the permanent bucket-token scope without session metadata) -->
...
1. The bucket-stable IndexedDB stores and canonical service worker remain durable per-user resources during ordinary session lifecycle: per-session DELETE and the authoritative orphan sweep remove only localStorage markers, never databases or workers. <!-- @impl: web-ui/src/lib/vault-cache.ts::cleanupSessionVaultCache --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (cleanupSessionVaultCache (REQ-VAULT-015 AC3 / REQ-VAULT-023)) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (sweepOrphanVaultCaches (REQ-VAULT-015 AC4 / REQ-VAULT-023)) -->
...
- Worker-to-browser Vault scripts remain explicit, self-contained authored source under [AD126](../../documentation/decisions/README.md#ad126-vault-browser-realm-scripts-are-authored-source-never-serialized-worker-functions).
- Worker-to-browser Vault scripts are never produced with `Function.prototype.toString()`, serialized from Worker-bundled callables, or allowed to depend on bundle-only helpers.
...
- Runtime coordination uses only one active request pointer and one request-specific execution snapshot; no receipt, lease, queue, scheduler, service, or new endpoint is introduced.
