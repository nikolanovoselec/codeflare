...
2. A full prewarm proof records a persistent per-browser marker stamped with the container's current start timestamp (`lastStartedAt`), falling back to a placeholder when the start has not polled in yet so the marker is always recorded once prewarm proves out (and upgrades to the real start on the next mark, never downgrading back). The reload-skip arms the control (green) with no click whenever the marker is present and live local readiness still holds, reporting NOT warm only on a positively-detected container restart — a known current start that differs from a known recorded start — so a resumed session re-initializes cleanly while a same-container return, including one whose start has not re-polled yet, re-arms green instead of forcing a re-init. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::markVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::hasVaultFullyPrewarmed --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports warm for the same container start that recorded the marker (a page reload)) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (reports NOT warm when the container start advanced, forcing a clean re-init on a resumed session) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (stays warm on return when the container start has not polled in yet) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (records a sentinel when prewarm proves out before the start polled, so a later return reads warm) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (upgrades a sentinel to the real start once polled, restoring resume detection) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (a start-less mark never downgrades a recorded real start, so resume detection survives) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (never reads warm against an empty store, even when the current start is unknown) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (shows armed without a click (no iframe) when the vault is already warm on this device (reload)) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a resumed session whose lastStartedAt advanced is not reload-skipped and stays available until click) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (a batch-status poll tick mid-prewarm does not tear down the in-flight iframe (arms green despite session-store churn)) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (preserves the prewarmed marker for an ACTIVE session during an orphan sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker for an ORPHAN session during a sweep) --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (removes the prewarmed marker on session DELETE) -->
