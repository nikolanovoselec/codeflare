...
6. The served worker tolerates a non-array remote `fetchFileList()` result (a transient proxy error or a stray Cloudflare Access 302 HTML body) without crashing the sync loop: the graft coerces the remote list to an empty array by wrapping the `o=` initializer with an array-coercing IIFE (so `o` stays a single binding in the minified `let s=…,o=…,r=…` declarator list), before `getNonSyncCandidates`'s `forEach` and the remote `map`, so a non-array response is a safe no-op cycle (zero candidates, no deletion, persisted snapshot preserved) instead of a repeating `forEach is not a function` throw. The coercion MUST NOT add a second `o=` declarator — that is a duplicate lexical binding that makes the whole served worker fail to parse (the browser then never registers the SW and the vault never becomes ready); the served worker therefore stays syntactically valid JavaScript. The coercion is a graft-layer string transform; the verbatim bytes and the SHA-256 drift guard are unchanged. <!-- @impl: src/routes/vault-native-sw.ts::graftVaultKeyRecovery --> <!-- @test: src/__tests__/routes/vault-native-sw-direct.test.ts (served sync cycle no-ops on non-array remote list, iterates a real array, verbatim chain throws without the coercion, served worker stays syntactically valid / duplicate-let graft rejected, graft preserves the SHA-256 version guard) -->
