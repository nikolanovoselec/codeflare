...
1. The Pi root invokes each Vault extraction through Pi's exact public background `subagent` tool pipeline with inherited context disabled, and never uses a private spawn service or model-chosen dispatch. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-015 AC4: invokes memory and Vault requests together while persisting bounded metadata) -->
