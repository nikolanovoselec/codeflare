...
3. Active-repo resolution walks up to the nearest git checkout, identified by a `.git` directory or a worktree `.git` file, and excludes the vault; a directory holding only graph output is not a repository, and reaching either leaves the sentinel and the global graph untouched. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::REPO --> <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::CANDIDATE --> <!-- @test: host/__tests__/graphify-active-repo.test.js (graphify-active-repo.sh / REQ-VAULT-004 (unified global graph merges vault + active repos)) --> <!-- @test: host/__tests__/graphify-active-repo.test.js (a bare graphify-out/ directory is NOT a repo root) -->
