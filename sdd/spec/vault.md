...
7. Local readiness accepts only the active worker associated with the recorded current Vault scope. <!-- @impl: src/lib/vault-view.ts::findExactVaultRegistration --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (binds the complete local-readiness decision to the exact current Vault scope) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (requires the active service worker at the exact scope recorded with the current databases) -->
...
**Intent:** The next Vault release deliberately starts one fresh browser-local SilverBullet generation without migrating or deleting historical browser state. After that one-time cutover, future sessions and worker updates reuse the same canonical scope while preserving the existing on-demand readiness experience.

**Applies To:** User

**Acceptance Criteria:**

1. The release changes the deterministic Vault token once, then reuses that token across future sessions and worker updates. <!-- @impl: src/lib/vault-bucket-token.ts::getVaultBucketToken --> <!-- @test: src/__tests__/lib/vault-bucket-token.test.ts (cuts over from the legacy scope while remaining deterministic for the release) -->
2. Scope cutover creates fresh current SilverBullet database identities. <!-- @impl: src/lib/vault-view.ts::installVaultIdbRecorder --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (invalidates only readiness markers when the canonical scope changes, then records fresh DB names) -->
3. Scope cutover leaves every historical service-worker registration and IndexedDB database untouched. <!-- @impl: src/lib/vault-view.ts::installVaultIdbRecorder --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (invalidates only readiness markers when the canonical scope changes, then records fresh DB names) -->
4. Only a present exact-current-scope registration whose active worker is not redundant can satisfy local readiness. <!-- @impl: src/lib/vault-view.ts::checkVaultBridgeLocalReadiness --> <!-- @impl: web-ui/src/lib/vault-local-readiness.ts::checkVaultLocalReadiness --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (binds the complete local-readiness decision to the exact current Vault scope) --> <!-- @test: web-ui/src/__tests__/lib/vault-local-readiness.test.ts (accepts an activating current worker during update but rejects a redundant worker) -->
...
- No browser-state migration, service-worker unregistration, IndexedDB deletion, broad retry, or client-asset authentication exemption is introduced.
- One canonical worker means one current scope; historical registrations may remain visible as untouched orphans.
...
**Verification:** Automated tests ([ownership burst](../../src/__tests__/lib/access.test.ts), [token cutover](../../src/__tests__/lib/vault-bucket-token.test.ts), [route burst](../../src/__tests__/routes/vault-auth-chain.test.ts), [scope behavior](../../src/__tests__/routes/vault-html-direct.test.ts), [local readiness](../../web-ui/src/__tests__/lib/vault-local-readiness.test.ts), [control lifecycle](../../web-ui/src/__tests__/components/Layout.test.tsx))
