...
6. Active-repo resolution walks up to the nearest git checkout, identified by a `.git` directory or a worktree `.git` file; a directory holding only graph output is not a repository and leaves both the sentinel and the global graph untouched. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::REPO --> <!-- @test: host/__tests__/graphify-active-repo.test.js (a bare graphify-out/ directory is NOT a repo root) -->
7. When the active checkout has no graph, the global graph holds the vault entry alone: every repository entry is removed and none is added. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::KEEP_TAG --> <!-- @test: host/__tests__/graphify-active-repo.test.js (a checkout without a graph leaves the vault alone in the global graph (REQ-VAULT-004 AC7)) -->
8. A change of checked-out branch, or a graph that disappeared at an unchanged repository path, re-runs reconciliation against the current state rather than being treated as unchanged. <!-- @impl: preseed/agents/claude/plugins/graphify/scripts/graphify-active-repo.sh::GRAPH_STATE --> <!-- @test: host/__tests__/graphify-active-repo.test.js (a branch switch triggers reconciliation even when the graph file is untouched) -->
