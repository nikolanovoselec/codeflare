...
- The reload-skip arms OPTIMISTICALLY the instant a marker exists and live local readiness holds, even before the current container start has polled in, so a same-container return re-greens with no click and no re-init. If the start later polls in and differs from the marker (a resumed container), the optimistic green is REVOKED and the control drops to 'available' for a normal prewarm. The brief green flicker on an actual resume is the accepted trade-off for keeping the common case (return) instant, and it never opens onto a stale store because the open is a fresh bootstrap-hop against live local readiness.
