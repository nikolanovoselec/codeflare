...
**Dependencies:** [REQ-VAULT-013](#req-vault-013-silverbullet-subpath-adapter), [REQ-VAULT-008](#req-vault-008-zero-ui-vault-encryption), [REQ-VAULT-024](#req-vault-024-vault-bootstrap-hop-key-arming-and-service-worker-retention)
...
- The serving URL and `cf_vault_sid` cookie are identical for every session of one user, so a second session in another tab overwrites the cookie and takes over serving (same bucket, no cross-tenant leak).
...
- The per-session identifier must be resolvable for every proxied request, carried via the `cf_vault_sid` cookie on the bucket-stable path or the URL on the session-keyed path ([REQ-VAULT-021](#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key)).
