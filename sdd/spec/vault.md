...
3. Browser prewarm starts ON DEMAND — never automatically — only after the user clicks the server-ready ('available') control; it requests best-effort persistent browser storage on that first click. A timeout or error remains visible and performs no background retry; the next user click starts the next bounded attempt. <!-- @impl: web-ui/src/components/Layout.tsx::Layout --> <!-- @impl: web-ui/src/lib/browser-storage-persistence.ts::requestBrowserStoragePersistence --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (retries a timed-out prewarm only after another user click) -->
...
5. Prewarm messages are accepted only from the mounted iframe, same origin, and current attempt. A ready proof must exactly match that iframe document's canonical 32-hex Vault base scope and include completed sync, index, required-file, and file-list evidence. <!-- @impl: web-ui/src/lib/vault-prewarm.ts::startVaultPrewarm --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (REQ-MOB-014 / REQ-VAULT-020: vault browser prewarm protocol) -->
6. The bridge is inert without valid prewarm parameters and emits ready only after SilverBullet runtime readiness, a completed space sync, the current object-index version with an empty queue, and a local listing containing every required authoritative file. The proof names the shell's canonical scope; it does not depend on session metadata embedded in the service-worker-cached shell. <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (reports only canonical-scope runtime/content proof after the new worker completes) -->
...
1. The bucket-stable IndexedDB stores and canonical service worker are durable per-user resources during ordinary session lifecycle: per-session DELETE and the authoritative orphan sweep remove only localStorage markers and never delete databases or unregister workers. The explicitly versioned clean cutover in [REQ-VAULT-029](#req-vault-029-canonical-browser-state-cutover-and-future-worker-safety) is the sole worker-unregistration exception. <!-- @impl: web-ui/src/lib/vault-cache.ts::cleanupSessionVaultCache --> <!-- @impl: web-ui/src/lib/vault-cache.ts::sweepOrphanVaultCaches --> <!-- @test: web-ui/src/__tests__/lib/vault-cache.test.ts (cleanupSessionVaultCache (REQ-VAULT-015 AC3 / REQ-VAULT-023)) -->
...
3. Before registering the canonical worker, the user-initiated bootstrap enumerates same-origin registrations and unregisters every Vault scope except the exact canonical scope; unrelated service workers are untouched. <!-- @impl: src/lib/vault-view.ts::unregisterStaleVaultServiceWorkers --> <!-- @impl: src/lib/vault-view.ts::registerCanonicalVaultServiceWorker --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (REQ-VAULT-029 clean worker cutover) -->
4. The ready bridge reports the canonical scope together with completed runtime, sync, index, required-file, and file-list evidence. <!-- @impl: src/lib/vault-view.ts::buildVaultPrewarmContentProof --> <!-- @impl: src/lib/vault-view.ts::postVaultPrewarmMessage --> <!-- @impl: src/lib/vault-view.ts::injectVaultPrewarmBridge --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (builds canonical-scope proof only after sync, index, queue, and required-file readiness) -->
5. Missing, superseded, or orphaned worker-scope evidence cannot trigger the controlled-reload path. <!-- @impl: src/lib/vault-view.ts::installVaultControlledReload --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does not let an active orphaned Vault scope trigger the current page reload) --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (does not reload on a genuine first boot with no vault service worker registered) -->
6. Scope cutover uses the existing white, accent-preparing, green-ready, instant-second-click lifecycle without automatic prewarm. <!-- @impl: web-ui/src/components/Layout.tsx::handleVaultOpen --> <!-- @impl: web-ui/src/components/VaultButton.tsx::VaultButton --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-018: stays available (no auto-prewarm) until click 1, then breathes preparing -> armed) --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-019: a cold-path armed click opens the vault tab synchronously) -->
7. Native-worker precaching does not cause unrelated Vault activity to expire or fail under the parallel asset burst. <!-- @impl: src/routes/vault/index.ts::handleVaultRequest --> <!-- @test: src/__tests__/routes/vault-auth-chain.test.ts (persists unrelated activity while the parallel precache consumes no same-key write budget) -->
8. After stale-worker removal, the bootstrap re-enumerates registrations and fails closed if any stale same-origin Vault worker remains. <!-- @impl: src/lib/vault-view.ts::unregisterStaleVaultServiceWorkers --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (fails closed without registering when an old Vault registration remains) -->
9. The dashboard accepts a ready message only from the mounted same-origin iframe for the current attempt, and only when its reported canonical scope exactly matches that iframe document; cross-origin and same-origin wrong-token proofs are rejected. <!-- @impl: web-ui/src/lib/vault-prewarm.ts::startVaultPrewarm --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (rejects a ready proof that claims a cross-origin Vault scope) --> <!-- @test: web-ui/src/__tests__/lib/vault-prewarm.test.ts (rejects a same-origin ready proof for a different Vault token) -->

**Constraints:**

- No browser-state migration, IndexedDB deletion, compatibility layer, background retry, or client-asset authentication exemption is introduced.
- Service-worker removal is confined to same-origin registrations whose pathname starts with `/api/vault/` and runs only after explicit user action. Historical IndexedDB stores remain orphaned and authoritative content remains in R2.
...
1. The Worker delivers the key through a one-time, GET-only bootstrap-hop page (other methods fall through) that registers the canonical native worker, posts the key to its `set-encryption-key` handler, persists an enable-encryption flag, and sets a bootstrap-completed cookie before redirecting to the shell. <!-- @impl: src/lib/vault-view.ts::injectVaultBootstrapHopHtml --> <!-- @impl: src/lib/vault-view.ts::completeVaultBootstrap --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (completeVaultBootstrap / REQ-VAULT-024) -->
...
6. The Vault open click navigates the new tab to the bootstrap-hop URL (`/api/vault/<sid>/.codeflare-bootstrap`) rather than the bare shell, so the first open arms the bucket-derived key via the hop's `set-encryption-key` post and SW-activation wait before redirecting to the editor. <!-- @impl: web-ui/src/components/Layout.tsx::openVaultTab --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-VAULT-024 AC6: the open click navigates the new tab to the bootstrap-hop URL, not the bare shell) -->
7. Before canonical registration, the user-initiated bootstrap removes stale same-origin Vault workers while preserving unrelated registrations; if cleanup cannot converge, bootstrap fails before key posting, cookie completion, or redirect. <!-- @impl: src/lib/vault-view.ts::registerCanonicalVaultServiceWorker --> <!-- @impl: src/lib/vault-view.ts::injectVaultBootstrapHopHtml --> <!-- @test: src/__tests__/routes/vault-html-direct.test.ts (REQ-VAULT-029 clean worker cutover) -->
