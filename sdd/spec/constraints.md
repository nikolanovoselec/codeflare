...
| CI token scope | PR complete-image verification receives `packages: read` for shared BuildKit cache import; only the deployment container job receives `packages: write` to publish cache updates. Top-level workflow permissions remain `contents: read`, limiting blast radius on token compromise. `bump-shadow-pins.yml` mirrors this read-only baseline, with only jobs that push bump branches and open PRs elevating to write. |
