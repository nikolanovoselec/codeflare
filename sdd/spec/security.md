...
7. A successful gate emits the scanner-provided package path and package URL for every accepted occurrence so reviewed identities can be audited and path-bound without weakening the exception. <!-- @impl: scripts/ci/validate-trivy-result.mjs::validateTrivyResult --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (reports scanner package identities for accepted reviewed findings) -->
