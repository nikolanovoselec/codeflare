...
1. WebSocket upgrades return container-stopped close code 4503 before the WS rate-limit check only when persisted Container SDK state is `stopping`, `stopped`, or `stopped_with_code`; stale KV `stopped` cannot reject a running or healthy container. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/lib/rate-limit-core.ts::checkRateLimit --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (REQ-SEC-020 AC1: persisted stopped states return 4503 before rate limiting; stale KV stopped with live SDK state proceeds) -->
2. WebSocket upgrades return retryable close code 1013 before the rate-limit check and forward when a running/healthy container's terminal service is not ready or when persisted state/readiness is unavailable. Probe failures fail closed to retry rather than forwarding or consuming rate-limit budget. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (REQ-SEC-020 AC2: warming, unavailable, and unknown state return 1013 before rate limiting and forwarding) -->
...
- The order is load-bearing: persisted-state and readiness short-circuits run BEFORE the rate limiter so the user budget is preserved across hibernation/warm-up; the forward timeout (AC3) runs after the rate-limit check at the forward step.
