...
8. Under Enterprise strict egress, the Worker re-signs intercepted R2 requests only with the session user's bucket-scoped credential and rejects a different bucket in the same account; it never falls back to the deployment-wide R2 credential. <!-- @impl: src/container/container-interception.ts::strictEgress --> <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @test: src/__tests__/egress-controller.test.ts (REQ-ENTERPRISE-016 / AD86: EgressController account-scoped exemption (own account direct, all else Gateway) / REQ-ENTERPRISE-023) -->
