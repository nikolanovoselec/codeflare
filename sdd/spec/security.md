...
4. `X-Frame-Options: DENY` is set on normal responses; proxied SilverBullet vault content and the code-server Browser IDE proxy content ([REQ-IDE-001](browser-ide.md#req-ide-001-per-session-browser-ide-served-through-the-worker-proxy)) may use `SAMEORIGIN` so the dashboard can run the authenticated hidden prewarm iframe and the IDE in its same-origin surface without allowing cross-site framing. <!-- @impl: src/index.ts::withSecurityHeaders --> <!-- @test: src/__tests__/security/early-return-security.test.ts (CF-001: security headers on pre-Hono early-return responses) -->
