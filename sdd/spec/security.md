...
8. The static landing documents additionally carry an Astro-built per-page `<meta http-equiv="content-security-policy">` (`security.csp`, SHA-256) layered on top of the Worker header (AC2). Inline `<style>`/`<script>` ELEMENTS are authorized by per-element hash rather than a blanket `style-src 'unsafe-inline'`; scripts are external-source-only (`'self'` + Turnstile + Cloudflare Insights, no inline); the dynamic `style="--i:N"` animation-stagger ATTRIBUTES — whose values vary per item and cannot be hashed — are allowed by a scoped `style-src-attr 'unsafe-inline'` that does not loosen element styles. The browser AND-enforces this meta with the Worker header, so the meta's hashes block an injected inline `<style>` even though the header keeps a now-redundant `unsafe-inline`; the meta's source lists mirror the header so it forbids nothing the landing legitimately loads. `frame-ancestors` is not valid in a meta CSP, so clickjacking stays covered by the Worker `X-Frame-Options: DENY` (AC4). <!-- @impl: landing/astro.config.mjs (security.csp) --> <!-- @test: landing/src/__tests__/csp-config.test.ts (SHA-256 hash policy; style elements hash-only no unsafe-inline; scripts external-only with Turnstile + CF Insights; style-src-attr scoped to attributes; mirrored sources; frame-ancestors omitted from meta) -->
