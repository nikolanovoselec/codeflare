...
2. Per-user concurrent-session limits are checked best effort: the default role-based values are 3 for standard users and 10 for stored admins, while simultaneous starts may exceed the observed limit because KV admission is not atomic. <!-- @impl: src/lib/constants.ts::getMaxSessions --> <!-- @impl: src/routes/container/lifecycle-validation.ts::validateSessionAndCheckLimits --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (Session limits / REQ-SUB-013 (concurrent session caps from MAX_SESSIONS_USER/MAX_SESSIONS_ADMIN with env overrides) / REQ-SEC-019 AC2 (per-user concurrent session caps)) --> <!-- @test: src/__tests__/routes/container-lifecycle-helpers.test.ts (REQ-SESSION-007 AC6 / REQ-SUB-013 AC5: concurrent checks are best effort and reserve no slot) -->
