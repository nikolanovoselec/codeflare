...
1. Owner-scoped D1 `starting`, `running`, or `unreachable` cannot produce definitive 4503 merely because the Container SDK transiently reports `stopping`, `stopped`, or `stopped_with_code`; SDK state alone cannot deny a verified surviving workload after authenticated owner-scoped D1 authorization and coordinator reconstruction: forward it through no-start. When the no-start health probe cannot verify a surviving runtime, return retryable 1013 without forwarding the WebSocket or spending the WS rate-limit budget. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (REQ-SESSION-012 AC2 / REQ-SEC-020 AC1: D1 %s remains retryable despite transient SDK stop) -->
...
4. Unavailable persisted state or failed readiness returns retryable close code 1013 before rate limiting and forwarding. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (returns retryable 1013 without rate-limit use when the existing runtime is unavailable) --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (returns retryable 1013 without forwarding or rate-limit use when /health fails) -->
...
- The order is load-bearing: authentication and owner-scoped D1 lifecycle precede verified runtime readiness and rate limiting. Absent/unverified runtime short-circuits before rate-limit writes; a verified surviving workload may forward via no-start under the normal sanctioned rate-limit policy. The forward timeout (AC5) applies at the forward step.
