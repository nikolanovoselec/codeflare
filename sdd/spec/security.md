...
3. The deploy pipeline fails before push if the scan finds an unexcepted fixable HIGH/CRITICAL vulnerability, if a bounded exception is missing, duplicated, additional, or differs from its reviewed identity, or if the built runtime lacks `libevent-core-2.1-7` at Debian's fixed `2.1.12-stable-8+deb12u1` version. <!-- @impl: .github/workflows/container-image.yml::image --> <!-- @impl: scripts/ci/validate-trivy-result.mjs::validateTrivyResult --> <!-- @impl: Dockerfile::RUN apt-get update --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (Trivy bounded exception gate) --> <!-- @test: host/__tests__/dockerfile-base-image.test.js (REQ-SEC-011 AC3: Dockerfile requests the fixed Debian libevent package) --> <!-- @manual -->
