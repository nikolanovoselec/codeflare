...
3. The deploy pipeline fails before push if the scan finds an unexcepted fixable HIGH/CRITICAL vulnerability, if a bounded exception is missing, duplicated, additional, or differs from its reviewed identity, or if the built runtime lacks `libde265-0` at Debian's fixed `1.0.11-1+deb12u3` version or `libevent-core-2.1-7` at Debian's fixed `2.1.12-stable-8+deb12u1` version. <!-- @impl: .github/workflows/container-image.yml::image --> <!-- @impl: scripts/ci/validate-trivy-result.mjs::validateTrivyResult --> <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::verifyCodeServerRuntime --> <!-- @test: host/__tests__/dockerfile-base-image.test.js (fixed Debian security packages) --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (Trivy bounded exception gate) --> <!-- @manual -->
