...
8. The static landing documents additionally carry an Astro-built per-page `<meta http-equiv="content-security-policy">` (`security.csp`, SHA-256) layered on top of the Worker header (AC2). Inline `<style>`/`<script>` ELEMENTS are authorized by per-element hash rather than a blanket `style-src 'unsafe-inline'`; scripts are external-source-only (`'self'` + Turnstile + Cloudflare Insights, no inline); the dynamic `style="--i:N"` animation-stagger ATTRIBUTES are build-static (the landing is statically generated) and hashed too, with `'unsafe-hashes'` in `styleDirective` letting those hashed style attributes apply (CSP3 requires it for inline style attributes even when their hash is present) without a blanket `unsafe-inline` — Astro's schema forbids a `style-src*` directive in the raw directive list, so the carve-out lives in `styleDirective`. The browser AND-enforces this meta with the Worker header, so the meta's hashes block an injected inline `<style>` on the landing even though the shared Worker header still permits inline styles (its `style-src 'unsafe-inline'` is required by the React SPA — CF-016 — and stays in the shared header); the meta's source lists mirror the header so it forbids nothing the landing legitimately loads. `frame-ancestors` is not valid in a meta CSP, so clickjacking stays covered by the Worker `X-Frame-Options: DENY` (AC4). <!-- @impl: landing/astro.config.mjs (security.csp) --> <!-- @test: landing/src/__tests__/csp-config.test.ts (SHA-256 hash policy; style elements hash-only no unsafe-inline; hashed attribute carve-out via 'unsafe-hashes'; no style-src*/script-src in the raw directives array; scripts external-only with Turnstile + CF Insights; mirrored sources; frame-ancestors omitted from meta) -->
...
**Verification:** [Automated test](../../src/__tests__/security/security-headers.test.ts), [Landing meta-CSP config contract](../../landing/src/__tests__/csp-config.test.ts)
