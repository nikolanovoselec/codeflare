...
- Layout spacing is controlled centrally, not tuned per section. The gap between a copy block (a section head, or the hero / mesh copy) and the proof terminal it introduces is one shared token, `--gap-copy-terminal`, reused as `.section-head`'s bottom margin and as the row-gap of `.split-band` and `.mesh-hero-grid`, so a plain section, a split band, and the mesh hero space head -> terminal identically when they stack on mobile; a split band's inner section head drops its own bottom margin so it never doubles against the grid row-gap. The primary calls to action also share one treatment: the hero, Inference Mesh, and dogfood CTAs are all the `.micro-cta` coral text link (`MicroCta.astro`), not a filled button.
