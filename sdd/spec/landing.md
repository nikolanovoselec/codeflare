...
2. With normal-motion JavaScript enhancement, the two Execution frames retain equal outer heights; the Hero first reserves its tallest authored command layout, then initial/font-ready layout and explicit viewport resize synchronize the mobile/tablet value without following its typing animation. <!-- @impl: landing/src/scripts/feature-terminals.ts::reserveHeroFrame --> <!-- @impl: landing/src/scripts/proof.ts::syncExecutionFrames --> <!-- @impl: landing/src/styles/global.css::execution-terminal --> <!-- @test: landing/src/__tests__/feature-terminals.script.test.ts (REQ-LANDING-014: reserves the Hero frame at its tallest command across typing) --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (matches the Hero frame and queues fractionally clipped context without visible resets) --> <!-- @manual -->
