...
2. The band leads with a bold positioning statement (`The agentic engine gets its own private inference fabric.`) whose `inference fabric` phrase — and only that phrase — carries the shared `[data-scramble]` churn, with a static readable fallback when JavaScript or motion is unavailable; the product line beneath it names `inference mesh` in a calm static coral gradient (no churn), so exactly one phrase in the band animates. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/scripts/scramble.ts --> <!-- @test: landing/src/__tests__/index-page.test.ts (the band leads with the .mesh-hero-statement h2 carrying the single data-scramble phrase equal to the content model, no second h1, and the product-line flare is a calm gradient with no data-scramble) -->
...
6. The band is anchored as a sibling hero, not a subsection: its bold positioning statement is the hero-caliber lead (an `<h2>` sized by the shared `.section h2` scale, above the calm `inference mesh` product line), and the page's section-level `~/` path-tag chiplet is deliberately not used, so the differentiating feature does not read as an ordinary section. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/styles/global.css --> <!-- @test: landing/src/__tests__/index-page.test.ts (the .mesh-hero-statement h2 leads the copy before the calm product line, and no .kicker chiplet is present) -->
