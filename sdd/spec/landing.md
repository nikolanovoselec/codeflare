...
- Sections are separated by background tint and vertical rhythm alone, never a horizontal rule; the trust marks are wordmark-free and share one height. Disclosure open/close animation (the FAQ items and the login page's enterprise-SSO `<details>`, which share one global `::details-content` rule) is pure CSS via `::details-content` + `interpolate-size`, gated on `prefers-reduced-motion` with a snap fallback, and snaps gracefully on browsers lacking `::details-content`; no JS.
