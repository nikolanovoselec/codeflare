...
4. During the hover decode, churn glyphs never paint outside the CTA's visible border; over-wide frames are clipped at the button box. <!-- @impl: landing/src/styles/global.css::nav-signin--matrix --> <!-- @test: landing/src/__tests__/scramble.script.test.ts (REQ-LANDING-006 AC4: the CTA clips over-wide churn frames at its own box (overflow hidden on the matrix variant)) -->
