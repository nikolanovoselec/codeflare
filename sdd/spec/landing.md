...
5. Client enhancements animate terminal proofs, capability text, orchestration, reveals, and the page flare while preserving the complete server-rendered resting state and honoring reduced-motion preferences. <!-- @impl: landing/src/scripts/proof.ts --> <!-- @impl: landing/src/scripts/reveal.ts --> <!-- @impl: landing/src/scripts/scramble.ts --> <!-- @test: landing/src/__tests__/proof.script.test.ts (proof.ts (REQ-LANDING-001)) -->
6. Sections use shared composition components and centrally controlled typography, spacing, terminal chrome, responsive breakpoints, and visual hierarchy so peer and subordinate content remain distinguishable at every viewport. <!-- @impl: landing/src/components/Section.astro --> <!-- @impl: landing/src/components/SectionHead.astro --> <!-- @impl: landing/src/components/Terminal.astro --> <!-- @test: landing/src/__tests__/components.test.ts (Section + SectionHead) -->
