...
4. Each settled terminal viewport keeps the engineer's requests and approvals visible alongside agent outcomes. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @test: landing/src/__tests__/index-page.test.ts (keeps engineer requests and approvals visible in each settled viewport) -->
...
- The software transcript keeps engineer intent and approvals visible while showing a real repository lifecycle from clone and graph update through planning, implementation, PR review/CI, merge, and develop realignment.
- The infrastructure transcript keeps engineer direction and execution approval visible while showing real SSH inspection, retained-coredump diagnosis, check-mode bounded patching, a configuration snapshot, validation, restart, and health verification, with cyan distinguishing diagnostic evidence.
