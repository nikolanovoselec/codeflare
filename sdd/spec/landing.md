...
- The rendered marketing copy is vendor-neutral: the prose, FAQ, manifests, and ledger name no underlying cloud platform, so the page reads as a standalone product. The trust-logo strip may include a platform vendor's logo (relationship-neutral, alphabetical), and functional third-party script URLs (e.g. the bot-protection loader) are exempt as non-copy.
