...
4. During the hover decode, the CTA border grows to accommodate over-wide churn frames; glyphs are never clipped and never paint outside the border. <!-- @impl: landing/src/styles/global.css::scramble-box--center --> <!-- @test: landing/src/__tests__/scramble.script.test.ts (REQ-LANDING-006 AC4: the CTA box grows with over-wide churn frames instead of clipping (in-flow grid stacking)) -->
