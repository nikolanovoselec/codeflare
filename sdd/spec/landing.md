...
6. Across supported desktop, tablet, and mobile widths, each initial log contains exactly eight approved rows so its first appended event advances only from the current viewport rather than skipping retained pre-event rows. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @impl: landing/src/scripts/proof.ts::startFeed --> <!-- @test: landing/src/__tests__/index-page.test.ts (keeps every owner-approved row in both coherent execution timelines) --> <!-- @manual -->
