...
4. Both server-rendered terminals expose the complete semantic session, including engineer requests and approvals, and a full eight-row resolved viewport without JavaScript. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @impl: landing/src/components/Transcript.astro::transcript-feed --> <!-- @test: landing/src/__tests__/index-page.test.ts (composes two shared Transcript simulations with full eight-row viewports) -->
...
- The infrastructure transcript keeps engineer direction and execution approval visible while showing real SSH inspection, retained-coredump diagnosis, rollback-safe patching, configuration validation, restart, and health verification, with cyan distinguishing diagnostic evidence.
