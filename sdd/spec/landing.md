...
2. With normal-motion JavaScript enhancement, the two Execution frames retain equal outer heights at every supported width and match the rendered Hero terminal height at mobile and tablet widths. <!-- @impl: landing/src/scripts/proof.ts::syncExecutionFrames --> <!-- @impl: landing/src/styles/global.css::execution-terminal --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (matches the Hero frame and queues fractionally clipped context without visible resets) --> <!-- @manual -->
