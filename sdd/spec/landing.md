...
3. After the initial entrance finishes, every remaining event appends below the retained history, scrolls into the clipped viewport, and types progressively without changing the terminal frame. <!-- @impl: landing/src/scripts/proof.ts::startFeed --> <!-- @impl: landing/src/scripts/proof.ts::scrollFeedToEnd --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (types the simulation to completion in a fixed scrolling log with final cursor) -->
...
2. Each terminal fixes its own outer height from its initial context before playback instead of stretching to match the other terminal's responsive wrapping. <!-- @impl: landing/src/styles/global.css::execution-card --> <!-- @impl: landing/src/styles/global.css::execution-terminal .transcript-feed-size-window --> <!-- @test: landing/src/__tests__/index-page.test.ts (sizes fixed scrolling logs from resolved then initial states without row gaps) --> <!-- @manual -->
...
5. Normal motion sizes each terminal from its complete initial context; no-JavaScript and reduced-motion rendering instead size against and expose the complete resolved event viewport. <!-- @impl: landing/src/components/Transcript.astro::transcript-feed-size --> <!-- @impl: landing/src/styles/global.css::execution-terminal .transcript-feed-size-window --> <!-- @test: landing/src/__tests__/components.test.ts (animate='feed' renders a resolved eight-row viewport with complete semantic content) --> <!-- @test: landing/src/__tests__/index-page.test.ts (sizes fixed scrolling logs from resolved then initial states without row gaps) --> <!-- @manual -->
6. Across supported desktop, tablet, and mobile widths, the initial viewport is content-filled and appended history overflows upward rather than creating trailing empty terminal-body space. <!-- @impl: landing/src/scripts/proof.ts::scrollFeedToEnd --> <!-- @impl: landing/src/styles/global.css::execution-terminal .transcript-feed --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (types the simulation to completion in a fixed scrolling log with final cursor) --> <!-- @manual -->
