...
<!-- @test: landing/src/__tests__/index-page.test.ts (REQ-LANDING-001 describe -> AC4 hero/terminal (governed-run transcript: t-warn drift + t-deny egress + alignment refrain)/spine strip/hero prompt run (data-ft-loop + data-ft-once + ft-typed, run[0] resting state, plays once)/nav/feature-terminal grid (shift section: tile titles + lines + caption feet)/hero terminal statusline foot (agentfoot: ctx + model + reason)/method self-healing enforcement gate (is-fail/is-pass) + label-prose clauses/legacy-rescue station (id=legacy, /sdd init + /sdd clean transcript + foot)/security merged boundary gate (pass rows + deny rows is-deny + egress rows is-redact under gate-subhead, no separate gate egress)/full station spine (station-marker + data-station 01..11 in order; operations folded into 04, tenancy into 08, runs-everywhere into 09, trusted into 11 as .substation sub-content, nothing floating)/context browser-isolated ingestion (open web distilled to markdown) + agent-steered e2e substation (CONTEXT.e2e heading + transcript lines + foot, the drive surface with a mobile-viewport verdict)/parallel review board + dispatch/orchestration own dynamic station 07 (Running N agents + per-agent tool/token counters + ctrl+o/ctrl+b affordances + data-orch hooks + normal agent commands, no ctx_batch_execute, between review and spend)/cost ledger + sample note/totals/FAQ/dogfood proof (id=dogfood DOGFOOD.lines + foot + PR #533, single GitHub CTA, no footer GitHub link) + Swiss Post trusted-by tail/form/launch path Sign in) -->
...
<!-- @test: landing/src/__tests__/feature-terminals.script.test.ts (REQ-LANDING-001 describe -> type/hold/delete loop DOM mutation + convergence to second loop word + data-ft-once hero plays through run and rests on final beat (no loop-back) + reduced-motion no-op (loop[0] resting state) + empty-loop/invalid-JSON skip) -->
...
4. The landing renders the full enterprise narrative statically (no JS required): a hero with a single legible terminal demo whose transcript follows one governed run (a spec drift caught and corrected, a denied direct-provider egress, the "specification, implementation and documentation aligned" refrain) carrying an agent statusline foot (context, model, reasoning level), a spine strip naming that run, and a single coherent prompt run on its bottom command line that plays once and rests on the merge (the shared feature-terminal typing engine in play-once mode, `data-ft-once`; the first command is the no-JS resting state); a feature-terminal grid in the shift section (four compact terminals, each showing one codeflare capability as a real command and its output with a one-line caption foot, replacing the former stat band and checkmark comparison); a spec-driven-development "method" section presenting SDD/TDD enforcement as a self-healing enforcement gate, with its two pillars as plain label-and-prose clauses (no numbered counter; the numbering lives in the page-wide station spine); a legacy-rescue station (`/sdd init` reverse-engineering a legacy codebase into a spec-driven baseline and `/sdd clean` realigning a drifted spec, shown as a terminal) placed between method and security; a security section whose unified boundary gate (approved and impossible paths as pass/deny rows sharing the same gate grammar as the enforcement gate) and the one egress call inspected beneath an in-terminal sub-label make zero-trust, DLP, and guardrails auditable, the boundary and the egress rows now folded into one terminal closing on a single in-chrome foot; a browser-isolation context pipe (the open web distilled to markdown) with an agent-steered e2e substation beneath it (the same throwaway browser driving a deployed flow from a mobile viewport and returning a pass/fail verdict, belonging to the same numbered station); a parallel review board; a live, dynamic agent-orchestration station of its own ("Running N agents" with per-agent tool-use and token counters that tick as they work via `orch.ts`, ordinary agent commands rather than internal tool names, and the `ctrl+o` / `ctrl+b` affordances); a cost attribution ledger; a vertical station spine numbering every section 01..11 in order (shift, spec, rescue, boundary, web, review, agents, spend, platform, answers, proof), with the former un-numbered bands (operations, tenancy, runs-everywhere, trusted) folded into a numbered station as `.substation` sub-content so nothing floats; all content sections with anchor ids matching the nav links; FAQ; a dogfood proof section (this page as REQ-LANDING-001, with its real @impl/@test anchors rendered as a terminal widget closing on an in-chrome foot, and the page's only GitHub link as its CTA); a "Trusted by" Swiss Post social-proof band folded into the proof station as its tail, before the contact section; the contact form; and a Sign in action (nav) linking to the login provider-chooser (`/login`, `APP_LINKS.signIn`); the footer is reduced to one quiet centered "Built with Codeflare" line (no Sign in, GitHub mark, or nav links). The governance sections carry the page; the platform-capability sections follow as the payoff the boundary makes safe.
