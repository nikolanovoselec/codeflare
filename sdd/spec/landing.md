...
<!-- @test: landing/src/__tests__/index-page.test.ts (REQ-LANDING-001 describe -> AC4 hero/terminal (governed-run transcript: t-warn drift + t-deny egress + alignment refrain)/spine strip/hero prompt run (data-ft-loop + data-ft-once + data-ft-shuffle + ft-typed, run[0] resting state, plays once, beats shuffled per load)/nav/feature-terminal grid (shift section: tile titles + lines + caption feet)/hero terminal statusline foot (agentfoot: ctx + model + reason)/method self-healing enforcement gate (is-fail/is-pass) + label-prose clauses/legacy-rescue station (id=legacy, /sdd init + /sdd clean transcript + foot)/security merged boundary gate (pass rows + deny rows is-deny + egress rows is-redact under gate-subhead, no separate gate egress)/full station spine (station-marker + data-station 01..11 in order; operations folded into 04, tenancy into 08, runs-everywhere into 09, trusted into 10 as .substation sub-content, nothing floating)/context browser-isolated ingestion (open web distilled to markdown) + agent-steered e2e substation (CONTEXT.e2e heading + transcript lines + foot, the drive surface with a mobile-viewport verdict)/parallel review board + dispatch/orchestration own dynamic station 07 (Running N agents + per-agent tool/token counters + ctrl+o/ctrl+b affordances + data-orch hooks + normal agent commands, no ctx_batch_execute, between review and spend)/cost ledger + sample note/totals/dogfood proof (id=dogfood DOGFOOD.lines + foot + PR #533, single GitHub CTA, no footer GitHub link) + Swiss Post trusted-by tail (logo links to the customer site)/FAQ/form/launch path Sign in) -->
