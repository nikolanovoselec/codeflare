...
2. The band headline is the product name `Inference Mesh` in the coral flare gradient, carrying the shared `[data-scramble]` churn, with a static readable fallback when JavaScript or motion is unavailable; `Codeflare` is deliberately not repeated as a display wordmark here (the header logo and hero lead already establish it), so exactly one phrase in the band animates. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/scripts/scramble.ts --> <!-- @test: landing/src/__tests__/index-page.test.ts (the .mesh-hero-headline h2 renders the single data-scramble phrase equal to INFERENCE_MESH.name and contains no other wordmark, no second h1) -->
...
6. The band is anchored as a sibling hero, not a subsection: the `Inference Mesh` product name is the hero-caliber `<h2>` headline (sized by the shared `.section h2` scale), right-aligned on desktop to mirror the left proof terminal; the page's section-level `~/` path-tag chiplet, any product subtitle line, and a repeated `Codeflare` display wordmark are deliberately not used, so the feature is showcased without over-repeating the brand or reading as an ordinary section. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/styles/global.css --> <!-- @test: landing/src/__tests__/index-page.test.ts (the .mesh-hero-headline leads the copy with no .kicker chiplet, no .mesh-hero-product subtitle line, and no Codeflare wordmark; ordered headline -> description -> CTA) -->
