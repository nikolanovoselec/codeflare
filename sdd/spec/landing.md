...
4. Each complete terminal timeline alternates explicit engineer requests or approvals with agent outcomes, so both the initial history and settled viewport preserve the user's intent. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @test: landing/src/__tests__/index-page.test.ts (keeps engineer intent visible throughout both coherent execution timelines) -->
...
2. Before live activity starts, each terminal reveals its complete populated context history from top to bottom. <!-- @impl: landing/src/scripts/proof.ts::prepareFeed --> <!-- @impl: landing/src/styles/global.css::term-type --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (prepares complete context histories with top-to-bottom entrance indices) -->
3. After the initial entrance finishes, the newest row remains bottom-aligned within the unchanged clipped frame as each remaining event appends and types and as the viewport resizes. <!-- @impl: landing/src/scripts/proof.ts::startFeed --> <!-- @impl: landing/src/scripts/proof.ts::scrollFeedToEnd --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (types the simulation to completion in a fixed scrolling log with final cursor) -->
...
5. Both simulations stop after their authored final event with every authored row retained in the scrolling log and a continuously blinking cursor on the last row. <!-- @impl: landing/src/scripts/proof.ts::settleFeed --> <!-- @impl: landing/src/styles/global.css::t-caret --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (types the simulation to completion in a fixed scrolling log with final cursor) --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (completes both authored simulations independently and never restarts them) -->
...
2. The two terminal frames retain equal outer heights while side by side at desktop and tablet widths. <!-- @impl: landing/src/styles/global.css::execution-card --> <!-- @impl: landing/src/styles/global.css::execution-terminal --> <!-- @test: landing/src/__tests__/index-page.test.ts (sizes fixed scrolling logs from resolved then initial states without row gaps) --> <!-- @manual -->
...
5. Normal motion sizes the equal side-by-side frames from the taller initial eight-row viewport; no-JavaScript and reduced-motion rendering instead size against and expose the complete resolved event viewport. <!-- @impl: landing/src/components/Transcript.astro::transcript-feed-size --> <!-- @impl: landing/src/styles/global.css::execution-terminal .transcript-feed-size-window --> <!-- @test: landing/src/__tests__/components.test.ts (animate='feed' renders a resolved eight-row viewport with complete semantic content) --> <!-- @test: landing/src/__tests__/index-page.test.ts (sizes fixed scrolling logs from resolved then initial states without row gaps) --> <!-- @manual -->
6. Across supported desktop, tablet, and mobile widths, each initial log contains enough authored context to fill its frame, and appended history overflows upward rather than creating trailing empty terminal-body space. <!-- @impl: landing/src/scripts/proof.ts::scrollFeedToEnd --> <!-- @impl: landing/src/styles/global.css::execution-terminal .transcript-feed --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (types the simulation to completion in a fixed scrolling log with final cursor) --> <!-- @manual -->
7. The 32 approved reel lines remain verbatim as the first line of their row when continuation evidence is present. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @test: landing/src/__tests__/index-page.test.ts (keeps engineer intent visible throughout both coherent execution timelines) --> <!-- @manual -->
