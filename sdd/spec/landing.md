...
2. The band headline reuses the product's own operator-console lockup — `Codeflare` in white over `Inference Mesh` in the coral flare gradient — with only `Codeflare` carrying the shared `[data-scramble]` churn (in white, not the gradient) and a static readable fallback when JavaScript or motion is unavailable, so exactly one phrase in the band animates. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/scripts/scramble.ts --> <!-- @test: landing/src/__tests__/index-page.test.ts (the .mesh-hero-headline h2 renders Codeflare as the single data-scramble phrase equal to the content model and Inference Mesh as a calm .flare with no data-scramble, no second h1) -->
3. The band description positions the idle machines you already own as a private inference fabric for your agents, with long-running sessions kept warm on otherwise-idle capacity, sensitive work never leaving the boundary, and reaching for a hosted model framed as a policy choice instead of the default. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @test: landing/src/__tests__/index-page.test.ts (inference mesh description is rendered verbatim from the typed content model) -->
...
6. The band is anchored as a sibling hero, not a subsection: the product's own console lockup (`Codeflare` / `Inference Mesh`) is the hero-caliber `<h2>` headline (sized by the shared `.section h2` scale), right-aligned on desktop to mirror the left proof terminal, with the value prop in the description; the page's section-level `~/` path-tag chiplet and any separate product subtitle are deliberately not used, so the differentiating feature does not read as an ordinary section. <!-- @impl: landing/src/content/site.ts --> <!-- @impl: landing/src/components/InferenceMeshHero.astro --> <!-- @impl: landing/src/styles/global.css --> <!-- @test: landing/src/__tests__/index-page.test.ts (the .mesh-hero-headline lockup leads the copy with no .kicker chiplet and no .mesh-hero-product subtitle line, ordered headline -> description -> CTA) -->
