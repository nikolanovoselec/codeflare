...
2. With normal-motion JavaScript enhancement, the two Execution frames retain equal outer heights; initial/font-ready layout and explicit viewport resize synchronize the mobile/tablet value to the rendered Hero terminal without following its command animation. <!-- @impl: landing/src/scripts/proof.ts::syncExecutionFrames --> <!-- @impl: landing/src/styles/global.css::execution-terminal --> <!-- @test: landing/src/__tests__/execution-reel.script.test.ts (matches the Hero frame and queues fractionally clipped context without visible resets) --> <!-- @manual -->
