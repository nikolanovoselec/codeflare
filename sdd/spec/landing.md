...
6. Every outgoing context row and its incoming event replacement have the same explicit line budget, and both eight-row feeds total fourteen meaningful lines while preserving the approved first lines. <!-- @impl: landing/src/content/site.ts::EXECUTION --> <!-- @test: landing/src/__tests__/index-page.test.ts (balances every context replacement pair into fourteen meaningful lines) --> <!-- @manual -->
