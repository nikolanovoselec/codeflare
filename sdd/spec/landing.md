...
<!-- @test: landing/src/__tests__/index-page.test.ts (REQ-LANDING-001 describe -> AC4 hero/terminal (governed-run transcript: t-warn drift + t-deny egress + alignment refrain)/spine strip/hero prompt loop (data-ft-loop + ft-typed, loop[0] resting state)/nav/feature-terminal grid (shift section: tile titles + lines + caption feet)/hero terminal statusline foot (agentfoot: ctx + model + reason)/method self-healing enforcement gate (is-fail/is-pass) + label-prose clauses/legacy-rescue station (id=legacy, /sdd init + /sdd clean transcript + foot)/security merged boundary gate (pass rows + deny rows is-deny + egress rows is-redact under gate-subhead, no separate gate egress)/station spine (station-marker + data-station 01..06)/operations section/context browser-isolated ingestion/parallel review board + dispatch + live agent-orchestration proof terminal (Running N agents + per-agent tool/token counters + ctrl+o/ctrl+b affordances)/cost ledger + sample note/totals/FAQ/trusted-by strip (Swiss Post logo before contact)/dogfood proof (id=dogfood DOGFOOD.lines + foot + PR #533, single GitHub CTA, no footer GitHub link)/form/launch path Sign in) -->
...
<!-- @test: landing/src/__tests__/feature-terminals.script.test.ts (REQ-LANDING-001 describe -> type/hold/delete loop DOM mutation + convergence to second loop word + reduced-motion no-op (loop[0] resting state) + empty-loop/invalid-JSON skip) -->
...
4. The landing renders the full enterprise narrative statically (no JS required): a hero with a single legible terminal demo whose transcript follows one governed run (a spec drift caught and corrected, a denied direct-provider egress, the "specification, implementation and documentation aligned" refrain) carrying an agent statusline foot (context, model, reasoning level), a spine strip naming that run, and a live prompt loop on its bottom command line (the shared feature-terminal typing engine; the first looped command is the no-JS resting state); a feature-terminal grid in the shift section (four compact terminals, each showing one codeflare capability as a real command and its output with a one-line caption foot, replacing the former stat band and checkmark comparison); a spec-driven-development "method" section presenting SDD/TDD enforcement as a self-healing enforcement gate, with its two pillars as plain label-and-prose clauses (no numbered counter; the numbering lives in the page-wide station spine); a legacy-rescue station (`/sdd init` reverse-engineering a legacy codebase into a spec-driven baseline and `/sdd clean` realigning a drifted spec, shown as a terminal) placed between method and security; a security section whose unified boundary gate (approved and impossible paths as pass/deny rows sharing the same gate grammar as the enforcement gate) and the one egress call inspected beneath an in-terminal sub-label make zero-trust, DLP, and guardrails auditable, the boundary and the egress rows now folded into one terminal closing on a single in-chrome foot; a browser-isolation context pipe; a parallel review board, alongside a live agent-orchestration proof terminal ("Running N agents" with per-agent tool-use and token counters and the `ctrl+o` / `ctrl+b` orchestration affordances); a cost attribution ledger; a quiet vertical station spine numbering the instrument stations (method, legacy, security, context, pipeline, cost) while the calmer tail sections stay un-numbered; all content sections with anchor ids matching the nav links; FAQ; a dogfood proof section (this page as REQ-LANDING-001, with its real @impl/@test anchors rendered as a terminal widget closing on an in-chrome foot, and the page's only GitHub link as its CTA); a "Trusted by" social-proof strip (the Swiss Post logo) placed immediately before the contact section; the contact form; and a Sign in action (nav) linking to the login provider-chooser (`/login`, `APP_LINKS.signIn`); the footer is reduced to one quiet centered "Built with Codeflare" line (no Sign in, GitHub mark, or nav links). The governance sections carry the page; the platform-capability sections follow as the payoff the boundary makes safe.
