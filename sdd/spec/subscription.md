...
3. Before provider fetch, the per-user Timekeeper issues a monotonic sync token. Its serialized apply accepts only the current token, so a later-started sync rejects older results, including same-millisecond starts and delayed check/write interleavings. <!-- @impl: src/routes/stripe-webhook.ts::syncSubscriptionState --> <!-- @impl: src/timekeeper/index.ts::Timekeeper --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (DEEP-22-002: an older in-flight provider result cannot overwrite a later-started sync) --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (serializes a newer sync while an older apply is delayed after its guard) -->
