...
2. The admin Subscription Management panel exposes the persisted operational properties used by its current consumer: storage cap, monthly compute, maximum concurrent sessions, trial cap, description, allowed modes, and external price IDs. Identity, login-policy, ordering, default-selection, and provider-returned display prices are not editable in this panel. <!-- @impl: web-ui/src/components/admin/SubscriptionManagement.tsx::SubscriptionManagement --> <!-- @test: web-ui/src/__tests__/components/admin/SubscriptionManagement.test.tsx (SubscriptionManagement (Admin)) -->
...
3. Before provider fetch or a subscription-termination reset, the per-user Timekeeper issues a monotonic sync token. Its serialized apply accepts only the current token. Termination applies billing-field cleanup plus the canceled/free/default reset through that same coordinator, so a later-started deletion rejects an older in-flight update result, including same-millisecond starts and delayed check/write interleavings. <!-- @impl: src/routes/stripe-webhook.ts::syncSubscriptionState --> <!-- @impl: src/routes/stripe-webhook.ts::handleSubscriptionDeleted --> <!-- @impl: src/timekeeper/index.ts::Timekeeper --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (DEEP-22-002: an older in-flight provider result cannot overwrite a later-started sync) --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (serializes a newer sync while an older apply is delayed after its guard) --> <!-- @test: src/__tests__/routes/stripe-webhook.test.ts (REQ-SUB-015 AC3: a deletion prevents an older in-flight update from restoring entitlements) --> <!-- @test: src/__tests__/timekeeper/index.test.ts (REQ-SUB-015 AC3: canceled/free cleanup rejects an older paid apply) -->
...
1. The usage page shows a progress bar for monthly usage and stat cards for today, this month, and the tier quota. <!-- @impl: web-ui/src/components/UsagePage.tsx::UsagePage --> <!-- @test: web-ui/src/__tests__/components/UsagePage.test.tsx (UsagePage / REQ-SUB-018 AC1 (usage ring + stat cards)) -->
...
6. When checkout schedules a free trial, the billing-cycle anchor is the first calendar-month boundary strictly after the scheduled trial end. Trial length itself is unaffected. <!-- @impl: src/routes/billing.ts::default --> <!-- @impl: src/lib/kv-keys.ts::getNextUtcMonthStart --> <!-- @test: src/__tests__/routes/billing.test.ts (passes billingCycleAnchor after trial end when trial is active (REQ-SUB-021)) -->
7. When quota enforcement terminates a trial early, the provider billing-cycle anchor is recomputed from the actual early end before billing begins. <!-- @impl: src/lib/stripe.ts::endTrialNow --> <!-- @test: src/__tests__/routes/billing.test.ts (DEEP-16-008: early trial termination billing anchor / anchors billing at the first UTC month boundary after the actual early end) -->
