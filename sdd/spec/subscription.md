...
7. The page shows a centered tier-and-mode label only for non-custom SaaS tiers; non-SaaS deployments and the custom (`unlimited`) tier show usage time without a plan label. <!-- @impl: src/routes/usage.ts::app --> <!-- @impl: web-ui/src/components/UsagePage.tsx::UsagePage --> <!-- @test: src/__tests__/routes/usage.test.ts (GET /api/usage / REQ-SUB-018 AC2 (real-time Timekeeper DO with KV fallback)) --> <!-- @test: web-ui/src/__tests__/components/UsagePage.test.tsx (UsagePage / REQ-SUB-018 AC1 / REQ-SUB-022 AC4 (usage stat and quota rendering)) -->
