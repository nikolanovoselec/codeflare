...
3. Before the provider fetch, the per-user Timekeeper Durable Object issues a monotonic sync-start token. The billing patch is applied through that same strongly consistent coordinator only when its token is still current, so a newer-started sync rejects every older apply even for same-millisecond starts and check/write interleavings. <!-- @impl: src/routes/stripe-webhook.ts::syncSubscriptionState --> <!-- @impl: src/timekeeper/index.ts::Timekeeper --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (DEEP-22-002: an older in-flight provider result cannot overwrite a later-started sync; serializes a newer sync while an older apply is delayed after its guard) -->
...
6. When a free trial is active, the billing-cycle anchor is the first calendar-month boundary strictly after the trial ends. Checkout anchors against the scheduled end; quota-triggered early termination updates the provider anchor against the actual early end before billing begins. Trial length itself is unaffected. <!-- @impl: src/lib/stripe.ts::endTrialNow --> <!-- @test: src/__tests__/routes/billing.test.ts (DEEP-16-008: early trial termination billing anchor) -->
