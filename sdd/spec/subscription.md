...
3. A sync-start version is captured before the provider fetch; a result is rejected when a later-started sync is already persisted, so a slow older provider result cannot overwrite newer state. Strict comparison permits equal versions. <!-- @impl: src/routes/stripe-webhook.ts::syncSubscriptionState --> <!-- @test: src/__tests__/routes/stripe-webhook-sync.test.ts (DEEP-22-002: an older in-flight provider result cannot overwrite a later-started sync) -->
