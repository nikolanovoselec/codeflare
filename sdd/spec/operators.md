...
3. An optional randomly generated 256-bit per-operator webhook key is displayed once with repository/environment or selected-organization Actions-secret instructions under `CODEFLARE_OPERATOR_WEBHOOK_KEY`. Rotation does not retain old keys indefinitely. Codeflare's master encryption key is not distributed. Key generation returns a base64url-encoded 32-byte random handoff value and its operator-bound webhook-purpose ciphertext; unavailable encryption returns neither. <!-- @impl: src/operators/protected-secrets.ts::createOperatorWebhookKey --> <!-- @test: src/__tests__/operators/webhook-key.test.ts (REQ-OPERATOR-002: optional per-operator webhook key) -->
