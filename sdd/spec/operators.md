...
8. An admitted package may optionally declare generic text resources. The parent projects exact digest-and-size-verified bundle bytes from package-declared canonical relative paths beneath a fixed non-synced parent-owned root, persists only inert non-authority data, restores the restriction on wake, and completes trusted restore before context configuration or startup; bundles without resources remain unchanged. <!-- @impl: src/operators/package-resources.ts --> <!-- @impl: src/operators/orchestrator.ts::runOperatorActivity --> <!-- @impl: src/operators/gate1-runtime.ts::ContainerOwnedSessionRuntime --> <!-- @impl: src/container/index.ts::container.configureOperatorResources --> <!-- @impl: entrypoint.sh --> <!-- @test: src/__tests__/operators/package-resources.test.ts (generic admitted Operator package resources) --> <!-- @test: src/__tests__/operators/gate1-runtime.test.ts (REQ-OPERATOR-005: owned container runtime) -->
...
2. The conversation supports bounded prompt, follow-up, steer, approved native-tool execution and observation, explicit approval-needed state, and awaited cancellation. Parent-composed modes may further confine SDK filesystem tools to exact approved inputs and outputs without granting broader host access. <!-- @impl: host/src/operator-pi-http.ts::OperatorPiHttpController --> <!-- @impl: host/src/operator-pi-review.ts::createOperatorPiReviewTools --> <!-- @test: host/__tests__/operator-pi-http.test.js (REQ-OPERATOR-021: fixed ensure/send/observe/abort API omits the private session file) --> <!-- @test: host/__tests__/operator-pi-review.test.js (REQ-OPERATOR-021: parent-composed Pi filesystem sandbox) -->
