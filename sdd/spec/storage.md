...
**Intent:** Startup and steady-state sync must not waste time on avoidable R2 round-trips. Two costs are addressed: (a) the steady-state bisync issued one HEAD per object to read its mtime metadata (the dominant sync cost, ~2,900 HEADs/cycle on a populated bucket); and (b) the blocking initial sync re-downloads the entire agent seed (~627 files, ~9 MB — about half of a populated bucket) every boot, because the container filesystem is ephemeral and the seed lives only in R2. The seed re-download is eliminated in Governed Mode by laying down an image-baked copy of the seed locally and letting a content-checksum sync transfer only the user's deltas. A third cost is the jiti prewarm cache: Pi's first-party extensions are transpiled into a baked jiti cache at image build, but jiti's cache key is `hash(absolute path + source + version)` — **path-sensitive, not content-only**. So a cache hit needs both (i) the warm bake to transpile each extension at the **exact runtime path** and (ii) the runtime extension **content** to equal the build; otherwise every session pays ~2.4s of cold transpile. The image warms the cache at the real runtime agent dir, and the entrypoint relays the managed extension code over the synced tree in all deployment modes so both halves hold.
...
4. The Pi extension jiti prewarm cache hits at runtime in **all** deployment modes, eliminating per-session cold transpile (~2.4s on the advanced extension set). jiti's cache key is `hash(absolute path + source + version)` — path-sensitive, not content-only — so two conditions are enforced: (a) the image warm-bake transpiles each extension at the **exact runtime path** `/home/user/.pi/agent/extensions` (via `PI_CODING_AGENT_DIR`/`HOME` in the warm `RUN`), never a throwaway tmpdir — a path mismatch is why advanced sessions cold-transpiled despite matching content; and (b) the runtime extension **content** equals the build, which the entrypoint guarantees by relaying the managed extensions from the unfiltered image source (`/opt/codeflare/pi-agent/extensions`), overwriting only managed files already present in the runtime (mode-independent, preserves mode gating, leaves user-added extensions and user-editable seed docs/rules untouched), synchronously before the `--resync` baseline with a fresh mtime so the relaid copy is the bisync source of truth. The image build is **fail-closed**: after the warm bake it asserts every extension in the source dir produced a baked cache entry, so an added/modified/skipped extension that would cold-transpile fails the build rather than regressing startup silently. <!-- @impl: Dockerfile --> <!-- @impl: entrypoint.sh::relay_managed_pi_extensions --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017 / AD90: post-sync managed Pi extension relay describe) -->
5. The background-init subshell (bisync `--resync` baseline, vault seed, sync/vault daemons) runs concurrently with the PTY pre-warm on the single vCPU, so it self-deprioritizes (lowest niceness + idle I/O class applied to its own PID before any work, inherited by the rclone and daemon children) so pi pre-warm preempts it for CPU and disk. The baseline then runs in pre-warm's slack and after it completes, instead of contending for the core and inflating startup latency. <!-- @impl: entrypoint.sh --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017: background init yields CPU/disk to pi pre-warm describe) -->
...
**Verification:** [Bisync server-modtime + lay-down/compare-flag + managed-extension relay test](../../host/__tests__/entrypoint-governed-sync.test.js) (AC1, AC3, AC4); [bake byte-identity test](../../src/__tests__/lib/agent-seed-bake.test.ts) (AC2)
