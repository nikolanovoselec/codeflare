...
- The jiti prewarm cache key is `hash(absolute path + source + version)` — path-sensitive, not content-only — which is why both the warm-bake path and the relayed runtime content must match the build for a hit; warming at a throwaway tmpdir path is what made advanced sessions cold-transpile despite identical bytes.
