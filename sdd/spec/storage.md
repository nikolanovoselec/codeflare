...
4. After the initial sync, in **all** deployment modes, the entrypoint relays the image-baked managed Pi extension code (`.pi/agent/extensions`) for the session mode over the synced tree, so the content-addressed jiti prewarm cache (baked from those exact files) always hits instead of cold-transpiling a stale-bucket copy. The relay is surgical — it overwrites only the codeflare-owned managed extension files, preserving user-added extensions and leaving user-editable seed docs/rules untouched — and runs synchronously before the `--resync` baseline with a fresh mtime so the relaid local copy becomes the bisync source of truth (self-healing R2). <!-- @impl: entrypoint.sh::relay_managed_pi_extensions --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017 / AD90: post-sync managed Pi extension relay describe) -->
