...
4. With any running session, a mismatch reports UPDATE PENDING, performs no seed mutation, disables another session in the UI, and the backend start boundary returns the same typed conflict until the final session stops and reconciliation succeeds. During deployment-cache failure, last-known-good startup remains allowed only when its applied mode matches the currently resolved mode. <!-- @test: src/__tests__/routes/session-batch-status.test.ts (reports update_pending and never starts reconciliation while a session is running) --> <!-- @test: src/__tests__/routes/session-batch-status.test.ts (blocks New Session when enabled curation is unavailable and no release was previously applied) --> <!-- @test: src/__tests__/routes/container-r2-start.test.ts (returns a typed 409 before user-bucket or container work when the active release is not applied) --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-STOR-020 AC4: dashboard maps pending managed reconciliation without a new poller) --> <!-- @test: src/__tests__/routes/container-r2-start.test.ts (blocks an old applied mode during a cache outage when Enterprise resolves advanced mode) --> <!-- @test: src/__tests__/routes/session-batch-status.test.ts (reports update_pending during an outage when Enterprise resolves a different applied mode) -->
5. Managed writes use the active release digest as provenance; cleanup compares the prior release document set and deletes an obsolete or mode-narrowed key only when its current marker still matches that prior digest. Signed `retiredPaths` also remove earlier baked or managed objects only while any Codeflare ownership marker remains; an absent or changed marker preserves the user-owned file. <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- @impl: src/lib/r2-seed.ts::deleteRetiredManagedConfigs --> <!-- @test: src/__tests__/lib/r2-seed-managed.test.ts (REQ-STOR-020 AC5: prior release markers guard managed cleanup) --> <!-- @test: src/__tests__/lib/r2-seed-managed.test.ts (deletes explicitly retired Codeflare-owned paths even without a prior remote release) -->
