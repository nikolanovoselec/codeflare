...
4. After the initial sync, in **all** deployment modes, the entrypoint relays the managed Pi extension code over the synced tree from the exact unfiltered image directory the jiti prewarm cache was baked from (`/opt/codeflare/pi-agent/extensions`), overwriting only managed extension files already present in the runtime, so the content-addressed cache always hits instead of cold-transpiling a stale-bucket copy. Sourcing from the unfiltered jiti-bake dir (not the mode-filtered seed bake, whose default subset omits advanced-only extensions like `codeflare-pi.ts`) is what makes the relaid bytes byte-identical to the cache. Overwrite-if-present is mode-independent and preserves mode gating: it fixes the content of whatever managed extensions the session loaded without adding mode-gated extensions, leaves user-added extensions and user-editable seed docs/rules untouched, and runs synchronously before the `--resync` baseline with a fresh mtime so the relaid local copy becomes the bisync source of truth (self-healing R2). <!-- @impl: entrypoint.sh::relay_managed_pi_extensions --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017 / AD90: post-sync managed Pi extension relay describe) -->
