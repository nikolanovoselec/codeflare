...
5. The container orchestrator's destroy budget exceeds the final-sync watchdog by enough time for a clean process exit so the orchestrator does not tear down mid-sync. <!-- @impl: src/container/container-lifecycle.ts::destroy --> <!-- coverage-gap: no genuine behavioral test — "The container orchestrator's destroy budget exceeds the final-sync watchdog by enough time..." is shell/process/build boot behavior with no isolatable runnable unit in the node:test pool -->
