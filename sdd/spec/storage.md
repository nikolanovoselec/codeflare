...
4. Disabling curation restores baked reconciliation expectations, or fails without clearing applied state when the prior release is unavailable. <!-- @impl: src/routes/storage/seed.ts::default --> <!-- @test: src/__tests__/routes/storage-seed-managed.test.ts (REQ-STOR-022 AC4 + REQ-STOR-024 AC5: cacheless disable fails closed with applied state intact) --> <!-- @test: src/__tests__/routes/storage-seed-managed.test.ts (REQ-STOR-022 AC4+AC5+AC6: disable restores baked state and preserves personal intent) -->
