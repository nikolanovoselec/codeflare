...
3. After the five-minute freshness window, the resolver may fetch and activate a newly discovered release; failed initial and background checks remain retryable. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @impl: web-ui/src/stores/session-polling.ts::refreshSessionStatuses --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (uses the stored ETag after five minutes and treats 304 as a fresh no-op) --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-STOR-023 AC3: retries the managed-release check after the initial batch request fails) -->
4. During cache failure, last-known-good startup is allowed only when its applied mode matches the resolved mode; status remains update-pending without a compatible verified active descriptor. <!-- @impl: src/lib/managed-release-active.ts::getActiveManagedRelease --> <!-- @impl: src/lib/session-mode.ts::resolveEffectiveSessionMode --> <!-- @impl: src/routes/session/lifecycle.ts::default --> <!-- @test: src/__tests__/routes/session-batch-status.test.ts (REQ-STOR-023 AC4: an outage rejects last-known-good state for another mode) --> <!-- @test: src/__tests__/routes/session-batch-status.test.ts (reports update pending when no compatible verified active release is available) -->
...
6. A background status refresh validates only the latest release after a runtime mismatch, records the degraded attempt for the five-minute freshness window, and leaves bounded history traversal to explicit fresh-required validation. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-STOR-023 AC3: status refresh bounds incompatible discovery to latest and caches the failed attempt) -->

**Constraints:** Unchanged polling does not parse or decompress a managed payload; an expired status refresh may validate only the latest payload.
