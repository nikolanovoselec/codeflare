...
4. Concurrent authenticated requests resolve the same owned bucket without rate-limit failures or weakened collision isolation. <!-- @impl: src/lib/access.ts::resolveBucketName --> <!-- @test: src/__tests__/lib/access.test.ts (does not project ownership into a same-key KV hot path during a 52-request burst) -->
