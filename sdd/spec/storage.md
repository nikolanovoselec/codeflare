...
6. Known per-session ephemeral agent-state files are statically excluded from all sync operations, including Codex plugin/general caches and log databases, Copilot SQLite temporary files, and Claude workflow artifacts. The full per-path inventory lives in [documentation/lanes/storage-and-sync.md](../../documentation/lanes/storage-and-sync.md). <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON --> <!-- @test: host/__tests__/entrypoint-rclone-filters.test.js (statically excludes ephemeral caches and the R2-secret rclone config in both modes (REQ-STOR-004 AC6)) -->
...
1. The browse endpoint lists objects under a given R2 prefix with directory-style navigation. <!-- @impl: src/routes/storage/browse.ts --> <!-- @test: src/__tests__/routes/storage-browse.test.ts (Storage Browse Routes / REQ-STOR-007 (web file browser: browse endpoint with prefix validation, rate-limited)) --> <!-- @impl: src/routes/storage/validation.ts::validateKey -->
...
4. The delete endpoint removes objects by key and/or prefix in a single server-side bulk operation. <!-- @impl: src/routes/storage/delete.ts --> <!-- @test: src/__tests__/routes/storage-delete.test.ts (Storage Delete Route) -->
...
3. The multipart complete endpoint finalizes the upload by assembling the recorded parts into the final object. <!-- @impl: src/routes/storage/upload.ts --> <!-- @test: src/__tests__/routes/storage-upload.test.ts (Storage Upload Routes / REQ-STOR-008 (file upload via direct-to-R2 PUT)) -->
...
2. The five most recent per-project session transcripts are retained by modification time; older transcripts are deleted. The exact filesystem path lives in [documentation/lanes/storage-and-sync.md](../../documentation/lanes/storage-and-sync.md). <!-- @impl: entrypoint.sh::cleanup_old_transcripts --> <!-- @test: host/__tests__/entrypoint-transcript-cleanup.test.js (cleanup_old_transcripts / REQ-STOR-012 (keeps 5 newest .jsonl, deletes older, leaves session dirs intact, excludes subagents)) -->
...
2. The session batch-status endpoint reuses the cached stats without an additional TTL check, relying on the source-of-truth cache for freshness. <!-- @impl: src/routes/session/lifecycle.ts --> <!-- @test: src/__tests__/routes/storage-stats.test.ts (Storage Stats Routes / REQ-STOR-006 (storage stats endpoint reports bytes + object counts per tier quota) / REQ-STOR-014 (R2 object listing pagination via continuationToken)) -->
...
4. The Pi extension jiti prewarm cache hits at runtime in **all** deployment modes, eliminating per-session cold transpile (~2.4s on the advanced extension set). Both halves of jiti's path+content cache key are enforced: (a) the image warm-bake transpiles each extension at the **exact runtime path** `/home/user/.pi/agent/extensions`; and (b) the entrypoint relays the managed extension **content** from the unfiltered image source over the synced tree before the `--resync` baseline, overwriting only managed files already present and leaving user-added extensions and mode gating untouched. <!-- @impl: Dockerfile --> <!-- @impl: entrypoint.sh::relay_managed_pi_extensions --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017 / AD90: post-sync managed Pi extension relay (entrypoint.sh relay_managed_pi_extensions)) -->
5. The background-init subshell (bisync `--resync` baseline, vault seed, sync/vault daemons) runs concurrently with the PTY pre-warm on the single vCPU, so it self-deprioritizes (lowest niceness + idle I/O class applied to its own PID before any work, inherited by the rclone and daemon children) so pi pre-warm preempts it for CPU and disk. The baseline then runs in pre-warm's slack and after it completes, instead of contending for the core and inflating startup latency. <!-- @impl: entrypoint.sh --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-017: background init yields CPU/disk to pi pre-warm (entrypoint.sh)) -->
...
- The jiti bake is a fail-closed build gate: after the warm bake the image asserts every extension in the source dir produced a baked cache entry (`extensions-<base>.<hash>.mjs`), so an added, modified, or skipped extension that would cold-transpile fails the build rather than silently regressing startup.
