...
1. Retention and R2 transcript persistence include only Claude Code and Pi main sessions; native child artifacts and all Codex stores retain their prior exclusion behavior. <!-- @impl: transcript-retention.mjs::discoverTranscripts --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON --> <!-- @test: host/__tests__/entrypoint-transcript-cleanup.test.js (main transcript retention / REQ-STOR-012) --> <!-- @test: host/__tests__/entrypoint-governed-sync.test.js (REQ-STOR-012: child transcripts stay outside R2 sync) -->
...
3. If one candidate has an unsupported shape or no recoverable native activity, that entire agent retains ten files by deterministic filesystem modification time instead. <!-- @impl: transcript-retention.mjs::parseClaudeTimestamp --> <!-- @impl: transcript-retention.mjs::parsePiTimestamp --> <!-- @impl: transcript-retention.mjs::retainLatest --> <!-- @test: host/__tests__/entrypoint-transcript-cleanup.test.js (main transcript retention / REQ-STOR-012) -->
4. Recoverable malformed or untimestamped records do not change an otherwise valid agent from native ordering to fallback ordering. <!-- @impl: transcript-retention.mjs::readLatestTimestamp --> <!-- @test: host/__tests__/entrypoint-transcript-cleanup.test.js (main transcript retention / REQ-STOR-012) -->
...
6. Restored transcript candidates are pruned before the first agent PTY is released. <!-- @impl: entrypoint.sh::cleanup_main_transcripts --> <!-- @test: host/__tests__/entrypoint-transcript-cleanup.test.js (main transcript retention / REQ-STOR-012) -->
...
- Adapter paths and native schema checks live in [storage-and-sync.md](../../documentation/lanes/storage-and-sync.md#session-transcript-cleanup); content heuristics and Codeflare capture/extraction state never classify candidates.
