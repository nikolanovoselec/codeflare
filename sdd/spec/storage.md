...
2. Each trust configuration advances monotonically within one runtime dependency set without same-sequence conflicts; selecting a different deployed runtime set may replace a globally newer incompatible pointer. <!-- @impl: src/lib/remote-curation-cache.ts::activateManagedRelease --> <!-- @test: src/__tests__/lib/remote-curation-cache.test.ts (REQ-STOR-020 AC1+AC2: active release cache is content-addressed and monotonic) --> <!-- @test: src/__tests__/lib/remote-curation-cache.test.ts (replaces an incompatible active pointer even when its global sequence is higher) -->
