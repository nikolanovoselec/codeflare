...
1. After the five-minute freshness window, the resolver rechecks the latest release and activates it when verification succeeds. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (uses the stored ETag after five minutes and treats 304 as a fresh no-op) --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (activates only a GitHub immutable release whose two immutable asset digests and signature verify) -->
