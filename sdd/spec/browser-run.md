...
1. Enterprise setup accepts an admin-global Browser Rendering token and account ID. With `ENCRYPTION_KEY`, token storage uses deploy-key encryption; without it, the accepted [AD32](../../documentation/decisions/README.md#ad32-encryption_key-is-optional) plaintext fallback applies. Prefill reveals only presence, and a blank save preserves the existing value. <!-- @impl: src/routes/setup/index.ts::app --> <!-- @impl: src/lib/kv-crypto.ts::encryptAndStore --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (REQ-BROWSER-007: admin Browser Rendering token prefill (masked)) -->
