...
3. At session start in enterprise mode the container receives ONLY the non-secret placeholder (`ENTERPRISE_BROWSER_TOKEN_PLACEHOLDER` = `'codeflare-enterprise'`) as `CLOUDFLARE_API_TOKEN` — emitted only when an admin token is configured — plus the non-secret `CLOUDFLARE_ACCOUNT_ID` (the wizard-configured Browser Rendering account, which browser-run uses to build its API URL). The real `Browser Rendering - Edit` token NEVER enters the container: `applyEnterpriseBrowserToken` substitutes the placeholder; the GitHub token and every other field pass through unchanged. The real token is injected worker-side at the `api.cloudflare.com` boundary by the CloudflareBrowserInterceptor ([REQ-BROWSER-008](#req-browser-008-browser-rendering-token-interception-never-in-the-container)) — superseding the prior "permitted in the container env" design. `applyEnterpriseBrowserToken` runs enterprise-only, so it is untouched in non-enterprise modes; a per-user PAT still passes its real token to the container, but a non-enterprise **OAuth** session now also gets a placeholder + boundary injection via [REQ-AGENT-078](agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary). <!-- @impl: src/lib/browser-render-token.ts::applyEnterpriseBrowserToken --> <!-- @impl: src/container/container-env.ts::buildEnvVars --> <!-- @test: src/__tests__/lib/browser-render-token.test.ts (applyEnterpriseBrowserToken (REQ-BROWSER-007) -> REQ-BROWSER-008: enterprise + configured sets the PLACEHOLDER (never the real token) + admin account, preserves githubToken) --> <!-- @test: src/__tests__/container/container-env-llm.test.ts (container secret hygiene describe -> in enterprise emits ONLY the placeholder CLOUDFLARE_API_TOKEN) -->
...
1. The container's `CLOUDFLARE_API_TOKEN` is the non-secret placeholder `ENTERPRISE_BROWSER_TOKEN_PLACEHOLDER` (`'codeflare-enterprise'`), emitted only when an admin token is configured (else absent → browser-run unregistered per [REQ-BROWSER-007](#req-browser-007-enterprise-admin-configured-browser-rendering-token) AC4). The real token is read worker-side (`getEnterpriseBrowserCreds`) and never written to container env. <!-- @impl: src/lib/browser-render-token.ts::applyEnterpriseBrowserToken --> <!-- @impl: src/lib/browser-render-token.ts::getEnterpriseBrowserCreds --> <!-- @impl: src/lib/constants.ts --> <!-- @test: src/__tests__/lib/browser-render-token.test.ts (applyEnterpriseBrowserToken sets the PLACEHOLDER + getEnterpriseBrowserCreds returns the real token worker-side) -->
2. A WorkerEntrypoint `CloudflareBrowserInterceptor` is wired (enterprise, whenever an admin token + account are configured) for `api.cloudflare.com` via `interceptOutboundHttps`, INDEPENDENT of the strict-egress toggle so browser-run works in every enterprise configuration; the per-host registration takes precedence over the strict-egress `'*'` catch-all. <!-- @impl: src/container/index.ts::wireCloudflareBrowserInterception --> <!-- @impl: src/cloudflare-browser-interceptor.ts::CloudflareBrowserInterceptor --> <!-- @impl: src/index.ts --> <!-- @test: src/__tests__/cloudflare-browser-interceptor.test.ts (REST path strips placeholder + injects real token, direct not Gateway) -->
3. On the TRUSTED path — `api.cloudflare.com/client/v4/accounts/<acct>/browser-rendering/*` where `<acct>` equals the wizard-configured Browser Rendering account id (`SETUP_KEYS.BROWSER_RENDER_ACCOUNT_ID`, bound at wiring) — the interceptor strips the placeholder `authorization` and injects the real token (`Bearer`), egressing direct (own-account platform backend, AD86). Both the REST quick-actions and the CDP WebSocket upgrade (`/browser-rendering/devtools/*`) are handled; the WS is bridged via a fresh `WebSocketPair`. <!-- @impl: src/cloudflare-browser-interceptor.ts::CloudflareBrowserInterceptor --> <!-- @impl: src/cloudflare-browser-interceptor.ts::isBrowserRenderingPath --> <!-- @test: src/__tests__/cloudflare-browser-interceptor.test.ts (REST injects the real token + CDP WebSocket bridges with the token injected) -->
4. EVERY other `api.cloudflare.com` request — a different account id, or any non-`browser-rendering` path — is NOT injected: forwarded through `env.EGRESS` (the customer's Zero Trust Gateway) for inspection when strict egress is on, else rejected `403`. The injected token can therefore only ever reach the one configured account's Browser Rendering API. Fail-closed: the trusted path with no configured token returns `401` with no upstream call; strict-on with `env.EGRESS` unbound returns `503`. <!-- @impl: src/cloudflare-browser-interceptor.ts::CloudflareBrowserInterceptor --> <!-- @impl: src/cloudflare-browser-interceptor.ts::isBrowserRenderingPath --> <!-- @test: src/__tests__/cloudflare-browser-interceptor.test.ts (account-scoped trust + rest→Gateway + fail-closed 401/403/503) -->
...
- The `CloudflareBrowserInterceptor` serves **two modes** on the same `api.cloudflare.com` host, discriminated by props: this enterprise mode (fixed admin token, `/browser-rendering/*`-path-scoped) and the non-enterprise OAuth mode ([REQ-AGENT-078](agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary): per-request refreshed token, all paths). The two never coexist on one session — OAuth wiring is `!isEnterpriseMode`-guarded — so there is no host collision; the enterprise branch here is unchanged by REQ-AGENT-078.
