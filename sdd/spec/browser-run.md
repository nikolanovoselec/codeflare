...
2. The extension registers nothing unless `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` are present, and is seeded only in Pro (advanced) session mode — so Standard mode and token-less deploys are byte-identical to today. <!-- @impl: preseed/agents/pi/extensions/browser-run.ts::AgentToolResult -->
...
1. Enterprise setup accepts an admin-global Browser Rendering token and account ID; token storage uses deploy-key encryption, prefill reveals only presence, and blank or masked saves preserve the existing value. <!-- @impl: src/routes/setup/index.ts::app --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (REQ-BROWSER-007: admin Browser Rendering token prefill (masked)) -->
