...
- The `CloudflareBrowserInterceptor` serves **two modes** on `api.cloudflare.com`, discriminated by props: this enterprise mode (fixed admin token, `/browser-rendering/*`-scoped) and the non-enterprise OAuth mode ([REQ-AGENT-078](agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary): per-request refreshed token, all paths). They never coexist — OAuth wiring is `!isEnterpriseMode`-guarded — so the enterprise branch here is unchanged.
