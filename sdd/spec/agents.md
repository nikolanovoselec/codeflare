...
6. Codeflare does not globally disable context-mode's bridge idle-reaper: neither the entrypoint nor the preseeded Pi runtime extension forces `CONTEXT_MODE_BRIDGE_IDLE_MS=0` (the extension clears any inherited value), so context-mode's own foreground/subagent split keeps the interactive bridge quiet while non-foreground bridge helpers self-release and never accumulate. <!-- @impl: entrypoint.sh --> <!-- @impl: preseed/agents/pi/extensions/context-mode-runtime.ts --> <!-- @test: host/__tests__/entrypoint-context-mode-runtime.test.js (entrypoint does not force a global CONTEXT_MODE_BRIDGE_IDLE_MS override) --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (Pi context-mode runtime extension is seeded and its default export clears an inherited bridge-idle override) -->
