...
1. Only a broad capability or onboarding request starts a guided, cumulative first-person catalog connecting end-to-end engineering ownership, proof, specialist coordination, browser workspaces, persistence boundaries, design, and configured Enterprise controls; a request scoped to a repository, file, component, failure, or task uses that context instead. <!-- @impl: preseed/agents/claude/skills/codeflare-capabilities/SKILL.md::First response --> <!-- @manual: Ask a fresh Standard and Advanced session what Codeflare can do and review response order and substance. -->
