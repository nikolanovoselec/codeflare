...
- The verdict is recognised structurally, by its stacked table header, divider, and data row in assistant text; a command or demand quoting the header inline is not a verdict, and tool calls sharing the message do not disqualify it.
- The round-stamped triage file the directive names is an equivalent verdict channel: its stamp must match the completion line being gated, so a stale round's file never clears a newer round.
