...
8. Retargeting an existing PR onto a protected base with `gh pr edit --base main|master` is a boundary trigger — the same enforced boundary `gh pr create --base main` establishes, applied after creation — so a PR opened against another base (or via the web UI) and later retargeted to `main` arms a review instead of relying on the durable reconciliation catch-up; a `gh pr edit` that changes only title/body/labels, or retargets onto a non-protected base, is not a trigger. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::prEditBoundaryBase --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::isPrBoundaryTrigger --> <!-- @test: src/__tests__/lib/review-trigger.test.ts (prEditBoundaryBase gh pr edit retarget: returns the protected base across --base/--base=/-B forms, undefined for non-main base / non-base edit / non-edit command, finds the retarget inside a compound command -> AC8; isPrBoundaryTrigger treats gh pr edit onto main/master as a boundary -> AC8) -->
...
3. An idle Pi session finalizes a completed durable review by acknowledging the exact head, persisting the merged summary to disk, and starting the autofix request. Because `pi.sendMessage` does not surface a visible message outside a live turn, the idle (no-ctx) path does NOT claim the chat-summary announcement marker — claiming it off-turn would permanently suppress the on-turn re-emit (the marker is exclusive-create, never retried) and the merged summary would be acked but never shown — so the visible merged-summary message is emitted by the next on-turn tick (`refreshReviewStatusFromDurable` → `publishSummaryForCurrentPr`); when there are actionable findings the autofix request triggers that turn, so the summary surfaces with no extra user input. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::finalizeCompletedReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::publishSummaryForCurrentPr -->
...
1. On lifecycle events, for an SDD repo with an open, non-draft, enforced PR whose resolved head is unacknowledged (no pending job, no open breaker), Pi recovers the missed boundary on a later turn, branching on whether the head is in-session continuation work. When the head DESCENDS from a head already acknowledged in this repo, the on-tool-end auto-start was dropped (a compound `&&` command, a here-doc body, a `gh pr edit` retarget, or a reload between the command and its event), so Pi AUTO-STARTS the review via the shared routine — identical to a captured in-session push boundary. Otherwise (a fresh clone/checkout, which carries no prior ack in `.git` to descend from) Pi OFFERS the review once — a `ctx.ui.notify` pointing at the `/review-run` and `/review-skip` commands — so `git clone` never launches an unstoppable review; the head stays unacked (merge gate blocked) until the user runs `/review-run` (spawns the window via the shared routine) or `/review-skip` (acks the head). <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::shouldReconcileOpenPr --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reconcileBoundaryAction -->
