...
4. The release compiler rejects traversal, absolute or hidden source paths, duplicate key-and-mode pairs, context-mode-owned paths, retired image-only Pi extension names, and undeclared package or native runtime requirements. <!-- @impl: scripts/agent-seed-core.mjs::compileAgentSeed --> <!-- @test: host/__tests__/agent-seed-release.test.js (REQ-AGENT-147 AC4: release compilation rejects paths and runtime requirements outside the managed contract) -->
5. Managed extension pins include independently reviewed exact byte size and SHA-256 in addition to publisher, name, strict version, and platform; Open VSX resolution must match those pins and the packaged engine, entrypoint, extension-pack, and dependency closure before the measured record is signed. <!-- @test: host/__tests__/agent-seed-release.test.js (REQ-AGENT-147 AC5: extension resolution measures and closes exact Open VSX artifacts) -->
