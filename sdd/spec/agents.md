...
5. Classic first output begins a fixed 1.5-second settlement period, after which readiness becomes true. <!-- @impl: host/src/server.ts::PREWARM_SETTLE_MS --> <!-- @impl: host/src/server.ts::server.listen --> <!-- @manual: Observe classic first PTY output and confirm `/health` reports `prewarmReady: true` only after the fixed settlement period. -->
6. Herdr first output and launcher bootstrap detection begin a fixed 1.5-second settlement period, after which readiness becomes true. <!-- @impl: host/src/server.ts::PREWARM_SETTLE_MS --> <!-- @impl: host/src/server.ts::server.listen --> <!-- @manual: Observe Herdr first PTY output and bootstrap detection, then confirm `/health` reports `prewarmReady: true` only after the fixed settlement period. -->
7. The 20-second no-output timeout safety net applies only to classic sessions. <!-- @impl: host/src/server.ts::server.listen --> <!-- @test: host/__tests__/terminal-mode.test.js (REQ-TERM-035 AC2: timeout never reports Herdr ready before bootstrap) -->
