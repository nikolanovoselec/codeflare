...
**Intent:** Review completion follows the user while every uncompleted eligible exposure gives the user the same choice and interrupted work carries no authority.

**Applies To:** User, Agent

**Acceptance Criteria:**

1. Root startup, resume, successful clone, branch switch, branch checkout, PR checkout, pull, checked-out-branch push, and checked-out-branch PR creation resolve only the active checkout's open `main`, `master`, or `develop` PR at its exact authoritative head. A valid exact marker stays silent; a miss offers exactly `Mark review complete` and `Launch review` with a clear `repo:branch` reason and no SHA or storage detail. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::classifyReviewBoundaryCommand --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/git-push-review-reminder.sh --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (Pi marker-or-dialog review ingress) --> <!-- @test: host/__tests__/git-push-review-reminder.test.js (Claude marker-or-dialog ingress) -->
2. Cancellation writes nothing. `Mark review complete` revalidates and atomically writes the exact identity. `Launch review` replans from the newest retained same-PR ancestor or the full protected-base diff, launches required reviewers together, and adds exact-head CI only for push or PR-create context. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/git-push-review-reminder.sh --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (repeats after cancellation and stays silent after marking complete) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (launches current contextual reviewers and CI and suppresses duplicate dialogs while active) -->
...
**Supersedes:** REQ-AGENT-041; REQ-AGENT-055 AC1-AC2 and AC5-AC7; REQ-AGENT-058; REQ-AGENT-063 AC2; REQ-AGENT-074 AC1 and AC4-AC7; REQ-AGENT-080 AC1-AC2 and AC6; REQ-AGENT-110; REQ-AGENT-120; REQ-AGENT-125 AC4; REQ-AGENT-126 AC1 and AC3-AC4; REQ-AGENT-132; REQ-AGENT-141; REQ-AGENT-145; REQ-AGENT-168; REQ-AGENT-169. These clauses remain historical evidence and no longer govern ingress or completion state.
