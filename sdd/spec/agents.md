...
8. context-mode's npm update-check probe is neutralized at image-build time in **both** installs — the Claude global install (`npm root -g`) and the Pi prewarm copy at `/opt/codeflare/pi-agent/npm/node_modules/context-mode`, which Pi loads as `npm:context-mode@<ver>` via a runtime symlink to the prewarm tree — so neither the Claude CLI/MCP surface nor the Pi chat ever renders an "Update available ... ctx_upgrade" notice; the version resolves to "unknown" and no outbound npm registry traffic is generated. The two context-mode pins (global `plugin.json` + Pi `package.json`) are bumped together by one shadow-pin PR ([REQ-OPS-020](operations.md#req-ops-020-shadow-pin-version-bump-automation) AC2) and the build FATALs if they ever drift. <!-- @impl: scripts/patch-context-mode-bundles.mjs --> <!-- @impl: Dockerfile --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile context-mode patch (REQ-AGENT-005 AC5 shim + AC8 update-check disable) -> AC8 neutralizes the npm update-check probe in both bundles + idempotent + warn-only when upstream removed the probe) --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile patches context-mode in BOTH installs (global + Pi prewarm) -> patches the global install + patches the Pi prewarm copy + asserts the two version pins match so they cannot silently drift) -->
9. Pi's `web_search` workflow defaults to `auto-summary` (a preseeded `~/.pi/web-search.json`, create-if-missing so a user override survives), skipping the interactive browser-curator fallback entirely — a headless container has no launchable browser for it, and pi-web-access's `openCuratorBrowser` crashes the whole `pi` process on that failure path (upstream bug, `sendCuratorFallbackUpdate` referenced outside its declaring scope). <!-- @impl: entrypoint.sh --> <!-- coverage-gap: entrypoint.sh boot-time JSON-write with no dedicated host test yet, same posture as this REQ's other undented entrypoint.sh Constraints bullets; verified directly against the installed pi-web-access package's actual config-resolution and workflow-gating logic (not assumed) -->
...
**Verification:** [Automated test](../../host/__tests__/entrypoint-context-mode.test.js); [Pi settings packages test](../../host/__tests__/pi-settings-packages.test.js) (AC5/AC7); [Dockerfile context-mode patch test](../../host/__tests__/dockerfile-context-mode-patch.test.js) (AC8)
