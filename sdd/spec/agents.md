...
6. Any extension lifecycle path with durable review state may start `review-monitor` through `subagentsService().spawn("review-monitor", prompt, { runInBackground: true, inheritContext: false })`; missing main-session ctx never produces a waiting state. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorSpawnDecision --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (monitor handoff is not gated on main-session ctx -> AC6) -->
