...
8. context-mode's npm update-check probe is neutralized at image-build time so the agent CLI/MCP surface never renders an "Update available ... ctx_upgrade" notice. context-mode polls the npm registry for its latest version (MCP server on boot and hourly; CLI on each stats/insight render) and exposes no environment variable or flag to suppress the notice, so the image build repoints the probe at a refused local address; the version then resolves to "unknown", the notice is never shown, and no outbound npm traffic is generated. A governed container is not a surface from which a user self-upgrades context-mode. <!-- @impl: Dockerfile --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile context-mode patch describe -> AC8 neutralizes the npm update-check probe in both bundles + idempotent + warn-only when upstream removed the probe) -->
