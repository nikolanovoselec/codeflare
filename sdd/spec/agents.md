...
8. context-mode's npm update-check probe is neutralized at image-build time so the agent CLI/MCP surface never renders an "Update available ... ctx_upgrade" notice; the version resolves to "unknown" and no outbound npm registry traffic is generated. <!-- @impl: scripts/patch-context-mode-bundles.mjs --> <!-- @impl: Dockerfile --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile context-mode patch (REQ-AGENT-005 AC5 shim + AC8 update-check disable) -> AC8 neutralizes the npm update-check probe in both bundles + idempotent + warn-only when upstream removed the probe) -->
