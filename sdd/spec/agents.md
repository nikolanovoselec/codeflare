...
5. During managed reconciliation, the dashboard displays planning as "Upgrading", writing with completed and total counts, and finalization as "Finalizing" while its whole-button background advances behind centered accessible text. <!-- @impl: web-ui/src/stores/session-polling.ts::refreshSessionStatuses --> <!-- @impl: web-ui/src/components/Dashboard.tsx::Dashboard --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-AGENT-175 AC5: mirrors managed release progress on transient polling) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-AGENT-175 AC5: whole-button managed upgrade progress preserves centered text and ordinary completion color) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-AGENT-175 AC5: shows planning and finalizing managed upgrade phases) -->
