...
4. On initial dashboard load, if an upgrade is needed, the frontend triggers the dedicated automatic reconcile in the background. <!-- @impl: web-ui/src/stores/session.ts::applyManagedReleaseBatch --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-STOR-033 AC6: should trigger the automatic upgrade endpoint when preseedNeedsUpgrade is true) -->
