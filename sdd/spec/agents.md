...
8. A name resolved only on the weakest evidence the resolver accepts -- a registered string, or a token in a dependency manifest -- is reported in its own class rather than as an ordinary pass, and the stronger forms are tried first so a real declaration is never demoted into it. <!-- @impl: preseed/agents/claude/skills/review-scope/scripts/lane-evidence.mjs::summarise --> <!-- @test: host/__tests__/lane-evidence.test.js (reports a name known only from a manifest as a weak resolution rather than a pass) --> <!-- @test: host/__tests__/lane-evidence.test.js (does not demote a real declaration that a manifest happens to also mention) -->
...
- Outside `package.json`, which is JSON and exact, no dependency manifest is parsed by its grammar. Seven consecutive defects in that grammar were all false cleans, so the file is tokenised and the result is offered as the weakest evidence class instead. A manifest setting resolving alongside a real package is the accepted cost, and it is stated to the lane rather than hidden inside a pass.
