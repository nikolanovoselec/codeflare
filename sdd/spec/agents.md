...
7. After a main/master SDD PR is opened or synced, the main session verifies the durable review job for the exact head and verifies or restarts `review-monitor`; a monitor task that stops or completes without `REVIEW_RESULT` is treated as undelivered and restarted from existing job prompt/result paths rather than trusted until TTL. <!-- @impl: preseed/agents/pi/skills/pr-workflow/SKILL.md --> <!-- @impl: preseed/agents/pi/skills/git-review-pipeline/SKILL.md --> <!-- coverage-gap: AC7 is a main-session operational workflow over Pi background-task UI notifications; the repo test pool cannot inject a stopped/no-output background `review-monitor` notification into the live assistant session. Verified by live PR-boundary smoke review. -->
...
1. Every Pi CI-producing push or PR creation starts `ci-monitoring` in a backgrounded agent unless the user explicitly skips that push/PR. <!-- @impl: preseed/agents/pi/rules/git-workflow.md::git-workflow-ci-route --> <!-- @impl: preseed/agents/pi/skills/pr-workflow/SKILL.md --> <!-- @impl: preseed/agents/pi/skills/git-review-pipeline/SKILL.md --> <!-- @impl: preseed/agents/pi/skills/ci-monitoring/SKILL.md::ci-monitor-detached-script --> <!-- coverage-gap: the PR-creation launch obligation is main-session skill/operator behavior, not an importable function; the monitor's detached exact-head behavior is tested below. -->
