...
4. Claude's permanently loaded safe-check rule remains below 400 characters, and Pi's pre-skill policy remains below 4,500 characters. <!-- @impl: preseed/agents/claude/rules/no-local-builds.md --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-157 AC4: permanently loaded policy stays bounded) -->
5. Canonical operational guidance reaches each lazy skill projection. <!-- @impl: preseed/agents/claude/skills/safe-local-checks/SKILL.md --> <!-- @test: host/__tests__/pi-native-review-assets.test.js (REQ-AGENT-157 AC5: canonical safe-check guidance reaches each lazy skill projection) -->
6. Pi keeps the safe-check skill explicitly invocable without duplicate native catalog injection. <!-- @impl: scripts/agent-seed-core.mjs::finalizePiSkillIndex --> <!-- @test: src/__tests__/lib/pi-compact-context.test.ts (keeps indexed skills explicitly invocable while omitting duplicate native XML entries) -->
