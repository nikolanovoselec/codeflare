...
6. Layer 2 false-positive filtering validates that the candidate push belongs to the currently open PR; same-session pushes may use local HEAD as the candidate SHA while GitHub's PR API catches up, with every directive carrying a freshness gate for the exact head. The head-currency check is tri-state: a pending review window is discarded only when the PR has definitively moved on or closed (and only with a visible warning); a `gh` query that could not be read leaves the window intact and retries, so a transient query failure never drops the merge gate or leaves a reviewed head un-acked.
