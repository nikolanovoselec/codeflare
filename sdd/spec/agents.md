...
<!-- @test: src/__tests__/lib/review-state.test.ts (shouldReconcileOpenPr decision gating -> AC1/AC6; reconcileBoundaryAction action gate: autostarts in-session continuation, offers a fresh clone once, no-ops on re-offer of a clone head and on a non-reconcilable head -> AC1; activeRepoCandidateForReview rejects an out-of-session remembered repo before accepting a guarded persisted repo -> AC8; every suppressed gate names a distinct non-empty reason -> AC4) -->
...
1. Lifecycle reconciliation re-evaluates open non-draft PRs to `main`/`master` from GitHub PR state on session and turn ticks, and after successful shell commands that invoke `git` or `gh`. If the enforced head is unacknowledged and has no pending review or breaker, a head advanced beyond this session's branch baseline starts review automatically; an inherited head is offered once and remains merge-blocking until the user starts or skips review. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mentionsGitOrGhCommand --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::shouldReconcileOpenPr --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reconcileBoundaryAction --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewInSessionContinuation --> <!-- @test: src/__tests__/lib/review-state.test.ts (shouldReconcileOpenPr gates enforced open PR heads; reviewInSessionContinuation distinguishes in-session advances from inherited heads -> AC1) -->
...
8. Commandless reconciliation can recover a nested SDD repo after reload only from a live, remembered, or persisted active-repo candidate that is a git repo, is under the session roots, and has the SDD marker; a rejected remembered candidate must not suppress a later valid persisted candidate. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::activeRepoForReviewRouting --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::activeRepoCandidateForReview --> <!-- @test: src/__tests__/lib/review-state.test.ts (activeRepoCandidateForReview rejects out-of-session remembered repo and falls through to guarded persisted repo -> AC8) -->
