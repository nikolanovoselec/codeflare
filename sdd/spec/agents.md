...
5. Every CI-producing push starts `ci-monitoring` in a backgrounded agent unless the user explicitly says to skip CI monitoring for that push. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-background-agent --> <!-- @impl: preseed/agents/claude/rules/git-workflow.md::git-workflow-ci-route -->
6. The CI monitor evaluates every workflow row returned for the monitored HEAD and requires the workflow/run-id fingerprint to stay stable before reporting success; CI failures are reported back to the main session and the backgrounded agent does not fix, commit, or push. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-workflow-row-fingerprint --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC6 ci monitor waits for a stable workflow/run set before success; reports failed workflow rows; times out when workflows never finish -> AC6) -->
7. Long-running waits, monitors, or polls never keep the main session busy; agents start backgrounded agent/subagent work, report how to check it, and stop. <!-- @impl: preseed/agents/claude/rules/git-workflow.md::git-workflow-hard-obligations --> <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-no-main-session-monitor --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::ENGINEERING_CONSTITUTION --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC5/AC7 ci monitor launcher starts detached work and returns immediately -> AC7) -->
...
2. A delivery announcement becomes visible only after durable conversation history proves the announcement-specific nonce was delivered to the user; a bare send attempt, tool output, event log, or assistant reasoning text never proves delivery. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sessionContainsNonce --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::transcriptEntryContainsDeliveryNonce --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::announcementReconcileDecision --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewAnnouncementNonce --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (transcript delivery proof accepts only nonce-bearing custom messages; nonce determinism/uniqueness; reconcile nonce->visible; autofix request carries the nonce -> AC2) -->
