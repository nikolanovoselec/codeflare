...
3. GitHub CLI detection recognises PR-head-moving operations (`gh pr create`, `gh pr merge`, `gh pr update-branch`, `gh repo sync`) and a `gh pr edit --base main|master` that retargets an existing PR onto a protected base — the same boundary `gh pr create --base main` establishes, applied after creation — so a PR opened against another base or via the web UI and later retargeted to `main` arms a review rather than waiting on the durable reconciliation catch-up. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::isPrBoundaryCommand --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::prEditBoundaryBase --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::isPrBoundaryTrigger --> <!-- @test: src/__tests__/lib/review-trigger.test.ts (prEditBoundaryBase gh pr edit retarget across --base/--base=/-B and compound forms, undefined for non-main base / non-base edit / non-edit command; isPrBoundaryTrigger treats gh pr edit onto main/master as a boundary -> AC3) -->
...
3. An idle Pi session finalizes a completed durable review by acknowledging the exact head, persisting the merged summary to disk, and starting the autofix request; because `pi.sendMessage` cannot surface a message outside a live turn, the no-ctx path does NOT claim the chat-summary announcement marker (claiming it off-turn would permanently suppress the on-turn re-emit and lose the summary), so the visible merged summary is emitted by the next on-turn tick (`refreshReviewStatusFromDurable` → `publishSummaryForCurrentPr`), which the autofix turn-trigger surfaces with no extra user input. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::finalizeCompletedReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::publishSummaryForCurrentPr -->
...
1. On a lifecycle tick, for an SDD repo with an open, non-draft, enforced PR whose resolved head is unacknowledged (no pending job, no open breaker), Pi recovers the missed boundary on a later turn — AUTO-STARTING the review via the shared routine when the head descends from a head already acked in this repo (an in-session push whose on-tool-end auto-start was dropped by a compound `&&`, here-doc, `gh pr edit`, or reload), and otherwise (a fresh clone/checkout with no prior ack to descend from) OFFERING it once via `ctx.ui.notify` → `/review-run` / `/review-skip` so `git clone` never auto-spawns, with the head staying unacked (merge gate blocked) until the user chooses. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::shouldReconcileOpenPr --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reconcileBoundaryAction -->
