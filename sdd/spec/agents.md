...
6. Claude Code is warmed by its install-time version smoke-test rather than the dedicated warm-up block; Go-based agents (OpenCode, Antigravity) are natively compiled and need no warm-up. <!-- @impl: Dockerfile --> <!-- coverage-gap: claude-code/go-based-agent warm-up exclusion is asserted only by the absence of a codex/copilot-specific test, no dedicated behavioral test names this claim -->
