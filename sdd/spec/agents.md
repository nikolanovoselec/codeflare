...
7. A missed-boundary offer is durable in the chat transcript, not only a transient toast: the offer branch emits a registered custom-type message (`codeflare-review-offered`, rendered as markdown via `pi.registerMessageRenderer`) through `pi.sendMessage` alongside the `ctx.ui.notify` toast and the `boundary_offered` audit event. The offer is deduped PER SESSION (process-scoped `globalThis` set `codeflare.reviewOfferSurfacedThisSession`, `Symbol.for`), not per-head-ever: a new `pi` launch on a still-unchosen offer re-surfaces it exactly once. This replaced an on-disk per-head marker that suppressed the offer forever — the "nothing offered, no question, nothing" symptom where a relaunched session saw no offer at all. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::markOfferSurfaced --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::offerSurfacedThisSession -->
