...
5. A reviewer-bearing PR boundary pauses one active Goal through its session-local control channel; only the matching FIX reminder resumes it, and that reminder owns the continuation turn without injecting a separate Goal prompt. Missing control, replacement, independent resume, persistence failure, or PR closure fails open and cannot strand review enforcement or a bridge-owned Goal. <!-- @impl: scripts/patch-pi-goal-review-control.mjs::patchPiGoalSource --> <!-- @impl: .github/workflows/bump-shadow-pins.yml::pi-extensions --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::pauseGoalForReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::releaseReviewGoalPause --> <!-- @test: host/__tests__/pi-goal-review-control-patch.test.js (REQ-AGENT-111: pi-goal review control patch) --> <!-- @test: src/__tests__/ci/suite-gates.test.ts (REQ-AGENT-111: pi-goal shadow bumps preflight the locked review-control patch) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: pauses an active Goal once at review launch and resumes the same Goal from the FIX reminder) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: transfers an owned pause to a replacement PR head and resumes after its FIX reminder) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: rolls back a paused Goal when replacement-head ownership cannot be recorded) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: fails open when the Goal extension is unavailable) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: keeps FIX delivery fail-open when Goal is removed during review) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-041/REQ-AGENT-058: a merged PR neither consumes bypass nor writes acknowledgement) -->
