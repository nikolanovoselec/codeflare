...
5. When the user explicitly asks to monitor CI or a deploy/merge gate requires a fresh result, the `ci-monitoring` skill starts one detached background monitor for the target HEAD; routine pushes do not auto-start CI monitoring. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::Detached monitor pattern --> <!-- @impl: preseed/agents/claude/rules/git-workflow.md::Triggers and routes -->
6. Long-running waits, monitors, or polls for CI, deploy status, review completion, log tailing, `watch`, `tail -f`, `gh run watch`, `while sleep`, Bash, or blocking `ctx_execute` never keep the main session busy; agents start detached/background work, report how to check it, and stop. <!-- @impl: preseed/agents/claude/rules/git-workflow.md::Hard obligations --> <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::Hard rule: never monitor in the main session --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::ENGINEERING_CONSTITUTION -->
