...
4. When the fast-start preference is disabled, the suppression channels are not applied, Codeflare-managed settings-file suppressors are removed, each CLI runs its normal update path on launch, and Pi runs `pi update` so Pi packages reconcile before the session starts.
...
- Fast Start ON sets `PI_OFFLINE=1`; restored user-added Pi packages outside the Codeflare image cache may require Fast Start OFF once to reinstall.
