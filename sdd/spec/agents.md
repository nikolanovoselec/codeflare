...
**Intent:** Pi review completion must be proven only by visible service-owned reviewer dispatches and their correlated native terminal records in the root transcript.

**Applies To:** User

**Acceptance Criteria:**

1. Only service dispatch records after the latest successful boundary can satisfy its review window. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-helpers.test.ts (REQ-AGENT-053/REQ-AGENT-059: correlates terminal service records only by dispatched agent ID) -->
2. A successful terminal record satisfies only the reviewer dispatch carrying its agent ID. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-helpers.test.ts (REQ-AGENT-053/REQ-AGENT-059: correlates terminal service records only by dispatched agent ID) -->
3. A failed terminal record leaves its lane unacknowledged and eligible for relaunch. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-074: retries only the lane whose service record failed) -->
4. A dispatch unknown to a reloaded service remains in flight, while a live service status of stopped, aborted, or error makes that reviewer or CI launch eligible for retry. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::serviceDispatchFailed --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-074: keeps unknown reload dispatches in flight but retries live stopped agents) -->
5. Acknowledgement changes only after every required reviewer reports successful completion. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-055/REQ-AGENT-074: acknowledges only matching successful service records) -->
...
- Service dispatch metadata is minimal session transcript evidence, not durable review job or result state.
...
**Intent:** Pi review completion must use the latest root-transcript boundary, service dispatch IDs, and terminal service records as its complete session-scoped window, without pending JSON, roll-forward state, or a merge interceptor.

**Applies To:** User

**Acceptance Criteria:**

1. Only reviewer service dispatches after the latest successful boundary belong to its active window. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-helpers.test.ts (REQ-AGENT-053/REQ-AGENT-059: correlates terminal service records only by dispatched agent ID) -->
...
**Intent:** Reviewer findings must reach the main session through each visible service-owned subagent result, without Codeflare-owned result files, summaries, severity parsing, or an automatic fix state machine.

**Applies To:** User

**Acceptance Criteria:**

1. Agent-ID correlation leaves each native reviewer result intact for the main session. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-helpers.test.ts (REQ-AGENT-053/REQ-AGENT-059: correlates terminal service records only by dispatched agent ID) -->
2. Successful terminal reviewer records are the completion proof. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-055/REQ-AGENT-074: acknowledges only matching successful service records) -->
3. Every ranged PR-boundary plan carries the executable diff work-set contract. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @impl: preseed/agents/pi/extensions/review-scope.ts::scopeContract --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-036/REQ-AGENT-063/REQ-AGENT-074: emits one ordered reviewer-then-CI launch plan) -->
...
**Intent:** Pi CI monitoring must have one owner and one result path. The PR-boundary dispatcher launches one dedicated background agent for an eligible main-bound PR, and that agent runs one attached deterministic monitor whose native task result is independent of PR review.

**Applies To:** Agent

**Acceptance Criteria:**

1. An eligible head-changing boundary resolves its affected PR to one complete `ci-monitor` request. <!-- @impl: preseed/agents/pi/skills/ci-monitoring/scripts/monitor-ci.mjs::resolveCiMonitorRequest --> <!-- @test: host/__tests__/pi-ci-monitor.test.js (REQ-AGENT-068 AC1: eligible push resolves the affected PR exactly once) -->
...
- The PR-boundary extension is the sole automatic trigger.
...
**Intent:** The Pi boundary extension must launch the minimal required report-only reviewers together through the stock session subagent service and must not mistake one slow lane for completion of its peers.
...
3. Every service-spawned reviewer and CI monitor appears in one above-editor review widget whose rows transition through queued, running, completed, or failed state from the matching live service record. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::createReviewVisualController --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-071: renders deterministic reviewer lifecycle state without a model launch turn) -->
...
- Service access uses the published `Symbol.for("@gotgenes/pi-subagents:service")`; no package import or Pi host change is required.
- The existing boundary extension owns only the review widget projection; no second subagent extension or execution path is introduced.
- Reviewer service spawns use `foreground: false` and `inheritContext: false`.
- Global `maxConcurrent: 1` serializes memory-heavy reviewer execution; the CI request uses `bypassQueue: true` after reviewer IDs are recorded so monitoring remains independent.
...
**Intent:** Pi's settled event must service-dispatch only missing review lanes and acknowledge only transcript-proven completion. It owns no review monitor, durable claim, or restart path.

**Applies To:** Agent

**Acceptance Criteria:**

1. Settled enforcement appends one extension-only follow-up window and service-dispatches every missing lane for the reminder head without a model launch turn. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-071/REQ-AGENT-080: spawns reviewers through the service before CI without a model launch turn) -->
...
3. Unknown reload dispatches are not duplicated, while live stopped, aborted, or failed reviewer and CI agents are retried from their immutable dispatch IDs. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::serviceDispatchFailed --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-074: keeps unknown reload dispatches in flight but retries live stopped agents) -->
...
6. Child sessions are inert for plans, settled dispatch, and acknowledgement. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-055/REQ-AGENT-058: keeps child sessions inert for reminders, settled follow-ups, and state writes) -->
7. After reload or newer unpublished local work, a persisted delayed terminal record acknowledges its reminder head only while the authoritative PR head still matches that reviewed head. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-055/REQ-AGENT-074: delayed completion acknowledges the reviewed PR head after reload and new local work) -->

**Constraints:**

- The settled dispatcher calls the already-published stock Pi subagent service directly.
- `ci-monitor` is never a reviewer or acknowledgement condition.
- No queue, database, Pi-core patch, custom host, or automatic restart exists.
...
2. A state-changing `/ctx` command reloads Pi into the selected enabled or disabled state. <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::handleContextModeCommand --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-076 AC2: /ctx reloads Pi into the selected state) -->
...
**Intent:** A successful Pi PR boundary must produce one ordered service dispatch so reviewer requests are recorded together before independent exact-head CI, without model-mediated launch instructions, duplicate triggers, or lifecycle coupling.

**Applies To:** Agent

**Acceptance Criteria:**

1. An eligible SDD head-changing boundary appends one extension-only structured window, then service-spawns its reviewer wave before exact-head `ci-monitor`. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-071/REQ-AGENT-080: spawns reviewers through the service before CI without a model launch turn) -->
2. An eligible non-SDD boundary emits a CI-only plan and service-spawns only its resolved monitor request. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-068: emits a CI-only launch plan outside SDD mode) -->
...
4. Transcript correlation recognizes a matching exact-head CI service dispatch independently from reviewer state. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-helpers.test.ts (REQ-AGENT-068/REQ-AGENT-080: recognizes a matching service-owned CI dispatch) -->
5. Settled follow-up retries only missing or live-failed reviewer lanes without duplicating dispatches unknown after reload. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::serviceDispatchFailed --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-053/REQ-AGENT-074: keeps unknown reload dispatches in flight but retries live stopped agents) -->
6. CI resolution and spawn occur only after every requested reviewer returns an agent ID; an empty resolution is persisted so reload cannot duplicate it. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-068/REQ-AGENT-080: persists an empty CI resolution so reload does not re-resolve it) -->
...
- The boundary extension service-spawns every reviewer together before CI.
- The resolver's CI request is translated only into the stock service's equivalent spawn options and submitted once.
- Git commands create no second CI trigger.
- All launches use isolated background subagents; they do not create assistant public-tool-call transcript blocks.
- Review-window entries are extension-only extension state and never enter LLM context.
- The dispatcher stores only session transcript windows and dispatch IDs, never durable job or result state.
...
1. Starting an in-process Pi subagent does not initialize a second context-mode owner. <!-- @impl: preseed/agents/pi/extensions/context-mode-runtime.ts::attachContextModeToForeground --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-089 AC1: one process owner rejects child context-mode initialization) -->
2. After the owning session shuts down, a new session can initialize context-mode. <!-- @impl: preseed/agents/pi/extensions/context-mode-runtime.ts::attachContextModeToForeground --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-089 AC2: owner shutdown permits context-mode reattachment) -->
