...
7. An active Plan workflow exposes its persisted allowed tools plus `plan_mode_question` and `plan_mode_complete`; configured `always` Goal visibility also retains Goal's terminal tools. <!-- @impl: preseed/agents/pi/extensions/capability-helpers.ts::activePlanTools --> <!-- @test: src/__tests__/lib/pi-capabilities.test.ts (REQ-AGENT-152/158: preserves only a restored Plan workflow policy plus required helpers) --> <!-- @test: src/__tests__/lib/pi-capabilities.test.ts (REQ-AGENT-158: keeps configured always-visible Goal tools during an active Plan) -->
