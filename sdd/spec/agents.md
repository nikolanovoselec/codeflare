...
6. Only the current or last remembered live main-session ctx may start `review-monitor`; task paths and absent ctx wait when no remembered main-session ctx exists. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorContextDecision --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorContextSource --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::isTaskSessionFile --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (remembered main-session context fallback -> AC6) -->
...
- Task/subagent contexts may reap lanes and write durable state, but they start `review-monitor` only by reusing an already remembered main-session context.
