...
6. In an OAuth session the container trusts the platform-mounted intercept CA (`/etc/cloudflare/certs/cloudflare-containers-ca.crt`) so the intercepted `api.cloudflare.com` / `gateway.ai.cloudflare.com` TLS validates in agent runtimes (Node/wrangler/curl) instead of failing `SELF_SIGNED_CERT_IN_CHAIN`. The trust runs in `entrypoint.sh` as an isolated non-enterprise block gated on `ENTERPRISE_MODE != active` **and** the CA file's presence, leaving the enterprise CA-trust ([REQ-ENTERPRISE-005](enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC2) byte-untouched. <!-- @impl: entrypoint.sh --> <!-- @test: host/__tests__/entrypoint-oauth-ca-trust.test.js (non-enterprise + CA mounted trusts the CA env in .bashrc; enterprise and CA-absent both skip; idempotent) -->
