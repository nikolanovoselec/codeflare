...
4. Starting Goal in Codeflare's capability-filtered session reveals `goal_complete` and `goal_blocked` for the active Goal without making them part of the initial tool set. <!-- @impl: preseed/agents/pi/pi-goal.json::toolVisibility --> <!-- @test: host/__tests__/pi-settings-packages.test.js (REQ-AGENT-111 AC4: reveals terminal tools when Goal starts under capability mode) -->
