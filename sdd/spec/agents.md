...
4. Acknowledgement is followed by a fix directive that states the head is already acknowledged and, when accepted fixes change files, commits them without renewed consent. It issues no push order, matching Pi's FIX follow-up, so the review push gate and the CI-result gate decide when the head is delivered; it never merges or creates a no-op commit. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::ROUND_COMPLETE_LINE --> <!-- @test: host/__tests__/enforce-review-spawn.test.js (drives the FIX phase once the verdict is published) -->
