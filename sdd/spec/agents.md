...
6. A Claude PR-boundary lane binds its scope to that PR's acknowledged ancestor and current head, overriding a stale full-base argument; an already acknowledged head invokes no model. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/run-review-lane.sh::BOUNDARY_PR --> <!-- @test: host/__tests__/run-review-lane.test.js (normalizes a PR-boundary full-diff request to the acknowledged incremental range) --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::acknowledge --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::ACK_FILE --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-041: consumes a one-shot bypass and acknowledges the exact PR head) -->
