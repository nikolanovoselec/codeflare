...
6. The display-only summary is delivered with a plain `pi.sendMessage` (no `triggerTurn`/`deliverAs`), taking Pi's synchronous append path that persists the nonce-bearing content into the transcript AND displays it in one step — the same path `/review-results` uses — gated on `pi.isIdle()` so it is never steered into a streaming turn, staying pending for the next idle tick (`agent_end`/`turn_end`/`session_start`) otherwise. The nonce-verify/retry phase (AC2/AC3) is the backstop, not the primary delivery path: a `triggerTurn`/`followUp` send instead routes through `_runAgentPrompt`/`agent.followUp`/`agent.steer`, whose custom-message persistence is exactly what off-turn/post-reload no-op'd the summary into never surfacing. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sendAnnouncement -->
