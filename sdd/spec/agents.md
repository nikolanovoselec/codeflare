...
5. Explicit CI monitoring starts one detached background monitor for the target HEAD. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-detached-monitor --> <!-- @impl: preseed/agents/claude/rules/git-workflow.md::git-workflow-ci-route --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC5/AC7 ci monitor launcher starts detached work and returns immediately -> AC5) -->
...
- Routine pushes must not auto-start CI monitoring; only an explicit user request or deploy/merge gate may invoke `ci-monitoring`.
