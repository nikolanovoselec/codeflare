...
2. The monitor waits until every required lane result file and `summary.md` exist, using a low-turn polling strategy so it can stay alive for its full budget; missing result files while a required lane marker is still `running` are waiting state, not failure. <!-- @impl: preseed/agents/pi/agents/review-monitor.md --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reviewMonitorPrompt --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorDecision --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (review monitor waits for every required lane file and summary.md before reporting ready) --> <!-- @test: src/__tests__/lib/review-state.test.ts (monitor decision requires complete lane results + summary before autofix_required) -->
...
4. Early lane failures return `REVIEW_RESULT failed` without writing `monitor.completed` only after a required lane marker explicitly reports `status: "failed"`; missing result files while a required lane is still `running` never count as failure. <!-- @impl: preseed/agents/pi/agents/review-monitor.md --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reviewMonitorPrompt --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorDecision --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (review monitor decision returns wait, not failed, when result files are missing and failedLanes is empty) --> <!-- coverage-gap: AC4's actual REVIEW_RESULT failed emission is the background review-monitor's runtime output path — a Pi runtime integration path the Workers vitest pool cannot inject; verified by live smoke/adversarial review. -->
...
6. Monitor handoff asks the main session for visible exact-head review and CI monitors; if the follow-up cannot be sent, a single hidden fallback monitor may be started without duplicate claims. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sendVisibleMonitorHandoff --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::visibleMonitorHandoffRequest --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (visible monitor handoff requests review and CI monitors for the exact head) -->
7. After a main/master SDD PR is opened or synced, the main session obeys the visible handoff, reports monitor agent IDs, and restarts any CI/review monitor task that stops or completes without `CI_RESULT`/`REVIEW_RESULT`. <!-- @impl: preseed/agents/pi/skills/pr-workflow/SKILL.md --> <!-- @impl: preseed/agents/pi/skills/git-review-pipeline/SKILL.md --> <!-- @impl: preseed/agents/pi/rules/git-workflow.md::git-workflow-hard-obligations --> <!-- coverage-gap: AC7 is a main-session operational workflow over Pi background-task UI notifications; the repo test pool cannot inject stopped/no-output background-task notifications into the live assistant session. Verified by live PR-boundary smoke review. -->

**Constraints:**

- The monitor receives all needed state in its explicit prompt and must not inherit parent context.
- A visible handoff or fallback monitor startup keeps the durable monitor claim so repeated lifecycle ticks do not spawn duplicates.
...
6. After `CI_RESULT`, the main session prints the CI summary first; if a monitor stops or returns no contract result, the main session restarts exact-head monitoring unless superseded or skipped. <!-- @impl: preseed/agents/claude/rules/engineering-constitution.md::CI-result handoff gate --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::ENGINEERING_CONSTITUTION --> <!-- @impl: preseed/agents/pi/rules/git-workflow.md::git-workflow-hard-obligations --> <!-- @impl: preseed/agents/pi/skills/pr-workflow/SKILL.md --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (CI-result handoff contract clauses are generated into all instruction surfaces) --> <!-- coverage-gap: stopped/no-output monitor restart is a main-session response to Pi background-task UI notifications; verified by live PR-boundary smoke review. -->
