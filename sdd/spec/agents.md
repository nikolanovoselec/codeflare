...
<!-- @test: src/__tests__/lib/review-state.test.ts (shouldReconcileOpenPr decision gating -> AC1/AC6; reconcileBoundaryAction action gate: autostarts in-session continuation, offers a fresh clone once, no-ops on re-offer of a clone head and on a non-reconcilable head -> AC1; activeRepoSentinelForReview accepts only nested SDD repos under the session root -> AC1; every suppressed gate names a distinct non-empty reason -> AC4) -->
