...
5. Every reviewer takes its round-limit decision from the deterministic gate it is directed to, counting any agent-authored tag that touched its lane rather than only its own. <!-- @impl: preseed/agents/claude/skills/spec-enforce/SKILL.md::Required execution manifest --> <!-- @impl: preseed/agents/claude/skills/spec-enforce/scripts/round-limit.mjs::action --> <!-- @test: host/__tests__/pi-native-review-assets.test.js (REQ-AGENT-084: the round-limit row routes the verdict to the gate in every runtime) -->
