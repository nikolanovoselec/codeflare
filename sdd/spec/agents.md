...
5. Explicit CI monitoring starts one detached background monitor for the target HEAD; routine pushes do not auto-start CI monitoring. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-detached-monitor --> <!-- @impl: preseed/agents/claude/rules/git-workflow.md::git-workflow-ci-route --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC5/AC7 ci monitor launcher starts detached work and returns immediately -> AC5) --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-021 AC5 propagates routine-push/no-monitor and detached CI instructions into seed surfaces -> AC5) -->
6. The CI monitor evaluates every workflow row returned for the monitored HEAD and requires the workflow/run-id fingerprint to stay stable before reporting success. <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-workflow-row-fingerprint --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC6 ci monitor waits for a stable workflow/run set before success -> AC6) -->
7. Long-running waits, monitors, or polls never keep the main session busy; agents start detached/background work, report how to check it, and stop. <!-- @impl: preseed/agents/claude/rules/git-workflow.md::git-workflow-hard-obligations --> <!-- @impl: preseed/agents/claude/skills/ci-monitoring/SKILL.md::ci-no-main-session-monitor --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::ENGINEERING_CONSTITUTION --> <!-- @test: host/__tests__/ci-monitoring-skill.test.js (REQ-AGENT-021 AC5/AC7 ci monitor launcher starts detached work and returns immediately -> AC7) -->
...
2. A delivery announcement becomes visible only after its unique nonce appears in the session transcript; a bare `sendMessage` result never proves delivery. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sessionContainsNonce --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::announcementReconcileDecision --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewAnnouncementNonce --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (nonce determinism/uniqueness; reconcile nonce->visible; autofix request carries the nonce -> AC2) -->
