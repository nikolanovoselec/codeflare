...
1. Activation rejects unsupported paths, image-owned content, duplicate ownership, and undeclared runtime requirements while retaining only bounded release metadata and one document body. <!-- @impl: src/lib/remote-curation.ts::verifyManagedReleaseStream --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-AGENT-150 AC1+AC2: independently rejects invalid release records before activation) -->
2. Activation rejects invalid extension records and non-semantic exact versions. <!-- @impl: src/lib/remote-curation.ts::verifyManagedReleaseStream --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-AGENT-150 AC1+AC2: independently rejects invalid release records before activation) -->
3. Activation aborts streaming release expansion beyond the shared byte limit, and successful validation can stream identical document payloads without retaining the complete expanded release. <!-- @impl: src/lib/remote-curation.ts::verifyManagedReleaseStream --> <!-- @impl: src/lib/remote-curation.ts::streamManagedReleaseDocuments --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-AGENT-150 AC3: aborts gzip expansion at the shared expanded-byte limit) --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-AGENT-150 AC3: validates a signed release without retaining document bodies and streams the identical documents) -->
