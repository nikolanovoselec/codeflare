...
5. A reviewer-bearing PR boundary pauses one active Goal, and only its matching FIX reminder resumes it. Missing control, replacement, independent resume, persistence failure, and PR closure remain fail-open. Replacement heads retain recoverable ownership when rollback fails. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::pauseGoalForReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::releaseReviewGoalPause --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: pauses an active Goal once at review launch and resumes the same Goal from the FIX reminder) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: transfers an owned pause to a replacement PR head and resumes after its FIX reminder) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: rolls back a paused Goal when replacement-head ownership cannot be recorded) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: retains failed rollback ownership and releases it after replacement-head review) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: fails open when the Goal extension is unavailable) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: keeps FIX delivery fail-open when Goal is removed during review) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-041/REQ-AGENT-058: a merged PR neither consumes bypass nor writes acknowledgement) -->
6. Capability initialization keeps both terminal Goal tools for an unfinished Goal or Goal's already-active `always` policy; absent, cleared, completed, malformed, or lazy fresh state does not independently widen the set. <!-- @impl: preseed/agents/pi/extensions/capability.ts::hasUnfinishedGoal --> <!-- @test: src/__tests__/lib/pi-capabilities.test.ts (REQ-AGENT-111: restores terminal Goal tools only for an unfinished session Goal) --> <!-- @test: src/__tests__/lib/pi-capabilities.test.ts (REQ-AGENT-111: preserves Goal tools already active under the always-visible policy) -->
7. Bridge-owned resume suppresses Goal's separate prompt; the matching FIX follow-up owns the continuation turn without populating user input. Package-layout drift fails before release. <!-- @impl: scripts/patch-pi-goal-review-control.mjs::patchPiGoalDirectory --> <!-- @impl: scripts/patch-pi-goal-review-control.mjs::patchPiGoalSource --> <!-- @impl: scripts/patch-pi-goal-review-control.mjs::patchPiGoalCommandsSource --> <!-- @impl: .github/workflows/bump-shadow-pins.yml::pi-extensions --> <!-- @test: host/__tests__/pi-goal-review-control-patch.test.js (REQ-AGENT-111: pi-goal review control patch) --> <!-- @test: host/__tests__/pi-goal-review-control-patch.test.js (REQ-AGENT-111: suppresses only the bridge-owned resume prompt) --> <!-- @test: src/__tests__/ci/suite-gates.test.ts (REQ-AGENT-111: pi-goal shadow bumps preflight the locked review-control patch) --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-111: pauses an active Goal once at review launch and resumes the same Goal from the FIX reminder) -->

**Constraints:** Goal upgrades remain exact-pinned and pass normal review, lock regeneration, build-time control-patch verification, and deployment image verification. PR enforcement never depends on Goal availability.
...
**Verification:** Entrypoint runtime behavior test; review-enforcement behavioral tests; deployment image verification
