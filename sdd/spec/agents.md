...
7. No review window is created from non-triggering states: non-SDD projects run no agents (vibe-coding), and passive lifecycle events never create a window from branch existence alone. Review starts only from an explicit command, persisted pending state, or the bounded open-PR reconciliation in [REQ-AGENT-058](#req-agent-058-pr-boundary-review-reconciliation-and-missed-event-recovery). <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::isSddProject --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview -->
...
<!-- coverage-gap: AC7's detached child-process lane execution (spawn, pid-group liveness, unref, exit) is a runtime behavior verified by an integration smoke test (a real detached `pi --mode json -p --no-extensions` lane that runs to agent_end, writes its transcript, and exits cleanly), with no dedicated automated test in the Workers vitest pool (which cannot spawn pi). The extension-source selection sub-behavior (which packages a lane `-e`-loads) is unit-tested via laneExtensionSources. -->
...
<!-- coverage-gap: the reap DECISION logic (reapLaneDecision) and transcript parsing (summarizeLaneTranscript) are unit-tested; the OS-level pieces they sit on — detached child-process spawn, pid-group liveness (process.kill(pid,0)), group kill, and the AC7 reload-safe setInterval driver (autonomousReviewReaperTick) — are runtime behaviors verified by an integration smoke test (spawn a real detached lane → it runs to agent_end → reap writes a result file), with no dedicated test in the Workers vitest pool. -->
...
7. The reaper runs both on Pi lifecycle ticks and on a reload-safe background interval, so an idle session with no user turn still reaps finished lanes, finalizes a completed review, and advances the next eligible lane. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::autonomousReviewReaperTick -->
...
1. On each lifecycle event, when an SDD repo has an open, non-draft, enforced PR to `main`/`master` whose resolved head is unacknowledged with no pending job and no open breaker, Pi creates the review window even if the originating boundary command's event was never observed. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reconcileOpenPrReview --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::shouldReconcileOpenPr -->
...
3. Head resolution tolerates the develop→main metadata lag: it reviews the local head only when that head is on the PR's own branch, descends from the reported PR head, and has been pushed (the remote-tracking ref contains it); otherwise it trusts GitHub's PR head. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::resolveEnforcedHead --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::enforcedHeadDecision --> <!-- @test: src/__tests__/lib/review-trigger.test.ts (enforcedHeadDecision pushed-vs-unpushed table -> AC3) -->
...
1. The `graphifyy` Python package is installed in every container image at build time with the MCP, SQL, and PDF extras, pinned to a single Dependabot-tracked version so version bumps rebuild the image in lockstep. Provider extras are not installed; extraction and labeling stay agent-driven.
2. Claude Code receives the graphify MCP server in every session (default and advanced). Pi receives equivalent first-party native tools (`graphify_query`/`graphify_path`/`graphify_explain`, registered by `graphify-native.ts`) — not an MCP server or npm wrapper, since Pi has no MCP client. Both surfaces shell the same upstream graphify engine.
...
4. Both the Claude MCP server and Pi's native surface tolerate a missing graph at startup: Claude shows an empty graph and rebinds when one appears; advanced-mode Pi clone triage asks before any graph work. Query tools answer against the active repo's `graphify-out/graph.json` once it exists. <!-- @impl: preseed/agents/pi/extensions/graphify-helpers.ts::graphifyCloneAction --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts::fallbackGraphifyToolResult -->
5. In advanced session mode only, the active repository is tracked so graphify queries scope to it; resolution walks up to the nearest ancestor containing a Git repo or graph artefact. Pi also tracks it from command-local `cd ... &&` and `git -C ...` forms.
...
- Pi's first-party graphify tools (`graphify-native.ts`) resolve each query's graph in precedence order — the cwd repo's `graphify-out/graph.json`, then the active-repo sentinel's graph, then the merged global graph (`~/.graphify/global-graph.json`) — and fail soft with a "build a graph first" message when none exists.
