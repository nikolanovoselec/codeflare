...
1. Creating an active PR-boundary review window starts at most one background `review-monitor` per `(repo, head)` at the same time as the durable lanes; a fresh started monitor suppresses duplicates until its TTL expires, and a `monitor.completed` marker suppresses future spawns. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::ensureReviewWindow --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewMonitorSpawnDecision -->
