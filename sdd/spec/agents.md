...
5. Every reviewer takes the round-limit decision from the deterministic gate it is directed to, counting any agent-authored tag that touched its lane rather than only its own, and reports both the counted total and that decision as enforcement evidence. <!-- @impl: preseed/agents/claude/skills/spec-enforce/SKILL.md::Required execution manifest --> <!-- @test: host/__tests__/pi-native-review-assets.test.js (REQ-AGENT-084: the round-limit row routes the verdict to the gate in every runtime) -->
