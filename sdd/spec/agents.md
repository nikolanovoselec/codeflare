...
5. Pre-warm readiness is detected by first PTY output (any terminal output means the agent is ready). <!-- @impl: host/src/prewarm-config.ts::getPrewarmConfig --> <!-- coverage-gap: AC5 (prewarm readiness detected by first PTY output) has no automated test exercising getPrewarmConfig readiness detection -->
6. A 20-second hard timeout exists as a safety net if the PTY produces no output. <!-- @impl: host/src/prewarm-config.ts --> <!-- coverage-gap: AC6 (20-second hard timeout safety net) has no automated test -->
...
3. Mode selection is available in Settings under the session-defaults area. <!-- @impl: web-ui/src/components --> <!-- coverage-gap: AC3 (mode selection available in Settings session-defaults area) is UI placement; no automated test -->
...
3. When a GitHub credential is present, the container configures git for authenticated HTTPS access. <!-- @impl: entrypoint.sh --> <!-- coverage-gap: the git credential.helper configuration (entrypoint.sh: `git config --global credential.helper` under `[ -n "$GH_TOKEN" ]`) is entrypoint shell logic; container-env.test.ts covers only env-var injection and git-clone.test.js covers only clone arg construction, neither asserts the credential-helper wiring. -->
4. The Cloudflare account ID is resolved automatically from the API token when one is stored, so users need not supply it separately. <!-- @impl: entrypoint.sh --> <!-- coverage-gap: the auto-resolution of CLOUDFLARE_ACCOUNT_ID from the API token is entrypoint shell logic; container-env.test.ts asserts only that an already-resolved CLOUDFLARE_ACCOUNT_ID is injected, not the from-token resolution step. -->
...
2. The endpoint is rate-limited (3/min). <!-- @impl: src/routes/storage/seed.ts::storageSeedRateLimiter --> <!-- coverage-gap: the rate limiter is wired (createRateLimiter maxRequests:3 windowMs:60_000, applied via app.use('*', storageSeedRateLimiter)) but storage-seed.test.ts asserts only the recreate and cache-invalidation paths, not the 3/min throttle behavior. -->
...
3. The skill scopes review by `--all` or `--diff` parsed from the appended command line, prints help and runs no phases when neither flag is present, and supports the `--deep` and `--verify-high` flags. <!-- @impl: preseed/agents/pi/extensions/review-command.ts --> <!-- coverage-gap: src/__tests__/lib/review-command.test.ts covers the /review-status renderer (REQ-AGENT-057), not /review flag parsing; the --all/--diff/--deep/--verify-high parsing and help-on-no-flag behavior have no dedicated behavioral test. -->
...
6. The extension is delivered advanced-only via the Pi manifest (`extensions/codeflare-commands.ts`) through the standard seed pipeline. <!-- @impl: preseed/agents/pi/manifest.json --> <!-- coverage-gap: AC6 is the manifest mode-gate for codeflare-commands.ts; the REQ-AGENT-051 describe in agent-seed-manifest.test.ts asserts command registration behavior, not the advanced-only manifest gate, so this AC has no dedicated automated assertion. -->
