...
**Verification:** [Automated test](../../host/__tests__/entrypoint-context-mode.test.js) (AC4)
...
1. Pi starts with context-mode ENABLED by default — its `ctx_*` tools and the bash-curl-redirect hook are active without an explicit `/ctx on`. The Codeflare Pi extension provides `/ctx status`, `/ctx on`, and `/ctx off`; `/ctx off` disables the context-mode package for the current running session and reloads resources, while the next Codeflare container start resets Pi back to enabled. Each Pi session also starts with `CONTEXT_MODE_BRIDGE_IDLE_MS=0`, from both the container entrypoint env and the preseeded Pi runtime extension, so context-mode bridge helpers do not emit idle-release notices into the TUI while still exiting with their parent/session. <!-- @impl: entrypoint.sh --> <!-- @impl: preseed/agents/pi/extensions/codeflare-pi.ts --> <!-- @impl: preseed/agents/pi/extensions/context-mode-runtime.ts --> <!-- @test: host/__tests__/entrypoint-context-mode.test.js (entrypoint-context-mode describe -> mode-gated context-mode preseed + hooks) --> <!-- @test: host/__tests__/pi-settings-packages.test.js (Pi settings.json packages assembly describe -> context-mode enabled by default + 5 tool extensions in required + coexistence/idempotence/dedup) --> <!-- @test: host/__tests__/context-mode-version-pin.test.js (context-mode plugin.json version pin >= v1.0.151 (issue #671 fix surface)) --> <!-- @test: host/__tests__/entrypoint-context-mode-runtime.test.js (entrypoint context-mode runtime defaults set CONTEXT_MODE_BRIDGE_IDLE_MS=0 even when an old nonzero value is inherited) --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (Pi context-mode runtime extension is seeded and its env helper returns a non-mutating disabled bridge env) -->
...
4. context-mode's npm update-check probe is neutralized at image-build time in **both** installs — the Claude global install (`npm root -g`) and the Pi prewarm copy at `/opt/codeflare/pi-agent/npm/node_modules/context-mode`, which Pi loads as `npm:context-mode@<ver>` via a runtime symlink to the prewarm tree — so neither the Claude CLI/MCP surface nor the Pi chat ever renders an "Update available ... ctx_upgrade" notice; the version resolves to "unknown" and no outbound npm registry traffic is generated. The two context-mode pins (global `plugin.json` + Pi `package.json`) are bumped together by one shadow-pin PR ([REQ-OPS-020](operations.md#req-ops-020-shadow-pin-version-bump-automation) AC2) and the build FATALs if they ever drift. <!-- @impl: scripts/patch-context-mode-bundles.mjs --> <!-- @impl: Dockerfile --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile context-mode patch (REQ-AGENT-076 AC1 shim + AC4 update-check disable) -> AC4 neutralizes the npm update-check probe in both bundles + idempotent + warn-only when upstream removed the probe) --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile patches context-mode in BOTH installs (global + Pi prewarm) -> patches the global install + patches the Pi prewarm copy + asserts the two version pins match so they cannot silently drift) -->
...
**Notes:** The upstream `pi-web-access` bug this default avoids (`openCuratorBrowser`/`sendCuratorFallbackUpdate` scoping) and its crash symptom are documented in [documentation/lanes/troubleshooting.md](../../documentation/lanes/troubleshooting.md#pi-web-search-crashed-the-session).
...
- context-mode exposes no environment variable or flag to suppress its update-check notice (`CI`/`NO_UPDATE_NOTIFIER` affect other tools only), so the AC4 disable repoints the probe URL to a refused local address inside the bundle at image-build time, applied by a shared module the Dockerfile and the host test both use. A governed container is not a surface from which a user self-upgrades context-mode. <!-- @impl: scripts/patch-context-mode-bundles.mjs --> <!-- @impl: Dockerfile -->
...
**Verification:** [Context-mode gating test](../../host/__tests__/entrypoint-context-mode.test.js) (AC1, AC2); [Pi settings packages test](../../host/__tests__/pi-settings-packages.test.js) (AC1, AC3); [Dockerfile context-mode patch test](../../host/__tests__/dockerfile-context-mode-patch.test.js) (AC4); [Pi web-search default test](../../host/__tests__/entrypoint-pi-web-search.test.js) (AC5)
