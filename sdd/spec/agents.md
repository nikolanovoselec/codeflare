...
- Enterprise is untouched: the OAuth CF-API interception-registry entry (`cloudflareOauthApi`, container-interception.ts) is double-guarded (`!isEnterpriseMode` **and** placeholder-value match), so it can never wire or collide on `api.cloudflare.com` in enterprise; the enterprise branch is unchanged.
