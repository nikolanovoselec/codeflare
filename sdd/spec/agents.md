...
3. Every reviewer and CI monitor is service-spawned as background work, so the stock `@gotgenes/pi-subagents` `AgentWidget` owns its width-safe queued/running/finished rendering, animation, elapsed time, tool and token metrics, compaction count, and timer cleanup. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @test: src/__tests__/lib/review-enforcement.test.ts (REQ-AGENT-071/REQ-AGENT-080: delegates background reviewers to the stock service widget before CI without a model launch turn) -->
...
- The boundary extension adds no widget renderer; the existing stock subagent extension displays the same background records it executes.
