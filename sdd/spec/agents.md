...
7. Release discovery selects the newest valid signed release whose runtime dependency hash matches the exact deployed build, even when newer releases target other builds. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (REQ-AGENT-150 AC7: activates the newest signed release matching this build hash) -->
