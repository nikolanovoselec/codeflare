...
3. Head resolution tolerates the develop→main metadata-visibility lag: when the local head is on the PR's own branch and descends from a stale reported PR head it reviews the local head, otherwise it trusts GitHub's PR head; a transient `gh` read failure preserves any pending window rather than discarding it. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::resolveEnforcedHead --> <!-- @test: (none — resolveEnforcedHead shells git/gh; lag-tolerance has no direct unit test, which is why this REQ is Partial) -->
