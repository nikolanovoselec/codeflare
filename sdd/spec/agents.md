...
3. Claude refuses root-response completion for an eligible unacknowledged pushed head targeting any of those three bases unless the head is acknowledged or the existing five-attempt access-preserving circuit breaker releases it. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::BASE_REF --> <!-- @test: host/__tests__/enforce-review-spawn.test.js (enforce-review-spawn.sh — PR state gating) --> <!-- @test: host/__tests__/enforce-review-spawn.test.js (enforce-review-spawn.sh - 5-strike circuit breaker GIVEUP state) -->
