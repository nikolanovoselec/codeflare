...
7. Both session modes set `disableAgentView`, so background agents never replace the terminal with the full-screen agent view; background subagents themselves keep running, so the capture hooks are unaffected. <!-- @impl: entrypoint.sh::SETTINGS_CONFIG --> <!-- @test: host/__tests__/entrypoint-hooks-merge.test.js (settings.json configuration / REQ-AGENT-015 (/review command)) -->
