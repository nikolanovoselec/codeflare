...
8. When all lanes are complete and `summary.md` exists but no valid `monitor.completed` latches within the monitor TTL — the monitor died, or a non-contract deliverer reported the result without writing the marker — the extension delivers the summary itself and only then reaps the head (writes the completion marker and acks), so a completed review never dead-ends the merge gate; if that self-delivery fails, it falls back to the blocked give-up so an ack never runs undelivered. Delivery always precedes ack. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::finalizeCompletedReview --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::writeReviewMonitorCompletionFromDisk --> <!-- @impl: preseed/agents/pi/extensions/review-job-helpers.ts::reviewCompletionResultFromCounts --> <!-- @test: src/__tests__/lib/review-monitor-reliability.test.ts (reviewCompletionResultFromCounts derives the self-reap completion result from on-disk lane counts) -->
...
6. Each Pi session starts with `CONTEXT_MODE_BRIDGE_IDLE_MS=0`, set by both the container entrypoint and the preseeded Pi runtime extension, so context-mode bridge helpers never emit idle-release notices into the TUI while still exiting with their parent session. <!-- @impl: entrypoint.sh --> <!-- @impl: preseed/agents/pi/extensions/context-mode-runtime.ts --> <!-- @test: host/__tests__/entrypoint-context-mode-runtime.test.js (entrypoint context-mode runtime defaults set CONTEXT_MODE_BRIDGE_IDLE_MS=0 even when an old nonzero value is inherited) --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (Pi context-mode runtime extension is seeded and its env helper returns a non-mutating disabled bridge env) -->
