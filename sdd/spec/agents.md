...
8. The shared engineering constitution includes a work-continuity rule: a new user message is queued while the current concrete step reaches a safe stopping point, unless the user explicitly says to stop, pause, or reprioritize. The rule is present in Claude's source rule and every adapted agent instruction file. <!-- @impl: preseed/agents/claude/rules/engineering-constitution.md --> <!-- @impl: scripts/generate-agent-seed.mjs --> <!-- @impl: src/lib/agent-seed.generated.ts --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (preseeds work continuity into every agent instruction surface -> AC8) -->
