...
4. Permanently loaded safe-check policy remains bounded. <!-- @impl: preseed/agents/claude/rules/no-local-builds.md --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-AGENT-157 AC4: permanently loaded policy stays bounded) -->
5. Canonical operational guidance reaches each lazy skill projection, and Pi keeps that skill explicitly invocable without duplicate native catalog injection. <!-- @impl: preseed/agents/claude/skills/safe-local-checks/SKILL.md --> <!-- @impl: scripts/agent-seed-core.mjs::finalizePiSkillIndex --> <!-- @test: host/__tests__/pi-native-review-assets.test.js (REQ-AGENT-157 AC5: canonical safe-check guidance reaches each lazy skill projection) --> <!-- @test: src/__tests__/lib/pi-compact-context.test.ts (keeps indexed skills explicitly invocable while omitting duplicate native XML entries) -->
