...
8. context-mode's npm update-check probe is neutralized at image-build time so the agent CLI/MCP surface never renders an "Update available ... ctx_upgrade" notice; the version resolves to "unknown" and no outbound npm registry traffic is generated. <!-- @impl: Dockerfile --> <!-- @test: host/__tests__/dockerfile-context-mode-patch.test.js (Dockerfile context-mode patch (REQ-AGENT-005 AC5 shim + AC8 update-check disable) -> AC8 neutralizes the npm update-check probe in both bundles + idempotent + warn-only when upstream removed the probe) -->
...
- context-mode exposes no environment variable or flag to suppress its update-check notice (`CI`/`NO_UPDATE_NOTIFIER` affect other tools only), so the AC8 disable repoints the probe URL to a refused local address inside the bundle at image-build time. A governed container is not a surface from which a user self-upgrades context-mode. <!-- @impl: Dockerfile -->
