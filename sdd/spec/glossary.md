...
| Strict Gateway Egress | An optional, enterprise-gated, default-OFF setup-wizard toggle (`setup:strict_egress`) that forces the container's direct-internet HTTP/HTTPS egress through the Workers VPC `EGRESS` binding and the customer's Cloudflare Gateway; platform-native Cloudflare primitives (R2, AI Gateway, Browser Rendering) are exempt and egress direct ([AD86](../../documentation/decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)). Fails closed (503 `EGRESS_UNAVAILABLE`) when the binding is unbound, byte-identical to today when OFF ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
| EgressController | The `WorkerEntrypoint` wired as the `interceptOutboundHttps('*', …)` catch-all when Strict Gateway Egress is ON: a transparent proxy (no identity stamping) for every host not owned by the LLM/GitHub interceptors, forwarding direct-internet hosts through `env.EGRESS.fetch` (and platform-native Cloudflare hosts direct via global `fetch`) and failing closed ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
