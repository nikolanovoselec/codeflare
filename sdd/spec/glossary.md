...
| EgressController | The `WorkerEntrypoint` wired as the `interceptOutboundHttps('*', …)` catch-all when Strict Gateway Egress is ON: a transparent proxy (no identity stamping) for every host not owned by the LLM/GitHub interceptors, forwarding direct-internet hosts (incl. any other account's Cloudflare host) through `env.EGRESS.fetch` and this deployment's own-account R2 / CF API direct via global `fetch` (`isAccountScopedDestination`, account id from `ctx.props.accountId`); proxies WebSocket upgrades verbatim; fails closed ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
