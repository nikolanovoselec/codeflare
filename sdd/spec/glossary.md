...
| Strict Gateway Egress | An optional, enterprise-gated, default-OFF setup-wizard toggle (`setup:strict_egress`) that forces all container HTTP/HTTPS egress through the Workers VPC `EGRESS` binding and the customer's Cloudflare Gateway; fails closed (503 `EGRESS_UNAVAILABLE`) when the binding is unbound, byte-identical to today when OFF ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
| EgressController | The `WorkerEntrypoint` wired as the `interceptOutboundHttps('*', …)` catch-all when Strict Gateway Egress is ON: a transparent proxy (no identity stamping) for every host not owned by the LLM/GitHub interceptors, forwarding through `env.EGRESS.fetch` and failing closed ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
...
| Cloudflare Mesh | The reserved Cloudflare network (id `cf1:network`) backing the Workers VPC `EGRESS` binding that carries public egress through the Gateway. Public egress needs no provisioned WARP Connector; the binding is enterprise-only and injected at deploy time. A future raw-TCP egress plane over a WARP Connector on this transport is a reserved seam ([REQ-ENTERPRISE-017](enterprise-mode.md#req-enterprise-017-raw-tcp-warpmesh-egress-plane-reserved)). |
