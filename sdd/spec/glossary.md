...
| EGRESS binding | The Workers VPC `[[vpc_networks]]` Fetcher binding (`binding="EGRESS"`, `network_id="cf1:network"`) that routes container egress through the customer's Cloudflare Gateway; committed commented-out in `wrangler.toml` until Cloudflare Mesh is provisioned ([REQ-ENTERPRISE-016](enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). |
...
| Cloudflare Mesh | The WARP Connector / mesh network (id `cf1:network`) backing the Workers VPC `EGRESS` binding that carries public egress through the Gateway. Not yet provisioned on the account, so the binding ships commented-out; a future raw-TCP egress plane over this transport is a reserved seam ([REQ-ENTERPRISE-017](enterprise-mode.md#req-enterprise-017-raw-tcp-warpmesh-egress-plane-reserved)). |
