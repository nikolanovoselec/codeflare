...
7. Only the Action redeems one activity-specific start capability and collects the terminal bundle once, retaining an independent receipt before publication. Lost start/result responses cannot replay consumed authority. Browser JWTs and publisher credentials never enter Pi or the compiled child.
...
**Verification:** Generation-fenced collection has test-only RED CI; the remaining handoff, collection, publisher, compiled production and Enterprise Integration evidence are pending.
