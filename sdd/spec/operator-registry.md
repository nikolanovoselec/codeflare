...
4. An uncertain start blocks another start until its prepared activity is reconciled through owner-scoped detail; failed preparation cannot be reconciled against a previous activity. <!-- @impl: web-ui/src/components/OperatorManagementActivity.tsx::OperatorManagementActivity --> <!-- @test: web-ui/src/__tests__/operators/operator-management-activity.test.tsx (REQ-OPERATOR-049: human-owned invocation and activity) -->
