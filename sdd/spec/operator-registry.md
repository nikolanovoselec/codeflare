...
6. Parent-composed profiles confine independent SDK task sessions to finite parent-approved input references, exact filesystem reads and outputs, immutable report staging, and no candidate extension or broader Host access. <!-- @impl: src/operators/session-initialization.ts::parseOperatorPiInitialization --> <!-- @impl: host/src/operator-pi-isolated.ts::createIsolatedPiTools --> <!-- @impl: host/src/operator-pi-isolated-runner.ts::runApprovedTasks --> <!-- @test: src/__tests__/operators/session-initialization.test.ts (REQ-OPERATOR-021: approved initialization scope) --> <!-- @test: host/__tests__/operator-pi-review.test.js (REQ-OPERATOR-021: existing Review filesystem boundary) --> <!-- @test: host/__tests__/operator-pi-isolated.test.js (REQ-OPERATOR-021: restricted filesystem tools) --> <!-- @test: host/__tests__/operator-pi-isolated-runner.test.js (REQ-OPERATOR-021: independent durable tasks) -->
...
3. Opaque attachments are restored only from the admitted digest-and-size-bound projection before Conductor work starts. Claimed Action packets use the source session's fixed credential-free Host task, conditional owner-bucket storage with exact readback or verified replay, and signed-claim/drive-bound immutable Activity descriptors. The package checks its complete packet checkpoint; the parent reads the current generation's private Activity checkpoint (never browser detail) before accepting initialization. Activity transactionally compares the session's request digest against the current attachment projection at reservation and denies later additions. <!-- @impl: src/operators/activity.ts::OperatorActivity.getCurrentDriveCheckpointJson --> <!-- @test: src/__tests__/operators/activity-state.test.ts (REQ-OPERATOR-003: instrumented activity state outcomes) --> <!-- @impl: src/operators/attachments.ts::projectOperatorAttachments --> <!-- @impl: src/operators/attachments.ts::persistApprovedPacketAttachment --> <!-- @impl: src/operators/activity.ts::OperatorActivity.saveApprovedPacketAttachment --> <!-- @impl: src/operators/conductor-production.ts::createConductorProductionCapability --> <!-- @impl: scripts/restore-operator-attachments.mjs::restoreOperatorAttachments --> <!-- @test: src/__tests__/operators/attachments.test.ts (operator opaque attachment ownership) --> <!-- @test: src/__tests__/operators/approved-packet-storage.test.ts (REQ-OPERATOR-050/052: parent-owned immutable packet storage) --> <!-- @test: src/__tests__/operators/conductor-production-packet.test.ts (REQ-OPERATOR-050/053: claimed parent packet crosses only the ordinary Host and sealed storage) -->
