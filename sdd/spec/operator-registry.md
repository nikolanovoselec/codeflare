...
2. At an eligible Codeflare-authenticated PR creation or push, the parent verifies the invoking human, pinned installation, numeric repository, current PR/head/base/merge-base and the installed, enabled and applicable trusted Action. The acknowledged head, exact target/range, and rejected triage findings with their reasoning and original evidence references arrive as bounded session-bound data, not principal authority; the parent independently verifies repository identity, current context and acknowledgement ancestry. It prebinds the actor and instance to one visible activity before the configured Action redeems its handoff; GitHub automatically starts that Action on the PR event. Conductor generates the canonical packet from the verified range after Action-started admission; rejected reasoning does not itself resolve findings. The authenticated Action atomically claims its run/attempt for the exact prebound context; a callback, session owner, workflow label, PR author or GitHub email cannot choose or renew the human identity.
...
**Constraints:** No new identity broker, Pi/Codeflare `workflow_dispatch`, workflow-selected principal, JWT renewal, candidate-controlled workflow or change to local `/review`. Human authority expiry stops protected execution. Actions without matching Codeflare handoff fail closed.
