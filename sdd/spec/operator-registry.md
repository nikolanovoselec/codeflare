...
7. Only the protected Action redeems one activity-specific start capability and separately redeems the terminal result once with its read capability, retaining an independent receipt before publication. Lost responses cannot replay consumed authority. Browser JWTs and publisher credentials never enter Pi or the compiled child. <!-- @impl: src/operators/review-boundary-claim.ts::claimVerifiedBoundaryAction --> <!-- @impl: src/operators/boundary-action-oidc.ts::verifyBoundaryActionOidc --> <!-- @impl: src/operators/registry.ts::OperatorRegistry --> <!-- @impl: src/operators/activity.ts::OperatorActivity --> <!-- @impl: src/lib/session-repository.ts::D1SessionRepository --> <!-- @impl: src/routes/operator-webhook.ts::app --> <!-- @test: src/__tests__/operators/review-boundary-claim.test.ts (REQ-OPERATOR-053: real prepared Registry and Activity owners at protected Action claim) --> <!-- @test: src/__tests__/lib/session-d1-lifecycle.test.ts (REQ-OPERATOR-053: Stop wins before claim or retains the exact Action activity until durable cancellation) -->

**Constraints:** Neither session ownership nor GitHub identity supplies human authority. No new identity broker, Pi/Codeflare `workflow_dispatch`, workflow-selected principal, JWT renewal, or candidate-controlled workflow. Local `/review` remains unchanged and never starts the operator. Uncertain applicability, disabled configuration, expired authority, or failed remote execution cannot silently fall back or clear a check. Human authority expiry stops protected execution. Actions without matching Codeflare handoff fail closed. The parent independently verifies numeric repository, PR/head/base/merge-base, acknowledged-head ancestry and installed Action bytes. Bounded session-bound range and rejected-finding evidence are not principal authority. GitHub triggers the Action automatically; only its authenticated claim binds run/attempt. At claim the parent verifies signed protected `pull_request_target` identity, current GitHub PR/run/workflow, sealed human authority and D1 session generation. Stop durably cancels the pending Activity before confirmed exit or restart; ambiguous responses stay pending/unknown. Conductor creates the canonical packet after Action start; rejected reasoning alone cannot resolve findings.
