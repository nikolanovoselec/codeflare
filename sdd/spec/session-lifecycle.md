...
6. When its monitor loses container transport, a session enters bounded recovery without stopping its existing workload before unavailability is confirmed. <!-- @impl: src/container/container-lifecycle.ts::onError --> <!-- @impl: src/container/container-metrics.ts::beginMonitorTransportRecovery --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-SESSION-021 AC6: reconstructs immediately when the SDK monitor loses container services after running becomes false) -->
