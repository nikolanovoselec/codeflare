...
7. Terminal recovery ownership ends only after the platform accepts the container stop; a failed stop re-arms a non-billable retry. <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-SESSION-022 AC7: retains exhausted recovery and retries when terminal container stop fails) -->
