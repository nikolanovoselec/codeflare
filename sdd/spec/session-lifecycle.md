...
6. A failed authoritative stopped-status write retains exhausted recovery ownership and re-arms a non-billable retry. <!-- @impl: src/container/container-metrics.ts::updateKvStatus --> <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-SESSION-022 AC3: retains exhausted recovery and retries a failed authoritative stopped write) -->

**Constraints:** Confirmed recovery resumes normal metrics cadence; failed reconstruction confirmation retains five-second monitoring, while a failed terminal status write retries after 60 seconds.
