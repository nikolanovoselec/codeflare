...
4. The inner Durable Object gate returns retryable 1013 when its volatile running flag is false, while stopped persisted state remains the sole source of authoritative 4503. <!-- @impl: src/container/index.ts::fetch --> <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @test: src/__tests__/container/index.test.ts (REQ-SESSION-012 AC4: returns retryable 1013 when volatile runtime state reads not-running) --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (returns 4503 without rate-limit use for Container state %s) -->
