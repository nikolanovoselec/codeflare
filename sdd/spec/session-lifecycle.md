...
<!-- @test: src/__tests__/container-metrics.test.ts (collectMetrics describe -> skips metrics write when stopped and graceful shutdown in flight -> AC4 deliberate-stop guard; re-asserts running when container is alive but KV reads stopped and no shutdown in flight -> AC4 self-heal) -->
