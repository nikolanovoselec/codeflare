...
4. A start request that confirms the container is running re-asserts running when KV status is stopped or missing. <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (returns already_running and reconciles missing KV status when the container is running) -->
