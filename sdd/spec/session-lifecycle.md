...
3. The session record, including its immutable workspace snapshot, is persisted durably and retrievable across restarts; a missing snapshot resolves to Terminal. <!-- @impl: src/routes/session/crud.ts::toWorkspaceApiSession --> <!-- @impl: src/lib/kv-keys.ts::putSessionWithMetadata --> <!-- @test: src/__tests__/routes/session-workspace.test.ts (REQ-IDE-048 AC1: immutable session workspace snapshot) -->
