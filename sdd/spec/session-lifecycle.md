...
3. Teardown records the session stopped while the identifiers that write requires are still in hand, and only once the deliberate-stop marker is durable, so a teardown that does not run to completion still leaves the session recorded as stopped rather than running and a concurrent metrics tick cannot read that write as a false stop and re-assert running. <!-- @impl: src/container/container-lifecycle.ts::destroy --> <!-- @test: src/__tests__/container/index.test.ts (records the session stopped BEFORE clearing the identifiers that write needs) -->
