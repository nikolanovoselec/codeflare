...
2. WebSocket upgrades receive authoritative 4503 only when persisted Container SDK state confirms the session is stopping or stopped. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @test: src/__tests__/routes/terminal-ws.test.ts (returns 4503 without rate-limit use for Container state %s) -->
...
4. When transient runtime state reports not-running at the no-wake terminal gate, the WebSocket receives retryable 1013 without container wake. <!-- @impl: src/container/index.ts::fetch --> <!-- @test: src/__tests__/container/index.test.ts (REQ-SESSION-012 AC4: returns retryable 1013 when volatile runtime state reads not-running) -->
