...
5. A transient private health or sessions probe failure remains retryable as `starting` or `verifying`; another unexpected startup-status computation failure is caught and returned as an error stage rather than propagating an unhandled 500. <!-- @impl: src/routes/container/status.ts::app --> <!-- @test: src/__tests__/routes/container-status.test.ts (GET /container/startup-status) --> <!-- @manual -->
