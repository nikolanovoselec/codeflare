...
6. A container-monitor network-loss signal enters the same durable recovery before a not-running reading can bypass the running-only probes. <!-- @impl: src/container/container-lifecycle.ts::onError --> <!-- @impl: src/container/container-metrics.ts::beginMonitorTransportRecovery --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-SESSION-021 AC6: reconstructs immediately when the SDK monitor loses container services after running becomes false) -->
