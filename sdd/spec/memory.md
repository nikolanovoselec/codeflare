...
3. When a deferred capture is pending, the hook blocks parent tools and unrelated children after spawn as well as before it; only a child whose harness-supplied agent identity exactly matches the authorized spawn identity may atomically claim and use tools. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::AUTH_FILE --> <!-- @test: host/__tests__/memory-capture-block.test.js (memory-capture-block.sh - child-correlated authorization / REQ-MEM-012 AC3+AC4) -->
