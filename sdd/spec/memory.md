...
1. In advanced mode, the user's vault directory (including its own graph output) is included in R2 sync. <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON --> <!-- coverage-gap: the Vault-included-in-advanced-mode rclone filter is only checked by the source-string-matching audit (host/__audits__/entrypoint-vault.audit.js); no behavioral test asserts the filter actually includes the Vault tree -->
2. On container boot, the vault is pulled from R2 before any initialization runs so returning sessions inherit their persisted content untouched. <!-- @impl: entrypoint.sh::establish_bisync_baseline --> <!-- coverage-gap: the establish_bisync_baseline-precedes-init_user_vault boot ordering is only checked by the source-string-matching audit (host/__audits__/entrypoint-vault.audit.js); no behavioral test exercises the boot-time pull-before-init ordering -->
3. Vault directory initialization is idempotent; re-running on a populated vault creates nothing. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- coverage-gap: init_user_vault's existence-guarded idempotency is only checked by the source-string-matching audit (host/__audits__/entrypoint-vault.audit.js); no behavioral test re-runs init against a populated vault to assert nothing is created -->
...
5. The ephemeral unified-graph layer is rebuilt locally on every container boot and is not synced. <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON --> <!-- coverage-gap: the .graphify-excluded-from-bisync filter is only checked by the source-string-matching audit (host/__audits__/entrypoint-vault.audit.js); no behavioral test asserts the unified-graph layer is excluded from sync -->
6. The shutdown handler watchdog allows the final bisync up to 120s to drain pending writes before forced termination. <!-- @impl: entrypoint.sh::shutdown_handler --> <!-- coverage-gap: the 120s (108s SIGTERM + 12s SIGKILL) shutdown watchdog budget is only checked by the source-string-matching audit (host/__audits__/entrypoint-vault.audit.js); no behavioral test exercises the watchdog timeout path -->
...
1. In default mode, the vault directory is not preserved across container recreations: the R2 sync filters include the Vault tree only in advanced mode and explicitly exclude it in default mode, so cross-session persistence is limited to advanced-mode sessions. <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON --> <!-- coverage-gap: only source-string-matching audit (host/__audits__/entrypoint-vault.audit.js) exercises the Vault-in-advanced-only rclone filter; no behavioral test asserts the default-mode Vault exclusion -->
...
3. Mode changes take effect only on explicit "Recreate AI agent skills & rules" click or new bucket creation. <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- coverage-gap: trigger-binding (recreate-click / new-bucket) has no behavioral test; reconcileAgentConfigs reconciliation logic is covered under AC4, but the explicit-trigger gate is not exercised by an automated test -->
