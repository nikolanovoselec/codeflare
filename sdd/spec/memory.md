...
- The size ceiling is a memory guard, not a latency one — the query's own timeout bounds the parse — and it is overridable, so a graph that outgrows the default cannot silently disable injection with no signal that it stopped working. The default is set from a measured parse-to-resident ratio rather than a round number, and a non-numeric override falls back to it so the guard fails closed rather than disappearing.
