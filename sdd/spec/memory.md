...
2. Claude and Pi count only real user messages; tool results and known synthetic agent envelopes are excluded, while genuine code-like prompts remain eligible. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture.sh::COUNTER_DIR --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::isSyntheticPrompt --> <!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (REQ-MEM-001 AC2: real-user prompt counting matches Claude synthetic-wrapper filtering) -->
...
5. Pi derives pending delivery from the durable root transcript: one emitted request remains pending without duplicate reminders until an exact public call occurs, each failed exact call advances at most one reminder, and six failures emit a structured GIVEUP summary with unchanged state and re-arm conditions; fifteen later real-user prompts may re-arm memory capture. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionDue --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-002: advances retries only after exact calls and re-arms after fifteen later prompts) -->
