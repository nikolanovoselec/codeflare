...
4. Capture-file timestamps reflect the user's local timezone, resolving `USER_TIMEZONE` before `TZ` and then UTC per [REQ-MEM-010](#req-mem-010-memory-capture-hook-plumbing) AC4. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/assert-iso-ts.sh::RESOLVED --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::captureTimestamp --> <!-- @test: host/__tests__/memory-prompt-iso-ts-assertions.test.js (assert-iso-ts.sh / REQ-MEM-010 AC5+AC6+AC7) --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (prefers USER_TIMEZONE over TZ when both are present) -->
...
6. Extracted chunks merge serially and atomically into cumulative `vault-graph.json`, then into the global graph under `user_vault`, queryable in the same turn. Capture success is reported only after merge and publication; either failure retains retry state. On Pi, post-commit note and chunk artifacts plus an exact successful background-task result precede root-owned counter advancement. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-agent-prompt.md::VARS_FILE --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::memorySuccessQualifies --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::finalizeMemorySuccess --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (requires the post-commit note and chunk before exact success advances the frozen counter) -->
...
3. When a deferred capture is pending, the hook blocks parent tools and unrelated children after spawn as well as before it; only the exact child that claims the one pending session-local capture authorization may use tools. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::AUTH_FILE --> <!-- @test: host/__tests__/memory-capture-block.test.js (memory-capture-block.sh - child-correlated authorization / REQ-MEM-012 AC3+AC4) -->
4. One memory-capture invocation may create the authorization. Replay, timeout, missing child identity, and a different child fail closed; correlation identifiers remain in the ephemeral session directory and are never emitted. The block clears when the carrier is removed. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::AUTH_TTL_SEC --> <!-- @test: host/__tests__/memory-capture-block.test.js (rejects missing child correlation and expired unclaimed authorization) -->
