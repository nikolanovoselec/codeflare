...
5. A graph beyond the configured size ceiling is passed over for the next candidate rather than ending the attempt, and the session sentinel is left unclaimed, so neither a large unified graph nor a temporary miss disables injection for the session. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-context-inject.sh::MAX_GRAPH_BYTES --> <!-- @test: host/__tests__/memory-context-inject.test.js (AC5: skips a graph past the ceiling without spending the session sentinel) --> <!-- @test: host/__tests__/memory-context-inject.test.js (AC5: falls back to the repo graph when the unified one is past the ceiling) --> <!-- @test: src/__tests__/lib/pi-memory-inject.test.ts (AC5: falls back to the repo graph when the unified one is past the ceiling) -->
