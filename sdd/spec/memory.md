...
6. The shutdown handler watchdog allows the final bisync up to 120s to drain pending writes before forced termination. <!-- @impl: entrypoint.sh::shutdown_handler --> <!-- coverage-gap: the 120s (108s SIGTERM + 12s SIGKILL) shutdown-watchdog budget is verified only by a source-string-matching audit (host/__audits__/entrypoint-vault.audit.js, not executed by CI); exercising a real 120s timeout / forced-termination path is impractical as a node:test unit -->
