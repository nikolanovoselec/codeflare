...
3. When triggered, Claude's background sonnet subagent prefilters the durable transcript into bounded chunks for its capture pipeline. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/prefilter-transcript.sh::is_synthetic_marker --> <!-- @test: host/__tests__/memory-capture-pipeline.test.js (prefilter-transcript.sh (REQ-MEM-001 AC3) / REQ-VAULT-002 (conversation captures land in vault as markdown)) -->
...
5. An emitted request remains pending without duplicate reminders until an exact public call occurs, and each failed exact call advances at most one of six delivery attempts. <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionTranscriptFacts --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionDue --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-015 AC5: advances retries only after exact public calls) -->
6. Six failed exact calls emit one structured GIVEUP notice with unchanged committed state and job-specific re-arm guidance. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-015 AC6: emits structured GIVEUP state and re-arm guidance) -->
...
7. Pi places each memory execution snapshot in the shared home-backed cache before exposing its public background `subagent` request, so child Bash can read the immutable input. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::memoryExecutionVarsPath --> <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (creates work on the fifteenth real prompt and emits a visible reminder without private spawn) -->
...
- Active legacy temp snapshots migrate to the shared cache before retry; `migrates an active legacy temp snapshot to the shared path before delivery` provides supplemental behavioral coverage.
