...
**Intent:** Pi extraction must read durable root-session input, invoke launch requests through Pi's exact public tool pipeline, and stay inert inside child subagent sessions so work survives reloads without polluting child transcripts.

**Applies To:** User

**Acceptance Criteria:**

1. Capture reads each runtime's durable resume transcript, never volatile memory, so reload and resume retain full history; an empty resolved transcript skips capture instead of writing a placeholder. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (creates work on the fifteenth real prompt and invokes the exact public tool request) -->
...
4. Every due capture/extract wave persists bounded delivery metadata whose request objects equal the arguments invoked together through Pi's exact public tool pipeline. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-015 AC4: invokes memory and Vault requests together while persisting bounded metadata) -->
5. Exact extraction dispatch does not trigger a provider turn. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (creates work on the fifteenth real prompt and invokes the exact public tool request) -->
...
7. Pi invokes each capture/extract launch as a public background `subagent` request with inherited context disabled through the normal tool transcript, so no private service spawn or model-chosen dispatch is required. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (creates work on the fifteenth real prompt and invokes the exact public tool request) -->
