...
7. Six failed exact calls emit one structured GIVEUP notice with unchanged committed state and job-specific re-arm guidance. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::sendDueExtractionMessages --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-015 AC7: emits structured GIVEUP state and re-arm guidance) -->
