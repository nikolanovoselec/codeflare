...
5. Successful managed mode-change reconciliation preserves preference fields changed concurrently during reconciliation. <!-- @impl: src/routes/preferences.ts::default --> <!-- @test: src/__tests__/routes/preferences.test.ts (REQ-MEM-011 AC5: target journaling preserves concurrent preference changes) -->
6. A managed mode change does not publish applied state after its selected mode or managed state changes concurrently. <!-- @impl: src/routes/preferences.ts::default --> <!-- @test: src/__tests__/routes/preferences.test.ts (REQ-MEM-011 AC6: aborts stale reconciliation without reverting a concurrent mode update) -->
