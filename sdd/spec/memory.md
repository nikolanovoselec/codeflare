...
- Claude: the capture subagent retains its existing tools and scratchpad pipeline; its retry carrier remains until both locked publication operations succeed, then is removed.
- Pi: the capture subagent uses Bash-only evidence transport with medium reasoning and a finite public turn ceiling; the root-owned counter advances only after exact success with post-commit note and chunk artifacts.
