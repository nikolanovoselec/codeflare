...
3. While a request is outstanding the hook relaunches it once per user prompt and counts each launch, instead of arming a second request; a prompt arriving while a capture is still running spends no attempt. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture.sh::MAX_ATTEMPTS --> <!-- @test: host/__tests__/memory-capture-hook.test.js (re-delivers an outstanding request on the next prompt instead of dropping it) --> <!-- @test: host/__tests__/memory-capture-hook.test.js (an armed request suppresses a second arm without closing the window) -->
