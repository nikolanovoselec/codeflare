...
5. Pi derives pending delivery from the durable root transcript: one emitted request remains pending without duplicate reminders until an exact public call occurs, each failed exact call advances at most one reminder, six failed exact calls produce GIVEUP, and fifteen later real-user prompts may re-arm a new request. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionDue --> <!-- @test: src/__tests__/lib/pi-memory-vault-delivery.test.ts (REQ-MEM-002: advances retries only after exact calls and re-arms after fifteen later prompts) -->
