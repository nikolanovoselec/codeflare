...
<!-- @test: src/__tests__/lib/agent-seed-manifest.test.ts (Pi memory-vault behavioral tests -> REQ-MEM-001 captureTimestamp AC4, compactMessages AC3, flock global merge AC7; parseSessionMessages durable transcript source + memory-vault.ts reads getSessionFile with skip-empty guard -> AC8) -->
...
8. The capture sources the conversation from the durable on-disk session transcript that each runtime already persists for session resume (Claude reads the transcript JSONL by line range; Pi reads its persisted session file via `getSessionFile()`), never from a volatile in-memory buffer. A capture triggered immediately after a reload or resume therefore sees the full conversation; if the resolved transcript is empty the capture is skipped rather than writing a placeholder "no substantive content" note.
