...
3. Deferred capture normally authorizes the exact correlated capture child and blocks parent and unrelated-child tools. If a recorded parent spawn remains unclaimed for 120 seconds because a harness correlation signal is missing, the gate fails open; blocked attempts append only harness metadata and authorization state to a session-local diagnostic log. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::AUTH_BREAKER_SEC --> <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::BLOCK_LOG --> <!-- @impl: entrypoint.sh::SETTINGS_CONFIG --> <!-- @test: host/__tests__/memory-capture-block.test.js (memory-capture-block.sh - child authorization breaker / REQ-MEM-012 AC3+AC4) --> <!-- @test: host/__tests__/entrypoint-hooks-merge.test.js (advanced mode registers each managed hook on its own event type) -->
4. Parent-spawn authorization is single-use, ephemeral, and fail-closed for replay and concurrency; the breaker is unavailable when no spawn authorization exists, stale authorization expires after 600 seconds, and successful publication removes the carrier. <!-- @impl: preseed/agents/claude/plugins/codeflare-memory/scripts/memory-capture-block.sh::AUTH_TTL_SEC --> <!-- @test: host/__tests__/memory-capture-block.test.js (allows only one of two concurrent parent spawn identities) --> <!-- @test: host/__tests__/memory-capture-block.test.js (memory-capture-block.sh - no bypass / REQ-MEM-012 AC4 stop-hook) -->

**Constraints:**

- The block applies only in advanced session mode.
- A carrier with no recorded capture spawn remains hard-blocked with no bypass file. The 120-second breaker applies only after the hook has recorded a capture launch.
