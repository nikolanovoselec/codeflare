...
5. The boot seed publishes the cumulative vault graph under the `user_vault` tag, never the empty scaffold graph the graph builder leaves alongside it. <!-- @impl: entrypoint.sh::init_user_vault --> <!-- @test: host/__tests__/entrypoint-vault-boot.test.js (publishes vault-graph.json under the user_vault tag, never the empty scaffold (REQ-MEM-009)) -->
...
- The graph builder leaves an empty `graph.json` scaffold in the vault's `graphify-out/`; the cumulative `vault-graph.json` beside it is the only valid publication source, so a path-only existence guard is not sufficient to decide whether there is anything to publish.
