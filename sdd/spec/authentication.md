...
4. The secret source varies by deployment mode but is unified under a single shared secret name at runtime. <!-- @impl: src/lib/access.ts::getUserFromRequest --> <!-- coverage-gap: no test asserts the per-mode secret source unified under one runtime secret name; existing tests cover AC1/2/3/5 only -->
...
3. The frontend detects the pending state and redirects the user to the subscription page. <!-- @impl: web-ui/src/App.tsx --> <!-- coverage-gap: no automated test covers the frontend pending->subscribe redirect in App.tsx -->
4. After subscription (self-service or admin approval), the user record is updated with an active tier. <!-- @impl: src/lib/user-record.ts::updateUserRecord --> <!-- coverage-gap: no automated test covers the post-subscription active-tier update -->
5. First-time active users are redirected to onboarding for guided setup. <!-- @impl: web-ui/src/App.tsx --> <!-- coverage-gap: no automated test covers the first-time active-user onboarding redirect -->
...
3. The refresh is transparent to the user (no redirect, no re-authentication). <!-- @impl: src/index.ts --> <!-- coverage-gap: tests assert the refresh threshold and TTL; none asserts the transparent no-redirect/no-re-auth property -->
...
2. Mobile renders as bottom sheet. <!-- @impl: web-ui/src/components/Header.tsx::Header --> <!-- coverage-gap: Header tests cover desktop dropdown and mobile no-hamburger behavior; none asserts the dropdown renders as a mobile bottom sheet -->
