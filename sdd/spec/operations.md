...
1. Zoxide, yazi, lazygit, and SilverBullet each have a parallel release-check job. SilverBullet's job atomically updates the server binary version, authoritative release-asset digest, and vendored native service worker so its client and server versions cannot drift. <!-- @impl: .github/workflows/bump-shadow-pins.yml::zoxide --> <!-- @impl: .github/workflows/bump-shadow-pins.yml::yazi --> <!-- @impl: .github/workflows/bump-shadow-pins.yml::lazygit --> <!-- @impl: .github/workflows/bump-shadow-pins.yml::silverbullet --> <!-- @impl: scripts/ci/update-silverbullet-pins.mjs::updateSilverBulletPins --> <!-- @test: src/__tests__/ci/suite-gates.test.ts (REQ-OPS-020: SilverBullet coupled-pin automation) -->
...
2. The bump derives the packaged code-server commit and embedded Code version from the immutable release artifact, verifies its package/product identities agree, and derives the embedded Code source commit from the immutable release gitlink. <!-- @impl: .github/workflows/bump-shadow-pins.yml::code-server --> <!-- @impl: scripts/ci/update-code-server-pins.mjs::updateCodeServerPins --> <!-- @test: src/__tests__/ci/suite-gates.test.ts (routes code-server bumps through one dedicated fail-closed updater) --> <!-- @test: src/__tests__/ci/suite-gates.test.ts (updates every coupled runtime pin and invalidates the checksum atomically) -->
