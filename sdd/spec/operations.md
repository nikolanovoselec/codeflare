...
1. Shared non-agent npm tools remain installed for every valid selection, while exact-pinned image-owned Oxlint remains available without joining managed runtime compatibility. <!-- @impl: scripts/ci/coding-agent-selection.mjs::selectedNpmManifest --> <!-- @impl: Dockerfile::/opt/codeflare/oxlint --> <!-- @test: host/__tests__/coding-agent-selection.test.js (derives an npm manifest containing only selected coding agents plus shared tools) --> <!-- @test: host/__tests__/dockerfile-dependency-integrity.test.js (image-owned Oxlint has an exact pin and complete committed integrity tree) -->
