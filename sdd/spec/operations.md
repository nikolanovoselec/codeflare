...
- Lanes run in parallel, each gated by a path filter over the diff; all lanes run on manual dispatch.
- If GitHub cannot supply the PR-files diff, exact checked-out commits are validated locally and every lane runs. <!-- @impl: scripts/ci/path-filter-fallback.sh --> <!-- @test: host/__tests__/nightly-pr-checks-routing.test.js (REQ-OPS-003: executes the fallback against exact commits and emits every lane) -->
- The `summary` job fails on any failed or cancelled lane while passing skipped unaffected lanes.
