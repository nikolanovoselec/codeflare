...
4. The workflow runs both backend and frontend test suites; the backend step preserves the test process exit status, so any assertion, collection, runtime, worker-pool, or incomplete-suite error fails the check even when other assertions passed. Node-compatible unit files run outside `workerd`; the five WorkerEntrypoint/WebSocket/HTMLRewriter files each run in a fresh Workers-runtime process, and the sequence stops on the first failure. <!-- @impl: .github/workflows/test.yml::Run backend tests --> <!-- @impl: scripts/run-backend-tests.sh::run_backend_tests --> <!-- @impl: scripts/run-workers-runtime-tests.mjs::runWorkersRuntimeTests --> <!-- @impl: vitest.config.ts::test --> <!-- @impl: vitest.workers.config.ts::test --> <!-- @test: host/__tests__/backend-test-launcher.test.js (backend test launcher preserves a crashed suite nonzero exit despite pass-looking output) --> <!-- @test: host/__tests__/backend-test-launcher.test.js (Workers-runtime runner stops at the first failed isolated process) -->
