...
1. The idle-detection layer fails safe in the direction of preserving user work, not minimizing compute. When the configured idle timeout cannot be resolved (storage corrupted, schema-validated value missing, parser fed garbage, code path skipped the user-pref resolution), the system falls back to the maximum supported value (4h) rather than the minimum. <!-- @impl: src/container/container-metrics.ts::parseSleepAfterMs = SLEEP_AFTER_FALLBACK_MS = 14_400_000 --> <!-- @test: src/__tests__/lib/sleep-timer-defaults.test.ts (fail-safe to 4h on corruption) -->
