...
5. A fallback on a `ready` bucket starts a one-time `mixed-recovery` scan only without a healthy container. Otherwise the bucket stays ready until a later fallback finds it quiescent. Recovery changes neither regime nor generation. <!-- @impl: src/lib/r2-migration.ts::markMixedRecovery --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (fetchObjectWithRegimeFallback (D2 reads stay up)) -->
