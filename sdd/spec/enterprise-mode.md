...
- The `EGRESS` binding is the Workers VPC `[[vpc_networks]]` Fetcher (`binding="EGRESS"`, `network_id="cf1:network"`, `remote=true`), committed **commented-out** in `wrangler.toml` until Cloudflare Mesh is provisioned (the account has none yet); uncommenting it after provisioning is the operator's deploy-time enable step, which is why `env.EGRESS` is undefined at runtime today and the feature stays dormant (fail-closed).
