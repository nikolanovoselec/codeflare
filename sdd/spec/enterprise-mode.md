...
2. When `ENTERPRISE_MODE` is unset, no interception work is performed and the container start path is byte-identical to current behavior. <!-- @impl: src/container/index.ts::startAndWaitForPorts --> <!-- @test: src/__tests__/container/index.test.ts (enterprise LLM interception wiring (REQ-ENTERPRISE-011)) -->
...
4. When the toggle is ON, the container DO wires the catch-all `interceptOutboundHttps('*', EgressController)` before the container starts, lower-precedence than the per-host LLM/GitHub registrations, so every otherwise-unintercepted host routes through it; when OFF or non-enterprise the catch-all is never wired. <!-- @impl: src/container/index.ts::wireEgressInterception --> <!-- @impl: src/container/index.ts::setupEnterpriseInterception --> <!-- @test: src/__tests__/container/index.test.ts (enterprise LLM interception wiring (REQ-ENTERPRISE-011)) -->
