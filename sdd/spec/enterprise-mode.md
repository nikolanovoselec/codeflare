...
8. A provider tool block becomes executable client output and stored replay state only after a native `tool_use` terminal reason. A `max_tokens` response retains safe text, usage, and `length`, but publishes and persists no incomplete tool call. <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::adaptBedrockAnthropicResponse --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-073/076: does not publish or persist an Invoke tool call truncated by max_tokens) --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-073/076: terminates Eventstream as length without publishing partial tool JSON) -->
9. A tool proposal with no result may be omitted from provider-facing history after a later nonempty text-only user turn proves it was abandoned; safe assistant prose remains. Any matching result, partial parallel result set, orphan/duplicate ambiguity, intervening assistant message, or non-text user content retains exact fail-closed replay validation. <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::omitAbandonedToolTurns --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-073/076: omits an unexecuted poisoned tool turn after a new user turn) --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-079: incomplete parallel tool results remain protected despite later assistant and user text) -->
...
4. A nonempty text-only user message starts a new turn when preceding tool-call IDs and results are completely paired without orphan or duplicate ambiguity. A preceding proposal with zero results is an abandoned, non-executed transaction and may be omitted; partial result sets remain protected. No final assistant answer is required for a completed pair. <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::classifyBedrockToolTurn --> <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::omitAbandonedToolTurns --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-073: accepts a new text question after paired interrupted tools) --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-073/076: omits an unexecuted poisoned tool turn after a new user turn) --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-079: incomplete parallel tool results remain protected despite later assistant and user text) -->
