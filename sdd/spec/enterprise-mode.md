...
3. When `ENTERPRISE_MODE` is set, every user is treated as a custom `unlimited` user for access, mode, billing, and compute quota: monthly Timekeeper quota is never enforced, and billing-status checks and trial logic are disabled. Concurrent-session admission currently follows the non-SaaS role-based best-effort resolver; [issue #880](https://github.com/nikolanovoselec/codeflare/issues/880) tracks the separately approved role-independent Enterprise limit. <!-- @impl: src/lib/subscription.ts::getEffectiveTier --> <!-- @impl: src/lib/constants.ts::getMaxSessions --> <!-- @impl: src/routes/container/lifecycle.ts::app --> <!-- @impl: src/routes/container/lifecycle-validation.ts::validateSessionAndCheckLimits --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (REQ-ENTERPRISE-001 AC3: enterprise currently applies the non-SaaS stored-user role limit) --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (REQ-ENTERPRISE-001 AC3: enterprise stored admin gets the role-based admin limit) --> <!-- @test: src/__tests__/routes/container-lifecycle-helpers.test.ts (REQ-ENTERPRISE-001 AC3: enterprise users are never blocked by the monthly compute quota) -->
