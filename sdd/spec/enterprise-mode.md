...
7. Save rejects a Dynamic Route whose name collides with a submitted native handle and derives typed policy references from the submitted native-target identities rather than from a string prefix. <!-- @impl: src/lib/admin-configuration.ts::validateConfigurationValues --> <!-- @test: src/__tests__/routes/reasoning-eligibility.test.ts (REQ-ENTERPRISE-055: rejects a Dynamic Route that collides with a submitted native handle before any routing write) -->
