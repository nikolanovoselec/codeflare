...
1. The catalog exposes the fifteen approved executable built-ins and keeps failed families as non-assignable notices. <!-- @impl: src/lib/reasoning-profiles.ts::BUILT_IN_REASONING_PROFILES --> <!-- @impl: src/lib/reasoning-profiles.ts::COMPATIBILITY_NOTICES --> <!-- @test: src/__tests__/lib/reasoning-profiles.test.ts (ships exactly the fifteen executable built-ins and keeps failed families as notices) -->
...
2. Provider tool-turn replay state remains confidential at rest. <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-073/077: continues an authentic %s High tool response through encrypted replay and Invoke) --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-073/077: dispatches provider-native Bedrock Invoke with exact reasoning controls and hides signed replay state) -->
...
1. Advanced native candidate listing offers the reusable Provider-default contract without asserting model capability; normal Discover automatically verifies and adopts it. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/reasoning-eligibility.test.ts (REQ-ENTERPRISE-075: offers the reusable native contract without a paid probe and preserves historical explicit confirmation) -->
2. Listing that template performs no paid request. Selecting or starting a target never probes automatically; normal Discover is the explicit test action. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) -->
3. The generic native contract requires explicit tools/replay and cache verification; administrator assertion alone cannot invent a qualifying receipt. Historical evidence-specific profiles retain explicit reconfirmation. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts -->
4. Native Runtime discovery uses at most four calls. The shared normal UI discloses the overall 40-submission/10-minute ceiling, 2,048 output tokens and 90 seconds each. Cache misses, absent counters and incomplete observations leave minimum inconclusive, not automatically unsupported; failed checks do not enable targets. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: explains an inconclusive cache check without enabling the native target) --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts -->
5. Dynamic Route profile selectors exclude provider-native Bedrock profiles. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-034: loads every detected inventory read-only and exposes models in the selected route outside advanced disclosures) -->
6. Older, current, and synthetic future models use the same reusable contract when compatible; saved explicit transports remain authoritative. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: selects older/current/synthetic future models with one reusable native profile and explicit verification) -->
7. Previously selectable provider-native Dynamic Route authority is removed from the draft until an allowed profile is selected. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: removes hidden provider-native authority from a Dynamic Route draft until an allowed profile is selected) -->
8. Normal native discovery is Bedrock-only, based on the observed adapter. One Discover action verifies/adopts the shared contract and receipt without changing saved transport. Other native protocols retain their advanced flow until deliberately extended. <!-- @impl: src/routes/admin/ai-capability-discovery.ts::routes --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/routes/target-capability-discovery.test.ts --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx -->

**Constraints:** Administration performs no provider fallback or paid retry.
...
5. Adapter identity advances for semantic authority changes: v2 added checkpoints, v3 validated streaming continuations, and v4 requires authentic active replay regardless of reasoning plus exact target-authority isolation. Old v1/v2/v3 documents remain readable but do not authorize v4. Upgrade uses explicit confirmation of retained evidence or generic verification; unrelated unchanged stale profiles may remain disabled. No receipt is fabricated or silently upgraded. <!-- @impl: src/lib/native-ai-targets.ts::nativeVerificationMatches --> <!-- @test: src/__tests__/lib/native-ai-targets.test.ts --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts -->
...
7. Generic native prefix serialization requires target-bound positive provider reads. Gateway HIT satisfies minimum qualification separately but never grants explicit native checkpoint authority. Missing historical capability state opts out, and explicit empty state revokes it. <!-- @impl: src/lib/native-ai-targets.ts::nativePromptCacheSupported --> <!-- @test: src/__tests__/lib/enterprise-route-config.test.ts --> <!-- @test: scripts/verify-bedrock-pi-prompt-cache.mjs -->
8. Selected Dynamic Bedrock verification evaluates tools/replay and cache on the exercised path only. Known conflicting backend observations cannot be combined. A miss, absent counters, or incomplete probe is inconclusive; Native serialization is never copied onto Dynamic. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts -->

**Constraints:** Cache eligibility and actual hits remain provider decisions. Valid stream framing is not proof of incremental delivery through a Gateway with response inspection. Existing reasoning mappings, saved transport identities, and Dynamic Route routing remain unchanged; continuation dispatch is governed separately by REQ-ENTERPRISE-077.
