...
5. Uncertain desired, applied, or policy state returns managed-update-pending without a user-object R2 request and explains that uploads and deletions remain blocked until the update finishes. <!-- @impl: src/lib/managed-storage-guard.ts::guardManagedStorageMutation --> <!-- @test: src/__tests__/lib/managed-storage-guard.test.ts (fails update-pending before policy lookup on %s mismatch) --> <!-- @test: src/__tests__/lib/managed-storage-guard.test.ts (explains why uploads and deletions are blocked during a managed update) -->
