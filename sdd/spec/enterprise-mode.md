...
4. Authentication, quota, server, transport, malformed-stream or unexpected buffered-envelope failures terminate further scan requests. Buffered-envelope rejection retains its HTTP status and transport rather than claiming a connection error. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (stops the whole candidate scan after HTTP %s) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (suppresses earlier matches when a later candidate encounters a fatal failure) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (stops all candidates after malformed SSE without retaining the malformed content) -->
...
- Discover and Advanced verification share one visible Check result above a single Advanced: choose a profile disclosure. Technical evidence starts collapsed; the verdict and Review changes → Confirm Save → next normal session guidance remain visible. Non-Bedrock Native profile controls start expanded. <!-- @impl: web-ui/src/components/admin/TargetCapabilityDiscovery.tsx::TargetCheckResult --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (REQ-ENTERPRISE-041: Discover leaves one visible result and the review/save next step outside optional profile controls) --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (REQ-ENTERPRISE-038: Advanced verification updates the same visible result without requiring the disclosure to stay open) --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (REQ-ENTERPRISE-041: non-Bedrock Native keeps its consolidated Advanced controls open and publishes verification outside them) --> <!-- @manual: Check single-column mobile fields, wrapped model identifiers, visible result/next step, and 44px action targets without horizontal overflow. -->
...
3. Native receipts require contract-appropriate evidence: observed tools/replay and cache for generic contracts, or explicit reconfirmation for retained evidence-specific profiles. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (upgrades one real old profile reference via explicit confirmation while retaining another disabled stale target) -->
4. Without qualifying Gateway HIT or positive provider-prefix reads, missing or incomplete cache evidence yields an inconclusive cache minimum, not proof of unsupported caching. Native failure parsing retains only validated diagnostic fields and the sanitized explanation, never raw provider bodies, signed replay or a successful receipt. <!-- @impl: web-ui/src/api/client.ts::checkNativeTarget --> <!-- @test: web-ui/src/__tests__/api/client.test.ts (REQ-ENTERPRISE-075: retains sanitized Native cache-refusal evidence through API parsing) --> <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (does not confuse absent cache observations with proof that caching is unsupported) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: explains an inconclusive cache check without enabling the native target) -->
...
- Advanced listing offers the reusable Provider-default template without certifying model capability.
- Native Runtime discovery uses at most four calls. <!-- @impl: src/lib/ai-capability-discovery/index.ts::discoverTargetCapabilities --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts (verifies an unlisted native identifier, restores authentic replay, then measures prefix reads) -->
...
- Discovery does not persist activation; failed checks never enable targets.
...
2. Opening a target row exposes its target fields, current check result and profile actions under one Advanced disclosure. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-051/056/064: renders named native target rows with expanded-only controls) -->
3. Targets without current proof show orange Not ready status; ready targets distinguish green Live-verified from Administrator-confirmed without a fresh check. <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (REQ-ENTERPRISE-043: saved Native %s authority has a distinct visible basis without a fresh check) --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-056: derives orange and green native readiness without an enable control) -->
...
5. Provider-default routes and generated single-mapping contracts offer all seven Pi thinking choices without expanding canonical backend mappings, while retaining administrator context; provider-default routes retain a 16,384-token output cap. <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig --> <!-- @test: src/__tests__/lib/enterprise-route-config.test.ts (REQ-ENTERPRISE-035/058: publishes seven Pi choices without inventing seven discovered reasoning mappings) --> <!-- @impl: entrypoint.sh::ENTERPRISE_ROUTE_CATALOG --> <!-- @test: host/__tests__/entrypoint-enterprise-pi-models.test.js (REQ-ENTERPRISE-058: emits honest Pi metadata for a provider-default native model) --> <!-- @test: preseed/agents/pi/test/enterprise-routing.test.mjs (REQ-ENTERPRISE-058: provider-default Dynamic routes expose and preserve all seven Pi thinking choices) --> <!-- @test: preseed/agents/pi/test/enterprise-routing.test.mjs (REQ-ENTERPRISE-058: the real provider-default thinking selector renders and selects all seven choices) -->
...
1. Pi `cacheControlFormat: "anthropic"` is published only for authorized native Runtime handles with retained validated evidence or target-bound positive provider-prefix reads. <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig --> <!-- @impl: src/lib/native-ai-targets.ts::nativePromptCacheSupported --> <!-- @test: src/__tests__/lib/enterprise-route-config.test.ts (REQ-ENTERPRISE-074: publishes synthetic future model capabilities only from qualifying %s evidence) --> <!-- @test: host/__tests__/entrypoint-enterprise-pi-models.test.js (REQ-ENTERPRISE-083: enables Pi checkpoints only for the authorized native Runtime handle) -->
...
- Reasoning length or token evidence may establish enabled reasoning without exposing content. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (grades reasoning content without token counters as $reasoning) -->
