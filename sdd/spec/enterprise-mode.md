...
- Existing sessions are grandfathered for wizard deactivation while their CLI remains build-installed; a later image that omits that CLI rejects start rather than launching a missing command.
...
**Intent:** Each deployment shows only applicable frontend controls: billing and consumption are SaaS-only, enterprise hides incompatible self-service and user-administration surfaces, and the signed-in identity remains visible even when its menu has no available actions.
...
1. Existing setup configure saves chip-list access groups and routes without a new endpoint. Names are trimmed, 1–256 characters, allow spaces, and reject commas, carriage returns, or line feeds; groups use lossless joined storage and prefill returns the trimmed values. <!-- @impl: src/routes/setup/index.ts::ConfigureBodySchema --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (rejects a group containing comma/carriage return/line feed before setup starts; round-trips trimmed access-group values through prefill) -->
...
5. When the toggle is ON, the container DO registers a catch-all HTTPS interceptor before the container starts, lower-precedence than the per-host LLM/GitHub registrations, so every otherwise-unintercepted host routes through it; when OFF or non-enterprise the catch-all is never wired. <!-- @impl: src/container/container-interception.ts::strictEgress --> <!-- @impl: src/container/container-interception.ts::wireContainerInterception --> <!-- @test: src/__tests__/container/enterprise-llm.test.ts (enterprise LLM interception wiring (REQ-ENTERPRISE-011)) -->
6. When the toggle is OFF or the deployment is non-enterprise, no catch-all is wired, the GitHub credential interceptor transport is unchanged, no toggle KV read/write occurs outside enterprise, and the container egress path plus the configure request/response shape are byte-identical to today. <!-- @impl: src/lib/controller-egress.ts::hasStrictGatewayEgress --> <!-- @impl: src/container/container-interception.ts::wireContainerInterception --> <!-- @test: src/__tests__/container/index.test.ts (container DO class / REQ-SESSION-002 (one container per session)) -->
...
2. Only this deployment's account-scoped R2 endpoint and Cloudflare API account path bypass the strict-egress binding; every other account uses the binding. <!-- @impl: src/lib/controller-egress.ts::isAccountScopedDestination --> <!-- @impl: src/lib/controller-egress.ts::isOwnAccountR2 --> <!-- @impl: src/lib/controller-egress.ts::controllerFetch --> <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @impl: src/lib/r2-client.ts::createR2Client --> <!-- @impl: src/container/container-interception.ts::strictEgress --> <!-- @test: src/__tests__/lib/controller-egress.test.ts (REQ-ENTERPRISE-016 / AD86: isAccountScopedDestination (own account only)) --> <!-- @test: src/__tests__/egress-controller.test.ts (REQ-ENTERPRISE-016 / AD86: EgressController account-scoped exemption (own account direct, all else Gateway) / REQ-ENTERPRISE-023) -->
...
5. A fallback hit on a `ready` bucket opportunistically starts a one-time `mixed-recovery` scan only when no healthy container is active. A hit while a healthy container exists leaves the bucket ready and defers recovery until a later fallback observes a quiescent bucket; recovery does not change regime or generation. <!-- @impl: src/lib/r2-migration.ts::markMixedRecovery --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (fetchObjectWithRegimeFallback (D2 reads stay up)) -->
...
5. When the toggle is OFF or the deployment is non-enterprise, no KV read occurs outside enterprise and the storage download path and the configure request/response shape are byte-identical to today; a transient policy KV read failure defaults OFF, keeps downloads available, and reports `downloadsDisabled: false`. <!-- @impl: src/lib/downloads-policy.ts::isDownloadsDisabled --> <!-- @test: src/__tests__/routes/storage-download.test.ts (REQ-ENTERPRISE-019 AC5: keeps downloads available when the enterprise policy KV read rejects) --> <!-- @test: src/__tests__/routes/user-profile.test.ts (REQ-ENTERPRISE-019 AC5: reports downloadsDisabled false when the policy KV read rejects) -->
