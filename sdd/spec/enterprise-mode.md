...
3. A streamed success terminator requires message stop and clean upstream EOF; generic stream repair never converts a native error into success. <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::adaptBedrockAnthropicResponse --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-080: rejects trailing corruption before emitting a successful stream terminator) --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-080: preserves native stream errors without adding a success terminator) -->
