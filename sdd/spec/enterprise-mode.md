...
1. Native candidate discovery offers the reusable Provider-default contract without asserting model capability. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/reasoning-eligibility.test.ts (REQ-ENTERPRISE-075: offers the reusable native contract without a paid probe and preserves historical explicit confirmation) -->
2. Offering that template performs no paid request; selecting or starting a target never probes automatically. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) -->
...
4. Generic Verify discloses at most four native calls, 2,048 output tokens and 90 seconds each, the long public cache prefix, and provider-dependent cost. A miss or absent counters leaves minimum qualification inconclusive, not automatically unsupported; failed checks do not enable the target. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: explains an inconclusive cache check without enabling the native target) --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts -->
