...
4. Authentication, quota, server, transport, or malformed-stream failures terminate further scan requests; HTTP 429 alone permits recommendations from previously completed candidates. <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (retains complete Gemma and GLM matches when a later candidate is rate limited) --> <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (stops the whole candidate scan after HTTP %s) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (suppresses earlier matches when a later candidate encounters a fatal failure) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (stops all candidates after malformed SSE without retaining the malformed content) -->
...
- Rate limiting never triggers automatic retries or custom-draft creation; completed matches remain explicit selections requiring Verify before activation. Other fatal failures suppress recommendations and drafts.
