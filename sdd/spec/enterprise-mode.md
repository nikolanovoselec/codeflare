...
4. An absent global default uses an empty route with Off reasoning; authoritative validation rejects another reasoning level without a route. <!-- @impl: src/lib/admin-configuration.ts::validateConfigurationValues --> <!-- @test: src/__tests__/routes/admin-configuration-preview.test.ts (REQ-ENTERPRISE-039: rejects reasoning without a global default route) -->
