...
4. When `ENTERPRISE_MODE=active`, Pi is configured with a custom provider entry pointing at `api.openai.com` — using the **Responses API** adapter (`api: "openai-responses"`) so reasoning models (gpt-5.5) accept function tools together with reasoning, a combination `/v1/chat/completions` rejects — with the same placeholder credential and the same slash-free handle, and its default provider and model are pinned so it reaches the gateway without any manual login step.
