...
3. When `ENTERPRISE_MODE` is set, billing-status checks, trial logic, and monthly Timekeeper quota enforcement are disabled. <!-- @impl: src/lib/subscription.ts::getEffectiveTier --> <!-- @test: src/__tests__/lib/enterprise-mode.test.ts (REQ-ENTERPRISE-001 AC3: disables billing-status downgrade under enterprise) --> <!-- @test: src/__tests__/routes/container-lifecycle-helpers.test.ts (REQ-ENTERPRISE-001 AC3: enterprise users are never blocked by the monthly compute quota) -->
...
8. Concurrent-session admission uses the non-SaaS stored-role resolver and remains best effort. <!-- @impl: src/lib/constants.ts::getMaxSessions --> <!-- @impl: src/routes/container/lifecycle-validation.ts::validateSessionAndCheckLimits --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (REQ-ENTERPRISE-001 AC8: enterprise currently applies the non-SaaS stored-user role limit) --> <!-- @test: src/__tests__/routes/container-lifecycle.test.ts (REQ-ENTERPRISE-001 AC8: enterprise stored admin gets the role-based admin limit) -->
...
- [Issue #880](https://github.com/nikolanovoselec/codeflare/issues/880) tracks the approved future role-independent Enterprise limit.
