...
1. When `ENTERPRISE_MODE` is set, the subscription/billing settings surfaces (tier display, plan switching, usage-quota controls) are hidden in the frontend. <!-- @impl: web-ui/src/components/Header.tsx::Header --> <!-- @impl: web-ui/src/components --> <!-- @test: web-ui/src/__tests__/components/Header.test.tsx (Header Component / REQ-VAULT-012 (vault button render and readiness gating) / REQ-AUTH-016 (header user dropdown)) -->
...
2. When `ENTERPRISE_MODE` is set, session creation rejects any agent type outside the enterprise allowlist. <!-- @impl: src/lib/agent-allowlist.ts::allowedAgents --> <!-- @impl: src/routes/session/crud.ts --> <!-- @test: src/__tests__/routes/session-agent-allowlist.test.ts (REQ-ENTERPRISE-003: Agent allowlist at session creation) -->
...
2. The AI Gateway URL and token are resolved entirely Worker-side — wizard KV first, deploy-secret env (`AIG_GATEWAY_URL`/`AIG_TOKEN`) fallback ([REQ-ENTERPRISE-017](#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard), `getAigConfig`) — and handed to the interceptor via per-session DO props; the interceptor reads its own env only as a fallback when a prop is absent. They are never sent to or readable from the container; there is no public Worker route carrying LLM traffic, so the path never traverses Cloudflare Access. The account id (URL path) and gateway id (`cf-aig-gateway-id` header) are parsed from the resolved gateway URL. <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-017: AI Gateway URL/token resolved from props (wizard) with env fallback) -->
...
- AI Gateway is a platform-native Cloudflare primitive (`api.cloudflare.com` REST / `gateway.ai.cloudflare.com` compat): the interceptor's upstream `fetch` ALWAYS egresses direct and never traverses the Strict Gateway Egress `cf1:network` path, independent of the toggle ([REQ-ENTERPRISE-016](#req-enterprise-016-strict-gateway-egress), [AD86](../../documentation/decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)). Routing, header stamping, and the 404 compat fallback are unaffected by the toggle.
...
7. The enterprise model picker is **routes-only** and the container carries no unused secrets. `buildEnvVars` drops `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` in **all** modes — they had no consumer (rclone reads its creds from rclone.conf built from `R2_*`) and their presence surfaced Pi's built-in `amazon-bedrock` provider in `/model`. In enterprise the Connect-to-Cloudflare deploy token never enters the container; the only `CLOUDFLARE_API_TOKEN` it may carry is the non-secret browser-run placeholder ([REQ-BROWSER-008](browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), and `CLOUDFLARE_ACCOUNT_ID` carries the wizard-configured Browser Rendering account (non-secret); `R2_*` are still emitted. The entrypoint also clears `~/.pi/agent/auth.json` to `{}` each enterprise start so no built-in provider (e.g. the seeded `openai-codex`) stays authenticated; combined, Pi lists only the `codeflare-gateway` dynamic routes. Non-enterprise keeps the real Connect-to-Cloudflare creds + a populated auth.json (byte-identical) and also no longer carries `AWS_*`. <!-- @impl: src/container/container-env.ts::buildEnvVars --> <!-- @impl: entrypoint.sh --> <!-- @test: src/__tests__/container/container-env-llm.test.ts (container secret hygiene: no AWS_* anywhere, CF token placeholder-only in enterprise) -->
...
1. `AIG_GATEWAY_URL` and `AIG_TOKEN` may be configured as Worker secrets so they are not stored in plaintext config or exposed to the container. As of [REQ-ENTERPRISE-017](#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard) these deploy secrets are an **OPTIONAL fallback**: the AI Gateway URL + token are normally configured in the setup wizard and persisted in KV, and the deploy secrets are used only when the wizard values are unset (`getAigConfig` resolves KV first, env second). When set, `AIG_TOKEN` must carry **both** the Workers AI permission (required for the REST API path, `Authorization: Bearer`) and the AI Gateway Run permission (required for the compat fallback path, `cf-aig-authorization: Bearer`); a token missing either scope is rejected with `error 10000` on the corresponding transport. <!-- @impl: .github/workflows/deploy.yml --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-004: compat fallback on REST 404 (dual transport — AD74 amendment)) --> <!-- coverage-gap: AIG_GATEWAY_URL/AIG_TOKEN are deploy-time Worker secrets; the dual-scope token requirement (error 10000) is enforced by the upstream Cloudflare API, not our code — verified operationally, not unit-testable -->
...
4. When `ENTERPRISE_MODE` is enabled, the interceptor fails closed (503) if the resolved AI Gateway URL (wizard KV or deploy-secret fallback — `getAigConfig`, [REQ-ENTERPRISE-017](#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)) is missing or unparseable (no `/v1/{account_id}/{gateway_id}` segments), rather than silently routing to nowhere, and the DO logs a warning when it skips interception wiring. <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-017: AI Gateway URL/token resolved from props (wizard) with env fallback) --> <!-- @impl: src/container/index.ts::wireLlmInterception --> <!-- coverage-gap: the DO warn-on-skip-wiring log (container/index.ts wireLlmInterception, AI Gateway URL unset) is a diagnostic, verified operationally not by unit test -->
...
- The resolved AI Gateway URL (wizard KV first, deploy secret fallback — `getAigConfig`, [REQ-ENTERPRISE-017](#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)) is the single source for the gateway coordinates: the interceptor parses the account id and gateway id from it for the REST API call ([REQ-ENTERPRISE-004](#req-enterprise-004-outbound-interception-llm-routing-to-customer-ai-gateway) AC2), so no separate account-id binding is required.
...
1. When `ENTERPRISE_MODE` is set, the user-management routes (`GET`/`PUT`/`DELETE`/`PATCH` under `/api/users`) return 403 and perform no mutation; user administration is delegated entirely to Cloudflare Access. <!-- @impl: src/routes/users.ts --> <!-- @test: src/__tests__/routes/enterprise-route-hardening.test.ts (REQ-ENTERPRISE-009 AC1: /api/users fails closed in enterprise mode) -->
2. When `ENTERPRISE_MODE` is set, the billing action routes (`POST /api/billing/checkout`, `/api/billing/portal`, `/api/billing/switch`) return 403, and `GET /api/billing/status` returns an empty/disabled billing state without contacting Stripe. <!-- @impl: src/routes/billing.ts --> <!-- @test: src/__tests__/routes/enterprise-route-hardening.test.ts (REQ-ENTERPRISE-009 AC2: /api/billing is disabled in enterprise mode) -->
3. When `ENTERPRISE_MODE` is set, the self-serve routes `POST /api/auth/subscribe` and `POST /api/auth/request-access` return 403 and send no email. <!-- @impl: src/routes/auth.ts --> <!-- @test: src/__tests__/routes/enterprise-route-hardening.test.ts (REQ-ENTERPRISE-009 AC3: self-serve auth routes 403 with no email) -->
4. When `ENTERPRISE_MODE` is set, the Stripe webhook route acknowledges the event without mutating any user's tier or billing state, so a late or stray Stripe event cannot downgrade an enterprise user. <!-- @impl: src/routes/stripe-webhook.ts --> <!-- @test: src/__tests__/routes/enterprise-route-hardening.test.ts (REQ-ENTERPRISE-009 AC4: Stripe webhook is a no-op in enterprise mode) -->
5. When `ENTERPRISE_MODE` is set, the admin tier/subscription configuration routes return 403 (there is a single effective tier, `unlimited`, for all users). <!-- @impl: src/routes/admin/tiers.ts --> <!-- @test: src/__tests__/routes/enterprise-route-hardening.test.ts (REQ-ENTERPRISE-009 AC5: admin tier config routes 403 in enterprise mode) -->
...
1. The setup wizard edits the access-group list and the dynamic-route catalog as add/Enter/chip-with-x lists (cloning the admin-user interaction), batch-saved through the existing `POST /api/setup/configure` (no new endpoint). Names validate 1–256 chars, trimmed, comma/newline forbidden (so a name with spaces like `Agentic AI Test Account` is allowed); access groups persist comma/newline-joined under `SETUP_KEYS.ENTERPRISE_ACCESS_GROUP`, split back via `parseAccessGroups`. <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @test: src/__tests__/routes/setup.test.ts (POST /api/setup/configure) -->
2. The route catalog persists as a JSON `string[]` (`SETUP_KEYS.DYNAMIC_ROUTES`) and one `{route, reasoning}` default (`SETUP_KEYS.DEFAULT_ROUTE`) where the first route added auto-becomes the default; `defaultRoute.route` must be a member of the catalog (rejected `400` otherwise). An unset default resolves at read time to the first catalog route with reasoning off. Optional per-group overrides persist as a JSON map under `SETUP_KEYS.GROUP_ROUTING` ([REQ-ENTERPRISE-013](#req-enterprise-013-per-group-dynamic-routing)). <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
3. `GET /api/setup/prefill` round-trips the stored groups, catalog, and default route so a setup re-run shows the current configuration; a malformed stored value degrades to empty defaults rather than failing the prefill. <!-- @impl: src/routes/setup/handlers.ts --> <!-- @impl: src/lib/kv-keys.ts::SETUP_KEYS --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (GET /prefill degrades to empty defaults when stored route JSON is malformed) -->
...
1. The wizard shows a per-route **context window** number input for each dynamic route, prefilled with `DEFAULT_ROUTE_CONTEXT_WINDOW` (`256000`), that the admin can raise (for a large-context route model) or reset to the default. Adding a route seeds its window to the default; removing a route drops its entry. The map persists as a JSON `{route: tokens}` under `SETUP_KEYS.ROUTE_CONTEXT_WINDOWS` (`setup:route_context_windows`), cleared when empty; each value validates as a positive integer (rejected `400` otherwise). It round-trips via `GET /api/setup/prefill` (a route present in the catalog but absent from the stored map is back-filled with the default on load), and is fanned to the container as `ENTERPRISE_ROUTE_CONTEXT_WINDOWS` so Pi's per-route `contextWindow` is correct ([REQ-ENTERPRISE-005](#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC4) instead of falling to Pi's built-in 128k default. <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx::ConfigureStep --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: src/routes/setup/handlers.ts --> <!-- @impl: src/lib/kv-keys.ts::SETUP_KEYS --> <!-- @test: web-ui/src/__tests__/stores/setup.test.ts (Setup Store) -->
...
3. Per-group config persists under `SETUP_KEYS.GROUP_ROUTING` as a JSON map `{ [group]: { routes, defaultRoute, reasoning } }` saved through `POST /api/setup/configure`; `configure` rejects (`400`) a group whose `defaultRoute` is not one of its `routes`, whose `routes` are not a subset of the global catalog, or whose key is not a configured Access group; an empty map is deleted rather than stored. <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: src/lib/kv-keys.ts::SETUP_KEYS --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
...
5. Admin groups persist under `SETUP_KEYS.ENTERPRISE_ADMIN_ACCESS_GROUP`, comma-joined (the format `parseAccessGroups` reads), saved through `POST /api/setup/configure`; an empty list deletes the key. They are excluded from per-group routing by construction — only `ENTERPRISE_ACCESS_GROUP` keys may carry a `GROUP_ROUTING` entry. <!-- @impl: src/lib/kv-keys.ts::SETUP_KEYS --> <!-- @impl: src/routes/setup/index.ts --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
6. The Setup wizard renders a "Cloudflare Admin Access Groups (optional)" chip field beside the user-access groups field; the email-based "Admin Users" list stays. `GET /api/setup/prefill` round-trips the stored list. Admin groups produce no per-group routing card. <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx::ConfigureStep --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: web-ui/src/__tests__/stores/setup.test.ts (Setup Store) -->
7. All reads/writes are inside the existing `isEnterpriseMode` gate; in non-enterprise modes the Setup request/response shape and admin authorization are byte-identical to before. <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
...
1. The setup wizard exposes an enterprise-gated Strict Gateway Egress toggle, default OFF, persisted to KV `SETUP_KEYS.STRICT_EGRESS` as `'active'`/`'inactive'` through the existing `POST /api/setup/configure` (no new endpoint); a non-enterprise configure never writes it, and enabling it while `env.EGRESS` is unbound is rejected pre-write (never persists a brick-the-egress `'active'`). <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx::ConfigureStep --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
2. `GET /api/setup/prefill` round-trips the toggle: a seeded `SETUP_KEYS.STRICT_EGRESS` prefills `true`, an absent key prefills `false`, and it is omitted from a non-enterprise prefill. <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (REQ-ENTERPRISE-016: strict gateway egress prefill) -->
3. The toggle is resolved by a single `hasStrictGatewayEgress(env)` gate-then-read helper = enterprise mode AND KV `SETUP_KEYS.STRICT_EGRESS === 'active'`, defaulting OFF when the key is absent or when the KV read throws (a transient error must not fault the container-start seam), and never reading KV in a non-enterprise deploy. The container DO resolves it **once** at wiring (in the constructor's `blockConcurrencyWhile`, before the container starts), stores it on `_strictEgress`, and passes it to the `EgressController` via `ctx.props.strict` and to `buildEnvVars`; the controller reads `props.strict` and **never re-reads KV per request**. <!-- @impl: src/lib/controller-egress.ts::hasStrictGatewayEgress --> <!-- @impl: src/container/index.ts (constructor resolves _strictEgress; setupEnterpriseInterception/wireEgressInterception reuse it) --> <!-- @test: src/__tests__/lib/controller-egress.test.ts (REQ-ENTERPRISE-016: hasStrictGatewayEgress) -->
...
6. When strict egress is ON, the container is started with `enableInternet=false`, so the Containers platform allows only ports 80/443 + Cloudflare DNS (the HTTP/HTTPS carried by the catch-all `EgressController` to the Gateway) and DENIES all raw TCP/UDP egress (SSH, arbitrary sockets, WireGuard/MASQUE) at the platform boundary the container cannot manipulate. When strict is OFF / non-enterprise, `enableInternet` is left at the platform default and raw egress is byte-identical to today. The flag is resolved once in the constructor alongside `_strictEgress` (AC3). <!-- @impl: src/container/index.ts (constructor: sets enableInternet=false when _strictEgress) --> <!-- @test: src/__tests__/container/index.test.ts (container DO class / REQ-SESSION-002 (one container per session)) -->

**Constraints:**

- The toggle is a global admin flag read straight from KV via `hasStrictGatewayEgress` (the `ENTERPRISE_MODE` precedent), not threaded per-session through `ContainerConfigPayload`/`setBucketName`; its KV value is written explicitly as `'active'`/`'inactive'` (never delete-on-off) so it round-trips deterministically, matching the `setup:onboarding_landing_page` boolean-toggle precedent.
...
- **Account-scoped exemption ([AD86](../../documentation/decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)):** only THIS deployment's own-account platform destinations — its R2 (`<accountId>.r2.cloudflarestorage.com` + vhost form) and its account-scoped CF API / Browser Rendering (`api.cloudflare.com/client/v4/accounts/<accountId>/...`) — egress **direct** and never traverse `cf1:network`, even when strict is ON. Any other account's R2/CF host, and an absent account id (fail-secure), ride the Gateway. They are codeflare's own control-plane backends (codeflare-managed credentials to Cloudflare-owned hosts, with their own audit trail), not the agent's external reach. The narrowing from a bare host match to an account-scoped match is the security measure: it closes the cross-account exfiltration channel while keeping the scale benefit — rclone's per-file R2 sync is the dominant container-egress volume, and routing it through `cf1:network` would couple every container's bootstrap-critical persistence to the shared, rate-limited Gateway egress path (account-wide connection/rate limits at fleet scale). The `LlmInterceptor` therefore never swaps to `env.EGRESS`; only the `GitHubInterceptor` (external) and the `EgressController`'s non-own-account path use it.
- The `EGRESS` binding is the Workers VPC `[[vpc_networks]]` Fetcher (`binding="EGRESS"`, `network_id="cf1:network"`, `remote=true`), enterprise-only: committed **commented-out** in `wrangler.toml` and injected by `deploy.yml` only when `ENTERPRISE_MODE=active` (its deploy-time enable step), so default/fork deploys and the test runtime are unaffected. On non-enterprise deploys `env.EGRESS` is undefined and the feature stays dormant (fail-closed). It carries both HTTP/HTTPS and WebSocket: the `EgressController` detects an `Upgrade: websocket` request and forwards it through the same selector, then **bridges** a fresh `WebSocketPair` to the upstream socket and returns a `101` carrying the client end — returning the upstream `101`+`webSocket` as-is does not propagate the socket back to the container ([REQ-ENTERPRISE-023](#req-enterprise-023-strict-gateway-egress-controller-transport) AC3).
...
1. `EgressController` is a transparent proxy for every host EXCEPT this account's own R2: it stamps no `Authorization`/`cf-aig-*`/identity header, preserves the caller's `authorization`/`cookie` on the request and `set-cookie` on the response, strips only hop-by-hop headers, and forwards with `redirect:'manual'` — through `env.EGRESS.fetch` for direct-internet hosts (incl. any OTHER account's Cloudflare host), or direct via global `fetch` for this account's own account-scoped CF API (AC2). This account's own R2 is the one exception: its placeholder `Authorization` is stripped and the request is RE-SIGNED with the worker-held R2 key (AC2). WebSocket upgrades are bridged (AC3). The controller reads the `strict` toggle from `ctx.props` (resolved once by the DO, [REQ-ENTERPRISE-016](#req-enterprise-016-strict-gateway-egress) AC3) — never a per-request KV read — and emits a per-op diagnostic debug log (host, account-scoped, transport `direct|EGRESS`, resigned, fetch latency; off by default, `LOG_LEVEL=debug` to enable) so routing + worker-side latency are observable. <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @impl: src/lib/controller-egress.ts::controllerFetch --> <!-- @test: src/__tests__/egress-controller.test.ts (REQ-ENTERPRISE-016 / AD86: EgressController account-scoped exemption (own account direct, all else Gateway)) -->
2. **Account-scoped exemption.** Only destinations belonging to THIS deployment's own Cloudflare account egress **direct** via global `fetch`, never through `env.EGRESS`/`cf1:network`, even when the binding is bound (and they succeed even when it is unbound — they never depend on it): its R2 endpoint `<accountId>.r2.cloudflarestorage.com` (+ the `<bucket>.<accountId>.…` vhost form; rclone sync) and its account-scoped CF API path `api.cloudflare.com/client/v4/accounts/<accountId>/…` (Browser Rendering / browser-run). These are codeflare's own control-plane backends, not the agent's external reach ([AD86](../../documentation/decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)). Any OTHER account's R2/CF host — and an absent/empty account id (fail-secure) — are NOT exempt and ride `env.EGRESS` (the Gateway) for inspection, closing the cross-account exfiltration channel. `gateway.ai.cloudflare.com` is intentionally not exempt: the container never reaches it directly (the worker-side `LlmInterceptor` does, [REQ-ENTERPRISE-024](#req-enterprise-024-strict-gateway-egress-host-specific-interceptor-routing) AC2). The account id is resolved once by the DO (`getR2Config`) and passed to the controller via `ctx.props.accountId` (alongside `ctx.props.strict`, [REQ-ENTERPRISE-016](#req-enterprise-016-strict-gateway-egress) AC3). This account's own R2 ({@link isOwnAccountR2}) additionally has its placeholder `Authorization` stripped and is **re-signed** with the worker-held R2 key (`aws4fetch` via `createR2Client`, reusing the request's `x-amz-content-sha256` so the body streams unbuffered and SSE-C headers are preserved) — the container holds only a placeholder R2 key (AC4); the account-scoped CF API stays a transparent passthrough — now a dormant fallback, since `api.cloudflare.com` Browser Rendering is claimed by the per-host `CloudflareBrowserInterceptor` ([REQ-BROWSER-008](browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), which takes precedence over this catch-all and injects the real token worker-side; this fallback can only ever carry the container's non-secret placeholder. <!-- @impl: src/lib/controller-egress.ts::isAccountScopedDestination --> <!-- @impl: src/lib/controller-egress.ts::isOwnAccountR2 --> <!-- @impl: src/lib/controller-egress.ts::controllerFetch --> <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @impl: src/lib/r2-client.ts::createR2Client --> <!-- @impl: src/container/index.ts::wireEgressInterception --> <!-- @test: src/__tests__/container/index.test.ts (container DO class / REQ-SESSION-002 (one container per session)) -->
3. **WebSocket proxying (bridged).** WebSocket upgrades reaching the `EgressController` catch-all are forwarded **transparently** (caller `Authorization` preserved) through the account-scoped selector (direct `fetch` for this account, `env.EGRESS` otherwise), then **bridged**: the controller `accept()`s the upstream `webSocket` and the server end of a fresh `WebSocketPair`, forwards messages/close/error in both directions, and returns a `101` carrying the pair's client end. Returning the upstream `101` as-is does NOT hand the socket back to the container (it stalls / is `canceled`), so the bridge is required; a failure returns `502 EGRESS_WS_FAILED`. Browser-run's interactive CDP WS (`…/browser-rendering/devtools/…`) is NOT carried here — it is claimed and bridged by the per-host `CloudflareBrowserInterceptor` ([REQ-BROWSER-008](browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), which injects the real token; this generic bridge serves any other catch-all WS. <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @impl: src/lib/controller-egress.ts::controllerFetch --> <!-- @test: src/__tests__/egress-controller.test.ts (REQ-ENTERPRISE-016: EgressController bridges WebSocket upgrades (catch-all fallback)) -->
4. **Container holds no real R2 key (strict only).** When strict is active, `buildEnvVars` emits a non-secret placeholder R2 access key/secret (`ENTERPRISE_R2_KEY_PLACEHOLDER`) into the container instead of the real key — rclone runs in signed mode with the placeholder and the `EgressController` re-signs at the R2 boundary (AC1/AC2), so the real R2 key never enters the container. Non-enterprise / strict-off sessions keep the real key (rclone connects to R2 directly), byte-identical to today. The non-secret `R2_ENDPOINT`/`CLOUDFLARE_ACCOUNT_ID` are unaffected by the R2-key swap; `CLOUDFLARE_API_TOKEN` carries only the non-secret Browser Rendering placeholder (never the real token — [REQ-BROWSER-008](browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)). <!-- @impl: src/container/container-env.ts::buildEnvVars --> <!-- @impl: src/lib/constants.ts::ENTERPRISE_R2_KEY_PLACEHOLDER --> <!-- @test: src/__tests__/container/container-env.test.ts (buildEnvVars (REQ-SESSION-016 AC3) / REQ-MEM-010 AC4 (USER_TIMEZONE feeds capture pipeline) / REQ-AGENT-031 (LLM API keys + agent-specific keys propagated to container env)) -->
...
6. The feature fails closed: when strict is ON but `env.EGRESS` is unbound, the `EgressController` (for direct-internet hosts, incl. any other account's Cloudflare host) and the `GitHubInterceptor` return `503 EGRESS_UNAVAILABLE` and never fall back to global `fetch`. This account's own platform destinations and the `LlmInterceptor` egress direct and never depend on `env.EGRESS` (AC2 and [REQ-ENTERPRISE-024](#req-enterprise-024-strict-gateway-egress-host-specific-interceptor-routing) AC2). <!-- @impl: src/lib/controller-egress.ts::controllerFetch --> <!-- @impl: src/egress-controller.ts::EgressController --> <!-- @impl: src/github-interceptor.ts::GitHubInterceptor --> <!-- @test: src/__tests__/lib/controller-egress.test.ts (REQ-ENTERPRISE-016: controllerFetch transport selection) -->
...
1. GitHub is external egress: when strict is ON, the per-host `GitHubInterceptor` swaps its single upstream `fetch` to `env.EGRESS.fetch`; credential injection, no-spoof scoping, `redirect:'manual'`, and response hygiene are byte-identical to the non-strict path, and the swap is inert (global `fetch`) when strict is OFF. <!-- @impl: src/github-interceptor.ts::GitHubInterceptor --> <!-- @test: src/__tests__/github-interceptor.test.ts (REQ-ENTERPRISE-016: strict gateway egress transport swap) -->
...
1. `POST /api/setup/configure` accepts an enterprise-only `aigGatewayUrl` (non-secret, stored plain at `SETUP_KEYS.AIG_GATEWAY_URL`) and `aigToken` (secret, encrypted at rest at `SETUP_KEYS.AIG_TOKEN` via kv-crypto). Both are no-clobber on blank (a blank/masked value keeps what is stored); a non-enterprise configure never writes either; and storing the token without `ENCRYPTION_KEY` is rejected pre-write (fail-closed, no partial state). The persistence surfaces as a `configure_ai_gateway` progress step. <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: src/lib/kv-keys.ts --> <!-- @test: src/__tests__/routes/setup.test.ts (Feature A/C: enterprise groups chip list + dynamic routes) -->
2. `GET /api/setup/prefill` round-trips the AI Gateway config (enterprise-only): it surfaces the non-secret `aigGatewayUrl` and a masked `aigTokenSet` boolean (never the token itself), reports unset/empty when nothing is stored, and omits both fields entirely in a non-enterprise prefill. <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (Setup Handlers / REQ-SETUP-005 (admin-only auth gate on POST setup endpoints) / REQ-SETUP-006 (setup config persistence + reload) / REQ-SETUP-008 (setup wizard step state machine and validation) / REQ-SETUP-011 (allowlist persisted as KV user records via setup endpoint)) -->
3. A single resolver `getAigConfig(env)` resolves the gateway URL + token wizard-first (KV) with the deploy-secret env (`AIG_GATEWAY_URL`/`AIG_TOKEN`) as a per-field fallback, returns `undefined` for a field unset in both, and degrades to the env fallback (never throws) on a KV/crypto error at the container-start seam. <!-- @impl: src/lib/aig-config.ts::getAigConfig --> <!-- @test: src/__tests__/lib/aig-config.test.ts (getAigConfig (REQ-ENTERPRISE-017)) -->
4. The container DO resolves the gateway URL + token once via `getAigConfig` and passes them to the `LlmInterceptor` via per-session props; the interceptor prefers the props and falls back to its own env only when a prop is absent. The token never enters the container (unchanged from [REQ-ENTERPRISE-004](#req-enterprise-004-outbound-interception-llm-routing-to-customer-ai-gateway) AC2). <!-- @impl: src/container/index.ts::wireLlmInterception --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-017: AI Gateway URL/token resolved from props (wizard) with env fallback) -->
5. The Setup wizard renders the enterprise-only AI Gateway URL + token fields (the token as a password input) inside an organized `SetupSection` group; they are not rendered outside enterprise mode, and their inputs route to the store's write-only `aigGatewayUrl`/`aigToken` (the stored token is never returned, so the token field hydrates blank with an `aigTokenSet` hint). <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx::ConfigureStep --> <!-- @impl: web-ui/src/components/setup/SetupSection.tsx::SetupSection --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @test: web-ui/src/__tests__/components/ConfigureStep.test.tsx (ConfigureStep) -->
6. The "Configuring Codeflare" progress screen reflects the enterprise configuration steps it runs: the configure endpoint emits named `configure_*` steps (`configure_access_groups`, `configure_model_routing`, `configure_ai_gateway`, `configure_browser_rendering`, `configure_strict_egress`) for the work it previously did silently, and the progress UI maps each to a friendly label. <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: web-ui/src/components/setup/ProgressStep.tsx --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) --> <!-- coverage-gap: the ProgressStep stepLabels mapping (configure_* key -> friendly label) is pure display lookup with a raw-key fallback; verified by inspection, not a render test -->
...
- Resolution order is per-field and KV-first: a wizard value overrides the deploy secret independently for the URL and the token, so a deployment can mix (e.g. URL from the wizard, token from a deploy secret) and the deploy secrets stay a silent backstop ([REQ-ENTERPRISE-006](#req-enterprise-006-deploy-time-aig-secrets-and-enterprise_mode-var) AC1).
- The Setup-wizard field reorganization (grouping fields into `SetupSection`s) is a behavior-preserving refactor of the existing Configure step — every prior field, store binding, and conditional `<Show>` gate is preserved; only the visual grouping changes. The `SetupSection` is a reusable structure-only component carrying no copy of its own.
...
**Verification:** [AIG resolver](../../src/__tests__/lib/aig-config.test.ts) (AC3), [interceptor props-with-env-fallback](../../src/__tests__/llm-interceptor.test.ts) (AC4), [DO passes resolved props](../../src/__tests__/container/index.test.ts) (AC4), [configure persistence + progress steps](../../src/__tests__/routes/setup.test.ts) (AC1, AC6) + [prefill](../../src/__tests__/routes/setup/handlers.test.ts) (AC2), [Configure step fields](../../web-ui/src/__tests__/components/ConfigureStep.test.tsx) + [SetupSection](../../web-ui/src/__tests__/components/SetupSection.test.tsx) + [setup store](../../web-ui/src/__tests__/stores/setup.test.ts) (AC5). The ProgressStep label mapping (AC6) is pure display lookup — carried as a coverage-gap.
...
1. `POST /api/setup/configure` accepts an enterprise-only `r2SseDisabled` boolean, persisted `'active'`/`'inactive'` at `SETUP_KEYS.R2_SSE_DISABLED` (default OFF on an absent key) and surfaced as a `configure_r2_sse` progress step; a non-enterprise configure never writes it. `GET /api/setup/prefill` round-trips it (enterprise-only, default false, omitted in non-enterprise). <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: src/lib/kv-keys.ts --> <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
2. `getSseHeaders`/`getSseCopyHeaders` suppress all SSE-C headers when the resolved disable flag is true even though `ENCRYPTION_KEY` is set; every R2 write call site (upload, seed, container ensure) chooses headers by the bucket's **committed regime** from the state object (`isR2SseDisabledForBucket` → `getRegimeState(...).regime`), and read call sites (download, preview) use the dual-regime fallback ([REQ-ENTERPRISE-021](#req-enterprise-021-governed-mode-migration-safety-and-access-boundary) AC4), so reads/writes stay correct during the rollout window before a bucket is migrated. <!-- @impl: src/lib/r2-sse.ts::getSseHeaders --> <!-- @impl: src/lib/r2-sse.ts::getSseCopyHeaders --> <!-- @impl: src/lib/r2-regime-state.ts::isR2SseDisabledForBucket --> <!-- @impl: src/routes/storage/upload.ts --> <!-- @impl: src/lib/r2-seed.ts::seedDocuments --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (regime helpers (REQ-ENTERPRISE-018)) -->
3. The bucket's resolved regime is forwarded to the container as `R2_SSE_DISABLED` (emitted only when the bucket is plain; always carried in the setBucketName body as a definite boolean so a regime flip OFF on a warm DO resets the state), and the entrypoint omits the SSE-C block from rclone.conf and re-enables checksums when it is set. `buildEnvVars` ALSO omits `ENCRYPTION_KEY` itself in Governed Mode — rclone (the container's only SSE-C consumer) does not use it there, so the high-power shared key (also the vault HKDF master + secret-at-rest key) must not sit unused in the container; non-Governed (SSE-C on) still emits it because rclone needs it. <!-- @impl: src/container/container-env.ts::buildEnvVars --> <!-- @impl: src/container/container-env.ts::applyBucketName --> <!-- @impl: src/container/container-env.ts::applyPrefsOnRestart --> <!-- @impl: src/routes/container/lifecycle-init.ts --> <!-- @impl: entrypoint.sh::create_rclone_config --> <!-- @test: src/__tests__/container/container-env.test.ts (applyBucketName / applyPrefsOnRestart propagate userTimezone (REQ-SESSION-016 AC3 wiring regression) / REQ-AGENT-029 (container env vars contract)) -->
4. The Setup wizard renders an enterprise-only Governed Mode toggle (default off, not rendered outside enterprise) whose change requires an explicit admin confirmation of the re-encrypt consequence before it flips the store value. <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx::ConfigureStep --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @impl: web-ui/src/lib/schemas.ts --> <!-- @test: web-ui/src/__tests__/components/ConfigureStep.test.tsx (ConfigureStep) -->
...
1. Re-encryption is a same-key server-side `CopyObject` with `MetadataDirective=REPLACE`: a single source-regime HEAD captures the object's metadata and ETag, re-supplied on the copy with `x-amz-copy-source-if-match`; SSE-C headers decrypt the source and encrypt the destination. A successful copy requires a parsed `<CopyObjectResult><ETag>` with no embedded `<Error>`. <!-- @impl: src/lib/r2-migration.ts::migrateBucketEncryption --> <!-- @impl: src/lib/r2-sse.ts::computeKeyMd5 --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (migrateBucketEncryption (lossless REPLACE re-encrypt)) -->
...
3. The per-bucket regime is a KV state machine (`r2-regime:<bucket>`, statuses `ready`/`migrating`/`mixed-recovery` — [AD91](../../documentation/decisions/README.md#ad91-governed-mode-migration-is-a-verified-gated-chunked-state-machine-replace-copy-not-a-boolean-marker-lazy-reconcile)). Each `batch-status` poll synchronously decides the transition: a `ready` bucket whose committed regime differs from policy flips to `migrating` when no container is healthy, else stays `pending` while a session runs. <!-- @impl: src/lib/r2-regime-state.ts::getRegimeState --> <!-- @impl: src/lib/r2-migration.ts::planRegimeReconcile --> <!-- @impl: src/routes/session/lifecycle.ts --> <!-- @test: src/__tests__/routes/session-batch-status.test.ts (REQ-ENTERPRISE-020: Governed Mode reconcile + chunk advance on batch-status) -->
4. Each poll's background work scans the bucket in `LIST_PAGE_SIZE`-sized pages (up to 1,000 keys), re-encrypting each page in `MIGRATION_CONCURRENCY`-sized slices (6). Per-object failures are isolated, and each R2 op runs under an `AbortController` timeout that frees its connection on expiry. <!-- @impl: src/lib/r2-migration.ts::advanceMigration --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (advanceMigration (chunked, verified, self-healing)) -->
5. The `cursor` is a `start-after` key checkpointed after every chunk, resuming past the last processed object with no re-scan. Each invocation starts another list+slice only while one more fits its ~22s work deadline (`WORK_DEADLINE_MS`), then releases the `leaseExpiresAt` lock before exit ([AD91](../../documentation/decisions/README.md#ad91-governed-mode-migration-is-a-verified-gated-chunked-state-machine-replace-copy-not-a-boolean-marker-lazy-reconcile)). <!-- @impl: src/lib/r2-migration.ts --> <!-- @impl: src/lib/r2-migration.ts::advanceMigration --> <!-- @impl: src/routes/session/lifecycle.ts --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (advanceMigration (chunked, verified, self-healing)) -->
...
- Termination is guaranteed: verify skips the same oversized objects migrate skips, per-object failures are isolated, and the bounded migrate↔verify retry (`MAX_VERIFY_RETRIES`) halts an unfixable object for admin review rather than looping forever.
- Session creation never migrates a bucket — it only resolves the committed regime, and a new bucket adopts the current policy immediately. <!-- @impl: src/routes/container/lifecycle-init.ts::ensureBucketAndSeed --> <!-- @impl: src/lib/r2-regime-state.ts::resolveBucketSseOnEnsure --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (resolveBucketSseOnEnsure (lazy-create paths) describe) --> <!-- @test: src/__tests__/routes/container-lifecycle-helpers.test.ts (ensureBucketAndSeed describe) -->
...
2. While a bucket's state is not `ready`, every R2 writer (container `/start`, upload, reseed, preference reconcile) rejects `409 BUCKET_MIGRATING`, and the sync fan-out returns empty. On migration start, running containers are drained once and in-flight multipart uploads are aborted. <!-- @impl: src/lib/error-types.ts::BucketMigratingError --> <!-- @impl: src/lib/r2-regime-state.ts::isBucketMigrating --> <!-- @impl: src/routes/container/lifecycle.ts --> <!-- @impl: src/routes/storage/upload.ts --> <!-- @impl: src/lib/migration-containers.ts::drainContainers --> <!-- @impl: src/lib/sync-fanout.ts::fanOutBisyncTrigger --> <!-- @impl: src/routes/session/lifecycle.ts --> <!-- @test: src/__tests__/routes/sessions-sync.test.ts (skips the entire fan-out while the bucket is migrating (no container is contacted)) -->
3. The dashboard reuses the REQ-AGENT-049 "Upgrading" affordance: `batch-status` returns `bucketMigrating` plus a 0–99 `bucketMigrationPercent` (omitted while `halted`), and the New Session button disables and labels "Migrating N%". Both the full session load and the 5s background poll mirror these flags. <!-- @impl: web-ui/src/components/Dashboard.tsx --> <!-- @impl: web-ui/src/stores/session.ts --> <!-- @impl: web-ui/src/stores/session-polling.ts --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (Session Store) -->
4. Read paths (download, preview) try the committed regime first and fall back once to the opposite regime on a `400`/`403` SSE-mismatch, so a partially-migrated bucket stays readable. <!-- @impl: src/lib/r2-migration.ts::fetchObjectWithRegimeFallback --> <!-- @impl: src/lib/r2-regime-state.ts::resolveReadRegime --> <!-- @impl: src/routes/storage/download.ts --> <!-- @impl: src/routes/storage/preview.ts --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (fetchObjectWithRegimeFallback (D2 reads stay up)) -->
...
- `mixed-recovery` respects D1 (no force-kill); key rotation is detect-only, with no old-key fallback. <!-- @impl: src/lib/r2-migration.ts::advanceMigration --> <!-- @test: src/__tests__/lib/r2-migration.test.ts (halts with a clear error when the SSE-C key rotated mid-migration (keyMd5 mismatch, detect-only) it) -->
...
**Verification:** [zero-secret invariant](../../src/__tests__/container/container-env-llm.test.ts) (AC1), [write gate 409](../../src/__tests__/routes/storage-upload.test.ts) + [sync-fanout skip](../../src/__tests__/routes/sessions-sync.test.ts) (AC2), [store bucketMigrating + background-poll flags](../../web-ui/src/__tests__/stores/session.test.ts) + [migrating-button label](../../web-ui/src/__tests__/components/Dashboard.test.tsx) (AC3), [dual-regime read fallback + self-heal recovery scan](../../src/__tests__/lib/r2-migration.test.ts) (AC4-5)
...
1. The Setup wizard exposes an enterprise-gated view-only-storage toggle, default OFF, persisted to KV `SETUP_KEYS.DOWNLOADS_DISABLED` as `'active'`/`'inactive'` through the existing `POST /api/setup/configure` (no new endpoint); a non-enterprise configure never writes it. <!-- @impl: src/routes/setup/index.ts --> <!-- @impl: web-ui/src/components/setup/ConfigureStep.tsx --> <!-- @impl: web-ui/src/stores/setup.ts::setupStore --> <!-- @test: src/__tests__/routes/setup.test.ts (Setup Routes / REQ-SETUP-001 (zero pre-config first-time setup) / REQ-SETUP-002 (step sequence) / REQ-SETUP-004 (idempotent setup) / REQ-SETUP-012 (setup completion record)) -->
2. `GET /api/setup/prefill` round-trips the toggle: a seeded `SETUP_KEYS.DOWNLOADS_DISABLED` prefills `true`, an absent key prefills `false`, and it is omitted from a non-enterprise prefill. <!-- @impl: src/routes/setup/handlers.ts --> <!-- @test: src/__tests__/routes/setup/handlers.test.ts (Setup Handlers / REQ-SETUP-005 (admin-only auth gate on POST setup endpoints) / REQ-SETUP-006 (setup config persistence + reload) / REQ-SETUP-008 (setup wizard step state machine and validation) / REQ-SETUP-011 (allowlist persisted as KV user records via setup endpoint)) -->
3. When the toggle is active (enterprise), `GET /api/storage/download` enforces view-only **server-side, before any R2 fetch**: a download (attachment) request returns a `403` carrying the distinct `DOWNLOADS_DISABLED` code; an inline-view request for a non-viewable type (archive/binary/unknown) returns the same `403 DOWNLOADS_DISABLED`; an inline-view request for a viewable type (text/Markdown/HTML-as-text/images/PDF, per `isInlineViewable`) is served inline. The distinct code (not a generic `FORBIDDEN`) lets the client tell a view-only block apart from an auth/ownership 403. The policy is resolved by `isDownloadsDisabled(env)` (enterprise mode AND KV active; non-enterprise never reads KV). <!-- @impl: src/routes/storage/download.ts --> <!-- @impl: src/routes/storage/download.ts::isInlineViewable --> <!-- @impl: src/lib/error-types.ts::DownloadsDisabledError --> <!-- @impl: src/lib/downloads-policy.ts::isDownloadsDisabled --> <!-- @test: src/__tests__/routes/storage-download.test.ts (Storage Download Routes) -->
4. `GET /api/user` returns `downloadsDisabled` so the client renders the Storage Panel download controls (toolbar "download selected" and the file-preview download button) as **blocked** — visible but disabled-looking and still tappable — and any interaction opens a notice that downloads are disabled by the administrator instead of fetching. As a **server-truth backstop**, the download paths also treat a `DOWNLOADS_DISABLED` 403 from `GET /api/storage/download` as the disabled state — surfacing the same notice, never a raw "download failed" alert / console error — so the guarantee holds even when the client's cached `downloadsDisabled` flag is stale (e.g. the admin flipped the toggle after the app loaded). The flag is additionally re-pulled when the Storage panel opens, so the blocked control reflects a post-load toggle proactively; the strict `/api/user` response schema (`UserResponseSchema`) must declare `downloadsDisabled` or it is stripped on parse and the flag can never set. The per-file open/view path is unchanged. The client controls + notice are UX only — the server guard (AC2) is the enforcement. <!-- @impl: src/routes/user-profile.ts --> <!-- @impl: web-ui/src/lib/schemas.ts::UserResponseSchema --> <!-- @impl: web-ui/src/components/storage/StorageToolbar.tsx --> <!-- @impl: web-ui/src/components/StorageBrowser.tsx --> <!-- @impl: web-ui/src/components/Dashboard.tsx --> <!-- @impl: web-ui/src/components/FilePreview.tsx --> <!-- @impl: web-ui/src/components/DownloadsDisabledPopup.tsx --> <!-- @impl: web-ui/src/lib/download.ts::downloadFile --> <!-- @impl: web-ui/src/stores/storage.ts::storageStore --> <!-- @impl: web-ui/src/stores/storage.ts::refreshDownloadsDisabled --> <!-- @test: web-ui/src/__tests__/components/StorageBrowser.test.tsx (StorageBrowser / REQ-STOR-016 AC1/AC2 (file browser drawer/bottom-sheet presentation, R2 as source of truth via Worker API)) -->
...
- Enforcement is **server-side** in `download.ts`; the frontend blocked-control + notice are convenience only, so a prompt-injected agent cannot bypass the policy by crafting the download URL. Inline view is restricted to a deny-by-default `isInlineViewable` allowlist so a non-viewable blob cannot be exfiltrated through the inline `text/plain` fallback.
