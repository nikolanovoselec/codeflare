...
4. Existing Chat Completions fallback, Responses passthrough, parsing, replay, and stream repair behavior remains unchanged. <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/llm-interceptor.test.ts (AC1: replays a model-routable request to the compat path when the REST API returns 404) --> <!-- @test: src/__tests__/llm-interceptor.test.ts (does not touch a non-chat-completions stream (e.g. /responses passes through unchanged)) --> <!-- @test: src/__tests__/lib/reasoning-discovery.test.ts (preserves the validated Pi 0.84.4 canary request and replay fixtures) --> <!-- @test: src/__tests__/llm-interceptor.test.ts (injects a finish_reason:"stop" chunk before [DONE] when the upstream omits it (dynamic-route bug)) -->
5. Until the atomic document exists, valid legacy GLM/Kimi assignments resolve in memory; unreadable, malformed, ambiguous, or GPT-OSS legacy state fails closed and is never persisted automatically. <!-- @impl: src/lib/reasoning-configuration.ts::parseReasoningConfigurationWithLegacyFallback --> <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @test: src/__tests__/lib/reasoning-configuration.test.ts (uses valid legacy assignments only when the atomic configuration is absent, without persistence) --> <!-- @test: src/__tests__/lib/enterprise-route-config.test.ts (keeps legacy installations startable by deriving supported levels in memory when atomic configuration is absent) --> <!-- @test: src/__tests__/llm-interceptor.test.ts (AC3: fails closed when atomic data is unreadable or legacy data is malformed, ambiguous, or GPT-OSS) -->
...
1. Discovery handles one target per run, enforces its limits, returns sanitized evidence, and never activates its result. Authentication, rate-limit, server, transport, and malformed-stream failures stop the entire route-only candidate scan. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/lib/reasoning-discovery.test.ts (rejects excessive ceilings and reasoning-probe budgets before provider I/O) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (stops the whole candidate scan after HTTP %s) -->
...
1. Route-scoped discovery recommends compatible enabled built-in and saved custom revisions by name and exact reference, preserving equivalent profile choices. The administrator explicitly selects a recommendation into the route draft; discovery neither assigns it automatically nor claims backend-family identification. Existing matches do not create duplicate custom profiles. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: web-ui/src/components/admin/ReasoningProfileEditor.tsx::ReasoningProfileEditor --> <!-- @test: src/__tests__/routes/admin-reasoning.test.ts (recommends existing catalog revisions without creating a duplicate profile draft) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (recommends the exact existing Kimi revision when non-off tools and replay pass but off still reasons) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (retains equivalent Gemma and GLM choices instead of mistaking deduplication order for identity) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (recommends a saved custom revision from matching bounded evidence without additional custom probes) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (uses the exact matched $name catalog revision only in the route draft) --> <!-- @test: web-ui/src/__tests__/components/ReasoningProfileEditor.test.tsx (shows every named existing match, including Kimi and a saved custom profile, without creating a duplicate) -->
...
4. When no existing profile's complete level mapping fits the observations, discovery generates a custom draft from one unambiguous set of passed modes. Failed, incomplete, and unproven off modes are excluded along with their aliases; removals and literal writes are preserved. <!-- @impl: src/routes/admin/reasoning.ts::observedCandidate --> <!-- @impl: src/routes/admin/reasoning.ts::generatedProfileDraft --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (creates a normalized custom draft from passed modes when no complete existing profile fits) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (does not recommend disabled custom revisions) -->
5. Discovery and selected-profile verification expose a user-selected completion ceiling from 32 to 16,384 tokens, defaulting to 32. Token-exhausted tool calls or replay remain inconclusive, not evidence of incompatibility, and require an explicitly initiated higher-ceiling run. Results retain sanitized failure stages and per-candidate compatible levels. <!-- @impl: src/lib/reasoning-discovery.ts::mappingDiagnostics --> <!-- @impl: web-ui/src/components/admin/ReasoningProfileEditor.tsx::ReasoningProfileEditor --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/lib/reasoning-discovery.test.ts (REQ-ENTERPRISE-035: reports completion limits at %s without retrying or claiming incompatibility) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (reports incomplete tool generation at the selected ceiling rather than unsupported reasoning) --> <!-- @test: src/__tests__/lib/reasoning-discovery.test.ts (REQ-ENTERPRISE-035: excludes off from compatible modes when hidden reasoning tokens are reported) --> <!-- @test: web-ui/src/__tests__/components/ReasoningProfileEditor.test.tsx (sends the explicit %s token ceiling and locks the controls during discovery) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (sends the chosen verification ceiling and exposes budget diagnostics without retrying (assignable: %s)) -->

**Constraints:**

- Match selection remains bounded, deterministic, sanitized, non-assigning, and non-activating.
- The finite built-in protocol bank bounds route-only probes; saved custom revisions are matched against those observations without multiplying paid probes. Unknown request properties are not inferred. Explicit selected-profile verification remains separate.
- Per-candidate request rejection stops that candidate without retry; the whole-scan stop conditions in REQ-ENTERPRISE-033 still apply. A stopped whole scan cannot publish recommendations or drafts.
- Complete existing-profile matches take precedence over partial candidate drafts. If no existing profile fits, partial candidates still require one unambiguous maximal mapping; contradictory protocols are never combined.
