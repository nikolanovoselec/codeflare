...
**Verification:** Automated test ([env-pipeline test](../../src/__tests__/container/container-env-llm.test.ts) (AC1/AC4/AC5 env injection; AC6 secret hygiene — no AWS_* in either mode, enterprise CLOUDFLARE_API_TOKEN placeholder-only); [Pi models.json build test](../../host/__tests__/entrypoint-enterprise-pi-models.test.js) (AC1 — per-route contextWindow; AC5 — inactive gate skips Pi provider config; AC6 — auth.json cleared to {}); [entrypoint CA-trust + Copilot BYOK test](../../host/__tests__/entrypoint-enterprise-ca-copilot.test.js) (AC2 — CA env prepended to .bashrc, idempotent, enterprise-gated; AC3 — Copilot BYOK vars + token-limit hints prepended, stale route overwritten on re-run, enterprise-gated). All acceptance criteria are covered by automated tests.)
...
9. Minimum requires tools with exact replay plus provider prefix-read or Gateway HIT, separately labelled. Acceptable adds Provider-default or observed enabled reasoning; Optimal adds cold incremental public delivery. Missing cache evidence remains inconclusive, not unsupported. <!-- @impl: src/lib/ai-capability-discovery/contract.ts::capabilityMinimum --> <!-- @impl: src/lib/reasoning-discovery.ts::discoverCache --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts -->
10. The finite request-form search is independent of model/catalog additions, bounded at 40 submissions/2,048 output tokens/90 seconds each/10 minutes overall, and stops at the first qualified result or fatal boundary. Hash-bound wire choices are shared with runtime; no arbitrary native protocol, URL or header is guessed. <!-- @impl: src/lib/ai-capability-discovery/index.ts::capabilityCandidates --> <!-- @impl: src/lib/ai-capability-discovery/compatibility-wire.ts::compatibilityResponse --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts --> <!-- @test: src/__tests__/lib/target-capability-runtime.test.ts -->
11. Evidence covers the exercised Dynamic branch, never a union of conflicting known backends. Multi-distinct-backend routes with unidentified selection remain inconclusive. Failed cache stages retain sanitized status/provider codes; stale draft/inventory results cannot authorize Save. <!-- @impl: src/lib/reasoning-discovery.ts::probeDiagnostic --> <!-- @impl: src/lib/ai-capability-discovery/index.ts::discoverTargetCapabilities --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx -->
...
**Intent:** Compatible Anthropic Messages releases share a reusable Provider-default contract; historical evidence-specific controls remain immutable and bounded.
...
6. A new compatible Anthropic Bedrock identifier uses the reusable native Provider-default profile without a source/model-list or profile-definition addition. Namespace recognition is a candidate, not capability or authorization evidence. <!-- @impl: src/lib/native-ai-target-draft.ts::bedrockAnthropicCandidate --> <!-- @test: src/__tests__/lib/bedrock-generic-contract.test.ts --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) -->
7. All seven Pi client preferences remain accepted for Provider default; the Worker removes reasoning overrides and claims neither exact effort nor true Off. Historical evidence-specific aliases do not leak through model-name substring matching. <!-- @impl: src/lib/reasoning-profiles.ts::translateRuntimeReasoningRequest --> <!-- @test: src/__tests__/lib/bedrock-generic-contract.test.ts -->
...
**Intent:** Administrators reuse a native protocol contract and explicitly establish target-bound capabilities through the existing verification workflow, while retaining historical confirmation authority.

**Applies To:** Admin

**Acceptance Criteria:**

1. Normal Bedrock Discover verifies and adopts the shared contract and target-bound receipt without changing saved transport; other native protocols retain Advanced flows until deliberately extended. <!-- @impl: src/routes/admin/ai-capability-discovery.ts::routes --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/routes/target-capability-discovery.test.ts (certifies a synthetic future native model through the saved Invoke transport and authentic replay) --> <!-- @test: src/__tests__/routes/target-capability-discovery.test.ts (does not infer Azure support or accept a browser endpoint/credential override) --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (selects a future Bedrock model and adopts its reusable native contract in one Discover action) -->
2. Advanced listing offers the reusable Provider-default template without asserting model capability or making a paid request. Selecting or starting a target never probes automatically; Discover is explicit. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig --> <!-- @test: src/__tests__/routes/reasoning-eligibility.test.ts (REQ-ENTERPRISE-075: offers the reusable native contract without a paid probe and preserves historical explicit confirmation) --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) -->
3. Generic native authority requires explicit tools/replay and cache verification, not administrator assertion; historical evidence-specific profiles retain explicit reconfirmation. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (upgrades one real old profile reference via explicit confirmation while retaining another disabled stale target) -->
4. Cache misses, absent counters, and incomplete observations leave minimum qualification inconclusive, not automatically unsupported; failed checks do not enable targets. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (does not confuse absent cache observations with proof that caching is unsupported) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: explains an inconclusive cache check without enabling the native target) -->
5. Dynamic Route selectors exclude provider-native Bedrock profiles and clear previously selectable native authority until an allowed profile is selected. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-034: loads every detected inventory read-only and exposes models in the selected route outside advanced disclosures) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: removes hidden provider-native authority from a Dynamic Route draft until an allowed profile is selected) -->
6. Compatible older, current, and synthetic future models reuse one contract; saved explicit profiles remain selectable in Advanced after generic adoption, with fresh exact verification required on reselection and saved transport unchanged. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-075: selects older/current/synthetic future models with one reusable native profile and explicit verification) --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (retains saved $profileId in Advanced after generic adoption, requiring a fresh exact check when reselected) -->

**Constraints:**

- Administration performs no provider fallback or paid retry.
- Native Runtime discovery uses at most four calls. The shared normal UI discloses the overall 40-submission/10-minute ceiling, 2,048 output tokens and 90 seconds per request. <!-- @impl: src/lib/ai-capability-discovery/index.ts::discoverTargetCapabilities --> <!-- @impl: web-ui/src/components/admin/TargetCapabilityDiscovery.tsx::TargetCapabilityDiscovery --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts (verifies an unlisted native identifier, restores authentic replay, then measures prefix reads) --> <!-- @test: web-ui/src/__tests__/components/TargetCapabilityDiscovery.test.tsx (selects a future Bedrock model and adopts its reusable native contract in one Discover action) -->
- Server receipts, not UI adoption or profile names, authorize Save and runtime. Discovery does not persist activation.
...
**Verification:** Automated tests for discovery authority, native call bounds, and visible pre-submission budget disclosure. CI remains the execution authority.
...
5. After authorization and replay validation, the retained Opus automatic control profile selects Eventstream through mapped High and Invoke for mapped XHigh/Max, for both initial and continuation turns. Sonnet XHigh/Max remain mapped High aliases. Generic Provider default sends no effort and uses Eventstream under auto, without inheriting a model-specific upper-effort rule. Explicit Invoke targets remain Invoke. <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::selectBedrockAnthropicTransport --> <!-- @test: src/__tests__/llm-interceptor.test.ts (REQ-ENTERPRISE-072/077/078: dispatches initial %s once with the evidenced mapped effort) --> <!-- @test: src/__tests__/lib/bedrock-anthropic-native-adapter.test.ts (REQ-ENTERPRISE-077/078: selects automatic transport from mapped %s without changing explicit transports) -->
...
5. A provider-controlled profile presents policy reasoning as Provider default rather than Off. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::PolicyFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-074/078: preserves saved %s / %s identity and real levels on unrelated edits) -->
...
**Constraints:** Administration remains retryable while profile lifecycle, identity, and Save authority stay with [REQ-ENTERPRISE-054](#req-enterprise-054-native-target-profile-and-lifecycle-administration), [REQ-ENTERPRISE-053](#req-enterprise-053-native-target-identity-and-document), and [REQ-ENTERPRISE-055](#req-enterprise-055-native-target-authority-and-save).
...
2. Opening a target row exposes its target fields and profile actions. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-051/056/064: renders named native target rows with expanded-only controls) -->
...
5. Access-policy checkboxes and default-route choices show each native target's provider and exact model. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::PolicyFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-056/064: presents named native group assignments while preserving their opaque handles) -->
...
2. Configured and assignable native targets display as `Native Route - <provider> - <exact model>`. <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (REQ-ENTERPRISE-056/064: presents named native group assignments while preserving their opaque handles) -->
...
**Verification:** Behavioral route-selection and submitted-identity fixtures, exact-head CI, and deployed Administration acceptance.
...
**Dependencies:** [REQ-ENTERPRISE-053](#req-enterprise-053-native-target-identity-and-document)
...
- The token never enters the container, as required by [REQ-ENTERPRISE-005](#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC4.
...
5. Semantic adapter changes invalidate old authority without making historical documents unreadable. Upgrade requires explicit confirmation of retained evidence or generic verification, never fabricated or silently upgraded receipts; unchanged stale targets may remain disabled. <!-- @impl: src/lib/native-ai-targets.ts::nativeVerificationMatches --> <!-- @test: src/__tests__/lib/native-ai-targets.test.ts (REQ-ENTERPRISE-074: binds provider-native Bedrock profiles to the validated model, region, and transport) --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (upgrades one real old profile reference via explicit confirmation while retaining another disabled stale target) --> <!-- @test: src/__tests__/routes/bedrock-generic-upgrade.test.ts (certifies, saves and authorizes a synthetic future model through the existing workflow without authoring a profile) -->
6. Pi `cacheRetention: none` emits no explicit checkpoints, not a guarantee that AWS implicit caching is disabled. Only five-minute retention is advertised. <!-- @impl: entrypoint.sh::prompt_cache --> <!-- @test: scripts/verify-bedrock-pi-prompt-cache.mjs (REQ-ENTERPRISE-083: locked Pi preserves checkpoint opt-in, opt-out and exact replay) --> <!-- @test: host/__tests__/entrypoint-enterprise-pi-models.test.js (REQ-ENTERPRISE-083: enables Pi checkpoints only for the authorized native Runtime handle) -->
7. Dynamic verification correlates reasoning, tools/replay, and cache only on the exercised path; conflicting observations or missing backend identity on a multi-backend route cannot qualify through either normal or Advanced discovery. Misses and incomplete evidence remain inconclusive, and Dynamic receives no native serialization. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts (does not combine tool evidence from one observed branch with cache evidence from another) --> <!-- @test: src/__tests__/lib/bedrock-capability-discovery.test.ts (accepts Dynamic Gateway %s honestly, without native serialization or an all-branches gate) --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (qualifies token-backed reasoning with tools/cache only on the same backend ($sameBackend)) --> <!-- @test: src/__tests__/routes/target-capability-discovery.test.ts (requires response identities on a multi-backend route (identified: %s)) -->

**Constraints:**

- Actual cache eligibility/hits remain provider decisions; Gateway HIT is whole-response reuse, never provider-prefix proof.
- Pi's final tool-result text checkpoint maps to the enclosing native `tool_result`; an interior checkpoint cannot move past later content. Unknown controls, unsupported TTLs, top-level automatic controls, and more than four checkpoints are rejected.
- Active replay and exact target-authority isolation apply regardless of reasoning under [REQ-ENTERPRISE-073](#req-enterprise-073-provider-native-bedrock-replay-integrity) and [REQ-ENTERPRISE-079](#req-enterprise-079-provider-native-bedrock-replay-confidentiality). Version history: [cache evidence](../../documentation/lanes/bedrock-prompt-caching.md), [generic support](../../documentation/lanes/bedrock-generic-model-support.md).
- Malformed cache-stage responses stop discovery before another contract, preserving sanitized stage diagnostics. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (stops after malformed HTTP 200 SSE at $stage without trying another contract) -->
- Observed reasoning length can establish enabled reasoning without token counters; neither reasoning text nor signed state is exposed. It does not prove graduated effort fidelity. <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility --> <!-- @test: src/__tests__/lib/target-capability-discovery.test.ts (grades reasoning content without token counters as $reasoning) -->
- Provider cache counters follow [REQ-ENTERPRISE-076](#req-enterprise-076-provider-native-bedrock-protocol-translation) AC6. No Gateway TTL/header/policy change, provider fallback, or all-branch probing is introduced.
- Framing alone does not prove incremental delivery. Existing reasoning mappings, saved transports and Dynamic routing remain unchanged; continuation follows [REQ-ENTERPRISE-077](#req-enterprise-077-provider-native-bedrock-transport-dispatch).
...
**Verification:** Automated synthetic boundary tests and the named locked-Pi interoperability test, executed by CI. Historical Runtime observations are separate [cache evidence](../../documentation/lanes/bedrock-prompt-caching.md), not deployed application acceptance.
