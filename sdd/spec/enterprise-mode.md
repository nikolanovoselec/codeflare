...
<!-- @test: web-ui/src/__tests__/components/Header.test.tsx (enterprise dropdown gating -> Guided Setup + Logout hidden AC8/AC9, Usage shown AC2, in Header menu) -->
<!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (enterprise dropdown gating -> Guided Setup + Logout hidden AC8/AC9, Usage shown AC2, in Dashboard menu) -->
### REQ-ENTERPRISE-008: Enterprise Frontend Surface Suppression

**Intent:** Frontend surfaces are suppressed along two axes. (a) The SaaS-billing surfaces — subscription, plans, the monthly-quota / "Upgrade" banners, the subscription-tier admin, and the Standard/Pro session-mode selector — are meaningful only in SaaS mode, so they render only when `SAAS_MODE` is active and are hidden in enterprise, onboarding, and default deployments alike (a non-SaaS deployment showing a "choose your plan" / "upgrade" surface is misleading; onboarding originally inherited these because the gate was `!enterprise`, which this REQ corrects to `saasMode`). Read-only usage visibility is the exception: the monthly-usage view is also shown in enterprise (the Timekeeper records compute in all modes), without any billing / quota / "Upgrade" framing, so enterprise users can see their own consumption. (b) In-product user administration, first-login routing, the username dropdown's "Guided Setup" (per-user onboarding) and "Logout" entries, and the setup "Regular Users" section are enterprise-specific suppressions keyed off `ENTERPRISE_MODE`.
...
2. The username dropdown — in both the Header menu and the Dashboard menu — renders the "Subscription" entry only when `SAAS_MODE` is active. The "Usage" entry renders when `SAAS_MODE` is active or `ENTERPRISE_MODE` is set — usage is read-only consumption visibility (the Timekeeper records monthly compute in all modes), so enterprise users see it; the `/app/usage` page is reachable in both, with its in-page "Subscription" billing button hidden outside SaaS. <!-- @impl: web-ui/src/components/Header.tsx::Header --> <!-- @impl: web-ui/src/components/Dashboard.tsx --> <!-- @impl: web-ui/src/components/UsagePage.tsx -->
...
6. Three-mode parity: in SaaS mode every surface in AC1–AC4 renders; in onboarding and default deployments the SaaS-billing surfaces (AC1 "Manage Subscriptions", AC2, AC3, AC4) do not render while AC1 "Manage Users" does; in enterprise mode every SaaS-billing surface in AC1–AC5 is suppressed except the read-only "Usage" entry (AC2), which renders. `/app/subscribe` is reachable only in SaaS mode — the client `SubscribeGuard` redirects to `/app/` whenever `saasMode` is not active. <!-- @impl: web-ui/src/App.tsx::App -->
...
8. When `ENTERPRISE_MODE` is set, the username dropdown's "Guided Setup" (per-user onboarding) entry is hidden in both the Header menu and the Dashboard menu — enterprise instances are configured by an admin via Setup, not per-user onboarding; when unset it renders unchanged. <!-- @impl: web-ui/src/components/Header.tsx::Header --> <!-- @impl: web-ui/src/components/Dashboard.tsx -->
9. When `ENTERPRISE_MODE` is set, the username dropdown's "Logout" entry is hidden in both the Header menu and the Dashboard menu — under SSO the in-app logout is ineffective (it clears the Cloudflare Access session but the IDP silently re-authenticates on the next request), so sign-out is governed by the IDP / device; when unset it renders unchanged. <!-- @impl: web-ui/src/components/Header.tsx::Header --> <!-- @impl: web-ui/src/components/Dashboard.tsx -->
