...
6. Discovery and selected-profile verification expose a completion-token ceiling from 32 to 16,384, defaulting to 32. <!-- @impl: web-ui/src/components/admin/ReasoningProfileEditor.tsx::ReasoningProfileEditor --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @test: web-ui/src/__tests__/components/ReasoningProfileEditor.test.tsx (sends the explicit %s token ceiling and locks the controls during discovery) --> <!-- @test: web-ui/src/__tests__/components/AiRoutingFields.test.tsx (sends the chosen verification ceiling and exposes budget diagnostics without retrying (assignable: %s)) -->
...
1. Discovery recommends compatible enabled catalog revisions by name and exact reference, retaining equivalent choices regardless of alias representation. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: src/routes/admin/reasoning.ts::distinctCandidateReports --> <!-- @impl: web-ui/src/components/admin/ReasoningProfileEditor.tsx::ReasoningProfileEditor --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (recommends the exact existing Kimi revision when non-off tools and replay pass but off still reasons) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (retains equivalent Gemma and GLM choices instead of mistaking deduplication order for identity) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (recommends a saved custom revision from matching bounded evidence without additional custom probes) --> <!-- @test: src/__tests__/routes/admin-reasoning-discovery.test.ts (retains equivalent Kimi and saved custom choices when alias representations differ) --> <!-- @test: web-ui/src/__tests__/components/ReasoningProfileEditor.test.tsx (shows every named existing match, including Kimi and a saved custom profile, without creating a duplicate) -->
