...
6. When `ENTERPRISE_MODE` is set, a session start for a user whose stored session-mode preference is not Pro reconciles the bucket's agent configs to Pro and stamps the preference; the stamp is written only after a successful reconcile, so a failure retries on the next start without failing the session. <!-- @impl: src/routes/container/lifecycle-init.ts::ensureBucketAndSeed --> <!-- @test: src/__tests__/routes/container-r2-start.test.ts (REQ-ENTERPRISE-001 AC6: enterprise upgrade reconcile for pre-existing users) -->
