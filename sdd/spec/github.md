...
1. `github.com`, `api.github.com`, and Copilot's remote GitHub MCP host `api.githubcopilot.com` are registered for outbound HTTPS interception and handled by a GitHub interceptor; AI hosts continue to route to the LLM interceptor. <!-- @impl: src/container/index.ts::wireGithubInterception --> <!-- @impl: src/github-interceptor.ts::interceptedGithubHosts --> <!-- @test: src/__tests__/container/index.test.ts (wires the GitHubInterceptor for github.com + api.github.com + api.githubcopilot.com with per-session user+bucket props) --> <!-- @test: src/__tests__/github-interceptor.test.ts (includes api.githubcopilot.com in the intercepted host set) -->
2. On each intercepted request the interceptor resolves + decrypts the user's token for the bound session, strips any client-supplied auth, and stamps the correct credential: git Basic on the web host; `gh`/REST `Bearer` + `X-GitHub-Api-Version` on the API host; and `Bearer` (no API-version header) on Copilot's `api.githubcopilot.com/mcp`, so the remote `github-mcp-server` connects authenticated instead of riding the strict-egress catch-all unauthenticated. <!-- @impl: src/github-interceptor.ts::GitHubInterceptor --> <!-- @test: src/__tests__/github-interceptor.test.ts (REQ-GITHUB-003: REST API credential injection / REQ-GITHUB-003: git Smart-HTTP credential injection / REQ-GITHUB-003: Copilot remote GitHub MCP credential injection) -->
