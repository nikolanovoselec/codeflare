...
5. No restore attempt starts after the overall budget of 180 seconds is spent, and each started attempt is separately capped at 120 seconds, so total wall time is bounded but not by the overall figure alone. <!-- @impl: entrypoint.sh::run_post_restore_startup --> <!-- @test: host/__tests__/git-clone.test.js (REQ-GITHUB-015 AC5: stops attempting repositories once the overall budget is spent) -->
...
8. A repository the user removes from the workspace stops being tracked. <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-GITHUB-015 AC1: stops tracking a repository the user removed from the workspace) -->
9. Tracking retains at most 20 repositories in a stable order. <!-- @impl: src/lib/clone-targets.ts::normalizeTrackedClones --> <!-- @test: src/__tests__/lib/clone-targets.test.ts (REQ-GITHUB-015 AC2: normalizeTrackedClones / retains at most MAX_TRACKED_CLONES repositories) -->
