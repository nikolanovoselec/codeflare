...
3. Single-panel mobile/flip mode derives the active face's used height from that face's own content, capped at the current shared viewport limit; flipping recomputes unequal face geometry, viewport resizing recomputes the cap, and short content stays compact. <!-- @impl: web-ui/src/components/Dashboard.tsx::measureLayout --> <!-- @impl: web-ui/src/components/Dashboard.tsx::activeFlipHeight --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-GITHUB-012: sizes each mobile flip face to its own capped content height) -->
4. Single-panel mobile/flip mode vertically centers the active panel, and overflowing repository or storage content scrolls inside its list rather than expanding beyond the viewport cap. <!-- @impl: web-ui/src/styles/dashboard.css::.dashboard-main --> <!-- @impl: web-ui/src/styles/dashboard.css::.dashboard-panel-right --> <!-- @impl: web-ui/src/styles/dashboard.css::.github-repo-rows --> <!-- @impl: web-ui/src/styles/dashboard.css::.storage-drop-zone --> <!-- @manual: On the protected integration deployment at a mobile viewport, confirm a short active face is vertically centered and long GitHub and Storage lists remain inside the capped face with internal scrolling. -->
...
**Verification:** Mixed: automated Dashboard and panel-allocation tests cover single-panel allocation and reactive face sizing; the protected integration procedures in AC1 and AC4 manually verify desktop anchors and mobile centering/scroll containment.
