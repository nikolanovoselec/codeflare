...
7. In onboarding or SaaS mode, Turnstile runs; finalization writes final state and marks setup complete. Inapplicable slots are omitted; enterprise extensions retain source order around canonical slots. <!-- @impl: src/routes/setup/index.ts::default --> <!-- @test: src/__tests__/setup-ac-coverage.test.ts (Setup AC Coverage) -->
...
1. Setup accepts an optional GitHub `owner/name` repository, repository-scoped read PAT, 64-hex Ed25519 public key, and enabled state in default, onboarding, SaaS, and Enterprise modes. <!-- @test: src/__tests__/routes/setup-managed-environment.test.ts (REQ-SETUP-013 AC1: every deployment mode accepts the same managed-environment boundary) -->
2. The PAT is encrypted with the existing AES-256-GCM KV boundary, never returned by prefill, and a blank value preserves the prior secret; prefill exposes only repository, key fingerprint, configured state, active sequence and digest prefix, freshness, expiry state, and bounded safe error. <!-- @test: src/__tests__/routes/setup-managed-environment.test.ts (REQ-SETUP-013 AC2: PAT storage and prefill preserve the secret boundary) -->
3. Enabling or replacing repository trust idempotently creates the deterministic deployment cache bucket with existing R2 credentials, primes the candidate configuration-fingerprint namespace with a complete verified release, and only then commits configuration; any bucket, fetch, identity, digest, signature, schema, sequence, ABI, path, or runtime failure leaves the prior configuration and active release unchanged. <!-- @impl: src/lib/remote-curation.ts::configureManagedEnvironment --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (stores the PAT only as AES ciphertext, preserves blanks, and commits replacement trust last) --> <!-- @test: src/__tests__/routes/setup-managed-environment.test.ts (REQ-SETUP-013 AC3: invalid replacement cannot change trusted configuration or active release) -->
4. Signing-key rotation is rotate-forward: Setup commits a new public key only after it verifies a higher release sequence signed by that key. <!-- @impl: src/lib/remote-curation.ts::configureManagedEnvironment --> <!-- @test: src/__tests__/routes/setup-managed-environment.test.ts (REQ-SETUP-013 AC4: signing-key replacement requires a higher verified sequence) -->
5. Disabling the feature retains verified cache history, marks baked reconciliation as expected, and never invokes user offboarding or destructive bucket cleanup. <!-- @impl: src/routes/setup/index.ts::default --> <!-- @test: src/__tests__/routes/setup-managed-environment.test.ts (REQ-SETUP-013 AC5: disabling curation schedules baked convergence without offboarding) -->
6. Repository authentication is sent only to `api.github.com`; release asset authorization is stripped before at most one redirect to the fixed GitHub object-host allowlist, and PAT bytes never enter logs, API responses, user R2, containers, or plaintext KV. <!-- @impl: src/lib/remote-curation.ts::downloadManagedAsset --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (downloads one exact allowed redirect without forwarding GitHub authorization) --> <!-- @test: src/__tests__/lib/remote-curation.test.ts (rejects an asset redirect outside the fixed GitHub object hosts) -->

**Constraints:** Setup remains configuration-only and cannot infer offboarding from omitted users. Production signing private keys remain outside Codeflare. The user-facing label is “Managed coding environment”; `remote-curation` remains an internal implementation term.

**Priority:** P1

**Dependencies:** [REQ-SETUP-005](#req-setup-005-post-setup-reconfiguration-requires-admin-auth), [REQ-SETUP-012](#req-setup-012-setup-wizard-step-sequence), [REQ-AGENT-147](agents.md#req-agent-147-signed-managed-agent-configuration-releases)
