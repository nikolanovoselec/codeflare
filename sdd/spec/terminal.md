...
2. Unknown control-message types are silently ignored so the wire protocol can grow without breaking older clients or servers. <!-- @impl: host/src/server.ts::wss --> <!-- @test: host/__tests__/ws-input-classification.test.js (WS input classification) -->
