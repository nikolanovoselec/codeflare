...
6. Attempting to create a seventh terminal in a session is rejected. <!-- @impl: host/src/session-manager.ts::SessionManager --> <!-- @test: host/__tests__/session-manager.test.js (seventh terminal rejected) --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (server "Session limit reached" close (server-side half)) -->
...
3. The terminal server spawns a login shell PTY with full-color terminal emulation so interactive TUI applications render correctly. <!-- @impl: host/src/session.ts::Session --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (login shell + xterm-256color env (host-side)) -->
4. Raw terminal data flows over the WebSocket without JSON wrapping so binary-clean PTY output is preserved. <!-- @impl: host/src/session.ts::Session --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (raw data send unwrapped (host-side)) -->
5. Out-of-band control messages (resize, process-name, restore, and client-requested PTY termination) are encoded as JSON objects identifiable by a leading type-discriminator field. <!-- @impl: host/src/session.ts::Session --> <!-- @impl: host/src/server.ts::wss --> <!-- @impl: web-ui/src/stores/terminal.ts::dispose --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (JSON control msgs + resize + type guard (host-side)) --> <!-- @test: web-ui/src/__tests__/stores/terminal.test.ts (client kill control frame) -->
6. Unknown control-message types are silently ignored so the wire protocol can grow without breaking older clients or servers. <!-- @impl: host/src/session.ts::Session --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (unknown control-type tolerance (host-side)) -->
7. No application-level ping/pong is implemented; the transport layer handles WebSocket keepalive on its own. <!-- @impl: host/src/session.ts::Session --> <!-- @test: host/__audits__/terminal-compound-key.audit.js (no app-level ping/pong (host-side)) -->
...
1. The Container DO passes the per-tab agent configuration to the terminal server at container start so the server knows which agent to launch in tab 1. <!-- @impl: host/src/prewarm-config.ts::getPrewarmConfig --> <!-- @test: host/__tests__/prewarm-readiness.test.js (tab 1 command extraction + first-token + ignore non-tab-1) --> <!-- @test: host/__tests__/server-prewarm.test.js (getPrewarmConfig TAB_CONFIG absent/empty + first-token) --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (tab-1 config propagation (server-boot half)) -->
2. Tab 1 is pre-warmed at container start: the terminal server spawns a dedicated pre-warm PTY whose login shell reads the user's shell init. <!-- @impl: host/src/session-manager.ts::SessionManager --> <!-- @test: host/__tests__/session-manager.test.js (PREWARM_SESSION_ID adoption + orphanTimeout clear) --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (server-boot sessions.set + prewarm PTY (server-boot half)) -->
3. The shell init reads the per-tab configuration and launches the configured agent (Claude Code, Codex, Antigravity, OpenCode, Copilot CLI, or Pi), each in non-interactive sandboxed mode appropriate for its CLI, or a plain bash shell when the tab is configured with no agent. <!-- @impl: entrypoint.sh::configure_tab_autostart --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (agent-launch dispatch wiring (server-boot half)) -->
4. Pre-warm readiness is detected by the first PTY output; a bounded hard timeout acts as a safety net so a permanently silent agent does not stall startup. <!-- @impl: host/src/session-manager.ts::SessionManager --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (readiness gate onData + 20s safety timeout (server-boot half)) -->
5. When the first WebSocket client connects for tab 1, the pre-warmed session is adopted (re-bound from the pre-warm identifier to the real terminal ID). If no client adopts it within a bounded window, the pre-warmed session is killed. <!-- @impl: host/src/session-manager.ts::SessionManager --> <!-- @test: host/__tests__/session-manager.test.js (adoption rename + orphan timeout kill) --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (adoption window orphan setTimeout (server-boot half)) -->
6. The startup status stage progresses through a fixed pipeline: starting -> syncing -> verifying -> mounting (pre-warm in progress, terminal canvas hidden) -> ready (pre-warm complete, "Open" control appears). <!-- @impl: host/src/session-manager.ts::SessionManager --> <!-- @test: host/__audits__/server-prewarm-lifecycle.audit.js (status pipeline terminalServiceReady (server-boot half)) -->
...
1. Tabs created by the user are marked manual in the tab configuration so downstream components can branch on the distinction. <!-- @impl: web-ui/src/stores/session-tabs.ts::addTerminalTab --> <!-- coverage-gap: frontend manual-flag marking in addTerminalTab has no behavioral test -->
2. The manual flag is propagated to the container via a query parameter on the WebSocket upgrade URL. <!-- @impl: web-ui/src/stores/terminal.ts::connect --> <!-- coverage-gap: ?manual=1 WS-URL query-param propagation in connect has no behavioral test -->
3. The terminal server exposes the manual flag to the PTY environment so the shell init can read it. <!-- @impl: host/src/session.ts::Session --> <!-- coverage-gap: session.ts MANUAL_TAB env exposure has no behavioral test (entrypoint test consumes the env, does not set it) -->
...
1. The terminal server emits process-name control messages over the WebSocket whenever the foreground process for a PTY changes. <!-- @impl: host/src/session.ts::Session --> <!-- coverage-gap: host process-name WS emit verified only by source-string-matching audit (terminal-process-name.test.ts); no behavioral emit test -->
2. The frontend distinguishes control messages from raw terminal data using the message's leading type-discriminator field. <!-- @impl: web-ui/src/stores/terminal.ts::connect --> <!-- coverage-gap: frontend control-message type discrimination verified only by source-string-matching audit; no behavioral parse test -->
...
6. The session store registers a process-name callback against the terminal store so process updates propagate without creating a circular import between the two stores. <!-- @impl: host/src/session.ts::Session --> <!-- coverage-gap: registerProcessNameCallback wiring verified only by source-string-matching audit; no behavioral no-circular-import test -->
