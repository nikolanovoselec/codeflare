...
4. Opening a ready terminal session reconnects any visible pane whose startup socket disconnected while the readiness overlay remained open, without replacing a healthy connection. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/stores/terminal.ts::reconnectDisconnectedTerminals --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-TERM-043 AC4: OPEN reconnects a terminal that disconnected behind the ready overlay) --> <!-- @test: web-ui/src/__tests__/stores/terminal.test.ts (REQ-TERM-043 AC4: reconnects only disconnected visible panes for the opened session) -->
