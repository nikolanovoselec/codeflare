...
4. Opening a ready terminal session restores current host screen state for every visible pane even when the browser retained a stale connected attachment. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/stores/terminal.ts::restartVisibleTerminals --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-TERM-043 AC4: OPEN replaces visible terminal attachments and dismisses readiness) --> <!-- @test: web-ui/src/__tests__/stores/terminal.test.ts (REQ-TERM-043 AC4: OPEN replaces every visible attachment and replays current host state) -->
