...
7. Output held past the cap discards its oldest data; held output is never written while manual ownership is active. <!-- @impl: web-ui/src/stores/terminal.ts::flushWriteBuffer --> <!-- @test: web-ui/src/__tests__/stores/terminal.test.ts (REQ-TERM-014 AC7: a cap-exceeding hold drops oldest chunks and never writes through a reader) -->
8. User input of any route while reading normal-buffer scrollback re-anchors the viewport to the live bottom; alternate-buffer applications are unaffected. <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @test: web-ui/src/__tests__/hooks/useTerminal.test.ts (REQ-TERM-014 AC8: user input while reading scrollback re-anchors the viewport to the live bottom) -->
9. Mouse-wheel scrollback navigation in the normal buffer moves the viewport by exactly the wheel delta; alternate-buffer and zoom-modified wheel events pass through to the application untouched. <!-- @impl: web-ui/src/lib/terminal-wheel.ts::attachWheelScrolling --> <!-- @test: web-ui/src/__tests__/lib/terminal-wheel.test.ts (terminal-wheel / REQ-TERM-014 AC9 buffer-authoritative wheel scrolling) -->

**Constraints:**

- The write buffer either defers, drops oldest held data, or writes; it never scrolls the viewport.
- Output-driven trimming stays delegated to xterm.
- User-driven scrollback navigation (touch gestures, mouse wheel, floating page controls) and every app-initiated bottom anchor scroll the buffer service directly with buffer-derived deltas and issue the paired viewport repaint ([AD105](../../documentation/decisions/README.md#ad105-streamed-output-defers-while-the-user-reads-scrollback-keyboard-open-swipes-are-always-terminal-input), [AD110](../../documentation/decisions/README.md#ad110-terminal-scrolling-is-buffer-authoritative-on-every-route-held-output-ring-drops)).
- Held output is capped at 2,000,000 characters with ring semantics: past the cap the oldest whole held chunks are dropped at message boundaries.
- Bottom-return release is bounded to 65,536 characters of whole chunks per flush tick, re-checking ownership between ticks.
- xterm's built-in input scrolling (`scrollOnUserInput`) is disabled; input re-anchoring is app-owned through the buffer service.
- A refit that does not re-anchor to the bottom re-commands the viewport's DOM scroll state from the buffer position.
