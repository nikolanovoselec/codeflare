...
4. Opening a ready terminal session immediately restarts any visible pane whose startup socket is disconnected or still connecting behind the readiness overlay, without replacing a healthy connection. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/stores/terminal.ts::reconnectOnVisibilityReturn --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-TERM-043 AC4: OPEN restarts a visible terminal stuck connecting behind the ready overlay) --> <!-- @test: web-ui/src/__tests__/stores/terminal.test.ts (REQ-TERM-043 AC4: restarts visible disconnected and connecting panes when OPEN dismisses readiness) -->
5. A terminal startup that reaches `ready` clears its readiness overlay and exposes the terminal without requiring an Open click or page reload. <!-- @impl: web-ui/src/stores/session.ts::startSession --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-TERM-043 AC5: ready terminal startup clears its overlay without OPEN or reload) -->
