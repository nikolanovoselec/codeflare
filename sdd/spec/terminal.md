...
7. When overlapping reconnects delay release of resize authority, the successor pane's latest dimensions are retained without taking authority early and applied to both the PTY and reconnect state as soon as authority transfers. <!-- @impl: host/src/session.ts::Session.resize --> <!-- @impl: host/src/session.ts::Session.detach --> <!-- @test: host/__tests__/session-resize-authority.test.js (REQ-TERM-016: applies a replacement pane resize when authority transfers after overlapping reconnects) -->
