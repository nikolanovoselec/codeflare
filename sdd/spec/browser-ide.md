...
**Intent:** A Pi-selected advanced session uses Codeflare as the account-free default entry point in the existing editor Inline Chat refactor area, with unrestricted turns continued into visible panel Chat before Pi inference.
...
**Intent:** Editor-originated requests retain Codeflare's bounded context and persistent unrestricted IDE-owned Pi runtime after the pinned host continues them into visible panel Chat, without presenting the selected range as a sandbox or rollback boundary.
