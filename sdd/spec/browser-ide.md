...
4. HTTP responses carry the embeddable-frame security headers (`frame: sameorigin`, `csp: false`); WebSocket upgrade responses are passed through untouched (status 101 no-op). <!-- @impl: src/index.ts --> <!-- @test: src/__tests__/security/early-return-security.test.ts (CF-001: security headers on pre-Hono early-return responses) -->
