...
**Intent:** Routine local IDE-agent edits and commands run without per-action interruption or editor-tab churn, while external and MCP actions retain bounded request-specific confirmation.

**Applies To:** User

**Acceptance Criteria:**

1. Pi edit, write, and Bash calls retain provenance, workspace-path, stale-content, and manifest validation but auto-confirm in the extension host without opening editor documents; unknown and MCP actions use one modal confirmation. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-007: Pi sidebar guarded approvals) --> <!-- @test: openvscode/agent-sidebar/test/vscode-approval-host.test.ts (REQ-IDE-007 AC1: routine local actions need no confirmation and open no editor tabs) -->
2. Official Claude remains in Anthropic Manual (`"default"`) mode with bypass and automatic modes disabled; Edit, Write, NotebookEdit, Bash, and Task auto-allow, while network, unknown, MCP, and Jupyter execution actions ask and read-only diagnostics proceed. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/pre-tool-use-permission.mjs::evaluatePreToolUse --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOpenVscodeSettings --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-007 AC2: native permission rules ask only for external and MCP actions) --> <!-- @test: openvscode/claude/test/pre-tool-use-permission.test.mjs (REQ-IDE-007 AC2: pre-tool-use auto-allows routine local Edit) --> <!-- @test: openvscode/claude/test/pre-tool-use-permission.test.mjs (REQ-IDE-007 AC2: pre-tool-use returns interactive ask for mcp__ide__executeCode) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-007 AC2: official Claude restart restores the externally managed safe settings) -->
3. A guarded Pi action opens no editor document; absent, rejected, cancelled, expired, stale, malformed, oversized, or unrelated confirmation leaves targets unchanged and starts no command, and a late modal answer cannot enter another request. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-007 AC2: an oversized serialized preview is denied before approval or mutation) --> <!-- @test: openvscode/agent-sidebar/test/approval-bridge.test.ts (REQ-IDE-007 AC2 + REQ-IDE-008 AC1: cancellation denies a pending approval and ignores its late modal answer) --> <!-- @test: openvscode/agent-sidebar/test/vscode-approval-host.test.ts (REQ-IDE-007 AC3: remaining guarded actions use one modal and no editor tab) -->
...
- Confirmation governs external, unknown, and MCP agent tool calls; routine local tools remain constrained by provenance, path, stale-content, and manifest validation.
