...
3. Codeflare requests no model authorization, and panel and editor requests invoke local Pi instead of either compatibility model's generation path. The hidden Copilot-vendor fallback exists only for the pinned extension host's absent-request-model lookup. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_FALLBACK_MODEL --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_VISIBLE_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::NativePiRuntime --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-005 AC5 + REQ-IDE-013 AC1 + REQ-IDE-019 AC2+AC3: native Pi registers account-free panel and editor Chat) -->
4. The Pi inventory disables the duplicate Claude personal-agent discovery path while retaining the Copilot-format personal-agent path, so each Codeflare custom agent appears once in Code OSS's Agent selector. <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-018 + REQ-IDE-019 AC4: Pi native Chat uses notifications and one personal agent source) -->
...
- The globally unique `codeflare` vendor keeps the selectable model outside the host's Copilot entitlement/setup path; tool-calling metadata remains pinned-host eligibility metadata, not a second inference path.
