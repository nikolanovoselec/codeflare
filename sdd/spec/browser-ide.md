...
5. Pi native Chat publishes one hidden, account-free, non-generating local model only to satisfy the pinned host's mandatory request-model boundary; the participant never sends a prompt to that model or reads `request.model`, and all inference still uses the fixed local Pi RPC backend without a VS Code authentication provider or login. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_COMPATIBILITY_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::FIXED_PI_SPAWN_SPEC --> <!-- @test: openvscode/agent-sidebar/test/packaging.test.ts (REQ-IDE-005 AC2: contributes Codeflare as the default native Pi Chat participant) --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-005 AC4 + REQ-IDE-006 AC1+AC3: visible Pi resolution uses only the fixed no-session spawn contract) -->
