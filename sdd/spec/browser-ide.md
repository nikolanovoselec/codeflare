...
### REQ-IDE-037: Lazy extension restoration and warning acknowledgement

**Intent:** Persisted user-extension intent restores safely after workbench startup without executing unacknowledged code or losing settings and failed intent.

**Applies To:** User

**Acceptance Criteria:**

1. Each missing manifest entry first installs with its exact `id@version` selector. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-016 AC3 + REQ-IDE-037 AC1+AC3+AC4: restores exact versions, falls back once, and preserves failures) -->
2. Missing-extension installation runs with at most two concurrent workers. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-037 AC2: restores at most two missing extensions concurrently) -->
3. Only a structured exact-version not-found result permits one unpinned fallback for that extension. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-016 AC3 + REQ-IDE-037 AC1+AC3+AC4: restores exact versions, falls back once, and preserves failures) -->
4. A failed restore leaves that manifest entry unchanged while remaining workers continue. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-016 AC3 + REQ-IDE-037 AC1+AC3+AC4: restores exact versions, falls back once, and preserves failures) -->
5. Contributed global settings restore only after missing extensions have registered their configuration keys. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-037 AC5: contributed settings restore after their missing extension registers) -->
6. A non-empty manifest without `securityWarningShown: true` cannot execute restored extension code until the user accepts the warning and the acknowledgement is atomically persisted. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-037 AC6: unacknowledged manifest intent never executes before the warning is accepted) -->
7. A persisted warning acknowledgement prevents repeat warnings across later restores, captures, removals, and fresh activations. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::restoreExtensionManifest --> <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::captureExtensionManifest --> <!-- @test: openvscode/agent-sidebar/test/extension-persistence.test.ts (REQ-IDE-036 AC4 + REQ-IDE-037 AC7: warns once before the first persisted user extension across fresh activations) -->
...
- Restore has no retry loop and does not resolve target platforms independently of code-server.
...
**Verification:** GitHub Actions runs exact-version/fallback, concurrency, failure-preservation, settings-ordering, acknowledgement, welcome-activation, and complete-image tests without local, browser-automation, Chromium, or manual gates.
