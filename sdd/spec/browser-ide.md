...
4. A VS Code workbench waits for restored terminals, then activates the existing `Codeflare Session Agent` profile exactly once or creates it when absent. <!-- @impl: openvscode/agent-sidebar/src/welcome-extension.ts::activate --> <!-- @test: openvscode/agent-sidebar/test/welcome-extension.test.ts (REQ-IDE-048 AC4: restored terminal hydration wins before activation creates a replacement) --> <!-- @test: openvscode/agent-sidebar/test/welcome-extension.test.ts (REQ-IDE-048 AC4: reconnect creates exactly one managed session-agent terminal) -->
