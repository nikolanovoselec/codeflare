...
3. A Claude session presents Anthropic's exact unmodified Claude Code extension while OpenVSCode's unrelated native Chat and Copilot setup remain disabled. <!-- @impl: openvscode/agent-sidebar/official-claude.json::sha256 = 63a3178c906684ccee40cc2190f2c72bfbed7224dd8ec1af9046e51f7a1f8eed --> <!-- @impl: Dockerfile::CLAUDE_VSCODE_SHA256 --> <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @test: openvscode/agent-sidebar/test/packaging.test.ts (REQ-IDE-005 AC1+AC3: stages native Pi, official Claude, and empty unsupported inventories) --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-005 AC3: Claude suppresses unrelated native Chat setup) -->
...
- Codeflare does not bridge, persist, or depend on VS Code Authentication; upstream manual Accounts sessions remain memory-only for the current workbench.
