...
4. A stopped VS Code session starts on the dashboard, and Open remains an explicit action enabled only after editor readiness; readiness failure preserves retry, stop, and delete actions. <!-- @impl: src/routes/container/status.ts::app --> <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer --> <!-- @impl: web-ui/src/components/Dashboard.tsx::vscodeWorkspaceAction --> <!-- @test: src/__tests__/routes/container-status.test.ts (REQ-IDE-048 AC4: reports VS Code ready and mirrors readiness into session metadata) --> <!-- @test: src/__tests__/routes/container-lifecycle-helpers.test.ts (clears stale readiness when KV says running but the container is stopped) --> <!-- @test: web-ui/src/__tests__/stores/session.test.ts (REQ-IDE-048 AC2 + AC4: completes VS Code startup without creating terminal state) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-IDE-048 AC4: editor failure exposes Retry plus Stop and Delete operations) -->
