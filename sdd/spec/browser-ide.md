...
3. A Claude session presents Anthropic's exact official Claude Code extension, configured externally without modifying its package, while unsupported sessions contain no agent extension. <!-- @impl: openvscode/agent-sidebar/official-claude.json --> <!-- @impl: Dockerfile --> <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @test: openvscode/agent-sidebar/test/packaging.test.ts (REQ-IDE-005 AC1+AC2: stages native Pi, official Claude, and empty unsupported inventories) -->
4. Neither Pi nor Claude starts an agent process before a native Chat request or Claude panel request; every Pi request uses one fresh isolated backend. <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::runNativePiChat --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @test: openvscode/agent-sidebar/test/native-chat.test.ts (REQ-IDE-005 AC4 + REQ-IDE-006 AC3: each native Chat request uses and reaps a fresh isolated Pi backend) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC4: a native Pi turn streams only assistant text, reports tool progress, and completes at agent_settled) -->
5. Pi native Chat calls the local Pi RPC backend without invoking a VS Code language-model or authentication provider and therefore requires no Microsoft, GitHub, Copilot, or Anthropic login. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::FIXED_PI_SPAWN_SPEC --> <!-- @test: scripts/ci/smoke-openvscode-sidebar-image.mjs (packaged native Chat registration rejects authentication and language-model access) -->
6. In the pinned complete image, OpenVSCode positively discovers both fixed extension IDs, packaged Pi activation registers the default participant without account/model APIs, and official Claude retains its exact identity, version, platform, permission, and process-laziness contracts. <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::verifyPackagedNativeChat --> <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::verifyOfficialClaudeExtension --> <!-- @test: .github/workflows/test.yml (Browser IDE complete image) -->
...
- The owner accepts bundling the exact unmodified official Anthropic VSIX from Open VSX in the server image despite its all-rights-reserved license notice. Codeflare neither patches nor serves the archive.
...
2. Official Claude receives active-file, selection, native diff, and diagnostics integration through Anthropic's authenticated loopback-only IDE MCP server. <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOfficialClaudeIde --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-005 AC2 + REQ-IDE-006 AC1: official Claude launch writes isolated OpenVSCode settings) --> <!-- @test: openvscode/claude/test/pre-tool-use-permission.test.mjs (REQ-IDE-005 AC2: official Claude may read native editor diagnostics without an approval round trip) -->
...
- Anthropic's IDE MCP binds only to `127.0.0.1` on a random port, uses a fresh token in the isolated mode-0700 config directory, and exposes no Codeflare-owned relay or public listener. This is an explicit owner-approved exception to the earlier no-loopback/no-MCP design.
...
**Intent:** Guarded IDE-agent actions require a bounded, request-specific user decision before any mutation or command starts.

**Applies To:** User

**Acceptance Criteria:**

1. A guarded Pi action changes a target or starts a command only after explicit confirmation through the owning OpenVSCode extension host. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-007: Pi sidebar guarded approvals) --> <!-- @test: openvscode/agent-sidebar/test/approval-bridge.test.ts (REQ-IDE-007 AC1: Pi approval resolves through extension-host manifest, diff, and confirmation authority) -->
2. Official Claude starts and restarts in Manual mode with bypass and automatic edit modes disabled; mutating built-ins and MCP tools ask, read-only IDE diagnostics may proceed, and Jupyter execution retains Anthropic's native per-call confirmation. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/pre-tool-use-permission.mjs::evaluatePreToolUse --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOpenVscodeSettings --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-007 AC1: native permission rules independently ask for guarded built-ins and MCP) --> <!-- @test: openvscode/claude/test/pre-tool-use-permission.test.mjs (REQ-IDE-007 AC1: pre-tool-use returns interactive ask for mcp__ide__executeCode) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-007 AC2: official Claude restart restores the externally managed safe settings) --> <!-- @test: host/__tests__/entrypoint-openvscode.test.js (REQ-IDE-007 AC2: Claude preparation failure prevents OpenVSCode launch) -->
3. If a Pi preview is oversized or confirmation is absent, rejected, cancelled, expired, stale, malformed, or unrelated to the request, the target remains unchanged and no command starts; a late modal answer cannot enter another request. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-007 AC2: an oversized serialized preview is denied before approval or mutation) --> <!-- @test: openvscode/agent-sidebar/test/approval-bridge.test.ts (REQ-IDE-007 AC2 + REQ-IDE-008 AC1: cancellation denies a pending approval and ignores its late modal answer) -->
...
**Intent:** Cancellation, failure, editor restart, and shutdown leave no pending approval or surviving IDE-agent process.

**Applies To:** User

**Acceptance Criteria:**

1. Cancelling during Pi startup sends no prompt; after prompt acceptance, cancellation sends Pi's correlated abort, denies a pending approval, waits for the durable `agent_settled` boundary, and reaps that request's process generation. <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::runNativePiChat --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @test: openvscode/agent-sidebar/test/native-chat.test.ts (REQ-IDE-008 AC1+AC3: native Chat cancellation is registered before the Pi request and cleanup still runs) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-008 AC1: cancellation during Pi startup cannot send a prompt after spawn completes) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC4: a native Pi turn streams only assistant text, reports tool progress, and completes at agent_settled) -->
2. Every native Pi request owns a fresh process generation; completion, failure, or extension deactivation stops and reaps every active generation without permitting a disposed session to respawn. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::deactivate --> <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::PiSession --> <!-- @impl: openvscode/agent-sidebar/src/process-generation.ts::reapSidebarGeneration --> <!-- @test: openvscode/agent-sidebar/test/native-chat.test.ts (REQ-IDE-005 AC4 + REQ-IDE-006 AC3: each native Chat request uses and reaps a fresh isolated Pi backend) --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-008 AC2: Pi disposal settles and reaps the request generation) --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-008 AC2+AC3: a disposed request session cannot spawn a replacement child) -->
3. Asynchronous Pi start, input, approval, output, or exit callbacks cannot survive, resurrect, or alter a completed or cancelled request. <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-008 AC2+AC3: disposal during Pi startup cannot resurrect the backend) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-008 AC3: an asynchronous Pi stdin failure stops the backend without escaping) --> <!-- @test: openvscode/agent-sidebar/test/approval-bridge.test.ts (REQ-IDE-007 AC2 + REQ-IDE-008 AC1: cancellation denies a pending approval and ignores its late modal answer) -->
