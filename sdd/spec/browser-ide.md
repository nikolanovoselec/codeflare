...
1. The Worker parses `/api/vscode/<sessionId>/...` at the route boundary before the Hono router (so WebSocket upgrades pass through), and rejects a first segment that fails `SESSION_ID_PATTERN` with a 400. <!-- @impl: src/routes/vscode-validation.ts::validateVscodeRoute --> <!-- @test: src/__tests__/routes/vscode-validation.test.ts (REQ-IDE-001: a valid session id yields isVscodeRoute with the remaining path; a malformed first segment returns a 400 errorResponse) -->
2. A vscode request passes the shared auth chain -- origin allowlist, then authenticate (with CSRF synthesis), then active-tier -- reusing the vault's session-safe guards; an unauthenticated request returns 401 and a disallowed origin returns 403. <!-- @impl: src/routes/vscode.ts::handleVscodeRequest --> <!-- @test: src/__tests__/routes/vscode-auth-chain.test.ts (REQ-IDE-001: the auth chain passes for an owned running session; unauthenticated -> 401, origin-not-allowed -> 403) -->
3. The Worker forwards the request to the session's container via `container.fetch` with the path unchanged (`/api/vscode/<sessionId>/...`) so OpenVSCode's `--server-base-path` matches; the container fetch wrapper supplies the container-auth Bearer. <!-- @impl: src/routes/vscode.ts::handleVscodeRequest --> <!-- @test: src/__tests__/routes/vscode-auth-chain.test.ts (REQ-IDE-001: the forwarded Request pathname equals the original /api/vscode/<sid>/... with no rewrite) -->
4. HTTP responses carry the embeddable-frame security headers (`frame: sameorigin`, `csp: false`); WebSocket upgrade responses are passed through untouched (status 101 no-op). <!-- @impl: src/index.ts --> <!-- @test: src/__tests__/security/early-return-security.test.ts (REQ-IDE-001 AC4: the vscode proxy response carries X-Frame-Options SAMEORIGIN + frame-ancestors CSP, not default-src none; the 101 no-op is the shared withSecurityHeaders test) -->
5. The host forwards `/api/vscode/*` to OpenVSCode on `127.0.0.1:13337` without stripping the prefix, so the base-path-native server receives its own path. <!-- @impl: host/src/vscode-proxy.ts::isVscodePath --> <!-- @test: host/__tests__/openvscode-proxy.test.js (REQ-IDE-001: isVscodePath matches /api/vscode/<sid>/... and the upstream path equals the input path, no strip) -->
6. Ownership and liveness are enforced after the auth chain: a session the user does not own returns 404, a stopped session returns 503, and an unhealthy container returns 503. <!-- @impl: src/routes/vscode.ts::handleVscodeRequest --> <!-- @test: src/__tests__/routes/vscode-auth-chain.test.ts (REQ-IDE-001: unowned session -> 404, stopped session -> 503, container-not-ready -> 503) -->
...
1. The OpenVSCode supervisor launches the server only after the init-complete flag exists AND a first-request trigger file is present; until both are present it waits without launching. <!-- @impl: entrypoint.sh::start_openvscode_supervisor --> <!-- @test: host/__tests__/entrypoint-openvscode.test.js (REQ-IDE-003: the supervisor loop does not exec the server until the init flag and the request-trigger file both exist) -->
2. The host writes the request-trigger file on the first `/api/vscode` request (idempotent) and, until OpenVSCode binds, serves a 503 auto-refreshing HTML warming page (not a raw JSON error) so a plain browser tab lands on the editor once it is up. <!-- @impl: host/src/server.ts --> <!-- @impl: host/src/vscode-proxy.ts::vscodeWarmingResponse --> <!-- @test: host/__tests__/openvscode-proxy.test.js (REQ-IDE-003: a /api/vscode request writes the trigger file; the warming response is a 503 auto-refreshing HTML page, not the old JSON payload) -->
...
5. The header IDE button renders only when the active session is advanced-mode AND running; clicking it opens the session's `/api/vscode/<sessionId>/`. <!-- @impl: web-ui/src/components/Layout.tsx --> <!-- @impl: web-ui/src/components/Header.tsx --> <!-- @test: web-ui/src/__tests__/components/Layout.test.tsx (REQ-IDE-003 Browser IDE button gating: onVscodeOpen passed only for an advanced running session, absent for default mode or a not-yet-running (initializing) session; the handler opens the session-keyed /api/vscode/<sid>/) --> <!-- @test: web-ui/src/__tests__/components/Header.test.tsx (Browser IDE button: renders iff the handler is provided, click invokes it) -->
