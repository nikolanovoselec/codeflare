...
3. A Claude session presents Anthropic's exact official Claude Code extension, configured externally without modifying its package, while unsupported sessions contain no agent extension. <!-- @impl: openvscode/agent-sidebar/official-claude.json::sha256 = 63a3178c906684ccee40cc2190f2c72bfbed7224dd8ec1af9046e51f7a1f8eed --> <!-- @impl: Dockerfile::CLAUDE_VSCODE_SHA256 --> <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @test: openvscode/agent-sidebar/test/packaging.test.ts (REQ-IDE-005 AC1+AC3: stages native Pi, official Claude, and empty unsupported inventories) -->
...
5. OpenVSCode can enter Codeflare Pi native Chat without account authorization; its panel-only compatibility model is hidden from user selection and rejects generation. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_COMPATIBILITY_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @test: scripts/ci/smoke-openvscode-sidebar-image.mjs (verifyPackagedNativeChat) -->
6. In the pinned complete image, OpenVSCode positively discovers both fixed extension IDs, packaged Pi activation registers the inert host model and default participant without account access, and official Claude retains its exact identity, version, platform, permission, and process-laziness contracts. <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::verifyPackagedNativeChat --> <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::verifyOfficialClaudeExtension --> <!-- @manual: Run the Browser IDE complete-image job and retain its extension-discovery, package-identity, permission, and process-laziness evidence artifact. -->
7. Codeflare Pi answers native Chat without depending on the editor's selected model or any Microsoft, GitHub, Copilot, or Anthropic login. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::NativePiRuntime --> <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::FIXED_PI_SPAWN_SPEC --> <!-- @test: scripts/ci/smoke-openvscode-sidebar-image.mjs (verifyPackagedNativeChat) --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-005 AC4 + REQ-IDE-006 AC1+AC3: visible Pi resolution uses only the fixed no-session spawn contract) -->
...
- The local Pi compatibility model is not user-selectable, requires no authorization, and fails closed if any caller attempts to use it for generation.
...
2. Official Claude starts and restarts in Anthropic Manual (`"default"`) permission mode with bypass and automatic edit modes disabled; mutating built-ins and MCP tools ask, read-only IDE diagnostics may proceed, and Jupyter execution retains Anthropic's native per-call confirmation. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/pre-tool-use-permission.mjs::evaluatePreToolUse --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOpenVscodeSettings --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-007 AC2: native permission rules independently ask for guarded built-ins and MCP) --> <!-- @test: openvscode/claude/test/pre-tool-use-permission.test.mjs (REQ-IDE-007 AC2: pre-tool-use returns interactive ask for mcp__ide__executeCode) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-007 AC2: official Claude restart restores the externally managed safe settings) --> <!-- @test: host/__tests__/entrypoint-openvscode.test.js (REQ-IDE-007 AC2: Claude preparation failure prevents OpenVSCode launch) -->
