...
2. A vscode request passes the shared auth chain (authenticate -> origin -> active tier -> session ownership); a session the user does not own returns 404 and a stopped session returns 503. <!-- @impl: src/routes/vscode.ts::handleVscodeRequest --> <!-- @test: src/__tests__/routes/vscode-auth-chain.test.ts (REQ-IDE-001: unauthenticated -> 401, origin-not-allowed -> 403, unowned session -> 404, stopped session -> 503, container-not-ready -> 503) -->
...
**Dependencies:** [REQ-VAULT-005](vault.md) (Worker vault proxy plumbing), [REQ-SESSION-004](session-lifecycle.md) (container health gate)
