...
2. Supported sidebars expose only Codeflare-owned functionality, unsupported selections expose no sidebar, and no Anthropic extension is present. <!-- @impl: entrypoint.sh::_openvscode_launch_once --> <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @test: host/__tests__/entrypoint-openvscode.test.js (REQ-IDE-005 AC1+AC2: tab one selects only a fixed sidebar agent inventory) --> <!-- @test: openvscode/agent-sidebar/test/packaging.test.ts (REQ-IDE-005 AC1+AC2: stages only the fixed Pi, Claude, and empty inventories) -->
3. Before the selected sidebar is visible, no sidebar agent runs; while it is visible, exactly one matching backend runs and the non-selected backend never starts. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/lifecycle.ts::SidebarLifecycle --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-005 AC3: activation is inert until visible resolution starts the selected backend) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-005 AC3: a visibility-triggered start failure is reported without escaping) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-005 AC3: Claude selection never constructs the Pi backend) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC3+AC7: an asynchronous Pi spawn failure cannot leave a running backend) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC3: cycle-thinking renders the correlated Pi response level) -->
4. Both backends share the current session workspace and approved credentials, routing, and configuration without copying secret values into settings, logs, reports, or webview messages. <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::FIXED_PI_SPAWN_SPEC --> <!-- @impl: openvscode/agent-sidebar/src/claude/pty-session.ts::ClaudePtySession --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareSidebarConfig --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-005 AC3+AC4+AC5: visible Pi resolution uses only the fixed no-session spawn contract) --> <!-- @test: openvscode/agent-sidebar/test/claude-pty.test.ts (REQ-IDE-005 AC3+AC4+AC5: Claude starts only the fixed no-shell PTY contract) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-005: projection links only allowlisted configuration and never copies secret bytes) -->
5. A sidebar conversation cannot list, attach to, or resume terminal conversation history; every new sidebar conversation starts with sidebar-only state. <!-- @impl: openvscode/agent-sidebar/src/pi/session.ts::PiSession --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::SIDEBAR_LINK_ALLOWLIST --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-005 AC3+AC4+AC5: visible Pi resolution uses only the fixed no-session spawn contract) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-005: projection excludes terminal history, runtime state, source settings, and unknown entries) --> <!-- @test: openvscode/claude/test/prepare-sidebar-config.test.mjs (REQ-IDE-005: projection rejects an allowlisted source entry redirected by a symbolic link) -->
6. A guarded sidebar action executes only after confirmation through its designated non-webview approval authority; absent, rejected, stale, or forged approval leaves the target unchanged and starts no command. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-005: Pi sidebar guarded approvals) --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-005: mutation boundary cannot follow a parent symlink swapped in after approval) --> <!-- @test: src/__tests__/lib/pi-sidebar-approval.test.ts (REQ-IDE-005: a failed staged replacement leaves the approved existing file intact) --> <!-- @test: openvscode/agent-sidebar/test/approval-bridge.test.ts (REQ-IDE-005 AC6: Pi approval resolves through extension-host manifest, diff, and confirmation authority) --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-005: native permission rules independently ask for guarded built-ins and MCP) --> <!-- @test: openvscode/agent-sidebar/test/webview-security.test.ts (REQ-IDE-005 AC6: webview messages cannot forge approval or choose process authority) -->
7. Abort, new conversation, extension deactivation, editor crash or restart, and session stop end pending sidebar work and remove all sidebar descendants before any replacement starts. <!-- @impl: entrypoint.sh::_openvscode_supervise_loop --> <!-- @impl: entrypoint.sh::kill_pidfile_subtree --> <!-- @impl: openvscode/agent-sidebar/src/process-generation.ts::reapSidebarGeneration --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @impl: openvscode/agent-sidebar/src/claude/node-pty-backend.ts::ClaudePtyBackend --> <!-- @test: host/__tests__/entrypoint-openvscode.test.js (OpenVSCode launch generations / REQ-IDE-005 AC7) --> <!-- @test: openvscode/agent-sidebar/test/process-generation.test.ts (REQ-IDE-005 AC7: one sidebar generation reaps a TERM-ignoring descendant in another process group) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC3+AC7: an asynchronous Pi stdin failure stops the backend without escaping) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC5+AC7: a late Pi approval response cannot enter a replacement conversation) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-005 AC5+AC7: a stale Claude exit callback cannot stop a replacement conversation) --> <!-- @test: openvscode/agent-sidebar/test/pi-session.test.ts (REQ-IDE-005 AC5+AC7: new Pi conversation reaps the old no-session process before replacement) --> <!-- @test: openvscode/agent-sidebar/test/claude-pty.test.ts (REQ-IDE-005 AC7: new Claude conversation reaps the old PTY before replacement) -->

**Constraints:**

- The sidebar is available only in advanced sessions and only for Pi or Claude.
- Codeflare owns the sidebar integration; Anthropic's proprietary extension is neither installed nor redistributed.
