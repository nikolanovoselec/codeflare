...
4. Healthy editor metrics and successful editor traffic reassert readiness from the latest session record, clearing a stale readiness error without overwriting concurrent session fields. <!-- @impl: src/container/container-metrics.ts::doCollectMetrics --> <!-- @impl: src/routes/vscode.ts::handleVscodeRequest --> <!-- @test: src/__tests__/container-metrics.test.ts (REQ-IDE-049 AC4: reasserts readiness without rolling back a concurrent session update) --> <!-- @test: src/__tests__/routes/vscode-auth-chain.test.ts (REQ-IDE-049 AC4: successful editor traffic reasserts readiness without losing concurrent session fields) -->
...
1. Whole-card interaction starts stopped or failed VS Code sessions, does nothing while preparing or stopping, and opens a running session only after editor readiness; no separate Open button is rendered. Its readiness dot is green when ready, yellow while preparing or stopping, gray when stopped, and red on error, independent of terminal WebSocket state. <!-- @impl: src/routes/container/status.ts::app --> <!-- @impl: web-ui/src/components/Dashboard.tsx::handleSessionSelect --> <!-- @impl: web-ui/src/components/SessionStatCard.tsx::SessionStatCard --> <!-- @test: src/__tests__/routes/container-status.test.ts (REQ-IDE-050 AC1: reports VS Code ready and mirrors readiness into session metadata) --> <!-- @test: web-ui/src/__tests__/components/Dashboard.test.tsx (REQ-IDE-050 AC1: opens from the whole card only after editor readiness) --> <!-- @test: web-ui/src/__tests__/components/SessionStatCard.test.tsx (REQ-IDE-050 AC1: maps VS Code readiness %o to %s) --> <!-- @test: web-ui/src/__tests__/components/SessionStatCard.test.tsx (REQ-IDE-050 AC1: makes a ready VS Code card the only Open interaction surface) -->
