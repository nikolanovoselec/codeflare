...
5. A stationary Herdr tap opens mobile input when the keyboard is closed and remains usable while it is open. <!-- @impl: web-ui/src/lib/terminal-mobile-input.ts::focusMobileTerminal --> <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @test: web-ui/src/__tests__/lib/touch-gestures.test.ts (REQ-MOB-020 AC5: forwards a keyboard-open tap through sub-threshold jitter) --> <!-- @test: web-ui/src/__tests__/hooks/useTerminal.test.ts (wires a trusted touch tap to same-cell SGR input before mobile focus) --> <!-- @manual: On Samsung Internet with the keyboard closed and open, tap Pi input and Herdr controls and confirm each activates once without selection. -->
