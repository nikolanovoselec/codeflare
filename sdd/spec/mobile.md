...
6. xterm 6's overlay scrollbar widget (the ScrollableElement `.scrollbar` container + `.slider` thumb) is removed so its per-render thumb-resize/fade does not flicker during streaming output; wheel, keyboard, and touch scrolling (all JS-driven by xterm's own scroll layer) remain fully functional — only the draggable thumb + visual position indicator are dropped. <!-- @impl: web-ui/src/styles/terminal.css --> <!-- coverage-gap: CSS hide of a third-party overlay widget; asserting the rule text would be CSS text-matching theater, so no genuine unit test (same posture as AC1/AC2 above) -->
