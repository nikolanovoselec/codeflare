...
## 2026-04-26
- Re-architected Stop hook push detection to use `git reflog` as the source of truth + a per-repo checkpoint (`.git/sdd-last-ack-push`) tracking the high-water mark of acknowledged pushes. Loose transcript regex remains as the candidate signal but no longer triggers false positives — `echo "git push"`, `gh pr create` bodies containing the phrase, `cat docs.md` with push examples, and similar text-only mentions are now silently ignored because the reflog has no matching `update by push` entry. The "which push got reviewed" connection is the temporal ordering: agent spawns must have transcript timestamps strictly greater than the latest un-acknowledged push timestamp, so each new push raises the bar above prior reviews. Per-repo isolation: state files live in each repo's `.git/`, parallel sessions in different repos run independent enforcement loops.
