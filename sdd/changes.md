...
## 2026-05-03
- SDD review pipeline switched from per-push to per-PR-boundary triggers (REQ-AGENT-021 AC4): code-reviewer + spec-reviewer + doc-updater now fire on PR open or on push to a branch with an open PR. Direct pushes to shared branches in `sdd_review.protected_branches` (default `main`, `master`) surface a non-blocking warning via `warn-direct-push-to-shared.sh` instead of running the full pipeline; pushes to feature branches without an open PR defer review until the PR opens.
- REQ-AGENT-005 AC4 hook list expanded to register the new warning hook alongside the PR-boundary classifier, and a new `sdd_review` section in `sdd/config.yml` configures `protected_branches` and `warn_on_direct_push`.
