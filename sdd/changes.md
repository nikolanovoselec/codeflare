...
## 2026-05-12
- REQ-AGENT-022 promoted to Implemented after the framework-audit fix wave landed. Resume Mode now refuses to start on a dirty working tree (same gate as `/sdd clean`), prints a one-line `mode: auto` suspension notice when applicable, and at transition closure clears `transition: true`, flips `enforce_tdd: false` to `true` in the same edit, and enters Plan Mode so the first feature work on the now-real spec is plan-gated. The PR-boundary review hooks (`git-push-review-reminder` PostToolUse and `enforce-review-spawn` Stop) short-circuit while triage is open so legacy code does not trigger reviewers. code-reviewer defers `Implements REQ-X-NNN` annotation findings on legacy source during transition. `/sdd autonomous unleashed` is rejected while `transition: true`. Discovery degrades gracefully when the GitHub corpus is unreachable (non-GitHub remote / `gh` auth failure / rate-limited / air-gapped) - working-tree + git-log evidence only, with a one-line notice recorded in `sdd/changes.md`.
- Added REQ-AGENT-022: legacy-codebase transition to SDD via `/sdd init` triage. Import Mode now produces two outputs from the same analysis pass — official REQs for behavior that is clear from source / tests / comments / commits / PRs, and a triage queue at `sdd/init-triage.md` for everything unclear, with the agent's Context (file:line, git author, commit refs, related tests / PRs) and Recommendation (best-guess answer with one-line Rationale) populated up front so the user decides on substance rather than performing archaeology. Five decisions per item: accept / correct (free-form prose describing what it's for and how it works) / lost (one-line Reason required) / skip / quit. Only accept and correct promote anything into the official spec — skip and lost write nothing, so intent is never fabricated. Re-running `/sdd init` while `sdd/init-triage.md` has open items resumes the interactive loop with refreshed Context. `sdd/config.yml` carries `transition: true` while the queue has open items; during transition spec-reviewer suppresses the Implemented → Partial auto-demote and `/sdd autonomous unleashed` is rejected. When the queue drains to zero, transition clears automatically and full SDD discipline + autonomous agentic development is unlocked. No new subcommand — `/sdd init` is the single entry point and detects greenfield / import / resume from project state.
- Three review-agent disciplines extended with content-quality checks (REQ-AGENT-021 AC 12). Closes codeflare#331.
  - **doc-discipline** gains Passes 7-12 on top of the six structural passes: verification-field truth-check (cited tests must actually exercise the section), Implements-vs-AC cross-walk (REQ ID claims must match the linked AC's behavior), stale code-block detection against current source, hatch catch-all detection (one ADR carrying more than three markers across more than one file), content-preservation on auto-trims (load-bearing clauses promote to prose or the trim reverts), and stranger cold-read (fresh subagent attempts a simulated task with only the doc). doc-updater reshaped from compliance-officer to authorial — derives real values from source before falling back to `TBD`.
  - **spec-discipline** gains CQ-1..CQ-6, the spec analogs: REQ-test truth-check (the REQ ID must appear in a test-block name and the assertions must reference what the AC describes), source-annotation-vs-AC cross-walk (`// Implements REQ-X-NNN` must implement an AC of that REQ), vendor/protocol drift detection (allowlisted vendor tokens in ACs must still appear in source), `<!-- sdd-allow-large -->` catch-all detection, content-preservation on shrink/split, and REQ Intent cold-read.
  - **tdd-discipline** gains antipattern 8 — test name lies about what's asserted. A test named `it('rejects expired JWT', ...)` whose body asserts `expect(result.length > 0)` is theatre even though every prior antipattern check (has a name, has an assertion, the assertion can fail) is satisfied. Especially relevant for tests named after a REQ ID, which satisfy the spec-discipline literal-match rule (CQ-1) but verify nothing the REQ promises.
