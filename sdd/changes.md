...
## 2026-05-17
- REQ-VAULT-001 / REQ-VAULT-005 / REQ-FS-010 fold-in. Vault directory renamed `.user_vault` -> `Vault` (non-hidden) in entrypoint, bisync filter, capture+extract prompts, plugin scripts, preseed rules, Worker route, docs, audits, tests. SilverBullet's `disk_space_primitives.go` FetchFileList aborts the disk walk when the root basename starts with `.` (`SkipDir` guard fires on the root entry itself), so the SB UI under a dot-prefixed vault returned an empty file listing even with notes on disk; the rename satisfies the guard without a symlink shim. Two new persistent user folders `Uploads/` and `Temporary/` auto-create alongside `Vault/` in `init_user_vault()` and bisync to R2 via filter additions (`+ Uploads/**`, `+ Temporary/**`). The R2 storage panel renders Workspace/Vault/Uploads/Temporary as "special folders" with an info icon (`mdiTrainCarContainer`) that expands a tooltip showing folder purpose plus the in-container path. Internal identifiers preserved: the `init_user_vault` function name and the `--as user_vault` global-graph tag stay (no manifest migration needed). No R2 migration code: clean cutover for the single existing user; prior `.user_vault/` content in R2 is abandoned.
- REQ-VAULT-001 Container Folders captures the four special-folder paths (`Workspace`, `Vault`, `Uploads`, `Temporary`) so the storage panel UX and entrypoint stay in sync.
