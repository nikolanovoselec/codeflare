...
## 2026-05-10
- Default container disk increased from 6 GB to 8 GB for all users on the default and saas tiers (REQ-OPS-002 AC7). Headroom for npm caches, Docker image extraction, transcript history, and bigger user workspaces; aligns the default tier with what the high tier already provided.
- context-mode `ctx_execute` and `ctx_batch_execute` are now ~2x faster (38 ms cold-start vs 78 ms) because Bun is installed in the container image and context-mode autodetects it as the JS/TS subprocess runtime (REQ-AGENT-005 AC5). Bun is a perf-only addition - the underlying #309 dynamic-require bug remains fixed by the build-time createRequire shim, not by Bun.
