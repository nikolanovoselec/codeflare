...
- The idle-detection layer (REQ-OPS-006 AC8-AC10) now fails safe in the direction of preserving user work. The Container DO's class-field default for `sleepAfter` flips from 5m to 2h, the parser's fallback for malformed values flips from 5m to 2h, the metrics-collection tick re-reads `sleepAfter` from Durable Object storage on every 60-second fire to correct any in-memory cache drift, and the worker route that builds the container init payload refuses to silently default a missing `sleepAfter` (now throws so the bug is observable). Issue codeflare#294 surfaced the symptom: containers dying before the configured 2h timer; this commit hardens every code path on the failure list so the next occurrence either does not happen or surfaces in logs.
