...
## 2026-05-09
- GitHub OAuth state is now stateless (REQ-AUTH-002 AC2-AC4). The state parameter is an HMAC-signed self-contained token (`nonce.iat.sig`, signed with `OAUTH_JWT_SECRET`, 30-minute window), not a cookie. Users on iOS WebKit (Safari, Brave) were getting "Invalid OAuth state" because ITP was suppressing the prior `SameSite=Lax` `oauth_state` cookie on the github.com bounce-back. The signed token in the URL is immune to that, and works identically in private browsing and ephemeral storage. State verification failure now redirects to `/?error=session-expired` (clean retry path) instead of returning raw JSON 403, and the login page surfaces a friendly message for that and for other OAuth-flow error codes (no-verified-email, access_denied, etc.).
