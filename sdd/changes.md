...
## 2026-05-16
- REQ-VAULT-005 vault config + service-worker MIME polish (no spec change): two follow-up fixes from the SilverBullet symptom investigation. (1) `init_obsidian_vault` in entrypoint.sh wrapped the SilverBullet `config.yaml` copy inside the `[ ! -d "$VAULT" ]` skeleton-create branch, so any vault that already existed (every container that round-tripped through R2 bisync from a prior session) silently never received the preseed config. Moved the copy outside the gate as an idempotent boot-time sync that only overwrites when `cmp` shows the preseed content differs, so a codeflare-side config update propagates to every active vault on the next boot. (2) SilverBullet 2.8.0 serves the service worker only at the root path `/service_worker.js`; any client request under `/.client/service_worker.js` falls through to the SPA HTML catch-all and the browser rejects the registration with a MIME-type error (the "ServiceWorker for scope ...: was received when fetching the script" overlay in screenshot 1). The base-href rewrite makes the SB client compute the correct root URL today, but stale browser scopes or future SB builds could still arrive at the `/.client/` path. Added a defensive rewrite in `host/src/server.ts` that remaps `/vault/.client/service_worker.js` to `/vault/service_worker.js` before forwarding to SilverBullet, so the canonical JS+correct MIME is always served regardless of which shape the client sends.
- REQ-VAULT-005 SilverBullet write CSRF fix (no spec change): SilverBullet's client.js writes pages via PUT/DELETE/PATCH without `X-Requested-With`, which `authenticateRequest`'s CSRF guard requires on state-changing methods. The visible symptom: clicking "Quick Note" in the vault editor surfaced an alert reading verbatim "You are not authenticated, going to reload and hope that that kicks off authentication" (SB client.js text, hard-coded on any 401/403), the browser reloaded into a sync-retry loop, and the user saw a white page until forcing a tab close + reopen. Root cause: vault.ts called `authenticateRequest(request, env)` directly, so any state-changing SB request hit the CSRF guard and got 403. Fix: when Origin was explicitly validated against `isAllowedOrigin` earlier in the handler, synthesise `X-Requested-With: XMLHttpRequest` on a request clone before passing to `authenticateRequest`. Defensive posture: requests without Origin (the edge case the CSRF guard is for) still fail the check, since browsers always set Origin on cross-origin state-changing requests per the Fetch spec. Session cookie remains the primary auth signal. Single-file change in `src/routes/vault.ts`.
- REQ-MEMORY-102 vault-monitor hook race fix (no spec change): the daemon ticks every 60s but a vault-extract sonnet run takes ~90s. During that overlap the daemon's "skip if VARS_FILE exists" guard fails (sonnet step 1 deleted it) and `find -newer LAST_MARKER` still returns the original files (sonnet hasn't touched LAST_MARKER yet), so the daemon writes a fresh VARS_FILE. When the sonnet eventually touches LAST_MARKER at step 6, that VARS_FILE is left behind — older than LAST_MARKER — and triggers a spurious additional sonnet on the next user prompt. Fix adds a single timestamp guard in `vault-monitor-hook.sh`: if VARS_FILE exists but is NOT newer than LAST_MARKER, the work is already done; the hook silently deletes the stale marker and exits 0. Invariant restored: VARS_FILE is only valid if newer than LAST_MARKER. Verified locally both ways (stale case → silent self-delete; fresh case → emits additionalContext as before).
- REQ-VAULT-002, REQ-VAULT-003 implementation correction (no spec change): vault-extract sonnet and memory-capture sonnet step 5 both called the headless `graphify extract --file` CLI, which requires an LLM provider key (`GEMINI_API_KEY`/`OPENAI_API_KEY`/etc.) that codeflare deliberately ships none of. Production symptom: every sonnet run returned 0 nodes (the broken backend path), so the vault contributed nothing to the unified global graph even though the symlink fix above made `graphify global add` reachable. Correct architecture (verified against upstream graphify skill.md): codeflare uses the agent-dispatch extraction path - the sonnet reads files itself, emits a chunk JSON matching graphify's extraction schema (nodes/edges/hyperedges, wikilinks as `file_type:concept` with `source_file:null` so `global_add`'s `external_labels` pass dedupes by label across vault and per-repo graphs), then runs `graphify.build.build_from_json` + `graphify.cluster.cluster` + `graphify.export.to_json` from graphify's Python API to produce graph.json, then `graphify global add`. No LLM provider key, no openai pip extra, no headless CLI. End-to-end verified locally: 10 nodes / 11 edges from a single test note, 5 concept nodes correctly emitted with `source_file:null` and visible in global graph as `vault::concept_*`. The `preseed/agents/claude/rules/vault.md` wikilink section also clarified: dedup is exact-string label match, not fuzzy - PascalCase concept names that you actively want unified across the graph.
- REQ-AGENT-023 implementation hardening (no spec change): the graphify CLI shim is now exposed at `/usr/local/bin/graphify` via Dockerfile symlink (`ln -sf /root/.local/share/uv/tools/graphifyy/bin/graphify`) with a defensive idempotent self-heal in `entrypoint.sh`. The shim previously lived only in `/root/.local/bin/` which is not on the system PATH, causing `command -v graphify` to return false in every bash subshell launched by a hook (graphify-active-repo.sh, memory-capture sonnet, vault-extract sonnet). The visible failure: `~/.graphify/global-graph.json` was never auto-seeded in production; the unified vault+repo graph promise of REQ-VAULT-002 was silently broken. Verified end-to-end on production session after fix: hook now seeds the global graph on first repo access (8552 nodes for codeflare added in <1s, manifest hit short-circuits on re-fire).
