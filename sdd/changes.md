...
## 2026-05-17
- REQ-VAULT-004 AC4 hardened: vault-skip path comparison now canonicalizes `$HOME` via `cd && pwd` (matching how `REPO` is resolved) and also matches on basename `.user_vault`. Pre-fix, the raw `$HOME/.user_vault` string compared against the canonicalized `REPO` would miss the vault on hosts where `$HOME` itself was a symlink or carried a trailing slash, silently re-tagging the vault as `.user_vault` on every vault file touch. New regression test (`vault skip: works when $HOME is a symlinked path`) covers the canonicalization case.
- REQ-SEC-012 AC5 reworded as observable behaviour (token survives DO hibernate/wake; no `Unauthorized` response after wake) -- implementation prose (`ctx.storage` key name, `blockConcurrencyWhile` placement) lives in the changelog narrative and `documentation/security.md`, not the AC bullet. Underlying behaviour unchanged.
- `updateEnvVars()` `ctx.storage.put('containerAuthToken', ...)` is now pinned to the request lifecycle via `ctx.waitUntil(...)` (guarded so test mocks without a `waitUntil` stub don't crash). Closes the narrow wake-then-immediately-hibernate window where the put would not have committed before the DO was evicted again, producing the same Unauthorized symptom that REQ-SEC-012 AC5 was added to prevent.
- `handleVaultRequest` rewrite no-op warning is now scoped to `status === 200 && path in {/, /index.html}`. Pre-fix the AC7 widening made the warning fire on every non-shell text/html response (error pages, 404 HTML) producing prod log noise; the warning's job is to surface a future SilverBullet template change on the load-bearing shell paths, not to annotate every error render.
...
- REQ-SEC-012 extended with AC5: the per-DO-lifecycle container auth token is now persisted to `ctx.storage` under key `containerAuthToken` and restored in `blockConcurrencyWhile` alongside other operational state. Pre-fix the token lived only in `this._containerAuthToken` (in-memory); every DO wake from hibernation re-generated a fresh UUID while the container process retained its original `CONTAINER_AUTH_TOKEN` env var, producing a mismatch and silent `{"error":"Unauthorized"}` on every proxied request from `host/src/server.ts:177-178` until the user manually recreated the session. Storage key is deleted in `destroy()` so the next session under the same DO ID gets a fresh token.
