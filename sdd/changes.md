...
- Transition closure: when the last open item is resolved or marked `lost`, the closure commit clears `transition: true`, flips `enforce_tdd: false` to `true` in the same edit, appends a closure entry to `sdd/changes.md`, and enters Plan Mode so the first feature work on the now-real spec is plan-gated. `sdd/init-triage.md` is preserved as the audit record.
