...
## 2026-05-10
- context-mode `ctx_execute` and `ctx_batch_execute` now work for Custom + Pro users (REQ-AGENT-005 AC5). Previously the bundled executor threw `Dynamic require of "node:fs" is not supported` on the first execute call because esbuild's ESM bundle leaves dynamic `require()` shims that fail under both Node and Bun ESM loaders. The package is now installed globally during the Docker image build, and the build also patches both bundled `.mjs` files with a 2-line `createRequire` shim so the dynamic requires resolve to the real Node module loader. Hooks and the MCP server invoke the bare `context-mode` binary instead of `bunx context-mode@<ver>`. Closes codeflare#309.
