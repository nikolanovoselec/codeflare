...
- Ripped out six overengineered features from the SDD framework (REQ-AGENT-021) after a third-wave architect review surfaced them as user-facing complexity without proportionate value:
...
- Second-wave architect triage applied (REQ-AGENT-022, REQ-AGENT-021). Twelve HIGH findings resolved across the SDD framework:
  ...
  - `/sdd init` re-invocation on an existing `sdd/` aborts with a /sdd-clean rescue hint instead of recommending `--force`. Resume Mode now creates `sdd/config.yml` from the template if missing (the triage queue is the authoritative state, config.yml is regenerable).
  - `/sdd mode status` argument dropped from enum (calling `/sdd mode` with no arg prints the mode and recent overriding ADRs).
  - code-reviewer transition gate moved from end-of-file to Phase 0 (runs FIRST) and now carries the literal grep regex snippet matching spec-reviewer / doc-updater shape.
  - Round-counter detection is now path-based per agent: spec-reviewer counts only commits touching `sdd/**`, doc-updater counts only commits touching `documentation/**`. Each agent's spiral guard is scoped to its own lane.
  ...
  - tdd-discipline antipattern 8 (test name lies about what's asserted) paired with spec-discipline CQ-1 at the same severity (MEDIUM).
...
- Transition closure: when the last open item is resolved or marked `lost`, the closure commit clears `transition: true`, appends a closure entry to `sdd/changes.md`, and enters Plan Mode so the first feature work on the now-real spec is plan-gated. `enforce_tdd` is NOT auto-flipped - the user changes it manually when ready for TDD enforcement (typically after adding source annotations and REQ-ID test names to the imported code). `sdd/init-triage.md` is preserved as the audit record.
- `/sdd autonomous` renamed to `/sdd mode` with a single-arg shape: `/sdd mode interactive`, `/sdd mode auto`, `/sdd mode unleashed`, or `/sdd mode` to print the current mode. Drops the on/off pair confusion. Same three underlying modes, cleaner surface.
- Triage entry canonical shape drops `**Reason:**` as a required field. Reason is appended only on `lost`-marked items (one-line explanation of why the information is genuinely unrecoverable).
...
- Three review-agent disciplines extended with content-quality checks (REQ-AGENT-021 AC 12). Closes codeflare#331.
  - **doc-discipline** gains Passes 7-12 on top of the six structural passes: verification-field truth-check, Implements-vs-AC cross-walk, stale code-block detection against current source, hatch catch-all detection (one ADR carrying more than three markers across more than one file), content-preservation on auto-trims, and stranger cold-read.
  - **spec-discipline** gains CQ-1..CQ-6: REQ-test truth-check, source-annotation-vs-AC cross-walk, vendor/protocol drift detection, `<!-- sdd-allow-large -->` catch-all detection, content-preservation on shrink/split, and REQ Intent cold-read. Canonical hatch ID form is `AD-N+` across spec-discipline, documentation-discipline, and the ADR template.
  - **tdd-discipline** gains antipattern 8 (test name lies about what's asserted) and aligns severity with spec-discipline: file-content text-matching, literal-fixture destructuring, and missing assertions stay HIGH; call-count-only and skipped-without-justification drop to MEDIUM.
- REQ-AGENT-005 acceptance criteria rewritten (REQ-AGENT-005) to describe the user-observable Standard-vs-Pro experience (memory persistence, hook coverage on every PR boundary, universal context-mode helpers, Custom-tier-only auto-routing). The detailed per-content-category delivery matrix moved to [documentation/preseed.md](../documentation/preseed.md#session-modes) where the engineering reference already lived; the spec now backlinks the matrix instead of duplicating it. No change in delivered behavior.
