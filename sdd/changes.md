...
## 2026-05-10
- Reverted: the disk bump from 6 GB to 8 GB on the default/saas tiers (REQ-OPS-002 AC7, REQ-OPS-007 AC1). Cloudflare containers cap disk at 2x memory in GiB, so 8 GB requires >=4 GiB memory; the default tier ships with 3 GiB and the deploy was rejected with VALIDATE_INPUT. Default disk stays at 6 GB. Raising it requires bumping memory first - tracked for a future tier upgrade with proper cost review.
