...
- SDD review pipeline now fires regardless of which tool surfaces `git push` or `gh pr create` (REQ-AGENT-021). When context-mode's strict PreToolUse hook denies `gh` in the Bash tool and forces the invocation through `mcp__context-mode__ctx_execute` / `ctx_batch_execute`, the PostToolUse review-reminder now sees the redirected command and emits the three-agent directive identically. Previously the hook was registered on the Bash matcher only and parsed `.tool_input.command` only — ctx-mode-routed commands passed through unseen, the directive never fired, and the user discovered the gap by noticing the review pipeline never ran. Closes codeflare#317.
