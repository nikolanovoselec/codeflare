...
3. `vault-monitor-hook.sh` (UserPromptSubmit) exits 0 immediately when `vault-extract.vars` is absent (zero-cost on idle prompts) and emits `additionalContext` instructing the main agent to dispatch the **vault-extract** named subagent (Task tool with `subagent_type="vault-extract"`) when present. The subagent's frontmatter pins the model to sonnet per AD58; the hook directive instructs the main agent not to pass a model override, so the model is bound at the agent-file level and cannot be silently downgraded.
