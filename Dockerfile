...
# ---------------------------------------------------------------------------
# undici-cve-patch
# Trivy scans every package.json in the final image, including npm's own bundled
# dependencies and agent/preseed tool trees. Node 24's bundled npm and some
# build-time npm installs can carry older undici copies after the public fix for
# CVE-2026-12151 is available. Replace only vulnerable vendored undici package
# dirs with the fixed patch release for the same major, then fail closed if any
# vulnerable copy remains.
RUN node <<'NODE'
...
const roots = ['/usr/local/lib/node_modules', '/opt/codeflare', '/app/host/node_modules'];
const fixedByMajor = new Map([[6, '6.27.0'], [7, '7.28.0'], [8, '8.5.0']]);
...
function fixedVersionFor(version) {
  const major = Number(version.split('.')[0]);
  const fixed = fixedByMajor.get(major);
  return fixed && compareVersions(version, fixed) < 0 ? fixed : null;
}

function unpackFixed(version) {
  if (unpacked.has(version)) return unpacked.get(version);
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), `undici-${version}-`));
  const tarball = cp.execFileSync('npm', ['pack', `undici@${version}`, '--silent'], { cwd: tmp, encoding: 'utf8' }).trim();
  cp.execFileSync('tar', ['-xzf', tarball, '-C', tmp], { cwd: tmp, stdio: 'inherit' });
  const packageDir = path.join(tmp, 'package');
  unpacked.set(version, packageDir);
  return packageDir;
}
...
function* packageJsonFiles(root) {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && entry.name === 'package.json') yield full;
    }
  }
}

function findVulnerableUndici() {
  const found = [];
  for (const root of roots) {
    for (const file of packageJsonFiles(root)) {
      ...
      if (pkg.name !== 'undici' || typeof pkg.version !== 'string') continue;
      const fixed = fixedVersionFor(pkg.version);
      if (fixed) found.push({ dir: path.dirname(file), version: pkg.version, fixed });
    }
  }
  return found;
}

const vulnerable = findVulnerableUndici();
for (const item of vulnerable) {
  const source = unpackFixed(item.fixed);
  fs.rmSync(item.dir, { recursive: true, force: true });
  fs.cpSync(source, item.dir, { recursive: true });
  console.log(`[Dockerfile] patched undici ${item.version} -> ${item.fixed} at ${item.dir}`);
}

const remaining = findVulnerableUndici();
if (remaining.length > 0) {
  for (const item of remaining) {
    console.error(`[Dockerfile] FATAL: vulnerable undici ${item.version} remains at ${item.dir}; fixed version is ${item.fixed}`);
  }
  process.exit(1);
}

console.log(`[Dockerfile] undici CVE patch complete; patched ${vulnerable.length} package(s)`);
for (const dir of unpacked.values()) fs.rmSync(path.dirname(dir), { recursive: true, force: true });
NODE
