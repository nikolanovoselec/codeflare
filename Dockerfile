...
# image builds. Install all of them from one committed lock so exact package
# bytes and transitive dependencies are reviewable.
...
RUN cd /opt/codeflare/npm-tools && \
    ...
    for b in claude codex opencode copilot pi bun bunx context-mode consult-llm-mcp chrome-devtools-mcp; do \
    ...
    cd /opt/codeflare/npm-tools/node_modules && \
    find . -maxdepth 1 -name 'opencode-*' ! -name 'opencode-ai' ! -name 'opencode-linux-x64' -type d -exec rm -rf {} + && \
    cd /opt/codeflare/npm-tools/node_modules/@github/copilot && \
    ...
    rm -rf /opt/codeflare/npm-tools/node_modules/@oven && \
    claude --version && \
    bun --version && \
