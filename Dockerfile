...
# EXCEPTION: pi is PINNED. The preseed extensions + the build-time jiti warm-up
# target a specific Pi extension API and jiti version; an unpinned pi made image
# rebuilds irreproducible and startup regressions unbisectable (the deploy that
# doubled session startup also silently changed the pi core). Bump deliberately.
RUN npm install -g @openai/codex@latest opencode-ai@latest @github/copilot@latest @earendil-works/pi-coding-agent@0.79.1 && \
