...
**Managed Environment** has shipped, and it makes that content manageable out of band. A deployment points at one private release repository, a repository-scoped read token, and an Ed25519 public key. The company publishes a signed, immutable release on its own schedule, and every user's environment converges to it before their next session starts. Skills, rules, hooks, agents, scripts, plugins, and required Browser IDE extensions all move without rebuilding or redeploying Codeflare, and without any user action. The image-baked preseed stays as an independent fallback. Verification happens entirely in the Worker, which checks repository identity, asset digests, the signature, a monotonic sequence, and declared limits before any byte reaches a user's storage; credentials and signing material never enter a session container. See [Managed Environment data flow](documentation/lanes/architecture.md#managed-environment-data-flow).

Codeflare's own curated skills, rules, hooks, agents, scripts, plugins, and company extension requirements live in [codeflare-curation](https://github.com/nikolanovoselec/codeflare-curation) (access required). It is the runtime master for that content; `preseed/agents/**` in this repository is only the image-baked fallback baseline. Each operator points at their own private release repository rather than this one.
...
2. Add `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` as GitHub Actions repository secrets using the maintained [operator token scope list](documentation/lanes/configuration.md#cloudflare-api-token-operator).
