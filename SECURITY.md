...
- **Self-service subscription:** Pending users choose a subscription tier (free, standard, max, unlimited) at `/app/subscribe`. Cloudflare Turnstile CAPTCHA protects the subscription form from bot abuse. Trial periods are configurable per tier and enforced server-side.
