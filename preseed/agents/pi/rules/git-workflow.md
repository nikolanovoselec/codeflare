...
- After opening or syncing an SDD PR to `main`/`master`, obey any `codeflare-visible-monitor-handoff` follow-up immediately: spawn the visible review-monitor and CI-monitor background agents for the exact head, then report their agent IDs. If no handoff appears but `.git/codeflare-review-jobs/<head>/job.json` exists, verify/start `review-monitor` immediately. This is review result delivery, not reviewer spawning. If a monitor stops/completes without `REVIEW_RESULT`, restart it from the existing job prompt/result paths.
