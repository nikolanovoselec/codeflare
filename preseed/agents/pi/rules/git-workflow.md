...
4. The boundary extension renders every dispatched reviewer and CI ID in one above-editor widget with queued, running, completed, or failed state from the live service record. These service launches are not assistant public-tool-call transcript blocks. Completion correlation uses the immutable agent ID in `subagents:record`; only a successful matching reviewer record can acknowledge the head.
...
7. Reject unsupported or overengineered proposals. Apply legitimate minimal fixes automatically unless the user explicitly requested approval or validation.
