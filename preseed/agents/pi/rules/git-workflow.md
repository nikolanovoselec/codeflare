...
- After any push or PR creation that can produce CI, invoke `ci-monitoring` unless the user explicitly says to skip CI monitoring for that push/PR.
...
- After opening or syncing an SDD PR to `main`/`master`, obey any `codeflare-visible-monitor-handoff` follow-up immediately: spawn the visible `review-monitor` background agent for the exact head, then report its agent ID. The handoff carries only the review-monitor; CI monitoring is owned by the `ci-monitoring` route above, so do NOT spawn a second CI monitor from the handoff. If no handoff appears but `.git/codeflare-review-jobs/<head>/job.json` exists, verify/start `review-monitor` immediately. This is review result delivery, not reviewer spawning. If the `review-monitor` stops/completes without `REVIEW_RESULT`, restart it from the existing job prompt/result paths.
- Skipping `ci-monitoring` after a CI-producing push/PR without an explicit user skip instruction is HIGH `ci-monitoring-skill-not-invoked`.
