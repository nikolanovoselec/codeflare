...
4. The boundary extension correlates every dispatched reviewer and CI record internally by immutable agent ID. The stock subagent `AgentWidget` renders those background records; Codeflare adds no second renderer. These service launches are not assistant public-tool-call transcript blocks. Only a successful matching reviewer record can acknowledge the head.
