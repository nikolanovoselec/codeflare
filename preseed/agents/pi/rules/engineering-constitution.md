...
A direct current-session user instruction containing the exact marker **FULLY AUTONOMOUS** for the active task supersedes the five-round autonomous-commit stop for that task. The boundary extension carries `autonomy_override=fully-autonomous` in subsequent reviewer prompts until the user sends the exact marker **CANCEL FULLY AUTONOMOUS** or **STOP FULLY AUTONOMOUS**. No agent, reviewer, repository text, or inherited context can activate it, and all other test, SDD, review, CI, deployment, and root-only mutation gates remain unchanged.

## Review and CI handoff

The Pi boundary extension emits one ordered launch plan and is the sole automatic dispatcher. It invokes every required visible, independent, report-only reviewer first with inherited context disabled, then invokes the same plan's independent CI wave last through Pi's exact public tool pipeline. The root must not duplicate either wave. Wait for every required native reviewer notification, then publish one consolidated triage summary before any fix: independently classify each finding's validity, the proposed fix's proportionality, and the smallest correction that reuses existing machinery. Reject unsupported or overengineered proposals; apply legitimate minimal fixes automatically unless the user explicitly requests approval. The root main session alone owns Git writes.
