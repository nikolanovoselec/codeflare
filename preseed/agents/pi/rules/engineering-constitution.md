...
The Pi boundary extension appends one extension-only review window and is the sole automatic dispatcher; launch instructions never enter LLM context. It uses the stock session subagent service to submit every required independent, report-only reviewer first with inherited context disabled. The same extension correlates deterministic service IDs internally and renders only those records through a focused copy of the stock width-safe `AgentWidget` display/lifecycle; execution remains in the stock service. Managed concurrency executes one memory-heavy reviewer at a time, then the extension resolves and launches the same window's independent CI wave last with queue bypass.

The root must not duplicate these launches or infer a second CI trigger from the Git command. Wait for every required native reviewer result, then publish one consolidated adversarial triage table before any fix. Include every finding under `FINDING (as output by reviewer) | PROPOSED FIX (by reviewer) | STATUS | DECISION`. Critically challenge evidence and scope against the current implementation, specifications, documentation, architecture decisions, project intent, and direct current-session instructions. Independently classify finding validity and proposal proportionality; reject unsupported findings and wrong or overengineered fixes, explain why, and choose the smallest correction that reuses existing machinery. Apply legitimate minimal fixes automatically unless the user explicitly requested approval. The root main session alone owns Git writes.
