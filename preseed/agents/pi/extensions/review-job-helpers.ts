...
export function visibleMonitorHandoffRequest(input: { repo: string; head: string; reason: string; reviewPrompt: string; branch?: string; prNumber?: number }): VisibleMonitorHandoffRequest {
  ...
  const message = [
    ...
    "Main-session action required: spawn the visible background review-monitor for this exact head now.",
    "This handoff carries only the review-monitor. CI monitoring is owned separately by the ci-monitoring route; do NOT spawn a CI monitor from this handoff (a second CI monitor would collide with the one the main session already starts after the CI-producing push).",
    "Spawn the review-monitor if missing; if a visible review-monitor for this exact head already returned its REVIEW_RESULT, do not duplicate it. If it stops/errors without REVIEW_RESULT, restart it from this message.",
    "",
    ...
    "```",
    "",
    "After spawning, report the visible agent ID. Do not start source/doc/spec edits until the visible review-monitor is running or already completed for this head.",
  ].join("\n");
  return {
    customType: "codeflare-visible-monitor-handoff-v1",
    message,
