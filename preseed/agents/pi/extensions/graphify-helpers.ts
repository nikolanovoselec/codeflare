...
export function renderGraphifyCloneDirective(action: GraphifyCloneAction): string {
  if (action.mode === "existing-graph") {
    return [
      ...
      "If stale, ask the user whether to run the free AST-only update (`bash /home/user/.pi/agent/scripts/safe-graphify-update.sh .`) or a full AST + semantic refresh using Pi Agent subagents.",
  ...
  return [
    ...
    "1. AST-only — free, local, no LLM/API key; builds structural code graph with `bash /home/user/.pi/agent/scripts/safe-graphify-update.sh .` then `graphify cluster-only .`.",
