...
4. Execute the complete `purpose=review` manifest and all focused evidence reads in one evidence wave. Use `ctx_batch_execute` when context-mode is available; otherwise issue the equivalent bounded `read`, `grep`, and `bash` calls together. Return one compact row per check with counts and failures only. Do not re-read policy, packet, or evidence already returned.
