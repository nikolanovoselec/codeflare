...
Scope is the work bound. Build and consume the `spec-reviewer` packet once inside the first Bash/Node processing call. The foreground-only context-mode extension is intentionally absent from in-process reviewers; invoke the seeded packet CLI directly and parse its JSON in memory without persisting the packet or returning raw packet JSON. In that same call also run `node ~/.pi/agent/skills/review-scope/scripts/lane-evidence.mjs --repo <root> --lane spec-reviewer --range <base>..<head>` and parse it in memory. It resolves anchor resolution across the touched spec files, the domain index, the pending backlog, and the decision ledger as id/title/status. Treat it as authoritative; `checked` with an empty `unresolved` is that check performed, not skipped. Re-running any of it spends a turn reproducing an answer already in hand. Start from SDD hunks. A changed source/test path alone invalidates no anchor: resolve each referenced implementation symbol or named test block and include it only when its line range overlaps `changedInputs[].hunks`. Consolidate the enforcement manifest and unresolved evidence instead of alternating reads.
