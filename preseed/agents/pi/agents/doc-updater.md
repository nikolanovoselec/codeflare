...
Build and consume the `doc-updater` packet once inside the first Bash/Node processing call. The foreground-only context-mode extension is intentionally absent from in-process reviewers; invoke the seeded packet CLI directly and parse its JSON in memory without persisting the packet or returning raw packet JSON. In that same call also run `node ~/.pi/agent/skills/review-scope/scripts/lane-evidence.mjs --repo <root> --lane doc-updater --range <base>..<head>` and parse it in memory. It resolves the index presence and layout, the index-versus-tree join in `indexIntegrity` (`unindexed` and `dangling`, both empty is that check passed), anchor resolution, every documented reference against real code, and the decision ledger as id/title/status. It also resolves why you were spawned: `docsCitingChanged` pairs each changed source file with the pages citing it and carries that file's own `patch`, because the packet is scoped to the files you own and this is where the change reaches you. Read the row's patch; do not run `git diff` for a file that has one, and gather yourself only where a row says `patchOmitted` or `patchTruncated`. Treat it as authoritative; `checked` with an empty `unresolved` is that check performed, not skipped. Re-running any of it spends a turn reproducing an answer already in hand. Start from documentation hunks. A changed source/spec path alone invalidates no documentation: resolve the referenced symbol/block and require overlap with `changedInputs[].hunks` unless a concrete documentation hunk identifies the contract dependency.
