...
5. The boundary extension correlates service-owned reviewer and CI IDs internally. The stock subagent `AgentWidget` renders those background records with its package-owned progress display; Codeflare adds no second renderer. These launches are not assistant public-tool-call transcript blocks.
