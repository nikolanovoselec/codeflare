...
2. Derive the pending manifest and concrete direct-impact candidates from that result. Consolidate independent deterministic checks into one evidence wave. A `ctx_execute` program inspects the complete work set and prints compact counts and failures; without context-mode, issue the equivalent available native calls together. A shell-only lane performs its reads, searches, and anchor checks inside consolidated shell programs. Context-mode changes transport only; the scoped checks, evidence, and dispositions are identical.
