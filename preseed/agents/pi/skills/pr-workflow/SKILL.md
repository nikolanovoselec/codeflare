...
- The Pi boundary extension emits one ordered launch plan automatically after successful `git push` or `gh pr create` delivery, or after the user accepts its review question for another Git/GitHub action exposing an unacknowledged exact PR head. Its first wave names any required SDD reviewers; its second wave names independent CI when eligible.
...
After successful `git push` or `gh pr create` leaves the checked-out branch at an unacknowledged exact head of an open PR targeting `main`/`master`/`develop`, **end the current assistant turn immediately**. Do the same after the user accepts the extension's review question for another Git/GitHub action. Becoming idle is what delivers the queued `deliverAs: "followUp"` boundary launch plan. Declining the question acknowledges that exact head without launching.
