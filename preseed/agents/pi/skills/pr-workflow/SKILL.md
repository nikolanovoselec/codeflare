...
After creating any PR that can produce CI, start CI monitoring unless the user explicitly says to skip it. For SDD PRs targeting `main`/`master`, obey any `codeflare-visible-monitor-handoff` follow-up immediately: spawn the visible `review-monitor` for the exact head, then report its agent ID (the handoff carries only the review-monitor; CI monitoring is the separate step above). If no handoff appears but `.git/codeflare-review-jobs/<head>/` exists, verify `monitor.json` is live or start `review-monitor` immediately. If the `review-monitor` stops, errors, or completes without `REVIEW_RESULT`, restart it for the same exact head instead of waiting for the full TTL; restart a CI monitor that stops without `CI_RESULT` per the CI-monitoring step above.
...
7. Create the PR.
8. Report the PR URL.
9. If a `codeflare-visible-monitor-handoff` follow-up appears, spawn the requested visible `review-monitor` for the exact head, then report its agent ID (the handoff no longer carries a CI monitor).
10. If the PR can produce CI, start one background CI monitor for the exact head unless the user explicitly skipped CI monitoring (CI monitoring is owned by the CI-monitoring step, independent of the review handoff).
...
- obey `codeflare-visible-monitor-handoff` follow-ups by spawning the visible `review-monitor` for the exact head
