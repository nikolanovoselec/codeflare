...
- **Use only local or in-session Graphify workflows.** Do not run `graphify label`, do not pass `--backend`, and do not route extraction through an external provider.
- **Never use the `graphify_build` tool.** Its semantic backend is external. Use the local scripts and flows in this skill instead (`build-graphify-ast.sh`, `safe-graphify-update.sh`, `graphify cluster-only`).
