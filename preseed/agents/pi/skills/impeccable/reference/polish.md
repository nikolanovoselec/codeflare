...
```bash
~/.pi/agent/skills/impeccable/scripts/impeccable critique-storage latest "<resolved target>" --json
```
...
```bash
~/.pi/agent/skills/impeccable/scripts/impeccable critique-storage close "<resolved target>" "<snapshot_file returned by latest>"
```
