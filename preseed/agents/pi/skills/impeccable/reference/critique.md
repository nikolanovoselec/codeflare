...
2. **Confirm the target slugs cleanly**:
   ```bash
   ~/.pi/agent/skills/impeccable/scripts/impeccable critique-storage slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
~/.pi/agent/skills/impeccable/scripts/impeccable detect --json [target]
```
...
4. If mutation is available, start `~/.pi/agent/skills/impeccable/scripts/impeccable live-server --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `PRODUCT.md` contains audience or brand context from `impeccable init`, also generate 1-2 project-specific personas from that evidence.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     ~/.pi/agent/skills/impeccable/scripts/impeccable critique-storage write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   ~/.pi/agent/skills/impeccable/scripts/impeccable critique-storage trend "<resolved target>" 5
   ```
...
- Do touch targets meet applicable platform and product guidance (44×44 pt on iOS; WCAG 2.5.8 baseline on the web)?
...
If `PRODUCT.md` contains audience or brand context generated by `impeccable init`, derive 1–2 additional personas from that evidence:
