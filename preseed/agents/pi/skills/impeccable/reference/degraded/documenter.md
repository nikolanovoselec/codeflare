...
5. Never canonize an unearned craft-floor pattern into the system. Record a detected aesthetic pattern only when the brief, platform, incumbent system, approved comp, or selected direction justifies it; otherwise list it as a defect the build carries. Accessibility and functional failures are never canonized.
