...
**Transition deferral.** If `sdd/config.yml` carries `transition: true` (the project is mid-`/sdd init` Import Mode with open triage items in `sdd/init-triage.md`), suppress the missing-annotation finding on **legacy source files** - any file present at the import-time commit on the current branch. The annotation convention applies post-transition; legacy code predates it and gets annotated as Resume Mode resolves each REQ's owning triage item. New source files created during the transition window (added in the diff under review, not present at import) ARE held to the rule because they're written under the new convention. Detection: a file is "legacy" if `git log --diff-filter=A --follow -- {file}` shows it was added before the first `[sdd-init]` commit on the branch. When in doubt, defer (under-flag during transition is preferable to over-flagging a legacy file).
