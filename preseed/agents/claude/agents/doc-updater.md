...
1. **You may not report a clean / passing / approving verdict while any in-scope MEDIUM or HIGH finding from the manifest is unaddressed.** The active `doc-enforce` scope decides whether an observation is a finding; `scope=diff` baseline debt is excluded. A run that surfaced a per-file line-budget overflow, a >50-word table cell, a lane violation, an api-reference shape break, a stale/orphaned `@impl` doc-anchor, or any other MEDIUM/HIGH is NOT a passing run until each finding is disposed of (`auto-fixed`, `escalated`, or interactive `deferred to user confirmation`). "No blockers", "looks good", or an all-zero report emitted over a fired finding is a false verdict.
