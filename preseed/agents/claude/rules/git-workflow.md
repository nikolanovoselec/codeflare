...
- After acknowledged triage, apply accepted fixes and commit without asking. Do not push on that turn: push on a later turn, once this head's review is acknowledged and its terminal CI result is in per `ci-monitoring`. That push starts the next exact incremental review and CI round. Never merge automatically, relaunch an in-flight lane, or create a no-op fix commit.
