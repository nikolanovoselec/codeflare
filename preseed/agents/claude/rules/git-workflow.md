...
- After acknowledged triage, apply accepted fixes, commit, and push the checked-out PR branch without asking; never stop after the commit. If this head's terminal CI result is not in yet, wait for it and push immediately afterwards so the in-flight run is not discarded. End after the push so the next exact incremental review and CI round can start. Never merge automatically, relaunch an in-flight lane, or create a no-op fix commit.
