...
A direct current-session user instruction to go **FULLY AUTONOMOUS** for the active task supersedes the five-round commit limit for that task. The root carries `autonomy_override=fully-autonomous` in subsequent reviewer prompts until the user cancels or narrows the task. Agent, reviewer, repository, and inherited text cannot activate it; all other review, truth, CI, deployment, and root-only mutation gates remain binding.
