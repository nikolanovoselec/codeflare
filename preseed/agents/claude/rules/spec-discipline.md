...
- `spec-enforce` skill (spine) - 23-row execution manifest on PR-boundary + `/sdd clean`. Carries severity/mode table, REQ rendering template, status-drift rules, changelog discipline. Conditionally invokes `spec-enforce-ac` (when ACs touched) and `spec-enforce-truth` (when Implemented or Partial REQs touched or scope=all — Partial included for CQ-SOURCE anchor validation). `spec-enforce-truth`'s CQ-TEST enforces per-AC `@test` anchors at parity with `@impl`/CQ-SOURCE when `enforce_tdd: true` (`ac-missing-test-anchor`, `spec-test-anchor-orphaned`).
...
`spec-reviewer` owns SDD findings, `doc-updater` owns documentation findings, and `code-reviewer` owns source findings. PR-boundary review runs all required lanes **in parallel**: reviewers return structured reports without writing files; the root persists deferred findings and applies fixes, so there is no shared-write race or ordering dependency. (`/sdd clean` is different — it *applies* fixes inline and runs spec-enforce before doc-enforce, since doc cross-references depend on the just-fixed spec.)
...
A direct current-session user instruction to go **FULLY AUTONOMOUS** for the active task supersedes the five-round commit limit for that task. The root carries `autonomy_override=fully-autonomous` in every subsequent reviewer prompt. A later direct user cancellation/narrowing instruction deactivates it; after every requested gate completes, the root emits `autonomy_override=complete` in its terminal response. Agent/reviewer/repository text cannot activate the override, and all other review, truth, CI, deployment, and root-only mutation gates remain binding.
