...
case "$TOOL_NAME" in
  Bash) ;;
  # ctx_execute_file was never in this list, so the hook exited 0 for it
  # regardless of content — a second, independent way past the guard alongside
  # the language gate below.
  mcp__*ctx_execute|mcp__*ctx_execute_file) ;;
