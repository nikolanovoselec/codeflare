...
CI_PROMPT=$(jq -cn --arg repo "$CURRENT_REPO" --argjson pr "$PR_NUMBER" --arg head "$CURRENT_PR_HEAD" --arg cwd "$PWD" '{repo:$repo,pr:$pr,head:$head,cwd:$cwd}') || exit 0
