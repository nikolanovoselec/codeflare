...
5. **Present the full draft as one review surface**: a tree showing the domain index + brief per-domain summary + the seeded ADR list + the glossary preview. Ask one question: `Accept as-is, edit a section (name it), or restart?` On `edit <section>`: re-draft only the named section in memory, re-present the full review surface, ask the same question again. On `restart`: discard the in-memory draft and return to step 3 (vision question). Loop until the user accepts. Phase 5 enrichment runs once, only after final acceptance, never per-edit.
