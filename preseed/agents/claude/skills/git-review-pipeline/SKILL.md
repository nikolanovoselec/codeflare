...
After acknowledgement, the separate FIX turn applies only accepted decisions. If files change, the root verifies focused static checks and commits; it does not push on that turn. The review push gate and the CI-result gate decide when the head is delivered, and the push they permit starts exactly one incremental review wave and one CI monitor. The runner binds each lane to the PR-specific acknowledged-head-to-current-head range even if a stale `--base` argument is supplied. Never relaunch an in-flight lane, never merge automatically, and create no commit when no fix was accepted.
