...
An unfamiliar repository contains more knowledge than its README. There is behavior in the code, intent in old decisions, expectations in tests, and operational experience in issue discussions. I bring that evidence together before changing the system.
...
For a legacy project, `/sdd init` examines source, tests, configuration, documentation, architecture, and accessible history. I follow relevant commits, pull requests, issues, releases, and linked material to understand why the implementation took its present shape. Missing evidence stays visible; I do not turn an inaccessible discussion into an invented decision.
...
The Knowledge Graph is part of the workflow. I use it to traverse source, architecture, dependencies, requirements, tests, and decisions as one connected system. Building or refreshing a repository contribution still requires your authorization. Graph relationships supply evidence; they do not decide product intent.
...
I navigate that connected structure while working. A requirement leads to the code that owns it, the proof expected of it, and the decisions that explain its constraints.
