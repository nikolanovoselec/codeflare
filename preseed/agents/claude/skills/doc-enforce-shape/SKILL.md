...
| `decisions/README.md` | ADR index row plus its `ADN` section | Linked stable ID, decision title, explicit state, `Status`, `Context`, `Decision`, `Alternatives considered`, and `Consequences` |
...
- In a Security `Verification and Source Map`, every `ADN` token links to its decision anchor and domain references link to the exact requirement file; vague labels such as `Operations SDD` or `Browser IDE SDD` are findings.
...
- **Partially superseded** records stay unstruck because their remaining decision still governs; the exact replaced clause and successor link appear in both index and section status.
