...
(function () {
  ...
  function handleServerLost() {
    ...
    if (state === 'GENERATING' || state === 'CYCLING' || state === 'SAVING') {
      showToast('Live server connection lost. Your session is saved; reopen this page or restart ~/.claude/skills/impeccable/scripts/impeccable live-poll to continue.', 6000);
    }
    hideBar();
    hideHighlight();
    hideShaderOverlay();
...
void main() {
  ...
  const AGENT_DISCONNECTED_TIP = 'Agent disconnected - run ~/.claude/skills/impeccable/scripts/impeccable live-poll to connect';
  ...
  function steerTimeoutMessage() {
    ...
    if (!agentPollingConnected) {
      return head + 'No agent is polling right now. Run ~/.claude/skills/impeccable/scripts/impeccable live-poll, then send it again.';
    }
