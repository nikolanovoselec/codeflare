...
1. **Stale row (marker already applied).** Parse each table row; extract `File` and `Line`. Read the indicated source file; check whether a marker comment exists at or within 5 lines of the indicated line. The detection regex is `//\s*AD-\d+\s*:` (or `//\s*CF-\d+\s*:`, or `//\s*{PREFIX}-\d+\s*:` per `adr_marker_style`) — the dash form is canonical; non-dash forms (`AD1` instead of `AD-1`) are NOT recognized. Authors writing markers MUST use the canonical form. If marker found AND matches the row's ADR ID: MEDIUM `adr-marker-sidecar-stale` (row should be deleted; the user applied the marker but forgot to clean the sidecar).
