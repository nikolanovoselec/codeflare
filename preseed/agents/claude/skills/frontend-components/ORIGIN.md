...
License: MIT. The managed runtime carries one shared notice at `skills/_licenses/vercel-agent-skills-MIT.txt`; upstream license evidence is recorded in `../frontend-patterns/ORIGIN.md`.
...
Codeflare adaptation: folded only explicit variants, children and slots, compound components, bounded state ownership, and provider-interface guidance into the existing skill. Preserved Codeflare's extract-after-two-repetitions rule, central content/style ownership, immutable refactors, accessibility, and behavioral-test contract. Omitted the separate upstream skill, generated guide, individual rule files, examples, metadata, templates, and build tooling.
