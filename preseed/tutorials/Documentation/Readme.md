...
Running processes, sockets, browser tabs, in-memory state, and files outside synchronized paths remain temporary. While the container and PTY remain alive, reconnecting restores bounded terminal output. After container replacement, synchronized agent session transcripts remain durable: Classic restores supported conversation history through `/resume`, while Herdr restores supported agent sessions automatically from persisted references. This does not restore arbitrary shell output, process memory, or the old process tree. Browser VS Code persists bounded UI state—theme, keyboard layout, Explorer state, and open-file resources—for restoration in future sessions; this does not persist live editor databases or make unsynchronized workspace files durable. Destroying a container also cannot undo a Git push, deployment, migration, API call, or synchronized file that already happened.
