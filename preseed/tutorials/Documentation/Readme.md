...
Running processes, sockets, browser tabs, in-memory state, and files outside synchronized paths remain temporary. Terminal scrollback is durable and restored when that session resumes. Browser VS Code persists bounded UI state—theme, keyboard layout, Explorer state, and open-file resources—for restoration in future sessions; this does not persist live editor databases or make unsynchronized workspace files durable. Destroying a container also cannot undo a Git push, deployment, migration, API call, or synchronized file that already happened.
