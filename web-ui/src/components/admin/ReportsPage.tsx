      ...
      <section class="admin-panel">
        ...
        <Show when={!history.loading} fallback={<div class="admin-state-panel"><div class="app-loading-spinner" /><p>Loading delivery history…</p></div>}>
          <Show when={!history.error && history()} fallback={<div class="admin-state-panel"><h2>Delivery history unavailable</h2><button type="button" class="admin-primary-button" onClick={() => void refetch()}>Retry</button></div>}>
            {(resolved) => <Show when={resolved().deliveries.length > 0} fallback={<div class="admin-state-panel"><h2>No deliveries yet</h2><p>Scheduled and test attempts will appear here.</p><A href="/admin/environment/usageReports">Review monthly report settings</A></div>}>
              <><div class="admin-table-scroll"><table class="admin-data-table admin-report-history-table"><thead><tr><th>Kind</th><th>Month</th><th>Recipient</th><th>State</th><th>Result</th><th>Attempts</th><th>Updated</th></tr></thead><tbody>
                <For each={resolved().deliveries}>{(delivery) => <tr><td data-label="Kind"><span class={`admin-status-badge is-${delivery.state}`}>{delivery.deliveryKind === 'test' ? 'Test' : 'Scheduled'}</span></td><td data-label="Month" class="admin-mono">{delivery.reportMonth}</td><td data-label="Recipient">{delivery.recipient}</td><td data-label="State">{delivery.state}</td><td data-label="Result">{delivery.reason === 'provider_unavailable' ? 'Email provider unavailable' : delivery.reason === 'attachment_too_large' ? 'Attachment too large' : delivery.state === 'accepted' ? 'Provider accepted' : 'Awaiting attempt'}</td><td data-label="Attempts" class="admin-mono">{delivery.attempt}/3</td><td data-label="Updated" class="admin-mono">{delivery.updatedAt}</td></tr>}</For>
