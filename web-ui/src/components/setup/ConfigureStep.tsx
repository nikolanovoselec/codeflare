...
const ConfigureStep: Component = () => {
  ...
  return (
    <div class="configure-step">
      ...
      {/* AI Gateway & Model Routing (enterprise-only). The customer's AI Gateway
          endpoint + token (REQ-ENTERPRISE-017) and the dynamic-route catalog that the
      ...
      <Show when={setupStore.enterpriseMode}>
        ...
        {/* Browser Rendering (enterprise-only). REQ-BROWSER-007: the admin-global
            Cloudflare Browser Rendering token. In enterprise the per-user deploy-keys
        ...
        {/* Security & Egress (enterprise-only). REQ-ENTERPRISE-016: strict gateway egress
      ...
      {/* Cloudflare Connection (admin, non-enterprise). Enterprise has no per-user
