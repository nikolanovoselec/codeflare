...
# context-mode FTS5 watchdog.
#
# context-mode's SQLite FTS5 index grows monotonically — every ctx_execute /
# ctx_search call writes a row and the project (as of v1.0.151) ships no
# in-session size cap or eviction. Its startup cleanup only removes DBs older
# than 14 days, which never fires for a long-running session.
#
# Symptom when the index gets large (observed at ~360MB on a multi-day
# session): the context-mode MCP server (Node, better-sqlite3) burns one
# whole vCPU on a hot SQLite-segment-merge loop. Issue #671 upstream is
# closed but its fix is staged across multiple versions and we cannot
# guarantee every user's installed version carries it. On a 1-vCPU
# container, a runaway context-mode also starves subagent spawns
# (spec-reviewer, code-reviewer, memory-capture) because the parent
# Claude process is already at ~25% and there is no idle CPU left.
#
# Defense in depth: poll the FTS5 directory every 5 minutes. If the
# content DBs exceed CTX_FTS5_MAX_MB (default 50), SIGKILL the
# context-mode process and wipe the DB files. The lazy MCP wrapper
# respawns it on the next call with an empty index, restoring CPU
# headroom. Subagent spawns work again immediately.
#
# This is purely a safety net — for normal Pro-tier sessions the index
# stays well under 50 MB.
CTX_FTS5_MAX_MB=${CTX_FTS5_MAX_MB:-50}
CTX_FTS5_DIR=${CTX_FTS5_DIR:-$USER_HOME/.claude/context-mode/content}
CTX_FTS5_SESSIONS_DIR=${CTX_FTS5_SESSIONS_DIR:-$USER_HOME/.claude/context-mode/sessions}
(
    while true; do
        sleep 300
        [ -d "$CTX_FTS5_DIR" ] || continue
        size_kb=$(du -sk "$CTX_FTS5_DIR" 2>/dev/null | awk '{print $1}')
        size_kb=${size_kb:-0}
        max_kb=$((CTX_FTS5_MAX_MB * 1024))
        if [ "$size_kb" -gt "$max_kb" ]; then
            pid=$(pgrep -f '/usr/local/bin/context-mode' || true)
            if [ -n "$pid" ]; then
            ...
            echo "[ctx-watchdog $ts] FTS5 was ${size_kb}KB (> ${max_kb}KB cap), killed pid=${pid:-none} and wiped index" >> /tmp/ctx-watchdog.log
        fi
    done
) &
echo "[entrypoint] context-mode FTS5 watchdog started (cap=${CTX_FTS5_MAX_MB}MB, polls every 5 min)"

# context-mode routing-bypass default.
#
# context-mode's enforce-ctx-mode.sh hook intercepts every Bash tool call
# and rejects it unless the command matches a small allowlist (git,
# mkdir, rm, mv, navigation). Routed calls are written into the FTS5
# index. On a 1-vCPU container this trade-off has proven net-negative:
# every `ls`, `df`, `ps`, `cat <small file>` becomes a 2-step round-trip
# through the MCP server and grows the index that the watchdog above
# has to keep nuking.
#
# The hook honors a sentinel file at /tmp/ctx-bypass: when present,
# Bash calls pass through to native execution untouched. Touching it
# at boot makes "native Bash by default; ctx_execute when explicitly
# desired" the production behaviour. Users who want full ctx_execute
# routing can `rm /tmp/ctx-bypass` at runtime; the sentinel does not
# survive container restart, so the default re-applies next boot.
#
# Override at run time with CODEFLARE_CTX_BYPASS_DEFAULT=false to keep
# the original "always route" hook policy.
if [ "${CODEFLARE_CTX_BYPASS_DEFAULT:-true}" = "true" ]; then
    touch /tmp/ctx-bypass 2>/dev/null || true
    echo "[entrypoint] context-mode routing-bypass enabled by default (rm /tmp/ctx-bypass to re-enable routing)"
fi
