...
# Production owns this fixed root; extracted test harnesses rewrite this one
# assignment when they need an isolated fixture.
...
echo "R2_ENDPOINT: ${R2_ENDPOINT:+configured}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
echo "R2_ACCESS_KEY_ID: ${R2_ACCESS_KEY_ID:+configured}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
echo "R2_SECRET_ACCESS_KEY: ${R2_SECRET_ACCESS_KEY:+configured}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
echo "R2_ACCOUNT_ID: ${R2_ACCOUNT_ID:+configured}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
echo "[entrypoint] === END R2 ENV STATUS ===" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
...
configure_pi_jiti_runtime_cache() {
    ...
    local runtime_tmp="$CODEFLARE_RUNTIME_ROOT/pi-tmp"
...
# Initialize sync log
init_sync_log() {
    echo "=== Sync Log Started: $(date '+%Y-%m-%d %H:%M:%S') ===" > $CODEFLARE_RUNTIME_ROOT/sync/sync.log
}
...
RCLONE_FILTERS_COMMON=(
    ...
    # Classic persists only the current Codeflare session's native agent UUID;
    # the agent-owned transcript follows its existing narrow sync policy.
    --filter "+ .codeflare/classic/"
    --filter "+ .codeflare/classic/sessions/"
    --filter "+ .codeflare/classic/sessions/cf-${SESSION_ID:-unknown}/"
    --filter "+ .codeflare/classic/sessions/cf-${SESSION_ID:-unknown}/agent-session-id"
    --filter "- .codeflare/classic/**"
    ...
    --filter "- .copilot/*.db-wal"           # SQLite WAL (ephemeral, unsafe to restore separately)
    --filter "- .copilot/*.db-shm"           # SQLite shared memory (ephemeral)

    # Codex — session recordings and SQLite temp files
    --filter "- .codex/sessions/**"          # TUI session recordings
    --filter "- .codex/plugins/cache/**"     # curated/plugin catalog cache (regenerated by plugin sync)
    --filter "- .codex/cache/**"             # app/tool metadata cache (regenerated)
    --filter "- .codex/logs*.sqlite*"        # log databases and WAL/SHM companions (ephemeral)
    --filter "- .codex/*.sqlite-shm"         # SQLite shared memory (ephemeral, corrupt on restore)
    --filter "- .codex/*.sqlite-wal"         # SQLite WAL (ephemeral, corrupt on restore)
    ...
    --filter "- .claude/projects/**/workflows/**" # per-session workflow artifacts (regenerated)
    ...
    --filter "- .claude/daemon-auth-*"       # daemon cooldown/status runtime state
    --filter "- .claude/gh-pr-status-cache.json" # regenerated PR status cache
    ...
    # MCP/server runtime state and desktop trash are ephemeral.
    --filter "- .local/state/**"
    --filter "- .local/share/code-server/coder-logs/**"
...
# ============================================================================
RECOVERY_FILTER_FILE="$CODEFLARE_RUNTIME_ROOT/sync/recovery-filters.txt"
...
recover_vanished_files() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$file_path" == workspace/* ]]; then
            echo "[sync-recovery] Workspace file vanished: $file_path (will retry without excluding)" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            recovered=1
            continue
        fi
        ...
        echo "[sync-recovery] Excluded vanished file: $file_path" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
        recovered=1
    ...
    for f in "$warm_src"/*.ts; do
        ...
        if [ -f "$dest/$base" ]; then
            if cp "$f" "$dest/$base"; then
                ...
                echo "[entrypoint] WARNING: managed-extension relay failed for ${base}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            fi
        fi
    done
        ...
        for f in "$bake_ext"/*.ts; do
            ...
            if [ ! -f "$dest/$base" ]; then
                if cp "$f" "$dest/$base"; then
                    ...
                    echo "[entrypoint] WARNING: mode-bake backfill failed for ${base}" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
                fi
            fi
        done
    else
        echo "[entrypoint] WARNING: mode-filtered bake missing at ${bake_ext}; skipping managed-extension backfill" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
    fi
...
establish_bisync_baseline() {
    ...
    for recovery_attempt in $(seq 1 $MAX_RECOVERY); do
        echo "[entrypoint] Step 2: Establishing bisync baseline (max ${BISYNC_TIMEOUT}s, attempt $recovery_attempt/$MAX_RECOVERY)..." | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
        ...
        if [ $SYNC_RESULT -eq 0 ]; then
            rm -f "$BASELINE_OUTPUT"
            echo "[entrypoint] Step 2 complete: Bisync baseline established" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            touch $CODEFLARE_RUNTIME_ROOT/sync/bisync-initialized
            SYNC_STATUS="success"
            return 0
        elif [ $SYNC_RESULT -eq 124 ]; then
            rm -f "$BASELINE_OUTPUT"
            echo "[entrypoint] WARNING: Bisync baseline timed out after ${BISYNC_TIMEOUT}s" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
            touch $CODEFLARE_RUNTIME_ROOT/sync/bisync-initialized
            SYNC_STATUS="timeout"
        ...
        if recover_vanished_files "$(cat "$BASELINE_OUTPUT")"; then
            rm -f "$BASELINE_OUTPUT"
            echo "[sync-recovery] Baseline attempt $recovery_attempt: excluded vanished file(s), retrying..." | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            rm -f "$SYNC_RUNTIME_DIR/rclone"/*.lck 2>/dev/null
            continue
        fi
    ...
    SYNC_STATUS="failed"
    echo "[entrypoint] ERROR: $SYNC_ERROR" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
    return 1
}
...
cleanup_agent_transcripts() {
    ...
    node "$SCRIPT" "$AGENT" "$ROOT" 10 2>&1 | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
}
...
bisync_with_r2() {
    ...
    local LOCK_FILE="$SYNC_RUNTIME_DIR/rclone/home_user..r2_${R2_BUCKET_NAME}.lck"
    local BISYNC_RUNNING=0
    if pgrep -f "rclone bisync" >/dev/null 2>&1; then
        BISYNC_RUNNING=1
    fi
    if [ -f "$LOCK_FILE" ] && [ "$BISYNC_RUNNING" -eq 0 ]; then
        echo "[sync] Removing stale bisync lock: $LOCK_FILE" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
        rclone deletefile "$LOCK_FILE" 2>/dev/null || rm -f "$LOCK_FILE"
    fi

    # Write output to known location so daemon can read it for recovery
    SYNC_OUTPUT="$CODEFLARE_RUNTIME_ROOT/sync/last-bisync-output.txt"
    ...
    cat "$SYNC_OUTPUT" >> $CODEFLARE_RUNTIME_ROOT/sync/sync.log
    cat "$SYNC_OUTPUT"
...
# PID at $CODEFLARE_RUNTIME_ROOT/sync/sync-daemon.pid. The trap toggles BISYNC_REQUESTED
...
start_sync_daemon() {
    ...
    while true; do
        ...
        if [ -f $CODEFLARE_RUNTIME_ROOT/sync/sync.log ] && [ "$(stat -c%s $CODEFLARE_RUNTIME_ROOT/sync/sync.log 2>/dev/null || echo 0)" -gt 524288 ]; then
            ...
            echo "[sync-daemon] Log rotated (exceeded 512KB)" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
        fi

        echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') Running periodic bisync..." | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
        ...
        if [ $SYNC_RESULT -eq 0 ]; then
            ...
            echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') Bisync failed with exit code $SYNC_RESULT (failure $CONSECUTIVE_FAILURES/3)" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            ...
            local LISTING_GLOB="$SYNC_RUNTIME_DIR/rclone/home_user..r2_${R2_BUCKET_NAME}.path*.lst"
            local HAS_LISTINGS=false
            # shellcheck disable=SC2086
            ls $LISTING_GLOB >/dev/null 2>&1 && HAS_LISTINGS=true

            if [ "$HAS_LISTINGS" = "false" ] && [ $SYNC_RESULT -eq 7 ]; then
                echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') No listing files found — immediate resync" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log
            ...
            if [ $CONSECUTIVE_FAILURES -ge 3 ]; then
                echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') 3 consecutive failures — falling back to --resync" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
                update_sync_status "failed" "Resync fallback triggered"
                if establish_bisync_baseline; then
                    echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') Resync fallback succeeded — resuming normal sync" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
                    CONSECUTIVE_FAILURES=0
                    update_sync_status "success" "null"
                else
                    local LAST_ERRORS
                    LAST_ERRORS=$(grep -i 'error\|fatal\|failed' $CODEFLARE_RUNTIME_ROOT/sync/sync.log | tail -3)
                    echo "[sync-daemon] $(date '+%Y-%m-%d %H:%M:%S') RESYNC FAILED — will retry next cycle. Recent errors:" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
                    echo "$LAST_ERRORS" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >&2
                    update_sync_status "failed" "${SYNC_ERROR:-Resync fallback failed}"
    ...
    SYNC_DAEMON_PID=$!
    echo "$SYNC_DAEMON_PID" > $CODEFLARE_RUNTIME_ROOT/sync/sync-daemon.pid
...
start_vault_monitor_daemon() {
    ...
    local MANIFEST="$VAULT_ROOT/graphify-out/vault-extract-manifest.json"
    ...
    while true; do
        ...
        # Content-hash change detection (REQ-VAULT-026): compare each vault
        # file's bytes against the persisted manifest, NOT its mtime. The R2
        # restore rewrites every vault file's mtime to download-time, so the
        # old `find -newer marker` matched the WHOLE vault on the boot where the
        # restore landed after the marker — the 200k-token full re-extraction.
        # The manifest survives restart in graphify-out/ (R2-synced allow-rule),
        # so "changed since last extraction" stays true across boots. The script
        # owns the exclusion set (agent-owned/derived subtrees + preseed pages)
        # and prints absolute paths, one per line. Guarded: the script is a
        # Claude-plugin asset absent under a Pi session (where this daemon still
        # runs but its output is unused), and `|| true` keeps a python error from
        # tripping `set -euo pipefail` and killing the loop.
        local CHANGED=""
        if [ -f "$MANIFEST_SCRIPT" ]; then
            CHANGED=$(python3 "$MANIFEST_SCRIPT" changed "$VAULT_ROOT" "$MANIFEST" 2>/dev/null | head -n 50 || true)
        fi
        ...
        if [ -n "$CHANGED" ]; then
            ...
            echo "[vault-monitor] $(date '+%Y-%m-%d %H:%M:%S') Detected vault changes, marker written" | tee -a $CODEFLARE_RUNTIME_ROOT/sync/sync.log >/dev/null
        fi
    done &

    VAULT_MONITOR_PID=$!
    echo "$VAULT_MONITOR_PID" > $CODEFLARE_RUNTIME_ROOT/services/vault-monitor.pid
...
start_silverbullet_supervisor() {
    ...
    setsid bash -c '
        ...
        while true; do
            ...
            "$SB_BIN" --hostname "$SB_HOST" --port "$SB_PORT" "$VAULT_ROOT" \
                >> $CODEFLARE_RUNTIME_ROOT/services/silverbullet.log 2>&1
            echo "[silverbullet] $(date '"'"'+%Y-%m-%d %H:%M:%S'"'"') exited (code $?), restarting in 5s..." | tee -a $CODEFLARE_RUNTIME_ROOT/services/silverbullet.log
            sleep 5
        done
    ...
    echo "$SILVERBULLET_SUPERVISOR_PID" > $CODEFLARE_RUNTIME_ROOT/services/silverbullet.pid
...
# extension inventory are container-local under $CODEFLARE_RUNTIME_ROOT/openvscode and
...
_openvscode_should_launch() {
    [ -n "${SESSION_ID:-}" ] \
        && [ -f "${CODEFLARE_INIT_FLAG_FILE:-$CODEFLARE_RUNTIME_ROOT/services/init-complete}" ] \
        && { [ "${CODEFLARE_SESSION_WORKSPACE:-terminal}" = "vscode" ] \
            || [ -f "${OPENVSCODE_REQUEST_TRIGGER:-$CODEFLARE_RUNTIME_ROOT/openvscode/requested}" ]; }
}
...
_openvscode_capture_extensions() {
    python3 /opt/codeflare/openvscode/browser-ide-extensions.py capture \
        ...
        --sync-pid-file "${CODEFLARE_SYNC_DAEMON_PIDFILE:-$CODEFLARE_RUNTIME_ROOT/sync/sync-daemon.pid}"
}
...
# $CODEFLARE_RUNTIME_ROOT/openvscode; only the post-reap REQ-IDE-002 UI allowlist
...
_openvscode_launch_once() {
    ...
    data_dir="${OPENVSCODE_DATA_DIR:-$CODEFLARE_RUNTIME_ROOT/openvscode/data}"
    ...
    CODEFLARE_SYNC_DAEMON_PIDFILE="${CODEFLARE_SYNC_DAEMON_PIDFILE:-$CODEFLARE_RUNTIME_ROOT/sync/sync-daemon.pid}" \
...
_openvscode_supervise_loop() {
    ...
    local data_dir="${OPENVSCODE_DATA_DIR:-$CODEFLARE_RUNTIME_ROOT/openvscode/data}"
    generation_pidfile="${OPENVSCODE_GENERATION_PIDFILE:-$CODEFLARE_RUNTIME_ROOT/openvscode/generation-${BASHPID}.pid}"
    ...
    while true; do
        ...
        CODEFLARE_OPENVSCODE_GENERATION="$current_generation" \
            ...
            >> $CODEFLARE_RUNTIME_ROOT/openvscode/openvscode.log 2>&1 &
        current_pid=$!
        ...
        _openvscode_cleanup_current_generation
        echo "[openvscode] $(date '+%Y-%m-%d %H:%M:%S') exited (code $exit_code), restarting in 5s..." >> $CODEFLARE_RUNTIME_ROOT/openvscode/openvscode.log
        sleep 5
    done
}
...
start_openvscode_supervisor() {
    ...
    export OPENVSCODE_GENERATION_PIDFILE="${OPENVSCODE_GENERATION_PIDFILE:-$CODEFLARE_RUNTIME_ROOT/openvscode/generation.pid}"
    ...
    echo "$OPENVSCODE_SUPERVISOR_PID" > $CODEFLARE_RUNTIME_ROOT/openvscode/supervisor.pid
...
shutdown_handler() {
    ...
    kill_pidfile_subtree $CODEFLARE_RUNTIME_ROOT/services/vault-monitor.pid
    ...
    if [ -f $CODEFLARE_RUNTIME_ROOT/sync/bisync-initialized ]; then
...
# is SIGKILLed leaving a stale lock, and pass 2 — seeing $CODEFLARE_RUNTIME_ROOT/sync/bisync-initialized
...
update_sync_status() {
    ...
    if [ "$error_val" = "null" ]; then
        jq -n --arg status "$1" --arg userPath "$USER_HOME" \
            '{status: $status, error: null, userPath: $userPath, ts: (now * 1000 | floor)}' > $CODEFLARE_RUNTIME_ROOT/sync/sync-status.json
    else
        jq -n --arg status "$1" --arg error "$error_val" --arg userPath "$USER_HOME" \
            '{status: $status, error: $error, userPath: $userPath, ts: (now * 1000 | floor)}' > $CODEFLARE_RUNTIME_ROOT/sync/sync-status.json
    fi
}
...
configure_tab_autostart() {
    ...
    # SESSION_ID, so a new Codeflare session starts empty while a stopped one
    # reopens its exact synced transcript. Herdr owns its equivalent reference.
    if [ "${CODEFLARE_TERMINAL_MODE:-classic}" != "herdr" ] \
        && [[ "${SESSION_ID:-}" =~ ^[a-z0-9]{8,24}$ ]]; then
        if [ -z "${TAB_CONFIG:-}" ]; then
            classic_agent_kind="claude"
        else
            configured_primary=$(printf '%s' "$TAB_CONFIG" | jq -er '
                [.[] | select(type == "object" and .id == "1" and (.command | type) == "string")]
                | if length == 1 then .[0].command else "" end
            ' 2>/dev/null || true)
            case "$configured_primary" in
                pi) classic_agent_kind="pi" ;;
                claude|claude\ --dangerously-skip-permissions) classic_agent_kind="claude" ;;
            esac
        fi

        if [ -n "$classic_agent_kind" ]; then
            classic_binding_dir="$USER_HOME/.codeflare/classic/sessions/cf-$SESSION_ID"
            ...
            if [ -f "$classic_binding_file" ]; then
                ...
                if { [ "$classic_agent_kind" = "pi" ] \
                    ...
                    if { [ "$classic_agent_kind" = "pi" ] \
                        ...
                        || { [ "$classic_agent_kind" = "claude" ] \
                            && find "$USER_HOME/.claude/projects" -type f -name "${classic_agent_id}.jsonl" -print -quit 2>/dev/null | grep -q .; }; then
                        classic_agent_state="resume"
                    else
                        classic_agent_state="unavailable"
                    fi
                else
                    classic_agent_state="unavailable"
                fi
            else
                classic_agent_id=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || true)
                if [[ "$classic_agent_id" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$ ]]; then
                    install -d -m 0700 "$classic_binding_dir"
                    classic_binding_tmp=$(mktemp "$classic_binding_dir/.agent-session-id.XXXXXX")
                    printf '%s\n' "$classic_agent_id" > "$classic_binding_tmp"
                    chmod 0600 "$classic_binding_tmp"
...
if [ -t 1 ] && [ -z "$TERMINAL_APP_STARTED" ]; then
    ...
    case "${TERMINAL_ID:-1}" in
        1)
            ...
            __CODEFLARE_CLASSIC_AGENT_LAUNCH__
...
BASHRC_EOF
        sed -i "s|__CODEFLARE_CLASSIC_AGENT_LAUNCH__|${classic_agent_launch:-claude --dangerously-skip-permissions}|" "$BASHRC_FILE"
    else
...
BASHRC_HEADER
        ...
        ${key})
            ...
            ${classic_agent_launch:-claude --dangerously-skip-permissions}
            ;;
CASE_EOF
                    ;;
                htop|yazi|lazygit)
...
CASE_EOF
                ...
                pi)
                    cat >> "$BASHRC_FILE" << CASE_EOF
        ${key})
            # Pi (bash stays as session leader for TTY stability)
            ${classic_agent_launch:-pi}
            ;;
CASE_EOF
                    ;;
...
BASHRC_FOOTER
    ...
    echo "configured" > $CODEFLARE_RUNTIME_ROOT/services/claude-autostart-status.txt
...
init_user_vault() {
    ...
    # (A preseed-page rewrite used to bump vault-extract.last so the next daemon
    # tick would not treat the agent cp as a user edit. That is obsolete under
    # content-hash detection: the four pages are excluded by vault-manifest.py,
    # and an unchanged page's bytes never register as a change regardless of mtime.)
    ...
    if command -v graphify >/dev/null 2>&1 && [ -f "$VAULT/graphify-out/vault-graph.json" ]; then
        flock -w 5 $CODEFLARE_RUNTIME_ROOT/locks/graphify-global.lock graphify global add \
    ...
    # vault-extract.last / vault-monitor.tick are ephemeral within-session
    # dedup + heartbeat markers (seed-if-absent so a mid-session restart doesn't
    # lose them). They are NO LONGER the change-detection source of truth — that
    # is the content-hash manifest below. Their mtimes only anchor the hook's
    # vars-staleness guard, which compares sibling ephemeral files.
...
# until $CODEFLARE_RUNTIME_ROOT/services/init-complete is written at the end of this script —
...
const required = [
  'npm:@gotgenes/pi-subagents@21.4.2',
...
update_pi_and_codex_when_fast_start_disabled() {
    ...
    local pi_installed=false codex_installed=false runtime_update_succeeded=false versions_resolved=true
    local specs=()
    npm_tools_dir="${CODEFLARE_NPM_TOOLS_DIR:-/opt/codeflare/npm-tools}"

    if command -v pi >/dev/null 2>&1; then
        pi_installed=true
        if latest_pi_version="$(npm view @earendil-works/pi-coding-agent@latest version 2>/dev/null)" &&
           printf '%s\n' "$latest_pi_version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+([-+][0-9A-Za-z.-]+)?$'; then
            specs+=("@earendil-works/pi-coding-agent@$latest_pi_version")
        else
            echo "[entrypoint] ERROR: Could not resolve the latest stable Pi version"
    ...
    if command -v codex >/dev/null 2>&1; then
        ...
        if latest_codex_version="$(npm view @openai/codex@latest version 2>/dev/null)" &&
           printf '%s\n' "$latest_codex_version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+([-+][0-9A-Za-z.-]+)?$'; then
            specs+=("@openai/codex@$latest_codex_version")
        else
            echo "[entrypoint] ERROR: Could not resolve the latest stable Codex version"
            versions_resolved=false
            update_failed=1
        fi
    ...
    if [ "${#specs[@]}" -gt 0 ] && [ "$versions_resolved" = true ]; then
    ...
    if [ "$pi_installed" = true ]; then
        ...
        if ! node /opt/codeflare/scripts/prune-npm-platform-artifacts.mjs "$npm_tools_dir/node_modules"; then
            echo "[entrypoint] ERROR: Could not prune alternate-platform npm payloads"
            update_failed=1
        fi
...
if [ "${SESSION_MODE:-default}" = "advanced" ]; then
    ...
    #
    # disableAgentView turns off background agents' agent view and its
    # supervisor. Agent view takes over the terminal with a full-screen
    # dispatch/session switcher, which is unusable on a phone — the swipe
    # navigation fights the mobile terminal. Background subagents themselves
    # still run, so the memory/vault capture hooks are unaffected — verified
    # under Claude Code 2.1.219 on 2026-07-25 by running a session with the
...
SYNC_PID=$(cat $CODEFLARE_RUNTIME_ROOT/sync/sync-daemon.pid 2>/dev/null || echo '')
if [ -n "$SYNC_PID" ]; then
