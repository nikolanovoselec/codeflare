...
3. **`createElement` monkey-patch** -- Uses `input[type=password]` instead of textarea (scoped to `terminal.open()`) to suppress autocorrect at OS level. Voice input is handled separately via the Web Speech API (`SpeechRecognition` / `webkitSpeechRecognition`) in `speech-input.ts` — completely decoupled from the keyboard/iframe input system. The floating microphone button starts recognition, and final transcribed text is sent directly to `terminal.input()`. `continuous=false` and `interimResults=false` for reliability. Hidden on browsers that don't support the API. Composition events (`compositionstart`/`compositionend`) buffer swipe typing text until the IME commits.
