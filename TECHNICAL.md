...
3. **`createElement` monkey-patch + dual-input voice toggle** -- Uses `input[type=password]` instead of textarea (scoped to `terminal.open()`) to suppress autocorrect at OS level. The iframe contains two inputs: `#pw` (password, default) and `#txt` (text, voice-friendly). Both are wired with identical event listeners; guards (`if (input !== _activeInput) return`) on `compositionend`, `keydown`, `input`, and `blur` ensure only the focused input's events are processed. The `focus` handler updates `_activeInput`, `iframeInputRef`, and the xterm WeakMap (`setIframeInput`). Toggling voice mode (`toggleVoiceMode()`) switches focus between the two inputs — the OS reconfigures its IME per element instance (attribute changes alone don't trigger IME reconfiguration). A `_switchingInput` flag (250ms TTL) suppresses blur handlers during focus transfer — Samsung fires stale `geometrychange` events during same-iframe focus switches that briefly report keyboard height as 0, causing `isVirtualKeyboardOpen()` to return false. Blur handlers also verify no terminal input is focused (`iframe.contentDocument.activeElement`) before disabling the keyboard overlay. Resets to password mode when keyboard closes. Composition events (`compositionstart`/`compositionend`) buffer voice dictation text until the IME commits.
