...
3. **`createElement` monkey-patch** -- Uses `input[type=password]` instead of textarea (scoped to `terminal.open()`). Password type suppresses autocorrect/autocomplete/autocapitalize at OS level. Voice input is available on demand via the floating microphone button (`mdiMicrophonePlus`), which toggles the iframe input between `type=password` (default, no autocorrect) and `type=text` (voice-friendly). Resets to password when keyboard closes. Composition events (`compositionstart`/`compositionend`) buffer voice dictation text until the IME commits.
