...
3. **`createElement` monkey-patch** -- Uses `input[type=text]` instead of textarea (scoped to `terminal.open()`). Previously used `type=password` to suppress autocorrect, but this blocked voice input (speech-to-text) on Android. Now uses `type=text` with `autocorrect=off`, `autocapitalize=off`, `spellcheck=false`, and `aria-autocomplete=none` attributes instead. Composition events (`compositionstart`/`compositionend`) buffer voice dictation text until the IME commits.
