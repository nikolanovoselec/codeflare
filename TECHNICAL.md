...
3. **`createElement` monkey-patch + dual-input voice toggle** -- Uses `input[type=password]` instead of textarea (scoped to `terminal.open()`) to suppress autocorrect at OS level. The iframe contains two inputs: `#pw` (password, default) and `#txt` (text, voice-friendly). Both are wired with identical event listeners; guards (`if (input !== _activeInput) return`) on `compositionend`, `keydown`, `input`, and `blur` ensure only the focused input's events are processed. The `focus` handler updates `_activeInput`, `iframeInputRef`, and the xterm WeakMap (`setIframeInput`). Toggling voice mode (`toggleVoiceMode()`) updates state and WeakMap but does NOT call `focus()` — cross-iframe focus from outside the iframe is unreliable on mobile. Instead, the FloatingTerminalButtons component calls `refocusTerminal()` after toggling, which uses `getIframeInput()` to focus the now-active input. Voice mode preference persists in `localStorage` (`codeflare:voiceMode`) and initializes from it on page load — the iframe `load` handler respects the persisted state when selecting the initial active input. `resetVoiceMode()` clears both module state and localStorage. A `_switchingInput` flag (250ms TTL) suppresses blur handlers during focus transfer — Samsung fires stale `geometrychange` events during same-iframe focus switches. Blur handlers verify no terminal input is focused (`iframe.contentDocument.activeElement`) before disabling the keyboard overlay. Composition events (`compositionstart`/`compositionend`) buffer voice dictation text until the IME commits.
