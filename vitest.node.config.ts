...
export default defineConfig({
  test: {
    environment: 'node',
    // Pinned, not inherited: these files call process.chdir and spawn
    // subprocesses, both of which throw ERR_WORKER_UNSUPPORTED_OPERATION under
    // the threads pool. Forks is vitest's current default, so relying on it
    // would make a pool switch or a major bump an opaque suite failure.
    pool: 'forks',
