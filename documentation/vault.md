...
2. `bootConfig.enableClientEncryption === true` — set by the Worker's `injectVaultEncryptionConfig` (`src/routes/vault.ts:191`), which rewrites the upstream `/.config` JSON before it reaches the SB client.
