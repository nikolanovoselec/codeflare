...
## Vault encryption and per-session IDB lifecycle (REQ-VAULT-008)

SilverBullet 2.8 ships full client-side IDB encryption via `EncryptedKvPrimitives` (`client/data/encrypted_kv_primitives.ts`). Activation requires three conditions checked in `client/boot.ts:96-143`: `localStorage["enableEncryption"]` is truthy, `bootConfig.enableClientEncryption === true` (set by the Worker in `injectVaultEncryptionConfig`), and a `CryptoKey` was posted to the per-origin service worker via `{type: "set-encryption-key"}`. With all three, SB wraps the `sb_data_<hash>` IDB so values are AES-GCM ciphertext at rest.
