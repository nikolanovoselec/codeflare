...
**Persistence across DO hibernate/wake (AC5):** The token is persisted to `ctx.storage` under key `containerAuthToken` on first generation and restored in the DO's `blockConcurrencyWhile` init alongside `bucketName` and other operational state. The persistence is non-negotiable: the container's `CONTAINER_AUTH_TOKEN` env var (set when the container process started) survives container hibernation, but the DO's in-memory `_containerAuthToken` field does not. Without storage-backed restore, every DO wake regenerates a fresh UUID via the lazy-init branch in `updateEnvVars()`, the proxied `Authorization: Bearer` header no longer matches the container's env var, and every request after the first wake returns `{"error":"Unauthorized"}` from `host/src/server.ts` -- the terminal, vault, and every other in-container HTTP surface become unreachable until the user manually recreates the session. The write is wrapped in `ctx.waitUntil(...)` so the Workers runtime cannot hibernate the DO before the storage put commits, closing the wake-then-immediately-hibernate window. On `destroy()` the key is removed so the next session under the same DO ID starts with a fresh token.
