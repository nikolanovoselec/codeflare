...
- Container build/scan/push moves to reusable `container-image.yml`. PR complete-image verification and deployment import one GHCR-backed BuildKit cache while preserving image outputs. PR verification receives read-only package access; deployment alone publishes cache updates. Fork PRs and Dependabot do not authenticate to it.
