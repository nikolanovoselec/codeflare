...
- Later refinement ([REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-15-user-messages), 2026-06-19): Pi treats the `.vars` carrier as the pending-capture lock and advances the prompt counter only after the capture note is written. A stopped capture therefore leaves the old counter intact and retries after the stale pending marker clears instead of skipping the 15-prompt window.
