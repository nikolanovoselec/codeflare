...
Automatic reconciliation compares the exact applied gzip directly with the active target, retaining only per-path content fingerprints. It writes added or release-changed target paths through at most six concurrent R2 operations and treats a target-digest object marker as a durable completion checkpoint. A release-identical path is untouched even when a user edit removed its marker. Fresh buckets and valid cache-missing histories plan the full target and use the same marker-backed resume. Direct cleanup considers only paths proven absent from the target and preserves markerless mutable content; observational KV progress never controls execution. Manual Recreate and non-dashboard callers retain their full-overwrite behavior. Applied state remains last, after context reconciliation and a final target, mode, policy, SSE, session-ownership, and migration check. Disabling the feature still converges through the existing path back to the baked image hash. <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest -->
