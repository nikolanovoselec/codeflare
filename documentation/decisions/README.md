...
- **Bounded, verified driver that always releases its lease.** The dashboard `batch-status` poll calls `planRegimeReconcile` synchronously (reports `bucketMigrating`) and `waitUntil(advanceMigration)` runs the copy work in **small pages** (`MIGRATION_PAGE_SIZE`) with concurrency capped at the platform's **6-outgoing-connection-per-invocation limit** (`MIGRATION_CONCURRENCY`), each R2 op under a **per-op timeout** (`withTimeout`). It persists the `cursor` after every page and transitions migrate→verify in one invocation when budget remains. Crucially, each invocation **stops voluntarily** — within `DEADLINE_MARGIN_MS` of an ≈8s budget (`WORK_BUDGET_MS`, well under the ~30s `waitUntil` ceiling) or before its estimated R2-op count nears `MAX_SUBREQUESTS` — and **releases the lease**, so the next poll resumes from the cursor; `MIGRATION_LEASE_MS` (60s) is now only a crash backstop. A migrate phase re-encrypts; a verify phase HEAD-scans every object under the target regime; the regime + `generation` advance **only after a clean full verify**. *(This took two iterations to land, both verified live on the 680-object integration bucket. Draft 1 ran one sequential page per poll with a 10-min lease → a page blew the `waitUntil` clock, was killed mid-page before persisting the cursor, and the long lease then blocked every poll until expiry. Draft 2 added concurrency + a multi-page loop on a 15s wall-clock budget — but 16-way concurrency exceeds the 6-connection limit and the loop overran the ~30s `waitUntil` window, so the isolate was force-killed (a kill is not a catchable exception) and the lease stayed **held** → still stalled (verified: lease held + `updatedAt` frozen while the dashboard polled). The fix is to bound each invocation under BOTH platform ceilings — outgoing connections AND the `waitUntil` window — with per-op timeouts, and exit voluntarily so the lease-release always runs. Confirmed via a second GPT-5.5 consult over the live KV/worker forensics.)*
