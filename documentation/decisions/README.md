...
**Decision:** CI monitoring is independent of review. After an eligible push or PR creation, the root Git workflow issues required reviewer calls first, then invokes the sole automatic CI resolver with the affected repository cwd and explicit review launch state. CI launches last without waiting for review completion. The resolver returns no action or one public background `ci-monitor` request for the authoritative PR number and `headRefOid`. The dedicated agent runs one attached Node process and returns `CI_RESULT` through native task notification; it uses no detached process or durable CI state.
