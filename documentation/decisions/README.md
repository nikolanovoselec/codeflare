...
| [AD135](#ad135-inline-chat-requires-one-host-correlated-result) | Require one host-correlated Inline result | Inline turns are edit-first and force one host-correlated result on Chat Completions and Responses routes, preventing optional tools and copied IDs from breaking settlement. | Architecture, Security | Active |
...
**Amendment (2026-08-20):** Every editor Inline invocation is now framed as a requested document change. `noChange` is reserved for an already-satisfied request or one for which no valid safe edit can be produced; the earlier informational escape hatch and rejection of an edit-first contract are historical. The exact result-tool constraint now covers structurally recognized OpenAI Chat Completions and Responses payloads, while unknown provider shapes remain unchanged.
