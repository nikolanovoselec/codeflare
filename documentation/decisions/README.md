...
| [AD105](#ad105-pi-extensions-invoke-exact-tools-through-the-normal-public-pipeline) | Pi extensions invoke exact tools through the normal public pipeline | Agents, Architecture |
...
The fix is the synchronous-drain RPC the "Alternative considered" below originally rejected: before signalling stop, the DO drains a fresh bisync while the container is still running and the DO holds teardown open. `drainFinalSync` (container-metrics.ts) calls a new awaitable host endpoint `POST /internal/final-sync` (host/src/server.ts) that triggers the daemon via SIGUSR1 and blocks until that run reaches a terminal status; completion is detected by a monotonic epoch-ms `ts` stamp on `sync-status.json` plus a `syncing` emission before each daemon run, so the endpoint waits for OUR triggered run (`syncing` stamped strictly after the trigger, then `success`/`failed`) and ignores an already-in-flight bisync.
...
**Decision:** CI monitoring is independent of review completion and acknowledgement ([REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring)). After an eligible head-changing boundary, the PR-boundary extension emits one ordered launch plan and invokes it through Pi's exact public tool pipeline: required reviewer calls first, then the existing resolver once with the affected repository cwd and explicit review launch state. The resolver returns no action or one public background `ci-monitor` request for the authoritative PR number and `headRefOid`, which the extension invokes unchanged without another provider turn.
...
**Related REQ:** [REQ-AGENT-081](../../sdd/spec/agents.md#req-agent-081-rpiv-todo-session-isolation), [Pi preseed](../lanes/preseed.md#agent-seed-system).
...
**Decision:** The root Pi session invokes public background requests through Pi's exact normal tool pipeline and reconstructs launch, running, failure, success, reminder, and GIVEUP state from durable session JSONL. Each visible launch stores the same bounded request objects in durable details metadata that the extension passes as exact tool arguments; no provider turn or model-chosen dispatch is involved. One tiny active request-ID pointer enables reload discovery, while one request-specific immutable execution snapshot prevents a late call from reading replacement work. Memory counters and the committed Vault manifest advance only after exact native success plus request-specific post-commit artifacts; Vault promotion validates staged bytes, prelaunch edits coalesce, and during-run edits become one follow-up request. Request-specific chunks and one lock spanning cumulative merge plus global publication prevent cross-request graph corruption. [AD103](#ad103-pi-extraction-agents-use-bounded-medium-reasoning-and-one-pass-inputs) bounds the workers without changing this delivery ownership.
...
Vault extraction builds a compact semantic graph rather than an exhaustive inline-token inventory. Noncritical visualization receives 15 seconds and cannot hold task success open.
...
### AD105: Pi extensions invoke exact tools through the normal public pipeline

**Category:** Agents, Architecture

**Status:** Accepted (2026-07-15)

**Context:** Custom messages can expose an exact subagent request to the root model, but they cannot guarantee that the model calls the tool. Direct `SubagentsService.spawn()` is deterministic but bypasses Pi's assistant tool-call entry, parent tool-use ID, ordinary hooks, and public transcript contract. Live extraction evidence showed a visible request followed by an invented completion and an invalid lookup using the extraction request ID as an agent ID. Review and CI plans shared the same liveness gap.

**Decision:** Pi gains an upstream `ExtensionAPI.invokeTools()` primitive that accepts exact registered tool names and argument objects, requires an idle session, and executes them through the same validation, hooks, lifecycle events, assistant tool-call/tool-result transcript entries, persistence, rendering, and cancellation path as model-produced tool calls without making a provider request. Codeflare's memory, Vault, PR-review, and boundary-CI extensions use this primitive only for dispatch; their existing triggers, exact-head/range correlation, native notifications, bounded reminders, artifact qualification, and root-only triage/mutation ownership remain separate.

The boundary extension records a visible plan, then invokes all required reviewers together when the root turn settles. It runs the existing CI resolver only after those calls start and invokes its zero-or-one request unchanged. Exact direct-user `FULLY AUTONOMOUS` activation is transcript-derived for reviewer prompt transport and ends only on an exact direct-user `CANCEL FULLY AUTONOMOUS` or `STOP FULLY AUTONOMOUS` marker. The model never interprets a launch plan as execution proof.

**Alternatives considered:** Stronger prompt wording remains probabilistic. Private service spawn loses public tool identity and would require new result relays and reload mappings. A Codeflare-owned dispatcher or durable queue duplicates existing workflow reducers and completion state. Keeping model-mediated review while fixing extraction leaves the same liveness defect in reviewer and CI launch plans.

**Consequences:** Automatic work has genuine public tool-use IDs and ordinary transcript evidence while remaining provider-independent. Visible launch messages no longer trigger a model turn or contain imperative duplicate-launch instructions. Codeflare requires an upstream Pi release containing `ExtensionAPI.invokeTools()` before this branch can be deployed; older Pi builds fail visibly rather than silently falling back to probabilistic dispatch. Root reasoning, findings triage, source changes, Git writes, and high-water promotion remain root-owned.

**Related REQ:** [REQ-MEM-014](../../sdd/spec/memory.md#req-mem-014-pi-capture-contract-transcript-prefilter-and-model-fidelity-lever), [REQ-MEM-015](../../sdd/spec/memory.md#req-mem-015-pi-extraction-transcript-visibility-and-child-session-guard), [REQ-VAULT-027](../../sdd/spec/vault.md#req-vault-027-pi-vault-extraction-delivery-is-visible-and-transactional), [REQ-AGENT-036](../../sdd/spec/agents.md#req-agent-036-pr-boundary-review-trigger-conditions), [REQ-AGENT-071](../../sdd/spec/agents.md#req-agent-071-pr-boundary-review-agent-dispatch), [REQ-AGENT-080](../../sdd/spec/agents.md#req-agent-080-unified-pi-pr-boundary-launch-plan), [REQ-AGENT-084](../../sdd/spec/agents.md#req-agent-084-pi-reviewer-policy-contract).
