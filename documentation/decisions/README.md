...
- **Concurrent, budgeted, verified driver.** The dashboard `batch-status` poll calls `planRegimeReconcile` synchronously (reports `bucketMigrating`) and `waitUntil(advanceMigration)` runs the copy work. Each page's objects are re-encrypted **concurrently** (`MIGRATION_CONCURRENCY`); `advanceMigration` **loops pages within a wall-clock budget** (`WORK_BUDGET_MS`), persisting the `cursor` after every page and transitioning migrate→verify within one invocation, so a typical bucket converges in one poll. On budget exhaustion it **releases the lease** (`MIGRATION_LEASE_MS`, sized to one invocation) so the next poll resumes immediately from the cursor. A migrate phase re-encrypts; a verify phase HEAD-scans every object under the target regime; the regime + `generation` advance **only after a clean full verify**. *(The original AD91 draft ran one sequential page per poll; on a real ~hundreds-of-objects bucket a sequential page exceeded the `waitUntil` wall-clock and was killed mid-page before persisting the cursor, and the lease then blocked every poll until expiry — verified live (86/680 objects, frozen). Concurrency + the budget loop + per-page persistence + lease-release-on-exit + a short lease fix it: throughput and resumability both bounded.)*
