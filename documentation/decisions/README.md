...
**Consequences:** Codeflare owns the sidebar renderer, PTY/RPC transports, security boundary, and behavioral tests. The image adds a small MIT-licensed terminal renderer and native addon instead of the official VSIX. Release remains blocked until the exact image proves native-addon compatibility, Claude history isolation and pre-execution prompts, Pi queued approvals, bounded resource use, and descendant cleanup. Approved commands and executable customizations remain trusted code in the shared container rather than a sandbox.
