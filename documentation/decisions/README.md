...
- **WebSocket upgrades** reaching the `EgressController` (e.g. browser-run Chrome DevTools at `…/browser-rendering/devtools/…`) are proxied **verbatim** through the same account-scoped selector and the upstream `101`+`webSocket` is returned as-is — this is what makes browser-run's interactive CDP work under strict egress.
