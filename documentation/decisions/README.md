...
Acknowledgement itself was also making the wrong claim. Advancing the checkpoint on lane exit records that three processes ran, not that anything was read, so a round returning into a session that never triaged it moved the checkpoint past its own unacted findings. The gate now requires the triage verdict — recognised structurally, by the table header and divider anywhere in the assistant text of a message following the last lane's completion, so the checkpoint stays visible without stalling the turn on it — and then issues the fix directive itself rather than trusting the session to remember. The Pi enforcement path already worked this way, so both runtimes key on one table shape. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::triage_published_after_line -->

A published verdict can also fail to reach the transcript at all: a message whose tool call this same gate rejects is never persisted by the harness, so a table sent only as chat text can be invisible to a later scan. The gate accepts either channel as proof — the stacked table in assistant text, or a round-stamped, session-keyed file the directive names (first line `round: <completion-line>`) that the gate itself permits writing while blocked — keeping the table user-visible while the file carries the machine-checked state. The stamp is a bare line number a compaction rewrite can re-issue, so closing the round consumes the file. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::triage_file_current -->
