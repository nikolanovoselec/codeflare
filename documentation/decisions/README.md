...
At each user prompt, after context-mode's lazy bridge registration and before provider serialization, the schema-free finalizer selects only `read`, `bash`, `edit`, `write`, and `capability`. Registered optional tools remain searchable and activate additively for the next model step. Active Goal and Plan workflows are narrow exceptions. An active Goal retains its three terminal controls. An active Plan outranks non-active Goal history and retains its frozen read-only policy plus two helpers; configured `always` Goal visibility also retains Goal's terminal tools. Plan Mode 0.55.3 is transformed at image build so `/plan` resolves helpers and configured policy from registered tools before filtering. Normal turns still expose exactly five tools. <!-- @impl: preseed/agents/pi/extensions/zz-tool-exposure-finalizer.ts::finalizeToolExposure --> <!-- @impl: preseed/agents/pi/extensions/capability-helpers.ts::registerInitialToolFilter --> <!-- @impl: scripts/patch-pi-plan-mode-tool-policy.mjs::patchPiPlanModeSource -->
