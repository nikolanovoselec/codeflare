...
Terminal admission reads persisted Container SDK state without waking the container. It emits 4503 only for stopped states and retryable 1013 when state or readiness is unavailable. Managed reconciliation checks that state for every listed session, so stale KV status cannot strand an update or permit hot mutation. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/lib/session-helpers.ts::hasOwningSessionContainer -->
