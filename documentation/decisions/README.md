...
**Context:** Terse editor change requests could still settle as informational `noChange` results because the shared prompt left that outcome to model discretion. GPT-5.6 Sol uses the OpenAI Responses shape, where automatic reasoning-summary selection can leave the panel silent until answer text arrives. The existing host-owned result envelope, range validation, native Keep/Close lifecycle, and Qwen reasoning presentation remain correct. <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::registerInlineEditMode --> <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::requestSidebarReasoningSummary --> <!-- @test: host/__tests__/pi-inline-edit-mode.test.js (REQ-IDE-025: inline result mode isolates provider tools and context then restores panel tools) --> <!-- @test: host/__tests__/pi-inline-edit-mode.test.js (REQ-IDE-027: sidebar panel requests ask OpenAI Responses models for visible reasoning summaries) -->
