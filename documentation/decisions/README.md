...
| [AD127](#ad127-native-inline-chat-uses-proposal-only-pi-turns-and-host-owned-text-edits) | Native Inline Chat uses proposal-only Pi turns and host-owned text edits _(review lifecycle superseded by [AD128](#ad128-inline-review-lifecycle-belongs-to-the-pinned-controller))_ | Architecture, Security, partially superseded |
| [AD128](#ad128-inline-review-lifecycle-belongs-to-the-pinned-controller) | Inline review lifecycle belongs to the pinned controller | Architecture, Security |
...
**Status:** Accepted (2026-08-16). Partially supersedes [AD114](#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration) only for editor Inline Chat execution. Its extension-owned confirmation, notification actions, decision registry, and editor reopening were superseded later that day by [AD128](#ad128-inline-review-lifecycle-belongs-to-the-pinned-controller); proposal-only Pi turns and host-owned edit validation remain active. Implements [REQ-IDE-020](../../sdd/spec/browser-ide.md#req-ide-020-native-pi-editor-proposal-execution), [REQ-IDE-025](../../sdd/spec/browser-ide.md#req-ide-025-shared-ide-pi-surface-isolation), [REQ-IDE-026](../../sdd/spec/browser-ide.md#req-ide-026-native-inline-chat-edit-validation), [REQ-IDE-028](../../sdd/spec/browser-ide.md#req-ide-028-native-inline-chat-dispatch-error-isolation), [REQ-IDE-029](../../sdd/spec/browser-ide.md#req-ide-029-native-inline-chat-feedback), and [REQ-IDE-030](../../sdd/spec/browser-ide.md#req-ide-030-native-inline-chat-proposal-envelope).
...
**Context:** Integration proved that Pi can generate valid host-owned edits, but extension-owned confirmation clicks create another participant request. The pinned extension host resolves that request's editor document before invoking Codeflare and throws when a review-owned URI is absent from `ExtHostDocuments`. Uploaded runtime evidence also showed duplicate file tabs and a side editor group. The pinned host supplies the invoking editor through private `location2`; Codeflare instead read whichever editor had focus when its handler ran. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate -->
...
**Decision:** Use `request.location2.document`, selection, and whole range as the sole editor identity. Missing or malformed editor location fails before Pi starts. Match the [pinned Copilot stream contract](https://github.com/microsoft/vscode/blob/df53daabb18cd157bdb08c7f01c34df936cf12f4/extensions/copilot/src/extension/intents/node/editCodeIntent.ts#L154-L160) with an empty text-edit start marker, one validated edit batch, and a completion marker for the same URI. Manage `accessibility.openChatEditedFiles` to `false` in the Pi profile because the [pinned chat-edit observer otherwise opens each edited URI](https://github.com/microsoft/vscode/blob/df53daabb18cd157bdb08c7f01c34df936cf12f4/src/vs/workbench/contrib/chat/browser/chatEditing/chatEditingServiceImpl.ts#L240-L259). `InlineChatController` owns Keep/Close, settlement, disposal, and navigation. Remove extension confirmation, notification actions, direct Chat Editing commands, decision correlation, and document reopening. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::parseInlineEditorLocation --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-020 + REQ-IDE-026 + REQ-IDE-029 + REQ-IDE-033: inline edits stay bound to the invoking host document) --> <!-- @test: openvscode/claude/test/managed-settings.test.mjs (REQ-IDE-018 + REQ-IDE-019 AC6 + REQ-IDE-021 AC1 + REQ-IDE-033: Pi settings keep Inline edits in the invoking editor) -->
...
**Consequences:** Pi execution, shared conversation, proposal-only editor tools, and host-owned edit validation remain unchanged. Result details retain the bounded explanation, but the pinned Inline view may not display it durably. Runtime acceptance now requires one invoking editor, native Keep/Close, immediate second requests after both outcomes, and no missing-document error before review resumes.
