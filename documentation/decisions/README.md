...
**Status:** Superseded by [AD95](#ad95-vault-reload-skip-strict-start-match-recorded-at-prewarm-completion) (2026-07-05) — the marker-keyed-to-`lastStartedAt` premise is retained; only the exact-match gate comparison is replaced.
...
**Context:** AD93 keyed the reload-skip to `lastStartedAt` but had two defects. (1) `Layout.tsx` read the container start as a TRACKED reactive value inside the on-demand prewarm effect, so a batch-status poll delivering `lastStartedAt` mid-prewarm re-ran the effect; its `onCleanup` cancelled the in-flight bootstrap iframe and cleared its status, so a fresh session's prewarm never reached `ready` and the control stuck breathing `preparing` (white), never arming green. (2) The start was captured at click time, so a click before the first poll recorded no marker — every return then re-initialized. A first attempt to relax the gate (record a placeholder, treat unknown/placeholder as warm) traded these for a worse bug: a resumed session whose start had not re-polled read warm and skipped prewarm onto stale files.
