...
| [AD74](#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api) | Route enterprise LLM calls through AI Gateway APIs | Discovered contracts bind compat; historical Dynamic assignments retain REST-first, while [AD152](#ad152-generalize-native-and-custom-provider-compat-dispatch) covers native/custom dispatch. | Architecture, Security | Partially superseded |
...
| [AD87](#ad87-egresscontroller-re-signs-own-account-r2-container-holds-a-placeholder-key-bridges-websocket-upgrades-and-resolves-strict-via-props) | Re-sign R2 and bridge WebSockets at the egress controller | R2 re-signing and WebSocket bridging remain; [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container) owns browser-token injection, while [AD143](#ad143-strict-r2-interception-signs-only-with-the-bound-users-scoped-credential) replaces deployment-wide R2 signer authority. | Architecture, Security | Partially superseded |
...
| [AD123](#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules) | Let Claude's FIX directive own conditional delivery | Claude's delivery decision remains; [AD156](#ad156-pi-fix-directive-owns-automatic-delivery) replaces only the former Pi standing-rule behavior with explicit automatic delivery. | Architecture, Cost | Partially superseded |
...
| [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) | Make D1 the complete session lifecycle authority | D1 ownership remains active; [AD157](#ad157-reset-stale-stopping-ownership-on-owner-status-reads) replaces only the requirement that every `stopped` transition have confirmed-exit evidence. | Architecture, Storage, Reliability | Partially superseded |
| [AD156](#ad156-pi-fix-directive-owns-automatic-delivery) | Make Pi's FIX directive own automatic delivery | After accepted corrections, Pi's FIX directive orders the push while preserving no-merge and no-op protections. | Agents, Architecture | Active |
| [AD157](#ad157-reset-stale-stopping-ownership-on-owner-status-reads) | Reset stale stopping ownership on owner status reads | Owner status reads reset three-minute stale Stop bookkeeping so an interrupted completion cannot retain lifecycle ownership indefinitely. | Architecture, Storage, Reliability | Active |
...
| [AD148](#ad148-memory-and-vault-capture-follow-successful-prompt-cadence) | Schedule memory and Vault work from successful prompt high-water marks | Memory capture and Vault checks run every 20 real prompts and on eligible resumed tails, without polling or consuming failed scans. | Agents, Memory, Storage | Active |
...
| [AD151](#ad151-container-lifecycle-and-terminal-transport-outrank-negative-eventual-kv-evidence) | Resolve lifecycle ownership outside eventual KV | Local startup and transport ownership still guard stale projections; SDK-only terminal stop authority is replaced by owner-scoped D1 ([REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC1–AC2). | Architecture, Session lifecycle, Storage | Partially superseded |
...
The former standalone `graph-first.md` rule was absorbed into `engineering-constitution.md` in 2026-07 and removed from it in 2026-09 when the constitution became technology-neutral. `graphify-routing.md` now owns the smaller advanced-mode query boundary.

Tier-gating is not part of the decision: Graphify query capability ships uniformly. Routing, mutation workflow, clone triage, active-repo tracking, and soft nudges remain keyed to advanced session mode.

**Consequences:**
- Default sessions receive query capability for explicit use, but no conditional route, clone triage, mutation skill, active-repo tracker, or PreToolUse nudge.
- Advanced sessions add the build/update workflow, clone choice, active-repo precision, and soft reminder.
...
- Later refinement ([REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-20-user-messages-and-on-resume), 2026-06-19): Pi treats the `.vars` carrier as the pending-capture lock and advances the prompt counter only after the capture note is written.
- Capture retry impact ([REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-20-user-messages-and-on-resume) AC5-AC6): a stopped capture keeps the prior counter and retries after the stale pending marker clears.
...
- The dashboard initially became a pure mirror of KV `status`; [AD151](#ad151-container-lifecycle-and-terminal-transport-outrank-negative-eventual-kv-evidence) later added bounded startup/transport guards and persisted-state terminal authority after eventual-consistency races disproved raw negative KV authority.
...
**Status:** Accepted (2026-06-05); partially superseded by [AD152](#ad152-generalize-native-and-custom-provider-compat-dispatch) for native/custom-provider dispatch and by the current discovery amendment below for newly discovered compatibility contracts. Historical Dynamic assignments retain REST-first transport.
...
**Token-scope consequence (supersedes the `AIG_TOKEN` consequence above):** because the two transports authenticate differently — REST API via `Authorization: Bearer` (Workers AI scope), compat via `cf-aig-authorization: Bearer` (AI Gateway Run scope) — `AIG_TOKEN` must carry both data-plane permissions (a token with only one is rejected by the other transport with `error 10000`). The capability-profile amendment below additionally requires AI Gateway Read for route inventory. This narrows the original "no migration debt" consequence: the transport still depends on the deprecated compat path for any provider not yet on the REST API (Google today), until Cloudflare closes the gap.
...
**Capability-profile administration amendment (2026-09-05):** The REST-first transport decision is unchanged. Chat Completions now resolves the selected route's one active immutable capability-profile revision and translates Pi's canonical reasoning level before the request reaches AI Gateway. Dynamic Routes remain opaque administrator-owned policy: AI Gateway chooses primary, conditional, percentage, limit, and fallback legs after translation, so per-leg profile references and discovery results are evidence only and never runtime selectors. `/responses` remains route-pinned passthrough, compat remains available only after a complete REST `404`, compat still removes only `store` and `prompt_cache_key`, and `ensureStreamTerminator` remains unchanged.

Administration now auto-discovers Dynamic Route names and active versions with the configured Worker-side gateway credential. Consequently `AIG_TOKEN`/`setup:aig_token` needs **AI Gateway Read** in addition to the already-required **Workers AI** and **AI Gateway Run** permissions. The same token stays Worker-side. The new admin surface adds exactly three read/discovery routes under `/api/admin/reasoning`: catalog, active-route inventory, and one-target deterministic Pi 0.84.4 streaming/tool/replay discovery. Saving custom revisions, route-wide assignments, and evidence remains owned by the existing configuration preview/run/Apply flow and its revision and warning-confirmation contract.

The code-owned executable catalog contains exactly seven revision-1 profiles, adding `bedrock-anthropic-compat` to the six profiles above. GPT-OSS tool replay, Gemini Chat Completions tools, GPT-6 Astra tools, and Responses-required behavior are notices, not assignable profiles. Custom profiles are immutable, hash-addressed, bounded scalar Chat Completions mappings; custom-provider backend identity is administrator-declared provenance. Custom revisions and exact assignments share one validated `setup:reasoning_configuration` document, while context windows retain their separate numeric key. Legacy GLM/Kimi assignments are proposed for migration; GPT-OSS remains unresolved and is never silently activated.

**Native Bedrock amendment (2026-09-09):** AD74's REST-first and zero-copy assertions remain the Dynamic Route behavior. A currently authorized opaque native target is the deliberate exception: the interceptor resolves its exact `aws-bedrock/<model>` selector Worker-side, sends one direct legacy compat request, strips incompatible OpenAI-only fields, and applies only the profile-gated repeated-complete tool-name stream repair ([REQ-ENTERPRISE-050](../../sdd/spec/enterprise-mode.md#req-enterprise-050-native-provider-compat-dispatch), [REQ-ENTERPRISE-059](../../sdd/spec/enterprise-mode.md#req-enterprise-059-native-provider-wire-adaptation)).

Native targets use provider-default reasoning and never claim configurable effort levels. Provider binding and exact model identity remain outside the container under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-provider-discovery-and-selection), [REQ-ENTERPRISE-049](../../sdd/spec/enterprise-mode.md#req-enterprise-049-unified-enterprise-model-authorization), [REQ-ENTERPRISE-050](../../sdd/spec/enterprise-mode.md#req-enterprise-050-native-provider-compat-dispatch), [REQ-ENTERPRISE-052](../../sdd/spec/enterprise-mode.md#req-enterprise-052-native-provider-verification-and-runtime-enforcement), [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-and-document), [REQ-ENTERPRISE-055](../../sdd/spec/enterprise-mode.md#req-enterprise-055-native-target-authority-and-save), [REQ-ENTERPRISE-058](../../sdd/spec/enterprise-mode.md#req-enterprise-058-native-model-container-publication), [REQ-ENTERPRISE-059](../../sdd/spec/enterprise-mode.md#req-enterprise-059-native-provider-wire-adaptation), [REQ-ENTERPRISE-060](../../sdd/spec/enterprise-mode.md#req-enterprise-060-native-target-input-validation), and [REQ-ENTERPRISE-061](../../sdd/spec/enterprise-mode.md#req-enterprise-061-native-target-administration-projection).

**Current discovery amendment (2026-09-13):** Newly discovered compatibility contracts explicitly bind `/compat/chat/completions`; discovery and runtime share the saved streaming or bounded completed-JSON contract and narrow complete-tool-name repair. REST-first dispatch with complete-404 compat replay remains the historical-assignment behavior, not the new discovery path. Earlier transport and zero-copy statements above are historical within this amended scope. See [Target capability discovery](../lanes/target-capability-discovery.md#contracts-not-a-model-list) and [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection). <!-- @impl: src/lib/ai-capability-discovery/index.ts::capabilityCandidates --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor -->

**Independent-capability clarification (2026-09-13):** The linked current-discovery descriptions now report tools/replay, reasoning, streaming and input caching independently, not cumulative grades or a cache prerequisite. This narrowly replaces their prior qualification description, not AD74 transport, Worker credentials, historical assignments or replay boundaries. Exact server receipts remain mandatory; strict legacy evidence retains its original authority rules without silent promotion. This amendment records the accepted contract, not new live or deployed acceptance.
...
**Related:** [REQ-AGENT-055](../../sdd/spec/agents.md#req-agent-055-pi-session-scoped-review-window), [REQ-AGENT-171](../../sdd/spec/agents.md#req-agent-171-user-scoped-review-completion-and-common-consent), [AD78](#ad78-pr-boundary-review-lanes-run-in-parallel-report-only-reviewers), and [AD98](#ad98-pi-pr-review-uses-visible-session-scoped-agents).
...
**Amendment (2026-09-07):** Retain server-modtime and fast listing. Pin the narrow rclone bookkeeping patch: each completed transfer records its own destination metadata rather than the source timestamp for both sides ([REQ-STOR-043](../../sdd/spec/storage.md#req-stor-043-completed-transfer-acknowledgment)). Validate upload identity against the existing S3 HEAD response, without adding all-object metadata requests ([REQ-STOR-042](../../sdd/spec/storage.md#req-stor-042-per-side-sync-change-tracking)). PR CI rejects unreviewed pins and incompatible exact source, then compiles and runs a focused completion-metadata probe. <!-- @impl: .github/workflows/test.yml::jobs.rclone-bookkeeping --> <!-- @impl: scripts/ci/rclone-bookkeeping-source.py::main --> <!-- @impl: scripts/ci/rclone-bookkeeping-source.py::compile_behavior_probe -->

This keeps the required PR lane below 60 seconds while the deployment image build still compiles the complete patched binary and runs its focused Go behavioral regression; the retained S3 regression remains available for release diagnosis. Disk-full errors block automatic resync until the cloud Sync now action requests recovery after local space is freed ([REQ-STOR-041](../../sdd/spec/storage.md#req-stor-041-disk-space-recovery)). Conflict copies are retained rather than blindly deleted ([REQ-STOR-042](../../sdd/spec/storage.md#req-stor-042-per-side-sync-change-tracking)). See [REQ-STOR-003](../../sdd/spec/storage.md#req-stor-003-bidirectional-sync-every-15-minutes-with-manual-triggers) and [REQ-STOR-017](../../sdd/spec/storage.md#req-stor-017-faster-startup-sync--bisync-head-storm-fix--governed-mode-preseed-bake).
...
**Amendment (2026-09-02):** SSE-C initial restore now uses rclone's default size plus modification-time comparison instead of `--size-only`. This preserves the no-bake boundary while materializing same-size R2 changes. Governed Mode retains the checksum path. <!-- @impl: entrypoint.sh::initial_sync_from_r2 -->

**Addendum (2026-06-29) — scope of the sibling relay:** A related mechanism in the same lay-down family, `entrypoint.sh::relay_managed_pi_extensions`, re-lays the image-baked managed Pi extension bytes before the bisync `--resync` baseline. Unlike the Governed-Mode-only seed bake above, the relay runs in **all** deployment modes: it guarantees the content half of the path-sensitive jiti prewarm cache key (see [AD79 Update](#ad79-image-baked-pi-extension-transpile-cache)) so the cache hits at runtime. In Governed Mode the subsequent `--checksum` sync then skips the unchanged relaid files; outside Governed Mode the relay precedes the size-plus-modification-time sync. This is why REQ-STOR-017 entrypoint code references the relay as all-modes even though this ADR's seed-bake decision is Governed-only.
...
The explicit user-only sentinel and post-boundary bypass wording are the sole exception: after fresh repository, branch, open-PR, base, and head validation they write that same reminder SHA directly, consume the sentinel when present, and launch no reviewers for it. They do not fabricate reviewer evidence or a FIX phase ([REQ-AGENT-098](../../sdd/spec/agents.md#req-agent-098-pi-review-triage-acknowledgement-barrier)). [AD144](#ad144-user-scoped-review-completion-uses-marker-or-dialog-ingress) replaces this sentinel and bypass state.
...
**Related REQ:** [REQ-MEM-001](../../sdd/spec/memory.md#req-mem-001-conversation-context-automatically-captured-to-vault), [REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-20-user-messages-and-on-resume), [REQ-MEM-014](../../sdd/spec/memory.md#req-mem-014-pi-capture-contract-transcript-prefilter-and-model-fidelity-lever), [REQ-MEM-016](../../sdd/spec/memory.md#req-mem-016-pi-extraction-requests-have-a-bounded-execution-profile), [REQ-MEM-018](../../sdd/spec/memory.md#req-mem-018-pi-extraction-agent-definitions-have-a-bounded-profile), [REQ-MEM-017](../../sdd/spec/memory.md#req-mem-017-session-memory-graph-identity-is-deterministic), [REQ-VAULT-027](../../sdd/spec/vault.md#req-vault-027-pi-vault-extraction-delivery-is-visible-and-transactional), [AD58](#ad58-sonnet-for-memory-capture-with-prefilter-and-scratchpad), [AD60](#ad60-pi-memory-capture-reuses-the-ad58-contract-and-transcript-prefilter), [AD102](#ad102-pi-extraction-delivery-is-root-owned-visible-and-transactional).
...
**Status:** Partially superseded by [AD156](#ad156-pi-fix-directive-owns-automatic-delivery) (2026-09-20) for Pi delivery; the Claude decision remains accepted.
...
**Status:** Accepted (2026-08-18; extended 2026-08-22 and 2026-08-31). Implements [REQ-SETUP-013](../../sdd/spec/setup.md#req-setup-013-managed-environment-configuration), [REQ-SETUP-014](../../sdd/spec/setup.md#req-setup-014-managed-repository-credential-boundary), [REQ-AGENT-147](../../sdd/spec/agents.md#req-agent-147-signed-managed-agent-configuration-releases), [REQ-AGENT-148](../../sdd/spec/agents.md#req-agent-148-protected-managed-release-publication), [REQ-AGENT-149](../../sdd/spec/agents.md#req-agent-149-shared-compiler-cli-compatibility), [REQ-AGENT-150](../../sdd/spec/agents.md#req-agent-150-independent-managed-release-activation-validation), [REQ-AGENT-154](../../sdd/spec/agents.md#req-agent-154-build-compatible-managed-release-discovery), and [REQ-AGENT-151](../../sdd/spec/agents.md#req-agent-151-bounded-managed-release-streaming).

Related storage and Browser IDE contracts are [REQ-STOR-020](../../sdd/spec/storage.md#req-stor-020-managed-environment-reconciliation), [REQ-STOR-021](../../sdd/spec/storage.md#req-stor-021-managed-content-ownership), [REQ-STOR-022](../../sdd/spec/storage.md#req-stor-022-managed-reconciliation-admission), [REQ-STOR-023](../../sdd/spec/storage.md#req-stor-023-managed-release-status-projection), [REQ-STOR-024](../../sdd/spec/storage.md#req-stor-024-managed-release-application), [REQ-STOR-033](../../sdd/spec/storage.md#req-stor-033-managed-release-delta-planning-and-resume), [REQ-STOR-034](../../sdd/spec/storage.md#req-stor-034-observational-managed-reconciliation-progress-writes), [REQ-STOR-035](../../sdd/spec/storage.md#req-stor-035-managed-reconciliation-cleanup-and-finalization), [REQ-STOR-036](../../sdd/spec/storage.md#req-stor-036-managed-reconciliation-progress-reads), [REQ-IDE-042](../../sdd/spec/browser-ide.md#req-ide-042-additive-company-extension-reconciliation), [REQ-IDE-044](../../sdd/spec/browser-ide.md#req-ide-044-exact-company-vsix-verification), [REQ-IDE-045](../../sdd/spec/browser-ide.md#req-ide-045-company-extension-reconciliation-orchestration), and [REQ-IDE-046](../../sdd/spec/browser-ide.md#req-ide-046-session-local-company-vsix-installation).
...
The Worker is the trust boundary. Explicit fresh-required validation conditionally checks GitHub and searches at most ten 100-record release-history pages for the first release matching the deployed runtime hash. Background status refresh validates only the latest release and caches a degraded attempt for five minutes, preventing release-history work from exhausting the batch-status hot path while preserving retry. A hash mismatch continues only the explicit bounded scan; any other managed-release validation failure stops it. Verification covers repository identity, immutable metadata, asset digests, signature, sequence, schema, paths, bounds, seed ABI, runtime hash, and extension records. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease -->
...
The fixed seed-v1 resource boundary is shared by compiler and Worker: <!-- @impl: scripts/agent-seed-release-limits.mjs::MANAGED_RELEASE_LIMITS --> 8 MiB compressed, 32 MiB expanded, 5,000 documents, 2 MiB per document, 24 MiB aggregate document content, 512 UTF-8 bytes per path, 5,000 retired paths, 20 managed extensions, 128 MiB per VSIX, 256 MiB aggregate VSIX bytes, and one HTTPS redirect.

The dashboard compares the already-verified active release descriptor plus the user's resolved mode with applied state without re-expanding an unchanged payload; the existing five-minute resolver verifies and caches the latest compatible release. <!-- @impl: src/lib/managed-release-active.ts::getActiveManagedRelease --> <!-- @impl: src/routes/session/lifecycle.ts::default --> Without a compatible verified active descriptor it reports UPDATE PENDING even when an older protected release remains applied. With a descriptor and no owning session it reports UPGRADING and calls the dedicated automatic agent-config upgrade route. While any session owns the bucket it reports UPDATE PENDING, leaves that session unchanged, and blocks another start until the final owner stops.

A `sessionMode` preference change enforces this update-pending gate only when a managed release has ever been applied or is currently active; an unconfigured deployment's plain sessionMode change is unaffected. When a managed environment is involved, a failed auto-reconcile propagates instead of being swallowed. Non-conflict failures restore user-facing pre-request preferences while retaining the target journal for repair. A conflict observed before publication instead preserves the newer preference record and blocks the stale applied stamp. <!-- @impl: src/routes/preferences.ts::default -->

Automatic reconciliation compares the active target with the exact applied gzip when that cached history is available, retaining only per-path content fingerprints. Missing valid applied history uses marker-resumable full-target planning. Otherwise, it writes added or release-changed target paths through at most six concurrent R2 operations. A release-identical path is untouched even when a user edit removed its marker. <!-- @impl: src/lib/r2-seed.ts::buildManagedAutomaticPlan --> <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest -->

Before writes begin, preferences retain a bounded set of target identities. If the active target changes, later reconciliation repairs only objects carrying an interrupted target marker and removes only interrupted-only marked paths. Another interruption keeps the set intact. Fresh buckets and valid cache-missing histories plan the full target and use the same marker-backed resume. Direct cleanup considers only paths proven absent from the target and preserves markerless mutable content; observational KV progress never controls execution. <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest --> <!-- @impl: src/routes/preferences.ts::default --> <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs -->

Manual Recreate and non-dashboard callers retain their full-overwrite behavior. Applied state remains last and clears the target set only after context reconciliation and final target, mode, policy, SSE, session-ownership, migration, and pending-target checks. Disabling the feature still converges through the existing path back to the baked image hash. <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest -->
...
**Consequences:** Managed content can change independently of a Codeflare image while mode entitlement and all non-curated behavior remain unchanged. A user edit on a release-identical path survives automatic upgrades; when a later release changes that path, target bytes become authoritative again. Browser closure can pause a large reconciliation, but the next dashboard trigger resumes from target markers instead of replaying completed PUTs. Availability fails open only to the last verified state for the same runtime dependency set or to already-applied user content, while invalid and incompatible candidates never activate. Operators must protect release immutability and the signing key, publish updates through higher sequences, and publish rollback as a new immutable release rather than mutating history.
...
**Decision:** Fresh container startup writes context-mode's disabled package marker while retaining its package, image patches, JITI cache, data, and managed foreground-owner path for explicit `/ctx on`. A dedicated `/ctx` command extension reaches Standard and Advanced without broadening advanced Codeflare commands. <!-- @impl: entrypoint.sh::warm_pi_npm_dependencies --> <!-- @impl: preseed/agents/pi/extensions/ctx-command.ts::handleContextModeCommand -->

At each user prompt, after context-mode's lazy bridge registration and before provider serialization, the schema-free finalizer selects only `read`, `bash`, `edit`, `write`, and `capability`. Registered optional tools remain searchable and activate additively for the next model step. <!-- @impl: preseed/agents/pi/extensions/zz-tool-exposure-finalizer.ts::finalizeToolExposure -->

Active Goal and Plan workflows are narrow exceptions. Any valid Goal history retains the two permitted terminal controls, `goal_complete` and `goal_blocked`, when no Plan is active; `goal_wait` remains disabled. An active Plan outranks non-active Goal history and retains its frozen read-only policy plus two helpers; configured `always` Goal visibility also retains the permitted Goal terminal tools. Plan Mode 0.55.3 is transformed at image build so `/plan` resolves helpers and configured policy from registered tools before filtering. Fresh normal turns expose exactly five tools. <!-- @impl: preseed/agents/pi/extensions/zz-tool-exposure-finalizer.ts::finalizeToolExposure --> <!-- @impl: preseed/agents/pi/extensions/capability-helpers.ts::registerInitialToolFilter --> <!-- @impl: scripts/patch-pi-plan-mode-tool-policy.mjs::patchPiPlanModeSource -->

Subagent activation includes its result and steering controls. <!-- @impl: preseed/agents/pi/extensions/capability-helpers.ts::activationGroup --> A schema-free local hook blocks resume attempts for queued or running records through the package's public service. Registered and active schema sizes remain separate diagnostics rather than a fixed promotion threshold. <!-- @impl: preseed/agents/pi/extensions/subagent-resume-guard.ts::subagentResumeGuard --> <!-- @impl: scripts/measure-pi-runtime-context.mjs::main -->

**Alternatives rejected:** Remove tools or skills; patch Pi; keep Plan helpers active in normal provider requests; patch package-owned descriptions; keep context-mode active but hide only some of its tools; impose a fixed provider-token gate; or reintroduce shared-preseed synchronization. These lose functionality, add schema cost, leave avoidable startup work, create brittle limits, or violate repository ownership.
...
**Consequences:** Strict interception retains AD13's user-bucket authority without exposing the usable credential to root inside the container. Durable Object reconstruction depends on the existing start payload to restore credentials in memory; a missing pair fails closed instead of silently acquiring broader authority. A warm replacement is committed only after the new catch-all is installed, so registration failure preserves the prior pair.

**Related REQs:** [REQ-SEC-003](../../sdd/spec/security.md#req-sec-003-per-user-r2-tokens-scoped-to-user-bucket), [REQ-ENTERPRISE-023](../../sdd/spec/enterprise-mode.md#req-enterprise-023-strict-gateway-egress-controller-transport), [REQ-ENTERPRISE-026](../../sdd/spec/enterprise-mode.md#req-enterprise-026-strict-r2-interception-preserves-user-bucket-authority).
...
**Decision:** Store one immutable marker for each normalized GitHub host, repository, PR, branch, protected base, and exact head under `~/.codeflare/review-state/v1`. Markers follow the user's R2-synced home across sessions, worktrees, clones, replacement containers, and devices that share the bucket. They expire after 30 days, and each repository and branch retains its ten newest markers. Writes publish atomically, rely on regular cadence or final sync for R2 convergence, and never read or migrate legacy `.git/sdd-review-*` state.

Startup, resume, clone, switch, branch checkout, PR checkout, pull, a successful PR merge that changes the active checkout or full `HEAD`, checked-out-branch push, checked-out-branch PR creation, and checked-out-branch PR reopen resolve the current open protected-base PR. A valid exact marker stays silent. Successful checked-out-branch push, PR creation, and PR reopen retain the deterministic automatic review-and-CI plan from the pre-recovery state machine. Other marker misses offer `Mark review complete` and `Launch review`; cancellation writes nothing. Fetch, inspection, local mutation, detached or path checkout, merges without that successful transition, and unrelated-ref push remain inert.

<!-- doc-allow-element: AD144 accepted decision paragraph is preserved verbatim -->
A selected or automatic launch uses the existing range and lane planner. The newest retained same-PR ancestor sets an incremental range when it is still an ancestor; otherwise the round reviews the full protected-base diff. Required reviewers start together, and automatic delivery adds exact-head CI immediately afterward. Before Claude emits a round, it privately and atomically records the exact head, PR, canonical runner, boundary, and scope in a session-scoped plan under `/run`; transcript launch evidence earns credit only when it matches that plan. Active coordination exists only in memory for Pi and after a session offset under `/run` for Claude. Stopped or interrupted work creates no checkpoint, retry plan, counter, or missing-work follow-up. The next delivery launches a fresh round; the next non-delivery exposure asks again.

After the root publishes canonical triage, FIX handling revalidates the exact identity, writes the marker, emits the existing FIX reminder, and releases review-owned Goal pause. A Pi table with failed or timed-out CI evidence but a malformed required CI row receives one correction follow-up without acknowledgement; the corrected table continues the same round. No triage, local write failure, or head drift means no marker and no FIX. Pi and Claude retain deterministic reviewer output, exact-head CI correlation, failure and timeout rows, and root-owned mutation.

**Enterprise exception (REQ-OPERATOR-053, in progress):** A confirmed applicable protected Boundary Reviews Action currently selects a remote-only plan for an authenticated Enterprise Pi PR boundary; Pi starts no local reviewer wave or local completion marker. Confirmed absence retains AD144's local plan, while uncertain selection stays non-green. Action claim, independent publication and remote completion are not yet proven; selection alone cannot clear Review. Claude, non-Enterprise, and user-invoked `/review` behavior remain unchanged.

**Consequences:** Completion survives clone and container boundaries, while partial work deliberately does not. Missing, stale, or mismatched Claude plan state fails closed without launch credit. Another device can repeat the prompt before regular sync convergence, but it cannot receive a false acknowledgement. Startup prunes only the bounded marker root and never scans workspace repositories. There is no review-state service, endpoint, database, migration, or second planner.
...
**Related REQs:** [REQ-TERM-001](../../sdd/spec/terminal.md#req-term-001-terminal-surface-count-follows-session-mode), [REQ-TERM-005](../../sdd/spec/terminal.md#req-term-005-herdr-runtime-and-configured-agent-startup), [REQ-TERM-006](../../sdd/spec/terminal.md#req-term-006-herdr-owns-in-session-terminal-topology), [REQ-TERM-011](../../sdd/spec/terminal.md#req-term-011-visible-terminal-panes-own-websocket-connections), and [REQ-TERM-012](../../sdd/spec/terminal.md#req-term-012-multiview-virtual-session-workspace).

---

### AD146: Terminal mode is an immutable per-session choice

**Category:** Architecture, Build / Container, Terminal

**Status:** Accepted (2026-08-28). Supersedes [AD145](#ad145-herdr-owns-topology-inside-one-codeflare-terminal-surface).

**Context:** Classic terminal behavior is proven across browsers and retains useful outer tabs and tiling, while Herdr provides richer terminal-native orchestration. Making either topology universal removes a useful experience. Running both topology layers inside one session would still duplicate controls and create focus and resize contention.

**Decision:** The Worker stamps terminal mode when it creates a session, and that mode remains immutable. The server-backed preference affects only future sessions and defaults to classic; missing or invalid historical stamps also resolve classic. Classic sessions retain Codeflare tabs, tiling, labels, saved layouts, right-click behavior, and terminal IDs `1` through `6`. Herdr sessions expose one Codeflare surface at terminal ID `1`; the fixed launcher attaches the official pinned client to `cf-<SESSION_ID>`, and Herdr owns every inner tab, pane, split, and workspace. <!-- @impl: src/routes/session/crud.ts::default --> <!-- @impl: src/types.ts::resolveTerminalMode -->

The persisted stamp governs start, resume, prewarm, routing, frontend ownership, cleanup, and compatibility behavior. Existing xterm.js rendering, authenticated WebSocket transport, host `Session`, `node-pty`, resize authority, input classification, idle accounting, and container lifecycle remain shared. MultiView may combine both modes. Browser IDE remains independent and starts no Herdr runtime. <!-- @impl: src/container/container-env.ts::buildEnvVars --> <!-- @impl: src/container/index.ts::container -->

Herdr live processes, pane history, sockets, logs, plugins, updater state, and user-edited settings stay ephemeral. Its official structural `session.json` follows normal R2 sync so replacement can restore topology and supported native agent references without claiming arbitrary process survival. The browser never speaks Herdr's private protocol, and no Herdr route becomes public. <!-- @impl: image/herdr/codeflare-herdr-terminal::prepare_runtime --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON -->

**Consequences:** Both complete terminal experiences remain supported. Mode-aware boundaries preserve classic right-click and mobile behavior while Herdr receives terminal mouse input, bounded OSC 52 clipboard writes, and fixed local attention events. Only Herdr shutdown stops a named runtime. Previously deleted browser-local classic layouts cannot be recovered, but future classic layouts persist normally.

**Related REQs:** [REQ-TERM-001](../../sdd/spec/terminal.md#req-term-001-terminal-surface-count-follows-session-mode), [REQ-TERM-005](../../sdd/spec/terminal.md#req-term-005-herdr-runtime-and-configured-agent-startup), [REQ-TERM-006](../../sdd/spec/terminal.md#req-term-006-herdr-owns-in-session-terminal-topology), [REQ-TERM-007](../../sdd/spec/terminal.md#req-term-007-classic-topology-remains-available), [REQ-TERM-011](../../sdd/spec/terminal.md#req-term-011-visible-terminal-panes-own-websocket-connections), [REQ-TERM-012](../../sdd/spec/terminal.md#req-term-012-multiview-virtual-session-workspace), and [REQ-TERM-033](../../sdd/spec/terminal.md#req-term-033-durable-herdr-structural-session-recovery).

---

### AD147: Active managed-resource policy supersedes provenance ownership
...
**Decision:** Derive one canonical `.codeflare/managed-paths.json` from the verified active release, signed retirements, fixed Codeflare paths, and the normalized `mutable | immutable | exclusive` mode. Setup stores desired policy globally, while existing no-session reconciliation applies it per idle user bucket and stamps its digest last; running sessions retain their applied policy, and new starts or Storage mutations wait on mismatch ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)). Protected sessions and Storage mutations verify one existing curation release identity plus the path digest.

`EgressController` classifies every protected-mode own-R2 mutation immediately before signing with the user's scoped credential. Protected exact paths and exclusive roots return S3 denial, and logs retain only operation, identity/hash prefixes, request ID, and reason ([REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement)). Container-restored policy generates rclone exclusions only and grants no authority.
...
**Related REQs:** [REQ-SETUP-013](../../sdd/spec/setup.md#req-setup-013-managed-environment-configuration), [REQ-SETUP-015](../../sdd/spec/setup.md#req-setup-015-managed-resource-persistence-controls), [REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety), [REQ-STOR-028](../../sdd/spec/storage.md#req-stor-028-canonical-managed-resource-persistence-policy), [REQ-STOR-029](../../sdd/spec/storage.md#req-stor-029-managed-resource-reconciliation), [REQ-STOR-030](../../sdd/spec/storage.md#req-stor-030-managed-resource-policy-loading), [REQ-STOR-031](../../sdd/spec/storage.md#req-stor-031-managed-resource-container-sync), [REQ-STOR-032](../../sdd/spec/storage.md#req-stor-032-exclusive-managed-resource-boundaries), [REQ-ENTERPRISE-027](../../sdd/spec/enterprise-mode.md#req-enterprise-027-managed-resource-admission-and-transport), [REQ-ENTERPRISE-028](../../sdd/spec/enterprise-mode.md#req-enterprise-028-managed-resource-request-classification), [REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement), [REQ-ENTERPRISE-030](../../sdd/spec/enterprise-mode.md#req-enterprise-030-managed-resource-storage-enforcement).
...
**Category:** Architecture, Session lifecycle, Storage

**Status:** Partially superseded (2026-09-24); the SDK-only terminal stop decision below is historical. Current terminal stop authority is owner-scoped D1 ([REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC1–AC2); SDK state still gates retryable readiness. Other decisions remain accepted.

**Context:** Session records and compact LIST metadata are stored in eventually-consistent KV. Whole-record writes can expose stale values independently from LIST metadata or temporarily omit a record. The dashboard treated `stopped` and missing observations as runtime truth, disposed terminal ownership, and redirected users while containers and WebSockets remained live. Terminal admission compounded the fault by converting stale KV `stopped` into close code 4503. Container start also issued two immediate whole-record writes, allowing the second stale read to restore pre-start status. <!-- @impl: src/lib/kv-keys.ts::putSessionWithMetadata --> <!-- @impl: web-ui/src/stores/session.ts::loadSessions --> <!-- @impl: web-ui/src/stores/session-polling.ts::refreshSessionStatuses --> <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/container-lifecycle.ts::onStart -->

**Decision:** Keep KV as the no-wake dashboard projection, not live-container authority. Full-list hydration and incremental polling retain a session while it is initializing, inside its three-minute startup guard, or owned by a terminal connection, attempt, or retry loop. Manual stop and terminal 4503 bypass this gate. <!-- @impl: web-ui/src/stores/session.ts::shouldRetainNegativeKv --> <!-- @impl: web-ui/src/stores/terminal.ts::connect -->

Terminal admission reads persisted Container SDK state without waking the container. It emits 4503 only for stopped states and retryable 1013 when state or readiness is unavailable. The inner Durable Object forwarding gate also emits 1013 when its volatile running flag is false; it cannot overrule healthy persisted state with a false terminal stop. Managed reconciliation checks persisted state for every listed session, so stale KV status cannot strand an update or permit hot mutation. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch --> <!-- @impl: src/lib/session-helpers.ts::hasOwningSessionContainer -->

A fresh-generation `onStart()` publishes `running`, `lastStartedAt`, and `lastActiveAt` from one snapshot; a same-generation replay validates durable ownership without projecting. Only marker-aware metrics repairs stale `stopped` or missing status by re-asserting `running`; the start route does not rewrite KV for an already-running container. <!-- @impl: src/container/container-lifecycle.ts::onStart --> <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer -->
...
**Status:** Accepted (2026-09-09), with the current discovery amendment below.

**Context:** AD74 remains the Dynamic Route transport decision, but its later Bedrock-only native amendment no longer describes the configured native and custom providers supported by the interceptor. <!-- @impl: src/llm-interceptor.ts::LlmInterceptor -->
...
**Current discovery amendment (2026-09-13):** The REST-first clause above applies to historical Dynamic assignments. New discovered compatibility contracts explicitly bind compat under [AD74's current amendment](#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api) and [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection); this does not migrate saved native transports. The separate native Runtime contract remains documented in [Generic Bedrock support](../lanes/bedrock-generic-model-support.md). <!-- @impl: src/lib/ai-capability-discovery/index.ts::capabilityCandidates --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor -->

**Independent-capability clarification (2026-09-13):** The linked Native discovery contract replaces its prior default-only/cache-prerequisite description with six audited per-mapping forms and an exclusive Provider-default fallback. Tools/replay and exact server receipts govern activation; cache permission remains separate. This narrow amendment leaves saved transports, historical evidence/profile authority, credential ownership, dispatch isolation and replay safeguards unchanged. See [REQ-ENTERPRISE-072](../../sdd/spec/enterprise-mode.md#req-enterprise-072-provider-native-bedrock-reasoning-profiles) and [REQ-ENTERPRISE-075](../../sdd/spec/enterprise-mode.md#req-enterprise-075-provider-native-bedrock-administration-authority); no new live or deployed acceptance is claimed.
...
**Context:** Twenty-prompt capture creates a useful append-only audit trail, but keeping every historical batch as a separate hot file makes direct recall and Vault synchronization increasingly noisy. Modification time cannot define age because R2 restore rewrites it. Existing graph nodes and edges also cite the individual source paths, so a filesystem-only move would leave valid evidence pointing at deleted files.

**Decision:** Run one image-owned compactor on a daily UTC boundary without a feature flag or session-mode gate. It approximates a latest-month hot set from each capture filename's leading calendar date, keeps unrecognized names hot, and renders cold captures in deterministic capture-date/name order into `Raw/Sessions/Archive.md`. The archive is machine-owned; capture and interactive agents never edit it or delete individual captures. <!-- @impl: scripts/compact-session-captures.mjs::buildSessionArchive --> <!-- @impl: entrypoint.sh::run_daily_vault_session_compaction --> <!-- @impl: entrypoint.sh::bisync_with_r2 -->

The compactor writes the local archive, relocates graph provenance, republishes the complete graph as `user_vault`, and deletes only the exact unchanged sources recorded in its manifest. One final bisync publishes the archive, graph update, and deletions together. A failed step leaves the UTC day unstamped for later synchronization and compaction attempts. <!-- @impl: entrypoint.sh::run_daily_vault_session_compaction --> <!-- @impl: preseed/agents/claude/plugins/codeflare-vault/scripts/merge-vault-graph.py::relocate_node_link_provenance --> <!-- @impl: preseed/agents/pi/scripts/merge-vault-graph.py::relocate_node_link_provenance -->
...
**Alternatives rejected:** Use mtime or file frontmatter for age; keep one archive per month; re-extract the archive; let agents summarize or prune old captures; or add a second sync daemon. These choices reintroduce restore-time age drift, unbounded file counts, duplicate semantics, unstable IDs, or competing persistence ownership.
...
**Status:** Partially superseded (2026-09-21) by [AD157](#ad157-reset-stale-stopping-ownership-on-owner-status-reads) only for the confirmed-exit requirement; D1 ownership, clean-slate cutover, unreachable incidents, and device-local terminal state remain active.
...
### AD156: Pi FIX directive owns automatic delivery

**Status:** Accepted (2026-09-20). Partially supersedes [AD123](#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules) for Pi delivery only.
...
### AD157: Reset stale stopping ownership on owner status reads

**Status:** Accepted (2026-09-21). Partially supersedes [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) only for the requirement that every `stopped` transition have confirmed-exit evidence.

**Context:** Stop completion is request-coupled: D1 first records `stopping`, and the same request records `stopped` after destruction returns. An interrupted completion can therefore leave a termination intent owning the session after the intended workload has stopped, which blocks managed reconciliation and later Start admission indefinitely. <!-- @impl: src/routes/session/lifecycle.ts::app -->

**Decision:** Before returning owner batch status, reset that owner's `stopping` rows whose transition is strictly older than three minutes to `stopped`. Clear termination and unreachable ownership, reset editor readiness, increment the response revision, and record `stop_timeout_forced_reset`. This is an explicit bookkeeping exception and does not claim process-exit evidence. <!-- @impl: src/routes/session/lifecycle.ts::app --> <!-- @impl: src/lib/session-repository.ts::D1SessionRepository.forceStopExpired -->

**Consequences:** Reopening the dashboard releases stale lifecycle ownership without a container probe, retry worker, cron, lease, or schema change. Recent stops, other states, and other owners remain unchanged. If an old workload survives past the bound, D1 may permit managed reconciliation or a later Start while it still exists; the owner-selected bound accepts that residual risk ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-records-reset-on-owner-status-read)).

**Related REQs:** [REQ-SESSION-018](../../sdd/spec/session-lifecycle.md#req-session-018-d1-lifecycle-evidence-is-generation-fenced), [REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-records-reset-on-owner-status-read).
