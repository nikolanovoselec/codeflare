...
`onStart()` publishes `running`, `lastStartedAt`, and `lastActiveAt` from one snapshot. Only marker-aware metrics repairs stale `stopped` or missing status by re-asserting `running`; the start route does not rewrite KV for an already-running container. <!-- @impl: web-ui/src/stores/session.ts::shouldRetainNegativeKv --> <!-- @impl: web-ui/src/stores/terminal.ts::connect --> <!-- @impl: src/lib/session-helpers.ts::hasOwningSessionContainer --> <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer -->
