...
- Runtime wiring: `entrypoint.sh` registers the `context-mode` MCP server in `~/.claude.json` (`bunx context-mode@<version>`) and appends four `bunx context-mode@<version> hook claude-code <event>` commands to `~/.claude/settings.json` when the plugin manifest is present and `SESSION_MODE=advanced`. This mirrors the wiring path used by `codeflare-memory` and `codeflare-hooks` rather than relying on the upstream Claude Code plugin loader to auto-register from `plugin.json`. Bun is the chosen runtime because context-mode's bundled executor uses dynamic `require('node:fs')` calls that Node's ESM loader rejects with `Dynamic require of "node:fs" is not supported` — `ctx_doctor` and `ctx_stats` work under Node, but `ctx_execute` / `ctx_batch_execute` need Bun's runtime which handles dynamic `node:` builtins natively. Bun is installed globally in the container image (Dockerfile).
