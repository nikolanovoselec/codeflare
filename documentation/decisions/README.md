...
| [AD136](#ad136-managed-coding-environments-reconcile-signed-releases-before-session-start) | Reconcile signed managed-environment releases before session start | The dashboard verifies and applies immutable releases through existing R2 upgrade machinery, avoiding a second container-start materializer and running-session conflicts. | Architecture, Security, Storage, Supply Chain | Active |
| [AD135](#ad135-inline-chat-requires-one-host-correlated-result) | Require one host-correlated Inline result | Inline turns isolate current context and force one edit-or-no-change result on the OpenAI route, preventing optional tools and model-copied IDs from breaking settlement. | Architecture, Security | Active |
---
...
Raw tool-start events precede Pi argument validation, so the backend may retain up to three attempts and accept the first valid host-correlated result. Invalid-only settlement reports a bounded outcome, summary, count, or geometry category; a second valid result still fails closed. <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::parseInlineEditResult --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-030: inline Pi accepts a valid retry after one invalid raw result) -->
...
### AD136: Managed coding environments reconcile signed releases before session start

**Category:** Architecture, Security, Storage, Supply Chain

**Status:** Accepted (2026-08-18). Implements [REQ-SETUP-013](../../sdd/spec/setup.md#req-setup-013-managed-coding-environment-configuration), [REQ-AGENT-147](../../sdd/spec/agents.md#req-agent-147-signed-managed-agent-configuration-releases), [REQ-STOR-020](../../sdd/spec/storage.md#req-stor-020-managed-coding-environment-reconciliation), and [REQ-IDE-042](../../sdd/spec/browser-ide.md#req-ide-042-additive-company-extension-reconciliation).

**Context:** Codeflare already compiles one manifest-driven agent seed, writes it to each user's R2 bucket with provenance metadata, detects image-hash drift during dashboard load, blocks session creation while reconciliation runs, and restores R2 before agent startup. A separate container-start downloader would duplicate transformation, trust, state, and failure machinery at the most fragile lifecycle seam. It would also risk changing a shared bucket while another session is syncing it. Operators nevertheless need one deployment-level source for company skills, rules, hooks, agents, scripts, plugins, and exact Browser IDE extension requirements without rebuilding Codeflare for each content release.

**Decision:** Expose a deployment-level Setup section named **Managed coding environment** in every deployment mode. It references one private GitHub release repository, an encrypted repository-scoped read PAT, and an Ed25519 public key. A shared side-effect-free compiler produces both default and advanced documents from the same transformations as the image seed. The private repository is the runtime master for curated content; Codeflare's baked preseed is an independent fallback and is not synchronized from private releases. Codeflare owns the shared compiler and runtime ABI, so compiler changes land here first and then advance the private repository's literal 40-hex pin. The private repository's one-trigger workflow builds and independently rebuilds bytes in a read-only job, signs only that artifact in a reviewer-protected environment, rechecks exact draft release and asset IDs and digests, and publishes immutable, monotonically sequenced `seed-v1.json.gz` and `seed-v1.sig` assets by release ID.

The Worker is the trust boundary. It conditionally checks GitHub, verifies repository identity, immutable release metadata, asset digests, signature, sequence, schema, paths, bounds, seed ABI, runtime dependency hash, and extension records, then stores content-addressed bytes in one deterministically named deployment R2 bucket. Immutable assets live under `releases/<digest>/`; each repository-and-public-key configuration has a fingerprinted `configs/<fingerprint>/active.json` pointer advanced by conditional compare-and-swap. Setup primes a new namespace before committing the KV configuration that selects it, so a failed PAT, repository, or signing-key replacement cannot move the prior pointer. GitHub and cache failure retain the last verified release; an enabled configuration with neither GitHub availability nor a verified last-known-good release blocks new/reset reconciliation rather than substituting baked content, while already-applied buckets remain unchanged. The PAT, signature key material, and release transport never enter a user bucket or container.

The fixed seed-v1 resource boundary is shared by compiler and Worker: 8 MiB compressed, 32 MiB expanded, 5,000 documents, 1 MiB per document, 24 MiB aggregate document content, 512 UTF-8 bytes per path, 5,000 retired paths, 20 managed extensions, 128 MiB per VSIX, 256 MiB aggregate VSIX bytes, and one HTTPS redirect.

The existing dashboard upgrade path compares the active release digest plus the user's resolved mode with applied state. With no running session it reports UPGRADING and calls the existing agent-config reconcile route. With any running session it reports UPDATE PENDING, leaves those sessions unchanged, and blocks another start until the final session stops. Reconciliation writes verified documents and one company extension manifest, deletes an obsolete key only when its prior release marker still proves company ownership, completes required image-owned context-mode reconciliation, and stamps applied state last. Disabling the feature converges through the same path back to the baked image hash.

Container startup remains R2-first. A narrow active flag plus the exact applied release digest skips Governed image pre-laydown and the image Pi extension relay only while a verified managed release is active and binds restored company-extension metadata to that release; initial restore, transcript retention before initial PTY release and outbound bisync, hook executable repair, agent startup, and sync ownership remain unchanged. Browser IDE settings add explicit company IDs without removing the wildcard personal-extension policy. After workbench readiness, exact verified VSIX bytes are downloaded to a bounded temporary file, checked by size and SHA-256, installed before the unchanged personal restore, and deleted; VSIX bytes never enter R2.
