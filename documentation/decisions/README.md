...
The Worker is the trust boundary. It conditionally checks GitHub and searches at most ten 100-record immutable release-history pages newest to oldest for the first release matching the exact deployed runtime hash. It verifies repository identity, immutable release metadata, asset digests, signature, sequence, schema, paths, bounds, seed ABI, runtime dependency hash, and extension records as a bounded stream, then stores content-addressed bytes in one deterministically named deployment R2 bucket. Immutable assets live under `releases/<digest>/`; each trust configuration advances monotonically within a runtime dependency set and may replace a globally newer pointer that belongs to an incompatible set, while a repository-stable conditional pointer records the authoritative selection. A losing concurrent signing-key change performs bounded KV repair and fails explicitly if the pointer does not settle. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @impl: src/lib/remote-curation.ts::configureManagedEnvironment -->
