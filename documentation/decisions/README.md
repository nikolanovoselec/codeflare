...
| [AD122](#ad122-superseded-ci-runs-are-cancelled-by-workflow-concurrency-not-by-the-monitor) | Superseded CI runs are cancelled by workflow concurrency, not by the monitor | Architecture, Build / Container |
...
### AD122: Superseded CI runs are cancelled by workflow concurrency, not by the monitor
...
**Context:** The CI-monitoring skill grew a launcher step that listed in-flight runs on the branch and cancelled the ones whose head was not the current HEAD. It was added to stop a superseded push burning a full matrix, and it was wrong three times in a row: an ancestry guard that excluded exactly the amended and force-pushed heads worth cancelling, then no guard at all, which exposed fork pull requests sharing a branch name, then a lineage guard that was merely less wrong. An older section of the same skill told the agent to cancel every non-completed run on the branch with no head filter at all, which cancels the run it is about to monitor.

Each round fixed the previous round's fix. That is the signal that the mechanism, not the guard, was the mistake.

**Decision:** The monitor does not cancel runs. Every workflow whose superseded runs are worth killing declares `concurrency` with `cancel-in-progress: true` keyed on `github.ref`. For a `pull_request` event that is the per-PR merge ref, so two pull requests sharing a head branch name never cancel each other while superseded pushes of the same pull request always do. No client reconstructing intent from a branch name can match that, because the branch name is precisely the ambiguous part.

The workflows that omit `cancel-in-progress` omit it on purpose. `deploy.yml` mutates the worker, secrets, KV, and registry in sequence and sets `cancel-in-progress: false` so a cancelled deploy cannot leave a half-configured target; `sign-release.yml` and `bump-shadow-pins.yml` serialise for the same reason. A cancel loop keyed on branch name cannot see that policy and will eventually cancel the one run that must never be cancelled.

**Consequences:** Both cancel paths are deleted from the skill, which keeps only its read-only supersession detection: the monitor still exits on `CI_RESULT superseded` or `CI_RESULT unpushed` rather than reporting a verdict for a head nobody is waiting on. `container-image.yml` and `nightly-pr-checks.yml` need no concurrency key, being `workflow_call` and `schedule` only, and `promotion-source.yml` is a seconds-long pull-request check with nothing to burn. A future agent that notices a superseded matrix still running must add or fix a `concurrency` block in the workflow, never a `gh run cancel` loop in a client script.
