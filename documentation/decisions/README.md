...
**2026-08-25 performance amendment:** Exact-head evidence showed CodeQL's single analysis phase still consumed 71 seconds after exclusions. CodeQL now analyzes two disjoint source groups concurrently—Worker/automation and container/UI—with unique upload categories; a dependent `Analyze` gate preserves the required-check identity. <!-- @impl: .github/workflows/codeql.yml::analyze --> <!-- @impl: .github/workflows/codeql.yml::gate -->
