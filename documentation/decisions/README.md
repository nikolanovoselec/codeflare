...
- **Per-bucket state object** `r2-regime:<bucket>` (`{status: ready|migrating|mixed-recovery, regime, from?, to?, generation, cursor?, phase?, leaseExpiresAt?, keyMd5?, stuckCount?, lastFailedKey?}`) is the single source of truth, replacing the boolean marker AND the standalone lock (the `leaseExpiresAt` is the per-chunk in-flight lock; a legacy `r2SseRegime='plain'` is still read one-way for pre-existing migrated buckets).
