...
| ~~[AD123](#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules)~~ | ~~Let Claude's FIX directive own conditional delivery~~ | [AD156](#ad156-pi-and-claude-fix-directives-own-automatic-delivery) makes both runtimes' FIX directives own automatic delivery after accepted corrections. | Architecture, Cost | Superseded |
...
| [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) | Make D1 the complete session lifecycle authority | D1 owns complete non-secret session rows and ordered lifecycle truth while the per-session Durable Object retains process control and terminal presence remains device-local. | Architecture, Storage, Reliability | Active |
| [AD156](#ad156-pi-and-claude-fix-directives-own-automatic-delivery) | Make both FIX directives own automatic delivery | After accepted corrections, each runtime's FIX directive orders the push while preserving no-merge and no-op protections. | Agents, Architecture | Active |
...
**Status:** Superseded by [AD156](#ad156-pi-and-claude-fix-directives-own-automatic-delivery) (2026-09-20).
...
### AD156: Pi and Claude FIX directives own automatic delivery

**Status:** Accepted (2026-09-20). Supersedes [AD123](#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules).

**Context:** Both runtimes acknowledge a reviewed exact head before a separate FIX turn applies accepted findings. Leaving Pi delivery implicit caused completed corrections to stop locally and require another user prompt even though the reviewed workflow already authorized delivery. Claude already carried explicit delivery ownership in its FIX directive.

**Decision:** Both runtimes' FIX directives order an automatic push after accepted corrections are complete. Pi states this explicitly in its review FIX follow-up. The instruction does not authorize merge, deployment, an empty commit, or a push when no accepted file change exists. The resulting push remains a new review boundary for the corrected head.

**Consequences:** Accepted corrections proceed through commit and push without renewed consent, and the next exact head receives a fresh review and CI round. A clean triage performs no Git write. Delivery ownership is explicit in the directive rather than inferred from lower-precedence standing rules.
