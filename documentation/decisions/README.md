...
- Container build/scan/push moves to reusable `container-image.yml`. PR complete-image verification and deployment share one GHCR-backed BuildKit cache while preserving separate image outputs. Fork PRs and Dependabot do not authenticate to it.

  GitHub's `type=gha` cache was rejected because its ref scoping kept deployment from reading layers written under a pull-request merge ref. Trusted same-repository heads receive package write permission. Login failure disables cache use; export errors cannot fail the image build.
