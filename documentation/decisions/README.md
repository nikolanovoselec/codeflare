...
**Decision:** Derive one canonical `.codeflare/managed-paths.json` from the verified active release, signed retirements, fixed Codeflare paths, and the normalized `mutable | immutable | exclusive` mode. Setup stores desired policy globally, while existing no-session reconciliation applies it per idle user bucket and stamps its digest last; running sessions retain their applied policy, and new starts or Storage mutations wait on mismatch. Protected sessions and Storage mutations verify one existing curation release identity plus the path digest. `EgressController` classifies every own-R2 mutation immediately before signing with the user's scoped credential; protected exact paths and exclusive roots return S3 denial, and logs retain only operation, identity/hash prefixes, request ID, and reason. Container-restored policy generates rclone exclusions only and grants no authority.
