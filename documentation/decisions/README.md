...
- The `[[vpc_networks]]` `EGRESS` binding is committed **commented-out** in `wrangler.toml` and injected at deploy time by `deploy.yml` **only when `ENTERPRISE_MODE=active`**, so default/fork deploys (accounts without Workers VPC / `cf1:network`) and the `vitest-pool-workers` test runtime are unaffected. **No WARP Connector is required** — public egress through the Gateway rides the reserved `cf1:network` path (a connector pertains only to the reserved raw-TCP plane, REQ-ENTERPRISE-017). On non-enterprise deploys `env.EGRESS` is undefined, so the feature fails closed and the dormant state (toggle OFF or binding absent) is inert — which is what makes shipping the code OFF safe.
