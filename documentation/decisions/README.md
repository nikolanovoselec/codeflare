...
**Status:** Accepted (2026-06-27). Refines [AD85](#ad85-controller-mediated-cloudflare-gateway-egress-as-a-mandatory-web-boundary-wizard-toggled-default-off): the "force *all* egress" boundary is narrowed to *direct-internet* egress; platform-native Cloudflare primitives egress direct.
...
**Decision:** Only genuine **direct-internet** egress takes the `cf1:network` path. Platform-native Cloudflare primitives egress **direct** (global `fetch`) and never traverse `cf1:network`, even when strict is ON:

- **R2** (`*.r2.cloudflarestorage.com`), **CF API** (`api.cloudflare.com` — Browser Rendering + AI Gateway REST), and **AI Gateway compat** (`gateway.ai.cloudflare.com`) are exempted in the `EgressController` via `isPlatformNativeHost()` (`src/lib/controller-egress.ts`), checked *before* the `env.EGRESS` guard so they reach upstream even when the binding is unbound.
- The **`LlmInterceptor`** no longer swaps to `env.EGRESS` — AI Gateway is platform-native, so it always egresses direct.
...
- The exemption is a fixed set scoped to exact Cloudflare-owned hosts (the `.r2.cloudflarestorage.com` suffix requires the leading dot, so lookalikes like `r2.cloudflarestorage.com.attacker.example` are not exempt). Adding a future platform primitive is a one-line change to the host set.
- **Residual surface (accepted, documented):** the exemption is host-based, not account-scoped — `*.r2.cloudflarestorage.com` matches *any* account's R2, `api.cloudflare.com` is the entire Cloudflare REST API for any account, and `gateway.ai.cloudflare.com` reaches any account's AI Gateway. So a compromised agent in strict mode keeps an un-gatewayed channel to those Cloudflare-owned hosts (e.g. `PUT` to an attacker-owned bucket), invisible to the customer's Gateway DLP. We do not account-scope: `api.cloudflare.com` must stay exempt for browser-run (the account is in the URL path, not the host) and is a strictly more capable channel than R2, so scoping R2 alone buys nothing. The exposure is bounded by codeflare-managed credentials, each host's own audit (R2 logs, AI Gateway analytics), and the feature being dormant while the `EGRESS` binding is unbound. Documented in the security lane so operators enabling strict egress for DLP know these hosts are outside the boundary by design.
