...
**Status:** Accepted (2026-06-27). Ships OFF; the live Workers VPC binding is deferred until Cloudflare Mesh is provisioned (see Consequences). Reuses the [AD72](#ad72-outbound-https-interception-over-a-worker-side-llm-proxy-for-enterprise-gateway-routing) interception mechanism for a different goal.
...
- The `[[vpc_networks]]` `EGRESS` binding is committed **commented-out** until Cloudflare Mesh / a WARP Connector is provisioned (the account has none today), because a `wrangler deploy` against an unresolvable binding would fail and break both integration and the next production deploy. With it absent `env.EGRESS` is undefined, so the feature fails closed and the dormant state (toggle OFF + binding unbound) is inert — which is what makes shipping the code OFF safe. Uncommenting it after Mesh provisioning is the operator's enable step.
- Raw-TCP / non-HTTP egress is out of scope; the transport indirection (the swappable `send` binding / `EGRESS` Fetcher) is the seam a future raw-TCP plane would extend ([REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-raw-tcp-warpmesh-egress-plane-reserved)).
