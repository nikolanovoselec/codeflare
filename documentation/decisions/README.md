...
- Ordinary test suites use the shared `.github/actions/vitest-suite` action, whose teardown-crash guard is gated by `scripts/ci/check-vitest-report.mjs` and a parsed Vitest JSON report rather than reporter-prose grepping. Path-specific coverage jobs instead share `.github/actions/coverage-suite`, whose result classifier requires the coverage table, rejects reported test or threshold failures, and bounds the backend-only crash exception.
...
**Consequences:** PR wall-clock drops to the slowest lane (sharded backend tests) and docs-only changes skip every lane; deploys skip the multi-GB build+scan on unchanged container inputs. Neither guard can pass an empty run; the ordinary-suite guard also no longer depends on reporter prose. Trade-offs accepted: shared test scaffolding is replicated across split test files (vi.mock hoisting is per module), a reused image's CVE verdict may be up to seven days old, and there is no scripted browser e2e — deployed-UI verification is the agent-driven browser-e2e path.
