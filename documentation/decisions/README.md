...
**Consequences:** Both agents now emit identical findings, severities, and category vocabulary for the same repository state, and parity can no longer rot silently because there is nothing to keep in parity. `grep -rn '@manual' sdd/` is the complete, always-current manual-verification inventory — 447 previously-exempt anchored ACs re-entered CQ-SOURCE/CQ-TEST enforcement, and future coverage work retrofits anchors AC-by-AC and deletes markers, shrinking the manual set monotonically. Pi reviewer prompts embed the full contract instead of the condensed rewrites, an accepted token-cost increase; the transform's compatibility note now appends only to SKILL.md files, which also fixed latent corruption of transformed executable aux files.
