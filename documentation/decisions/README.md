...
- Repeated pull-request runs fingerprint tracked Browser IDE image inputs and may skip the complete-image job only when a bounded search validates a direct successful execution from the same repository and PR, an unchanged workflow contract, and a matching immutable receipt. The fingerprint includes `linux/amd64` and an ISO-week freshness salt, and uncovered Dockerfile inputs disable reuse. Full runs and invalid evidence execute the job normally; the required summary rejects an unverified relevant skip.
