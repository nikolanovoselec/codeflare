...
Editor requests publish immediate bounded progress and provider reasoning through native Inline Chat while keeping final-answer markdown hidden. After the host emits the validated edit transaction, the participant returns a native result detail with the proposed edit count and shows non-blocking Keep/Undo actions. Those actions invoke the pinned host's URI-scoped [`chatEditing.acceptFile` or `chatEditing.discardFile`](https://github.com/microsoft/vscode/blob/df53daabb18cd157bdb08c7f01c34df936cf12f4/src/vs/workbench/contrib/chat/browser/chatEditing/chatEditingActions.ts#L144-L190) command; dismissing the notification leaves native review pending. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-020 + REQ-IDE-026 + REQ-IDE-029: inline-first renders one host-owned edit with native feedback) -->
