...
- **Bounded, verified driver that always releases its lease.** The dashboard `batch-status` poll calls `planRegimeReconcile` synchronously (reports `bucketMigrating`) and `waitUntil(advanceMigration)` runs the copy work in **single-slice pages** — `MIGRATION_PAGE_SIZE` is pinned EQUAL to `MIGRATION_CONCURRENCY` (the platform's 6-outgoing-connection-per-invocation limit), so one page is one concurrent batch and its worst-case wall-clock is a ListObjectsV2 (~4s) plus one object's op-chain (~16s) = `MAX_PAGE_WALLCLOCK_MS` (~20s). Each R2 op runs under a **per-op timeout that AbortController-cancels** on expiry (`withTimeout` + `fetchWithTimeout`), freeing the connection. It persists the `cursor` after every page and transitions migrate→verify in one invocation when budget remains. Crucially, each invocation **starts a new page only while a full worst-case page (`MAX_PAGE_WALLCLOCK_MS`) still fits before its ~26s `waitUntil` budget** (`WAITUNTIL_BUDGET_MS`, under the ~30s ceiling) — or its estimated R2-op count nears `MAX_SUBREQUESTS` — then **releases the lease**, so the next poll resumes from the cursor; `MIGRATION_LEASE_MS` (60s) is now only a crash backstop. A migrate phase re-encrypts; a verify phase HEAD-scans every object under the target regime; the regime + `generation` advance **only after a clean full verify**. *(This took three iterations, each verified live on the 680-object integration bucket. Draft 1: one sequential page per poll, 10-min lease → a page blew the `waitUntil` clock, killed mid-page before persisting the cursor, long lease blocked every poll until expiry. Draft 2: concurrency + a multi-page loop on a 15s wall-clock budget — but 16-way concurrency exceeds the 6-connection limit and the loop overran the ~30s window → force-killed (uncatchable) → lease held → still stalled. Draft 3 (this one): even with concurrency 6 + per-op timeouts, a multi-slice 32-object page could still run `ceil(32/6) × (HEAD+HEAD+PUT) ≈ 144s` and overrun the window mid-page on slow R2 — so the page is pinned to ONE slice and the budget gate checks the worst-case PAGE wall-clock, guaranteeing any started page finishes before the kill and the lease-release always runs. Root-caused via two GPT-5.5 consults + a code-review pass over the live KV/worker forensics.)*
- **Backend gate + container drain (the safety boundary).** While not `ready`, every writer returns `409 BUCKET_MIGRATING`, the sync fan-out is skipped, running containers are drained (best-effort — a drain failure leaves a brief stray-write window, caught by the verification rescan + read self-heal), and in-flight multipart uploads are aborted. The frontend reuses the "Upgrading" affordance (New Session → "Migrating").
