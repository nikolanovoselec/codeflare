...
- Bun for executor perf: Bun is installed globally in the image (`npm install -g bun`). context-mode autodetects Bun on first run and uses it as the JS/TS subprocess runtime for `ctx_execute` / `ctx_batch_execute`. Measured speedup on this image's stack (Node 24, debian-slim, 1 vCPU): a representative workload averages 78 ms cold-start under Node and 38 ms under Bun, ~2x per call. Bun is a perf-only addition; the shim above is what fixes #309.
