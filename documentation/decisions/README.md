...
**Consequences:** PR wall-clock drops to the slowest lane (sharded backend tests) and docs-only changes skip every lane; deploys skip the multi-GB build+scan on unchanged container inputs. The guard can no longer pass on reporter-format drift or empty runs. Trade-offs accepted: shared test scaffolding is replicated across split test files (vi.mock hoisting is per module), a reused image's CVE verdict may be up to seven days old, and and there is no scripted browser e2e — deployed-UI verification is the agent-driven browser-e2e path.
