...
**Status:** Accepted (2026-08-16). Adds an evidence gate to [AD128](#ad128-inline-review-lifecycle-belongs-to-the-pinned-controller) without changing its review ownership. Implements [REQ-IDE-034](../../sdd/spec/browser-ide.md#req-ide-034-bounded-inline-lifecycle-diagnostics).
...
**Consequences:** The probe commit is intentionally diagnostic, not a claimed product fix. The next runtime report must include the activation, request, stream, tab-change, and delayed snapshot lines. A proxy or extension correction follows only from that evidence, then the full one-tab Keep/Undo sequence is repeated in a fresh session.
