...
The entrypoint symlinks `/tmp/jiti` → the baked cache at boot (the same pattern as the npm preseed `node_modules` symlink). <!-- @impl: Dockerfile::PI_CODING_AGENT_DIR --> <!-- @impl: entrypoint.sh::relay_managed_pi_extensions --> At the time of this decision, all coding agents—including Pi—resolved from `@latest`; the warm run therefore used the exact Pi/Jiti installed in that build. The 2026-08-02 update below supersedes that mutable-resolution policy.
...
**2026-08-03 amendment:** Executable `git` and `gh` commands are now candidates only. The command repository's checked-out branch and local `HEAD` must exactly match that branch's authoritative open protected-base PR head; command arguments no longer supply push source, destination, configured push branch, or merge identity. PR-number-specific checkpoints preserve independent incremental review ranges. This amendment supersedes the push source/destination details in the preceding historical decision while retaining its session-scoped review window and repository binding. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::currentReview -->

The live handler also records that each recognized boundary was evaluated, including ineligible no-plan outcomes. After reload, settled enforcement reconstructs only a persisted successful boundary lacking both that evaluation marker and a correlated launch plan, then emits its initial plan once ([REQ-AGENT-110](../../sdd/spec/agents.md#req-agent-110-pi-pr-boundary-missing-launch-follow-up)). <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->
