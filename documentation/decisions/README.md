...
Keep the public `/api/vscode/<sessionId>/` location unchanged. After the Worker validates authentication, ownership, session ID, host, and protocol, it forwards canonical proxy identity. The host strips only the exact session prefix, preserves non-selector path/query bytes, and applies the same transformation to HTTP and WebSocket upgrades. AD120 rejects public workspace selectors and adds the fixed folder only on the private root hop. Redirects, cookie paths, service-worker scope, assets, and reconnect URLs must remain beneath the external session location; this is a behavioral integration contract, not an assumption from code-server's CLI flags.
...
**Context:** code-server's CLI folder made `/home/user/workspace` the initial location but its root redirect exposed that path as a public `folder` query, and callers could substitute `folder`, `workspace`, or `ew`. Code OSS also contributed an account-backed **Code Review** action while setup was incomplete. Users want theme, Explorer, and open-file continuity, but syncing live code-server storage would also carry mutable databases, extension/global state, SecretStorage, authentication, chat history, logs, and unsafe WAL/SHM companions.
...
**Consequences:** Public Browser IDE URLs remain clean and cannot choose another workspace through code-server's supported selectors. Safe UI preferences follow the user without making live editor or credential state bucket-stable. The context-key workaround and exact Code OSS menu gate must be reverified on every coupled code-server bump and during pass@3 integration. Larger image reductions require a separate measured proposal; this change deliberately takes no Pi deduplication saving.
