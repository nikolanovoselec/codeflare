...
- Pi reviewers run as visible session-scoped subagents using the native Graphify tool surface.
...
**Consequences:** Review execution is visible and deterministic through a focused adapter of the stock `AgentWidget`. The copied display structure supplies queued/running/finished presentation, animation, elapsed time, tool and token metrics, compaction count, terminal-width truncation, and timer cleanup for only the correlated review records. The adapter uses the public service API and adds no second agent extension or execution path.

Durable dispatch records remain completion and retry evidence; reload restores rows only when the reloaded service still has the matching live record. Service launches and review windows remain extension-owned, never prompt the root model, and do not appear as assistant public-tool-call transcript blocks. Serial reviewer execution fits the shared-process memory budget, while CI queue bypass preserves independent monitoring. Reload cannot fabricate completion or duplicate a dispatch unknown to the new service; within the live service, stopped, aborted, and error states are retried from the recorded agent ID. AD76's detached lanes and AD80's hard merge gate no longer govern Pi.
...
**Consequences:** CI launch and monitoring truth become behaviorally testable through one boundary plan, one resolver script, stock service dispatch, and one tiny agent. The separate root-rule trigger is removed, so a Git command cannot create a duplicate launch. Non-SDD repositories and default-mode sessions receive CI-only plans. Review and CI never wait for or restart each other, and CI never becomes a review lane. Claude CI behavior is unchanged.
...
**Decision:** The root Pi session emits public background requests and reconstructs launch, running, failure, success, reminder, and GIVEUP state from durable session JSONL. Every launch serializes the same bounded request items into model-facing custom-message content and durable details metadata, because Pi exposes only custom-message content to the model. One tiny active request-ID pointer enables reload discovery, while one request-specific immutable execution snapshot prevents a late call from reading replacement work.
