...
**Related REQ:** [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring), [REQ-AGENT-070](../../sdd/spec/agents.md#req-agent-070-claude-on-demand-ci-monitoring-policy).
...
**Context:** The first post-reload transactional Vault smoke contained one frozen 51 KB markdown file. The Pi `vault-extract` worker inherited the foreground reasoning level and broad read/search/context tools. It reread extraction/Vault/Graphify policy, read the file more than once, ran independent heading/wikilink/identifier/inline-code discovery passes, and reached 84.2k tokens, 12 tool calls, and 336 seconds without writing or committing a chunk. The prior legacy Vault task took 762.9 seconds and 130.6k tokens. Pi session capture also repeatedly replayed up to 200 historical turns at 8000 characters and imposed Claude's multi-pass scratchpad after the root had already prefiltered the transcript. The first bounded live capture produced a strong note in two tool calls, but model-authored graph JSON used four noncanonical `concept:Label` IDs, duplicated five edges, and labeled the document with its filename rather than its H1.

**Decision:** Pi's `memory-capture` and `vault-extract` workers are finite extraction jobs, not open-ended research agents. Generated frontmatter and every public request set provider-neutral `thinking: medium`; public requests stop after four agent turns; generated workers expose only Bash. Their embedded contract forbids skill/project discovery and gives a two-call normal path: memory capture reads the prompt plus one self-contained snapshot whose `transcript` field is the sole conversation input (no `INPUT_FILE` or separate transcript path), while Vault extraction reads its prompt, snapshot, and frozen files; each then performs one write/required graph-commit call. Pi memory captures only prompts after the last successful counter and bounds the retained retry window to 40 text turns at 4000 characters each; it synthesises the note directly instead of building/rereading scratch files. A deployed deterministic helper builds the memory graph from that note, using its H1, stable Vault-relative document ID, canonical `concept_<normalised_label>` IDs, and deduplicated edges; the model no longer hand-authors graph JSON. The shared cumulative merge also normalizes serialized edge evidence because Graphify's undirected-build conversion can otherwise duplicate a unique input edge. Vault extraction builds a compact semantic graph rather than an exhaustive inline-token inventory. Noncritical visualization gets 15 seconds and cannot hold task success open. <!-- @impl: scripts/generate-agent-seed.mjs::adaptAgentFrontmatter --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::buildPublicExtractionRequest --> <!-- @impl: preseed/agents/pi/scripts/build-memory-graph.py::build_graph --> <!-- @impl: preseed/agents/pi/scripts/merge-vault-graph.py::merge_node_link_evidence -->
...
**Decision:** Terminal scrolling has one owner per mode. A synchronous scroll-event guard owns `FOLLOW_OUTPUT` and yields to correlated user intent. Wheel, pointer, navigation-key, touch-scroll, and registered external intent enter `READ_SCROLLBACK`; ownership persists until the viewport returns to the live bottom, while xterm alone owns every output-driven trim, including a legitimate zero offset after viewed lines age out. `MOBILE_INPUT_LOCKED` remains the explicit exception: tapping opens the keyboard, the keyboard lifecycle performs fit plus bottom anchoring, generic correction stays inactive, and vertical swipes remain terminal input or fullscreen application wheel gestures. The write buffer performs no scroll correction. <!-- @impl: web-ui/src/hooks/useScrollCorrection.ts::useScrollCorrection --> <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @impl: web-ui/src/stores/terminal.ts::flushWriteBuffer --> <!-- @impl: web-ui/src/lib/touch-gestures.ts::attachSwipeGestures -->
