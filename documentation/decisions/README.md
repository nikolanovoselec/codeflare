...
| [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) | Make D1 the complete session lifecycle authority | D1 `stopped` again requires generation-fenced confirmed exit; the timeout exception in AD157 is superseded. | Architecture, Storage, Reliability | Active |
...
| ~~[AD157](#ad157-reset-stale-stopping-ownership-on-owner-status-reads)~~ | ~~Reset stale stopping ownership on owner status reads~~ | [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) supersedes this historical timeout policy for future sessions because time cannot prove process exit. | Architecture, Storage, Reliability | Superseded |
...
**Status:** Partially superseded (2026-09-24); the SDK-only terminal stop decision below is historical. Current terminal stop authority is owner-scoped D1 ([REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC1–AC2); failed no-start health verification or terminal-not-ready gates retryable readiness, while a verified survivor can forward despite stale SDK state. Other decisions remain accepted.
...
**Status:** Active. The 2026-09-21 timeout exception in [AD157](#ad157-reset-stale-stopping-ownership-on-owner-status-reads) was superseded for future sessions on 2026-09-24; generation-fenced confirmed exit remains required.
...
**Status:** Superseded for future sessions (2026-09-24). Historical 2026-09-21 decision; [AD155](#ad155-d1-owns-complete-session-lifecycle-authority) again requires confirmed exit.
...
**Historical decision (superseded):** Before returning owner batch status, reset that owner's `stopping` rows whose transition is strictly older than three minutes to `stopped`, without process-exit evidence. Future sessions do not use this policy: a live process might still own resources and managed mutation stays blocked until confirmed exit.

**Consequences:** The old policy could release ownership of a surviving workload. Future status reads preserve `stopping` and the termination intent, leaving Start and managed mutation blocked until a generation-fenced exit is positively confirmed ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-retains-ownership-until-confirmed-exit)).

**Related REQs:** [REQ-SESSION-018](../../sdd/spec/session-lifecycle.md#req-session-018-d1-lifecycle-evidence-is-generation-fenced), [REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-retains-ownership-until-confirmed-exit).
