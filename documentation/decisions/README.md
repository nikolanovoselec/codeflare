...
**Enterprise exception (REQ-OPERATOR-053):** A confirmed applicable protected Boundary Reviews Action selects remote-only Review for an authenticated Enterprise Pi PR boundary; Pi starts no local reviewer wave or local completion marker. Confirmed absence retains AD144's local plan, while uncertain selection stays non-green. This does not change Claude, non-Enterprise, or user-invoked `/review` behavior; Action claim and independent publication require separate proof.
