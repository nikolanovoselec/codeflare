...
| [AD145](#ad145-terminal-mode-is-an-immutable-per-session-choice) | Make terminal ownership an immutable per-session choice | Classic remains the default full Codeflare terminal; opt-in Herdr sessions expose one surface and give inner topology to an ephemeral Herdr runtime. | Architecture, Build / Container, Terminal | Active |
---
...
### AD145: Terminal mode is an immutable per-session choice

**Category:** Architecture, Build / Container, Terminal

**Status:** Accepted (2026-08-27); amended 2026-08-28.

**Context:** Codeflare's classic terminal owns up to six outer PTYs, tabs, process labels, saved layouts, and fixed tiling. Herdr provides terminal-native tabs, panes, splits, workspaces, agent state, and detach or reattach semantics. Making either topology universal removes a useful fallback: classic behavior is proven across browsers, while Herdr provides richer terminal-native orchestration. Running both topology layers inside one session would still duplicate controls and create focus and resize contention.

**Decision:** Terminal mode is stamped by the Worker when a session is created and is immutable for that session. The server-backed `herdrEnabled` preference affects only future sessions and defaults false; a missing or invalid historical stamp resolves to `classic`. Classic sessions retain the full Codeflare tab, tiling, label, layout, right-click, and terminal-ID `1` through `6` behavior. Herdr sessions expose one Codeflare surface at terminal ID `1`; the fixed launcher starts or attaches the official pinned client to `cf-<SESSION_ID>`, and Herdr owns every inner tab, pane, split, and workspace.

The persisted session stamp, never the current preference or browser input, governs create, start, resume, prewarm, routing, frontend ownership, cleanup, and compatibility behavior. Existing xterm.js rendering, authenticated WebSocket route, host `Session`, `node-pty`, resize authority, input classification, idle accounting, and container lifecycle remain shared. MultiView may combine both modes, resolving a classic member to its active outer terminal and a Herdr member to terminal `1`. Browser IDE remains independent and starts no Herdr runtime.

Herdr runtime state stays ephemeral under mode-0700 `/run/codeflare/herdr`; R2 does not persist topology, pane history, sockets, logs, plugins, updater state, or user-edited Herdr settings. The browser never speaks Herdr's private protocol and no Herdr route becomes public.

**Consequences:** Both complete terminal experiences remain supported instead of treating classic behavior as a partial fallback. Mode-aware boundaries preserve classic right-click and mobile behavior while Herdr receives terminal mouse input, bounded OSC 52 clipboard writes, and fixed local attention events. Only Herdr shutdown stops a named runtime; browser disconnect preserves it until Codeflare lifecycle stops the container. Previously deleted browser-local classic layouts cannot be recovered, but future classic layouts persist normally.

**Related REQs:** [REQ-TERM-001](../../sdd/spec/terminal.md#req-term-001-terminal-surface-count-follows-session-mode), [REQ-TERM-005](../../sdd/spec/terminal.md#req-term-005-herdr-runtime-and-configured-agent-startup), [REQ-TERM-006](../../sdd/spec/terminal.md#req-term-006-herdr-owns-in-session-terminal-topology), [REQ-TERM-007](../../sdd/spec/terminal.md#req-term-007-classic-topology-remains-available), [REQ-TERM-011](../../sdd/spec/terminal.md#req-term-011-visible-terminal-panes-own-websocket-connections), and [REQ-TERM-012](../../sdd/spec/terminal.md#req-term-012-multiview-virtual-session-workspace).
