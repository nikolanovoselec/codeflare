...
**Consequences:** Review execution is visible and deterministic through the stock `AgentWidget`. The package-owned renderer supplies queued/running/finished presentation, animation, elapsed time, tool and token metrics, compaction count, terminal-width truncation, and timer cleanup for the same records the service executes. Codeflare owns no second timer or custom TUI component.

Durable dispatch records remain completion and retry evidence but are not replayed as synthetic widget rows after reload. Service launches and review windows remain extension-owned, never prompt the root model, and do not appear as assistant public-tool-call transcript blocks. Serial reviewer execution fits the shared-process memory budget, while CI queue bypass preserves independent monitoring. Reload cannot fabricate completion or duplicate a dispatch unknown to the new service; within the live service, stopped, aborted, and error states are retried from the recorded agent ID. AD76's detached lanes and AD80's hard merge gate no longer govern Pi.
