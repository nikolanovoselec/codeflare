...
The Worker is the trust boundary. It conditionally checks GitHub, verifies repository identity, immutable release metadata, asset digests, signature, sequence, schema, paths, bounds, seed ABI, runtime dependency hash, and extension records as a bounded stream, then stores content-addressed bytes in one deterministically named deployment R2 bucket. Immutable assets live under `releases/<digest>/`; each trust configuration advances its fingerprinted active pointer by conditional compare-and-swap, while a repository-stable conditional pointer serializes concurrent signing-key selections before selected KV state can settle. <!-- @impl: src/lib/remote-curation.ts::resolveManagedEnvironmentRelease --> <!-- @impl: src/lib/remote-curation.ts::configureManagedEnvironment -->
