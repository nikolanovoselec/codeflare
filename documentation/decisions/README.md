...
**Consequences:** Codeflare owns the sidebar renderer, PTY/RPC transports, security boundary, and behavioral tests. The image adds a small MIT-licensed terminal renderer and native addon instead of the official VSIX. The required Browser IDE lane checks the exact image, native addon, state isolation, permission controls, queued approvals, resource evidence, and descendant cleanup before deployment. Approved commands and executable customizations remain trusted code in the shared container rather than a sandbox.
