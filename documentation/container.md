...
**Tier-split gating (AD52):** the MCP server is registered in `~/.claude.json` for both default and advanced session modes. The discipline pieces - SessionStart context-injection hook, PostToolUse-on-clone triage hook, `graph-first.md` rule, and `graphify/SKILL.md` - ship in advanced session mode only. Default session mode users have the capability without the proactive discipline.
