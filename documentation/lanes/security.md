...
The Browser IDE consumes coder/code-server 4.130.0 as an exact unmodified MIT-licensed release ([REQ-IDE-001](../../sdd/spec/browser-ide.md#req-ide-001-per-session-browser-ide-served-through-the-worker-proxy), [REQ-SEC-011](../../sdd/spec/security.md#req-sec-011-container-image-scanned-for-cves-before-deploy), [AD119](../decisions/README.md#ad119-replace-openvscode-with-pinned-code-server-behind-the-existing-session-proxy)). The image build verifies the linux-amd64 archive SHA-256, code-server package/product commit, and actual embedded Code OSS package version; Shadow Pins derives the recorded upstream VS Code gitlink from the immutable release tag. The migration does not carry AD97's OpenVSCode vulnerability exceptions forward: the deployment gate must surface every fixable HIGH/CRITICAL code-server finding, while its documented `ignore-unfixed` policy still applies; any code-server exception requires fresh evidence and explicit review.

code-server binds only to `127.0.0.1:13337` and uses `--auth none` because Cloudflare Access, allowed-Origin validation, session ownership, the Durable Object selector, and the container bearer check already gate the route.

The Worker deletes all client `Forwarded`/`X-Forwarded-*` values, writes canonical host/protocol identity from the validated request URL, and preserves only an Origin that passed the allowlist check. The host validates the container bearer and exact `/api/vscode/<SESSION_ID>` prefix before any upstream HTTP or WebSocket connection, strips the prefix, removes injected authorization and hop headers, and lets code-server compare the preserved caller Origin with the canonical external Host. No wildcard trusted-origin bypass is configured. Per-session container isolation limits blast radius but does not make untrusted workspace content safe; each runtime bump still requires complete-image scan and integration evidence.
