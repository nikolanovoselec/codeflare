...
The **master** `CLOUDFLARE_API_TOKEN` — the deploy / account-management token stored in Worker secrets (GitHub Secrets -> Worker secrets) — never enters the container. Containers receive R2 credentials (a scoped key pair) for storage, never that master token. In **non-enterprise** modes a container's own `CLOUDFLARE_API_TOKEN` env var, when present, is the user-provided **Connect-to-Cloudflare** deploy token (a different credential entirely from the master). In **enterprise**, the container holds NO real Cloudflare token at all — only a non-secret placeholder for browser-run, described next.
