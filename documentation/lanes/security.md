...
Pi runs through code-server's native Chat participant API, not a custom webview. The extension retains one local compatibility provider for the native participant's model-entry contract; its panel/editor model is selectable, account-free, and unable to generate. Deployment image smoke verifies its packaged registration and fail-closed generation contract; deployed activation evidence remains tracked by [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent) and [REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility). <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_COMPATIBILITY_PROVIDER -->
