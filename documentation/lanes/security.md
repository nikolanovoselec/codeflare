...
A hidden, non-selectable `copilot` fallback exists only for the pinned extension host's absent-request-model lookup. The globally unique `codeflare` vendor keeps the visible model outside Code OSS's Copilot entitlement and sign-in path; tool-calling capability is only pinned Code OSS filter metadata. Neither adapter introduces a GitHub provider, authentication, or inference path. The visible participant/provider label is **Codeflare** while stable private IDs remain unchanged. On startup the inventory marks generic Chat setup complete, hiding Code OSS's account-backed **Code Review** action without disabling native Chat or **Review with Codeflare**. It disables only Code OSS's duplicate `~/.claude/agents` personal source while retaining `~/.copilot/agents`; native Pi's own agent discovery is unchanged. The participant does not read `request.model`, invoke that model for inference, or access `vscode.authentication`; it calls the fixed local RPC binary directly. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings -->
