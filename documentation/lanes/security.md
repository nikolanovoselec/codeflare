...
**Mitigation:** Tool execution is deliberately unrestricted by user direction. Closed inventories, bounded untrusted context, fresh Pi processes, isolated official-Claude configuration, exact package checks, loopback token authentication, and generation cleanup still protect unrelated process, context, credential, and supply-chain boundaries. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::buildNativePiPrompt --> <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::sidebarApproval --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOfficialClaudeIde --> <!-- @impl: openvscode/agent-sidebar/src/process-generation.ts::reapSidebarGeneration -->
...
Explorer and editor-context review canonicalize the selected or active file against `/home/user/workspace` and reject symbolic-link aliases or escapes before opening Chat. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::canonicalWorkspaceFilePath --> Editor collection applies the same boundary to file URIs, rejects non-file values and malformed ranges, bounds active content, selection, open files, diagnostics, references, history, and total UTF-8 bytes, and marks the result as untrusted workspace data. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput --> Each request gets a fresh no-session process, preventing hidden cross-chat or terminal history. <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::runNativePiChat -->
...
Every IDE session seeds code-server User settings that disable workspace trust and ignore extension recommendations for all agent kinds, so the workspace opens without a trust or recommended-extensions prompt. Live storage remains under `/tmp`; after code-server is reaped, only theme values and key-specific Explorer/open-file resource schemas enter a bounded atomic snapshot; unknown fields and opaque strings are rejected ([REQ-IDE-016](../../sdd/spec/browser-ide.md#req-ide-016-ui-state-capture-and-restore-ordering)). Raw databases, workspace/global extension storage, SecretStorage, Accounts authentication, chat history, logs, WAL, and SHM never enter that snapshot. On restore, managed Codeflare and Claude settings override preferences before launch. A settings-preparation failure refuses the launch. <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::writeOpenVscodeProfileState --> <!-- @impl: entrypoint.sh::_openvscode_prepare_agent -->

Disabling workspace trust removes VS Code's own gate on the untrusted repository input noted above; it adds no new exposure because IDE agents already run unrestricted and the container — not the trust prompt — is the residual boundary. <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::writeOpenVscodeProfileState -->
