...
Claude's external managed settings select Anthropic Manual (`"default"`) permission mode and disable bypass, automatic edits, Remote Control, IDE auto-install, updates, and telemetry. OpenVSCode refuses to launch when those settings cannot be restored. The temporary config projection links approved credentials/configuration but excludes terminal projects, history, state, caches, logs, and unknown paths. `disableLoginPrompt` prevents a second Anthropic login while inherited approved routing and linked credentials remain available. Credential bytes are never copied into generated files. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareSidebarConfig -->
