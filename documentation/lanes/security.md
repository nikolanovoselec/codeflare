...
Integration runs `30893082736` and `30893082817` at head `8a745b736aa948e9c88351ce5b2bcd5098ae69e2` produced the same complete six-finding Node.js set before publication. CVE-2026-69192 appeared as one `ip-address` 10.1.0 declaration and two 10.2.0 declarations, although every affected committed runtime lock installs patched 10.4.0. CVE-2026-13697 appeared as two upstream Pi declarations of undici 8.5.0, while both Pi runtime locks install patched 8.9.0, plus one undici 7.28.0 occurrence in Node/npm image tooling. Codeflare does not enable undici's shared cache interceptor, and npm use remains inside the authenticated user's single-tenant container. The tuple gate requires those exact versions and multiplicities until upstream package metadata and the pinned Node image advance; none of these IDs is globally suppressed.
