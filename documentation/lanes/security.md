...
Pi runs through OpenVSCode's native Chat participant API, not a custom webview. Because the pinned host refuses to enter any participant without a model, the extension registers one local compatibility provider whose panel-only model is hidden, account-free, and unable to generate. The participant does not read `request.model`, invoke that model for inference, or access `vscode.authentication`; it calls the fixed local RPC binary directly. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_COMPATIBILITY_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate -->
