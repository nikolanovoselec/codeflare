...
Pi runs through OpenVSCode's native Chat participant API, not a custom webview. The participant does not access `request.model`, `vscode.lm`, or `vscode.authentication`; it calls the fixed local RPC binary directly. Editor collection canonicalizes file URIs against `/home/user/workspace`, rejects symbolic-link aliases, escapes, non-file values, and malformed ranges, bounds active content, selection, open files, diagnostics, references, history, and total UTF-8 bytes, and marks the result as untrusted workspace data. Each request gets a fresh no-session process, preventing hidden cross-chat or terminal history. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::runNativePiChat -->
