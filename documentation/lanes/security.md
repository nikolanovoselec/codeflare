...
Sidebar Pi leaves the standard built-in tools unrestricted instead of registering guarded replacements. The extension host returns approval for any confirmation request without opening an editor document or modal. It separately validates and bounds documented `select` and `input` RPC dialogs, correlates their value or cancellation response, and rejects malformed or unknown blocking UI methods. There is no path, stale-content, command-content, unknown-tool, or MCP approval gate. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::sidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost -->
