...
Pi runs through code-server's native Chat participant API, not a custom webview. The extension retains two local compatibility providers for the native participant's separate fallback and picker contracts; the hidden fallback and selectable account-free panel/editor model are both unable to generate. GitHub Actions image smoke verifies its packaged registration and fail-closed generation contract under [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent), [REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility), and [REQ-IDE-019](../../sdd/spec/browser-ide.md#req-ide-019-codeflare-eligibility-in-editor-inline-chat) and [REQ-IDE-020](../../sdd/spec/browser-ide.md#req-ide-020-unrestricted-pi-editor-request-execution). <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_FALLBACK_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_VISIBLE_PROVIDER -->
...
The visible participant/provider label is **Codeflare** while stable private IDs remain unchanged. On startup the inventory marks generic Chat setup complete, hiding Code OSS's account-backed **Code Review** action without disabling native Chat or **Review with Codeflare**. It disables only Code OSS's duplicate `~/.claude/agents` personal source while retaining `~/.copilot/agents`; native Pi's own agent discovery is unchanged. <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings -->
