...
**Landing page meta CSP (hash-based, AND-enforced with the Worker header):** The static landing documents additionally carry an Astro-built per-page `<meta http-equiv="content-security-policy">` (SHA-256, configured via `security.csp` in `landing/astro.config.mjs`) layered on top of the Worker CSP header. Browsers AND-enforce both: the meta's per-element hashes authorize inline `<style>` and `<script>` ELEMENTS without a blanket `style-src 'unsafe-inline'`, so an injected inline `<style>` is blocked by the meta even though the Worker header retains `'unsafe-inline'` in `style-src` (required for the React SPA — CF-016 — and intentionally kept in the shared header). Animated components use `style="--i:N"` attributes; because the landing is statically built, those per-item values are resolved and hashed at build time, and `'unsafe-hashes'` in the meta's `style-src` lets the hashed inline style attributes apply (CSP3 requires it for inline style attributes even when their hash is present) without a blanket `'unsafe-inline'`. All landing scripts are external-source-only (`'self'` + Turnstile + Cloudflare Insights); no inline scripts are emitted. The landing CSS is inlined into each document (`build.inlineStylesheets: 'always'`) and authorized by its SHA-256 hash in the meta. `frame-ancestors` is not valid in a `<meta>` CSP, so clickjacking protection remains the Worker's `X-Frame-Options: DENY`; see [REQ-SEC-008](../../sdd/spec/security.md#req-sec-008-security-headers-on-every-response) AC8.
