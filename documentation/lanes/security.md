...
Native Pi and Claude IDE agents run with owner-approved broad container capability. Pi direct writes/commands are not transactional editor edits; Claude bypasses tool approval by design. Credentials, IDE configuration projection, and public routing are constrained, but a compromised trusted agent can still act with its legitimate session capabilities. [Browser IDE requirements](../../sdd/spec/browser-ide.md) own the product contract.
