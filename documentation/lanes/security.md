...
The Pi inventory contains one Codeflare-owned workspace extension; the Claude inventory contains Anthropic's exact official package; the unsupported inventory contains no extension. It begins empty, and code-server may create only a regular `extensions.json` registry file containing `[]` after startup. The build validates source directories, publisher/name/version/entry point, executable platform binary, symbolic links, and retained VSIX files before atomically making both extension trees root-owned and immutable. The complete-image smoke rejects any unsupported extension or unknown inventory entry. The owner accepts the proprietary extension's license ambiguity under [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration). <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::main -->

Pi runs through code-server's native Chat participant API, not a custom webview. The extension retains one local compatibility provider for the native participant's model-entry contract; its panel-only model is hidden, account-free, and unable to generate. Complete-image smoke verifies its packaged registration and fail-closed generation contract; deployed activation evidence remains tracked by [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent) and [REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility). <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_COMPATIBILITY_PROVIDER -->
