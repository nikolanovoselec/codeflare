...
| Provider and deployment credentials | Prompt injection or compromised container reads and exfiltrates a token | Withhold master credentials, scope credentials that must enter a container, and inject at a validated egress boundary only where interception is implemented | A legitimately scoped operation can still be destructive; provider policy remains external |
...
The **master** `CLOUDFLARE_API_TOKEN` — the deploy / account-management token stored in Worker secrets (GitHub Secrets -> Worker secrets) — never enters the container. Containers receive R2 credentials (a scoped key pair) for storage, never that master token.

In **non-enterprise** modes a container's own `CLOUDFLARE_API_TOKEN`, when present, is the user-provided deploy token (a different credential entirely from the master): a pasted **PAT** flows to the container as its real value, but a **Connect-to-Cloudflare OAuth** token does **not** — because a dashboard OAuth access token is short-lived, the container gets only the non-secret placeholder `codeflare-oauth` and the boundary interceptor stamps a freshly-refreshed token per request, so a running session survives past the access-token expiry ([REQ-AGENT-078](../../sdd/spec/agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary), described below under "Non-enterprise Cloudflare OAuth token"). In **enterprise**, the container holds NO real Cloudflare token at all — only a non-secret placeholder for browser-run, described next.

In enterprise mode the per-user "Push & Deploy" accordion is hidden, so browser-run's Cloudflare **Browser Rendering** token is the admin-global value configured once in the Setup wizard ([REQ-BROWSER-007](../../sdd/spec/browser-run.md#req-browser-007-enterprise-admin-configured-browser-rendering-token)), stored encrypted at rest (kv-crypto, AAD-bound to its KV key).

The real token is **never placed in the container** ([REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)): `applyEnterpriseBrowserToken` emits only the non-secret placeholder `codeflare-enterprise` as `CLOUDFLARE_API_TOKEN` (plus the non-secret account id), so browser-run runs in authed mode while holding no credential.

Like the GitHub token, the real token is **egress-injected**. A `CloudflareBrowserInterceptor` WorkerEntrypoint strips the placeholder and stamps the real `Bearer` token at the `api.cloudflare.com` boundary, but only for the wizard-configured account's `/client/v4/accounts/<acct>/browser-rendering/*` path (REST + the CDP WebSocket, bridged); every other `api.cloudflare.com` request is routed to the Gateway when strict, else `403`.

The blast radius of a prompt-injected read inside the container is **zero**: there is no Browser Rendering token to read, and the injected credential can only ever reach the one configured account's Browser Rendering API. When no Browser Rendering token is configured the entire browser-run surface (the MCP servers, the Pi extension, and the `browser-run`/`browser-e2e` skills) is withheld from the agents.

### Non-enterprise Cloudflare OAuth token

A dashboard **OAuth** access token (from "Connect to Cloudflare") is short-lived by design — it expires within hours, and nothing can refresh an env var already baked into a running container, so a session that once held the real token began failing every `wrangler`/browser-run call with `9109 Invalid access token` once it expired.
...
The token is resolved **solely** from the session-bound bucket (`props.bucket`, bound at wiring), never from a request header, so no in-container request can widen resolution to another user's bucket; the interceptor fails closed `401` with no upstream call when no valid token can be minted. This mode is double-guarded off enterprise — the OAuth CF-API interception-registry entry (`cloudflareOauthApi`, container-interception.ts) acts only when `!isEnterpriseMode` **and** the container's `CLOUDFLARE_API_TOKEN` equals the OAuth placeholder (which is distinct from the enterprise `codeflare-enterprise` value) — so it can never wire or collide on that host in enterprise, and the enterprise browser-rendering branch above is unchanged.
...
The OAuth mode also intercepts the AI Gateway data-plane host `gateway.ai.cloudflare.com`, stamping the refreshed token as `cf-aig-authorization` ([REQ-AGENT-078](../../sdd/spec/agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary) AC5). Both intercepted hosts are TLS-terminated by a platform-mounted intercept CA (`/etc/cloudflare/certs/cloudflare-containers-ca.crt`), so the container must **trust** it or its HTTPS clients reject the connection with `SELF_SIGNED_CERT_IN_CHAIN`.

`entrypoint.sh` installs the CA into the system store and exports `NODE_EXTRA_CA_CERTS`/`SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE` into the agent `.bashrc` — in a block **separate from** the enterprise CA-trust, gated on `ENTERPRISE_MODE != active` **and** the CA file's presence (AC6), so the enterprise trust path is untouched and a deploy with no interception (no CA mounted) is unchanged.
...
**No credentials in the container.** The AI Gateway URL and gateway token are resolved entirely Worker-side: wizard-configured in KV first (`setup:aig_gateway_url` plain, `setup:aig_token` encrypted), with the deploy-time `AIG_GATEWAY_URL`/`AIG_TOKEN` secrets as an optional fallback (`getAigConfig`, [REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)). They are handed to the `LlmInterceptor` WorkerEntrypoint via per-session props.

They are never emitted as container env vars, never logged, and never forwarded inside the container boundary. The container only receives `ENTERPRISE_MODE=active` (a non-secret deploy var) and a constant non-secret placeholder credential (`codeflare-enterprise`) that puts each agent CLI into API mode.

The interceptor strips the placeholder (along with the container's `x-api-key`/`host`/`content-length`) before forwarding. On the REST API path (`api.cloudflare.com/.../ai/v1/*`) it re-authenticates with `AIG_TOKEN` as `Authorization: Bearer` (Workers AI scope); on the compat fallback path (`gateway.ai.cloudflare.com/.../compat/*`) it re-authenticates with `AIG_TOKEN` as `cf-aig-authorization: Bearer` (AI Gateway Run scope).

In both cases the placeholder never reaches the AI provider or the gateway. This satisfies [REQ-ENTERPRISE-004](../../sdd/spec/enterprise-mode.md#req-enterprise-004-outbound-interception-llm-routing-to-customer-ai-gateway) AC3; the REST transport is recorded in [AD74](../decisions/README.md#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api).

**Container secret hygiene — only `CONTAINER_AUTH_TOKEN` under strict egress + Governed Mode.** Beyond the AI Gateway, `buildEnvVars` keeps every other credential out of the enterprise container: the per-user GitHub token and the admin Browser Rendering token are non-secret placeholders, injected at their egress boundaries ([REQ-GITHUB-003](../../sdd/spec/github.md#req-github-003-enterprise-egress-injected-github-credentials), [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container));

`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` are not emitted in any mode (they had no consumer and merely surfaced Pi's `amazon-bedrock` provider); per-user LLM keys are suppressed; the R2 access key is a non-secret placeholder under strict egress (re-signed worker-side — see "R2 key containment" below); and `ENCRYPTION_KEY` is omitted in Governed Mode (rclone, its only in-container consumer, does not use it there).

The net invariant ([REQ-ENTERPRISE-021](../../sdd/spec/enterprise-mode.md#req-enterprise-021-governed-mode-migration-safety-and-access-boundary) AC1): **under strict Gateway egress AND Governed Mode together, the container's only real secret is the DO-issued `CONTAINER_AUTH_TOKEN`** — every other env var is a non-secret placeholder, identifier, or routing hint.
...
**CA trust for TLS interception.** Platform outbound-HTTPS interception TLS-terminates the container's connections to LLM provider hosts and re-presents them with a certificate signed by the Cloudflare containers CA (`/etc/cloudflare/certs/cloudflare-containers-ca.crt`). `entrypoint.sh` installs this CA into the system trust store (`update-ca-certificates`, which covers `curl`/`openssl`) and **persists** `NODE_EXTRA_CA_CERTS` and `REQUESTS_CA_BUNDLE` exports into `.bashrc` so the Node- and Python-based agent runtimes also trust the intercepted connections.

The exports are written to `.bashrc` — not merely exported in the entrypoint process — because the agents run in PTYs the terminal server spawns later; those shells source `.bashrc` and do not inherit the entrypoint's environment, so a process-only export would never reach them. Without this step the agents' TLS clients reject the platform-presented certificate and LLM calls fail with an opaque connection error before the request ever reaches the interceptor (`curl` keeps working via the system store, which masks the cause).

The CA is mounted by the platform at runtime — it is not embedded in the image — so a missing CA file (`$CF_CA_SRC not found`) is logged as a WARNING and LLM calls fail at the TLS layer rather than silently succeeding with an unverified cert.
...
**Per-user scoped R2 tokens:** Each container receives a scoped R2 API token restricted to its owner's bucket. Authentication resolves that bucket through a strongly consistent bucket-keyed Durable Object claim; known legacy ownership is preserved, later normalized-name collisions use a digest-suffixed v2 identity, and ambiguous legacy collisions are denied before any storage route can select a bucket. Tokens are created on first login via `getOrCreateScopedR2Token()` in `r2-admin.ts` (called from `lifecycle-init.ts`), which calls `POST /accounts/{accountId}/tokens` with a bucket-specific Object Read + Write policy. Tokens are cached in KV as `r2token:{email}` (encrypted via AES-256-GCM when `ENCRYPTION_KEY` is set) and revoked on user deletion via `deleteScopedR2Token()`. This requires the `API Tokens: Edit` permission on the deploy token.
...
Strict Gateway Egress ([REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress)) is an optional, enterprise-gated, default-OFF setup-wizard toggle that makes the customer's Cloudflare (Zero Trust) Gateway a **mandatory boundary** for the container's direct-internet HTTP/HTTPS egress. Its security properties:

**Transparent proxy — no identity stamping.** The catch-all `EgressController` (wired as `interceptOutboundHttps('*', controller)` for every host not owned by the LLM/GitHub interceptors) is deliberately **not** an identity-stamping interceptor. Unlike `LlmInterceptor`/`GitHubInterceptor` — which strip the container placeholder and re-stamp the real gateway/GitHub credential — the `EgressController`, **with a single exception (own-account R2, re-signed with the worker-held key — see "R2 key containment" below)**, adds no `Authorization`, `cf-aig-*`, or identity header and preserves the caller's `authorization`/`cookie` on the request and `set-cookie` on the response (stripping only the eight RFC 7230 hop-by-hop headers).

On the transparent path it carries no secret; its only effect is to force the traffic onto the Gateway hop. This keeps the proxy from becoming a new credential-injection surface.

**Account-scoped exemption — only direct-internet egress is policed.** The Gateway boundary polices the workload's reach to the *outside world*, not codeflare's own Cloudflare backends for this deployment's account.

The `EgressController` exempts only THIS deployment's own-account destinations via `isAccountScopedDestination(url, accountId)` (`src/lib/controller-egress.ts`), checked **before** the `env.EGRESS` guard so the exemption holds even when the binding is unbound: own-account R2 (`<accountId>.r2.cloudflarestorage.com` + the `<bucket>.<accountId>.…` vhost form, used by rclone for vault/workspace bisync) and the own-account CF API path (`api.cloudflare.com/client/v4/accounts/<accountId>/...`) egress **direct** — a dormant fallback now: `api.cloudflare.com` Browser Rendering is claimed by the per-host `CloudflareBrowserInterceptor` ([REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), which takes precedence over this catch-all and can only carry the container's non-secret placeholder.

The account id is resolved once by the DO (`getR2Config`) and passed to the controller via `ctx.props.accountId`; an absent/empty id exempts nothing (fail-secure → all egress Gateway-inspected). Any OTHER account's R2/CF host rides `env.EGRESS`/the Gateway like any direct-internet egress, closing the cross-account channel. The own-account exemptions carry codeflare-managed credentials to Cloudflare-owned hosts and have their own audit trail (R2 access logs, AI Gateway analytics), so policing them at the customer's Gateway adds no boundary value. The `LlmInterceptor` (AI Gateway upstream) **always** egresses direct and never swaps to `env.EGRESS`;

GitHub (`github.com`/`api.github.com`, via `GitHubInterceptor`) is **external**, so it still rides `env.EGRESS`/the Gateway when strict. **WebSocket upgrades** that reach this catch-all are proxied by **bridging a fresh `WebSocketPair`** to the upstream socket (the original upgrade request is forwarded verbatim, frames/close/error forwarded both ways); returning the upstream `101` as-is left the socket stalled (`canceled`). Browser-run's Chrome DevTools CDP WS (`…/browser-rendering/devtools/…`) does NOT reach here — it is claimed and bridged by the per-host `CloudflareBrowserInterceptor` ([REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), which injects the real token worker-side.

See [AD86](../decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network) and [AD87](../decisions/README.md#ad87-egresscontroller-re-signs-own-account-r2-container-holds-a-placeholder-key-bridges-websocket-upgrades-and-resolves-strict-via-props).

**R2 key containment — the real R2 key never enters the container (strict only).** When strict is active, `buildEnvVars` emits a non-secret placeholder R2 access key/secret into the container (`ENTERPRISE_R2_KEY_PLACEHOLDER`, the canonical `codeflare-enterprise` placeholder) instead of the real key. rclone runs in signed mode with the placeholder; for own-account R2 the `EgressController` **strips the placeholder `Authorization` and re-signs** the request with the worker-held R2 key (`createR2Client`/aws4fetch) at the R2 boundary — reusing the request's existing `x-amz-content-sha256` so the body streams through unbuffered and signing every present header so SSE-C `x-amz-*` headers are preserved.

So a prompt-injected agent that reads `rclone.conf` (or the container env) obtains only a non-secret placeholder; R2 access is gated by the worker-held key the agent cannot reach. This closes the key-exfil gap the account-scoped R2 exemption would otherwise leave (own-account R2 egresses direct, so its DLP is not Gateway-visible — but now the *key* is not in the sandbox to exfil). Non-enterprise / strict-off sessions keep the real key in the container (rclone connects to R2 directly), byte-identical to today.

See [AD87](../decisions/README.md#ad87-egresscontroller-re-signs-own-account-r2-container-holds-a-placeholder-key-bridges-websocket-upgrades-and-resolves-strict-via-props) and [REQ-ENTERPRISE-023](../../sdd/spec/enterprise-mode.md#req-enterprise-023-strict-gateway-egress-controller-transport) AC1/AC4.

**Residual surface — account-scoped, cross-account closed.** The exemption is bound to this deployment's own Cloudflare account, so the cross-account channel is **closed**: another account's R2 (`<other>.r2.cloudflarestorage.com`) or CF API path (`/accounts/<other>/...`) is no longer exempt and rides `env.EGRESS` → the Gateway, inspected like any direct-internet egress (e.g. a `PUT` to an attacker-owned bucket in a *different* account is now Gateway-visible). What remains is bounded and own-account only: traffic to *this deployment's own* R2 and account-scoped CF API egresses direct, so the customer's Gateway DLP does not see it.

That is by design ([AD86](../decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)) — these are codeflare's own control-plane backends, reached with codeflare-managed credentials to the customer's own account, each carrying its own audit trail (R2 access logs, AI Gateway analytics), and browser-run itself runs *in* Cloudflare's Browser Rendering where the account's Gateway policies/inspection/DLP apply by design.

So the residual is **not an un-gatewayed channel to arbitrary destinations**; it is potential DLP evasion of the customer's own Gateway for traffic to the customer's own account, which the account's own R2/AI-Gateway/Browser-Rendering audit already records. The exposure is further bounded by the `EGRESS` binding being unbound today (the whole feature is dormant). Operators enabling strict egress for DLP should treat the own-account control-plane backends as **outside the Gateway boundary by design**.

**Mandatory Gateway boundary + fail-closed.** When the toggle is ON the single upstream `fetch` on every direct-internet path is `env.EGRESS.fetch` (the Workers VPC binding → Cloudflare Gateway); there is **no fallback to global `fetch`** for direct-internet egress. When `env.EGRESS` is unbound — e.g. a non-enterprise deploy, where the `[[vpc_networks]]` `EGRESS` binding (enterprise-only, injected by `deploy.yml` when `ENTERPRISE_MODE=active`) is absent — the `EgressController` (on direct-internet hosts, incl. any other account's Cloudflare host) and the `GitHubInterceptor` return `503 EGRESS_UNAVAILABLE` and send nothing, while this account's own-account destinations above keep egressing direct.

Failing closed is the entire security point: a misconfiguration can never silently leak direct (un-gatewayed) egress to the outside world. The dormant state (toggle OFF + binding unbound) is inert, which is why shipping the feature OFF is safe.
...
**Bundled Cloudflare skills retrieve docs from `developers.cloudflare.com`.** The advanced-seed Cloudflare skills ([REQ-AGENT-075](../../sdd/spec/agents.md#req-agent-075-cloudflare-platform-skills-bundled-into-the-advanced-seed)) are retrieval-first: rather than shipping an offline doc mirror, they fetch live docs via WebFetch of `https://developers.cloudflare.com/...`. That host is **external**, so under strict Gateway egress it rides `env.EGRESS` → the customer's Gateway like any direct-internet egress. Operators enabling strict egress should **allowlist `developers.cloudflare.com`** in their Gateway policy so the skills' doc retrieval works; the upstream remote-MCP config (`docs.mcp.cloudflare.com` et al.) is deliberately not bundled (strict-egress-blocked + interactive-OAuth incompatible), so no MCP allowlisting is needed.
...
**Off-state parity.** When the toggle is OFF or the deployment is non-enterprise, the catch-all is never wired, the GitHub interceptor transport swap is inert (global `fetch`), and no toggle KV read/write occurs outside enterprise — the egress path is byte-identical to today. See [AD85](../decisions/README.md#ad85-controller-mediated-cloudflare-gateway-egress-as-a-mandatory-web-boundary-wizard-toggled-default-off) and the [Architecture — Strict Gateway Egress](architecture.md#strict-gateway-egress) data flow.

**KV-startup edge case.** `hasStrictGatewayEgress` reads the toggle from KV at the container-start seam. If KV is transiently unavailable at that moment while strict egress is ON, the read defaults to `false` (OFF) so the container boots with direct egress rather than failing to start — a deliberate availability-over-strictness trade-off, bounded because the binding is unbound today (egress is dormant regardless) and the KV error's effect is limited to the container-start wiring decision (the catch-all is simply not wired for that container's lifetime). When the `EGRESS` binding is live, operators who require fail-closed-on-KV-error at startup should monitor KV availability; the trade-off is revisitable if it proves material.
...
### Browser IDE response headers

**Threat:** The editor's inline scripts and styles are incompatible with the default deny-all CSP, while relaxing framing or unrelated error responses would expose more surface than the proxied editor requires.

**Mitigation:** Proxied code-server responses under `/api/vscode/<sessionId>/*` receive `Content-Security-Policy: frame-ancestors 'self'` and `X-Frame-Options: SAMEORIGIN`. The pre-Hono, pre-auth `INVALID_SESSION` response retains the full default header set. <!-- @impl: src/index.ts::withSecurityHeaders -->

**Verification:** [Early-return security tests](../../src/__tests__/security/early-return-security.test.ts) cover same-origin framing on Browser IDE responses and the default policy on route-validation errors.

**Implements:** [REQ-SEC-008](../../sdd/spec/security.md#req-sec-008-security-headers-on-every-response) AC4, [REQ-IDE-001](../../sdd/spec/browser-ide.md#req-ide-001-per-session-browser-ide-served-through-the-worker-proxy) AC4.

### code-server supply-chain and reverse-proxy boundary

**Threat:** A substituted editor artifact, directly reachable unauthenticated listener, spoofed forwarding metadata, mismatched session path, public workspace selector, or cross-origin browser request could expose one session's workspace or weaken the established Worker-to-container boundary.

**Mitigation:** The image verifies the exact MIT-licensed code-server 4.132.0 archive SHA-256, package/product commit, and embedded Code OSS version, then removes the bundled GitHub Copilot extension without patching code-server or Code OSS application source; the only bundled dependency replacement lifts the root `js-yaml` within code-server's declared semver range. Deployment image smoke proves the Copilot directory is absent. It carries no OpenVSCode scanner exception forward. Shadow Pins derives the recorded VS Code source gitlink from the immutable release tag. code-server binds only to `127.0.0.1:13337` with its redundant password layer disabled behind Codeflare's deployment-aware authentication, allowed-Origin validation, session ownership, the Durable Object selector, and container-bearer validation. <!-- @impl: entrypoint.sh::_openvscode_launch_once --> <!-- @impl: src/lib/access.ts::getUserFromRequest --> <!-- @impl: Dockerfile::CODE_SERVER_VERSION = "4.132.0" -->

The Worker removes client `Forwarded`/`X-Forwarded-*` values, supplies canonical host/protocol identity, preserves only an allowlisted caller Origin, and rejects public `folder`, `workspace`, and `ew` selectors before the container boundary. <!-- @impl: src/routes/vscode.ts::handleVscodeRequest -->

The host independently rejects the same decoded selector keys before opening an HTTP or WebSocket upstream, validates the bearer and exact `/api/vscode/<SESSION_ID>` prefix, strips that prefix, injects `/home/user/workspace` only on the private root request, removes selectors from redirects, removes injected authorization and hop headers, and lets code-server compare the caller Origin with the canonical external Host. No wildcard trusted-origin bypass is configured. <!-- @impl: host/src/request-router.ts::createRequestHandler --> <!-- @impl: host/src/upgrade-dispatcher.ts::createUpgradeDispatcher --> <!-- @impl: host/src/vscode-proxy.ts::vscodeUpstreamPath -->

Code OSS reads workspace selection from the browser location or its workbench configuration, not the private request URL. The host therefore overwrites the successful root document's `folderUri` with the fixed remote workspace while preserving unrelated configuration. Exact marker count, JSON, authority, compression, and size checks fail closed; no returned HTML or configuration is logged. Non-root responses remain unbuffered. This is navigation confinement, not a filesystem sandbox; terminals, trusted extensions, and agents retain root-container access. <!-- @impl: host/src/vscode-proxy.ts::projectVscodeWorkbenchWorkspace --> <!-- @impl: host/src/request-router.ts::createRequestHandler -->

**Verification:** [Worker route tests](../../src/__tests__/routes/vscode-auth-chain.test.ts), [host HTTP proxy tests](../../host/__tests__/request-router.test.js), [host WebSocket proxy tests](../../host/__tests__/browser-ide-upgrade.test.js), [launch tests](../../host/__tests__/entrypoint-openvscode.test.js), and the [deployment image smoke](../../scripts/ci/smoke-openvscode-sidebar-image.mjs) cover authentication, identity, prefix, runtime, and loopback contracts. The Docker build verifies the replacement package's committed SHA-512 digest and exact installed version before deployment smoke runs. The deployment image workflow independently runs Trivy with the documented `ignore-unfixed` policy; any code-server exception requires fresh evidence and explicit review. <!-- @impl: Dockerfile::JS_YAML_SHA512 -->

**Implements:** [REQ-IDE-001](../../sdd/spec/browser-ide.md#req-ide-001-per-session-browser-ide-served-through-the-worker-proxy), [REQ-IDE-002](../../sdd/spec/browser-ide.md#req-ide-002-session-isolated-ide-not-bucket-stable), [REQ-IDE-012](../../sdd/spec/browser-ide.md#req-ide-012-fixed-clean-browser-ide-workspace-selection), [REQ-IDE-015](../../sdd/spec/browser-ide.md#req-ide-015-fixed-workspace-projection-and-clean-browser-ide-url), [REQ-SEC-011](../../sdd/spec/security.md#req-sec-011-container-image-scanned-for-cves-before-deploy), [REQ-SEC-022](../../sdd/spec/security.md#req-sec-022-container-proxy-bearer-validation), [AD119](../decisions/README.md#ad119-replace-openvscode-with-pinned-code-server-behind-the-existing-session-proxy).
...
**Mitigation:** Tool execution is deliberately unrestricted by user direction. Closed inventories, bounded untrusted context, one isolated IDE-owned Pi process with failure-triggered replacement, separate terminal state, isolated official-Claude configuration, exact package checks, loopback token authentication, and bounded generation cleanup protect unrelated process, credential, and supply-chain boundaries. <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::NativePiRuntime --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput --> <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::sidebarApproval --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOfficialClaudeIde --> <!-- @impl: openvscode/agent-sidebar/src/process-generation.ts::reapSidebarGeneration -->

**Verification:** [Native context tests](../../openvscode/agent-sidebar/test/native-chat.test.ts), [Explorer review tests](../../openvscode/agent-sidebar/test/activation.test.ts), [unrestricted-action tests](../../openvscode/agent-sidebar/test/vscode-approval-host.test.ts), [process-generation tests](../../openvscode/agent-sidebar/test/process-generation.test.ts), [official Claude tests](../../openvscode/claude/test), [host cleanup tests](../../host/__tests__/entrypoint-openvscode.test.js), [packaging identity tests](../../openvscode/agent-sidebar/test/packaging.test.ts), and [deployment image smoke](../../scripts/ci/smoke-openvscode-sidebar-image.mjs) validate the packaged boundary.

**Implements:** [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent), [REQ-IDE-006](../../sdd/spec/browser-ide.md#req-ide-006-ide-conversation-context-and-credential-isolation), [REQ-IDE-007](../../sdd/spec/browser-ide.md#req-ide-007-ide-guarded-approval), [REQ-IDE-008](../../sdd/spec/browser-ide.md#req-ide-008-ide-agent-process-lifecycle), [REQ-IDE-009](../../sdd/spec/browser-ide.md#req-ide-009-frictionless-workspace-open-for-every-ide-agent), [REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility), [REQ-IDE-011](../../sdd/spec/browser-ide.md#req-ide-011-file-review-with-codeflare), [REQ-IDE-013](../../sdd/spec/browser-ide.md#req-ide-013-account-backed-code-review-suppression), [REQ-IDE-014](../../sdd/spec/browser-ide.md#req-ide-014-active-editor-review-with-codeflare), [REQ-IDE-017](../../sdd/spec/browser-ide.md#req-ide-017-unsupported-ide-inventory-runtime-metadata), [REQ-IDE-019](../../sdd/spec/browser-ide.md#req-ide-019-codeflare-eligibility-in-editor-inline-chat), [REQ-IDE-020](../../sdd/spec/browser-ide.md#req-ide-020-unrestricted-pi-editor-request-execution), [REQ-IDE-021](../../sdd/spec/browser-ide.md#req-ide-021-account-free-browser-ide-chrome), [REQ-IDE-022](../../sdd/spec/browser-ide.md#req-ide-022-native-pi-blocking-ui-protocol)

The Pi inventory contains one Codeflare-owned workspace extension; the Claude inventory contains Anthropic's exact official package; the unsupported inventory contains no extension. It begins empty, and code-server may create only a regular `extensions.json` registry file containing `[]` after startup. The build validates source directories, publisher/name/version/entry point, executable platform binary, symbolic links, and retained VSIX files before atomically making both extension trees root-owned and immutable. The deployment image smoke rejects any unsupported extension or unknown inventory entry. The owner accepts the proprietary extension's license ambiguity under [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration). <!-- @impl: openvscode/agent-sidebar/src/package-extension.ts::stageSidebarExtension --> <!-- @impl: scripts/ci/smoke-openvscode-sidebar-image.mjs::main -->

Pi runs through code-server's native Chat participant API, not a custom webview. The extension retains two local compatibility providers for the native participant's separate fallback and picker contracts; the hidden fallback and selectable account-free panel/editor model are both unable to generate. GitHub Actions image smoke verifies its packaged registration and fail-closed generation contract under [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent), [REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility), [REQ-IDE-019](../../sdd/spec/browser-ide.md#req-ide-019-codeflare-eligibility-in-editor-inline-chat), [REQ-IDE-020](../../sdd/spec/browser-ide.md#req-ide-020-unrestricted-pi-editor-request-execution), and [REQ-IDE-021](../../sdd/spec/browser-ide.md#req-ide-021-account-free-browser-ide-chrome). <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_FALLBACK_PROVIDER --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::HOST_VISIBLE_PROVIDER -->

A hidden, non-selectable `copilot` fallback exists only for the pinned extension host's absent-request-model lookup. The distinct `codeflare` vendor keeps the visible model outside Code OSS's Copilot entitlement and sign-in path; tool-calling capability is only pinned Code OSS filter metadata. Neither adapter introduces a GitHub provider, authentication, or inference path. The participant does not read `request.model`, invoke either model for inference, or access `vscode.authentication`; it calls the fixed local RPC binary directly. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate -->

The visible participant/provider label is **Codeflare** while stable private IDs remain unchanged, and immediate owned-extension activation registers its provider before initial picker resolution. Ephemeral web-profile preparation preserves unrelated state and adds only `chat.statusBarEntry` to Code OSS's supported hidden-status list, leaving authentication APIs and agent credentials intact. Shared web-profile state hides the left-side Accounts control without removing those APIs. <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::writeOpenVscodeProfileState --> On startup the inventory marks generic Chat setup complete, hiding Code OSS's account-backed **Code Review** action without disabling native Chat or **Review with Codeflare**. It disables only Code OSS's duplicate `~/.claude/agents` personal source while retaining `~/.copilot/agents`; native Pi's own agent discovery is unchanged. <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings -->

Explorer and editor-context review canonicalize the selected or active file against `/home/user/workspace` and reject symbolic-link aliases or escapes before opening Chat. Editor collection applies the same boundary to file URIs, rejects malformed ranges, bounds active content, selection, open files, diagnostics, references, history, and total UTF-8 bytes, and marks the result as untrusted workspace data. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::canonicalWorkspaceFilePath --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput -->

Panel and editor share one hidden IDE transcript by explicit product choice, but never attach to terminal Pi. Warm turns do not replay host history; cold or replacement processes receive only the requesting surface's bounded visible history. Cancellation or failure retires that generation before replacement. <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::NativePiRuntime --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::buildNativePiPrompt -->

Sidebar Pi leaves the standard built-in tools unrestricted instead of registering guarded replacements. The extension host returns approval for any confirmation request without opening an editor document or modal. It separately validates and bounds documented `select` and `input` RPC dialogs, honors their bounded timeouts, correlates their value or cancellation response, and rejects malformed or unknown blocking UI methods. There is no path, stale-content, command-content, unknown-tool, or MCP approval gate. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::sidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost -->

Anthropic's official extension runs its documented IDE MCP server on `127.0.0.1` at a random port with unencrypted WebSocket transport. Every activation creates a fresh token in `$CLAUDE_CONFIG_DIR/ide/<port>.lock`; the config directory is mode 0700 and the lock file mode 0600. Codeflare adds no relay, public listener, or routable port. The owner explicitly accepts this local authenticated MCP because it is the mechanism for selections, native diffs, and diagnostics ([REQ-IDE-006](../../sdd/spec/browser-ide.md#req-ide-006-ide-conversation-context-and-credential-isolation), [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration)). Codeflare config requests unrestricted `bypassPermissions` for the official extension; loopback token authentication protects only the local transport. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings -->

Claude's external managed settings select unrestricted `bypassPermissions` mode, allow dangerous permission skipping, install no ask rules or permission hook, and disable Remote Control, IDE auto-install, updates, telemetry, and code-server's unrelated native Chat/Copilot setup. code-server refuses to launch when those settings cannot be restored.

The temporary config projection links approved credentials/configuration but excludes terminal projects, history, state, caches, logs, and unknown paths. `disableLoginPrompt` prevents a second Anthropic login while inherited approved routing and linked credentials remain available. The Accounts control is hidden while authentication APIs remain separate; no Codeflare-owned path requests, bridges, exports, persists, or syncs credentials. Credential bytes are never copied into generated files. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareSidebarConfig -->

No IDE-agent tool call is inspected, approval-gated, sandboxed, or confined to an Inline Chat selection. Direct Pi writes and commands are not host text-edit transactions, and Inline Chat Keep/Undo is not promised to reverse workspace mutations or external side effects. Executable user hooks, plugins, the proprietary extension itself, and other trusted root-container code are also unsandboxed. [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration) and [REQ-IDE-007](../../sdd/spec/browser-ide.md#req-ide-007-ide-guarded-approval) define that residual boundary.

Every IDE session seeds code-server User settings that disable workspace trust and ignore extension recommendations for all agent kinds, so the workspace opens without a trust or recommended-extensions prompt. Live storage remains under `/tmp`; after code-server is reaped, only theme values, string-valued `keyboard.layout`, and key-specific Explorer/open-file resource schemas enter a bounded atomic snapshot; other User settings, unknown fields, and opaque strings are rejected ([REQ-IDE-016](../../sdd/spec/browser-ide.md#req-ide-016-ui-state-capture-and-restore-ordering)).

Raw databases, workspace/global extension storage, SecretStorage, Accounts authentication, chat history, logs, WAL, and SHM never enter that snapshot. On restore, managed Codeflare and Claude settings override preferences before launch. A settings-preparation failure refuses the launch. <!-- @impl: scripts/browser-ide-ui-state.py::capture --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildBaseOpenVscodeSettings --> <!-- @impl: entrypoint.sh::_openvscode_prepare_agent -->

Disabling workspace trust removes VS Code's own gate on the untrusted repository input noted above; it adds no new exposure because IDE agents already run unrestricted and the container — not the trust prompt — is the residual boundary. <!-- @impl: openvscode/claude/managed-settings.mjs::buildBaseOpenVscodeSettings -->

**Cloudflare Web Analytics CSP allowance:** The default CSP permits the Web Analytics beacon with two narrow additions in `src/index.ts`: `static.cloudflareinsights.com` in `script-src` (the beacon loader `beacon.min.js`) and `cloudflareinsights.com` in `connect-src` (the beacon's telemetry POST endpoint). The beacon is injected as a manually-authored `<script src=...>` tag (gated on `PUBLIC_CF_BEACON_TOKEN`, see [configuration.md](./configuration.md#onboarding-variables-and-secrets)) specifically so the CSP does not have to be weakened with `'unsafe-inline'`: a script element with an allowlisted host is the strict-CSP-compatible alternative to Cloudflare's auto-injected inline snippet. The landing's head-only design-ready gate is the sole inline executable exception: `script-src` carries its exact SHA-256 hash from the shared `design-ready.ts` source, with a behavioral integrity test preventing script/hash drift.
...
File download responses set `Content-Disposition: attachment` with sanitized filenames by default — special characters are stripped before encoding and filenames truncated to prevent header injection ([REQ-SEC-013](../../sdd/spec/security.md#req-sec-013-content-disposition-hardening-on-downloads)). When the caller passes `?disposition=inline`, the endpoint serves the file for in-browser viewing.
...
1. **Session-stopped gate ([REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC1):** stopped sessions are rejected before the rate-limit counter.

   WebSocket upgrade requests for sessions whose KV status is `stopped` receive close code 4503 (`container-stopped`). A browser reconnect storm against a hibernated or crashed container does not consume the user's 30-connection budget.

2. **Warm-up gate ([REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC2):** warming containers are rejected before the rate-limit counter.

   After the stopped check, the worker peeks the container `/health` endpoint. If the container is up but still initializing (port 8080 bound but R2 sync and `.bashrc` autostart writes not yet complete), the host sets `terminalServiceReady=false` in the health response.

   The worker rejects the upgrade with close code 1013 (`container-warming-up`). This prevents reconnect storms during the ~10s cold-start window from consuming rate-limit budget and from spawning PTYs against pre-sync state (bare bash, no agent autostart).

   The `/health` probe is fail-open: any probe error or missing `terminalServiceReady` field falls through to the normal rate-limit path. The host server applies the same gate directly at the WebSocket accept layer ([REQ-SESSION-015](../../sdd/spec/session-lifecycle.md#req-session-015-container-port-readiness-gating-with-pre-warm-pre-condition) AC2).
...
Terminal OSC payloads are hostile and do not prove agent provenance. The browser registers the OSC 777 handler only for terminal tab 1 of a session selected as Pi or Claude. That trusted selection supplies the displayed agent title; payload titles cannot impersonate another producer. The parser accepts only `notify` with non-empty plain text, rejects C0/C1 and Unicode-format controls, and bounds composed native titles to 64 UTF-8 bytes, bodies to 256 bytes, and session names to 64 bytes. Payloads never become markup, URLs, commands, action buttons, logs, or persisted history.

Notification permission is requested only from the Settings user gesture and remains browser-owned; terminal output cannot trigger the prompt. Delivery uses an existing same-origin service-worker registration and creates no push subscription, backend call, queue, storage, retry, poll, container wake, or cross-session relay. The worker has no fetch/cache/push/sync handlers. Clicks may focus only the existing client whose full URL matches the notification's recorded originating-session URL; they never open or substitute another client or URL.

Pi's repository-owned OSC extension emits only fixed input-needed and settled-completion text, never question or model content, and is disabled in RPC mode so terminal bytes cannot corrupt code-server native Chat's strict JSONL transport. The official Claude IDE package remains unmodified and retains its upstream in-product notifications. The implementation is present, while REQ-TERM-023 and REQ-IDE-018 remain Partial pending deployed desktop/mobile, permission, focus, lifetime, and native-panel verification.
...
**Mitigation:** Every fresh-image deploy invokes Trivy through reusable `container-image.yml`. Identical-input deploys reuse the scanned image, bounded by the seven-day weekly salt in [AD112](../decisions/README.md#ad112-ci-runs-as-parallel-path-filtered-lanes-and-deploys-reuse-content-addressed-container-images). A retained digest is reused only after provenance verifies against the reusable workflow; failure triggers a fresh build and scan, while authorized deployment binds the verified digest. `ignore-unfixed: true` limits the gate to vulnerabilities with an available fix; reviewed exceptions remain scanner-identity-bound and fail closed on drift. The validator blocks any unreviewed fixable HIGH/CRITICAL p7zip vulnerability reported by Trivy.

Image assembly also replaces code-server's semver-compatible root `js-yaml` 4.3.0 with integrity-pinned 4.3.1, a defence-in-depth pin one patch above the vendored copy. <!-- @impl: Dockerfile::JS_YAML_SHA512 --> <!-- @impl: scripts/ci/validate-trivy-result.mjs::validateTrivyResult --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (rejects p7zip RCE findings without a reviewed exception) -->

**Verification:** The reusable workflow blocks image publication before a successful scan. Successful validation logs the scanner-provided package path and PURL for every accepted occurrence, using `<unavailable>` when either field is absent, so reviewed exceptions can be audited and later bound to exact package identities. [Trivy gate tests](../../host/__tests__/trivy-exception-gate.test.js) cover scan ordering, unfixed handling, bounded exceptions, and the emitted occurrence evidence. [Retained-image provenance tests](../../host/__tests__/container-image-reuse-provenance.test.js) cover provenance authorization, fresh-stage gating, and digest-bound deployment. The Docker build separately verifies the replacement `js-yaml` tarball's committed SHA-512 digest and exact installed version before the image reaches Trivy. <!-- @impl: Dockerfile::JS_YAML_SHA512 -->
...
CVE-2026-56852 remains narrower than a global ID suppression. Integration deployment run `30612952117` for head `82244a1d117194227c0082b9555f3654f903fbd2` retired the gh occurrence when v2.97.0 stopped emitting it. Lazygit 0.64.0 then moved to patched [`x/text` 0.40.0](https://github.com/jesseduffield/lazygit/blob/v0.64.0/go.mod), retiring the remaining occurrence. Neither tuple is accepted now: any CVE-2026-56852 recurrence in gh, lazygit, or another target fails as an unexpected HIGH/CRITICAL finding. The CVE is not in the global ignore file.

Integration runs [`30893082736`](https://github.com/nikolanovoselec/codeflare/actions/runs/30893082736) and [`30893082817`](https://github.com/nikolanovoselec/codeflare/actions/runs/30893082817) at head `8a745b736aa948e9c88351ce5b2bcd5098ae69e2` produced the same complete six-finding new Node.js set before publication. CVE-2026-69192 appeared as one `ip-address` 10.1.0 declaration and two 10.2.0 declarations, although every affected committed runtime lock installs patched 10.4.0. Code-server 4.132.0 then lifted its own server tree to patched 10.4.0 and retired that occurrence; only the bundled VS Code tree still declares 10.2.0, so a server-tree recurrence now fails closed as an unexpected finding. <!-- @impl: scripts/ci/validate-trivy-result.mjs::validateTrivyResult -->

CVE-2026-13697 appeared as two upstream Pi declarations of undici 8.5.0, while both Pi runtime locks installed patched 8.9.0, plus one undici 7.28.0 occurrence in Node/npm image tooling. Codeflare does not enable undici's shared cache interceptor, and npm use remains inside the authenticated user's [single-session container](../../sdd/spec/session-lifecycle.md#req-session-002-one-container-per-session-isolation). <!-- @impl: preseed/npm-tools/package-lock.json::node_modules/@earendil-works/pi-coding-agent/node_modules/undici --> <!-- @impl: preseed/agents/pi/package-lock.json::node_modules/@earendil-works/pi-coding-agent/node_modules/undici --> <!-- @impl: scripts/ci/validate-trivy-result.mjs::REVIEWED_FINDINGS --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (rejects retired Pi findings after the runtime upgrade) -->

Pi 0.84 subsequently removed both 8.5.0 declarations and both `brace-expansion` 5.0.7 declarations from the scanner result. Integration runs `31740630696` and `31740633639` supplied the missing-tuple evidence, so those four exceptions are retired and any recurrence now fails closed. <!-- @impl: scripts/ci/validate-trivy-result.mjs::REVIEWED_FINDINGS --> <!-- @test: host/__tests__/trivy-exception-gate.test.js (rejects retired Pi findings after the runtime upgrade) -->

Standard integration run `30896944558` recorded the scanner path and PURL for every Node.js finding; the now-retired lazygit occurrence historically supplied a PURL but no package path. The gate binds each remaining reviewed occurrence to its observed identity, version, and multiplicity until upstream package metadata or the pinned image advances. Movement between npm, code-server, shared Pi tooling, or Pi's dedicated runtime fails closed, and none of these IDs is globally suppressed.
...
- [REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent) - Native agent ownership and availability boundary
...
**Migration is lossless, gated, and verified.** Switching regimes re-encrypts each object in place via a same-key server-side `CopyObject` with `MetadataDirective=REPLACE` (object bytes never leave R2, metadata preserved) rather than deleting and recreating the bucket, so no user content is lost.

A per-bucket state object (`r2-regime:<bucket>`) keeps reads correct during the rollout window. While a bucket migrates every R2 writer is gated `409` and running containers are drained; a best-effort drain failure leaves a stray-write window of ~tens of seconds, caught by the post-migrate verification rescan and read self-heal.

The regime commits only after a full verification HEAD-scan. A partial/crashed pass never leaves the bucket unreadable, and a stray cross-regime object self-heals on read ([AD91](../decisions/README.md#ad91-governed-mode-migration-is-a-verified-gated-chunked-state-machine-replace-copy-not-a-boolean-marker-lazy-reconcile), supersedes [AD89](../decisions/README.md#ad89-governed-mode-deployment-wide-r2-sse-c-disable-via-a-kv-toggle-with-lossless-in-place-re-encrypt-migration)).
