...
The matching raw 64-hex public key is non-secret verification material. Codeflare Setup stores only that public key and an AES-256-GCM-encrypted repository-scoped read PAT. A replacement key selects a new fingerprinted cache namespace only after a current release verifies with it; failure preserves the prior trust boundary. Neither value enters a user bucket or container. <!-- @impl: src/lib/remote-curation.ts::configureManagedEnvironment -->
