...
The visible participant/provider label is **Codeflare** while stable private IDs remain unchanged. Ephemeral profile preparation preserves unrelated state and adds only `chat.statusBarEntry` to Code OSS's supported hidden-status list, leaving authentication APIs and agent credentials intact. On startup the inventory marks generic Chat setup complete, hiding Code OSS's account-backed **Code Review** action without disabling native Chat or **Review with Codeflare**. It disables only Code OSS's duplicate `~/.claude/agents` personal source while retaining `~/.copilot/agents`; native Pi's own agent discovery is unchanged. <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings -->
...
The temporary config projection links approved credentials/configuration but excludes terminal projects, history, state, caches, logs, and unknown paths. `disableLoginPrompt` prevents a second Anthropic login while inherited approved routing and linked credentials remain available. Generic upstream Accounts authentication remains separate; no Codeflare-owned path requests, bridges, exports, persists, or syncs its credentials. Credential bytes are never copied into generated files. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareSidebarConfig -->
