...
**Platform-native exemption — only direct-internet egress is policed.** The Gateway boundary polices the workload's reach to the *outside world*, not codeflare's own Cloudflare backends. The `EgressController` exempts platform-native hosts via `isPlatformNativeHost()` (`src/lib/controller-egress.ts`), checked **before** the `env.EGRESS` guard so the exemption holds even when the binding is unbound: R2 object storage (`*.r2.cloudflarestorage.com`, used by rclone for vault/workspace bisync), the AI Gateway (`api.cloudflare.com` REST + `gateway.ai.cloudflare.com` compat — the `LlmInterceptor` upstream, which now **always** egresses direct and no longer swaps to `env.EGRESS` under strict), and Browser Rendering (`api.cloudflare.com/client/v4/accounts/<id>/browser-rendering/...`) all egress **direct**. These carry codeflare-managed credentials to Cloudflare-owned hosts and have their own audit trail (R2 access logs, AI Gateway analytics), so policing them at the customer's Gateway adds no boundary value. GitHub (`github.com`/`api.github.com`, via `GitHubInterceptor`) is **external, not platform-native**, so it still rides `env.EGRESS`/the Gateway when strict. See [AD86](../decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network).

**Residual surface — the exemption is host-based, not account-scoped.** The platform-native match is purely by hostname, with no account binding: `*.r2.cloudflarestorage.com` matches *any* Cloudflare account's R2, `api.cloudflare.com` is the entire Cloudflare REST API for any account, and `gateway.ai.cloudflare.com` reaches any account's AI Gateway. So a *compromised* agent in strict mode retains an un-gatewayed channel to those Cloudflare-owned hosts — e.g. a `PUT` to an attacker-owned R2 bucket, or the CF API with an attacker-supplied token — that the customer's Gateway DLP never sees. This is an accepted, deliberate trade-off ([AD86](../decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)): `api.cloudflare.com` **must** stay exempt for browser-run (the account is in the URL path, not the host, so it cannot be host-scoped), and it is a strictly more capable channel than R2 — so account-scoping R2 alone would add plumbing without actually closing the boundary. The exposure is bounded by codeflare-managed credentials, each host's own audit trail (R2 access logs, AI Gateway analytics), and the fact that the `EGRESS` binding is unbound today (the whole feature is dormant). Operators enabling strict egress for DLP should treat Cloudflare-owned platform hosts as **outside the boundary by design**.

**Mandatory Gateway boundary + fail-closed.** When the toggle is ON the single upstream `fetch` on every direct-internet path is `env.EGRESS.fetch` (the Workers VPC binding → Cloudflare Gateway); there is **no fallback to global `fetch`** for direct-internet egress. When `env.EGRESS` is unbound — e.g. a non-enterprise deploy, where the `[[vpc_networks]]` `EGRESS` binding (enterprise-only, injected by `deploy.yml` when `ENTERPRISE_MODE=active`) is absent — the `EgressController` (on direct-internet hosts) and the `GitHubInterceptor` return `503 EGRESS_UNAVAILABLE` and send nothing, while the platform-native hosts above keep egressing direct. Failing closed is the entire security point: a misconfiguration can never silently leak direct (un-gatewayed) egress to the outside world. The dormant state (toggle OFF + binding unbound) is inert, which is why shipping the feature OFF is safe.
