...
Integration runs `30893082736` and `30893082817` at head `8a745b736aa948e9c88351ce5b2bcd5098ae69e2` produced the same complete six-finding new Node.js set before publication. CVE-2026-69192 appeared as one `ip-address` 10.1.0 declaration and two 10.2.0 declarations, although every affected committed runtime lock installs patched 10.4.0. Code-server 4.132.0 then lifted its own server tree to patched 10.4.0 and retired that occurrence; only the bundled VS Code tree still declares 10.2.0, so a server-tree recurrence now fails closed as an unexpected finding.

CVE-2026-13697 appeared as two upstream Pi declarations of undici 8.5.0, while both Pi runtime locks installed patched 8.9.0, plus one undici 7.28.0 occurrence in Node/npm image tooling. Codeflare does not enable undici's shared cache interceptor, and npm use remains inside the authenticated user's single-tenant container.
