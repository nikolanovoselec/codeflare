...
**Mitigation:** The image verifies the exact MIT-licensed code-server 4.130.0 archive SHA-256, package/product commit, and embedded Code OSS version, then removes the bundled GitHub Copilot extension without patching code-server or Code OSS source; complete-image smoke proves the directory is absent. It carries no OpenVSCode scanner exception forward. Shadow Pins derives the recorded VS Code source gitlink from the immutable release tag. code-server binds only to `127.0.0.1:13337` with its redundant password layer disabled behind Cloudflare Access, allowed-Origin validation, session ownership, the Durable Object selector, and container-bearer validation. <!-- @impl: entrypoint.sh::_openvscode_launch_once --> <!-- @impl: Dockerfile::CODE_SERVER_VERSION = "4.130.0" -->
