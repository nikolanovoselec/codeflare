...
**Threat:** An IDE agent could read outside the workspace, treat untrusted file text as authority, import terminal history, invoke an unrelated account provider, bypass guarded tools, expose a local control channel, substitute a privileged extension package, or leave credential-bearing descendants running.

**Mitigation:** Closed inventories, canonical path confinement, bounded untrusted context, fresh Pi processes, host-owned Pi approval, isolated official-Claude configuration, exact package checks, loopback token authentication, and generation cleanup fail closed. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-native-chat.ts::collectNativePiPromptInput --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::buildNativePiPrompt --> <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareOfficialClaudeIde --> <!-- @impl: openvscode/agent-sidebar/src/process-generation.ts::reapSidebarGeneration -->

**Verification:** [Native context tests](../../openvscode/agent-sidebar/test/native-chat.test.ts), [approval tests](../../openvscode/agent-sidebar/test/approval-bridge.test.ts), [process-generation tests](../../openvscode/agent-sidebar/test/process-generation.test.ts), [official Claude tests](../../openvscode/claude/test), [host cleanup tests](../../host/__tests__/entrypoint-openvscode.test.js), and [complete-image smoke](../../scripts/ci/smoke-openvscode-sidebar-image.mjs) validate the packaged boundary.
...
Pi edit, write, and Bash calls remain replaced by the guarded tools in the fixed RPC child. The guard confines canonical targets, rejects unsafe links and non-regular files, serializes same-file windows, writes a mode-0600 expiring manifest, and rechecks the original hash before atomic replacement. The extension host validates ownership, mode, request-bound digest, schema, ID, nonce, and expiry, then auto-confirms edit, write, and Bash by tool name without opening editor documents. Unknown and MCP actions retain one modal; cancellation races it to correlated denial and late answers are ignored. <!-- @impl: preseed/agents/pi/extensions/sidebar-approval.ts::registerSidebarApproval --> <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost --> <!-- @impl: openvscode/agent-sidebar/src/pi/approval-bridge.ts::ApprovalBridge -->

Anthropic's official extension runs its documented IDE MCP server on `127.0.0.1` at a random port with unencrypted WebSocket transport. Every activation creates a fresh token in `$CLAUDE_CONFIG_DIR/ide/<port>.lock`; the config directory is mode 0700 and the lock file mode 0600. Codeflare adds no relay, public listener, or routable port. The owner explicitly accepts this local authenticated MCP because it is the mechanism for selections, native diffs, and diagnostics. Of the model-visible IDE tools, read-only `mcp__ide__getDiagnostics` is allowed and `mcp__ide__executeCode` still asks through both the Codeflare hook and Anthropic's native per-call confirmation.

Claude's external managed settings select Anthropic Manual (`"default"`) permission mode and disable bypass, automatic edits, Remote Control, IDE auto-install, updates, telemetry, and OpenVSCode's unrelated native Chat/Copilot setup. OpenVSCode refuses to launch when those settings cannot be restored. The temporary config projection links approved credentials/configuration but excludes terminal projects, history, state, caches, logs, and unknown paths. `disableLoginPrompt` prevents a second Anthropic login while inherited approved routing and linked credentials remain available. Generic upstream Accounts authentication remains separate; no Codeflare-owned path requests, bridges, exports, persists, or syncs its credentials. Credential bytes are never copied into generated files. <!-- @impl: openvscode/claude/managed-settings.mjs::buildManagedSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::prepareSidebarConfig -->

These controls govern agent tool calls. They do not inspect or sandbox auto-allowed Pi or Claude Bash and Task inputs, which can perform destructive or external actions; nor do they sandbox executable user hooks, plugins, the proprietary extension itself, or other trusted code in the root container. [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration) and [REQ-IDE-007](../../sdd/spec/browser-ide.md#req-ide-007-ide-guarded-approval) define that residual boundary.
...
- [REQ-IDE-007](../../sdd/spec/browser-ide.md#req-ide-007-ide-guarded-approval) - Native IDE approval authority
