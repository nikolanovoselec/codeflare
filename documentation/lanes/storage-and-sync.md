...
- **Bisync compares via server-modtime (AD88, all modes).**
    ...
    The required Rclone bookkeeping CI lane reproduces the original bug, exercises isolated S3 transfers, same-size edits and conflicts, and measures LIST/HEAD requests. A version or source-anchor mismatch fails closed: future rclone bumps require explicit patch revalidation. See [REQ-STOR-040](../../sdd/spec/storage.md#req-stor-040-per-side-sync-change-tracking).
