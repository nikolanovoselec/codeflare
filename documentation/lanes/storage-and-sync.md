...
A valid UUID filename and native schema allow the adapter to rank a candidate by transcript activity. Claude validates its session identity and supported 2.1 version; Pi validates its version-3 `session` header. Unsupported filenames or schemas remain deletion candidates but switch the whole agent to mtime fallback. The adapters scan bounded head and tail windows, so large transcripts are not loaded into memory. Malformed interior or trailing records are harmless when the header and latest timestamp remain recoverable. If one main candidate has an unknown shape or no recoverable timestamp, the whole agent falls back to deterministic `(mtime_ns, path)` ordering for that pass.

Mixing native and restored filesystem chronology would be worse than either policy, so it is deliberately forbidden. <!-- @impl: transcript-retention.mjs::parseClaudeTimestamp --> <!-- @impl: transcript-retention.mjs::parsePiTimestamp --> <!-- @impl: transcript-retention.mjs::readLatestTimestamp -->
