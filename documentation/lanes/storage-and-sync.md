...
- **Bisync empty listing**: Initial `establish_bisync_baseline()` uses `--resync` to create the baseline. During steady-state operation, exit code 7 with no prior listing triggers immediate baseline re-establishment because another ordinary retry cannot use absent state; otherwise the daemon waits for three consecutive unrecoverable failures before the same last-resort fallback. Resilient/recover semantics and vanished-file repair run first whenever listing state exists. See [REQ-STOR-003](../../sdd/spec/storage.md#req-stor-003-bidirectional-sync-every-15-minutes-with-manual-triggers) AC6 and [AD125](../decisions/README.md#ad125-bounded-automatic-resync-after-exhausted-recovery).
