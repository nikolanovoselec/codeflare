...
Restricted operator sessions use the explicit parent-owned persistence contract in [REQ-OPERATOR-023](../../sdd/spec/operators.md#req-operator-023-explicit-operator-synchronization) and [REQ-OPERATOR-024](../../sdd/spec/operators.md#req-operator-024-independent-synchronization-verification), not whole-home restore or bisync. Trusted startup fixes the activity, session, policy digest, local output root, owner bucket, remote operation prefix, and authority deadline. Candidate requests may name only a stable operation ID, request digest, and a bounded file manifest; they cannot choose credentials, a bucket, an arbitrary local root, or storage operations such as list, copy, delete, or sync. <!-- @impl: host/src/operator-sync-service.ts::createOperatorSyncService -->

For an accepted operation, the host persists a credential-free receipt before effects, opens only regular non-symlink files beneath the fixed output root, and verifies every declared size and SHA-256. It uploads at most 128 files and 8 MiB under the fixed operation prefix. Only after all declared objects succeed does it write the bounded `manifest.json`, so the manifest is the final object and binds activity, session, operation, request and policy digests plus the exact file list. The receipt then records the manifest digest. <!-- @impl: host/src/operator-sync.ts::OperatorSyncService --> <!-- @impl: host/src/operator-sync-io.ts::OwnedOperatorSyncFiles -->

The activity-owned durable seal is separate from the host receipt: the parent prepares the operation scope before host effects, authorizes writes only while prepared, seals the prefix after upload, and accepts independent verification only for that sealed manifest digest. A changed repeat conflicts. A lost or nonterminal response becomes `unknown` and is never automatically replayed. Shutdown may wait for an already uploading receipt but does not initiate persistence; ordinary bisync and final-sync routes are denied in the restricted lane. <!-- @impl: src/operators/activity.ts::OperatorActivity --> <!-- @impl: entrypoint.sh::drain_operator_sync_shutdown -->
...
`--max-delete 5000` allows bisync to propagate compaction and bulk workspace deletions while retaining a fixed upper bound. The rclone default of 50% aborts bisync when more than half the files are deleted in one cycle - in a config-heavy sync with few files, even a single folder deletion can exceed this threshold.
...
Managed/UI-continuity keys are excluded; failed gallery restores remain as intent, and only `.obsolete` proves uninstall. First-match filters admit those bounded manifests, structural Herdr `session.json` files, the managed extension manifest, and `review-state/v1/**`, then exclude every other `~/.codeflare/**` path. Changed extension intent signals the existing bisync daemon; review completion follows periodic cadence or final drain without forcing another run. <!-- @impl: preseed/agents/pi/extensions/review-completion-state.ts::writeCompletion --> Sync-now, coalescing, and newest-wins convergence remain unchanged. <!-- @impl: entrypoint.sh::_openvscode_supervise_loop --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON -->
...
- Codex volatile state: `.codex/sessions/**`, `.codex/plugins/cache/**`, `.codex/cache/**`, `.codex/logs*.sqlite*`, `.codex/*.sqlite-shm`, `.codex/*.sqlite-wal`, `.codex/.tmp/**`, `.codex/log/**`, `.codex/models_cache.json`, `.codex/.personality_migration`, `.codex/shell_snapshots/**`, `.codex/tmp/**`, `.codex/version.json`, `.codex/skills/.system/**`.
...
- More Claude volatile state: `.claude/shell-snapshots/**`, `.claude/stats-cache.json`, `.claude.json.backup.*`, `.claude/usage-data/**`, `.claude/backups/**`, `.claude/tasks/**`, `.claude/sessions/**`, `.claude/history.jsonl`, `.claude/daemon/**`, `.claude/daemon.*`, `.claude/daemon-auth-*`, `.claude/gh-pr-status-cache.json`, `.claude/paste-cache/**`, `.claude/jobs/**`, `.claude/*.bak.*`, `.claude/settings.json.bak*`, `.claude/skills.bak.*/**`.
...
Memory-capture counter files used to live at `~/.memory/counter/**` and required an explicit exclude. They now live at `/tmp/.memory-counter/`, which is not synced because Cloudflare Containers use ephemeral disk; see [REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-20-user-messages-and-on-resume).
...
**Daemon-side mechanism.** The host's authenticated `/internal/bisync-trigger` endpoint sends SIGUSR2 for explicit cloud Sync now requests, authorizing disk-space recovery. Ordinary triggers and final drain use SIGUSR1 and cannot clear that block. Both traps inside the daemon subshell toggle the existing coalescing flags: `BISYNC_REQUESTED=1` (interrupt the current `sleep 900`) or `BISYNC_RERUN_REQUESTED=1` (queue exactly one rerun after the current cycle, if a bisync is mid-flight). N signals during one cycle coalesce to exactly one rerun. See [REQ-STOR-015](../../sdd/spec/storage.md#req-stor-015-explicit-sync-trigger-from-ui) AC5.
...
Cleanup runs once after the initial R2 restore, before agent PTYs are released, and again inside every regular or final `bisync_with_r2()` call. Deletions therefore reach R2 in the following sync. Each call remains isolated with `|| true`, so cleanup cannot kill startup or the sync daemon. Existing R2 exclusions for Claude subagents and Pi tasks remain; Pi conflict copies are also excluded. Codex is unchanged too: its session recordings stay excluded and its SQLite-backed transcript state has no smart-retention adapter. <!-- @impl: entrypoint.sh::release_agent_pty_after_cleanup --> <!-- @impl: entrypoint.sh::cleanup_main_transcripts --> <!-- @impl: entrypoint.sh::bisync_with_r2 --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON -->

Under [REQ-STOR-051](../../sdd/spec/storage.md#req-stor-051-pi-transcript-conflict-cleanup), Pi cleanup deletes every `.conflict*` file beneath `~/.pi/agent/sessions/`, including task paths and divergent copies. Pi cannot resume these files, and unique turns in them are intentionally disposable. The conflict-file sweep does not select canonical transcript filenames; normal ten-transcript retention still applies. Regular bisync also purges matching remote objects and excludes them from future synchronization. <!-- @impl: transcript-retention.mjs::deletePiConflictFiles --> <!-- @impl: entrypoint.sh::cleanup_remote_pi_transcript_conflicts --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON -->

## Conflict Resolution

Pi transcript conflict copies are always deleted locally and purged from their R2 session prefix under [REQ-STOR-051](../../sdd/spec/storage.md#req-stor-051-pi-transcript-conflict-cleanup). They are excluded from synchronization, so a failed remote purge cannot restore or nest stale copies; the purge retries with the next regular bisync. Disk-full/preallocation errors latch a block across later missing-listing failures, preventing repeated automatic resync. Free **local container disk** space, then click the storage browser's cloud **Sync now** button to authorize recovery. Another disk-full failure blocks again; success resumes periodic sync. Deleting from R2 alone does not immediately free local space, and resync merges one-sided files rather than reconstructing deletion history. The existing failure notice identifies the retry action. [REQ-STOR-003](../../sdd/spec/storage.md#req-stor-003-bidirectional-sync-every-15-minutes-with-manual-triggers).

Newest file wins (`--conflict-resolve newer`). `--resilient` + `--recover` handle transient bisync failures (e.g., interrupted transfers, listing mismatches) without losing deletion tracking. The sync daemon retries two minutes after a failure (or sooner if manually triggered via the storage panel); successful cycles retain the 15-minute cadence. `--max-delete 5000` on ALL bisync commands (`establish_bisync_baseline` and `bisync_with_r2`) allows compaction and bulk workspace deletions to propagate. Final bisync at shutdown runs via the DO-side synchronous drain (`POST /internal/final-sync`, 120s budget) before stop — not the legacy SIGTERM-trap watchdog (see [AD57](../decisions/README.md#ad57-135-second-shutdown-budget-for-final-bisync)). All bisync commands use `--ignore-checksum` to prevent false hash-mismatch aborts - rclone v1.73 introduced stricter post-transfer MD5 verification that fails when files change during sync.
...
- **Bisync compares via server-modtime (AD88, all modes).**
    ...
    Both `rclone bisync` invocations in `entrypoint.sh` (the retrying `--resync` baseline and steady-state cycle) pass `--use-server-modtime` and `--checkers 64`. Recovery baselines use `--resync-mode newer`, so repairing missing listings converges from the newer side instead of rewriting every local object to R2. `--use-server-modtime` compares the `LastModified` already returned by the bulk `--fast-list` instead of issuing one mtime HEAD per object, eliminating the per-cycle HEAD storm (the dominant steady-state cost).
...
- **Governed Mode delta initial sync (AD90, Governed Mode only).**
    ...
    The blocking `initial_sync_from_r2` normally restores the R2 tree into the ephemeral container. In [Governed Mode](#governed-mode-r2-sse-c-disabled) the entrypoint lays the image-baked seed (see [Preseed](preseed.md)) into the user home first, then runs initial sync with `--checksum` (usable MD5 ETags, available only when SSE-C is off), so unchanged seed files are skipped and only user deltas transfer. Under SSE-C, no bake is laid down and rclone compares size plus modification time; this detects same-size R2 changes that `--size-only` missed. <!-- @impl: entrypoint.sh::initial_sync_from_r2 -->
