...
`cleanup_old_pi_transcripts()` ([REQ-STOR-020](../../sdd/spec/storage.md#req-stor-020-pi-session-transcript-retention)) runs immediately after the Claude cleanup in the same daemon loop. It considers the five newest `~/.pi/agent/sessions/**/*.jsonl` files (excluding `tasks/` subdirs), then removes oldest candidates until retained historical transcripts fit within 256 MiB. The newest transcript is always preserved, even when the active session alone exceeds that budget. Pi cleanup also deletes each removed transcript's companion `tasks/` subdirectory, since those logs are only meaningful with their parent session. The same subshell + `|| true` pattern isolates benign cleanup failures. <!-- @impl: entrypoint.sh::cleanup_old_pi_transcripts -->
...
- [REQ-STOR-020](../../sdd/spec/storage.md#req-stor-020-pi-session-transcript-retention) - Pi Session Transcript Retention
