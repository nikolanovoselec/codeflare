...
| `~/.codeflare/ide-ui-state.json` | Yes | The only persistent Browser IDE state: an atomic, maximum-1-MiB allowlist of theme values, string-valued `keyboard.layout`, and key-specific canonical Explorer/open-file resources, captured after code-server is reaped; all other User settings, unknown fields, and opaque strings are rejected. Every other `~/.codeflare/**` path is excluded ([REQ-IDE-002](../../sdd/spec/browser-ide.md#req-ide-002-session-isolated-ide-not-bucket-stable), [REQ-IDE-016](../../sdd/spec/browser-ide.md#req-ide-016-ui-state-capture-and-restore-ordering)). <!-- @impl: scripts/browser-ide-ui-state.py::capture --> |
