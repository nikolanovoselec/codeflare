...
| `~/.codeflare/ide-ui-state.json` | Yes | The only persistent Browser IDE state: an atomic, maximum-1-MiB allowlist of theme, Explorer expansion, and canonical in-workspace open files, captured after code-server is reaped. Every other `~/.codeflare/**` path is excluded ([REQ-IDE-002](../../sdd/spec/browser-ide.md#req-ide-002-session-isolated-ide-not-bucket-stable)). |
