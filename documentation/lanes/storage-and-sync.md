...
Under [REQ-STOR-024 AC5](../../sdd/spec/storage.md#req-stor-024-managed-release-application), user reconciliation reads only the current verified release from the disposable cache and never GitHub. Applying that release does not require the previous cached bundle. A cacheless disable instead fails before bucket mutation and retains the applied state because prior managed paths cannot be identified safely. <!-- @impl: src/routes/storage/seed.ts::default --> <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs -->
