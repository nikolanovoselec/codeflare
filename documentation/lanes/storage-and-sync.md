...
| `~/.claude/projects/**/*.jsonl` | Yes (partial) | Up to ten main-session transcripts sync for resume; nested `subagents/`, `tool-results/`, and `workflows/` remain excluded. |
| `~/.claude/projects/**/subagents/**`, `tool-results/**`, `workflows/**` | **NO** | Native subagent transcripts, tool artifacts, and per-session workflow state are not required to resume the main session. |
...
| `~/.pi/agent/sessions/**/*.jsonl` | Yes (partial) | Up to ten main-session transcripts sync for `--resume`. Task subdirs (`**/tasks/**`) and the context-mode FTS5 store (`~/.pi/context-mode/**`) remain excluded. `~/.pi/agent/npm/node_modules/` is an image-seeded cache; see [container.md](container.md#pi-extension-npm-cache). |
...
Both Browser IDE manifests are atomic mode-`0600` regular files. The 1 MiB UI snapshot contains only allowlisted theme values, string-valued `keyboard.layout`, and key-specific canonical Explorer/open-file resources captured after code-server is reaped. The separate 64 KiB extension manifest contains lowercase IDs, exact versions, optional audit metadata, a warning acknowledgement, and bounded contributed global settings. Managed/UI-continuity keys are excluded; failed gallery restores remain as intent, and only `.obsolete` proves uninstall. The first-match filter admits those two exact paths and excludes every other `~/.codeflare/**` path. Changed extension-manifest writes signal the existing bisync daemon; periodic cadence, Sync-now, coalescing, newest-wins convergence, and final drain remain unchanged. <!-- @impl: scripts/browser-ide-ui-state.py::capture --> <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::captureExtensionManifest --> <!-- @impl: scripts/browser-ide-extensions.py::capture --> <!-- @impl: entrypoint.sh::_openvscode_supervise_loop --> <!-- @impl: entrypoint.sh::RCLONE_FILTERS_COMMON -->
...
Claude Code and Pi each keep at most ten main raw transcript files. The cleanup flattens nested project/session directories into one quota per agent, then ranks files by the latest recoverable native transcript timestamp. Claude candidates are UUID-named JSONL files below `~/.claude/projects/`; `subagents/`, `tool-results/`, and `workflows/` remain excluded. Pi candidates are timestamp-and-UUID JSONL files below `~/.pi/agent/sessions/`; `tasks/` remains excluded. Cleanup deletes transcript files only and leaves every directory and excluded child artifact alone.

The adapters validate Claude's native session identity and supported 2.1 transcript version or Pi's version-3 `session` header before trusting a timestamp. They scan bounded head and tail windows, so large transcripts are not loaded into memory. Malformed interior or trailing records are harmless when the header and latest timestamp remain recoverable. If one main candidate has an unknown shape or no recoverable timestamp, the whole agent falls back to deterministic `(mtime_ns, path)` ordering for that pass. Mixing native and restored filesystem chronology would be worse than either policy, so it is deliberately forbidden.
