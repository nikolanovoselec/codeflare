...
Pi cleanup also removes a numbered `.jsonl.conflictN` copy when every byte is already present at the start of its canonical transcript. It compares bounded chunks and verifies that both files stayed unchanged before deletion. Divergent copies, task paths, missing canonicals, symlinks, and concurrent changes are preserved. This removes obsolete append-only snapshots without risking unique conversation content. <!-- @impl: transcript-retention.mjs::removeContainedConflictCopy --> <!-- @impl: transcript-retention.mjs::removeRedundantPiConflicts -->

## Conflict Resolution

Conflict copies are preserved unless Pi cleanup proves a numbered copy is wholly contained in its unchanged canonical transcript. Inspect and preserve all divergent content before any manual cleanup. Disk-full/preallocation errors latch a block across later missing-listing failures, preventing repeated automatic resync. Free **local container disk** space, then click the storage browser's cloud **Sync now** button to authorize recovery. Another disk-full failure blocks again; success resumes periodic sync. Deleting from R2 alone does not immediately free local space, and resync merges one-sided files rather than reconstructing deletion history. The existing failure notice identifies the retry action. [REQ-STOR-003](../../sdd/spec/storage.md#req-stor-003-bidirectional-sync-every-15-minutes-with-manual-triggers).

Newest file wins (`--conflict-resolve newer`). `--resilient` + `--recover` handle transient bisync failures (e.g., interrupted transfers, listing mismatches) without losing deletion tracking. The sync daemon retries on the next 15-minute cycle after a failure (or sooner if manually triggered via the storage panel). `--max-delete 100` on ALL bisync commands (`establish_bisync_baseline` and `bisync_with_r2`) allows bulk workspace deletions to propagate. Final bisync at shutdown runs via the DO-side synchronous drain (`POST /internal/final-sync`, 120s budget) before stop — not the legacy SIGTERM-trap watchdog (see [AD57](../decisions/README.md#ad57-135-second-shutdown-budget-for-final-bisync)). All bisync commands use `--ignore-checksum` to prevent false hash-mismatch aborts - rclone v1.73 introduced stricter post-transfer MD5 verification that fails when files change during sync.
