...
Restricted operator sessions use the explicit parent-owned persistence contract in [REQ-OPERATOR-005](../../sdd/spec/operators.md#req-operator-005-owned-session-structured-pi-and-explicit-persistence), not whole-home restore or bisync. Trusted startup fixes the activity, session, policy digest, local output root, owner bucket, remote operation prefix, and authority deadline. Candidate requests may name only a stable operation ID, request digest, and a bounded file manifest; they cannot choose credentials, a bucket, an arbitrary local root, or storage operations such as list, copy, delete, or sync. <!-- @impl: host/src/operator-sync-service.ts::createOperatorSyncService -->
