...
**Self-heal:** When `collectMetrics` reaches its running branch (successful `/health` probe), but KV reads `stopped` and no graceful-shutdown marker is in flight, it re-asserts `running` in KV, bounding any false-stopped window to a single alarm tick (~60 s). The self-heal does not apply when `destroy()` or user Stop set the shutdown-in-flight marker — those deliberate stops are left to settle.
