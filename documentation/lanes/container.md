...
If transport stays unreachable, three post-reset confirmations permit one final reconstruction. This confirmation budget also applies when a monitor-loss reconstruction still reads not-running, where no host probe can run. Three failures after that second attempt persist `exhausted`; a still-running container keeps 60 s checks and usage accounting, while a still-not-running container returns to the persisted exit-confirmation window and converges to stopped without another reset. Logs correlate reset, confirmation, success, and exhaustion with DO and attempt identities, counts, elapsed time, container state, and a bounded route failure category (`timeout`, `network-lost`, `connection-refused`, or `other`). A persisted `shutdownRequested` marker ends recovery; an unreadable marker suppresses reconstruction and alarm re-arming. The SDK constructor's running-container path is expected to reattach `container.monitor()`; browser reconnection to the existing PTY remains a deployed smoke check.
