...
**Vendored undici patch (build-time):** Trivy scans npm's own bundled dependencies and baked agent/preseed tool trees. The Dockerfile's `undici-cve-patch` layer replaces vulnerable bundled `undici` copies with the fixed patch release for the same major version, then fails closed if any vulnerable copy remains. This remediates fixable image CVEs without adding `.trivyignore` suppressions ([REQ-SEC-011](../../sdd/spec/security.md#req-sec-011-container-image-scanned-for-cves-before-deploy)).
