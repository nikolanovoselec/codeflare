...
**Terminal runtimes:** `CODEFLARE_TERMINAL_MODE` is copied from the persisted session stamp. Classic starts `/bin/bash -l` and retains up to six outer `node-pty` sessions. For an immutable Pi or Claude tab 1, a new Classic Codeflare session creates one native UUID under its current-session-only `.codeflare/classic` recovery path. Restoring that stopped session passes the same UUID to the agent so its synced transcript resumes, while a different Codeflare session starts empty ([REQ-AGENT-211](../../sdd/spec/agents.md#req-agent-211-classic-agent-transcript-resume)). <!-- @impl: entrypoint.sh::configure_tab_autostart -->
