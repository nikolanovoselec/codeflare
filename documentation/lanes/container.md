...
Terminal admission ignores KV liveness and checks persisted Container SDK state: `stopping`, `stopped`, and `stopped_with_code` produce authoritative 4503; unavailable state or readiness produces retryable 1013. The inner Durable Object forwarding gate also uses 1013 when its volatile running flag is false, preventing a transient runtime reading from falsely stopping the browser while still blocking container wake. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch -->

**Managed-update ownership:** Reconciliation status and mutation admission enumerate every listed session and read its persisted Container SDK state without waking the container. `stopped` and `stopped_with_code` release ownership even when LIST metadata still says running; every other or unavailable state fails closed. The same helper gates batch discovery, automatic/manual reconciliation, and managed mode changes, so those paths cannot disagree and strand an update. <!-- @impl: src/lib/session-helpers.ts::hasOwningSessionContainer --> <!-- @impl: src/routes/session/lifecycle.ts::app --> <!-- @impl: src/routes/storage/seed.ts::assertNoOwningSession --> <!-- @impl: src/routes/preferences.ts::app -->
