...
**Workspace-open settings ([REQ-IDE-009](../../sdd/spec/browser-ide.md#req-ide-009-frictionless-workspace-open-for-every-ide-agent)):** Before launch, `_openvscode_prepare_agent` seeds ephemeral OpenVSCode User settings that disable workspace trust and ignore extension recommendations for every agent kind, so the session workspace opens without a trust prompt or a recommended-extensions prompt. Pi and the empty inventory receive these base settings alone; the Claude path above also carries them. A preparation failure refuses the launch. <!-- @impl: entrypoint.sh::_openvscode_prepare_agent --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildBaseOpenVscodeSettings -->
...
**Tier-split gating ([AD52](../decisions/README.md#ad52-graphify-mcp-available-everywhere-discipline-advanced-only), [AD53](../decisions/README.md#ad53-graphify-hot-reload-wrapper-with-multi-repo-sentinel-tracking)):** the MCP server + `graphify-mcp-lazy.py` wrapper are registered in `~/.claude.json` for both default and advanced session modes (ambient capability). All hooks - SessionStart context-injection, PostToolUse-on-clone triage, PreToolUse graph-first nudge, and the active-repo tracker - plus the graph-first discipline (a section of `engineering-constitution.md` since 2026-07-25) and `graphify/SKILL.md` ship in advanced session mode only. Default session mode users have the capability without the proactive discipline and without multi-repo tracking precision.
