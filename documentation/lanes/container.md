...
Copilot 1.0.86 is installed from the pinned npm dependency. <!-- @impl: preseed/npm-tools/package.json::@github/copilot --> The image includes `slirp4netns`, `iptables`/`ip6tables` and util-linux sandbox tooling. <!-- @impl: Dockerfile::RUN apt-get update --> Installing them does not grant TUN access, privileged containers or local-network permission; platform restrictions and explicit sandbox policy still apply.
...
`.cache-bust` invalidates these layers on each deploy, while `npm ci` preserves reviewed registry integrity and transitive versions. The build removes alternate operating-system, architecture, baseline, and musl payloads while retaining each canonical Linux x64 package. Pruning tests verify the retained directories; complete-image smoke rejects alternate packages and executes every selected launcher. The pruning boundary reports reclaimed bytes ([REQ-OPS-040](../../sdd/spec/operations.md#req-ops-040-selected-coding-agent-packaging) AC4).
...
**Terminal runtimes:** `CODEFLARE_TERMINAL_MODE` is copied from the persisted session stamp. Classic starts `/bin/bash -l` and retains up to six outer `node-pty` sessions. For an immutable Pi or Claude tab 1, a new Classic Codeflare session creates one native UUID under its current-session-only `.codeflare/classic` recovery path. Classic seeds Pi's empty native session header before launching its exact ID, so fresh startup does not report the deliberate ID as missing. Restoring that stopped session passes the same UUID to the agent so its synced transcript resumes, while a different Codeflare session starts empty. Herdr retains its separate native session-reference lifecycle ([REQ-AGENT-211](../../sdd/spec/agents.md#req-agent-211-classic-agent-transcript-resume)). <!-- @impl: entrypoint.sh::configure_tab_autostart -->

For Classic Pi, each successful root `/resume` selection atomically replaces the current binding. Repeated switches therefore preserve the last active transcript across restart without allowing startup, non-root session paths, or invalid or mismatched session IDs to replace it. <!-- @impl: preseed/agents/pi/extensions/classic-session-binding.ts::adoptClassicResumedSession -->

Herdr attaches one official client to `cf-<SESSION_ID>` under mode-0700 `/run/codeflare/herdr/<SESSION_ID>`. Managed configuration sets mobile layout, mouse capture, browser-owned clipboard policy, 10 MB pane scrollback, palette, labels, and static status symbols. Fresh Herdr Pi starts fullscreen so Pi owns transcript history ([REQ-TERM-040](../../sdd/spec/terminal.md#req-term-040-stable-herdr-pane-scrollback)). <!-- @impl: image/herdr/codeflare-herdr-terminal::bootstrap -->

Explicit Herdr copy uses bounded OSC 52 browser writes. If Samsung rejects a write, the latest failure remains available to the next trusted floating Paste control, which retries the browser write and pastes through xterm ([REQ-TERM-032](../../sdd/spec/terminal.md#req-term-032-herdr-clipboard-compatibility-boundary), [REQ-TERM-041](../../sdd/spec/terminal.md#req-term-041-rejected-herdr-clipboard-recovery)). Pi and Herdr toasts confirm OSC 52 emission, not browser acceptance. <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @impl: web-ui/src/lib/osc52.ts::beginClipboardWrite --> <!-- @impl: web-ui/src/components/FloatingTerminalButtons.tsx::pasteFromClipboard -->

Updater checks, remote manifest checks, sound, pane history, nested launches, and Kitty graphics remain disabled. Live process state, pane history, sockets, logs, and user-edited settings stay ephemeral. Herdr's official structural `session.json` is the sole durable runtime artifact and follows normal `~/.codeflare` R2 sync ([REQ-TERM-033](../../sdd/spec/terminal.md#req-term-033-durable-herdr-structural-session-recovery)). Browser IDE workspaces start no Herdr runtime.
...
Pi and Claude completion notifications aggregate every recognized agent pane across Herdr tabs and splits. The host subscribes through the private public-API socket, waits until every tracked pane is `idle` or `done` for ten minutes after work, and cancels both the timer and queued completion when any pane resumes. No pane identifier or socket path reaches the browser. Classic has no completion producer ([REQ-TERM-029](../../sdd/spec/terminal.md#req-term-029-herdr-status-gated-terminal-completion), [AD149](../decisions/README.md#ad149-herdr-semantic-status-owns-completion-notification-timing)). <!-- @impl: host/src/herdr-agent-status.ts::HerdrAgentStatusMonitor -->
...
- Before the terminal host starts, entrypoint exports `TMPDIR=/run/codeflare/pi-tmp` and links its `jiti` child to the image cache. Fast Start Off replaces that link with a fresh runtime-owned directory after updates; the baked cache is untouched. <!-- @impl: entrypoint.sh::configure_pi_jiti_runtime_cache --> <!-- @impl: scripts/verify-pi-lockstep.mjs::resetRuntimeJitiCache -->
...
| `/run/codeflare/sync/sync-status.json` | Initialization/synchronization status read by the private health surface |
| `/run/codeflare/sync/sync.log` | Runtime synchronization diagnostics |
| `/run/codeflare/sync/rclone/` | Bisync listings and lock state |
| `/run/codeflare/services/` | Terminal, Vault monitor, SilverBullet, init-complete records, and detached CI-monitor scripts/logs |
...
flowchart TD
    ...
    D --> E["touch /run/codeflare/services/init-complete\n- releases PTY pre-warm"]
```
...
code-server receives `--auth none` only behind the Worker's Access/ownership chain and the container bearer check; it also receives disabled telemetry, updates, built-in proxying, Getting Started override, and workspace trust. Its exact ephemeral user-data path is `/run/codeflare/openvscode/data/data`, whose User settings are prepared at `/run/codeflare/openvscode/data/data/User`; fixed extensions remain under `/opt/codeflare/openvscode/extensions/{pi,claude,none}`.

Every inventory manages **Bash** as the integrated-terminal default. Its `/bin/bash -l` profile sets `MANUAL_TAB=1`, so the existing `.bashrc` contract skips tab-1 agent autostart; users can explicitly choose the adjacent **Codeflare Session Agent** login-shell profile to run the configured agent instead. The profile map suppresses only code-server's auto-detected lowercase `bash` duplicate, preserving unrelated detected profiles ([REQ-IDE-047](../../sdd/spec/browser-ide.md#req-ide-047-bash-first-browser-ide-terminals)). No additional launcher or process is introduced. <!-- @impl: openvscode/claude/managed-settings.mjs::buildBaseOpenVscodeSettings --> <!-- @impl: entrypoint.sh::configure_tab_autostart -->
...
The host buffers at most 2 MiB and fails closed when either authority or the pinned meta-element shape is missing, duplicated, malformed, compressed, or oversized; assets and protocol traffic remain streaming. Advanced-mode only; torn down through the retained `/run/codeflare/openvscode/supervisor.pid` generation lifecycle. See [REQ-IDE-001](../../sdd/spec/browser-ide.md#req-ide-001-per-session-browser-ide-served-through-the-worker-proxy) and [REQ-IDE-035](../../sdd/spec/browser-ide.md#req-ide-035-canonical-browser-ide-workspace-projection). <!-- @impl: host/src/vscode-proxy.ts::projectVscodeWorkbenchWorkspace --> <!-- @impl: host/src/request-router.ts::createRequestHandler -->
...
Pi adds native-chat disablement, edited-file accessibility, response and confirmation notifications, and disables the duplicate Claude agent-file source. The empty inventory receives base settings with native chat disabled, while Claude adds its isolated config, login, permission, and sidebar settings. A preparation failure refuses the launch. <!-- @impl: openvscode/claude/managed-settings.mjs::buildPiOpenVscodeSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildUnsupportedOpenVscodeSettings --> <!-- @impl: openvscode/claude/managed-settings.mjs::buildOpenVscodeSettings --> <!-- @impl: entrypoint.sh::_openvscode_launch_once -->

**Bounded IDE continuity ([REQ-IDE-002](../../sdd/spec/browser-ide.md#req-ide-002-session-isolated-ide-not-bucket-stable), [REQ-IDE-016](../../sdd/spec/browser-ide.md#req-ide-016-bounded-ide-state-capture-and-restore-ordering), [REQ-IDE-036](../../sdd/spec/browser-ide.md#req-ide-036-persistent-user-managed-extensions), [REQ-IDE-037](../../sdd/spec/browser-ide.md#req-ide-037-lazy-extension-restoration), [REQ-IDE-038](../../sdd/spec/browser-ide.md#req-ide-038-extension-warning-acknowledgement)):** Live data, extension packages, and SQLite companions remain under `/run/codeflare/openvscode/data`; user extensions live under `/run/codeflare/openvscode/data/extensions`.

Only after a launch generation is reaped, `browser-ide-ui-state.py` exports a maximum-1-MiB JSON allowlist containing theme values, string-valued `keyboard.layout`, and key-specific Explorer/open-file resource schemas. Independently, the always-present welcome extension lazily restores and captures a mode-0600 `~/.codeflare/ide-extensions.json` bounded to 64 KiB, 50 lowercase IDs, and 32 KiB of contributed global User settings. Extension, registry, and contributed-setting changes share one capture path; welcome deactivation flushes pending settings, and `browser-ide-extensions.py` repeats registry-only capture after reap. Invalid content is retained unchanged, fixed IDs are excluded, and `.obsolete` alone proves uninstall. <!-- @impl: entrypoint.sh::_openvscode_supervise_loop --> <!-- @impl: scripts/browser-ide-ui-state.py::capture --> <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::activateExtensionPersistence --> <!-- @impl: scripts/browser-ide-extensions.py::capture -->
...
Pi updates validate required dependencies and exercise real image processing, not only `pi --version`. An incomplete installation gets one reinstall from its lockfile; failures remain visible and do not block terminal readiness. Update downloads use temporary runtime storage, cleared afterward. Image builds also validate both installed Pi copies. ([REQ-AGENT-206](../../sdd/spec/agents.md#req-agent-206-fast-start-off-runtime-updates))
...
When Fast Start is disabled (`FAST_CLI_START=false`), `entrypoint.sh` unsets the Dockerfile-level env vars (`DISABLE_AUTOUPDATER`, `DISABLE_INSTALLATION_CHECKS`) and the entrypoint-level update suppressors (`OPENCODE_DISABLE_AUTOUPDATE`, `COPILOT_AUTO_UPDATE`, `PI_OFFLINE`, `PI_SKIP_VERSION_CHECK`), then removes Codeflare-managed Codex settings-file suppressors. Before terminal readiness, installed Pi and Codex CLIs enter one explicit latest-runtime update, Pi reconciles configured packages, and startup logs each installed CLI's before and after versions or visible failure evidence. An update failure does not strand startup ([REQ-AGENT-206](../../sdd/spec/agents.md#req-agent-206-fast-start-off-runtime-updates)). Fast Start ON sets `PI_OFFLINE=1`, so Pi skips startup network checks and will not install restored user-added Pi packages that are absent from the image cache until Fast Start is turned off.
...
**Self-heal:** When `collectMetrics` reaches its running branch (successful `/health` probe), but KV status is `stopped` or missing and the persisted deliberate-stop marker (`shutdownRequested` in DO storage) is absent, it re-asserts `running` in KV. An explicit start request that finds an already-running container does not write KV: that route cannot inspect the durable marker and could race a deliberate stop or recreate a deleted record. Marker-aware metrics owns convergence. The self-heal does not apply when `destroy()` has written the marker: `destroy()` persists `shutdownRequested` as its first action — before clearing session identifiers — and also drops the `collectMetrics` alarm, so the guard survives a DO eviction mid-teardown. <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer -->

**Read-side authority:** KV status and LIST metadata remain the no-wake dashboard projection, but negative observations are not runtime proof while the frontend owns a startup guard or terminal connection/attempt/retry loop. Both full session hydration and incremental polling preserve those sessions across stale `stopped` values and temporary LIST omissions. <!-- @impl: web-ui/src/stores/session.ts::shouldRetainNegativeKv -->

Terminal admission reads the owner-scoped D1 lifecycle: only D1 `stopping` or `stopped` produces definitive 4503. Failed no-start health verification or terminal-not-ready returns retryable 1013 before rate limiting; a verified survivor can forward despite stale or unavailable SDK state under normal WS rate limits. The ordinary inner Durable Object fetch gate uses 1013 when its volatile running flag is false; authenticated no-start `forwardExisting()` can instead reach a port-verified survivor without waking a replacement. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch -->

**Managed-update ownership:** Reconciliation status and mutation admission enumerate owner-scoped D1 sessions without waking the container. Only D1-confirmed `stopped` releases ownership; SDK `stopped` and `stopped_with_code` do not prove exit. The same owner check gates batch discovery, automatic/manual reconciliation, and managed mode changes. <!-- @impl: src/lib/session-helpers.ts::hasOwningSessionContainer --> <!-- @impl: src/routes/session/lifecycle.ts::app --> <!-- @impl: src/routes/storage/seed.ts::assertNoOwningSession --> <!-- @impl: src/routes/preferences.ts::app -->

`destroy()` persists deliberate-stop intent before teardown, but neither that marker nor SDK `onStop()` is process-exit evidence. D1 records `stopped` only after awaited destruction or a generation-fenced positive monitor completion; an interrupted teardown retains ownership for retry ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-retains-ownership-until-confirmed-exit) AC1-AC3).

The stop and delete API routes treat a rejected `destroy()` as unconfirmed teardown. Stop retains the D1 intent for retry, while delete retains the session record; neither route reports success until graceful destruction and its final-sync boundary complete ([REQ-SESSION-006](../../sdd/spec/session-lifecycle.md#req-session-006-user-can-stop-restart-and-delete-sessions) AC2/AC5).
...
**Restart admission:** `startOrRestartContainer` may claim a replacement generation only after D1 confirms the previous execution stopped. A stale KV or SDK state cannot authorize replacement while a process may survive; a failed bucket-forward or teardown remains retryable rather than inventing a stopped session ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-retains-ownership-until-confirmed-exit) AC2-AC3).
...
If transport stays unreachable, three post-reset confirmations permit one final reconstruction. Three failures after the second attempt persist `exhausted` and re-arm a non-billable check. Exhaustion, timeout and SDK not-running observations do not prove exit, request `SIGTERM`, or release D1 workload ownership. A positively resolved existing-process monitor or awaited destruction confirms the generation before D1 becomes `stopped`; failure to confirm retains the stop intent for retry. No-start forwarding may verify a surviving process even when the reconstructed coordinator's SDK flag reads not-running.

A failed recovery-ownership read attempts to re-arm the one-shot alarm. When exit confirmation or boundary cancellation fails, retain ownership and retry against the same generation; do not infer termination from a failed health probe. A process lost before the reconstructed coordinator verifies and attaches to it may remain owner-held until independent exit evidence is available. This describes the intended repair for future sessions, not a deployed production state.
...
**Requirements:** [REQ-STOR-020](../../sdd/spec/storage.md#req-stor-020-managed-environment-reconciliation), [REQ-STOR-022](../../sdd/spec/storage.md#req-stor-022-managed-reconciliation-admission), [REQ-STOR-023](../../sdd/spec/storage.md#req-stor-023-managed-release-status-projection), [REQ-STOR-024](../../sdd/spec/storage.md#req-stor-024-managed-release-application), [REQ-IDE-042](../../sdd/spec/browser-ide.md#req-ide-042-additive-company-extension-reconciliation), [REQ-IDE-045](../../sdd/spec/browser-ide.md#req-ide-045-company-extension-reconciliation-orchestration)
...
**Mode-split gating ([AD52](../decisions/README.md#ad52-graphify-mcp-available-everywhere-discipline-advanced-only), [AD53](../decisions/README.md#ad53-graphify-hot-reload-wrapper-with-multi-repo-sentinel-tracking)):** the MCP server and `graphify-mcp-lazy.py` reach default and advanced modes. The conditional query-routing rule, PostToolUse-on-clone triage, PreToolUse soft nudge, active-repo tracker, and `graphify` build/update skill remain advanced-only. Default sessions can query Graphify when explicitly requested without proactive routing, clone, refresh, or search-nudge overhead.
...
| Classic agent continuity | [REQ-AGENT-211](../../sdd/spec/agents.md#req-agent-211-classic-agent-transcript-resume) | `entrypoint.sh`, Pi `classic-session-binding.ts` | Exact initial UUID and last explicit `/resume` selection survive restart |
...
| Managed-resource persistence | [REQ-ENTERPRISE-027](../../sdd/spec/enterprise-mode.md#req-enterprise-027-managed-resource-admission-and-transport), [REQ-ENTERPRISE-028](../../sdd/spec/enterprise-mode.md#req-enterprise-028-managed-resource-request-classification), [REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement) | lifecycle admission, DO security props, `EgressController` | Exact policy verification before start/signing; container receives only non-authoritative identity |
