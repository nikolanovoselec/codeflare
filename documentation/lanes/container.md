...
- jiti's cache key is **path-sensitive** — the entry filename encodes `hash(abspath + source + jiti version)`, so identical bytes at two different paths produce different entries that never hit each other. The warm run therefore transpiles each extension at exactly the path Pi loads it from at runtime (`/home/user/.pi/agent/extensions/`), using the real `PI_CODING_AGENT_DIR`/`HOME` — not a throwaway tmpdir. npm packages hit regardless because warm and runtime both resolve through the same symlink realpath (`/opt/codeflare/pi-agent/npm`). The entrypoint's `relay_managed_pi_extensions()` keeps the on-disk extension bytes equal to the build (the content half of the key) in all deployment modes.
