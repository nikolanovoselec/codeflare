...
Terminal admission reads the owner-scoped D1 lifecycle: only D1 `stopping` or `stopped` produces definitive 4503. Transient Container SDK stop or unavailable state/readiness while D1 still owns the session produces retryable 1013 without waking the container. The ordinary inner Durable Object fetch gate uses 1013 when its volatile running flag is false; authenticated no-start `forwardExisting()` can instead reach a port-verified survivor without waking a replacement. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch -->
