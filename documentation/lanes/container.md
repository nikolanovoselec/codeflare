...
Failed status writes or stop requests retain explicit terminal recovery ownership and schedule a non-billable 60 s retry; a scheduling failure is logged and propagated while that durable ownership remains. A retained pre-upgrade `exhausted` record with stopped or absent KV state migrates to terminal stop ownership before probes can restore `running`. A failed early ownership read re-arms the one-shot alarm. A not-running container continues through persisted exit confirmation without another reset.
