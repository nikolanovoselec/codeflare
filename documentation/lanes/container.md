...
**File:** `Dockerfile` installs coder/code-server 4.133.0 from the pinned linux-amd64 release with SHA-256 verification. The build also verifies code-server commit `d2f7a122522456b351e9b3ddd39e4f3fb9fd5318`, the actual embedded Code package version 1.133.0, and product provenance; it records VS Code gitlink `a5b500951314efd502d07465bd138dfbd714a960`, which the Shadow Pins job derives from the immutable release tag. Only `/usr/local/bin/code-server` is exposed; no OpenVSCode runtime binary remains. The dedicated `code-server` Shadow Pins job derives the packaged commit and Code version from the immutable upstream release artifact, cross-checks its package/product identities, derives the VS Code source gitlink from the release tag, and invalidates the checksum for operator review. See [AD119](../decisions/README.md#ad119-replace-openvscode-with-pinned-code-server-behind-the-existing-session-proxy).
...
**Self-heal:** When `collectMetrics` reaches its running branch (successful `/health` probe), but KV status is `stopped` or missing and the persisted deliberate-stop marker (`shutdownRequested` in DO storage) is absent, it re-asserts `running` in KV, bounding any false-stopped window to a single alarm tick (~60 s). An explicit start request also reconciles missing or stopped KV status before returning `already_running` for a verified-running container. These repeated verified-running writes make an eventually consistent stale activity write converge without treating missing status as running on read. The self-heal does not apply when `destroy()` has written the marker: `destroy()` persists `shutdownRequested` as its first action — before clearing session identifiers — and also drops the `collectMetrics` alarm, so the guard survives a DO eviction mid-teardown. <!-- @impl: src/container/container-metrics.ts::collectMetrics --> <!-- @impl: src/routes/container/lifecycle.ts::startOrRestartContainer -->
