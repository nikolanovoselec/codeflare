...
- jiti caches transpiles under `$TMPDIR/jiti`, so the warm run redirects `TMPDIR` and moves the result to `/opt/codeflare/jiti-cache`; before the terminal host starts, `entrypoint.sh` exports stable `TMPDIR=/run/codeflare/pi-tmp` and symlinks its `jiti` child to the image cache.
