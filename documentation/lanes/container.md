...
**Terminal runtimes:** `CODEFLARE_TERMINAL_MODE` is copied from the persisted session stamp. Classic starts `/bin/bash -l` and retains up to six outer `node-pty` sessions. For an immutable Pi or Claude tab 1, a new Classic Codeflare session creates one native UUID under its current-session-only `.codeflare/classic` recovery path; restoring that stopped session passes the same UUID to the agent so its synced transcript resumes, while a different Codeflare session starts empty ([REQ-AGENT-003](../../sdd/spec/agents.md#req-agent-003-agent-cli-auto-started-in-tab-1) AC8). Herdr attaches one official client to `cf-<SESSION_ID>` under mode-0700 `/run/codeflare/herdr/<SESSION_ID>`. Managed configuration sets mobile layout, mouse capture, browser-owned clipboard policy, 10 MB pane scrollback, palette, labels, and static status symbols. Fresh Herdr Pi starts fullscreen so Pi owns transcript history ([REQ-TERM-040](../../sdd/spec/terminal.md#req-term-040-stable-herdr-pane-scrollback)). <!-- @impl: entrypoint.sh::configure_tab_autostart --> <!-- @impl: image/herdr/codeflare-herdr-terminal::bootstrap -->
