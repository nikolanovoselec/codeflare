...
The shared npm-tool set—agent CLIs, Bun, context-mode, `consult-llm-mcp`, and `chrome-devtools-mcp`—installs from `preseed/npm-tools/package.json` and its committed lock. Image-owned `oxlint` installs from its dedicated committed lock, receives weekly cooldown-backed Dependabot updates, and exposes `/usr/local/bin/oxlint` without changing managed-seed compatibility.
