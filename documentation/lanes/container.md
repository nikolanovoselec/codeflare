...
`onStart()` clears the deliberate-stop marker and any partial transport-failure streak on the next fresh container lifecycle. A Durable Object-only reconstruction does not run `onStart()` because the workload never stopped; `reconcileContainerTransport()` clears its own completed streak immediately before aborting the DO. Any restart path that destroys the container must go on to start it (AC5) rather than reporting it already running.
...
**Poll timeout and transport reconstruction ([REQ-SESSION-020](../../sdd/spec/session-lifecycle.md#req-session-020-the-metrics-alarm-outlives-a-container-that-stops-answering) AC1-AC2, AC7-AC8):** Every in-container request `collectMetrics()` awaits — `/activity`, `/health`, and the Timekeeper ping — is bounded to 10 s (`CONTAINER_POLL_BUDGET_MS`). The re-arm is the alarm's last statement and the schedule is one-shot, so a container that accepted the TCP connection but never answered did not merely skip a reading: it ended the loop for the rest of the session's life, with neither `onStart` (fresh start only) nor `onError` (SDK-observed exit only) able to restore it for a container that is wedged but still reported running.

A timed-out request now contributes to a persisted DO-to-container transport-failure streak only when both independent host probes fail in the same tick. The first two complete failures retry after 5 s. The third calls `ctx.abort()` after clearing the streak, resetting the Durable Object without stopping the container or writing KV `status: 'stopped'`. The Containers SDK constructor sees the still-running process, reattaches `container.monitor()`, and the browser reconnects to the existing in-container PTY. Any response from either endpoint clears the streak; an HTTP non-OK still proves the private port transport works. A persisted `shutdownRequested` marker ends the metrics tick without reconstructing or re-arming, so this recovery cannot interrupt a deliberate stop or delete.
