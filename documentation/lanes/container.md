...
| `/run/codeflare/markers/` | Process-lifetime idempotency markers |
| `/run/codeflare/pi-tmp/` | Pi/Jiti and context-mode preload scratch that must survive disposable `/tmp` cleanup |
| `/tmp` | Disposable scratch only; deletion must not affect a running service |

Path ownership is a runtime contract under [REQ-OPS-047](../../sdd/spec/operations.md#req-ops-047-cleanup-safe-container-runtime-state); storage semantics and finalization authority remain in [Storage & Sync](storage-and-sync.md).
