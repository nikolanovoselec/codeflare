...
`onStart()` clears the deliberate-stop marker and any partial transport-failure streak on the next fresh container lifecycle ([REQ-SESSION-021](../../sdd/spec/session-lifecycle.md#req-session-021-unreachable-container-transport-reconstructs-without-replacing-the-workload) AC4). A Durable Object-only reconstruction does not run `onStart()` because the workload never stopped; `reconcileContainerTransport()` clears its own completed streak immediately before aborting the DO. Any restart path that destroys the container must go on to start it ([REQ-SESSION-020](../../sdd/spec/session-lifecycle.md#req-session-020-the-metrics-alarm-outlives-a-container-that-stops-answering) AC5) rather than reporting it already running.
...
**Poll timeout and transport reconstruction ([REQ-SESSION-020](../../sdd/spec/session-lifecycle.md#req-session-020-the-metrics-alarm-outlives-a-container-that-stops-answering) AC1-AC2; [REQ-SESSION-021](../../sdd/spec/session-lifecycle.md#req-session-021-unreachable-container-transport-reconstructs-without-replacing-the-workload) AC1-AC7):** Every in-container request `collectMetrics()` awaits — `/activity`, `/health`, and the Timekeeper ping — is bounded to 10 s (`CONTAINER_POLL_BUDGET_MS`). The re-arm is the alarm's last statement and the schedule is one-shot, so a container that accepted the TCP connection but never answered did not merely skip a reading: it ended the loop for the rest of the session's life, with neither `onStart` (fresh start only) nor `onError` (SDK-observed exit only) able to restore it for a container that is wedged but still reported running.

A timed-out request contributes to a persisted DO-to-container transport-failure streak only when both independent host probes fail in the same tick. The first complete failure records the normal tick's usage and schedules a retry after 5 s. Subsequent accelerated confirmation ticks do not add usage or ping Timekeeper. The third failure calls `ctx.abort()` after clearing the streak, resetting the Durable Object without stopping the container or writing KV `status: 'stopped'`. Any response from either endpoint clears the streak; an HTTP non-OK still proves the private port transport works. A persisted `shutdownRequested` marker ends the metrics tick without reconstructing or re-arming, so recovery cannot interrupt a deliberate stop or delete. The SDK constructor's running-container path is expected to reattach `container.monitor()`; browser reconnection to the existing PTY remains a deployed smoke check.
