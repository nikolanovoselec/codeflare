...
Pi and Claude completion notifications use the same primary-agent status that Herdr renders. The host subscribes through the private public-API socket, waits four minutes after `working` changes to `idle` or `done`, and cancels both the timer and queued completion when work resumes. No pane identifier or socket path reaches the browser. Classic has no completion producer ([REQ-TERM-029](../../sdd/spec/terminal.md#req-term-029-herdr-status-gated-terminal-completion), [AD149](../decisions/README.md#ad149-herdr-semantic-status-owns-completion-notification-timing)). <!-- @impl: host/src/herdr-agent-status.ts::HerdrAgentStatusMonitor -->
