...
Terminal admission reads the owner-scoped D1 lifecycle: only D1 `stopping` or `stopped` produces definitive 4503. Transient Container SDK stop or unavailable state/readiness while D1 still owns the session produces retryable 1013 without waking the container. The inner Durable Object forwarding gate also uses 1013 when its volatile running flag is false, preventing a transient runtime reading from falsely stopping the browser while still blocking container wake. <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch -->
