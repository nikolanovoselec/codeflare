...
- The build is **fail-closed**: after warm-up, it requires a cache artifact for every local preseed extension and for Goal.

    Local extensions are checked by cache filename (`extensions-<base>.<hash>.mjs`). Goal's expected artifact is derived from the installed package's resolved source path and must be a regular file ([REQ-AGENT-111](../../sdd/spec/agents.md#req-agent-111-reviewed-goal-workflows-in-pi-sessions) AC3). An added extension, one modified into a non-loading state, a skipped extension, or a Pi CLI change that breaks warm-up therefore fails the build instead of silently regressing production startup.
...
`onStart()` clears the marker on the next fresh start, which is why any restart path that destroys the container must go on to start it (AC5) rather than reporting it already running.
