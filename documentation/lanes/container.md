...
**Read-side authority:** KV status and LIST metadata remain the no-wake dashboard projection, but negative observations are not runtime proof while the frontend owns a startup guard or terminal connection/attempt/retry loop. Both full session hydration and incremental polling preserve those sessions across stale `stopped` values and temporary LIST omissions. Terminal admission ignores KV liveness and checks persisted Container SDK state: `stopping`, `stopped`, and `stopped_with_code` produce authoritative 4503; unavailable state or readiness produces retryable 1013. The inner Durable Object forwarding gate also uses 1013 when its volatile running flag is false, preventing a transient runtime reading from falsely stopping the browser while still blocking container wake. <!-- @impl: web-ui/src/stores/session.ts::shouldRetainNegativeKv --> <!-- @impl: src/routes/terminal.ts::handleWebSocketUpgrade --> <!-- @impl: src/container/index.ts::fetch -->
