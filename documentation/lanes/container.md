...
For Classic Pi, each successful root `/resume` selection atomically replaces the current binding. Repeated switches therefore preserve the last active transcript across restart without allowing startup, child sessions, or malformed transcript files to replace it. <!-- @impl: preseed/agents/pi/extensions/classic-session-binding.ts::adoptClassicResumedSession -->
