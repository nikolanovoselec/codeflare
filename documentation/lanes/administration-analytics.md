...
Administration discovers Dynamic Routes from the configured AI Gateway. Each configured route has a positive context window and one active capability-profile revision; unrelated unconfigured gateway routes remain visible without blocking Save. The assignable built-ins are exactly `openai-gpt-chat-tools-reasoning`, `openai-gpt-chat-tools-off`, `workers-ai-gemma-thinking`, `workers-ai-kimi-k-thinking`, `workers-ai-glm-thinking`, and `codeflare-inference-mesh-binary-thinking`. GPT-OSS tool replay, Gemini Chat Completions tools, GPT-6 Astra tools, and Responses-required behavior appear only as nonassignable notices. ([REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration))

Each route shows **Detected models** automatically, followed by its profile selector, context window, **Map Profile**, and distinct **Verify Profile** action. Loading or refreshing inventory is read-only: it neither changes assignments nor starts paid compatibility checks. The gateway still chooses conditional and fallback legs after route-wide translation. Profile limitations, route version, and administrator-declared custom-provider backend descriptions remain under **Profile and gateway reference**. Changing a backend description invalidates its evidence and requires Save before Verify. ([REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

Choose an existing reasoning profile for a route and **Save**; discovery is optional. Alternatively, **Map Profile** starts the bounded Pi 0.84.4 streaming/tool/replay check using the saved Worker-side AI Gateway connection and may incur provider usage. Every passing enabled built-in or saved custom revision appears by name and supported levels. Each named result row has an **Assign profile** button associated with that profile name. Choose one for the route draft, then Save. Different passing mappings are choices, not an ambiguity error or backend-family identification. No duplicate custom revision is created for an existing match. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))
...
Verify Profile presents **Selected profile checks** as a level-by-check table: Compatibility, Tool call, and Tool replay. Green Passed, red Failed, and orange Unclear labels distinguish evidence from incomplete or unattempted checks. Off-disabled evidence is separate. Accepted level fields and successful Pi tool replay do not prove increasing reasoning effort. Multi-model success stays orange **Observed path passed**: the gateway exercised one path, not every backend, and Codeflare does not fabricate per-leg records or green whole-route verification. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

Verification reads inventory before and after the check. Only complete, current Verified evidence with tool replay and the Chat Completions ingress attaches automatically to the draft's exact profile reference, for one unchanged node, provider, model, backend description, and route version. Save keeps that evidence through the existing configuration document. Reloaded green status requires matching fresh single-model inventory and complete saved evidence; inventory alone never certifies compatibility. Changing profiles invalidates evidence even when switching back. Starting a recheck invalidates earlier evidence, so failure cannot retain green status. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-040](../../sdd/spec/enterprise-mode.md#req-enterprise-040-enterprise-pi-check-lifecycle))

A new custom profile must be saved before Verify; the disabled action explains why. Global and group default reasoning choices come only from their default route's assigned profile, not verification results. Associated helpers explain loading, missing assignment, and single-mode profiles. A one-mode selector is disabled, but hidden form values retain that mode for Save. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-039](../../sdd/spec/enterprise-mode.md#req-enterprise-039-enterprise-pi-default-reasoning-controls))

The interface marks the named draft unsaved until the Save confirmation succeeds. Save uses the existing preview/Apply machinery and the same configuration revision; the server recomputes required warnings and rejects stale revisions. Custom revisions and route assignments are validated together and written as one `setup:reasoning_configuration` document. Reloading reads that saved document rather than relying on the editor's local draft. ([REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection), [REQ-ENTERPRISE-036](../../sdd/spec/enterprise-mode.md#req-enterprise-036-enterprise-pi-custom-profile-draft-lifecycle)) <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @impl: src/lib/admin-configuration.ts::buildConfigurationPreview -->
