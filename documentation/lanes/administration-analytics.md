...
Custom profiles are immutable revisions created by discovery rather than by asking administrators to author request paths and scalar mappings. The administrator opens discovery for one auto-discovered Dynamic Route, starts the bounded Pi 0.84.4 streaming/tool/replay check using the already-saved Worker-side AI Gateway connection, reviews the compatible protocol behavior, and supplies a profile name. The result does not claim to identify the backend model family: multiple families can share the same request mapping. Continuing adds an unassigned, inactive revision to the configuration draft and opens Review immediately.

Incomplete, stale, heterogeneous, unsupported, inconclusive, or ambiguous observations produce no generated revision; existing enabled profiles remain available for explicit warned testing during Review. The interface marks the named draft unsaved until **Apply change** succeeds. Preview and Apply use the same configuration revision; Apply recomputes required warnings and rejects stale revisions. Custom revisions and assignments are validated together and written as one `setup:reasoning_configuration` document. ([REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-administration-workflow)) <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields --> <!-- @impl: src/lib/admin-configuration.ts::buildConfigurationPreview -->
