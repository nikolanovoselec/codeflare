...
The gateway, not Codeflare, chooses conditional and fallback legs after request translation, so runtime activation is route-wide. The primary Administration view therefore gives each route one **Map Profile** action beside its context window and reasoning-profile selector. Inventory records each reachable leg, conditional/fallback path, profile reference, and sanitized compatibility record, but remains inside **Advanced route details** with the explicit **Verify selected profile** diagnostic because per-leg records document observed behavior and never select the runtime profile. Custom-provider backend identity is administrator-declared provenance; changing it makes its compatibility record stale. A shared level is reported only when every reachable leg has current Pi tool/replay observations through Chat Completions and byte-identical normalized removals and scalar writes. ([REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration))

Choose an existing reasoning profile for a route and **Save**; discovery is optional. Alternatively, **Map Profile** starts the bounded Pi 0.84.4 streaming/tool/replay check using the saved Worker-side AI Gateway connection and may incur provider usage. Every passing enabled built-in or saved custom revision appears by name and supported levels. Choose which one to **Assign** to the route draft, then Save. Different passing mappings are choices, not an ambiguity error or backend-family identification. No duplicate custom revision is created for an existing match. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))
...
Both mapping and advanced selected-profile verification default to 4,096 completion tokens per request and allow an explicit ceiling up to 16,384. A tool call or replay that exhausts the ceiling is incomplete, not proof of incompatibility. Increase the ceiling and initiate another run only when wanted; there is no automatic escalation. Diagnostics identify the failed stage, tested modes, and safe failure category without retaining generated content or provider error text. Authentication, quota, server, transport, and malformed-stream failures stop the entire scan. Ambiguous or wholly unsuccessful observations produce no generated revision; existing enabled profiles remain available for explicit warned testing during Review. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection), [REQ-ENTERPRISE-037](../../sdd/spec/enterprise-mode.md#req-enterprise-037-enterprise-pi-custom-profile-generation)) <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility -->
