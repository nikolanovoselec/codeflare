...
Administration discovers Dynamic Routes from the configured AI Gateway. Each activated route has a positive context window and one live-verified or administrator-confirmed Pi compatibility-profile revision; unrelated unconfigured gateway routes remain visible without blocking Save. The assignable built-ins are exactly `openai-gpt-chat-tools-reasoning`, `openai-gpt-chat-tools-off`, `workers-ai-gemma-thinking`, `workers-ai-kimi-k-thinking`, `workers-ai-glm-thinking`, and `codeflare-inference-mesh-binary-thinking`. GPT-OSS tool replay, Gemini Chat Completions tools, GPT-6 Astra tools, and Responses-required behavior appear only as nonassignable notices. ([REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration))

Connection, Routes, Native providers, and Access & fallback are separate sections. Routes start as a compact status overview; expanding one shows its detected models, Pi compatibility profile, context window, Discover Profile, optional Verify Profile, and Mark as verified. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

Inventory reads neither change assignments nor start paid checks. The route editor omits advanced revision/provenance metadata and does not require custom-backend descriptions. Existing descriptions remain stored; a live check without custom provenance proves only the observed path. Built-in labels identify tested providers: Workers AI · Kimi/GLM/Gemma, OpenAI · GPT, and Codeflare Inference Mesh · Qwen/Ornith. Labels do not identify the current backend or change immutable references. ([REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

Native providers lists added provider-model targets as collapsed Ready or Not ready rows and reveals target fields and profile actions only when expanded. Current verification or administrator confirmation makes a target Ready and available to access policies without a separate activation control. Group and fallback controls show the provider and exact model while saved policy identity remains an opaque native handle. ([REQ-ENTERPRISE-054](../../sdd/spec/enterprise-mode.md#req-enterprise-054-native-target-profile-and-lifecycle-administration), [REQ-ENTERPRISE-056](../../sdd/spec/enterprise-mode.md#req-enterprise-056-native-target-disclosure-and-readiness)) <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::PolicyFields -->

Choose an existing Pi compatibility profile, then either Verify Profile or explicitly Mark as verified based on your own assessment. Assign group access and confirm Save to activate it. Administrator confirmation performs no model probes and remains visibly distinct from live verification. Alternatively, Discover Profile runs the bounded Pi 0.84.4 check and may incur provider usage. Every passing enabled built-in or saved custom revision appears by name and supported levels. Assign profile selects the exact revision in the draft; mapping alone does not authorize activation. Different passing mappings remain choices, not ambiguity errors or backend-family identification. No duplicate custom revision is created for an existing match. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))
...
Both UI checks use a fixed 4,096 completion tokens per request, without token controls or a secondary Check button. Discover Profile starts once when its editor opens; Verify Profile starts only on explicit activation. Neither retries nor escalates automatically. Exhausted tool calls or replays remain incomplete, not incompatible. Discovery shows compatible choices and concise incomplete-check notices, without candidate diagnostic panels. A failed candidate does not color another matched profile. Selected-profile verification retains its technical disclosure. Authentication, quota, server, transport, and malformed-stream failures stop the scan. After HTTP 429, completed matches remain selectable with a rate-limit notice; incomplete candidates supply no recommendations. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

Mapping and verification show one compact indeterminate progress indicator beside the active route, without repeated headings or a duplicate bottom status. After mapping ends, another profile can be selected without refreshing. ([REQ-ENTERPRISE-041](../../sdd/spec/enterprise-mode.md#req-enterprise-041-enterprise-pi-administrator-workspace))
...
Verification compares inventory before and after the check. Complete selected-profile success returns a temporary server receipt bound to the profile, gateway credential context, topology, any supplied provenance, and canary. Explicit administrator confirmation uses the same receipt and Save machinery with a distinct administrator method and no fabricated live results. Save validates that receipt or unchanged saved authority; client evidence flags cannot create eligibility. Reload requires current server-owned verification and matching fresh inventory. Profile or connection changes invalidate draft eligibility, and a failed recheck cannot retain it. A checked inactive draft can be saved without granting access. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-040](../../sdd/spec/enterprise-mode.md#req-enterprise-040-enterprise-pi-check-lifecycle))

New custom profiles can be live-verified or administrator-confirmed before Save. Group and optional fallback policies offer only eligible routes. A single available route is preselected when adding a group; new reasoning defaults prefer Medium, then Off, then the first supported level. Provider-controlled profiles show Provider default instead of falsely reporting reasoning as Off; they expose no selectable Pi reasoning level until that transport has verified control semantics. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-039](../../sdd/spec/enterprise-mode.md#req-enterprise-039-enterprise-pi-default-reasoning-controls), [REQ-ENTERPRISE-044](../../sdd/spec/enterprise-mode.md#req-enterprise-044-enterprise-pi-minimum-save-and-access-policies), [REQ-ENTERPRISE-045](../../sdd/spec/enterprise-mode.md#req-enterprise-045-pi-compatibility-profile-communication))

A checked gateway connection, a verified inactive Dynamic Route profile, and complete disabled provider drafts can be saved before group assignments exist; only verified or administrator-confirmed targets become eligible for access. Unfinished routes do not block Save. Disabled fallback gives unmatched users no routes. The first matching configured group remains authoritative even when empty; it never falls through to a different group or fallback. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-044](../../sdd/spec/enterprise-mode.md#req-enterprise-044-enterprise-pi-minimum-save-and-access-policies), [REQ-ENTERPRISE-066](../../sdd/spec/enterprise-mode.md#req-enterprise-066-native-provider-draft-persistence-before-access), [REQ-ENTERPRISE-069](../../sdd/spec/enterprise-mode.md#req-enterprise-069-dynamic-route-profile-persistence-before-access))
