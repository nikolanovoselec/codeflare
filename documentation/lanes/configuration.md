...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-target-administration). It stores exact Bedrock model selectors, raw default provider-config identity, profile reference, enabled state, and verification Worker-side. Browser and container projections use only derived `cf-native-<uuid>` handles and safe labels. | Administration AI Routing → KV (no redeploy) |
...
The seven immutable revision-1 built-ins are `openai-gpt-chat-tools-reasoning`, `openai-gpt-chat-tools-off`, `workers-ai-gemma-thinking`, `workers-ai-kimi-k-thinking`, `workers-ai-glm-thinking`, `codeflare-inference-mesh-binary-thinking`, and `bedrock-anthropic-compat`. The catalog also exposes exactly four nonassignable notices: GPT-OSS tool replay, Gemini Chat Completions tools, GPT-6 Astra tools, and Responses-required behavior. <!-- @impl: src/lib/reasoning-profiles.ts::BUILT_IN_REASONING_PROFILES --> <!-- @impl: src/lib/reasoning-profiles.ts::COMPATIBILITY_NOTICES -->
...
**Native Bedrock targets:** Under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-target-administration) and [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-bedrock-provider-default-capability-verification), the Native providers section detects whether the gateway has one default `aws-bedrock` provider configuration. Detection proves presence only. Administrators enter an exact model or inference-profile identifier and a context window greater than 16,384; route-derived names are optional suggestions, not an allowlist. Verify Profile runs a bounded compat tool-call and replay canary. Mark as verified records explicit administrator authority without claiming live evidence. Raw provider IDs and exact model selectors remain Worker-side; Pi and Copilot receive opaque handles. The **AWS Bedrock · Anthropic Claude** profile uses provider-default reasoning, with no Off, Medium, or High claim.
