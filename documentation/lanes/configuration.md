...
| `setup:aig_gateway_id` | Enterprise only: the non-secret AI Gateway name required with the account API URL. It is used for Dynamic Route management paths and the `cf-aig-gateway-id` inference header. Legacy URLs carry this name in the URL and do not need the separate value. | Administration → KV |
