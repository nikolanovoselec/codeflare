...
`setup:reasoning_configuration` atomically contains custom revisions and exact route assignments. Built-ins remain in code, and `setup:route_context_windows` remains a separate numeric map. Until Administration Apply creates the atomic document, runtime may derive valid legacy GLM/Kimi assignments from the historical combined route settings in memory; malformed, ambiguous, or GPT-OSS legacy state fails closed and is never persisted automatically. Each assignment may retain active-route version, leg provenance/evidence, and a common mapping, but runtime always uses its one `activeProfile`: AI Gateway chooses conditional/fallback legs only after translation. A common level requires current Pi tool/replay evidence and byte-identical removal/write mappings across every reachable leg. Missing, stale, unsupported, inconclusive, or heterogeneous evidence warns during Review; a structurally valid enabled Chat Completions profile may be activated only after explicit warning confirmation.
