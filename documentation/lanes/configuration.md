...
Wrangler SSH is explicitly enabled for running container instances. ([REQ-OPS-014](../../sdd/spec/operations.md#req-ops-014-container-binding-and-scaling-from-image)) <!-- @impl: wrangler.toml::containers.ssh --> It exposes no public port, and connecting requires a configured `ssh-ed25519` public key plus account-level Containers Edit (`cloudchamber.write` for OAuth); see Cloudflare's [SSH guide](https://developers.cloudflare.com/containers/guides/ssh/) and [account permission reference](https://developers.cloudflare.com/fundamentals/api/reference/permissions/#account-permissions).
