...
Provider-native Bedrock uses one shared transaction boundary for replay validation and initial-versus-continuation transport. A completed tool transaction remains exact and fail-closed. A provider tool block becomes executable output and stored replay only after native `tool_use`; `max_tokens` retains safe text and usage but cannot publish a partial call.
