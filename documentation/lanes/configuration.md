...
| `setup:strict_egress` | Enterprise only: Strict Gateway Egress on/off toggle (`'active'`/`'inactive'`, **default OFF** when absent). When ON, **all** container HTTP/HTTPS egress is forced through the Workers VPC `EGRESS` binding → the account's Cloudflare (Zero Trust) Gateway: the per-host LLM/GitHub interceptors swap their single upstream `fetch` to `env.EGRESS.fetch`, and every other host rides the `EgressController` catch-all (`interceptOutboundHttps('*', …)`). **Fails closed** (503 `EGRESS_UNAVAILABLE`) when the `[[vpc_networks]]` `EGRESS` binding is unbound, so OFF + unbound is inert. No new GitHub var/secret and no `deploy.yml` change; rollback = flip the toggle OFF. See [REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress), [security.md](./security.md#strict-gateway-egress-enterprise-mode), and [deployment.md](./deployment.md#strict-gateway-egress-enterprise-mode). | Setup wizard → KV (re-run setup to change; no redeploy) |
