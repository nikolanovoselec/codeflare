...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-target-administration). It stores the exact native/custom provider and model selector, raw provider-config identity and alias, immutable profile reference, enabled state, and verification Worker-side. Browser and container projections use only derived `cf-native-<uuid>` handles and safe labels. | Administration AI Routing → KV (no redeploy) |
...
**Native provider targets:** Under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-target-administration) and [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-native-provider-capability-verification), the Native providers section shows every gateway-scoped provider configuration. A sole binding is selectable; multiple bindings require one unambiguous default. Custom-provider discovery determines the Worker-owned `custom-<slug>` selector. Detection proves presence only. Administrators enter an exact model identifier and a context window greater than 16,384, then select, discover, or save an immutable built-in/custom profile revision. Verify Profile runs a bounded direct-compat tool-call and replay canary; Mark as verified records explicit administrator authority without claiming live evidence. Raw provider IDs, aliases, custom classification, and exact selectors remain Worker-side; Pi and Copilot receive opaque handles.
