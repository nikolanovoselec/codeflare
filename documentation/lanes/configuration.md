...
| Per-route context windows (`setup:route_context_windows`) | Enterprise only: JSON `{route: tokens}` map of each dynamic route's model context window (positive ints). Set per route in the **setup wizard** (prefilled with the `256000` default, editable + resettable), stored in KV (no redeploy), fanned to the container as `ENTERPRISE_ROUTE_CONTEXT_WINDOWS` so Pi's `models.json` advertises each route's real window instead of falling to its built-in 128k default. A route with no stored value uses the default. | `256000` per route | no | Setup wizard -> KV (enterprise) | [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) |
...
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | S3 compatibility (non-enterprise mirrors the R2 keys). **Omitted entirely in enterprise** — rclone reads its creds from rclone.conf (`R2_*`), so these are redundant there, and their presence surfaced Pi's built-in `amazon-bedrock` provider in `/model`; dropping them keeps the picker routes-only ([REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC7). `R2_*` are still emitted in enterprise. | - | no | Mirrors R2 keys (non-enterprise only) | [REQ-STOR-001](../../sdd/spec/storage.md#req-stor-001-dedicated-per-user-r2-bucket), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) |
| `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` | Connect-to-Cloudflare deploy creds for the container. **Omitted in enterprise** — that feature does not exist in enterprise, so the deploy token must never enter the container ([REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC7, [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers)). Emitted in non-enterprise only when present. | - | no | Worker -> DO (non-enterprise only) | [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls), [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers) |
...
| `setup:route_context_windows` | JSON `{route: tokens}` map of each dynamic route's context window (positive ints; default `256000` per route when absent). Fanned to the container as `ENTERPRISE_ROUTE_CONTEXT_WINDOWS` for Pi's `models.json`. `configure` deletes the key when the map is empty. See [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list) AC8. | Setup wizard → KV (re-run setup to change; no redeploy) |
