...
| `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` | Non-enterprise: a per-user **PAT** deploy token passes its real value to the container (emitted when present). A **Connect-to-Cloudflare OAuth** session (`cloudflareTokenSource='oauth'`) instead receives only the non-secret placeholder `codeflare-oauth` (`CLOUDFLARE_OAUTH_TOKEN_PLACEHOLDER`) — the short-lived OAuth access token never enters the container; a freshly-refreshed token is stamped worker-side by the `CloudflareBrowserInterceptor` (OAuth mode) at the `api.cloudflare.com` boundary, so `wrangler` + browser-run survive past the access-token expiry ([REQ-AGENT-078](../../sdd/spec/agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary)). **Enterprise:** that feature is absent, so the deploy token never enters the container — `CLOUDFLARE_API_TOKEN` is only the non-secret browser-run placeholder `codeflare-enterprise` (real Browser Rendering token injected worker-side by the CloudflareBrowserInterceptor, [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), and `CLOUDFLARE_ACCOUNT_ID` is the wizard-configured Browser Rendering account (non-secret). | - | no | Worker -> DO | [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls), [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers) |
