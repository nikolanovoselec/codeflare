...
Until Administration Apply creates the atomic document, runtime may derive valid legacy GLM/Kimi assignments from the historical combined route settings in memory. Malformed, ambiguous, or GPT-OSS legacy state fails closed and is never persisted automatically. ([REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation)) A common level requires current Pi tool/replay evidence and byte-identical removal/write mappings across every reachable leg. Missing, stale, unsupported, inconclusive, or heterogeneous evidence warns during Review; a structurally valid enabled Chat Completions profile may be activated only after explicit warning confirmation.
