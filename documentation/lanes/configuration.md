...
| `setup:aig_gateway_url` | Enterprise only: the customer's non-secret AI Gateway address. Administration accepts the legacy full gateway form `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>/` and the current account API form `https://api.cloudflare.com/client/v4/accounts/<account_id>/`; pasted account API suffixes such as `/ai/v1/chat/completions` or `/ai/run` are removed automatically. Wizard-first source with `AIG_GATEWAY_URL` as fallback ([REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)). | Setup wizard or Administration → KV |
