...
| `AIG_TOKEN` | **A Cloudflare API token with the "Workers AI" permission** (`Read` is the inference scope). It is sent as `Authorization: Bearer` to `api.cloudflare.com/client/v4/accounts/<acct>/ai/v1/*` — an endpoint in the **Workers AI** namespace; the `cf-aig-gateway-id` header (parsed from `AIG_GATEWAY_URL`) is what routes the call through the gateway (see [AD74](decisions/README.md#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api)). **Two credential types are rejected with `error 10000`:** (a) an AI Gateway *authentication token* (the `cfut_`-style token minted in the gateway's own settings — it only works on the deprecated `gateway.ai.cloudflare.com` path), and (b) a CF API token scoped to **"AI Gateway: Run"** (the wrong permission for this endpoint — confirmed live 2026-06-05). Note the `cfut_` prefix is shared across CF API token types and does **not** indicate scope — verify the permission, not the prefix. **⚠️ Do _not_ use the gateway's _Authenticated Gateway_ → "Create authentication token" button**: despite Cloudflare's docs pointing at it, that button mints an **"AI Gateway: Run"** token which this `/ai/v1/*` endpoint rejects with `error 10000` (confirmed by the operator). Create the token **manually** with the **Workers AI** permission instead. Read exclusively by the `LlmInterceptor` WorkerEntrypoint; never injected into the container. | GitHub Actions secret -> `wrangler secret put AIG_TOKEN` (deploy.yml) |
