...
| Per-route context windows (`setup:route_context_windows`) | Enterprise only: JSON `{route: tokens}` map of each dynamic route's model context window (positive ints). Set per route in the **setup wizard** (prefilled with `256000`, editable + resettable), stored in KV (no redeploy), fanned as `ENTERPRISE_ROUTE_CONTEXT_WINDOWS` to Pi's `models.json`. Absent routes default to `256000`. | `256000` per route | no | Setup wizard -> KV (enterprise) | [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) |
...
| `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` | Non-enterprise: the real Connect-to-Cloudflare deploy token + account (emitted when present). **Enterprise:** that feature is absent, so the deploy token never enters the container — `CLOUDFLARE_API_TOKEN` is only the non-secret browser-run placeholder `codeflare-enterprise` (real Browser Rendering token injected worker-side by the CloudflareBrowserInterceptor, [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)), and `CLOUDFLARE_ACCOUNT_ID` is the wizard-configured Browser Rendering account (non-secret). | - | no | Worker -> DO | [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls), [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers) |
...
| `CONTEXT_MODE_BRIDGE_IDLE_MS` | Disables context-mode bridge-child idle reaping/noisy idle-release notices when `0`; set by entrypoint and reinforced by the preseeded Pi runtime extension | `0` (via entrypoint) | no | entrypoint / `context-mode-runtime.ts` | [REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) AC1 |
