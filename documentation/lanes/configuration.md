...
Provider-native Bedrock uses one shared `classifyBedrockToolTurn` classification for replay validation and initial-versus-continuation transport. A new turn requires nonempty text-only user input after all earlier tool-call IDs/results are paired without orphan or duplicate ambiguity; interrupted tools need no final assistant answer. Image-bearing, unknown, or empty content is not a boundary. Absent historical signed state is allowed; available blocks must restore exactly and match the complete tool-call identity, malformed stored blocks fail, and absent active adaptive-thinking state fails before provider I/O. Profiles and one-operation/no-fallback transport authority remain unchanged. See [REQ-ENTERPRISE-073](../../sdd/spec/enterprise-mode.md#req-enterprise-073-provider-native-bedrock-replay-integrity), [REQ-ENTERPRISE-076](../../sdd/spec/enterprise-mode.md#req-enterprise-076-provider-native-bedrock-protocol-translation), [REQ-ENTERPRISE-077](../../sdd/spec/enterprise-mode.md#req-enterprise-077-provider-native-bedrock-transport-dispatch), [REQ-ENTERPRISE-079](../../sdd/spec/enterprise-mode.md#req-enterprise-079-provider-native-bedrock-replay-confidentiality), and Anthropic's [thinking guidance](https://platform.claude.com/docs/en/build-with-claude/thinking). Regression coverage is fixture-based, not paid or live-provider validation.
