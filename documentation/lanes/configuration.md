...
**`setup:active_agents`:** Enterprise only: JSON array of the coding agents users can pick when starting a new session, chosen in the Setup wizard's Coding Agents section (`configure_active_agents` step). Enforced identically on every session-creation surface (the dashboard New Session dialog and the GitHub clone picker). The selectable universe is capped at the gateway-capable coding agents (`copilot`, `pi` — [AD74](../decisions/README.md#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api) excludes Claude Code); `configure` rejects an empty or non-capable selection (minimum one), and `bash` is always selectable and never stored. An absent or invalid value resolves to **all capable agents active** (pre-feature behavior). Enforced at session creation and `lastAgentType` preference writes; existing sessions keep a later-deactivated agent (creation-only gate). Wizard contract in [REQ-ENTERPRISE-025](../../sdd/spec/enterprise-mode.md#req-enterprise-025-active-coding-agents-configured-in-the-setup-wizard); enforcement in [REQ-ENTERPRISE-003](../../sdd/spec/enterprise-mode.md#req-enterprise-003-agent-allowlist-in-enterprise-mode).
