...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document under [REQ-ENTERPRISE-051](../../sdd/spec/enterprise-mode.md#req-enterprise-051-native-ai-gateway-target-administration-workspace). It stores the exact native/custom provider and model selector, raw provider-config identity and alias, immutable profile reference, enabled state, and verification Worker-side. Browser and container projections use only derived `cf-native-<uuid>` handles and safe labels. | Administration AI Routing → KV (no redeploy) |
