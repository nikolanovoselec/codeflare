...
| `REVIEW_LANE_TIMEOUT` | Seconds a PR-boundary review lane subprocess may run before it is reaped. A zero, empty, or non-numeric value resolves to the default rather than disabling the bound (`timeout 0` means no timeout at all). Expiry escalates to `SIGKILL` 30s after `SIGTERM`. | `1800` | no | Lane process env | [REQ-AGENT-102](../../sdd/spec/agents.md#req-agent-102-claude-reviewer-headless-lane-transport) |
...
| `CODING_AGENTS` | Canonical non-empty coding-agent selection for image installation and Worker R2 projection. Accepted values: `claude-code`, `codex`, `copilot`, `antigravity`, `opencode`, `pi`; unset selects all. Invalid explicit values fail before user-bucket mutation. | all six coding agents | no | Environment-scoped GitHub Actions variable / Worker runtime env <!-- @impl: scripts/ci/coding-agent-selection-core.mjs::resolveCodingAgents --> | [REQ-OPS-038](../../sdd/spec/operations.md#req-ops-038-build-selected-coding-agent-clis), [REQ-OPS-040](../../sdd/spec/operations.md#req-ops-040-selected-coding-agent-packaging), [REQ-OPS-039](../../sdd/spec/operations.md#req-ops-039-reduced-image-capability-preservation) |
...
| `CONTAINER_SSH_PUBLIC_KEY` | Optional single-line OpenSSH `ssh-ed25519` public key. A repository value enables Wrangler SSH with one `codeflare-operator` authorized key across deployment environments; absence keeps SSH disabled. The matching private key remains outside GitHub and the deployment. | - | no | Repository-level GitHub Actions secret; same-name environment secrets must remain absent because GitHub gives them precedence | [REQ-OPS-060](../../sdd/spec/operations.md#req-ops-060-optional-persistent-container-ssh-authorization) |
...
| `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` | rclone auth. **Enterprise strict-egress only:** the non-secret placeholder `codeflare-enterprise` (`ENTERPRISE_R2_KEY_PLACEHOLDER`) — the real R2 key never enters the container; `EgressController` strips it and re-signs only the bound bucket with the Worker-held user-scoped key. Non-enterprise / strict-off keeps the real key. | - | no | Worker -> DO (preferred) or DO `this.env` fallback | [REQ-STOR-001](../../sdd/spec/storage.md#req-stor-001-dedicated-per-user-r2-bucket), [REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress), [REQ-ENTERPRISE-026](../../sdd/spec/enterprise-mode.md#req-enterprise-026-strict-r2-interception-preserves-user-bucket-authority) |
...
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | **Not emitted in any mode.** S3-compat duplicates of the R2 keys with no consumer (rclone reads rclone.conf built from `R2_*`); their only effect was surfacing Pi's `amazon-bedrock` provider in `/model`, so they were dropped ([REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC6). `R2_*` are authoritative. | - | no | (removed) | [REQ-STOR-001](../../sdd/spec/storage.md#req-stor-001-dedicated-per-user-r2-bucket), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) |
| `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` | Non-enterprise PAT sessions send their token. OAuth sessions receive `codeflare-oauth`, refreshed worker-side at `api.cloudflare.com` ([REQ-AGENT-078](../../sdd/spec/agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary); [security](security.md#non-enterprise-cloudflare-oauth-token)). Enterprise sends no deploy token: only `codeflare-enterprise` ([REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container)) and the non-secret Browser Rendering account id. | - | no | Worker -> DO | [REQ-AGENT-078](../../sdd/spec/agents.md#req-agent-078-cloudflare-oauth-token-refreshed-at-the-apicloudflarecom-boundary), [REQ-BROWSER-008](../../sdd/spec/browser-run.md#req-browser-008-browser-rendering-token-interception-never-in-the-container), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls), [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers) |
...
| `CODEFLARE_TERMINAL_MODE` | Immutable session terminal ownership (`classic` or `herdr`); missing/invalid stored values resolve classic before transport | `classic` | no | Worker -> DO -> container host | [AD146](../decisions/README.md#ad146-terminal-mode-is-an-immutable-per-session-choice) <!-- @impl: src/container/container-env.ts::buildEnvVars --> |
| `TAB_CONFIG` | JSON configured-session-command snapshot; classic carries entries `1`-`6`, Herdr carries only entry `1`; also supplies Browser IDE agent inventory | - | no | Worker -> DO; terminal host; Herdr launcher; Browser IDE setup | [REQ-AGENT-002](../../sdd/spec/agents.md#req-agent-002-agent-selection-at-session-creation), [REQ-AGENT-003](../../sdd/spec/agents.md#req-agent-003-agent-cli-auto-started-in-tab-1), [REQ-TERM-005](../../sdd/spec/terminal.md#req-term-005-herdr-runtime-and-configured-agent-startup) |
...
| `CONTAINER_AUTH_TOKEN` | Auth token for container API calls, scoped to one DO lifecycle. See [security.md](./security.md#container-auth-token-req-sec-012-req-sec-022). | - | no | DO-generated; container host | [REQ-SEC-012](../../sdd/spec/security.md#req-sec-012-container-auth-token-per-do-lifecycle), [REQ-SEC-022](../../sdd/spec/security.md#req-sec-022-container-proxy-bearer-validation), [REQ-SESSION-002](../../sdd/spec/session-lifecycle.md#req-session-002-one-container-per-session-isolation) |
| `MANUAL_TAB` | Browser IDE Bash-profile compatibility flag; `1` skips configured-agent shell autostart | - | no | Browser IDE terminal profile; shell initialization | [REQ-IDE-047](../../sdd/spec/browser-ide.md#req-ide-047-bash-first-browser-ide-terminals) |
| `FAST_CLI_START` | Disables auto-update for all 6 AI tools when `'true'` (default); when `'false'`, update suppressors are removed and installed Pi and Codex CLIs update explicitly during startup | `'true'` | no | Worker -> DO | [REQ-AGENT-012](../../sdd/spec/agents.md#req-agent-012-fast-cli-start-configurable) |
...
| `CONTEXT_MODE_BRIDGE_IDLE_MS` | Not force-set by Codeflare. `context-mode-runtime.ts` clears inherited overrides and limits context-mode ownership to one Pi session per process; in-process subagents use native transports | unset (context-mode default) | no | entrypoint / `context-mode-runtime.ts` | [REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) AC7, [REQ-AGENT-089](../../sdd/spec/agents.md#req-agent-089-pi-context-mode-foreground-ownership) |
| `CODEFLARE_CODING_AGENTS` | Canonical coding-agent set baked into the image. Startup validates it before restore, lays down only selected agent roots, and gates agent-owned relay/configuration. | all six coding agents | no | Dockerfile ENV <!-- @impl: entrypoint.sh::validate_coding_agent_selection --> | [REQ-OPS-038](../../sdd/spec/operations.md#req-ops-038-build-selected-coding-agent-clis) |
...
| `CODEFLARE_OPENAI_API_KEY` | Optional OpenAI key for consult-llm-mcp. The `CODEFLARE_` namespace stops coding agents (Pi, opencode, antigravity) auto-detecting it as their own credential; the entrypoint maps it to bare `OPENAI_API_KEY` only inside the server's scoped `env`. Not injected in enterprise mode. | - | no | Worker -> DO (from KV `llm-keys:{bucket}`) | [REQ-AGENT-031](../../sdd/spec/agents.md#req-agent-031-consult-llm-key-isolation-subscription-backend-and-multi-agent-parity), [REQ-AGENT-009](../../sdd/spec/agents.md#req-agent-009-llm-api-key-storage-encrypted-in-kv) |
...
| `ENCRYPTION_KEY` | AES-256 key (base64) for rclone SSE-C, appended to `rclone.conf` as `sse_customer_key_base64`. **Omitted in Governed Mode** (`R2_SSE_DISABLED`, [REQ-ENTERPRISE-018](../../sdd/spec/enterprise-mode.md#req-enterprise-018-governed-mode-toggle-and-configuration-surface)): SSE-C is off and the key never enters the container, which then holds no real secret but `CONTAINER_AUTH_TOKEN`. | - | no | Worker -> DO (from `env.ENCRYPTION_KEY`; not in Governed Mode) | [REQ-SEC-002](../../sdd/spec/security.md#req-sec-002-api-tokens-never-enter-containers), [REQ-SEC-005](../../sdd/spec/security.md#req-sec-005-r2-files-encrypted-at-rest-with-sse-c-when-operator-configures-an-encryption-key), [REQ-ENTERPRISE-018](../../sdd/spec/enterprise-mode.md#req-enterprise-018-governed-mode-toggle-and-configuration-surface) |
...
| `CODEFLARE_SESSION_WORKSPACE` | Immutable session workspace (`terminal` or `vscode`). Missing and invalid stored values resolve to Terminal. Clients cannot supply this environment value. | `terminal` | no | Session KV -> Worker -> DO via `setBucketName` | [REQ-IDE-048](../../sdd/spec/browser-ide.md#req-ide-048-default-workspace-and-dashboard-owned-vs-code-sessions) |
| `REMOTE_CURATION_MANIFEST_DIGEST` | Lowercase 64-hex SHA-256 of the exact Worker-synthesized `.codeflare/managed-extensions.json` bytes. | absent | required when remote curation is active | Worker -> DO; Browser IDE settings, reconciliation, and reap capture | [REQ-IDE-042](../../sdd/spec/browser-ide.md#req-ide-042-additive-company-extension-reconciliation), [REQ-IDE-045](../../sdd/spec/browser-ide.md#req-ide-045-company-extension-reconciliation-orchestration), [REQ-STOR-024](../../sdd/spec/storage.md#req-stor-024-managed-release-application) |
...
**Managed Herdr keyboard:** `/etc/codeflare/herdr/config.toml` pins Herdr v0.8.2's standard `Ctrl+B` prefix, `prefix+?` Help binding, and `prefix+s` Settings binding. In Herdr sessions, the browser terminal forwards the prefix's canonical control byte and suppresses the browser bookmark action; Herdr owns all following action keys. Classic keyboard handling is unchanged. See [REQ-TERM-037](../../sdd/spec/terminal.md#req-term-037-browser-keyboard-interaction-with-herdr).

**`REMOTE_CURATION_MANIFEST_DIGEST`:** `managedExtensionsDocumentDigest` hashes the exact UTF-8 bytes produced by the Worker's sole managed-extension-manifest serializer after successful reconciliation. The digest travels through `setBucketName` and the container environment to OpenVSCode settings preparation, live TypeScript reconciliation, and the Python generation-reap capture backstop. When active curation has no digest or the restored bytes differ, no company-extension install or uninstall runs; the live reconciler preserves current company ownership and shows the **Managed Browser IDE extensions could not be verified** remediation warning. Personal `ide-extensions.json` remains separate.
...
Container SSH authorization is independently optional. The environment-bound deploy job reads GitHub's resolved secret value, so same-name environment secrets must remain absent for the repository public key to govern every target. Deployment receives no matching private key. ([REQ-OPS-060](../../sdd/spec/operations.md#req-ops-060-optional-persistent-container-ssh-authorization), [REQ-OPS-061](../../sdd/spec/operations.md#req-ops-061-persistent-container-ssh-identity-lifecycle)) Provisioning, rotation, Vault custody, connection, and revocation are maintained in the private [persistent container SSH runbook](https://github.com/nikolanovoselec/codeflare-private/blob/main/docs/operations/container-ssh.md).

**Worker-level R2 credentials are derived from the API token** for bucket administration. Per-user scoped R2 tokens are separate, created on first login, and revoked when the API token changes. If that token is rotated, setup must be re-run.
...
Wrangler SSH is disabled by default. A valid repository-level `CONTAINER_SSH_PUBLIC_KEY` enables one persistent `codeflare-operator` authorized key across deployment environments when no same-name environment secret overrides it; malformed values stop deployment before Worker promotion. The private key remains under operator-controlled client custody. ([REQ-OPS-060](../../sdd/spec/operations.md#req-ops-060-optional-persistent-container-ssh-authorization)) <!-- @impl: scripts/ci/configure-container-ssh.mjs::configureContainerSsh --> SSH exposes no public port, and connecting requires the matching private key plus account-level Containers Edit (`cloudchamber.write` for OAuth); see Cloudflare's [SSH guide](https://developers.cloudflare.com/containers/guides/ssh/) and [account permission reference](https://developers.cloudflare.com/fundamentals/api/reference/permissions/#account-permissions).
...
| `setup:enterprise_admin_access_group` | Comma/newline-separated Cloudflare Access group names/ids granting admin (Setup / user-administration) access, parallel to the email-based Admin Users list. Resolved live per-request in `requireAdmin` (never the hot auth path); excluded from per-group routing. See [details](#admin-access-group-configuration). | Setup wizard → KV (re-run setup to change; no redeploy) |
| `setup:dynamic_routes` | JSON string array of Dynamic Route handles; routine Administration discovers them from AI Gateway before Apply, and the interceptor maps each to `dynamic/<name>` for container agents. | Setup first-run or Administration AI Routing → KV (no redeploy) |
| `setup:native_ai_targets` | Enterprise-only versioned native-target document containing Worker-owned native/custom target identity under [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-and-document), plus profile, lifecycle, and verification state. See [Native provider targets](#native-provider-targets). | Administration AI Routing → KV (no redeploy) |
| `setup:default_route` | JSON `{route, reasoning}` default route. When unset, runtime resolves the first catalog route with no reasoning default; when set, `route` must exist in `setup:dynamic_routes` and its level must be supported by the route-wide profile. | Setup first-run or Administration AI Routing → KV (no redeploy) |
| `setup:route_context_windows` | JSON map of each route's positive context window, for example `{ "development": 262144 }`. See [route model settings](#route-model-settings). | Administration → KV (no redeploy) |
| `setup:reasoning_configuration` | Bounded JSON document containing immutable custom profile revisions and exact route/leg profile references. Built-ins remain code-owned. | Administration preview/Apply → KV (no redeploy) |
| `setup:group_routing` | Optional JSON map `{ [group]: { routes: string[], defaultRoute, reasoning } }` of many-to-many per-Access-group route policies. | Setup first-run or Administration AI Routing → KV (no redeploy) |
...
| `setup:active_agents` | Enterprise only: JSON array of the active coding agents users can pick at session creation (subset of the gateway-capable `{copilot, pi}`, minimum one; `bash` always selectable). **Absent/invalid ⇒ all capable agents active.** | Setup wizard → KV (re-run setup to change; no redeploy) |
| `setup:aig_gateway_url` | Enterprise-only non-secret AI Gateway address in either supported URL form. Wizard-first source with `AIG_GATEWAY_URL` as fallback ([REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)). | Setup wizard or Administration → KV |
| `setup:aig_gateway_id` | Enterprise only: the non-secret AI Gateway name required with the account API URL under [REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard). It is used for Dynamic Route management paths and the `cf-aig-gateway-id` inference header. Legacy URLs carry this name in the URL and do not need the separate value. | Administration → KV |
| `setup:aig_token` | Enterprise only: the AI Gateway token, **encrypted at rest** (kv-crypto, AAD bound to the key), masked on prefill, and preserved on blank under [REQ-ENTERPRISE-062](../../sdd/spec/enterprise-mode.md#req-enterprise-062-ai-gateway-setup-persistence-boundaries). | Setup wizard → KV (re-run setup to change; no redeploy) |

Additional details:

<a id="route-model-settings"></a>
**Route model settings:** Administration auto-discovers route names from the configured AI Gateway, then assigns one exact executable capability-profile revision and one positive context window to each route. The same Worker-side token used for inference must carry **Workers AI**, **AI Gateway Run**, and now **AI Gateway Read** so the control surface can list routes and inspect active versions. Legacy GLM/Kimi assignments are proposed for migration, while GPT-OSS remains unresolved. See [REQ-ENTERPRISE-022](../../sdd/spec/enterprise-mode.md#req-enterprise-022-per-route-context-windows-for-dynamic-routes), [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), and [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation).

The ten immutable revision-1 built-ins are `openai-gpt-chat-tools-reasoning`, `openai-gpt-chat-tools-off`, `workers-ai-gemma-thinking`, `workers-ai-kimi-k-thinking`, `workers-ai-glm-thinking`, `codeflare-inference-mesh-binary-thinking`, `native-google-ai-studio-compat`, `native-openai-compat`, `native-codeflare-inference-mesh-compat`, and `bedrock-anthropic-compat`. The catalog also exposes exactly four nonassignable notices: GPT-OSS tool replay, the older unadapted Gemini Chat Completions path, GPT-6 Astra tools, and Responses-required behavior. <!-- @impl: src/lib/reasoning-profiles.ts::BUILT_IN_REASONING_PROFILES --> <!-- @impl: src/lib/reasoning-profiles.ts::COMPATIBILITY_NOTICES -->

Custom revisions use only the `ai-gateway-chat-completions` ingress and literal string, finite-number, boolean, or null writes. The document is limited to 256 KiB, 32 custom IDs, and 64 retained revisions; IDs are 64 characters, names 128, descriptions/operator notes/limitations 512, with at most 16 limitations. Each distinct level permits at most 16 removal paths and 16 writes; paths are 128 characters and four object-key segments with no array indexing, protected request roots, or transport/provider controls. Scalar strings are limited to 256 characters and each revision retains at most 20 sanitized validation/evidence summaries. Validation rejects rather than truncates. Revisions are hash-addressed and immutable; referenced revisions cannot be disabled or collected.

**Native provider targets:** Under [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-provider-discovery-and-selection), [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-native-provider-capability-catalog), [REQ-ENTERPRISE-052](../../sdd/spec/enterprise-mode.md#req-enterprise-052-native-provider-verification-and-runtime-enforcement), [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-and-document), [REQ-ENTERPRISE-060](../../sdd/spec/enterprise-mode.md#req-enterprise-060-native-target-input-validation), [REQ-ENTERPRISE-061](../../sdd/spec/enterprise-mode.md#req-enterprise-061-native-target-administration-projection), and [REQ-ENTERPRISE-055](../../sdd/spec/enterprise-mode.md#req-enterprise-055-native-target-authority-and-save), a sole provider binding is selectable while multiple bindings require one unambiguous default. Custom-provider discovery determines the Worker-owned `custom-<slug>` selector. Detection proves presence only.

Administrators enter an exact model identifier and a context window greater than 16,384, then select, discover, or save an immutable built-in/custom profile revision. Bedrock identifiers reject path, URL, ARN, traversal, wildcard, query, and fragment forms; other providers retain the bounded slash/colon syntax their model identifiers may require. Verify Profile runs a bounded direct-compat tool-call and replay canary; Mark as verified records explicit administrator authority without claiming live evidence. Raw provider IDs, aliases, custom classification, and exact selectors remain Worker-side; Pi and Copilot receive opaque handles. Save rejects a Dynamic Route name that collides with a submitted opaque native handle and derives typed policy references from submitted target identity rather than the handle prefix alone.

Administration labels configured rows and Access choices as `Dynamic Route - <route name>` or `Native Route - <provider> - <exact model>`. Built-in compatibility profiles use the same category prefix followed by provider and supported model family, while stored route names, target handles, and profile references remain unchanged. ([REQ-ENTERPRISE-064](../../sdd/spec/enterprise-mode.md#req-enterprise-064-route-and-compatibility-profile-presentation))

The Worker translates runtime reasoning hints to an exact executable level, otherwise the next higher, otherwise the highest lower level within the assigned profile. Cataloged Bedrock, Gemini, and Mesh provider-default profiles discard client reasoning overrides while preserving unrelated fields and tool replay; the OpenAI native profile maps only `off` to `reasoning_effort: none`, and GPT-6 Astra remains excluded. Gemini preserves its bounded opaque thought signature through Pi replay metadata, while Bedrock alone receives repeated-tool-name repair. See [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-native-provider-capability-catalog) and [REQ-ENTERPRISE-059](../../sdd/spec/enterprise-mode.md#req-enterprise-059-native-provider-wire-adaptation). <!-- @impl: src/lib/reasoning-profiles.ts::BUILT_IN_REASONING_PROFILES --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor -->

Pi offers all seven thinking choices for provider-default routes; these are client hints, not newly supported backend controls. The Worker still discards overrides, and backend profiles remain unchanged. Startup publishes `Dynamic Route - <route name>` and `Native Route - <native label>` as model names. All gateway model pickers, thinking settings, selection/default status messages, and the footer use those published names; selection and requests retain unchanged routing IDs, including opaque native handles. See [REQ-ENTERPRISE-058](../../sdd/spec/enterprise-mode.md#req-enterprise-058-native-model-container-publication) and [REQ-ENTERPRISE-082](../../sdd/spec/enterprise-mode.md#req-enterprise-082-pi-native-model-display).

Provider-native Bedrock uses one shared transaction boundary for replay validation and initial-versus-continuation transport. A completed tool transaction remains exact and fail-closed. A provider tool block becomes executable output and stored replay only after native `tool_use`; `max_tokens` retains safe text and usage but cannot publish a partial call. See [REQ-ENTERPRISE-085](../../sdd/spec/enterprise-mode.md#req-enterprise-085-provider-native-bedrock-incomplete-tool-recovery).

If Pi already retained such a proposal, a later nonempty text-only user turn may abandon it only while it has zero tool results; safe assistant prose remains, while partial results, orphan/duplicate ambiguity, assistant boundaries, and non-text user content stay protected. This structural recovery also lets summarization omit an old unexecuted proposal without weakening authentic signed replay. Profiles, adapter identity, and one-operation/no-fallback transport authority remain unchanged. See [REQ-ENTERPRISE-073](../../sdd/spec/enterprise-mode.md#req-enterprise-073-provider-native-bedrock-replay-integrity), [REQ-ENTERPRISE-085](../../sdd/spec/enterprise-mode.md#req-enterprise-085-provider-native-bedrock-incomplete-tool-recovery), [REQ-ENTERPRISE-076](../../sdd/spec/enterprise-mode.md#req-enterprise-076-provider-native-bedrock-protocol-translation), [REQ-ENTERPRISE-077](../../sdd/spec/enterprise-mode.md#req-enterprise-077-provider-native-bedrock-transport-dispatch), and [REQ-ENTERPRISE-079](../../sdd/spec/enterprise-mode.md#req-enterprise-079-provider-native-bedrock-replay-confidentiality). Regression coverage is fixture-based, not paid or live-provider validation.

Generic native-provider discovery can produce a provider-default custom revision with empty reasoning levels, mappings, and aliases; naming it preserves its custom identity rather than assigning a Mesh built-in. Verification and explicit Save remain required. See [REQ-ENTERPRISE-037](../../sdd/spec/enterprise-mode.md#req-enterprise-037-enterprise-pi-custom-profile-generation).

`setup:reasoning_configuration` atomically contains custom revisions and exact route assignments. Built-ins remain in code, and `setup:route_context_windows` remains a separate numeric map. Each assignment may retain active-route version, leg provenance/evidence, and a common mapping, but runtime always uses its one `activeProfile`: AI Gateway chooses conditional/fallback legs only after translation.

Runtime requires the atomic document and matching server-owned verification; legacy GLM/Kimi migration proposals and client evidence flags grant no access. Administration verifies the exact profile, gateway and route inventory before confirmed Save. Inference checks saved authority without management polling; topology is revalidated during administrative checks and Save. A successful observed path is eligible with an untested-backend warning, not proof of every leg. Common mappings still require current byte-identical evidence across reachable legs. ([REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation), [REQ-ENTERPRISE-043](../../sdd/spec/enterprise-mode.md#req-enterprise-043-enterprise-pi-verified-route-activation))

**`setup:group_routing`:** JSON map `{ [group]: { routes: string[], defaultRoute, reasoning } }` of per-Access-group policies. The first matching configured policy wins before eligibility filtering; an empty policy denies access rather than falling through. Unmatched users receive only the explicitly enabled `reasoningConfiguration.fallbackRouting` subset, or no routes when fallback is disabled or absent. Administration retains empty deny policies until explicit removal and can save a checked gateway connection or complete disabled provider drafts before any route policy exists. Legacy Setup still validates group membership, catalog subsets and defaults. See [REQ-ENTERPRISE-013](../../sdd/spec/enterprise-mode.md#req-enterprise-013-per-group-dynamic-routing), [REQ-ENTERPRISE-044](../../sdd/spec/enterprise-mode.md#req-enterprise-044-enterprise-pi-minimum-save-and-access-policies), and [REQ-ENTERPRISE-066](../../sdd/spec/enterprise-mode.md#req-enterprise-066-native-provider-draft-persistence-before-access).

**`setup:strict_egress`:** Enterprise only: Strict Gateway Egress on/off toggle (`'active'`/`'inactive'`, **default OFF**). When ON, the container's **direct-internet** HTTP/HTTPS egress (and WebSocket) is forced through the Workers VPC `EGRESS` binding → the account's Cloudflare Gateway, while only THIS deployment's own-account destinations (its R2 + account-scoped CF API / Browser Rendering) are exempt and egress direct; any other account's R2/CF host rides the Gateway ([AD86](../decisions/README.md#ad86-platform-native-cloudflare-primitives-bypass-strict-gateway-egress-only-direct-internet-egress-takes-cf1network)); **fails closed** (503) for direct-internet egress when the binding is unbound. Rollback may flip it OFF only while managed-resource policy is mutable; transition protected policy to mutable in the same Setup update or beforehand ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)). Mechanics + the `developers.cloudflare.com` allowlist note (for the bundled CF skills' retrieval) in [security.md](./security.md#strict-gateway-egress-enterprise-mode) + [deployment.md](./deployment.md#strict-gateway-egress-enterprise-mode); contract in [REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress).

**`setup:downloads_disabled`:** Enterprise only: view-only-storage toggle (`'active'`/`'inactive'`, **default OFF**). When ON, the storage download route serves only inline views of allowlisted viewable types (text/Markdown/HTML-as-text/images/PDF) and blocks everything else, so bulk exfiltration (zipping a repo and downloading it) is prevented. Enforced **server-side**; the Storage Panel also hides the Download button (UX only). Upload/delete unaffected. Contract in [REQ-ENTERPRISE-019](../../sdd/spec/enterprise-mode.md#req-enterprise-019-view-only-storage-download-disable); status codes in [api-reference.md](./api-reference.md#storage-r2-file-browser); mechanics in [security.md](./security.md#view-only-storage-enterprise-anti-exfil).

**`setup:aig_gateway_url`:** Administration accepts the legacy gateway form `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>/`, with an optional `/compat` suffix normalized away, and the account API form `https://api.cloudflare.com/client/v4/accounts/<account_id>/`. For account API URLs, only `/ai`, `/ai/run`, `/ai/v1`, `/ai/v1/chat/completions`, `/ai/v1/responses`, `/ai/v1/messages`, and `/ai/v1/models` suffixes are accepted and removed; all other suffixes are rejected. Editing equivalent coordinates or a replacement token retains saved native-only policy authority in the draft through Check connection so Review changes remains available; preview rebinds proof only when resolved gateway coordinates, provider configuration, target identity, and profile remain unchanged. ([REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation), [REQ-ENTERPRISE-065](../../sdd/spec/enterprise-mode.md#req-enterprise-065-native-verification-credential-rebinding)) <!-- @impl: src/lib/ai-gateway-management.ts::parseGatewayUrl -->

**`setup:aig_token`:** Enterprise only: the AI Gateway token, **encrypted at rest** (kv-crypto, AAD bound to the key), masked on prefill, no-clobber on blank. Wizard-first, with the deploy-time `AIG_TOKEN` secret as a fallback. It must carry Workers AI for REST inference, AI Gateway Run for compat inference, and AI Gateway Read for automatic route listing and active-version inventory. Read only via `getAigConfig`; never returned to the browser, never injected into the container ([REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)).

**`setup:active_agents`:** Enterprise only: JSON array of the coding agents users can pick when starting a new session, chosen in the Setup wizard's Coding Agents section (`configure_active_agents` step). It is enforced identically on the dashboard New Session dialog and GitHub clone picker. The selectable universe intersects build-installed `CODING_AGENTS` with gateway-capable agents (`copilot`, `pi` — [AD74](../decisions/README.md#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api) excludes Claude Code); `bash` is always selectable and never stored. An absent or invalid value resolves to all installed capable agents.

Session creation and `lastAgentType` writes enforce that intersection. Existing sessions survive later wizard deactivation while their CLI remains installed, but an image that omits their CLI rejects start. The wizard contract is [REQ-ENTERPRISE-025](../../sdd/spec/enterprise-mode.md#req-enterprise-025-active-coding-agents-configured-in-the-setup-wizard); build selection is [REQ-OPS-038](../../sdd/spec/operations.md#req-ops-038-build-selected-coding-agent-clis), and runtime enforcement is [REQ-AGENT-123](../../sdd/spec/agents.md#req-agent-123-installed-agent-runtime-availability).
...
| `COPILOT_PROVIDER_MAX_PROMPT_TOKENS` | Enterprise prompt budget advertised to Copilot. Dynamic Route defaults use `920000`; native defaults use the configured context minus the `16384` output reserve. | (unset off-enterprise) | no | `entrypoint.sh` (enterprise block) → `.bashrc` | [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC3, [REQ-ENTERPRISE-058](../../sdd/spec/enterprise-mode.md#req-enterprise-058-native-model-container-publication) AC6 |
| `COPILOT_PROVIDER_MAX_OUTPUT_TOKENS` | Enterprise output budget advertised to Copilot: `128000` for Dynamic Route defaults and `16384` for native defaults. | (unset off-enterprise) | no | `entrypoint.sh` (enterprise block) → `.bashrc` | [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC3, [REQ-ENTERPRISE-058](../../sdd/spec/enterprise-mode.md#req-enterprise-058-native-model-container-publication) AC6 |
| `ENTERPRISE_ROUTE_CATALOG` | Enterprise-only JSON route-name list. Pi exposes each route through `/model`; credentials and gateway details stay outside the container. The interceptor maps each slash-free handle to its dynamic gateway route. Pi's custom provider disables OpenAI's `developer` role so the managed prompt uses the `system` role supported across dynamic-route backends. | (unset off-enterprise or empty catalog) | no | `entrypoint.sh` (enterprise block) | [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC1, [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation) AC1 |
...
| `ENTERPRISE_DEFAULT_REASONING` | Enterprise only: the default route's reasoning grade (e.g. `medium`, `off`; `state._defaultReasoning`). Explicit grades set Pi's `defaultThinkingLevel`; empty/unset values omit it and remove any restored thinking override for provider-default startup. | (unset off-enterprise or no reasoning configured) | no | `entrypoint.sh` (enterprise block) | [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC1 |
| `ENTERPRISE_ROUTE_CONTEXT_WINDOWS` | Enterprise only: JSON `{route: tokens}` per-route context windows (`state._routeContextWindows`, fanned when enterprise AND non-empty). `entrypoint.sh` sets each Pi `models.json` `contextWindow` from it, defaulting absent routes to `256000` so Pi avoids its built-in 128k fallback. | (unset off-enterprise or empty map) | no | `entrypoint.sh` (enterprise block) | [REQ-ENTERPRISE-022](../../sdd/spec/enterprise-mode.md#req-enterprise-022-per-route-context-windows-for-dynamic-routes), [REQ-ENTERPRISE-005](../../sdd/spec/enterprise-mode.md#req-enterprise-005-container-side-enterprise-routing-ca-trust--constant-base-urls) AC1, [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation) AC1 |
| `ENTERPRISE_ROUTE_REASONING_LEVELS` | Enterprise-only JSON map of each allowed route to the canonical levels supported by its active profile. `entrypoint.sh` emits every allowed slash-free route into Pi `models.json`; Pi starts on the scope default and `/model` can switch to any listed route. | (unset off-enterprise or empty map) | no | Worker → DO → `entrypoint.sh` | [REQ-ENTERPRISE-032](../../sdd/spec/enterprise-mode.md#req-enterprise-032-enterprise-pi-route-selection-and-runtime-translation) |

Additional details:

**Copilot token budgets:** Enterprise advertises route-dependent limits so the custom model handle does not fall back to built-in defaults. A Dynamic Route default uses a `920000` prompt budget and `128000` output budget for gpt-5.5. An opaque native default uses a `16384` output cap and a prompt budget equal to its configured context minus that reserve. Values are prepended to `.bashrc` under [REQ-ENTERPRISE-058 AC6](../../sdd/spec/enterprise-mode.md#req-enterprise-058-native-model-container-publication).
...
| `GRAPHIFY_VIZ_NODE_LIMIT` | Positive-integer node limit passed to Graphify's HTML exporter. Graphs above the limit use Graphify's aggregated community view; non-integer or non-positive values abort publication. | `100000` | no | Pi AST, architecture, semantic-merge, and label-apply Graphify paths | [REQ-AGENT-128](../../sdd/spec/agents.md#req-agent-128-graph-visualization-node-limits) |
