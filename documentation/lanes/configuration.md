...
| `setup:route_context_windows` | JSON map combining each route's positive `contextWindow` and finite `reasoningProfile`, for example `{ "development": { "contextWindow": 262144, "reasoningProfile": "workers-ai-gpt-oss" } }`. See [route model settings](#route-model-settings). | Setup wizard → KV (re-run setup to change; no redeploy) |
...
**Route model settings:** Supported reasoning profiles are `workers-ai-gpt-oss`, `workers-ai-glm-5.3`, and `workers-ai-kimi-k2.6`. Legacy `{route: tokens}` values remain readable and fan only context windows to the container, but chat-completion requests fail closed until the route receives a profile. See [REQ-ENTERPRISE-022](../../sdd/spec/enterprise-mode.md#req-enterprise-022-per-route-context-windows-for-dynamic-routes) and [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-gateway-provider-compatibility).
