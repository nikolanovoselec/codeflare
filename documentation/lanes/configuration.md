...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document containing Worker-owned native/custom target identity, profile, lifecycle, and verification state under [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-and-document). See [Native provider targets](#native-provider-targets). | Administration AI Routing → KV (no redeploy) |
