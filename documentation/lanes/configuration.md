...
**`setup:aig_gateway_url`:** Administration accepts the legacy gateway form `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>/` and the account API form `https://api.cloudflare.com/client/v4/accounts/<account_id>/`. Account API suffixes such as `/ai/v1/chat/completions` or `/ai/run` are removed automatically. <!-- @impl: src/lib/ai-gateway-management.ts::parseGatewayUrl -->
