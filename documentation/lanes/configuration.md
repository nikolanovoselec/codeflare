...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document under [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-persistence-and-save-lifecycle). It stores the exact native/custom provider and model selector, raw provider-config identity and alias, immutable profile reference, enabled state, and verification Worker-side. Browser and container projections use only derived `cf-native-<uuid>` handles and safe labels. | Administration AI Routing → KV (no redeploy) |
