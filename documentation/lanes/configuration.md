...
| `CONTEXT_MODE_BRIDGE_IDLE_MS` | Not force-set by Codeflare. `context-mode-runtime.ts` clears inherited overrides, owns one context-mode extension registration for the foreground Pi session, and prevents in-process subagents from spawning bridge helpers; subagents use native/Bash fallbacks | unset (context-mode default) | no | entrypoint / `context-mode-runtime.ts` | [REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) AC7-AC8 |
