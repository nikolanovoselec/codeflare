...
**`setup:aig_gateway_url`:** Administration accepts the legacy gateway form `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>/` and the account API form `https://api.cloudflare.com/client/v4/accounts/<account_id>/`. The accepted `/ai`, `/ai/run`, `/ai/v1`, `/ai/v1/chat/completions`, `/ai/v1/responses`, `/ai/v1/messages`, and `/ai/v1/models` suffixes are removed automatically; all others are rejected. ([REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation)) <!-- @impl: src/lib/ai-gateway-management.ts::parseGatewayUrl -->
