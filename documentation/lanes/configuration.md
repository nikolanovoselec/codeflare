...
| `CONTAINER_SSH_PUBLIC_KEY` | Optional single-line OpenSSH `ssh-ed25519` public key. Presence enables Wrangler SSH with one `codeflare-operator` authorized key in every deployment environment; absence keeps SSH disabled. The matching private key remains outside GitHub and the deployment. | - | no | Repository-level GitHub Actions secret; deployment configuration only | [REQ-OPS-058](../../sdd/spec/operations.md#req-ops-058-optional-persistent-container-ssh-authorization) |
...
Container SSH authorization is independently optional. Deployment reads only the repository-level public key and never uploads the matching private key to GitHub, Cloudflare, Worker bindings, or containers. Provisioning, rotation, Vault custody, connection, and revocation are maintained in the private [persistent container SSH runbook](https://github.com/nikolanovoselec/codeflare-private/blob/main/docs/operations/container-ssh.md).
