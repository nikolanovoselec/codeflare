...
| `VAPID_SUBJECT` | Stable Web Push sender contact URI (`mailto:` or `https:`) for this deployment environment. | - | yes | Environment-scoped GitHub Actions variable / Worker runtime env | [REQ-SEC-023](../../sdd/spec/security.md#req-sec-023-agent-notification-trust-boundaries), [REQ-OPS-013](../../sdd/spec/operations.md#req-ops-013-deploy-command-and-post-deploy-hooks) |
| `VAPID_PUBLIC_KEY` | Stable Web Push public key paired with `VAPID_PRIVATE_KEY`. It is sourced from an environment-scoped GitHub Actions variable and uploaded through the bulk secret file so deployment logs do not print key material; the authenticated notification config route may return this public value. | - | yes | GitHub Actions variable / Worker runtime binding | [REQ-SEC-023](../../sdd/spec/security.md#req-sec-023-agent-notification-trust-boundaries), [REQ-OPS-013](../../sdd/spec/operations.md#req-ops-013-deploy-command-and-post-deploy-hooks) |
| `VAPID_PRIVATE_KEY` | Stable Web Push private key paired with `VAPID_PUBLIC_KEY`. Secret-only: never place it in `wrangler.toml`, logs, or responses. | - | yes | Environment-scoped GitHub Actions secret / Worker secret | [REQ-SEC-023](../../sdd/spec/security.md#req-sec-023-agent-notification-trust-boundaries), [REQ-OPS-013](../../sdd/spec/operations.md#req-ops-013-deploy-command-and-post-deploy-hooks) |
