...
| Dynamic-route catalog (`setup:dynamic_routes`, `setup:default_route`) | Enterprise only: selectable gateway dynamic routes (`string[]`, **at least one required** — the wizard blocks Continue and `configure` returns `400` otherwise) and the default `route:reasoning` (the first route added becomes the default), configured in the **setup wizard** and stored in KV (no redeploy). The `LlmInterceptor` maps the agent's slash-free handle to `dynamic/<route>` on egress; the catalog is fanned to the container for `/model` listing. (Replaces the removed `AIG_LANGUAGE_MODEL` var.) | - | no | Setup wizard -> KV (enterprise) | [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-007](../../sdd/spec/enterprise-mode.md#req-enterprise-007-gateway-route-pinning) |
