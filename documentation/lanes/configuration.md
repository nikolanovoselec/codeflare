...
| `REVIEW_LANE_TRIAGE_MAX_BYTES` | Largest Phase 0 triage block inlined into a lane's opening prompt. The block carries the SDD config verbatim, so an oversized one would sit in the prompt prefix of every turn. Above this the verbatim config is shed first and the resolved decisions are kept; only a block still over the cap makes the lane derive Phase 0 itself. | `32768` | no | Lane process env | [REQ-AGENT-103](../../sdd/spec/agents.md#req-agent-103-deterministic-lane-triage) |
