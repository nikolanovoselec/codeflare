...
**`setup:active_agents`:** Enterprise only: JSON array of the coding agents users can pick when starting a new session, chosen in the Setup wizard's Coding Agents section (`configure_active_agents` step). Enforced identically on every session-creation surface (the dashboard New Session dialog and the GitHub clone picker). The selectable universe is the intersection of build-installed `CODING_AGENTS` and gateway-capable agents (`copilot`, `pi` — [AD74](../decisions/README.md#ad74-enterprise-llm-transport-on-the-ai-gateway-rest-api) excludes Claude Code); `bash` is always selectable and never stored. An absent or invalid value resolves to all installed capable agents. Session creation and `lastAgentType` writes enforce the intersection. Existing sessions survive later wizard deactivation while their CLI remains installed, but an image that omits their CLI rejects start. Wizard contract is in [REQ-ENTERPRISE-025](../../sdd/spec/enterprise-mode.md#req-enterprise-025-active-coding-agents-configured-in-the-setup-wizard); build selection is in [REQ-OPS-038](../../sdd/spec/operations.md#req-ops-038-build-selected-coding-agent-clis), and runtime enforcement is in [REQ-AGENT-123](../../sdd/spec/agents.md#req-agent-123-installed-agent-runtime-availability).
