...
| `CODEFLARE_SESSION_WORKSPACE` | Immutable session workspace (`terminal` or `vscode`). Missing and invalid stored values resolve to Terminal. Clients cannot supply this environment value. | `terminal` | no | Session KV -> Worker -> DO via `setBucketName` | [REQ-IDE-052](../../sdd/spec/browser-ide.md#req-ide-052-immutable-session-workspace-snapshot) |
