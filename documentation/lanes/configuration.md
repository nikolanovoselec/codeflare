...
| `setup:native_ai_targets` | Enterprise-only versioned native-target document containing Worker-owned native/custom target identity, profile, lifecycle, and verification state. See [Native provider targets](#native-provider-targets). | Administration AI Routing → KV (no redeploy) |
