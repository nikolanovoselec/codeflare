...
The six immutable revision-1 built-ins are `openai-gpt-chat-tools-reasoning`, `openai-gpt-chat-tools-off`, `workers-ai-gemma-thinking`, `workers-ai-kimi-k-thinking`, `workers-ai-glm-thinking`, and `codeflare-inference-mesh-binary-thinking`. The catalog also exposes exactly four nonassignable notices: GPT-OSS tool replay, Gemini Chat Completions tools, GPT-6 Astra tools, and Responses-required behavior.
