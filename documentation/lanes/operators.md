...
`OperatorDispatcherCapability` exposes only bounded GitHub reads, inference and the exact pinned facet scheduler/fiber bridge. The parent rechecks human eligibility and pinned installation revisions, then constructs existing `GitHubInterceptor`/`LlmInterceptor` loopbacks with parent-owned props. Credentials, the Activity stub, namespaces and direct networking never enter the child. Read scope is the invocation repository/PR, first-page files, at most 100 checks in bounded 10-run pages, and observed head; only the check fields used by the package cross the parent boundary. Missing, overlapping or over-limit pages cannot become complete evidence. Inference uses the current eligible human default. Non-null resource profiles fail closed. The precise wire/limits live in the [registry contract](../../sdd/spec/operator-registry-contract.md#parent-capability-operations). <!-- @impl: src/operators/activity.ts::OperatorDispatcherCapability -->

Verification: `dispatcher-production.test.ts` covers bounded pages, exact-settlement result collection, fail-closed output and SDK cleanup with instrumented transports. The pinned generated-Flue native fixture proves the message shape; it does not independently prove physical production eviction. Enterprise Integration's installed v0.1.2 Activity returned a durable read-only PR #1173 result with incomplete assessment; this is execution proof, not a positive Renovate finding or production test.
...
Implements the browser-facing portion of [REQ-OPERATOR-027](../../sdd/spec/operators.md#req-operator-027-owned-activity-user-surface), [REQ-OPERATOR-033](../../sdd/spec/operators.md#req-operator-033-activity-surface-resilience), and [REQ-OPERATOR-040](../../sdd/spec/operators.md#req-operator-040-owned-activity-control-presentation).

Enterprise activity list/detail/start/cancel/result routes derive the current signed human owner key and never accept owner identity from request data. Mutations require the existing CSRF boundary; projections remain bounded and secret-free. The responsive header control and activity detail states distinguish loading, empty, attention, terminal, and unknown outcomes without treating queued work as complete. The overview panel renders at document level so no filtered ancestor can become its containing block, takes its desktop anchoring from measured trigger coordinates, closes when the viewport width changes, and moves focus into the panel on open and back to the trigger when focus is still inside the panel at dismissal. Backend and UI tests are complete; actual desktop/mobile and deployed owner-isolation acceptance remain pending. <!-- @impl: src/routes/operator-activities.ts --> <!-- @impl: web-ui/src/components/OperatorActivityButton.tsx --> <!-- @impl: web-ui/src/components/admin/ActivityPage.tsx -->
...
The protected `boundary-reviews.yml` candidate checks out only its Codeflare base commit. Its thin Action client authenticates, starts, monitors, transports the one terminal result and publishes Conductor's presentation through the parent journal; it never checks out or executes a private Conductor repository. This [independent publication](../../sdd/spec/operator-registry.md#req-operator-056-independent-review-publication) candidate is not installed or live-proven. <!-- @impl: .github/workflows/boundary-reviews.yml::publish --> <!-- @impl: scripts/operator-boundary-action.mjs::publishBoundaryResult -->
