...
- **backend-tests** — three coverage-emitting Workers jobs use longest-first assignment from exact-head per-file durations, plus a Node-runtime leg; all run via `.github/actions/vitest-suite` ([Backend Tests](#backend-tests) has the fail-closed gate). Unmeasured new files receive the largest recorded weight until the next evidence refresh. <!-- @impl: scripts/ci/select-weighted-backend-tests.mjs::assignWeightedFiles --> <!-- @impl: scripts/ci/backend-test-weights.json -->
...
Every CI package tree, including the inspected Pi prompt runtime, restores `node_modules` from an OS-, architecture-, directory-, and lockfile-specific cache; only a miss runs `npm ci`, and the Pi tree retains `--ignore-scripts`. <!-- @impl: .github/actions/install-deps/action.yml::runs -->
