...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   - Computes the image input hash over `Dockerfile`, ignore/scan policy, copied production paths (`entrypoint.sh`, host package manifests, `host/tsconfig.json`, `host/src/`, `openvscode/`, `preseed/`, seed and image-smoke scripts), and an ISO-week salt. Changes under `host/__tests__/` do not invalidate the deployment image.
