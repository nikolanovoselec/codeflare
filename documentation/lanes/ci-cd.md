...
`workflow_run` already carries its exact green gate. `outcome` fails a run in which nothing was deployed, so a green Deploy means a deploy happened. The same-repository green-`push` gate is repeated on every job because a job skipped via `if:` counts as success for `needs:` resolution.

(Implements [REQ-OPS-001](../../sdd/spec/operations.md#req-ops-001-deploy-workflow-trigger-and-pre-deploy-pipeline), [REQ-OPS-029](../../sdd/spec/operations.md#req-ops-029-automatic-manual-deploy-verification-reuse), and [REQ-OPS-026](../../sdd/spec/operations.md#req-ops-026-concurrent-deploy-dispatches-are-legible-and-independently-verified).)
