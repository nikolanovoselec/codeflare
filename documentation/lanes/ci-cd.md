...
Job graph: `verify` → `prepare` → (`build-worker` ∥ `container`) → `deploy` → `outcome`. `verify` runs PR Checks inline on manual dispatch (skipped on `workflow_run`, where the trigger already guarantees a green run for the same SHA); `outcome` fails a run in which nothing was deployed, so a green Deploy means a deploy happened. The run title (`run-name`) resolves and displays the deploy target (production / enterprise / enterprise integration / integration) plus the source ref, so the Actions list and `gh run list` answer "what did this deploy to?" without opening the run. The inline `verify` job passes its own `github.run_id` to `test.yml` as `concurrency_key`, which is appended to that workflow's concurrency group — without it, dispatching two environment deploys off one branch puts both inline verifies in the same group and the second cancels the first, surfacing as a cancelled run that deployed nothing. The pwn-request gate (workflow_run must be a same-repo `push` event that concluded green) is repeated on every job because a job skipped via `if:` counts as success for `needs:` resolution.
...
- **browser-ide-image:** builds the image without pushing and validates the immutable Pi/Claude/empty extension inventories against the packaged-boundary contract ([REQ-IDE-005](../../sdd/spec/browser-ide.md#req-ide-005-selected-native-ide-agent) AC6).
...
**What browser-ide-image checks:** it checksum-verifies and installs Anthropic's exact official VSIX, has the pinned OpenVSCode CLI positively discover both extension IDs, registers packaged Pi against a guarded API mock without authentication or language-model access, checks official Claude's identity/binary/production config/permissions, starts both host inventories, proves no agent child starts before use, and records image size, idle process count, and RSS.
