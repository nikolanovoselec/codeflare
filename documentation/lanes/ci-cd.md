...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - When immutable Node and embedded Code OSS artifacts lag a fixable package release, the Docker build may overlay one exact integrity-verified registry artifact across every affected runtime path and verify each installed version before later package operations. The current node-tar 7.5.21 overlay removes CVE-2026-73566 from npm and code-server without weakening the Trivy gate; it should be removed when both upstream artifacts carry that floor directly.
