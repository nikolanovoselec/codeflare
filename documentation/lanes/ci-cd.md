...
| `test.yml` | PRs to `main` or `develop`, push to `main` + `workflow_dispatch` | PR checks: lint (oxlint), tests, typecheck, build verification, dead code check (knip), `npm audit --omit=dev`, dependency review. The host-test lane installs rclone so the real sync-filter behavioral suite executes instead of skipping. Push-to-`main` provides the post-merge signal that `deploy.yml` gates on. Push-to-`develop` is intentionally absent: an open develop→main PR already fires a `pull_request` synchronize run, so a separate `push` trigger would double-run. |
