...
- **frontend-tests** — four duration-weighted file groups through the same action, so the jsdom suite gets the identical report gate. Only shard 3 also runs `npm run build`, placing the production-breakage check on the fastest measured group rather than making it a test dependency.
