...
- **quality** — agent-seed drift guard, oxlint (backend + frontend), knip dead-code check (both), and a `bash -n` syntax pass over every tracked shell script. When a production lockfile changes or every lane is requested, one-minute fail-closed `npm audit --package-lock-only --audit-level=high --omit=dev` steps validate backend and frontend locks independently of restored `node_modules`; unchanged-lock PRs retain GitHub Dependency Review without waiting on the npm audit endpoint ([REQ-OPS-003](../../sdd/spec/operations.md#req-ops-003-pr-checks-run-lint-test-typecheck-and-security-audit)).
