...
|----------|---------|-------------|
| `deploy.yml` | `workflow_run` when PR Checks complete green on `main` + `workflow_dispatch` (production/integration/enterprise/enterprise integration; `registry` selector cloudflare/dockerhub; optional `verified_run_id`) | Validates an explicit successful PR Checks run and its tested-tree receipt or falls back to inline checks, then runs staged `prepare` → (`build-worker` ∥ `container`) → `deploy`. |
...
Job graph: (`verify-existing` or `verify`) → `prepare` → (`build-worker` ∥ `container`) → `deploy` → `outcome`. Manual dispatch may supply `verified_run_id`: the workflow checks the run's repository, workflow, head, completed result, required `test` job, and immutable tested-tree receipt against the deploy checkout. Missing input falls back to inline PR Checks; invalid input fails closed. `workflow_run` already carries its exact green gate. `outcome` fails a run in which nothing was deployed, so a green Deploy means a deploy happened. The pwn-request gate (workflow_run must be a same-repo `push` event that concluded green) is repeated on every job because a job skipped via `if:` counts as success for `needs:` resolution.
...
- **quality** — agent-seed drift guard, oxlint (backend + frontend), knip dead-code check (both), `npm audit --package-lock-only --audit-level=high --omit=dev` (both, independent of restored `node_modules`), and a `bash -n` syntax pass over every tracked shell script.
