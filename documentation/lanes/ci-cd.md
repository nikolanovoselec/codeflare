...
- **changes:** `dorny/paths-filter` classifies the diff into `backend`, `webui`, `landing`, `host`, `ide`, `workflows`, and `dependencies`; the last selects production lockfiles. `changes.outputs.full` means "no diff was filtered, run everything".
...
Pull requests use fail-closed GitHub Dependency Review instead of waiting on the npm registry. Post-merge lockfile changes and full workflow runs execute one-minute fail-closed backend, frontend, and Browser IDE `npm audit` checks; backend and frontend use lockfile-only mode without restored `node_modules`.
