...
**Workflow permissions:** top-level is `contents: read` (read-only default). Deployment's `container` job and PR Checks' complete-image job add `packages: write` only for the shared GHCR BuildKit cache. Fork pull requests do not log in or read/write that cache, and GitHub independently downgrades their token. Code-scanning least-privilege hardening (#56).
