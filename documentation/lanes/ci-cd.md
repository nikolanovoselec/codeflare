...
- **changes:** `dorny/paths-filter` classifies the diff into `backend`, `webui`, `landing`, `host`, `ide`, and `workflows`. If GitHub returns `diff/not_available`, the checked-out fallback verifies the exact base/head commits, records changed files, and selects every lane. `changes.outputs.full` means "no diff was filtered, run everything".
