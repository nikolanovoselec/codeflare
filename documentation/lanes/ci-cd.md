...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - Otherwise buildx uses the deployment-owned GHCR registry cache, emits plain per-layer timing, and retains that log for 14 days.

     Pi extensions are copied after dependency/toolchain layers but before Jiti prewarm. Browser IDE artifacts and generated seeds assemble after unrelated runtime installs, retaining dependency cache without stale outputs. See [REQ-OPS-050](../../sdd/spec/operations.md#req-ops-050-hosted-image-build-critical-path-optimization).
   ...
   - Locked Trivy primes daily vulnerability and Java DBs, then copies them into isolated per-process caches before concurrent scan, SBOM, and Wrangler preparation.

     The workflow awaits all three, applies `ignore-unfixed: true` plus `.trivyignore`, enforces the bounded JSON verdict, and uploads the SBOM before push. See [Security §Container Image Scanning](security.md#container-image-scanning-req-sec-011) and [REQ-OPS-050](../../sdd/spec/operations.md#req-ops-050-hosted-image-build-critical-path-optimization).
