...
Dependabot runs weekly against `develop` for six npm directories: `/`, `/.github/npm-tools/wrangler`, `/web-ui`, `/host`, `/landing`, and `/openvscode/agent-sidebar`. It also covers Docker images and GitHub Actions. Every ecosystem declares a `cooldown` (7 days default, 30 for majors — 7 is zizmor's floor for `default-days`): the waiting period gives the ecosystem time to revoke a malicious release before automation proposes it.

The root npm lane owns application Wrangler. Stress and container-image workflows instead use the dedicated `/.github/npm-tools/wrangler` manifest and committed lock, installed with `npm ci`; its separate Dependabot lane owns that workflow-only pin.
...
Each bump opens its own PR. Shared npm-tool jobs update the owning manifest and regenerate its committed lock atomically ([REQ-OPS-033](../../sdd/spec/operations.md#req-ops-033-lock-backed-npm-bump-coherence)). Pi changes additionally regenerate the prewarm lock and embedded seed through `scripts/regenerate-pi-preseed-lock.mjs`, which suppresses lifecycle scripts because the committed payload differs deliberately from flattened runtime npm layout ([REQ-OPS-025](../../sdd/spec/operations.md#req-ops-025-pi-preseed-bump-artifact-coherence)).
...
**Workflow permissions:** top-level is `contents: read` in [PR Checks](../../.github/workflows/test.yml) and [Deploy](../../.github/workflows/deploy.yml). Deployment's inline `verify` job passes `actions: read`, `contents: read`, and `packages: read`; the called workflow uses them for prior complete-image evidence lookup and read-only GHCR BuildKit cache import. PR Checks' complete-image job likewise receives only `packages: read`. Deployment's `container` job receives `packages: write` to import and publish that cache, plus `id-token: write` and `attestations: write`; only fresh-image runs invoke provenance attestation. Fork pull requests and Dependabot do not authenticate to or read/write the shared cache. Login failure disables cache use; deployment export errors are ignored without failing the image build. Code-scanning least-privilege hardening (#56). <!-- @impl: .github/workflows/deploy.yml::container --> <!-- @impl: .github/workflows/test.yml::browser-ide-image --> <!-- @impl: .github/workflows/container-image.yml::image -->
...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - If `in-<hash>` already exists in the target registry, the retained image is reused and build/scan/push are skipped.
     ...
     Reuse preserves the original scan and uploaded SBOM without regenerating or revalidating them. Provenance exists only if the original post-push attestation succeeded, and reuse does not check it. The weekly salt bounds the reused CVE verdict to seven days.
   - Otherwise buildx uses the shared GHCR registry cache. Login failure disables cache use; export errors are ignored without failing or restarting the image build.

     Unlike GitHub's ref-scoped `type=gha` cache, this cache is readable from the later deployment branch ref. Fork pull requests and Dependabot do not authenticate to it. The base image comes from the AWS ECR Public Node mirror to avoid Docker Hub anonymous pull limits.
...
Path-gated workload lanes run in parallel. The `changes` job classifies their inputs; orchestration jobs such as complete-image evidence resolution and the required summary still run. Full workflow dispatches and nightly schedules execute every workload lane.
...
- **browser-ide-image-reuse:** computes the image fingerprint and resolves direct prior evidence on eligible pull-request runs ([REQ-OPS-030](../../sdd/spec/operations.md#req-ops-030-browser-ide-complete-image-verification-reuse)).
- **browser-ide-image:** builds the image without publishing a runnable image, writes trusted same-repository layers to the cross-ref GHCR BuildKit cache, and validates the immutable Pi/Claude/empty extension inventories against the packaged-boundary contract when its tracked inputs lack reusable direct evidence. ([REQ-IDE-010](../../sdd/spec/browser-ide.md#req-ide-010-pinned-ide-inventory-compatibility), [REQ-OPS-030](../../sdd/spec/operations.md#req-ops-030-browser-ide-complete-image-verification-reuse))
...
- **summary** — required `test` context; rejects failed, cancelled, or unverified relevant lanes and publishes the current exact-tree receipt.

Complete-image reuse binds tracked image and smoke inputs, `linux/amd64`, and the ISO week. Eligible PR runs search one bounded newest-first page for a direct successful execution from the same repository and PR, then validate its workflow contract and immutable receipt. Uncovered Dockerfile inputs or invalid evidence cause normal execution. The summary receipt records complete-image provenance and remains the authority for optional exact-tree deploy reuse.
...
**Why the filters are broad:** the shared anchor — pipeline files, root manifests, the Dockerfile and its ignore/suppression lists, both vitest node-suite files, the lint and dead-code rulesets — sits in every filter, so anything that can change what a lane MEANS re-runs every lane. `host` additionally takes all of `.github/**`, because its tests assert on the structure of other workflows. This PR-wide classification deliberately remains conservative; complete-image reuse is a separate fail-closed decision based on the current tracked image-input fingerprint, not on the latest commit's path list.
...
- [REQ-OPS-030](../../sdd/spec/operations.md#req-ops-030-browser-ide-complete-image-verification-reuse) - Browser IDE complete-image verification reuse
