...
Path-gated workload lanes run at maximum parallelism after the `changes` classifier, with no container build in PR Checks. Backend, frontend, and host tests expose seven, three, and two matrix legs concurrently. Coverage merge is the only package-specific fan-in before the required summary. [Run 32808002321](https://github.com/nikolanovoselec/codeflare/actions/runs/32808002321) completed its required gate in 154 seconds and workflow in 155 seconds, meeting the affected exact-head target. Backend jobs finished in 65–90 seconds and host jobs in 61–69 seconds; current recorded weights derive from that run ([REQ-OPS-045](../../sdd/spec/operations.md#req-ops-045-parallel-pr-checks-performance)).
