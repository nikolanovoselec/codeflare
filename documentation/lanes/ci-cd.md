...
- **backend-tests** — twelve coverage-emitting `vitest --shard` jobs plus a Node-runtime leg, all via `.github/actions/vitest-suite` ([Backend Tests](#backend-tests) has the fail-closed gate).
...
- **host-tests** — five `node --test --test-shard` jobs over one selection reconciled against `host/__tests__/ci-excluded.txt`; every leg fails on zero assertions and installs rclone for sync-filter behavioral coverage.
