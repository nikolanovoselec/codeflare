...
| `test.yml` | PRs to `main` or `develop`, push to `main`, `merge_group`, `workflow_dispatch`, and `workflow_call` | Reusable parallel path-filtered quality (lint, knip, audit, seed drift), typecheck, workflow-audit, bundle-size, coverage, test-suite, host, and dependency-review lanes. One fail-closed action runs six measured backend Workers groups, a Node leg, three frontend shards, and landing. The required `test` summary fails failed/cancelled lanes and passes unaffected skipped lanes. |
...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - When immutable artifacts lag a fixable dependency, the image overlays one exact integrity-verified artifact across every affected path, then verifies each installed version and runtime operation before scan or push. <!-- @impl: Dockerfile::NODE_TAR_VERSION --> See [REQ-OPS-046](../../sdd/spec/operations.md#req-ops-046-fixable-dependencies-in-immutable-runtime-artifacts).
...
Path-gated workload lanes run at maximum parallelism after the `changes` classifier, with no container build in PR Checks. Backend, frontend, and host tests expose seven, three, and two matrix legs concurrently. Coverage merge is the only package-specific fan-in before the required summary. [Run 32808444076](https://github.com/nikolanovoselec/codeflare/actions/runs/32808444076) completed its required gate in 148 seconds and workflow in 149 seconds, meeting the affected exact-head target. Backend Workers jobs finished in 60–90 seconds, Node in 25 seconds, frontend in 34–51 seconds, and host in 66–68 seconds; current recorded weights derive from [measurement run 32808002321](https://github.com/nikolanovoselec/codeflare/actions/runs/32808002321) ([REQ-OPS-045](../../sdd/spec/operations.md#req-ops-045-parallel-pr-checks-performance)).

- **changes:** `dorny/paths-filter` classifies the diff into `backend`, `webui`, `landing`, `host`, `pi`, `ide`, and `workflows`. `changes.outputs.full` means "no diff was filtered, run everything". <!-- @impl: .github/workflows/test.yml::changes -->
...
- **backend-tests** — six Workers jobs use recorded duration plus per-file overhead for longest-first assignment; Node runs separately. New files receive maximum recorded weight. <!-- @impl: scripts/ci/select-weighted-backend-tests.mjs::assignWeightedFiles --> <!-- @impl: scripts/ci/backend-test-weights.json -->
...
- **host-tests** — two jobs combine all five native `node --test --test-shard` partitions exactly once according to measured 45/46-second groups over one audited selection; every leg fails on zero assertions and installs rclone for sync-filter behavioral coverage. <!-- @impl: .github/workflows/test.yml::host-tests -->
...
CodeQL runs two JavaScript/TypeScript source shards in parallel: Worker/automation source and container/UI source. Each shard has a unique upload category, and a dependent `Analyze` gate preserves the existing required-check identity. Generated aggregates, vendored bundles, and non-runtime test trees remain excluded ([REQ-OPS-019](../../sdd/spec/operations.md#req-ops-019-security-posture-scanning-workflows)). <!-- @impl: .github/workflows/codeql.yml::analyze --> <!-- @impl: .github/workflows/codeql.yml::gate --> <!-- @impl: .github/codeql/codeql-worker.yml --> <!-- @impl: .github/codeql/codeql-container-ui.yml -->

Every package tree installed by PR Checks, including the inspected Pi prompt runtime, restores `node_modules` from an OS-, architecture-, directory-, and lockfile-specific cache; only a miss runs `npm ci`, and the Pi tree retains `--ignore-scripts`. <!-- @impl: .github/actions/install-deps/action.yml::runs -->
