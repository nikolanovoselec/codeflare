...
The `pi-extensions` bump is data-driven: `pi-extensions-discover` lists every dependency in `preseed/agents/pi/package.json` except context-mode. That set currently contains eleven packages: `@gotgenes/pi-subagents`, the three `@juicesharp` packages, the three `@narumitw` workflow packages, `pi-caveman`, `pi-evaluate`, `pi-web-access`, and `pi-mcp-adapter`. A `fail-fast: false` matrix gives each package its own bump leg and PR; the dedicated `context-mode` job owns its coupled copies.
