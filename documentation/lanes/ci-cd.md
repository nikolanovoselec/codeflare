...
The Impeccable native-engine PR Checks lane finishes within one minute without building a container. It verifies the checksum-pinned source identity, compiles the exact upstream and patched Rust wait and raster-traversal functions into focused probes, reproduces both upstream regressions, and proves the corrected idle-grace and no-symlink boundaries. The deployment image build still compiles and exercises the complete native binary before publication. Impeccable bundle refreshes reject engine versions that the image has not adopted. ([REQ-AGENT-163](../../sdd/spec/agents.md#req-agent-163-impeccable-browser-question-idle-lifecycle), [REQ-AGENT-164](../../sdd/spec/agents.md#req-agent-164-impeccable-raster-scan-traversal), [REQ-AGENT-181](../../sdd/spec/agents.md#req-agent-181-design-specialist-compatibility), [REQ-OPS-003](../../sdd/spec/operations.md#req-ops-003-pr-checks-run-lint-test-typecheck-and-security-audit))
