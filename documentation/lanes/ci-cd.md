...
**Workflow permissions:** top-level is `contents: read` (read-only default). Deployment's `container` job and PR Checks' complete-image job add `packages: write` only for the shared GHCR BuildKit cache. Container jobs always receive `id-token: write` and `attestations: write`, while only fresh-image runs invoke provenance attestation. Fork pull requests and Dependabot do not authenticate to or read/write the shared cache. Login failure disables cache use; export errors are ignored without failing the image build. GitHub also independently downgrades fork and Dependabot tokens. Code-scanning least-privilege hardening (#56).
