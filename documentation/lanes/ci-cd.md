...
| `zizmor.yml` | PRs touching workflow surfaces, pushes to `main` touching those surfaces, and `workflow_dispatch` | Records the workflow security audit as SARIF in code scanning, so alert history is preserved. The separately selected `workflow-audit` lane in `test.yml` is the blocking PR check; this branch filter is not described as a merge-gate bypass. Its zizmor version is pinned (the action defaults to `latest`, which floats the auditor). |
...
- **coverage-backend / coverage-frontend** — run for affected backend/frontend pull requests as well as full runs. Existing global package floors remain authoritative; pull requests additionally apply bounded LCOV checks to changed production lines only (80% backend, 70% frontend), with renames mapped to their destination and required missing/malformed evidence failing closed.
