...
- **changes** — `dorny/paths-filter` classifies the diff into `backend`, `webui`, `landing`, `host`, `workflows`. The shared anchor (pipeline files, root manifests, the Dockerfile and its ignore/suppression lists, both vitest node-suite files, the lint and dead-code rulesets) is in every filter, so anything that can change what a lane MEANS re-runs every lane; `host` additionally takes all of `.github/**`, because its tests assert on the structure of other workflows. `changes.outputs.full` (the filter step's own `skipped` outcome) is the single flag meaning "no diff was filtered, run everything".
...
- **landing-tests** — Container-API render + unit tests, plus `astro build` so a broken production build fails the PR rather than the deploy. **host-tests** — `node --test` over a selection reconciled against `host/__tests__/ci-excluded.txt`, failing if the selection is empty or executes zero assertions; installs rclone for the sync-filter behavioral tests.
