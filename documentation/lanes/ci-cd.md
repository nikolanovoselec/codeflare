...
| `test.yml` | PRs to `main` or `develop`, push to `main`, `workflow_dispatch` + nightly schedule (all lanes) | Parallel path-filtered lanes: quality (lint/knip/audit/seed-drift), typecheck, backend tests (four parallel-pool vitest shards + a Node-runtime leg, fail-closed), frontend, landing, host, dependency-review. The `summary` job (check name `test`, the required branch-protection context) fails on any failed/cancelled lane and passes skipped (unaffected) lanes. |
...
- **backend-tests** — four `vitest --shard` jobs via `.github/actions/backend-tests` ([Backend Tests](#backend-tests) has the fail-closed gate). Each shard runs its Workers pool across several workers (`maxWorkers` in `vitest.config.ts`, capped at 4 because every worker is a full workerd + miniflare instance). Shard count is the dominant lever: most of a shard's wall clock is per-file transform and isolate setup, roughly half of it in the main process where extra workers cannot help, and that half divides across shards. The unshardable Node-runtime suite (`vitest.node.config.ts`) runs as its own matrix leg under the same JSON gate, rather than riding a shard and making it the straggler.
...
- **summary** — check name `test` (the required branch-protection context). Fails when any needed lane failed or was cancelled; passes skipped lanes (their area was untouched). Reconciles shard coverage (`scripts/ci/check-suite-completeness.mjs`, see [Backend Tests](#backend-tests)) and publishes a backend test-result table (`scripts/ci/render-test-summary.mjs`) to the run summary.
...
**Cross-shard completeness gate (fail-closed):** the per-shard gate can only vouch for files it was handed, so `scripts/ci/check-suite-completeness.mjs` runs in the `summary` job and reconciles the union of every shard report against the `src/**/*.test.ts` files actually in the tree. A file lost to a mis-shard, a stale exclude, or a worker dying mid-run fails the required context instead of passing as a smaller-but-green run. It also fails when the backend lane reported success yet uploaded no reports, so a flaked artifact download cannot silently disarm it. `vitest.node-suite.mjs` is the single list of Node-runtime backend tests, shared by both vitest configs and this gate.
