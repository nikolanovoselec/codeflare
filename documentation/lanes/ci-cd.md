...
**Workflow permissions:** top-level is `contents: read` (read-only default). Deployment's `container` job and PR Checks' complete-image job add `packages: write` only for the shared GHCR BuildKit cache. The deployment container job always receives `id-token: write` and `attestations: write`, while only fresh-image runs invoke provenance attestation. Fork pull requests and Dependabot do not authenticate to or read/write the shared cache. Login failure disables cache use; export errors are ignored without failing the image build. GitHub also independently downgrades fork and Dependabot tokens. Code-scanning least-privilege hardening (#56). <!-- @impl: .github/workflows/deploy.yml::container --> <!-- @impl: .github/workflows/test.yml::browser-ide-image --> <!-- @impl: .github/workflows/container-image.yml::image -->
