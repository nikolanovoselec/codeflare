...
- **browser-ide-image-reuse:** fingerprints the tracked production-image and smoke inputs with the `linux/amd64` platform and ISO-week freshness salt; an uncovered Dockerfile COPY/ADD source disables reuse. On pull-request runs only, it searches one bounded newest-first page for a direct successful `browser-ide-image` execution from the same repository and PR, then validates the source workflow contract and immutable receipt before authorizing reuse. Invalid or unavailable evidence produces an ordinary image execution rather than a skip ([REQ-OPS-030](../../sdd/spec/operations.md#req-ops-030-browser-ide-complete-image-verification-reuse)).
...
- **summary** — check name `test` (the required branch-protection context). Fails when any needed lane failed or was cancelled; accepts a relevant complete-image skip only with validated reuse, and publishes the checked-out commit/tree receipt with complete-image evidence provenance for optional exact-tree deploy reuse.
