...
Pull requests use fail-closed GitHub Dependency Review instead of waiting on the npm registry. Post-merge lockfile changes execute applicable one-minute fail-closed audits; full runs execute all three. Browser IDE audit requires its lane or a full run. All three audits use lockfile-only mode rather than asking the registry to validate restored `node_modules` trees ([REQ-OPS-003](../../sdd/spec/operations.md#req-ops-003-pr-checks-run-lint-test-typecheck-and-security-audit); [PR Checks](../../.github/workflows/test.yml) `jobs.quality` and `jobs.browser-ide`).
