...
| `test.yml` | PRs to `main` or `develop`, push to `main`, `merge_group`, `workflow_dispatch`, and `workflow_call` | Reusable parallel path-filtered quality (lint, knip, audit, seed drift), typecheck, workflow-audit, bundle-size, coverage, test-suite, host, and dependency-review lanes. One fail-closed action runs eight duration-weighted Workers backend groups, a Node leg, three frontend shards, and landing. Host tests run in two groups balanced from five measured native partitions. The required `test` summary fails failed/cancelled lanes and passes unaffected skipped lanes. |
