...
`workflow_run` already carries its exact green gate. After verification succeeds, `outcome` fails an eligible non-cancelled run in which nothing was deployed, so a green Deploy means a deploy happened. Workflow cancellation remains cancelled without forcing `outcome` to run. The same-repository green-`push` gate is repeated on every job because a job skipped via `if:` counts as success for `needs:` resolution. <!-- @impl: .github/workflows/deploy.yml::outcome --> <!-- @impl: scripts/ci/assert-deploy-outcome.mjs::deployOutcome -->
