...
| `test.yml` | PRs to `main` or `develop`, push to `main`, `merge_group`, `workflow_dispatch` + nightly schedule (all lanes) | Parallel path-filtered lanes: quality (lint/knip/audit/seed-drift), typecheck, bundle size, every vitest suite through one fail-closed action (four backend shards, a Node leg, three frontend shards, landing), host, dependency-review. The `summary` job (check name `test`, the required branch-protection context) fails on any failed/cancelled lane and passes skipped (unaffected) lanes. |
