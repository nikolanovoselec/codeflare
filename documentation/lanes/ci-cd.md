...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - Otherwise buildx uses the deployment-owned GHCR registry cache, emits plain per-layer timing, and uploads that log for 14 days. Pi extension source is copied after its expensive dependency/toolchain layers but before Jiti prewarm, while Browser IDE artifacts and generated seeds assemble after unrelated runtime installs; common source-only edits therefore retain dependency cache without retaining stale outputs. See [REQ-OPS-050](../../sdd/spec/operations.md#req-ops-050-hosted-image-build-critical-path-optimization).
   ...
   - One locked Trivy installation primes the runner-owned daily vulnerability and Java DBs, copies them into isolated per-process caches, then runs the HIGH/CRITICAL scan and CycloneDX generation concurrently with locked Wrangler preparation. The workflow awaits all three, applies `ignore-unfixed: true` plus `.trivyignore`, enforces the bounded JSON verdict, and uploads the SBOM before push — see [Security §Container Image Scanning](security.md#container-image-scanning-req-sec-011) and [REQ-OPS-050](../../sdd/spec/operations.md#req-ops-050-hosted-image-build-critical-path-optimization).
