...
**Workflow permissions:** top-level is `contents: read` (read-only default). Deployment's `container` job and PR Checks' complete-image job add `packages: write` only for the shared GHCR BuildKit cache. Fresh deployment image builds additionally receive `id-token: write` and `attestations: write` for provenance. Fork pull requests and Dependabot do not log in or read/write the shared cache; cache login or export failure falls back to an uncached build. GitHub also independently downgrades fork and Dependabot tokens. Code-scanning least-privilege hardening (#56).
...
3. **container** — calls `container-image.yml` to build, scan, and push the image, or to reuse an existing one:
   ...
   - If `in-<hash>` already exists in the target registry, the already-scanned image is reused and build/scan/push are skipped. That path relies on the scan, SBOM, and provenance evidence produced with the retained image; deployment does not regenerate or independently revalidate those artifacts. The weekly hash salt bounds the age of the reused CVE verdict.
   - Otherwise buildx builds with `ghcr.io/<owner>/<repo>/container-build-cache:linux-amd64`, the same registry-backed layer cache populated by trusted same-repository PR complete-image runs. Unlike GitHub's ref-scoped `type=gha` cache, this cache is readable from the later deployment branch ref, so deployment reuses the exact Docker layers instead of rerunning every install step. Fork pull requests and Dependabot build without cache credentials, and any cache login/export failure leaves the underlying image build running without that optimization. The base image comes from `public.ecr.aws/docker/library/node:24-bookworm-slim` (AWS ECR Public mirror avoids Docker Hub anonymous pull rate limits).
