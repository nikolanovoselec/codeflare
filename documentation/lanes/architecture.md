...
After the root creates an open protected-base PR or pushes one explicit branch whose destination has such a PR, `active-repo-memory.ts` resolves tool-level cwd, `git -C`, `cd … &&`, and fail-fast multiline `cd` context while leaving non-fail-fast transitions inert. `review-enforcement.ts` compares the pushed source ref with fresh PR branch and head state on a bounded retry schedule and emits one launch plan only for an exact match. If the live result handler is missed, the first settled pass after reload reconstructs that successful persisted boundary and emits its absent initial plan once; a live-evaluated ineligible boundary is marked inert.
