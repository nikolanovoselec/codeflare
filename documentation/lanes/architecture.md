...
A successful supported `gh pr merge` takes a separate post-command evidence path: the selected PR must be merged into `develop`, and its full merge commit must become the exact head of the open `develop → main/master` PR before the normal plan runs. A stale exact-shape downstream head remains pending for settled or reload recovery, keyed by boundary tool-use ID and exhausted after three attempts. Confirmed structural mismatches become inert immediately. If the live result handler is missed, settled recovery reconstructs the persisted boundary without duplicating an existing plan. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mergedDevelopPromotion --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::retainRetryableMerge --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->
