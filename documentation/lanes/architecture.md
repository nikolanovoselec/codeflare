...
Every native Pi request snapshots bounded active-document content, selection, open workspace files, diagnostics, explicit references, and native Chat history. Canonical path checks reject outside-workspace and symbolic-link escapes. A fresh `--mode rpc --no-session` child handles that request and streams assistant text and tool progress. Normal completion waits for `agent_settled`; cancellation completes after the correlated abort and bounded generation reaping. The participant never reads `request.model`, sends inference through the compatibility provider, or accesses an authentication provider.
