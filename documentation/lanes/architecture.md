...
The progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) has two phases. `proof.ts` restores context with authored `--i` indices, retaining the complete eight-row opening on desktop and the largest fully visible prefix inside the synchronized frame on mobile/tablet. The `.terminal.is-live .t-line` treatment reveals that prefix from top to bottom. During the existing three-second hold, a layout-neutral blinking caret rests on the final opening row without affecting its measured fit, adding a row, or advancing history. Each remaining context row and authored event then appends in order beneath the retained history and starts browser-controlled smooth scrolling; typing begins independently after the existing 420 ms phase at the 58 ms cadence.
