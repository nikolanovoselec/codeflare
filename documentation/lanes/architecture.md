...
Pi review is session-scoped and independent of CI ([AD98](../decisions/README.md#ad98-pi-pr-review-uses-visible-session-scoped-agents), [REQ-AGENT-055](../../sdd/spec/agents.md#req-agent-055-pi-session-scoped-review-window), [REQ-AGENT-059](../../sdd/spec/agents.md#req-agent-059-pi-native-review-findings-handoff)). The shared scope resolver produces one lane packet containing the normalized work set, exact ancestry-validated range, lane-owned files/hunks, and cross-lane changed inputs. Each changed input carries old/new hunk ranges; consumers call the shared intersection predicate before following an anchored symbol or named test, so path equality alone cannot fan out review scope. Diff-scoped reviewers retain their complete enforcement families but use gather-then-reason evidence waves: policy and packet inputs load once, deterministic checks and focused reads run in one batch, and any concrete unresolved candidates share one additional evidence batch. Context-mode carries those batches when available; after `/ctx off`, native `read`, `grep`, and `bash` perform the identical scoped work. Every scoped hunk and manifest row still receives a disposition. Whole-file reads require a hunk-backed candidate, retrieved evidence is not reopened, and unchanged baseline debt is reserved for full-tree scope. After a successful persisted root boundary, `active-repo-memory.ts` resolves `cd` and tool-level cwd context in both session modes; `review-enforcement.ts` then compares that repository's local `HEAD` with fresh protected-base PR state on a bounded retry schedule and emits one triggering launch plan only for an exact match. With a valid prior acknowledgement, the plan and each counted public reviewer prompt carry the exact acknowledged-to-current range; otherwise the full PR is reviewed. Unmatched calls remain in flight until native terminal notification, so queued or slow reviewers are not duplicated. Code, specification, and documentation lanes use the provider-neutral `medium` thinking profile without changing their scope or enforcement policy ([REQ-AGENT-087](../../sdd/spec/agents.md#req-agent-087-pi-reviewer-execution-profile)).
...
- [ ] [REQ-AGENT-086](../../sdd/spec/agents.md#req-agent-086-claude-reviewer-direct-evidence-and-root-handoff) — trigger a Claude SDD PR-boundary review; confirm each reviewer returns structured findings without writing files, then confirm the root alone persists triage and applies an approved fix.
