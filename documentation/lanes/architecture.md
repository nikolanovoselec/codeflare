...
OpenVSCode stays inside the session container behind the existing authenticated `/api/vscode/<sessionId>` proxy. The Pi inventory contains Codeflare's owned default native Chat participant plus its hidden local host-compatibility model. The Claude inventory contains Anthropic's exact official Open VSX extension; the unsupported inventory is empty. No Codeflare relay, public process API, second container, Copilot login, or VS Code Authentication bridge is introduced.

OpenVSCode 1.109.5 requires a non-optional model before entering a native participant. The account-free compatibility provider supplies only fixed panel-default model metadata, is not user-selectable, and rejects generation. Every native Pi request snapshots bounded active-document content, selection, open workspace files, diagnostics, explicit references, and native Chat history. Canonical path checks reject outside-workspace and symbolic-link escapes. A fresh `--mode rpc --no-session` child handles that request and streams assistant text and tool progress. Normal completion waits for `agent_settled`; cancellation completes after the correlated abort and bounded generation reaping. The participant never reads `request.model`, sends inference through the compatibility provider, or accesses an authentication provider.

Official Claude is installed unchanged at image build from a version- and checksum-pinned `linux-x64` VSIX. External settings point its bundled CLI at an isolated allowlisted config projection, select Anthropic Manual (`"default"`) permission mode, and disable OpenVSCode's unrelated native Chat/Copilot setup. Generic upstream Accounts controls remain separate; no Codeflare-owned path requests, bridges, exports, persists, or syncs their credentials. Anthropic's documented loopback IDE MCP uses a fresh private token to supply active selection, native diffs, and diagnostics. The owner accepts that local MCP and proprietary-package boundary under [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration).

Pi keeps the protected local manifest boundary: the guard sends one opaque ID plus SHA-256 digest, and the OpenVSCode extension host validates and returns one correlated decision. Edit, write, and Bash auto-confirm without opening editor documents; unknown and MCP actions use one modal. Cancellation races pending confirmation to denial, so a late result cannot enter another request. Claude auto-allows Edit, Write, NotebookEdit, Bash, and Task by tool name; WebFetch, WebSearch, unknown tools, Jupyter execution, and non-diagnostic MCP actions ask, while read-only IDE diagnostics proceed. <!-- @impl: openvscode/agent-sidebar/src/pi/vscode-approval-host.ts::VsCodeApprovalHost --> <!-- @impl: openvscode/claude/pre-tool-use-permission.mjs::evaluatePreToolUse -->
