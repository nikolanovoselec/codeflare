...
The compatibility provider preserves the native participant's account-free entry contract against the pinned Code OSS host. The host resolves defaults only from its reserved `copilot` vendor key, so the provider uses that internal selector without invoking GitHub Copilot or authorization. Its fixed metadata is selectable and default for panel and editor Inline Chat, reports tool calling for the pinned editor filter, and rejects generation.
