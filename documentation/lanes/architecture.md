...
When the root turn settles, the extension calls the already-published stock `@gotgenes/pi-subagents` service for every missing reviewer. Successful spawns append only head, range, lane, and agent ID to the session transcript. Reviewers and CI stay background records in that service, so its existing `AgentWidget` renders the same records with package-owned queued/running/finished states, animation, elapsed time, tool and token metrics, compaction count, terminal-width truncation, and timer cleanup. Codeflare adds no second widget renderer or redraw loop.
