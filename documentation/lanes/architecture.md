...
Responsive stability ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the Hero-scale pair side by side with equal stretched outer heights and balanced authored transcripts at desktop/tablet widths, stacks it on mobile, wraps long real command lines inside each frame, and reserves the completed active row—including its inline command prompt—while it types so terminal and page geometry remain fixed. Actual trailing-space acceptance remains a deployed manual check.
