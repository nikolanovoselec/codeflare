...
Pi CI monitoring has a separate owner and lifecycle ([AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent)). After an eligible Git action, the root launches every required reviewer first, then invokes the resolver once with the affected repository cwd and explicit review launch state. CI launches last without waiting for review completion. No stdout means no action; otherwise the returned request is submitted unchanged once through public `subagent`.
