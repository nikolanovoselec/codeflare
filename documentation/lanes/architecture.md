...
| Session readiness and metrics overlays | Session | Workers KV `session-editor:*` and `session-metrics:*` metadata | Session lifetime | Browser IDE routes and Container DO | Dashboard batch-status | Session lifecycle and deletion |
| Live-status correction | Session | Workers KV `session-status-correction:*` metadata | Bounded to 120 seconds | Container DO | Dashboard, access guards, limits, sync fan-out | Lifecycle tombstone or TTL |
