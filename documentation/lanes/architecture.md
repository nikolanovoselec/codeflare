...
Immediately after the two-member Hero family, the `#execution` overview ([REQ-LANDING-010](../../sdd/spec/landing.md#req-landing-010-execution-overview-reel)) presents permanent software-delivery and private-preview infrastructure terminals before the detailed proof sections. `ExecutionReel.astro` composes two thin `ExecutionRun.astro` instances from the shared `<Terminal>` and `<Transcript animate="feed">` components, with both runs sourced from `site.ts::EXECUTION`. Each server-rendered terminal exposes the complete semantic transcript and a populated resolved five-line viewport. The content uses concrete shell commands, test output, service state, and change evidence rather than actor-labelled marketing narration.

The progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) is owned by the same `proof.ts` row controller used by the existing rolling proof terminals. Normal motion restores five populated context rows before entry; a separate zero-margin observer starts each feed only when its terminal itself enters view. The shared 420 ms row transition pushes the oldest line out and the established 58 ms cadence types the next event at the bottom. Five events run once, then the feed settles without a cursor. Browsers without intersection observation keep the resolved feed static. Reduced motion ([REQ-LANDING-012](../../sdd/spec/landing.md#req-landing-012-execution-reel-reduced-motion-accessibility)) also leaves the populated resolved five-line viewports and complete semantic transcripts untouched. Capture readiness ([REQ-LANDING-013](../../sdd/spec/landing.md#req-landing-013-execution-reel-capture-readiness)) retains the stable `data-readme-reel="execution"` marker. Responsive stability ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the terminals side by side on desktop and tablet, stacks them on mobile, wraps complete rows within each card, and reserves each completed line's wrapped height while its visible copy types.
...
`proof.ts` adds `.is-live` to each `[data-proof]` artifact once on scroll-in for a one-shot reveal. Markup ships the resolved final state, so proof artifacts remain legible without JavaScript. Its shared row controller also owns the bounded REQ-LANDING-011 Transcript feeds described above; those feeds run once and do not extend the perpetual feature-terminal loop. `agentfoot.ts` adds a calm hero-terminal statusline with a slow context-percent tick and occasional compaction beat; the server-rendered foot is the reduced-motion and no-JS fallback.
...
The operations section (`id="operations"`) is a top-level peer placed directly before security, marked private preview with the shared capability-status badge, and presenting the "operate" surface of the Directed Execution Model as a governed infrastructure run in the security gate grammar (Zero-Trust-scoped reach, operator-approved plan, out-of-scope paths denied).
