...
The progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) has two phases. `proof.ts` first restores each complete context history with authored `--i` indices, and the shared `.terminal.is-live .t-line` treatment reveals them from top to bottom. After the entrance window, each event appends beneath the retained history and starts browser-controlled smooth scrolling into the clipped viewport; typing begins independently after the existing 420 ms phase at the 58 ms cadence. During command typing, the row's shared coral prompt moves into both overlaid copies so it remains beside the live text and contributes the same wrapped geometry to the hidden reservation. The authored simulation runs once, retains every authored row in the log, and settles with a blinking cursor.
