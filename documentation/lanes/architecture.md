...
| ready | 100% | Terminal sessions and pre-warm ready; sync may be complete, skipped, or running on demand. No browser terminal attaches until Open acknowledges readiness ([REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating)). |
| error | 0% | Startup-status handler or initial-sync failure |

Ready-stage terminal transition follows [REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating): `OPEN` only dismisses readiness. That transition lets the terminal hook create its first WebSocket attachment, and the host immediately replays serialized PTY state. No hidden preconnection or separate OPEN reconnect competes with the hook that owns transport cleanup. Ordinary visibility return still preserves connected sockets and restarts only disconnected or connecting panes. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @impl: web-ui/src/stores/terminal.ts::reconnectOnVisibilityReturn -->
