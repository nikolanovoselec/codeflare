...
- [ ] [REQ-AGENT-088](../../sdd/spec/agents.md#req-agent-088-review-ownership-and-triage) — run `/review --diff --deep` with at least two surfaced findings; confirm each subagent returns without writes, the root persists every report, triage records exactly one decision per finding, defer/ignore/debt decisions reach `sdd/.review-decisions.md`, and no fix is applied before explicit approval.
