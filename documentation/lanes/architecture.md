...
Directly under the primary hero, the landing renders a second hero band (`#inference-mesh`), a `<header>` that mirrors the primary hero rather than a `main > section`, positioning the private inference layer as part of the Codeflare family: the idle machines a company already owns become a private inference fabric for its agents, long-running sessions stay warm, and reaching for a hosted model is framed as a policy choice. The band is anchored as a sibling hero by the product's own operator-console lockup — `Codeflare` in white (carrying the shared `[data-scramble]` churn) over `Inference Mesh` in the coral flare — right-aligned on desktop to mirror the left proof terminal. It reuses the landing's static Astro composition and the shared Terminal/Transcript proof chrome — a concrete `codeflare-mesh` inference call whose bottom command line runs the shared typed reel (`data-ft-loop`) — rather than a new route or a new animation. ([REQ-LANDING-005](../../sdd/spec/landing.md#req-landing-005-inference-mesh-family-hero))
