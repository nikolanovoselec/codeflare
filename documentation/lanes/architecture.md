...
When the root turn settles, the extension calls the already-published stock `@gotgenes/pi-subagents` service for every missing reviewer. Successful spawns append only head, range, lane, and agent ID to the session transcript. The extension projects those same live service IDs into one above-editor widget whose rows show queued, running, completed, or failed state. Managed `maxConcurrent: 1` executes one memory-heavy reviewer at a time; CI is submitted last with queue bypass so monitoring remains independent. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::createReviewVisualController -->
