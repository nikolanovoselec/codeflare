...
Immediately after the two-member Hero family, the `#execution` overview ([REQ-LANDING-010](../../sdd/spec/landing.md#req-landing-010-execution-overview-reel)) presents one wide software-to-infrastructure run before the detailed proof sections. `ExecutionReel.astro` composes two instances of the reusable `ExecutionRun.astro` face on the shared `<Terminal>` chrome, with both runs sourced from `site.ts::EXECUTION`. The server render shows complete static transcripts. Its progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) collapses them into the fixed-height software face before entry, then starts one sequence at the first intersection: recent context fills, events append from the bottom, older history scrolls out, and the terminal flips once to private-preview infrastructure work.

`execution-reel.ts` owns that one deterministic fifteen-second sequence. It starts only on first intersection, mutates only aria-hidden visual copies, settles without looping, and leaves the infrastructure cursor active. The reduced-motion contract ([REQ-LANDING-012](../../sdd/spec/landing.md#req-landing-012-execution-reel-reduced-motion-accessibility)) keeps the complete two-run static layout when animation is disabled. The capture contract ([REQ-LANDING-013](../../sdd/spec/landing.md#req-landing-013-execution-reel-capture-readiness)) owns deterministic media states and the stable `data-readme-reel="execution"` marker. The responsive contract ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) contains the reel at supported viewport sizes without shifting surrounding content.
