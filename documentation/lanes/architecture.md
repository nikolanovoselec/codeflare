...
The dedicated `ci-monitor` timeout-bounds each GitHub command, verifies the authoritative PR head, and reports through Pi's native task notification. Review acknowledgement never waits for CI, and neither lifecycle recovers the other. After the public launch is recorded, a separate per-PR CI checkpoint prevents later sessions from repeating CI for the unchanged head without acknowledging review; enabling review still launches its lanes. Reload may abort monitoring; only an explicit user request can relaunch that head, while a later extension-issued plan applies to a changed head. See [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring).
