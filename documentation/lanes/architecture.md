...
**Transport reconstruction (running process, unreachable port):** `ctx.container.running` proves process existence, not that the Durable Object can still reach the private container port. `collectMetrics()` persists a streak only when both `/activity` and `/health` fail in the same tick, retries the first two complete failures after 5 s, then calls `ctx.abort()` on the third. This path does not write `stopped`, signal the container, run final sync, or bill the accelerated confirmation ticks ([REQ-SESSION-021](../../sdd/spec/session-lifecycle.md#req-session-021-unreachable-container-transport-reconstructs-without-replacing-the-workload) AC1-AC7). The SDK constructor's running-container path is expected to reattach its monitor; browser reconnection to the existing PTY remains a deployed smoke check, not a unit-tested contract.
