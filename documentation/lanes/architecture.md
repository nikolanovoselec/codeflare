...
A fresh `--mode rpc --no-session` child handles that request and streams assistant text and tool progress. Normal completion waits for `agent_settled`; cancellation completes after correlated abort and bounded generation reaping. Pi's documented `select` and `input` requests use cancellable native VS Code dialogs, bounded RPC timeouts, and correlated responses; malformed or unknown blocking UI requests remain fail-closed. The selectable model's tool capability is pinned-host eligibility metadata and its distinct vendor avoids Copilot setup; generation stays fail-closed and inference remains direct Pi RPC without authentication. Pi writes and commands are unrestricted direct effects, not transactional Inline Chat edits with a Keep/Undo guarantee.
