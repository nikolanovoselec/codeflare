...
> **Strict Gateway Egress (Enterprise Mode):** For the optional toggle that forces the container's direct-internet egress through the customer's Cloudflare Gateway (only this deployment's own-account R2 + account-scoped CF API / Browser Rendering egress direct; the AI Gateway upstream always does), see the [EgressController](#egresscontroller-strict-gateway-egress-enterprise-mode) component and the [Strict Gateway Egress](#strict-gateway-egress) data flow.
...
sequenceDiagram
    ...
    Note over X: Own-account R2 + account-scoped CF API / Browser Rendering<br/>are exempt via isAccountScopedDestination(url, accountId) and egress direct;<br/>other accounts ride the Gateway; own-account R2 re-signed with the worker key;<br/>WS upgrades bridged via WebSocketPair (AD86/AD87)
...
- [REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress) - Strict Gateway Egress (EgressController catch-all with account-scoped exemption + WebSocket proxying + GitHub interceptor transport swap; fail-closed)
