...
Responsive stability ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the Hero-scale pair side by side at desktop/tablet widths and stacks it on mobile. With normal-motion enhancement at mobile and tablet widths, `proof.ts::syncExecutionFrames` measures the rendered Hero terminal and writes one shared frame height for both Execution terminals; a Hero `ResizeObserver`, font readiness, and viewport resize keep that value current before exposure. Desktop retains the equal-height grid pair, while no-JavaScript and reduced-motion rendering use intrinsic sizing and complete resolved viewports.
