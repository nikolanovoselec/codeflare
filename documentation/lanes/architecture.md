...
The progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) has two phases. `proof.ts` restores complete context with authored `--i` indices. The first eight rows establish base sizing; equal-height stretching can expose additional context. The `.terminal.is-live .t-line` treatment reveals every populated row fitting the opening viewport from top to bottom, while further context stays retained beyond its clipped edge. After the entrance window, each event appends beneath the retained history and starts browser-controlled smooth scrolling into the clipped viewport; typing begins independently after the existing 420 ms phase at the 58 ms cadence.
