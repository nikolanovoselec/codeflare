...
The dedicated `ci-monitor` timeout-bounds each GitHub command, verifies the authoritative PR head, and reports through Pi's native task notification. Review acknowledgement never waits for CI, and neither lifecycle recovers the other. The resolver carries canonical GitHub repository, PR number, head, and local repository path as one JSON identity. After the public launch returns a successful tool result matching all four fields, a separate per-PR CI checkpoint prevents later sessions from repeating CI for the unchanged head without acknowledging review; failed or mismatched launches remain retryable, and enabling review still launches its lanes. Reload may abort monitoring; only an explicit user request can relaunch that head, while a later extension-issued plan applies to a changed head. See [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring).
