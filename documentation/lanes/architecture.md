...
When the root turn settles, the extension calls the already-published stock `@gotgenes/pi-subagents` service for every missing reviewer. Successful spawns append only head, range, lane, and agent ID to the session transcript. Managed `maxConcurrent: 1` executes one memory-heavy reviewer at a time; CI is submitted last with queue bypass so monitoring remains independent. Matching `subagents:record` entries prove completion by agent ID. A dispatch unknown to the post-reload service remains in flight, while a live service status of stopped, aborted, or error makes that agent eligible for retry. Service-owned launches remain visible but do not appear as assistant public-tool-call blocks. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts -->
