...
| ready | 100% | Terminal sessions and pre-warm ready; sync may be complete, skipped, or running on demand. Open creates a fresh attachment for every visible pane and restores current host screen state ([REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating)). |
| error | 0% | Startup-status handler or initial-sync failure |

Ready-stage terminal transition follows [REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating): `OPEN` replaces every visible browser attachment for the active session before dismissing readiness. Each fresh host attachment replays serialized PTY state, replacing a screen that went stale while the browser was suspended. Ordinary visibility return still preserves connected sockets and restarts only disconnected or connecting panes. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/stores/terminal.ts::restartVisibleTerminals --> <!-- @impl: web-ui/src/stores/terminal.ts::reconnectOnVisibilityReturn -->
