...
Pi keeps the protected local manifest boundary: the guard sends one opaque ID plus SHA-256 digest, and the OpenVSCode extension host validates and returns one correlated decision. Routine edit, write, and Bash calls auto-confirm without opening editor documents; unknown and MCP actions use one modal. Cancellation races pending confirmation to denial, so a late result cannot enter another request. Claude's managed settings disable bypass and automatic modes; routine local built-ins proceed, while network, unknown, MCP, and Jupyter execution actions ask.
