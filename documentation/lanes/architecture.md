...
The Pi inventory immediately activates Codeflare's owned provider before initial model-picker resolution and contains one owned default participant for panel and editor Inline Chat plus hidden-fallback and selectable local host-compatibility models. Every fixed inventory seeds Code OSS's supported web-profile visibility value to add only `chat.statusBarEntry` to the existing hidden-entry list, removing the Copilot sign-in status control without patching code-server or disabling authentication. Shared web-profile state also hides the left-side Accounts control without removing authentication APIs. <!-- @impl: openvscode/agent-sidebar/package.json::activationEvents --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/claude/prepare-sidebar-config.mjs::writeOpenVscodeProfileState -->
