...
The progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) has two phases. `proof.ts` restores context with authored `--i` indices, retaining the complete eight-row opening on desktop and the largest fully visible prefix inside the synchronized frame on mobile/tablet. The `.terminal.is-live .t-line` treatment reveals that prefix from top to bottom. After the entrance window, every remaining context row and authored event appends in order beneath the retained history and starts browser-controlled smooth scrolling; typing begins independently after the existing 420 ms phase at the 58 ms cadence.
...
Responsive stability ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the Hero-scale pair side by side at desktop/tablet widths and stacks it on mobile. With normal-motion enhancement at mobile and tablet widths, `proof.ts::syncExecutionFrames` measures the rendered Hero terminal and writes one shared frame height for both Execution terminals. Initial layout and font readiness set the value before exposure; an actual viewport-width change refreshes it. The Hero command animation is deliberately not observed, so typing cannot resize visible Execution frames. Desktop retains the equal-height grid pair, while no-JavaScript and reduced-motion rendering use intrinsic sizing and complete resolved viewports.

Under the responsive stability contract ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)), each synchronized log promotes any context that cannot fully fit into the typed queue before animation starts, so the first appended row advances from visible history instead of skipping hidden pre-event rows. Each live log is size-contained in a `minmax(0, 1fr)` track; the terminal body clips overflow and exposes no user scrolling, while the script advances appended history with the contiguous line rhythm used by the other landing terminals. The newest row remains bottom-aligned during typing and after viewport resizing.
