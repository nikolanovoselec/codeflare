...
README media reuses the deployed landing artifacts rather than maintaining screenshot-only product variants ([REQ-LANDING-013](../../sdd/spec/landing.md#req-landing-013-canonical-readme-media-capture)). Execution, Browser E2E, and deployment are one-shot GIFs; the Browser VS Code, review, and Inference Mesh views retain their canonical loops. Each animation has an exact-dimension PNG fallback selected by reduced-motion media, descriptive alt text, and repository-enforced per-file and aggregate budgets. The Architecture section keeps the Mermaid topology instead of duplicating the removed IDE screenshot. Final status remains Partial until GitHub renders all six blocks legibly from the branch.
