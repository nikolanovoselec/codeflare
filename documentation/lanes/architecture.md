...
After a successful persisted root boundary, `active-repo-memory.ts` resolves `cd` and tool-level cwd context. `review-enforcement.ts` compares that repository's local `HEAD` with fresh protected-base PR state on a bounded retry schedule and emits one passive launch plan only for an exact match. A valid acknowledgement yields the acknowledged-to-current range; otherwise the full PR is reviewed. <!-- @impl: preseed/agents/pi/extensions/active-repo-memory.ts::rememberActiveRepoFromToolResult --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->

When the root turn settles, the extension calls the already-published stock `@gotgenes/pi-subagents` service for every missing reviewer. Successful spawns append only head, range, lane, and agent ID to the session transcript. Matching `subagents:record` entries prove completion by agent ID; absent records remain in flight across reload, so queued or slow reviewers are not duplicated. Service-owned launches remain visible but do not appear as assistant public-tool-call blocks. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sendLaunchMessage --> <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts -->
