...
Immediately after the two-member Hero family, the `#execution` overview ([REQ-LANDING-010](../../sdd/spec/landing.md#req-landing-010-execution-overview-reel)) presents permanent software-delivery and private-preview infrastructure terminals before the detailed proof sections. `ExecutionReel.astro` composes two instances of the reusable `ExecutionRun.astro` on the shared `<Terminal>` chrome, with both runs sourced from `site.ts::EXECUTION`. Each server-rendered terminal contains five stable context lines and five complete event lines. Its progressive-motion contract ([REQ-LANDING-011](../../sdd/spec/landing.md#req-landing-011-execution-reel-progressive-motion)) keeps both terminals visible, starts each timeline independently when that terminal enters view, and progressively types its five reserved event lines without flipping, scrolling, or removing completed history.

`execution-reel.ts` owns the two deterministic one-shot timelines. It mutates only aria-hidden visual copies, settles each terminal after its fifth event, and leaves no active cursor. The reduced-motion contract ([REQ-LANDING-012](../../sdd/spec/landing.md#req-landing-012-execution-reel-reduced-motion-accessibility)) keeps both complete static transcripts when animation is disabled. The capture contract ([REQ-LANDING-013](../../sdd/spec/landing.md#req-landing-013-execution-reel-capture-readiness)) owns deterministic media states and the stable `data-readme-reel="execution"` marker. The responsive contract ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the terminals side by side on desktop and tablet, stacks them on mobile, and contains them without shifting surrounding content.
...
`proof.ts` adds `.is-live` to each `[data-proof]` artifact once on scroll-in for a one-shot reveal. Markup ships the resolved final state, so proof artifacts remain legible without JavaScript. `execution-reel.ts` independently owns the bounded REQ-LANDING-011 sequence described above; it does not extend the perpetual feature-terminal loop or the proof-row roller. `agentfoot.ts` adds a calm hero-terminal statusline with a slow context-percent tick and occasional compaction beat; the server-rendered foot is the reduced-motion and no-JS fallback.
