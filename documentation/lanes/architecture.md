...
The software session follows one checkout-timeout repair from read-only diagnosis and test-first implementation through explicit `code-reviewer`, `doc-updater`, and `spec-reviewer` review, exact-head CI, merge, and branch realignment. The infrastructure session follows one nginx incident from read-only retained-coredump diagnosis through check mode, approved snapshot and patching, restart, health verification, and closure. Both initial histories and settled viewports alternate engineer requests or approvals with resulting agent evidence, keeping human intent visible throughout. <!-- @impl: landing/src/content/site.ts::EXECUTION -->
