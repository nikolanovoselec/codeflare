...
- [ ] [REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) — start a fresh Pi session and call one `ctx_*` tool; run `/ctx off` and confirm root `ctx_*` tools disappear, then `/ctx on` and confirm working tools return.
