...
| ready | 100% | Terminal sessions and pre-warm ready; the existing browser attachment remains behind the readiness overlay until Open enables focus ([REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating)). |
| error | 0% | Startup-status handler or initial-sync failure |

Terminal attachment follows [REQ-TERM-043](../../sdd/spec/terminal.md#req-term-043-visible-terminal-readiness-gating): once startup reaches `mounting`, the visible tab attaches exactly once and adopts the prepared PTY behind the readiness overlay. `OPEN` only dismisses readiness and enables focus; it does not create or replace transport. This preserves the prewarm lifecycle that predates the ready-stage regression while keeping one hook responsible for connection cleanup. Under [REQ-TERM-044](../../sdd/spec/terminal.md#req-term-044-terminal-restore-and-readiness-rendering), reattaching an existing PTY still receives serialized host replay. Readiness keeps the renderer layout-active but opacity-concealed, and temporarily unavailable dimensions yield to later resize instead of aborting OPEN. Ordinary visibility return restarts only disconnected or connecting panes. <!-- @impl: web-ui/src/components/Layout.tsx::handleOpenSessionById --> <!-- @impl: web-ui/src/components/Terminal.tsx::Terminal --> <!-- @impl: web-ui/src/hooks/useTerminal.ts::useTerminal --> <!-- @impl: web-ui/src/lib/xterm-internals.ts::resyncViewportScrollState --> <!-- @impl: web-ui/src/stores/terminal.ts::reconnectOnVisibilityReturn -->
