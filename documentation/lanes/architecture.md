...
After the root creates an open protected-base PR or pushes one explicit branch whose destination has such a PR, `active-repo-memory.ts` resolves tool-level cwd, `git -C`, `cd … &&`, and fail-fast multiline `cd` context while leaving non-fail-fast transitions inert. `review-enforcement.ts` compares the pushed source ref with fresh PR branch and head state on a bounded retry schedule and emits one launch plan only for an exact match. <!-- @impl: preseed/agents/pi/extensions/active-repo-memory.ts::rememberActiveRepoFromToolResult --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->

The Pi and Claude post-command merge-promotion paths proceed only after GitHub reports the selected PR merged into `develop` and the open canonical `develop → main/master` PR at that exact merge commit. A stale exact-shape downstream head remains pending for settled or reload recovery, keyed by boundary tool-use identity and exhausted after three attempts. Confirmed structural mismatches become inert immediately. Pi persists recovery in session entries; Claude uses validated atomic state under the shared Git common directory, then overrides ambient source-branch state with the downstream PR and head before entering its existing Stop lifecycle. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mergedDevelopPromotion --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/lib/gh-pr-state.sh::resolve_develop_merge_boundary --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::MERGE_OVERRIDE -->
