...
Pi review is session-scoped and independent of CI ([AD98](../decisions/README.md#ad98-pi-pr-review-uses-visible-session-scoped-agents), [REQ-AGENT-055](../../sdd/spec/agents.md#req-agent-055-pi-session-scoped-review-window), [REQ-AGENT-059](../../sdd/spec/agents.md#req-agent-059-pi-native-review-findings-handoff)). Diff-scoped reviewers may inspect wider context but report only defects introduced or newly exposed by that diff; unchanged baseline debt is reserved for full-tree scope. After a successful persisted root boundary, `review-enforcement.ts` checks the current protected-base PR and emits a triggering follow-up; settled recovery rechecks the PR when GitHub initially reports the previous head. With a valid prior acknowledgement, the reminder and each counted public reviewer prompt carry the exact acknowledged-to-current range; otherwise the full PR is reviewed. Unmatched calls remain in flight until native terminal notification, so queued or slow reviewers are not duplicated.

Only the reminder head can be acknowledged after every required correlated notification. Reload can lose unfinished work but cannot fabricate completion; a later supported boundary may repeat it. Pi has no pre-command merge interceptor or durable review execution state. See [REQ-AGENT-036](../../sdd/spec/agents.md#req-agent-036-pr-boundary-review-trigger-conditions), [REQ-AGENT-053](../../sdd/spec/agents.md#req-agent-053-pi-native-review-result-correlation), [REQ-AGENT-055](../../sdd/spec/agents.md#req-agent-055-pi-session-scoped-review-window), [REQ-AGENT-058](../../sdd/spec/agents.md#req-agent-058-supported-boundary-recovery), [REQ-AGENT-071](../../sdd/spec/agents.md#req-agent-071-pr-boundary-review-agent-dispatch), and [REQ-AGENT-074](../../sdd/spec/agents.md#req-agent-074-pi-settled-review-handoff).

### Pi CI Monitoring Data Flow

Pi CI monitoring has a separate owner and lifecycle ([AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent), [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring)). After an eligible Git action, the root launches every required reviewer first, then invokes the resolver once with the affected repository cwd and explicit review launch state. CI launches last without waiting for review completion. No stdout means no action; otherwise the returned request is submitted unchanged once through public `subagent`.

The dedicated `ci-monitor` timeout-bounds each GitHub command, verifies the authoritative PR head, and reports through Pi's native task notification. Review never owns CI recovery. Reload may abort monitoring; only a later eligible Git action or explicit user request can launch another monitor. See [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring).
