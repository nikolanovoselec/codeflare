...
Directly under the primary hero, the landing renders a second hero band (`#inference-mesh`), a `<header>` that mirrors the primary hero rather than a `main > section`, positioning the private inference layer as part of the Codeflare family: the idle machines a company already owns become private inference capacity for its agents, long-running sessions stay warm, and reaching for a hosted model is framed as a policy choice. The band reuses the landing's static Astro composition, the shared Terminal/Transcript proof chrome (a concrete `codeflare-mesh` inference call in the page's type-on-view cursor treatment), and the shared `[data-scramble]` hook on the `inference mesh` phrase, rather than a new route or a new animation. ([REQ-LANDING-005](../../sdd/spec/landing.md#req-landing-005-inference-mesh-family-hero))
