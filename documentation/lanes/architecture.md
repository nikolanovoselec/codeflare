...
- [ ] [REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) — start a fresh Pi session and call one `ctx_*` tool; launch code/spec/doc reviewers and confirm their tool manifests contain `bash` but no `ctx_*`; after they finish, verify the Pi process owns exactly one `context-mode/server.bundle.mjs` child; run `/ctx off` and confirm root `ctx_*` tools disappear, then `/ctx on` and confirm one foreground bridge and working tools return.
