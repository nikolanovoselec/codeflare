...
`execution-reel.ts` owns that one deterministic fifteen-second sequence. It starts only on first intersection, mutates only aria-hidden visual copies, settles without looping, and leaves the infrastructure cursor active. The accessibility and layout contract ([REQ-LANDING-012](../../sdd/spec/landing.md#req-landing-012-execution-reel-accessibility-and-layout-stability)) keeps the complete two-run static layout for reduced-motion and no-JavaScript visitors, contains the reel at supported viewport sizes, and prevents the fixed visual viewport from shifting surrounding content. The reel carries a stable `data-readme-reel="execution"` marker so the deployed component can later become a GitHub README asset without a duplicate capture-only UI.
