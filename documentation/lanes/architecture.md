...
A successful supported `gh pr merge` takes a separate post-command evidence path: the selected PR must be merged into `develop`, and its full merge commit must become the exact head of the open `develop → main/master` PR before the normal plan runs. If the live result handler is missed, the first settled pass after reload reconstructs that successful persisted boundary and emits its absent initial plan once; a live-evaluated ineligible boundary is marked inert. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mergedDevelopPromotion --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->
