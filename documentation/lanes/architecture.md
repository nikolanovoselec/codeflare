...
Memory snapshots carry the bounded transcript inline as their sole conversation input—there is no secondary `INPUT_FILE` or transcript path. Each launch puts identical bounded request items in model-facing custom-message content and durable details metadata; root-session JSONL supplies launch/reminder counts and correlates native terminal notifications by tool-use ID. Missing or failed work receives the initial directive plus five reminders, then GIVEUP; no queue, receipt, lease, scheduler, or private spawn service exists. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::registerMemoryVault --> <!-- @impl: preseed/agents/pi/extensions/memory-vault-helpers.ts::extractionTranscriptFacts -->

For memory, the root snapshots only prompts after the last successful counter (at most 40 text turns × 4000 characters). The worker writes the note once, then `build-memory-graph.py` deterministically derives its semantic H1 document node, canonical concept nodes, and unique reference/concept edges before publication. The shared merge normalizes final node-link edges by source/target/relation/source-file after Graphify conversion and copies those exact cumulative bytes to `graph.json`, preventing conversion duplicates or stale reduced outputs. Exact native success qualifies only when the note and request chunk both appear after graph publication; the root then advances the counter with `max(current, frozenPromptCount)` and removes only matching state. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::finalizeMemorySuccess -->

For Vault edits, prelaunch changes coalesce under one request ID, launched work stays frozen, and later edits become one follow-up. The full content-hash manifest is staged beside the committed manifest and promoted by same-directory rename only after exact success, post-commit chunk qualification, and hash validation; rename-before-cleanup recovery accepts matching committed bytes idempotently. <!-- @impl: preseed/agents/pi/extensions/memory-vault.ts::finalizeVaultSuccess --> <!-- @impl: preseed/agents/pi/extensions/vault-manifest-fs.ts::promoteVaultManifest -->
...
After a successful persisted root boundary, command parsing resolves shell `cd`, Git `-C`, explicit push refspecs, and `gh pr create --head`. `review-enforcement.ts` uses that command-selected repository and PR target, verifies fresh protected-base PR state on a bounded retry schedule, and appends one extension-only review window only for the authoritative head. The window never enters LLM context. A valid acknowledgement yields the acknowledged-to-current range; otherwise the full PR is reviewed. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::classifyReviewBoundaryCommand --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::boundaryContext --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->

When the root turn settles, the extension calls the already-published stock `@gotgenes/pi-subagents` service for every missing reviewer. Successful spawns append only head, range, lane, and agent ID to the session transcript. The public service records omit the invocation metadata used by the stock widget's background filter, so the boundary extension projects only its correlated reviewer/CI IDs through the stock-derived `review-widget-renderer.ts`. It copies the package's queued/running/finished layout, active-first overflow, spinner cadence, terminal-width truncation, metrics, and timer lifecycle while retaining the public service as the sole execution path.

Managed `maxConcurrent: 1` executes one memory-heavy reviewer at a time; CI is submitted last with queue bypass so monitoring remains independent. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::dispatchReviewWindow -->

Matching `subagents:record` entries prove completion by agent ID. A dispatch unknown to the post-reload service remains in flight, while a live service status of stopped, aborted, or error makes that agent eligible for retry. Service-owned launches do not appear as assistant public-tool-call blocks. <!-- @impl: preseed/agents/pi/extensions/review-helpers.ts::reviewTranscriptFacts -->

Each successful reviewer completion queues an adjacent model-facing triage reminder after the stock notification. The reminder requires one consolidated row per finding under `FINDING | PROPOSED FIX | STATUS | DECISION`, adversarial validity and proportionality review, and the smallest accepted fix. The final reviewer marks the wave complete; the root publishes the table and applies legitimate fixes automatically unless the user explicitly requested approval. CI completion is excluded. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::createReviewTriageController --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::reviewTriageReminder -->
...
Pi CI monitoring has a separate execution lifecycle ([AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent), [REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring)). The PR-boundary extension owns two structured waves: required reviewers first, then independent CI. After every requested reviewer returns an agent ID, the extension invokes the existing resolver with the affected repository cwd and explicit review launch state. It translates a returned request to equivalent stock-service options and spawns it once; an empty result is persisted as resolved so reload cannot duplicate resolution.

CI launches last without waiting for review completion. Non-SDD repositories and default-mode sessions receive CI-only plans for eligible boundaries.
