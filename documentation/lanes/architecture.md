...
Official Claude is installed unchanged at image build from a version- and checksum-pinned `linux-x64` VSIX. External settings point its bundled CLI at an isolated allowlisted config projection, select Anthropic Manual (`"default"`) permission mode, and disable OpenVSCode's unrelated native Chat/Copilot setup. Generic upstream Accounts sessions remain memory-only for the current workbench; Codeflare does not use or persist them. Anthropic's documented loopback IDE MCP uses a fresh private token to supply active selection, native diffs, and diagnostics. The owner accepts that local MCP and proprietary-package boundary under [AD114](../decisions/README.md#ad114-native-pi-chat-and-the-official-claude-extension-own-editor-integration).
