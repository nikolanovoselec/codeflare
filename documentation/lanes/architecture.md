...
Only the reminder head can be acknowledged after every required correlated notification. The root publishes triage, makes no mutation, and ends that turn; `agent_settled` writes the existing full-SHA acknowledgement and queues one separate FIX follow-up. This barrier preserves the reviewed head as the next push's incremental range base.
