...
**Authority:** An active authenticated user may create the record. Creation and start are distinct operations: creation does not consume a concurrent-running slot; start checks role/tier concurrency and compute quota, binds the Container DO, then writes KV `running` before asynchronous startup. REQ-SESSION-007 requires a hard per-user cap, but the current KV count and later status write are not atomic, so simultaneous starts can exceed it; this is an incomplete runtime contract, not an accepted policy relaxation. Deployment `max_instances` is the separate hard platform capacity boundary. Successful host readiness owns service availability.
...
| Concurrent-session cap | Non-SaaS defaults: 3 per user, 10 per admin; SaaS varies by effective tier | REQ-SESSION-007 requires a hard cap; the current KV check/write gap is an incomplete runtime contract that permits simultaneous starts to exceed it | [Configuration](configuration.md#worker-environment), [Billing](billing.md), [REQ-SESSION-007](../../sdd/spec/session-lifecycle.md#req-session-007-running-session-count-limited-per-tier) |
