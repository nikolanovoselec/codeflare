...
Responsive stability ([REQ-LANDING-014](../../sdd/spec/landing.md#req-landing-014-execution-reel-responsive-layout-stability)) keeps the Hero-scale pair side by side with equal stretched outer heights at desktop/tablet widths and stacks it on mobile. Each feed overlays its live rows on a hidden stack of every authored viewport, so the tallest state fixes the terminal body while the visible rows retain the contiguous line rhythm used by the other landing terminals. Matching context/event replacement budgets keep both eight-row feeds at fourteen explicit, meaningful lines; approved lines remain verbatim as each row's first line and concise evidence continues below them. Long commands wrap inside each frame, while the completed active row—including its inline prompt—is reserved during typing so terminal and page geometry remain fixed. Rendered trailing space and transition continuity remain deployed manual checks.
