...
`pi-goal-list-loop-audit` is pinned at 0.34.16 after review of the published npm tarball and repository provenance (MIT, exact declared `extensions/loops/goal.ts` entrypoint, no runtime dependencies beyond Pi peers). The image's derived package warm-up loads it through the same real npm path used at runtime and fails unless the Goal jiti artifact is present.
