...
A transient `gh` API failure during the PR-state check does not strand a live round: the hook caches the last successful `gh_pr_state` answer per branch and reuses it, but only while that PR's plan file is on disk and only for failures gh does not answer authoritatively — the "no PR" exit keeps the old fail-safe silence, as does an idle branch, and a superseded cache is neutralized by the head-equality gate. Every blocking Stop directive opens with `[review directive — hook working as designed, not an error]`, because Claude Code renders each one as "Stop hook error: …" and that label cannot be changed from the hook. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::PRINFO_CACHE --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::REVIEW_DIRECTIVE_TAG -->
