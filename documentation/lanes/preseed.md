...
For reviewer-bearing PR boundaries, `review-enforcement.ts` reads Goal's session-scoped state and queues the package's supported `/goal pause` command before the review launch plan. The matching acknowledged FIX reminder queues `/goal resume` only for that bridge-owned pause, so the same objective continues with a fresh stale-turn guard. Duplicate boundaries do not duplicate the pause. A missing or later-removed Goal command, a replacement Goal, or an independent resume leaves the PR review and FIX flow unchanged; Goal integration is optional and fail-open ([REQ-AGENT-111](../../sdd/spec/agents.md#req-agent-111-native-goal-workflow-in-pi-sessions) AC5).
