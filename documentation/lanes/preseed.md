...
Code, specification, and documentation reviewers use the shared provider-neutral medium profile. <!-- @impl: preseed/agents/pi/agents/code-reviewer.md::thinking: medium --> <!-- @impl: preseed/agents/pi/agents/spec-reviewer.md::thinking: medium --> <!-- @impl: preseed/agents/pi/agents/doc-updater.md::thinking: medium --> Agent-end enforcement reads current live session state, revalidates the exact identity, writes user-scoped completion, and emits one FIX follow-up. That follow-up orders the push after accepted corrections are complete; it never authorizes merge, deployment, a no-op commit, or a push when triage accepted no file change. The push starts a fresh exact-head boundary. <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::sendFixFollowUp --> Reload never recovers partial work.
