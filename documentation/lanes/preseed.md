...
Dependencies are read from npm exactly and from every other manifest by a best-effort heuristic over two token shapes: a quoted name, and the first token on a line. That covers `Cargo.toml`, `pyproject.toml`, `requirements.txt`, `go.mod`, `Gemfile` and `composer.json`; it does not recover a Gradle `group:artifact:version` coordinate or a Mix `{:dep, ...}` tuple, and a name it misses is reported unresolved rather than wrongly resolved. Excluding directives everywhere ate `build`, `api`, `path` and `version`, which are real published packages.

What a leading token means depends on the manifest grammar. In `go.mod`, `Gemfile`, `Package.swift`, `mix.exs` and a Gradle build it is syntax, so `gem`, `source` and `require` are excluded. In `Cargo.toml`, `pyproject.toml` and a `Pipfile` it is a key, and a key is a package name only where the file's own structure says so. Three shapes admit it, and two of them are read in opposite directions. A heading whose last dotted segment names a dependency table -- `[dependencies]`, `[dev-dependencies]`, `[build-dependencies]`, `[tool.poetry.dependencies]`, `[packages]` -- puts its packages in the body. A heading whose FIRST segment names that table instead, Cargo's `[dependencies.serde]`, names the package in the heading and fills the body with that package's own metadata, so the name is taken from the heading and the body is not admitted; reading it the other way resolves `version` as a published crate. A heading that merely contains the word, `[tool.setuptools.packages.find]`, admits nothing. Outside any such heading a line is admitted by its own key, which is what reaches PEP-621's `dependencies` and `[build-system]`'s `requires` under `[project]`; because both write a list across several lines, the admission is held open until the closing bracket, or every package sits on a continuation line matching nothing and the file silently resolves empty. Everywhere else every line is one dependency.
