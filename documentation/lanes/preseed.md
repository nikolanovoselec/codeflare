...
A direct current-session user instruction to go **FULLY AUTONOMOUS** activates a narrower task-scoped override: it supersedes only the five-round autonomous-commit stop. The root carries `autonomy_override=fully-autonomous` in every later reviewer prompt for that task. A later direct user cancellation/narrowing instruction deactivates it; after every requested gate completes, the root emits `autonomy_override=complete` in its terminal response. Review, CI, test, specification, deployment, and root-only mutation gates remain unchanged ([REQ-AGENT-091](../../sdd/spec/agents.md#req-agent-091-explicit-fully-autonomous-direction-overrides-the-review-round-stop)).
