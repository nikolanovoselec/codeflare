...
Stop directives are delivered on stderr with exit 2, not as a JSON `{"decision":"block"}`. The JSON form is rendered by the client through a fixed `<event> hook error: …` template that the hook cannot override, so every directive this gate issued — all of them working as designed — reached the user labelled a failure. Exit 2 against a hook entry carrying `asyncRewake` takes the rewake path instead: the model is woken with the same text under a "Stop hook feedback" banner, and the entry's `rewakeSummary` names the line the user sees. The registration in `entrypoint.sh` must carry `asyncRewake`; without it exit 2 is a blocking error again. The full FIX contract is delivered once per PR and every later round gets a one-line form that keeps the load-bearing clauses. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::FIX_SHOWN_FILE --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::emit_block -->
