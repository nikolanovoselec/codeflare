...
Default and advanced Pi projections include an on-demand Herdr orchestration skill. It first checks for a live Herdr pane, then documents the local CLI needed to create helper panes, start and steer named coding agents, wait on bounded lifecycle states, and read their output. Plain Codeflare terminals do not start Herdr. `SYSTEM.md` carries only the conditional skill link ([REQ-AGENT-173](../../sdd/spec/agents.md#req-agent-173-pi-can-orchestrate-coding-agents-through-herdr)). <!-- @impl: preseed/agents/pi/SYSTEM.md --> <!-- @impl: preseed/agents/pi/skills/herdr/SKILL.md -->
