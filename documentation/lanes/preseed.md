...
| Pi extension packages (`@juicesharp/rpiv-advisor`, `@juicesharp/rpiv-ask-user-question`, `@juicesharp/rpiv-todo`, `pi-web-access`, `pi-mcp-adapter`, `pi-goal-list-loop-audit`) | Yes (always-on `required`) | Yes (always-on `required`) | Yes (always-on `required`) |
...
`pi-goal-list-loop-audit` is pinned at 0.34.16 after review of the [published npm tarball](https://registry.npmjs.org/pi-goal-list-loop-audit/-/pi-goal-list-loop-audit-0.34.16.tgz), which declares the MIT license, `extensions/loops/goal.ts` entrypoint, and only peer dependencies on Pi packages and typebox. The image explicitly warms its entrypoint through the same real npm path used at runtime, derives jiti's expected cache filename from that resolved path, and fails unless the exact Goal artifact is present ([REQ-AGENT-111](../../sdd/spec/agents.md#req-agent-111-reviewed-goal-workflows-in-pi-sessions) AC2/AC3).
