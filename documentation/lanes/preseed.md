...
The shared compiler and runtime ABI remain Codeflare-owned. Compiler-pin automation compares three compiler scripts and three managed npm lockfiles from an exact successful `main` or `develop` deployment. It advances only when one of those six inputs differs, then curation CI validates that exact checkout. A newer source checkpoint or native-only image change does not by itself move the pin. Never copy dirty or untracked compiler files into the private repository.
