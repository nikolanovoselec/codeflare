...
On the normal non-bypassed path, an automatic delivery or clone-confirmed boundary makes the extension emit a numbered Markdown runbook with PR/head/scope context, deterministic reviewer output paths, and the order `REVIEWERS → CI → JOINT TRIAGE + ACK → FIX`. Reviewer calls start together and CI starts immediately afterward. Joint triage waits for every required reviewer plus terminal exact-head CI success, failure, or timeout from the correlated monitor. Failure and timeout require a dedicated `Exact-head CI` row whose proposed-fix cell is exactly `CI_RESULT failure` or `CI_RESULT timeout` before acknowledgement. Ordinary Git and GitHub activity is inert. This delivery-only ingress supersedes [AD121](../decisions/README.md#ad121-a-review-boundary-is-a-delivery-subcommand-not-any-git-invocation) through [AD142](../decisions/README.md#ad142-review-ingress-is-delivery-only-and-completion-is-joint). The explicit user-only bypass exception is described below and specified by [REQ-AGENT-041](../../sdd/spec/agents.md#req-agent-041-pr-boundary-review-bypass-surfaces).
...
The Claude `Stop` hook (`enforce-review-spawn.sh`) only fires in advanced mode when `sdd/` and `sdd/README.md` are present. Its automatic boundary vocabulary is limited to executable `git push` and `gh pr create`; ordinary Git/GitHub activity and `gh pr merge` are inert. Successful clone consent is handled separately by the post-tool reminder. Lane coverage is anchored to the delivery that opened the round, so later read-only activity cannot uncover lanes that already returned ([AD121](../decisions/README.md#ad121-a-review-boundary-is-a-delivery-subcommand-not-any-git-invocation)). <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/lib/boundary-classifier.cjs::boundaryEvent --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::COVERAGE_LINE -->
...
The round is user-visible and blocking. A successful push or PR creation auto-launches it without renewed consent. If that PostToolUse directive is missed, a later delivery command may recover the loop when the exact synchronized head descends from that PR's PR-specific acknowledgement. Successful clone is the only consent boundary; ordinary Git and GitHub activity is inert. The directive requires the root session to print an overview before the lanes run, naming which lanes are about to run, why the others were excluded, and the exact range. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/git-push-review-reminder.sh::DIRECTIVE -->

It issues lane calls, launches the attached CI Agent last, and **ends its turn**, doing nothing else until every lane and exact-head CI have returned. Only then does it publish the fixed triage table with one row per finding across all lanes. Failure or timeout uses a dedicated `Exact-head CI` row whose proposed-fix cell is exactly the matching `CI_RESULT` token. A fully clean successful round publishes the header and divider without synthetic clean-lane or CI rows. That tool-free response ends the turn. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/git-push-review-reminder.sh::DIRECTIVE -->
...
FIX cannot begin before the attached CI Agent returns and joint triage records its terminal result. Failure or timeout is a finding and must be addressed according to the accepted triage row before another push; absent CI evidence keeps acknowledgement closed. Claude's FIX directive still owns delivery once that gate has passed ([AD123](../decisions/README.md#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules), [AD142](../decisions/README.md#ad142-review-ingress-is-delivery-only-and-completion-is-joint)).
