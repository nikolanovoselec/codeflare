...
Pi's broader post-command backstop also runs after successful shell commands that invoke `git` or `gh`, including `env VAR=value git ...` and similar wrappers. That path bypasses the PR cache before reading GitHub PR state, so an unclassified push cannot be hidden behind a stale cached PR head.
