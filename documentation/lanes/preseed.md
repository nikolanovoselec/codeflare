...
For reviewer-bearing PR boundaries, `review-enforcement.ts` reads Goal's session state and requests a synchronous control-channel pause before emitting the review launch plan. Only the matching acknowledged `pr-boundary-fix-follow-up` requests resume, so CI and individual reviewer notifications cannot restart Goal. Duplicate boundaries do not duplicate the pause. Missing control, a replaced or independently resumed Goal, and PR closure remain fail-open; closure releases an owned pause before returning ([REQ-AGENT-111](../../sdd/spec/agents.md#req-agent-111-native-goal-workflow-in-pi-sessions) AC5).
