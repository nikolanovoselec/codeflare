...
A push launch follows only when that branch and full SHA match the authoritative open PR head targeting `main`, `master`, or `develop`. After a merge, Pi instead requires GitHub to report the selected PR merged into `develop` and the open `develop → main/master` PR to report that exact merge commit before reusing the same launch lifecycle. An unavailable source or downstream lookup, or an exact-shape stale downstream PR, gets at most three boundary attempts across live, settled, and reload handling; structural mismatches become inert immediately ([REQ-AGENT-121](../../sdd/spec/agents.md#req-agent-121-pi-downstream-boundary-after-develop-merge)). <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mergedDevelopPromotion --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::retainRetryableMerge --> <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::registerReviewEnforcement -->
