...
The launcher also cancels in-flight runs from superseded heads before starting its own poll, guarded by the single condition that the local checkout is the remote tip, because cancelling the current head's CI is irreversible. Once that holds every in-flight run on the branch with a different head is stale by definition, so no ancestry test applies and amended or force-pushed heads are cancelled rather than left burning.
