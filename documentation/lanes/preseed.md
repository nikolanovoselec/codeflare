...
Pi's PR-boundary extension is the sole automatic dispatcher for review and CI ([AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent)). Unpublished local commits are not review heads and never authorize pre-push reviewers; only the plan emitted after an eligible push, protected-base PR creation, or verified downstream develop merge does. On the normal non-bypassed path, that Git action makes the extension emit a numbered Markdown runbook with PR/head/scope context and the order `REVIEWERS → CI → TRIAGE + ACK → FIX`. Reviewer calls start together; CI starts immediately afterward without waiting. The explicit user-only bypass exception is described below and specified by [REQ-AGENT-041](../../sdd/spec/agents.md#req-agent-041-pr-boundary-review-bypass-surfaces).
...
The Claude `Stop` hook (`enforce-review-spawn.sh`) only fires in advanced mode when `sdd/` and `sdd/README.md` are present. Its transcript-based trigger surface is `git push`, synchronous `gh pr merge`, and protected-base `gh pr edit --base main|master`; `git-push-review-reminder.sh` handles the matching in-turn reminder plus `gh pr create`. PR creation and push boundaries target `main`, `master`, or `develop`; a verified merge into `develop` binds both hooks to the exact downstream `develop → main/master` head, while the legacy retarget surface remains limited to `main` and `master`. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/lib/gh-pr-state.sh::resolve_develop_merge_boundary -->
...
A push launch follows only when that branch and full SHA match the authoritative open PR head targeting `main`, `master`, or `develop`. After a synchronous merge, Pi and Claude require GitHub to report the selected PR merged into `develop` and the open canonical `develop → main/master` PR to report that exact merge commit before reusing their existing review lifecycles ([REQ-AGENT-121](../../sdd/spec/agents.md#req-agent-121-downstream-boundary-after-develop-merge)). Auto-merge, cross-repository, and ambiguous forms remain inert. An unavailable source or downstream lookup, or an exact-shape stale downstream PR, gets at most three boundary attempts across live, settled, and reload handling; structural mismatches become inert immediately ([REQ-AGENT-122](../../sdd/spec/agents.md#req-agent-122-downstream-merge-retry-and-recovery)). <!-- @impl: preseed/agents/pi/extensions/review-enforcement.ts::mergedDevelopPromotion --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/lib/gh-pr-state.sh::resolve_develop_merge_boundary -->

Branch deletion, tags, dry-run/follow-tag, ambiguous or multi-ref pushes, PR edit/update commands, explicit cross-repository merge forms, failed or unconfirmed merges, quoted examples, child sessions, passive startup, detached HEAD, and PRs targeting any other integration branch are inert. A forced single-branch push still requires the same exact branch-and-head match ([REQ-AGENT-063](../../sdd/spec/agents.md#req-agent-063-pr-boundary-command-parsing)).
