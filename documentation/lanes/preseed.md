...
Pi also loads the exact-pinned Caveman extension from the image-owned npm cache. Both public and curated preseeds carry `caveman.json`, while container startup remains authoritative: every start atomically restores full compression mode with the extension's animated status/footer disabled. The package entrypoint is explicitly JITI-warmed and verified, and normal Pi-extension shadow-pin discovery owns future coherent version updates ([REQ-AGENT-155](../../sdd/spec/agents.md#req-agent-155-image-owned-caveman-response-policy)). <!-- @impl: entrypoint.sh::configure_pi_caveman -->
