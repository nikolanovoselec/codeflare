...
Herdr now owns completion timing. The host reads the primary Pi or Claude agent's public semantic status and starts four minutes only after `working` changes to `idle` or `done`. Moving between `idle` and `done` keeps the timer. A return to `working` cancels it and clears any queued completion, so yesterday's timer cannot announce readiness over today's output. Initial idle, blocked, and unknown states do nothing ([REQ-TERM-029](../../sdd/spec/terminal.md#req-term-029-herdr-status-gated-terminal-completion), [REQ-TERM-038](../../sdd/spec/terminal.md#req-term-038-herdr-semantic-status-owns-completion-readiness)). <!-- @impl: host/src/herdr-agent-status.ts::HerdrCompletionDelay --> <!-- @impl: host/src/herdr-agent-status.ts::HerdrAgentStatusMonitor -->
