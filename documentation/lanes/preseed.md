...
For Pi, the acknowledged full SHA remains at `.git/sdd-last-ack-pr-head`. A successful persisted boundary lists missing reviewer lanes and, when an acknowledgement exists, the exact acknowledged-to-current range. Every service-dispatched reviewer prompt and dispatch record carries that range. Agent IDs unknown to a reloaded service stay in flight, live stopped/aborted/error states retry, and only the reminder head can be acknowledged. A delayed successful record can acknowledge that head after reload or newer unpublished local work while the PR still points to it; unfinished or replaced work may repeat at a later boundary ([AD98](../decisions/README.md#ad98-pi-pr-review-uses-visible-session-scoped-agents)).
