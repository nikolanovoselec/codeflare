...
It issues lane calls, launches the attached CI Agent last, and **ends its turn**, doing nothing else until every lane and exact-head CI have returned. Only then does it publish the fixed triage table with one row per finding across all lanes plus any failed or timed-out CI result. A fully clean successful round publishes the header and divider without synthetic clean-lane rows. That tool-free response ends the turn. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/git-push-review-reminder.sh::DIRECTIVE -->
