...
Review ingress now has one durable fact: the current exact PR identity has a user-scoped completion marker, or it does not. Pi resolves only the active checkout at startup, resume, clone, switch, checkout, pull, push, and PR creation. Claude applies the same rule through SessionStart and PostToolUse. A successful checked-out-branch push or PR creation with no marker emits the deterministic review-and-CI launch plan automatically. Other marker misses offer `Mark review complete` and `Launch review`. Fetch, inspection, local mutation, merge, detached or path checkout, and unrelated-ref pushes stay inert ([REQ-AGENT-171](../../sdd/spec/agents.md#req-agent-171-user-scoped-review-completion-and-common-consent)).
