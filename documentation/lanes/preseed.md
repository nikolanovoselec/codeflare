...
For reviewer-bearing PR boundaries, `review-enforcement.ts` records review ownership before requesting a session-local control-channel pause and emitting the review launch plan. If ownership cannot be recorded, review proceeds without pausing the Goal. Apart from PR closure, only the matching acknowledged `pr-boundary-fix-follow-up` requests resume, so CI and individual reviewer notifications cannot restart Goal. Missing control, Goal replacement, and independent reactivation remain fail-open; closure releases an owned pause without blocking review handling ([REQ-AGENT-113](../../sdd/spec/agents.md#req-agent-113-review-owned-goal-release) AC1-AC7).

Settled recovery waits one cycle after the initial plan before reporting launch work as missing, preventing a recovery message from duplicating the plan. Duplicate boundaries do not duplicate the pause, and a replacement PR head transfers ownership of the existing pause instead of stranding it on the superseded head. If that transfer cannot be persisted, the bridge requests rollback; successful rollback clears ownership, while failed rollback retains recoverable ownership for the replacement head's FIX release ([REQ-AGENT-112](../../sdd/spec/agents.md#req-agent-112-goal-pause-ownership-across-pr-heads) AC1-AC7).
