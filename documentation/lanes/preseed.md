...
If this head's terminal CI result has not landed, the FIX turn waits for it and pushes immediately afterwards, so the in-flight run finishes and its outcome rides the same push rather than being discarded. That wait can only delay the push, never cancel it: no CI monitor for this head, or a log that has not advanced since the last read, means the FIX turn pushes now ([AD123](../decisions/README.md#ad123-the-claude-fix-directive-owns-delivery-pi-leaves-it-to-standing-rules)). That push is the delivery boundary that launches one incremental reviewer wave and one CI monitor. The FIX turn never merges, and a round with no accepted fix creates no commit and pushes nothing.
