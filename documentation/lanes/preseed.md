...
Two further terminal lines cover a head the monitor has outlived: `CI_RESULT superseded head=<sha> tip=<sha>` (exit 125) when the branch has advanced past this monitor's head, which is not a verdict and satisfies no gate, and `CI_RESULT unpushed head=<sha> tip=<sha>` (exit 126) when that head is not an ancestor of the tip and so never reached the remote, which does need investigation. The launcher also cancels in-flight runs from superseded heads before starting its own poll, but only when the local checkout is the remote tip and only for runs whose head is an ancestor of it, because cancelling the current head's CI is irreversible.
