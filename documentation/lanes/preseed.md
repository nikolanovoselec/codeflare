...
What a leading token means depends on the manifest grammar. In `go.mod`, `Gemfile`, `Package.swift`, `mix.exs` and a Gradle build it is syntax, so `gem`, `source` and `require` are excluded. In `Cargo.toml`, `pyproject.toml` and a `Pipfile` it is a key, and only under a dependencies heading is that key a package name. Everywhere else every line is one dependency.
