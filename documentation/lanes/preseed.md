...
The Custom-tier column reflects the extra Claude Code delivery surface for users on the `unlimited` subscription tier in Advanced mode. Container startup writes the context-mode-enabled Pi package setting by default, so the foreground session exposes `ctx_*` tools and the bash-curl-redirect hook without `/ctx on`. `/ctx` persists the selected shared Pi setting and reloads the active Pi process; the next Codeflare container start resets the setting to enabled.
