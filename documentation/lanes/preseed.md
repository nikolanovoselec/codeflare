...
Two further terminal lines cover a head the monitor has outlived: `CI_RESULT superseded head=<sha> tip=<sha>` (exit 20) when the branch has advanced past this monitor's head, which is not a verdict and satisfies no gate, and `CI_RESULT unpushed head=<sha> tip=<sha>` (exit 21) when that head is not an ancestor of the tip and so never reached the remote, which does need investigation. Both codes avoid the reserved 125-127 range so a monitor that failed to exec cannot be read as either state. Ancestry is only judged once the tip is a local object; when it is not, the monitor polls again rather than guessing, and a persistent failure ends on the existing timeout line.

The monitor never cancels a workflow run. Superseded runs are cancelled by the workflows themselves, which declare `concurrency` with `cancel-in-progress` keyed on `github.ref`; the ones that omit it, `deploy.yml` among them, omit it because a half-finished run is worse than a redundant one. A client-side cancel driven from a branch name cannot distinguish those cases, so the skill forbids it ([AD122](../decisions/README.md#ad122-superseded-ci-runs-are-cancelled-by-workflow-concurrency-not-by-the-monitor)).
