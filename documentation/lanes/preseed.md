...
Managed `seed-v43` is owned by `codeflare-curation/main`. It selects one web, mobile, desktop, static, or incumbent authority and keeps motion, components, performance, and finishing subordinate. The pinned compiler projects agent-neutral content to supported runtimes; Pi receives one compact routing rule, Copilot receives compact global routing without projected skill directories because it has no skill-directory contract, and Canvas retains required Apache-2.0 attribution. This repository's baked fallback carries identical shared design content. <!-- @impl: scripts/agent-seed-core.mjs::compileAgentSeed -->
