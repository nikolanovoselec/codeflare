...
Pi also loads the exact-pinned Caveman extension from the image-owned npm cache. Default and advanced Codeflare Pi seeds carry `caveman.json`, while container startup remains authoritative: before startup completes, it atomically restores full compression mode with the extension's animated status/footer disabled and fails closed if that policy cannot be written. The package entrypoint is explicitly JITI-warmed and verified, and normal Pi-extension shadow-pin discovery owns future coherent version updates ([REQ-AGENT-155](../../sdd/spec/agents.md#req-agent-155-image-owned-caveman-response-policy)). <!-- @impl: entrypoint.sh::configure_pi_caveman -->
...
Codeflare owns Pi prompt assembly, `SYSTEM.md`, compact shared `AGENTS.md`, executable hard-policy guards, compiler support, and the image fallback. `codeflare-curation` owns its managed source inventory, modes, invocation visibility, managed projection measurement, and signed publication. The public fallback is verified in Codeflare first; completion still requires an exact deployed compiler pin, declared prompt-policy synchronization, and guarded curation evidence for both managed modes. The lossless migration ledger is `scripts/pi-prompt-rule-ledger.json` ([REQ-AGENT-156](../../sdd/spec/agents.md#req-agent-156-bounded-lossless-pi-prompt)).
