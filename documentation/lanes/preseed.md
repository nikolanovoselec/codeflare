...
Pi also loads the exact-pinned Caveman extension from the image-owned npm cache. Its configuration is excluded from Codeflare's default and advanced agent seeds: the image carries the policy file, and before startup completes the entrypoint atomically restores lite compression mode with the extension's animated status/footer disabled. The image applies one version-locked exception so capability and onboarding tutorials, including numbered replies, retain their owning answer contract before Caveman resumes for ordinary responses. Herdr keeps its private XDG state while directing Pi to that canonical `~/.pi/agent` configuration root. A missing, invalid, or unwritable image policy blocks startup. The package entrypoint is explicitly JITI-warmed and verified, and normal Pi-extension shadow-pin discovery owns future coherent version updates ([REQ-AGENT-155](../../sdd/spec/agents.md#req-agent-155-image-owned-caveman-response-policy), [REQ-AGENT-172](../../sdd/spec/agents.md#req-agent-172-herdr-preserves-the-pi-extension-policy), [REQ-AGENT-189](../../sdd/spec/agents.md#req-agent-189-layered-codeflare-capability-discovery)). <!-- @impl: entrypoint.sh::configure_pi_caveman --> <!-- @impl: scripts/patch-pi-caveman-tutorial-exemption.mjs::patchPiCavemanSource --> <!-- @impl: image/herdr/codeflare-herdr-terminal::prepare_runtime -->
