...
The file channel exists because the harness does not persist an assistant message whose tool call the gate rejects, so chat text alone can be invisible to the transcript scan; the directive demands both, keeping the table user-visible while the file carries the machine checkpoint, and triage flows straight into fixes without ending the turn. Because the stamp is a bare transcript line number that a compaction rewrite can re-issue, the file is unlinked at both round-closing sites once its verdict is consumed, so a stale file can never clear a colliding future round.
