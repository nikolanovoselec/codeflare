...
On reload, `capability-helpers.ts` keeps those tools active only when the session's latest canonical Goal state is unfinished, allowing the same Goal to restore without independently widening fresh, completed, or merely already-active lazy sessions ([REQ-AGENT-111](../../sdd/spec/agents.md#req-agent-111-native-goal-workflow-in-pi-sessions) AC4/AC5).
