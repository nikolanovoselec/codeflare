...
Pi loads the exact-pinned Caveman extension from the image-owned npm cache. Its configuration is excluded from default and advanced agent seeds: before startup completes, the entrypoint restores lite compression with animated status/footer disabled. One version-locked exception lets capability and onboarding tutorials, including numbered replies, retain their answer contract before Caveman resumes for ordinary responses. A missing, invalid, or unwritable policy blocks startup ([REQ-AGENT-155](../../sdd/spec/agents.md#req-agent-155-image-owned-caveman-response-policy), [REQ-AGENT-189](../../sdd/spec/agents.md#req-agent-189-layered-codeflare-capability-discovery)). <!-- @impl: entrypoint.sh::configure_pi_caveman --> <!-- @impl: scripts/patch-pi-caveman-tutorial-exemption.mjs::patchPiCavemanSource -->

Herdr retains private XDG state while directing Pi to the canonical `~/.pi/agent` configuration root. The package entrypoint is JITI-warmed and verified, and normal Pi-extension shadow-pin discovery owns future coherent version updates ([REQ-AGENT-172](../../sdd/spec/agents.md#req-agent-172-herdr-preserves-the-pi-extension-policy)). <!-- @impl: image/herdr/codeflare-herdr-terminal::prepare_runtime -->
