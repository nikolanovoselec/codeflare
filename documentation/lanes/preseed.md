...
The same script is additionally registered under `PreToolUse` (empty matcher, all tools) as the mid-turn triage gate ([REQ-AGENT-104](../../sdd/spec/agents.md#req-agent-104-review-acknowledgement-requires-a-published-verdict) AC7). Once every review lane spawned in the transcript has a completed notification and no canonical triage table follows the last of them, every tool except `Read`, `TaskOutput`, `TaskGet`, `TaskList`, and `AskUserQuestion` is refused with the verdict directive until the verdict exists in either channel: the stacked table in assistant text, or the round-stamped triage file `$TMPDIR/sdd-review-triage-<transcript-cksum>.md` — session-keyed like the gate's other state files, resolved path named in the directive — whose first line names the completion line being gated (the one Write the gate permits while blocked).

The file channel exists because the harness does not persist an assistant message whose tool call the gate rejects, so chat text alone can be invisible to the transcript scan; the directive demands both, keeping the table user-visible while the file carries the machine checkpoint, and triage flows straight into fixes without ending the turn. Because the stamp is a bare transcript line number that a compaction rewrite can re-issue, the file is unlinked at every round-closing ack path once its verdict is consumed, so a stale file can never clear a colliding future round.
