...
verbatim, so detector scripts are never mangled by Claude-to-Pi text adaptation. Runtime activation grants no mutable package self-update. A fail-closed updater overlay preserves configured idle grace, evidence-bound audit scoring, platform-aware target guidance, `PRODUCT.md` context, contextual browser-surface advice, and neutral specialist framing across upstream refreshes. Behavioral tests cover executable wait state and updater mutation boundaries without pinning aesthetic prose. The vendored bundle is shadow-pinned by `bump-shadow-pins.yml`, whose reviewed repository update refreshes both copies, updates both manifests, and regenerates the seed. ([REQ-AGENT-137](../../sdd/spec/agents.md#req-agent-137-design-skill-review-boundary), [REQ-AGENT-163](../../sdd/spec/agents.md#req-agent-163-impeccable-browser-question-idle-lifecycle), [REQ-AGENT-181](../../sdd/spec/agents.md#req-agent-181-design-specialist-compatibility))
