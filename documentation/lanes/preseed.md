...
Every blocking Stop directive opens with `[review directive — hook working as designed, not an error]`, because Claude Code renders each one as "Stop hook error: …" and that label cannot be changed from the hook; a parallel `systemMessage` carries the same clarification as a clean non-error notice where the client renders one. The full FIX contract is delivered once per PR and every later round gets a one-line form that keeps the load-bearing clauses, so the unavoidable label sits over one glanceable line instead of a re-sent wall. <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::FIX_SHOWN_FILE --> <!-- @impl: preseed/agents/claude/plugins/codeflare-hooks/scripts/enforce-review-spawn.sh::REVIEW_DIRECTIVE_TAG -->
