...
- **Herdr Pi fullscreen when the keyboard opens:** xterm has no application history to anchor. A trusted tap sends Pi's standard `End` sequence after activation and before focus; Pi resets its own viewport to the live bottom. Taps while the keyboard is already open do not reset it. ([REQ-MOB-002](../../sdd/spec/mobile.md#req-mob-002-virtual-keyboard-opens-reliably-on-tap) AC7-AC8)
...
Herdr instead prevents the `touchend` default, activates and focuses through its trusted `onHerdrTap` path, and consumes one delayed compatibility click. ([REQ-MOB-020](../../sdd/spec/mobile.md#req-mob-020-terminal-touch-activation) AC1-AC2)
