...
### Pi native Chat fails or lacks editor context ([REQ-IDE-006](../../sdd/spec/browser-ide.md#req-ide-006-ide-conversation-context-and-credential-isolation), [REQ-IDE-007](../../sdd/spec/browser-ide.md#req-ide-007-ide-guarded-approval), [REQ-IDE-019](../../sdd/spec/browser-ide.md#req-ide-019-codeflare-eligibility-in-editor-inline-chat), [REQ-IDE-020](../../sdd/spec/browser-ide.md#req-ide-020-unrestricted-pi-editor-request-execution), [REQ-IDE-021](../../sdd/spec/browser-ide.md#req-ide-021-account-free-browser-ide-chrome))
...
**Cause:** `Language model unavailable` means the pinned Code OSS host rejected the request before entering the participant because the hidden `copilot` fallback was missing from absent-request-model resolution. A Copilot-only Models control means the visible `codeflare` model was missing, was not panel/editor default, or did not satisfy the editor tool-calling filter. Other failures can mean the active URI is outside the canonical workspace or uses a symbolic-link alias, editor context exceeds its bound, or the fixed RPC child emitted invalid JSONL.
...
If a normal Pi question fails, inspect the emitted `extension_ui_request`: only bounded `select` and `input` dialogs are supported in addition to the manifest-backed `confirm` contract; malformed, `editor`, and unknown blocking methods intentionally stop generation. The bottom-right provider **Sign In** control is `chat.statusBarEntry`, not the Accounts icon; profile preparation adds only that entry to the existing hidden-entry list and must not patch code-server or disable authentication.
