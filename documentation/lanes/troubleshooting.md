...
| Session stays `running` but goes unresponsive for many minutes with no idle or health updates, then the tab reconnects roughly once a second once it is torn down | The metrics alarm awaited an in-container poll that was accepted and never answered; because the re-arm is the tick's last statement and the schedule is one-shot, the loop ended and the platform killed the handler at its 15-minute wall-time limit | Fixed: those polls are bounded to 10 s so a hang re-arms on the next tick ([REQ-SESSION-020](../../sdd/spec/session-lifecycle.md#req-session-020-the-metrics-alarm-outlives-a-container-that-stops-answering)), and a rejected terminal forward now closes retryably ([REQ-TERM-022](../../sdd/spec/terminal.md#req-term-022-an-unreachable-container-ends-the-upgrade-instead-of-escaping-it)). If seen on an older deploy, stop the session from the dashboard to clear it. |
...
| Claude Code reports a "SessionStart hook error" with "Permission denied" in container logs | A hook in `~/.claude/hooks/` is registered by bare path and spawned through its shebang, but R2 carries object content and modification time, not POSIX modes, so the sync that restored or rewrote it dropped the exec bit | Repaired automatically. entrypoint.sh restores the bit on every run and, since PR #302's boot-only repair could be undone by any later sync round, again after every successful bisync ([REQ-STOR-002](../../sdd/spec/storage.md#req-stor-002-file-persistence-across-sessions) AC4). A session already running when a sync stripped the bit recovers within seconds; no redeploy needed. |
