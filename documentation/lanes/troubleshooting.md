...
Switching from `exclusive` to `immutable`, or from protected mode to `mutable`, takes effect only after reconciliation stamps the weaker policy; deleting the index first leaves session and Storage mutations fail-closed. Filter tampering inside a container can cause repeated bisync `403` failures but cannot persist protected mutations; replace the container or restore the filter rather than weakening Worker enforcement.
