...
The `LlmInterceptor` maps an authorized Dynamic Route handle to `dynamic/<route>` and an authorized opaque native handle to the Worker-held `aws-bedrock/<model>` selector ([REQ-ENTERPRISE-049](../../sdd/spec/enterprise-mode.md#req-enterprise-049-unified-enterprise-model-authorization-and-publication)). Verify that AI Routing can read the gateway catalog, the intended default is assigned, and its exact profile or native target has current verification. Dynamic Routes retain REST-first fallback behavior; native Bedrock targets use one direct compat request. (Claude Code is not in the enterprise agent set; only Copilot, Pi, and Bash run under `ENTERPRISE_MODE=active`.)
