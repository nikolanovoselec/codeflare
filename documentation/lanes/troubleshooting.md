...
### User-installed Browser IDE extensions do not return or uninstall ([REQ-IDE-036](../../sdd/spec/browser-ide.md#req-ide-036-persistent-user-managed-extensions))
...
**Fix:** Inspect only metadata, never setting values: the manifest must be a mode-0600 regular file with version 1, lowercase IDs, at most 50 entries, and at most 32 KiB of bounded settings. Confirm code-server uses `/tmp/openvscode-data/extensions`, fixed entries there are symlinks into `/opt/codeflare/openvscode/extensions/<kind>`, and user entries are real directories. Check `extensions.json` and bounded `.obsolete` identities; absence without a matching obsolete key must not remove intent. Verify `extensions.allowed` retains both `"*": true` and the explicit Codeflare entry, the welcome builtin activates on `onStartupFinished`, and the existing `/tmp/sync-daemon.pid` receives `SIGUSR1` only after a changed atomic write. Do not copy VSIX files, extension directories, `User` data, SecretStorage, or Accounts into R2. A structured exact-version not-found receives one unpinned fallback; every other failure waits for a later fresh activation without a retry loop. <!-- @impl: openvscode/agent-sidebar/src/extension-persistence.ts::activateExtensionPersistence --> <!-- @impl: scripts/browser-ide-extensions.py::capture --> <!-- @impl: entrypoint.sh::_openvscode_capture_extensions -->
