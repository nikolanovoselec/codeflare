...
If the dashboard continuously returns to `Upgrading` and Workers Observability shows automatic requests using `POST /api/storage/seed/agent-configs`, the deployment still has the old full-overwrite path. Deploy the version with `POST /api/storage/seed/agent-configs/upgrade`; do not remove release files, change modes, rotate the signer, or republish the same release. The corrected automatic route compares the exact applied and target cached bundles, resumes target-marked writes, mutates added or release-changed paths, and repairs release-identical paths only when they retain an interrupted-target marker. Manual Recreate still uses `/api/storage/seed/agent-configs` and rewrites every desired object. A successful final applied stamp ends `Upgrading` on the next status poll. <!-- @impl: src/lib/remote-curation.ts::verifyManagedReleaseStream --> <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest -->
