...
Fixed: a green (`pw === 'ready'`) button now opens DIRECTLY via the bootstrap-hop with no per-open readiness/key re-verify (`handleVaultOpen`) — a green button is ready by definition. The reload-skip also now arms green OPTIMISTICALLY on return (marker present + live local readiness), so a returning session re-greens with no click and no re-init, and revokes only if the polled-in start proves a genuine resume ([AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)). RESIDUAL: only when the `cf_vault_sid` routing cookie was cleared (the readiness marker is then unreadable) does a returning session need one on-demand click; the marker is keyed by session id, not bucket, and the durable bucket-key fix is deferred ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) Constraints, [AD84](../decisions/README.md#ad84-retain-the-vault-sw-encryption-key-in-memory-neuter-the-proactive-flush-and-open-a-green-vault-button-directly)).
