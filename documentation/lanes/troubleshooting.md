...
**Check the outcome before changing configuration:** Map Profile lists every compatible existing revision; choose one to Assign, then Save. Several passing profiles, even with different mappings, are not an error. Their names do not identify the gateway-selected model. The Kimi preset supports non-off levels and can fit Kimi-compatible behavior without creating a new profile. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

`completion_limit` means an off-mode reasoning observation, tool call, or replay exhausted the selected ceiling. That affected mode is incomplete, not proven incompatible. Check the overall `outcome`: other passed modes can still yield a usable existing match or custom draft. Increase the visible ceiling (up to 16,384) and explicitly run again only if you want to verify the incomplete modes and accept the cost. The default is 4,096; Codeflare never raises it automatically. Technical details distinguish rejected requests, missing tools, rejected replay, incomplete final completion, and off that still reasons. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))
