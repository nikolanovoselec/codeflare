...
For immutable-resource update pending, confirm Enterprise mode, Strict Gateway Egress, the `EGRESS` binding, the applied `resourcePolicy` and `managedPathsDigest`, and the exact `.codeflare/managed-paths.json` object. Do not hand-edit its bytes or applied stamp. Policy changes roll out per user without draining the deployment: existing sessions retain their applied policy, while new starts and Storage mutations wait until that user's sessions stop and managed reconciliation completes ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)).

Switching from `exclusive` to `immutable`, or from protected mode to `mutable`, takes effect only after reconciliation stamps the weaker policy; deleting the index first leaves session and Storage mutations fail-closed. Filter tampering inside a container can cause repeated bisync `403` failures but cannot persist protected mutations; replace the container or restore the filter rather than weakening Worker enforcement ([REQ-STOR-029](../../sdd/spec/storage.md#req-stor-029-managed-resource-reconciliation), [REQ-STOR-031](../../sdd/spec/storage.md#req-stor-031-managed-resource-container-sync), [REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement), [REQ-ENTERPRISE-030](../../sdd/spec/enterprise-mode.md#req-enterprise-030-managed-resource-storage-enforcement)).
