...
| Pi review lanes completed but no monitor result appears ([REQ-AGENT-062](../../sdd/spec/agents.md#req-agent-062-pi-pr-boundary-review-result-delivery)) | The background `review-monitor` did not start, is still waiting for lane files / `summary.md`, or an old preseed still uses the removed nonce announcement path. | Check `.git/codeflare-review-jobs/<head>/monitor.json` and `monitor.completed`. If no monitor was started, update the preseed so `review-monitor.md`, `review-enforcement.ts::startReviewMonitor`, and `review-job-helpers.ts::reviewMonitorSpawnDecision` are present, then reload Pi. `/review-results` remains the manual fallback for the saved summary. |
