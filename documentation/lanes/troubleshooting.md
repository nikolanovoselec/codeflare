...
| Pi CI resolver returns no request ([REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring), [AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent)) | Repository cwd or review launch state was omitted, the Git action was unchanged/unsupported, or no open PR targets `main`/`master`/`develop`. | Run from the affected repository after required reviewer calls with explicit `cwd` and `reviewState`; otherwise wait for a later eligible event or explicit request. |
