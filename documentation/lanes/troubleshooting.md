...
| Session remains `stopping` after a stop request | Status confirmation timed out or repeatedly failed. A refresh retains recent `stopping` state, but after three minutes it resets stale bookkeeping to `stopped` without process-exit evidence ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-records-reset-on-owner-status-read)). | Refresh once. If it remains `stopping`, retry Stop. If it resets to `stopped`, verify the workload separately before relying on teardown. |
