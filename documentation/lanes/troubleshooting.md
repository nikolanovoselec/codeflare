...
Note: the monitor design has no `autofix.requested` marker and no nonce announcement to inspect; the main session reacts to the background agent's `REVIEW_RESULT` handoff. `review-monitor` is an agent/subagent, while `preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor` starts it from a live main-session context, owns the durable claim state, and sends a fallback message if startup fails.
