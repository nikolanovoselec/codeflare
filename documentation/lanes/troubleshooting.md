...
Start with the response request ID and correlate it with Worker authentication logs. Then verify the active OAuth callback configuration and the deployed session-signing secret before retrying; these three checks distinguish request handling failures, provider mismatch, and invalid session issuance without rotating credentials blindly.
...
| WebSocket fails with close code 1013 (`container-warming-up`) | Container started but not yet ready - port 8080 bound before R2 sync and `.bashrc` autostart completed | See [note](#websocket-fails-with-close-code-1013-container-warming-up). |
...
| `graphify: command not found` in terminal ([REQ-AGENT-023](../../sdd/spec/agents.md#req-agent-023-knowledge-graph-capability-graphify)) | System-PATH symlink absent in the image, or entrypoint self-heal did not run | See [note](#graphify-command-not-found-in-terminal-req-agent-023). |
...
| Enterprise Mode: agent fails with opaque "Connection error" but `curl https://api.openai.com` from the same container succeeds | See note below. | See [note](#enterprise-mode-agent-fails-with-opaque-connection-error-but-curl-httpsapiopenaicom-from-the-same-container-succeeds). |
...
| Pi `review-monitor` or CI-monitor task's final visible output is a vault-extraction result instead of `REVIEW_RESULT`/`CI_RESULT` ([REQ-MEM-015](../../sdd/spec/memory.md#req-mem-015-pi-memory-capture-transcript-source-and-child-session-guard) AC2, [REQ-VAULT-003](../../sdd/spec/vault.md#req-vault-003-user-curated-edits-are-detected-and-ingested-within-60s) AC7) | See note below. | See [note](#pi-review-monitor-or-ci-monitor-task-s-final-visible-output-is-a-vault). |
| Pi session memory grows over a long, subagent-heavy session; `ps aux` shows many long-lived `bun`/`node` `server.bundle.mjs` (context-mode bridge) processes accumulating (one session measured ~19 helpers / ~1.3GB) | An older build forced `CONTEXT_MODE_BRIDGE_IDLE_MS=0` container-wide (`entrypoint.sh` + `context-mode-runtime.ts`) to silence a TUI notice, which disabled the bridge idle-reaper entirely — subagents inherit the override, so their bridge children never self-released ([REQ-AGENT-076](../../sdd/spec/agents.md#req-agent-076-pi-context-mode-enablement-and-tool-extension-defaults) AC6) | Update the preseed (re-run `entrypoint.sh` or trigger an R2 sync). Current builds no longer force the override — context-mode's own foreground/subagent split (upstream `#868`) keeps the interactive bridge quiet while non-foreground helpers self-release after ~3 min. |
| Pi background CI-monitor task fails immediately after a push with "Agent is already processing a prompt" ([REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-ci-monitoring-background-agent-policy)) | Older preseed started a per-push CI child, then the PR-boundary monitor handoff was injected into that child and killed it. | Update the preseed. CI monitoring now triggers only from the visible monitor handoff or explicit user request; confirm skill version `1.2.0`. |
...
| Vault readiness button never goes ready and the SilverBullet service worker never registers (browser console shows a `SyntaxError` from `service_worker.js`, e.g. `Identifier 'o' has already been declared`) | Duplicate `o=` declarator in the served worker's sync-cycle declaration list, so the browser refuses to register the SW. | See [note](#vault-readiness-button-never-goes-ready-and-the-silverbullet-service-worker-never-registers-browser-console-shows-a-syntaxerror-from-service_workerjs-eg-identifier-o-has-already-been-declared). |
| Vault re-indexes from scratch on every new session open (SilverBullet shows "Syncing…" for several minutes and re-downloads all notes each session) | The bucket-stable IndexedDB persistence from [REQ-VAULT-021](../../sdd/spec/vault.md#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) has broken: SilverBullet is being served under a different `location.href` per session, so its `sb_data_*`/`sb_files_*` IDB names differ and the prior store is unreachable | See [note](#vault-re-indexes-from-scratch-on-every-new-session-open-silverbullet-shows-syncing-for-several-minutes-and-re-downloads-all-notes-each-session). |
...
| Enterprise Governed Mode: after flipping the R2 SSE-C toggle, R2 sync fails and the vault never becomes ready (some objects 400 on read) | The bucket is **mixed** — objects exist in both regimes after an opposite-regime write or partial migration. | See [note](#enterprise-governed-mode-after-flipping-the-r2-sse-c-toggle-r2-sync-fails-and-the-vault-never-becomes-ready-some-objects-400-on-read). |
| Enterprise Governed Mode: the New Session button is stuck on "Migrating" / a flip never starts | A flip waits for the bucket's running sessions to stop (D1: no force-kill); `batch-status` reports `bucketMigrationPending` while a container is healthy ([REQ-ENTERPRISE-020](../../sdd/spec/enterprise-mode.md#req-enterprise-020-governed-mode-re-encrypt-migration-engine) AC3, [REQ-ENTERPRISE-021](../../sdd/spec/enterprise-mode.md#req-enterprise-021-governed-mode-migration-safety-and-access-boundary) AC3) | Stop the bucket's sessions (close them in the dashboard); the next `batch-status` poll flips the bucket to `migrating` and advances a chunk per poll. Keep the dashboard open until it completes. A stuck `migrating` state self-recovers once the per-chunk lease (`leaseExpiresAt`) elapses and the next poll takes over. |
| Enterprise Mode: Strict Gateway Egress ON and **direct-internet** container HTTP/HTTPS calls (and GitHub) fail with 503 `EGRESS_UNAVAILABLE`, while R2 sync / LLM / browser-run keep working | See [note](#enterprise-mode-strict-gateway-egress-on-and-direct-internet-container-httphttps-calls-and-github-fail-with-503-egress_unavailable-while-r2-sync--llm--browser-run-keep-working). | Expected when the toggle was turned ON on a deploy that has no `EGRESS` binding. Either flip the toggle OFF (KV, no redeploy) to restore direct egress, or redeploy to an enterprise environment so the binding is injected (see [deployment.md](deployment.md#strict-gateway-egress-enterprise-mode)). |
...
<a id="pi-review-lanes-completed-but-no-monitor-result-appears"></a>
#### Pi review lanes completed but no monitor result appears ([REQ-AGENT-062](../../sdd/spec/agents.md#req-agent-062-pi-pr-boundary-review-result-delivery))

**Fix detail:**

Check `.git/codeflare-review-jobs/<head>/monitor.json`, `monitor.completed`, and `.git/codeflare-review-events.jsonl` for `visible_monitor_handoff_sent` or `review_monitor_fallback_message_sent`. Per [REQ-AGENT-077](../../sdd/spec/agents.md#req-agent-077-pi-pr-boundary-review-fallback-and-claim-recovery) AC2, a stopped/errored monitor self-heals: the reaper reclaims the dead claim via subagents-service liveness and re-spawns it next tick. Restart `review-monitor` manually only if liveness reads `unknown` (post-reload) or the handoff never fired; `/review-results` stays the fallback.

<a id="pi-review-monitor-or-ci-monitor-task-s-final-visible-output-is-a-vault"></a>
#### Pi `review-monitor` or CI-monitor task's final visible output is a vault-extraction result instead of `REVIEW_RESULT`/`CI_RESULT` ([REQ-MEM-015](../../sdd/spec/memory.md#req-mem-015-pi-memory-capture-transcript-source-and-child-session-guard) AC2, [REQ-VAULT-003](../../sdd/spec/vault.md#req-vault-003-user-curated-edits-are-detected-and-ingested-within-60s) AC7)

**Cause detail:**

`@gotgenes/pi-subagents` children (review-monitor, CI monitors, memory-capture, vault-extract) are created via `sessionManager.newSession({ parentSession })` and always load the parent's extensions, so `memory-vault.ts`'s capture/extract triggers ran inside those child sessions too; when the native `service.spawn` path was unavailable, its `pi.sendUserMessage("Agent(...)")` fallback injected a new user message into the CHILD transcript, overwriting the monitor's visible result.

**Fix detail:**

Update the preseed (re-run `entrypoint.sh` or trigger an R2 sync). Current builds guard all three `memory-vault.ts` lifecycle handlers (`session_start`, `before_agent_start`, `agent_end`) with `isChildSession(ctx)`, detected via the session header's `parentSession` pointer (live `getHeader()`, falling back to the persisted session-file first line). Confirm with `grep -n isChildSession ~/.pi/agent/extensions/memory-vault.ts`.
