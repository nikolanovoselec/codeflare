...
If an edit opens another tab or the Inline widget lacks Keep/Close, open **View: Toggle Output**, select **Codeflare Inline Chat**, reproduce once, and copy the activation, request, stream, `tabsChanged`, and snapshot lines. The revision and settings distinguish stale rollout; request admission distinguishes the wrong surface; matching paths with different schemes or authorities identify edit/session URI divergence. The channel is bounded and contains URI/tab metadata but no document content. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::createInlineDiagnostics -->
