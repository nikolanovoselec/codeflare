...
| A Pi skill requests a missing `ctx_*` tool after `/ctx off` | Stale seeded skill text treated context-mode as mandatory instead of an optional transport. | Reload the corrected seed. Review/SDD fall back to bounded `read`/`grep`/`bash`; web retrieval falls back to `fetch_content`/`get_search_content`. The work set must not narrow. |
