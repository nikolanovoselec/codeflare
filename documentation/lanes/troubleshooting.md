...
**Cause:** Two stacked failures. Codeflare's original write-side distance restoration and scroll-event reset correction overrode xterm's native full-buffer anchor, then reacted to the programmatic scroll events they generated, causing repeated edge snaps. Removing those corrections exposed the underlying problem: at the 1000-line cap, every output line trims the top of the buffer, so xterm's content anchor drags any scrolled-up reader to `viewportY = 0` within seconds and then destroys the content beneath them ([REQ-TERM-014](../../sdd/spec/terminal.md#req-term-014-terminal-scroll-anchoring-under-scrollback-trimming) AC2/AC3).

**Fix:** Batched writes are deferred while the user owns the viewport in the normal buffer — the display freezes under the reader, so no trimming can move them — and the held output is released in one write when the viewport returns to the live bottom (or a 2M-character cap forces a flush). Writes never alter viewport position. Correlated user intent (refreshed on every `touchmove` during a drag) establishes manual ownership until bottom return; touch-keyboard mode remains the explicit bottom-anchored exception. ([AD105](../decisions/README.md#ad105-streamed-output-defers-while-the-user-reads-scrollback-keyboard-open-swipes-are-always-terminal-input))

**Verify:** Scroll up during dense output: the reading position must not move at all, and swiping back to the bottom must reveal the held output at once. CI's `terminal.test.ts` drives the WebSocket batching path and verifies output defers for an owned normal-buffer viewport (full, partial, and at-top), flushes without correction for a bottom follower or an alternate-buffer application, and force-flushes past the held-output cap without injecting scroll calls.
...
| Pi reports that a merged PR head was never acknowledged ([REQ-AGENT-058](../../sdd/spec/agents.md#req-agent-058-supported-boundary-recovery) AC7) | `gh pr merge` completed before the required reviewer notifications could acknowledge that head; Pi has no pre-command merge interceptor. | Treat the notice as visibility only. Review the merged head manually if needed; Pi deliberately does not write a false acknowledgement. |
| Pi CI resolver returns no request ([REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring), [AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent)) | Repository cwd or review launch state was omitted, the Git action was unchanged/unsupported, or no open PR targets `main`/`master`. | Run from the affected repository after required reviewer calls with explicit `cwd` and `reviewState`; otherwise wait for a later eligible event or explicit request. |
