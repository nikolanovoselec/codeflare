...
**Fix:** For stale state, leave the dashboard open through its next background refresh; the Worker uses ETag validation and selects the newest signed release in its bounded history scan whose runtime dependency hash matches the deployed build. It may deliberately skip the latest release when that release targets another build. For degraded state, confirm the scoped PAT reads the private repository, the configured public key matches the signer, R2 credentials remain valid, and managed releases contain exactly `seed-v1.json.gz` and `seed-v1.sig` with GitHub SHA-256 digests. Already-applied buckets continue from their last verified compatible release.

For update pending, stop every session for that user; reconciliation starts only after the bucket is idle. Do not bypass the typed 409 or seed baked content while curation is enabled. For a missing private change, confirm the protected workflow published an immutable non-draft release whose runtime hash matches the deployed image and that Setup points to the same repository and signer. No compatible release in the bounded scan is a discovery failure. Discovery uses dashboard refresh, not image deployment or container restart.
