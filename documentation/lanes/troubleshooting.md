...
| Enterprise Mode: Strict Gateway Egress ON and **direct-internet** container HTTP/HTTPS calls (and GitHub) fail with 503 `EGRESS_UNAVAILABLE`, while R2 sync / LLM / browser-run keep working | See [note](#enterprise-mode-strict-gateway-egress-on-and-direct-internet-container). | Expected when the toggle was turned ON on a deploy that has no `EGRESS` binding. Redeploy to an enterprise environment so the binding is injected, or restore direct egress by turning protected managed-resource policy mutable in the same Setup update that turns strict egress OFF (or beforehand). Setup rejects OFF while protected policy remains active ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)); see [deployment.md](deployment.md#strict-gateway-egress-enterprise-mode). |
