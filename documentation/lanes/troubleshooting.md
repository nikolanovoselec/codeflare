...
If no request fails, start one new attempt and select the hidden Vault iframe's execution context while the button breathes. Confirm `window.sbRuntime?.ready`, `window.client?.fullSyncCompleted`, `window.client?.systemReady`, `window.client?.pageListLoaded`, and `window.client?.clientSystem?.scriptsLoaded` are true; confirm `window.client?.objectIndex?.hasFullIndexCompleted` exists and returns true; and confirm `await window.client?.mq?.getQueueStats('indexQueue')` reports zero queued, processing, and dead-letter items. Correct the observed condition, then click Vault once to retry. Verify that the button turns green only after two complete checks and opens on the following click ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC3/AC6, [REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC4).
