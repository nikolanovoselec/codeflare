...
**Cause:** Historically, an unguarded failing command in the `ENTERPRISE_MODE=active` block of `entrypoint.sh` aborted the script under `set -euo pipefail` and killed PID 1. The first instance used `jq --arg def …`/`$def`; `def` is a reserved jq keyword and a compile error on the bookworm image's jq 1.6 (see the [reserved-keyword regression test](../../host/__tests__/entrypoint-enterprise-pi-models.test.js)). Current provider-building substitutions set `PI_GATEWAY_CONFIG_OK=0` on failure, warn, and leave Pi unpinned instead of crashing the container.
