...
| Session stays `running` but terminal, `/activity`, and `/health` all stop responding while the PTY was last reported alive | The container process still exists but the Durable Object's private connection to container services is stale; `ctx.container.running` and the SDK process monitor do not prove port reachability | Self-heals under [REQ-SESSION-020](../../sdd/spec/session-lifecycle.md#req-session-020-the-metrics-alarm-outlives-a-container-that-stops-answering): polls are bounded to 10 s, complete dual-probe failures retry after 5 s, and the third resets the Durable Object with `ctx.abort()` without stopping the container or changing KV status. The reconstructed DO reattaches to the running container and existing PTY. Search Workers logs for `resetting Durable Object to reconstruct container transport`. |
