...
**Cause:** Historically, an unguarded failing command in the `ENTERPRISE_MODE=active` block of `entrypoint.sh` aborted the script under `set -euo pipefail` and killed PID 1. The first instance used `jq --arg def …`/`$def`; `def` is a reserved jq keyword and a compile error on the bookworm image's jq 1.6 (see the [reserved-keyword regression test](../../host/__tests__/entrypoint-enterprise-pi-models.test.js)). Current provider-building substitutions set `PI_GATEWAY_CONFIG_OK=0` on failure and warn instead of terminating the container.
...
**Symptom:** While Pi regular-mode output is streaming — even with the viewport parked at the live bottom — the terminal intermittently jumps to older content and visibly scrolls through the transcript back to the prompt within a split second.

**Cause:** Pi regular mode authors every redraw as a DEC 2026 synchronized frame, and a full replay (`CSI 2J`/`CSI H`/`CSI 3J` + the whole transcript) spans hundreds of kilobytes across ~100 WebSocket messages. The host forwards raw PTY chunks and the frontend fed them to `terminal.write()` incrementally, so xterm's 1,000 ms synchronized-output safety timeout (armed at the first buffered row) measured network arrival plus batching. When it expired mid-frame, xterm abandoned atomicity and painted the partially rebuilt transcript, then walked through it as later chunks parsed. Independently, the scrollback wipe (`CSI 3J`) leaves xterm's internal user-scroll lock stale (upstream xterm.js#6046), which can pin the regrowing buffer at the top until an anchor clears it.

**Fix:** The write path reassembles synchronized frames at ingest (`web-ui/src/lib/terminal-frames.ts`): everything from `ESC[?2026h` to the first `ESC[?2026l` reaches xterm as exactly one write, which parses in one synchronous task — the timeout can never fire mid-frame. Fresh Herdr Pi additionally starts fullscreen, where Pi owns transcript history and viewport anchoring. Ordinary output keeps the 33 ms batching; malformed frames fail open within fixed bounds; the read-hold releases whole frames; and the zero-delta bottom anchor clears the stale scroll lock and re-commands the DOM position. ([REQ-TERM-021](../../sdd/spec/terminal.md#req-term-021-synchronized-output-frame-atomicity), [AD111](../decisions/README.md#ad111-synchronized-output-frames-are-delivered-atomically-at-the-write-boundary))
...
(4) A returning user's first session after a deploy could restore an extension tree that predates `native-notifications.ts`, because the relay only overwrote files already present and the Worker-side R2 seed of missing keys races the boot sync. (5) The authenticated notification config route can be unavailable, or the browser can hold no current Push subscription or a subscription that was never re-registered after server cleanup or VAPID rotation. (6) Claude and Pi emit immediate input-needed signals, but completion comes only from Herdr after every recognized Pi or Claude pane stays `idle` or `done` for ten minutes after work. Classic deliberately has no completion producer ([REQ-TERM-029](../../sdd/spec/terminal.md#req-term-029-herdr-status-gated-terminal-completion), [REQ-TERM-038](../../sdd/spec/terminal.md#req-term-038-herdr-semantic-status-owns-completion-readiness), [REQ-TERM-039](../../sdd/spec/terminal.md#req-term-039-herdr-completion-delivery-is-readiness-oriented)).

(7) Push display is best effort: OS focus modes, permission revocation, offline duration, battery policy, or a killed PWA can prevent presentation.

**Fix:** An absent control means Web Push is not configured ([REQ-TERM-025](../../sdd/spec/terminal.md#req-term-025-per-device-notification-enrollment)). Add the complete optional VAPID sender group and redeploy if Push delivery is wanted ([REQ-OPS-013](../../sdd/spec/operations.md#req-ops-013-deploy-command-and-post-deploy-hooks)). On iOS, add Codeflare to the Home Screen (Share → Add to Home Screen), open it from there, and enable notifications in Settings — the hint marks this state.

For configured deployments, confirm `/api/notifications/config` returns the public key to the authenticated user and enabling sends the current subscription to `/api/notifications/subscription`; switch notifications off and on to replace stale local capability state. The web UI registers the OSC 777 handler unconditionally and resolves the session at event time; the enable/display paths wait for the activated worker; the entrypoint relay backfills missing managed extensions from the mode-filtered bake ([REQ-STOR-017](../../sdd/spec/storage.md#req-stor-017-faster-startup-sync--bisync-head-storm-fix--governed-mode-preseed-bake) AC4). Start a fresh session after deploying so the container runs the current image.

**Verify:** First keep the document visible, window focused, terminal view visible, originating session active, and terminal-one pane focused; the event must be suppressed. Then make at least one of those five predicates false on every connected client. Trigger a reviewed input-needed event, or let every recognized Herdr agent pane remain `idle` or `done` for ten minutes after work; exactly one local display or the bounded Push fallback is expected. Confirm permission is granted, a valid local subscription exists, and the authenticated config/subscription requests succeed. `useTerminal.test.ts` covers five-factor disposition and late suppression, `agent-notifications.test.ts` covers activated-worker and subscription repair, `herdr-agent-status.test.js` covers semantic-status timing and reconnects, and `entrypoint-governed-sync.test.js` covers bake backfill.
...
During a pending managed update, ordinary-file uploads and deletions outside managed namespaces remain available. Requests that may affect managed resources, including ancestor-prefix deletions and mixed selections, remain blocked until reconciliation completes. Browsing and downloads continue subject to administrator restrictions; encryption-migration locks still apply to mutations. Retry blocked requests after reconciliation completes. <!-- @impl: src/lib/managed-storage-guard.ts::guardManagedStorageMutation -->

If the dashboard continuously returns to `Upgrading` and Workers Observability shows automatic requests using `POST /api/storage/seed/agent-configs`, the deployment still has the old full-overwrite path. Deploy the version with `POST /api/storage/seed/agent-configs/upgrade`; do not remove release files, change modes, rotate the signer, or republish the same release. <!-- @impl: src/routes/storage/seed.ts::default -->

The corrected route compares the target and, when available, applied cached bundles. Missing valid applied history uses marker-resumable full-target planning. Otherwise, it mutates added or release-changed paths and repairs release-identical paths only when they retain an interrupted-target marker. Manual Recreate still rewrites every desired object through `/api/storage/seed/agent-configs`. A successful final applied stamp ends `Upgrading` on the next status poll. <!-- @impl: src/lib/remote-curation.ts::verifyManagedReleaseStream --> <!-- @impl: src/lib/r2-seed.ts::reconcileAgentConfigs --> <!-- @impl: src/routes/storage/seed.ts::reconcileAgentConfigsForRequest -->

**Immutable-resource requirements:** [REQ-SETUP-015](../../sdd/spec/setup.md#req-setup-015-managed-resource-persistence-controls), [REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety), [REQ-STOR-029](../../sdd/spec/storage.md#req-stor-029-managed-resource-reconciliation), [REQ-STOR-030](../../sdd/spec/storage.md#req-stor-030-managed-resource-policy-loading), [REQ-STOR-031](../../sdd/spec/storage.md#req-stor-031-managed-resource-container-sync), [REQ-STOR-032](../../sdd/spec/storage.md#req-stor-032-exclusive-managed-resource-boundaries), [REQ-ENTERPRISE-027](../../sdd/spec/enterprise-mode.md#req-enterprise-027-managed-resource-admission-and-transport), [REQ-ENTERPRISE-028](../../sdd/spec/enterprise-mode.md#req-enterprise-028-managed-resource-request-classification), [REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement), [REQ-ENTERPRISE-030](../../sdd/spec/enterprise-mode.md#req-enterprise-030-managed-resource-storage-enforcement)

For immutable-resource update pending, confirm Enterprise mode, Strict Gateway Egress, the `EGRESS` binding, the applied `resourcePolicy` and `managedPathsDigest`, and the exact `.codeflare/managed-paths.json` object. Do not hand-edit its bytes or applied stamp. Policy changes roll out per user without draining the deployment: existing sessions retain their applied policy, while new starts and potentially managed Storage mutations wait until that user's sessions stop and managed reconciliation completes ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)).

Switching from `exclusive` to `immutable`, or from protected mode to `mutable`, takes effect only after reconciliation stamps the weaker policy; deleting the index first leaves session starts and potentially managed Storage mutations fail-closed. Filter tampering inside a container can cause repeated bisync `403` failures but cannot persist protected mutations; replace the container or restore the filter rather than weakening Worker enforcement ([REQ-STOR-029](../../sdd/spec/storage.md#req-stor-029-managed-resource-reconciliation), [REQ-STOR-031](../../sdd/spec/storage.md#req-stor-031-managed-resource-container-sync), [REQ-ENTERPRISE-029](../../sdd/spec/enterprise-mode.md#req-enterprise-029-managed-resource-egress-enforcement), [REQ-ENTERPRISE-030](../../sdd/spec/enterprise-mode.md#req-enterprise-030-managed-resource-storage-enforcement)).
...
| WebSocket fails with close code 4503 (`container-stopped`) | Owner-scoped D1 session lifecycle is `stopping` or `stopped` | A confirmed session stop does not count against the WS rate-limit budget (see [WebSocket Rate Limit](security.md#websocket-rate-limit-req-sec-007)). A D1-owning session returns retryable 1013 if no-start health verification fails; a verified survivor may forward despite stale SDK state. |
| WebSocket fails with close code 1013 (`container-warming-up` or `container-unreachable`) | Readiness is unverified, or a forward to the existing container failed after a successful health probe | See [note](#websocket-fails-with-close-code-1013) for separate diagnostics. |
| Session stays `running` but terminal, `/activity`, and `/health` all stop responding, or the SDK monitor reports `Network connection lost` | Both routes share port 8080 and one Node event loop. Failure may be the DO attachment, container network, listener, CPU starvation, host wedge, or a vanished container; `running`, `ptyAlive`, and a WebSocket `101` alone do not prove a functioning workload. | [REQ-SESSION-021](../../sdd/spec/session-lifecycle.md#req-session-021-unreachable-container-transport-initiates-coordinator-reconstruction) and [REQ-SESSION-022](../../sdd/spec/session-lifecycle.md#req-session-022-transport-recovery-is-confirmed-and-bounded) permit at most two coordinator resets. Monitor loss enters recovery immediately; failed probes require three confirmations. Correlate `recoveryAttemptId` across recovery logs. Exhaustion preserves D1 workload ownership without billing or an inferred stop; probe the existing runtime through no-start. A resolved low-level exit monitor or awaited destroy is required to confirm termination. |
...
| Session remains `stopping` after a stop request | Exit confirmation is unavailable; an owner read or elapsed time cannot prove termination ([REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-retains-ownership-until-confirmed-exit)). | Retry Stop, which awaits destruction. Do not start a replacement or run managed mutation while the process may survive; if destruction cannot be confirmed, keep the session owning and investigate rather than assuming `stopped`. |
...
| Pi emits no review request after expected PR-boundary work ([REQ-AGENT-036](../../sdd/spec/agents.md#req-agent-036-pr-boundary-review-trigger-conditions), [REQ-AGENT-171](../../sdd/spec/agents.md#req-agent-171-user-scoped-review-completion-and-common-consent), [AD98](../decisions/README.md#ad98-pi-pr-review-uses-visible-session-scoped-agents)) | The command failed or is unsupported, the session is a child, SDD is absent or transitioning, the fresh PR is not open against `main`/`master`/`develop`, or GitHub has not yet reported the exact local head. | Lookup-failed activity launches nothing. After GitHub reports the exact head, use a fresh supported exposure: delivery launches automatically, while another eligible exposure offers mark-or-launch consent. Do not recreate the push. |
| Claude suppresses a fresh exact-head review or its Stop hook cannot find the live round | The ingress and Stop hooks resolved different or unwritable review-session directories. | Confirm both hooks resolve `${CODEFLARE_REVIEW_SESSION_DIR:-/run/codeflare/review-session}` to the same writable directory. The default is `/run/codeflare/review-session`; override it only for an isolated runtime or test. |
...
| Enterprise Mode: Strict Gateway Egress ON and **direct-internet** container HTTP/HTTPS calls (and GitHub) fail with 503 `EGRESS_UNAVAILABLE`, while R2 sync / LLM / browser-run keep working | See [note](#enterprise-mode-strict-gateway-egress-on-and-direct-internet-container). | Expected when `EGRESS` binding is absent ([REQ-ENTERPRISE-016](../../sdd/spec/enterprise-mode.md#req-enterprise-016-strict-gateway-egress)). Redeploy to Enterprise, or follow the linked note to disable strict egress safely ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)). |
...
#### WebSocket fails with close code 1013 (`container-warming-up` or `container-unreachable`)

**Fix detail:**

`container-warming-up` is normal during the ~10s cold-start window. Failed no-start health verification or terminal-not-ready returns retryable 1013 before rate limiting; a port-verified survivor can forward despite stale or unavailable SDK state under normal WS rate limits.

If 1013 persists beyond 30 seconds, check the close reason and Worker logs:

1. For `container-warming-up`, check `/run/codeflare/sync/sync.log` for a stalled R2 sync (the same causes as "Loading screen hangs indefinitely" above). If sync is healthy, locate the last startup log before the missing flag. Guarded warm-up warnings are non-fatal; inspect the next unguarded step. See [REQ-SESSION-015](../../sdd/spec/session-lifecycle.md#req-session-015-container-port-readiness-gating-with-pre-warm-pre-condition).
2. For `container-unreachable`, check the Worker `Container WS forward failed` reason/error and `Existing container port unavailable` logs; a successful earlier health probe does not prove that the subsequent upgrade succeeded. Investigate private-port availability rather than assuming startup or R2 sync is stalled.

A container that actively **rejects** the WebSocket forward — rather than hanging — maps onto retryable 1013 instead of falling through to the route's generic 500 ([REQ-TERM-022](../../sdd/spec/terminal.md#req-term-022-an-unreachable-container-ends-the-upgrade-instead-of-escaping-it)). A failed no-start `/health` probe closes 1013 as `container-warming-up` before forwarding; a rejected, timed-out, or 503 forward after a successful probe closes 1013 as `container-unreachable`. Other non-101 handshakes may reach the browser as retryable 1006 without establishing a socket. A 101/1013 close establishes the socket but does not guarantee increasing reconnect delays: the client resets its attempt count on open.
...
A live terminal connection, connection attempt, or retry loop retains the session when eventually-consistent KV returns stale `stopped` metadata or temporarily omits the record; full list hydration and incremental polling use the same gate. Terminal admission returns definitive 4503 only when owner-scoped D1 lifecycle is `stopping` or `stopped`; a failed no-start health verification or terminal-not-ready returns retryable 1013 before rate limiting, while a verified survivor can forward despite stale SDK state under normal WS rate limits. Marker-aware `collectMetrics` repairs stale KV `stopped`/missing to `running` on a later healthy tick.

Check Worker logs for `collectMetrics: container running but KV stopped, re-asserting running` and the owner-scoped D1 lifecycle. Terminal admission's no-start health result gates retryable readiness; SDK state alone neither confirms exit nor vetoes a verified survivor. `session stopped with shutdown in flight, leaving stopped` means a deliberate stop owns the transition. See [REQ-SESSION-010](../../sdd/spec/session-lifecycle.md#req-session-010-session-lifecycle-is-observable-from-one-d1-projection) AC2-AC3, [REQ-SESSION-012](../../sdd/spec/session-lifecycle.md#req-session-012-transport-retry-never-invents-lifecycle-state) AC2, [REQ-SEC-020](../../sdd/spec/security.md#req-sec-020-ws-upgrade-rate-limit-short-circuits) AC1-AC4 and AC7, and [REQ-SESSION-018](../../sdd/spec/session-lifecycle.md#req-session-018-d1-lifecycle-evidence-is-generation-fenced) AC4-AC7.
...
When `ENTERPRISE_MODE=active`, entrypoint.sh registers the authorized catalog under the `codeflare-gateway` provider and pins Pi via `~/.pi/agent/settings.json` (`defaultProvider`/`defaultModel`). If Pi shows the wrong provider or model, check that `~/.pi/agent/models.json` contains the expected slash-free Dynamic Route handle or opaque `cf-native-<uuid>` handle and that `settings.json` has the matching default pin. Do not configure a slash-bearing model ID in the container because Pi parses `a/b` as `provider/model` and misroutes.

The `LlmInterceptor` maps an authorized Dynamic Route handle to `dynamic/<route>` and an authorized opaque native handle to the Worker-held `<provider>/<exact-model>` or `custom-<slug>/<exact-model>` selector ([REQ-ENTERPRISE-049](../../sdd/spec/enterprise-mode.md#req-enterprise-049-unified-enterprise-model-authorization), [REQ-ENTERPRISE-050](../../sdd/spec/enterprise-mode.md#req-enterprise-050-native-provider-compat-dispatch)). Verify that AI Routing can read the gateway catalog, the intended default is assigned, and its exact immutable profile or native/custom target has current verification. New discovered compatibility contracts explicitly bind `/compat/chat/completions`; historical Dynamic assignments retain REST-first/404 fallback. Native/custom-provider targets retain their saved compat or Bedrock Runtime transport. Discovered compat contracts include narrow complete-tool-name repair; historical provider-specific adapters retain Bedrock tool-name repair and Gemini thought-signature replay ([REQ-ENTERPRISE-059](../../sdd/spec/enterprise-mode.md#req-enterprise-059-native-provider-wire-adaptation)). (Claude Code is not in the enterprise agent set; only Copilot, Pi, and Bash run under `ENTERPRISE_MODE=active`.)

**Transport contract:** [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection), [REQ-ENTERPRISE-077](../../sdd/spec/enterprise-mode.md#req-enterprise-077-provider-native-bedrock-transport-dispatch). <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @impl: src/lib/ai-capability-discovery/index.ts::capabilityCandidates -->
...
Normal **Discover** selects and verifies one shared contract; review Tool calling, Reasoning, Streaming and Input caching, assign access and Save. Complete tools/replay and exact server receipt authority are required; caching is optional, and administrator confirmation cannot invent missing verification. See [Target capability discovery](target-capability-discovery.md#evidence-and-qualification) and [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection). The instructions below apply to retained **Advanced** workflows, not normal Discover. <!-- @impl: src/lib/ai-capability-discovery/index.ts::discoverTargetCapabilities --> <!-- @impl: web-ui/src/components/admin/AiRoutingFields.tsx::AiRoutingFields -->

**Check the outcome before changing configuration:** Discover Profile lists every compatible existing revision in named rows; choose **Assign profile**, Verify the selected draft, assign group access, then confirm Save. Several passing profiles, even with different mappings, are not an error. Their names do not identify the gateway-selected model. The Kimi preset supports non-off levels and can fit Kimi-compatible behavior without creating a new profile. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

`completion_limit` means a reasoning observation, tool call, or replay exhausted the fixed 4,096-token UI budget. That mode is incomplete, not proven incompatible; other passed modes may still yield a match or custom draft. There is no token control or secondary Check button. Discover Profile runs once when opened; another run requires explicit reopening. It shows rate-limit notices beside retained matches and a result explanation when no match or custom draft is available; it does not show candidate diagnostic panels. Verify Profile is a separate explicit action with **Technical check details** for selected-profile failures. Neither automatically retries nor escalates. The API still accepts 32–16,384 with a 4,096 default, independently of the UI. ([REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

If no existing revision fits but a unique subset passes, name it and choose Create & Assign, live-verify or explicitly confirm the draft, assign group access, then confirm Save. Failed modes are excluded. Conflicting partial mappings produce no custom draft; this does not suppress independently passing existing profiles. Authentication, rate-limit, provider-server, transport, and malformed-stream failures stop the whole scan. Fix that boundary before another run. Unknown provider request properties cannot be discovered by the finite protocol bank; do not invent mapping values to bypass a failure. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

#### Verify Profile is disabled or does not stay green

**Diagnose:** Detected models load automatically through read-only inventory; this is not a compatibility check. Choose a saved or canonical draft revision. Verify Profile is optional: use it for a live check, or choose Mark as verified to confirm your own assessment without paid model probes. Neither multiple models nor missing custom-backend descriptions disables these actions. A ready connection, loaded inventory, and selected profile are still required. Assign group access and confirm Save afterward. Global and group reasoning selectors show only the assigned profile's supported levels; loading, unassigned, and single-mode helpers explain disabled choices. One-mode values are still retained for Save. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

**Interpret:** The selected-profile table labels Compatibility, Tool call, and Tool replay per level as green Passed, red Failed, or orange Unclear. These observations do not prove increasing reasoning effort. Orange **Observed path passed** on a multi-model route is expected: one gateway-selected path cannot certify every backend. Technical check details describe the selected profile's check, not another discovery match. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))

**Verify:** Live Verified status requires complete current tool/replay evidence, exact profile identity, and matching fresh single-model inventory. Administrator-confirmed is a separate status and supplies no live-check results. Missing inventory or changes to version, node, provider, model, or backend description prevent it. Profile changes invalidate evidence even when switching back; starting a recheck invalidates earlier evidence so failure cannot retain green. A successful unchanged single-model check attaches evidence only to the draft. Confirm Save to retain it across reload, including any recomputed warnings at the same `baseRevision`; reload and preview again after a revision conflict. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification), [REQ-ENTERPRISE-040](../../sdd/spec/enterprise-mode.md#req-enterprise-040-enterprise-pi-check-lifecycle))

<a id="enterprise-reasoning-migration-or-activation-is-blocked"></a>
#### Enterprise reasoning migration or activation is blocked

**Symptom:** A legacy route has no executable profile, GPT-OSS cannot be selected, a Kimi default at `off` is rejected, or Save asks for warning confirmation.

**Cause:** GLM 5.3 and Kimi K2.6 are migration proposals only; GPT-OSS tool replay remains a nonassignable unresolved notice. Kimi's built-in has no `off` mapping. Missing or stale server-owned verification or administrator confirmation, including changed route identity, blocks activation. A successful observed-path check can instead carry a warning that other backends remain untested.

**Fix:** Select the proposed GLM/Kimi profile and a supported default, or another executable built-in or bounded custom revision for GPT-OSS. Inspect the active inventory, then Verify Profile or explicitly Mark as verified before assigning group access and confirming Save. Both alternatives require a matching server-issued receipt or retained current authority; warning acknowledgement alone cannot replace that explicit action. For a successful observed-path check, acknowledge the untested-backend warning without treating it as whole-route certification. Reload and preview again if the `baseRevision` changed. ([REQ-ENTERPRISE-043](../../sdd/spec/enterprise-mode.md#req-enterprise-043-enterprise-pi-verified-route-activation))
...
**Cause:** Pi treats the root transcript as delivery truth ([REQ-MEM-002](../../sdd/spec/memory.md#req-mem-002-capture-triggers-every-20-user-messages-and-on-resume), [REQ-VAULT-027](../../sdd/spec/vault.md#req-vault-027-pi-vault-extraction-delivery-is-visible-and-transactional)).
...
Redeploy to Enterprise so `EGRESS` is bound. To restore direct egress instead, transition protected managed-resource policy to mutable in the same Setup update that turns Strict Gateway Egress OFF, or make that transition first. Setup rejects OFF while protected policy remains active ([REQ-SETUP-016](../../sdd/spec/setup.md#req-setup-016-managed-resource-policy-safety)).
