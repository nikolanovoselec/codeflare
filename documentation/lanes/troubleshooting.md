...
| Pi review lanes completed but no monitor result appears ([REQ-AGENT-062](../../sdd/spec/agents.md#req-agent-062-pi-pr-boundary-review-result-delivery)) | The main-session assistant has not started/awaited the background `review-monitor`, the monitor is still waiting for lane files / `summary.md`, or an old preseed still uses the removed nonce announcement path. | Check `.git/codeflare-review-jobs/<head>/monitor.json` and `monitor.completed`, then start/await `review-monitor` from the main session for that exact head. `/review-results` remains the manual fallback for the saved summary. |
...
Note: the monitor design has no `autofix.requested` marker and no nonce announcement to inspect; the main session reacts to the background agent's `REVIEW_RESULT` handoff. `review-monitor` is an agent/subagent, while `preseed/agents/pi/extensions/review-enforcement.ts::startReviewMonitor` owns the durable handoff/claim state.
