...
The `LlmInterceptor` maps an authorized Dynamic Route handle to `dynamic/<route>` and an authorized opaque native handle to the Worker-held `<provider>/<exact-model>` or `custom-<slug>/<exact-model>` selector ([REQ-ENTERPRISE-049](../../sdd/spec/enterprise-mode.md#req-enterprise-049-unified-enterprise-model-authorization-and-publication)). Verify that AI Routing can read the gateway catalog, the intended default is assigned, and its exact immutable profile or native/custom target has current verification. Dynamic Routes retain REST-first fallback behavior; native and custom-provider targets use one direct compat request. Only Bedrock receives repeated-complete tool-name repair, and only Gemini receives thought-signature replay. (Claude Code is not in the enterprise agent set; only Copilot, Pi, and Bash run under `ENTERPRISE_MODE=active`.)
