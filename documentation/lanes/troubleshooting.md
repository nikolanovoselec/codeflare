...
| Pi CI resolver returns no request ([REQ-AGENT-068](../../sdd/spec/agents.md#req-agent-068-independent-pi-ci-monitoring), [AD99](../decisions/README.md#ad99-pi-ci-monitoring-uses-one-attached-native-background-subagent)) | The Git action was unchanged or unsupported, or no open PR targets `main`/`master`. | Verify the PR base and state. No request is expected until a later eligible head-changing push, main-bound PR creation, or explicit user request. |
