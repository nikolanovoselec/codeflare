...
| Session stays `running` but terminal, `/activity`, and `/health` all stop responding while the PTY was last reported alive | The container process still exists but the Durable Object's private connection to container services is stale; `ctx.container.running` and the SDK process monitor do not prove port reachability | Self-heals under [REQ-SESSION-021](../../sdd/spec/session-lifecycle.md#req-session-021-unreachable-container-transport-reconstructs-without-replacing-the-workload): 10 s probe bounds, 5 s confirmation retries, and third-failure `ctx.abort()`. The container, PTY, and KV status are preserved. Search logs for `resetting Durable Object to reconstruct container transport`. |
