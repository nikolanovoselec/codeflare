...
| Pi review says `summary_announcement_visible`, but no summary appears in chat ([REQ-AGENT-062](../../sdd/spec/agents.md#req-agent-062-pi-pr-boundary-review-result-delivery) AC2) | Delivery proof accepted a raw nonce substring from non-delivery transcript content or a stale/wrong remembered file instead of a persisted review custom-message entry. | Update the preseed so delivery proof parses JSONL and accepts only the expected nonce-bearing `custom_message` entry. Workaround: run `/review-results` to display the saved summary. |
