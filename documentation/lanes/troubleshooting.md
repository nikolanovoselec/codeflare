...
Normal during the ~10s cold-start window. The client's retry backoff reconnects once `terminalServiceReady` is true. Failed no-start health verification or terminal-not-ready returns retryable 1013 before rate limiting; a port-verified survivor can forward despite stale or unavailable SDK state under normal WS rate limits.
