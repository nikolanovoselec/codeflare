...
`completion_limit` means an off-mode reasoning observation, tool call, or replay exhausted the selected ceiling. That affected mode is incomplete, not proven incompatible. Check the overall `outcome`: other passed modes can still yield a usable existing match or custom draft. Increase the visible ceiling (up to 16,384) and explicitly run again only if you want to verify the incomplete modes and accept the cost. The default is 32; Codeflare never raises it automatically. Technical details identify the affected stage and distinguish request rejection, missing tool calls, replay rejection, incomplete final completion, and off that still emits reasoning.
