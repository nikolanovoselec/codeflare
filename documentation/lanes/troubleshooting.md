...
A container that actively **rejects** the WebSocket forward — rather than hanging — maps onto retryable 1013 instead of falling through to the route's generic 500 ([REQ-TERM-022](../../sdd/spec/terminal.md#req-term-022-an-unreachable-container-ends-the-upgrade-instead-of-escaping-it)). An unavailable existing private port returns a 101 socket closing 1013 (`container-unreachable`), distinct from the warm-up close (`container-warming-up`). Other non-101 handshakes may still reach the browser as 1006, retryable but without an established socket; those retries can remain at the 500 ms base. The 101/1013 path lets reconnect backoff advance.
