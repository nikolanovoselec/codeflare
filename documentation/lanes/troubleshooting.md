...
| Session shows `stopped` on the dashboard but container is actually running | `onError` fired on a transient platform event (deploy-roll, monitor blip) and a prior clobber-race guard prevented `collectMetrics` from correcting the status | Self-heals within one `collectMetrics` tick (~60 s) via AC4: when the `/health` probe confirms the container is running but KV reads `stopped` and no graceful-shutdown marker is set, `collectMetrics` re-asserts `running`. If the dashboard still shows stopped after ~2 min, check `wrangler tail` for `collectMetrics: container running but KV stopped, re-asserting running`; absence of that log line means the container genuinely stopped. See [REQ-SESSION-018](../../sdd/spec/session-lifecycle.md#req-session-018-persisted-status-is-authoritative-on-container-exit) AC4. |
