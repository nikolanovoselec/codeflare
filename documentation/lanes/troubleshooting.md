...
If an edit opens another tab or the Inline widget lacks Keep/Close, open **View: Toggle Output**, select **Codeflare Inline Chat**, reproduce once, and copy the activation, request, stream, `tabsChanged`, and snapshot lines. The revision and settings distinguish stale rollout; request admission distinguishes the wrong surface. An ordinary duplicate file tab in a new side group immediately after a same-resource stream means the renderer still has a different internal remote URI, even when the extension API normalizes both tab inputs to the same `file:` scheme and basename. On pinned code-server, verify the root workbench projection gives `folderUri.authority` the canonical public browser host rather than the server's `remote` placeholder. Do not add confirmation UI, reopen the editor, or invoke Chat Editing commands to mask this identity defect. The channel is bounded and retains only sanitized scheme, authority without userinfo, basename, and input type; it excludes full paths, query, fragment, tab labels, and document content. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::createInlineDiagnostics --> <!-- @impl: host/src/vscode-proxy.ts::projectVscodeWorkbenchWorkspace -->
