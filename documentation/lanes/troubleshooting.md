...
**Fix:** An absent control means Web Push is not configured; add the complete optional VAPID sender group and redeploy if Push delivery is wanted ([REQ-TERM-025](../../sdd/spec/terminal.md#req-term-025-per-device-notification-enrollment)). On iOS, add Codeflare to the Home Screen (Share → Add to Home Screen), open it from there, and enable notifications in Settings — the hint marks this state.
