...
| Pi review monitor reports findings but the main session does not autofix ([REQ-AGENT-059](../../sdd/spec/agents.md#req-agent-059-pi-durable-review-fix-loop), [REQ-AGENT-062](../../sdd/spec/agents.md#req-agent-062-pi-pr-boundary-review-result-delivery)) | Current preseed expects the main session to act on the background `review-monitor` completion result: first print the monitor's overview, then read `summary.md`, verify MEDIUM/HIGH/CRITICAL findings, and fix legitimate findings by default. If no fix starts, the latest user instruction may have said not to autofix / wait for approval / do not push, the monitor may have returned `REVIEW_RESULT clean|failed`, or required lane files / `summary.md` were incomplete. There is no `autofix.requested` marker or nonce announcement to inspect. | Read the monitor result, then run `/review-results` to display the exact summary and inspect `.git/sdd-review-results/<head>/`. If actionable findings remain and the user did not opt out, tell the main session to fix the legitimate findings from the summary. |
