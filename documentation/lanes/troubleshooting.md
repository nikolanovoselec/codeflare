...
If no existing revision fits but a unique subset passes, name it and choose Create & Assign, Verify the draft, assign group access, then confirm Save. Failed modes are excluded. Conflicting partial mappings produce no custom draft; this does not suppress independently passing existing profiles. Authentication, rate-limit, provider-server, transport, and malformed-stream failures stop the whole scan. Fix that boundary before another run. Unknown provider request properties cannot be discovered by the finite protocol bank; do not invent mapping values to bypass a failure. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

#### Verify Profile is disabled or does not stay green

**Diagnose:** Detected models load automatically through read-only inventory; this is not a compatibility check. Complete required backend descriptions, then Verify the selected saved or canonical draft revision. Unsaved custom revisions and changed backend descriptions do not require Save first. After successful verification, assign group access and confirm Save. Global and group reasoning selectors show only the assigned profile's supported levels; loading, unassigned, and single-mode helpers explain disabled choices. One-mode values are still retained for Save. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))
...
**Cause:** GLM 5.3 and Kimi K2.6 are migration proposals only; GPT-OSS tool replay remains a nonassignable unresolved notice. Kimi's built-in has no `off` mapping. Missing or stale server-owned verification, including changed custom-provider identity, blocks activation. A successful observed-path check can instead carry a warning that other backends remain untested.

**Fix:** Select the proposed GLM/Kimi profile and a supported default, or another executable built-in or bounded custom revision for GPT-OSS. Inspect the active inventory, complete backend descriptions, then Verify the selected profile before assigning group access and confirming Save. Activation requires a matching server-issued receipt or retained current verification; warning acknowledgement cannot replace it. For a successful observed-path check, acknowledge the untested-backend warning without treating it as whole-route certification. Reload and preview again if the `baseRevision` changed. ([REQ-ENTERPRISE-043](../../sdd/spec/enterprise-mode.md#req-enterprise-043-enterprise-pi-verified-route-activation))
