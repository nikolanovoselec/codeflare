...
1. For `container-warming-up`, check `/run/codeflare/sync/sync.log` for stalled R2 sync. If sync is healthy, locate the last startup log before the missing flag. Guarded warnings are non-fatal; inspect the next step. See [REQ-SESSION-015](../../sdd/spec/session-lifecycle.md#req-session-015-container-port-readiness-gating-with-pre-warm-pre-condition).
2. For `container-unreachable`, check Worker `Container WS forward failed` reason/error and `Existing container port unavailable` logs. A successful health probe does not prove the upgrade succeeded; investigate private-port availability instead of assuming stalled startup or R2 sync.
