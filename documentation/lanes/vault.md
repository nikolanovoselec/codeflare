...
Timeout/error states stay guarded and retry in the background; the button stays visible and click/tap feedback explains that this browser is preparing or retrying the Vault cache. A green ('armed') control — whether armed via the cold path or the reload-skip — opens DIRECTLY via the bootstrap-hop with no per-open recheck of local readiness or key recoverability ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC1); the key is armed for real at open time by the bootstrap-hop + SW `__cfRecover`.

On the cold path, the arming poll greens on the prewarm readiness proof alone (`pw === 'ready'`); `checkVaultKeyRecoverable` still runs at arm time but only as a NON-BLOCKING diagnostic — it no longer gates the green, because a `/.vault-key` false-negative on a healthy vault had left a fresh session breathing accent forever, never green ([REQ-VAULT-019](../../sdd/spec/vault.md#req-vault-019-vault-key-recoverable-open-diagnostic), [AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)).
...
- [REQ-VAULT-019](../../sdd/spec/vault.md#req-vault-019-vault-key-recoverable-open-diagnostic) - Vault key-recoverable open diagnostic
