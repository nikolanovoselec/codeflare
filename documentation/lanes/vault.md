...
| `/api/vault/:sid/` returns 503 | SilverBullet supervisor not ready | The Vault button stays visible but guarded until `Layout.tsx`'s `probeVaultReady()` receives `{ vaultReady: true }` from `GET /api/vault/:sid/status` and the current browser prewarm completes. Click/tap the button for its preparing/retrying status; the probe starts the hidden prewarm automatically once SilverBullet binds port 3030. |
...
| On a 2nd session in the same browser the vault opens empty and/or the terminal loses input focus and never recovers; the Service Worker console shows `[sync] File deleted on secondary, deleting from primary` for every plug/file right after `Performing a full sync cycle...` | Older image: with the bucket-stable persistent local store (REQ-VAULT-021), the SW ran its full-sync reconciliation while the in-container SilverBullet server was still warming up (~1-2 min) and its `fetchFileList()` returned empty/non-array, so every local file looked "deleted on secondary" and the whole local store was wiped (the editor then churned re-syncing and never handed focus back). | Redeploy. The `graftVaultKeyRecovery` not-ready guard ([REQ-VAULT-017](../../sdd/spec/vault.md#req-vault-017-silverbullet-native-service-worker) AC6 / [REQ-VAULT-021](../../sdd/spec/vault.md#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC8) aborts a sync cycle whose remote list is empty while the local store or snapshot is non-empty, deferring reconciliation until SB is actually serving the real list. The 2nd session now serves the persistent store immediately and deletes nothing until SB confirms the file list. |
