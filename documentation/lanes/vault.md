...
The marker is **always** recorded once the proof lands: if `lastStartedAt` has not polled in yet a placeholder value stands in and upgrades to the real start on the next mark (never downgrading back), so a return whose start has not re-polled still has a marker to arm from. On a later page load or return from the vault tab, where the in-memory prewarm status has reset, `Layout.tsx` skips re-mounting the bootstrap iframe and marks the control armed (green) directly with no click — unless the container has *demonstrably* restarted since the marker was recorded ([AD93](../decisions/README.md#ad93-key-the-vault-reload-skip-to-the-container-start-so-resumed-sessions-re-initialize-cleanly)).

That reload-skip fires whenever the marker is present and live local readiness still holds, and is refused only on a **positively-detected** container restart: a known current start that differs from a known recorded start. A missing current start (the batch-status poll has not re-delivered `lastStartedAt` yet on a mobile reload) or a placeholder marker is not proof of a restart, so the control re-arms green rather than going back to white and forcing a re-init every return — the regression that keying warmth to strict start-equality caused.
