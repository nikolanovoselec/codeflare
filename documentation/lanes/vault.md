...
The marker records the container's start (`lastStartedAt`) at prewarm COMPLETION, read untracked: a batch-status poll delivering the start mid-prewarm therefore does not re-run the effect and tear the in-flight iframe down (which had left a fresh session breathing `preparing`/white, never arming), and by completion the real start has almost always polled in, so even a click made before the first poll records a real marker. On a later page load or return from the vault tab, where the in-memory prewarm status has reset, `Layout.tsx` skips re-mounting the bootstrap iframe and marks the control armed (green) directly with no click — but only when the recorded start matches the container running now ([AD95](../decisions/README.md#ad95-vault-reload-skip-strict-start-match-recorded-at-prewarm-completion)).

That reload-skip is STRICT: it fires only when the recorded start equals the container start running now AND live local readiness still holds. A same-container return matches and re-arms green once `lastStartedAt` polls in — a brief `available` window on reload, then green, no click. A resumed container (advanced start) never matches, so the skip is refused and the vault re-initializes with a normal prewarm exactly like a fresh session. A start not yet polled is likewise unverified and never skips — an unverifiable container must never open onto a possibly-stale store.

`checkVaultLocalReadiness` verifies recorded `sb_data_*`/`sb_files_*` plus an active service worker, not evicted, with the liveness probe bounded by a short timeout. If the probe does not settle, the marker is absent, the current start has not polled in yet, or the recorded start does not match the container running now (a resumed session, advanced `lastStartedAt`), the control stays 'available' for an on-demand click and is never auto-mounted.
...
Once the vault is ready (`pw === 'ready'` in `Layout.tsx`'s `vaultButtonStatus`) the button is green and stays green for the rest of the live session, identical on mobile, tablet, and desktop because green carries no reload-dependent settle state ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC6). A same-container reload of an already-warm device shows `armed` (green) immediately and opens on a single click. A session resumed after the container restarted does not inherit that green state — its marker's `lastStartedAt` no longer matches the new start, so it re-initializes exactly like a fresh session ([AD95](../decisions/README.md#ad95-vault-reload-skip-strict-start-match-recorded-at-prewarm-completion)).
...
| Vault button shows armed (green, one-click-to-open) immediately after a page reload, with no prewarm iframe visible | This device completed a full prewarm earlier in the SAME container run, so the persistent `vault-session-<sid>-prewarmed` marker (stamped with the container's `lastStartedAt`) still matches and a live local-readiness check lets `Layout.tsx` skip re-mounting the bootstrap iframe ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC2, [AD95](../decisions/README.md#ad95-vault-reload-skip-strict-start-match-recorded-at-prewarm-completion)). | Expected, not stale: the skip needs a live `checkVaultLocalReadiness` proof (recorded `sb_data_*`/`sb_files_*` + active service worker) AND a marker `lastStartedAt` matching the current start. If storage was evicted or the container restarted (a resumed session), the skip is refused — the button stays 'available' and a click re-prepares the cache. |
