...
The current token salt is deliberately versioned. The v2 cutover changes `baseURI` once, which makes SilverBullet derive a fresh encrypted `sb_data_*`/`sb_files_*` pair and installs one new canonical worker scope. Historical registrations and databases are not enumerated, migrated, unregistered, or deleted; they remain visible as orphaned browser state. Every future session and ordinary worker update reuses the stable v2 scope, so current registrations no longer accumulate.
