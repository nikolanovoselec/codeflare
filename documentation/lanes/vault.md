...
| On mobile, after opening the vault and returning to the dashboard, the button's behaviour differs from desktop — the "ready" tooltip re-pops on every reload/remount and the post-open state is not identical across platforms | Older image: the button "settled" from green to a neutral icon after opening, persisted in `localStorage` so it would survive the mobile reload. That settle-on-return state could never be made identical across platforms — the mobile standalone PWA reloads on return from the vault tab, so it diverged to breathing green forever on mobile and, once persisted, over-corrected so the control never returned to green; the "ready" tooltip also re-fired on every remount. | Redeploy. The settle-on-return behaviour is removed: once the vault is ready the button is GREEN and stays green for the rest of the session (no settle, no persisted state), identical on mobile/tablet/desktop. The "ready" tooltip auto-shows only on the genuine `preparing` -> `armed` transition (tracked via `prevStatus`), so it no longer re-pops on a warm reload ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC11). |
