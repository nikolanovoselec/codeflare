...
- `GET /api/vault/<sid>/.vault-key` is an auth-gated endpoint that returns `{key}` JSON via the DO's `ensureVaultKey()`. Intended for the SW to recover the encryption key after browser idle termination ([REQ-VAULT-008](../../sdd/spec/vault.md#req-vault-008-zero-ui-vault-encryption) AC7). Same auth chain as `.codeflare-bootstrap`; `Cache-Control: no-store`. NOTE (AD69 Increment 1): the endpoint exists, but the native worker is currently served WITHOUT the recovery graft that consumes it - so after an idle-termination cycle the worker holds no key and SB re-auths. The graft (a vendored-byte edit or a client-side re-post of `set-encryption-key`) is deferred to Increment 2; until then AC7 is unmet and REQ-VAULT-008 is Partial.
- The service worker is SilverBullet's native worker (`VAULT_NATIVE_SERVICE_WORKER_JS`), the full SB sync engine plus its native `set-encryption-key` / `get-encryption-key` message handlers (over SB's private `encryptionKeyMemoryStore`). It holds the key in module memory only, scoped to `/api/vault/<sid>/`, gone the moment the browser tears the worker down. The former key-shim (`VAULT_KEY_SHIM_SERVICE_WORKER_JS`) is retained in `vault-html.ts` as a one-line revert but is no longer served.
...
| Vault reopen after closing the tab shows "Authentication not enabled" 403 on `.auth` | Browser terminated the idle SW, losing the in-memory encryption key. The bootstrap cookie was still set so the proxy skipped the bootstrap-hop and served SB directly. SB could not decrypt IDB and navigated to `.auth`. | KNOWN, AD69 Increment 1: the native worker is served without the `.vault-key` recovery graft, so this idle-termination re-auth is currently expected (REQ-VAULT-008 AC7 deferred to Increment 2). Workaround: clear the `codeflare_vault_bootstrap` cookie from DevTools and reload to force a fresh bootstrap-hop, which re-posts the key. The graft (Increment 2) will make reopen seamless again. |
