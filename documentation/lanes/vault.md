...
**Pi runtime divergence.** Pi implements REQ-VAULT-003 through the `memory-vault.ts` extension rather than a shell daemon + hook, but the graph result is identical to Claude. The extension is a pure trigger: it detects vault changes, advances the marker, re-publishes the existing cumulative `vault-graph.json` to the global graph, and spawns the `vault-extract` subagent.
...
**Sentinel namespacing.** The entrypoint vault-monitor daemon also runs under Pi because it is gated on advanced mode, not runtime. It writes the shared-namespace `vault-extract.vars`, which only Claude's UserPromptSubmit hook consumes; under Pi that file is never picked up. The Pi extension therefore reads its own trigger sentinels (`vault-extract.pi.vars` / `vault-extract.pi.in-flight`) and shares only the `vault-extract.last` high-water marker.

Advancing `.last` on each turn makes the daemon's `find -newer` come up empty, so the daemon stays quiet rather than wedging Pi's trigger gate with a file Pi can never clear. A `vault-extract.pi.vars` left behind by a crashed subagent self-clears once it ages past the 30-minute in-flight TTL (`VAULT_EXTRACT_INFLIGHT_TTL_MS`).
...
Timeout/error states stay guarded and retry in the background; the button stays visible and click/tap feedback explains that this browser is preparing or retrying the Vault cache. A green ('armed') control — whether armed via the cold path or the reload-skip — opens DIRECTLY via the bootstrap-hop with no per-open recheck of local readiness or key recoverability ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC1); the key is armed for real at open time by the bootstrap-hop + SW `__cfRecover`. On the cold path, the arming poll greens on the prewarm readiness proof alone (`pw === 'ready'`); `checkVaultKeyRecoverable` still runs at arm time but only as a NON-BLOCKING diagnostic — it no longer gates the green, because a `/.vault-key` false-negative on a healthy vault had left a fresh session breathing accent forever, never green ([REQ-VAULT-019](../../sdd/spec/vault.md#req-vault-019-vault-key-recoverable-open-diagnostic), [AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)).

On a successful full prewarm the bridge's complete ready proof (runtime ready + space sync + object index complete + `/.fs/` file listing) records a persistent per-browser marker, `vault-session-<sid>-prewarmed`, in `localStorage`, stamped with the container's current start timestamp (`lastStartedAt`).

The marker records the container's start (`lastStartedAt`) at prewarm COMPLETION, read untracked: a batch-status poll delivering the start mid-prewarm therefore does not re-run the effect and tear the in-flight iframe down (which had left a fresh session breathing `preparing`/white, never arming), and by completion the real start has almost always polled in, so even a click made before the first poll records a real marker. On a later page load or return from the vault tab, where the in-memory prewarm status has reset, `Layout.tsx` reads the marker's recorded start with `readVaultPrewarmMarker(sid)` and skips re-mounting the bootstrap iframe, marking the control armed (green) directly with no click ([AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)).

That reload-skip is OPTIMISTIC with revocation. At page-load a RETURN (same container) and a RESUME (restarted container) look identical — the browser has no live `lastStartedAt` until the batch poll delivers it — so a strict "unknown ⇒ not-green" rule re-initialized on every return (the original hated bug) while a lenient "unknown ⇒ green" rule skipped a resume onto stale files. Instead, the control arms green OPTIMISTICALLY the instant the marker exists and live local readiness holds, WITHOUT waiting for the start to poll in, so a same-container return re-greens immediately (no click, no re-init). The effect also tracks the container start; when it polls in and DIFFERS from the marker (a genuine resume), it REVOKES the optimistic green — dropping the control to 'available' so the next click runs a normal prewarm, exactly like a fresh session. The brief green flicker on an actual resume is the accepted trade-off for keeping the common case instant, and it never opens onto a stale store because the open is a fresh bootstrap-hop against live local readiness.

`checkVaultLocalReadiness` verifies recorded `sb_data_*`/`sb_files_*` plus an active service worker, not evicted, with the liveness probe bounded by a short timeout. If the probe does not settle or the marker is absent, the reload-skip is ineligible and the control stays 'available' for an on-demand click, never auto-mounted; if the marker is present but the polled-in start later proves a resume, the optimistic green is revoked back to 'available'.
...
Once the vault is ready (`pw === 'ready'` in `Layout.tsx`'s `vaultButtonStatus`) the button is green and stays green for the rest of the live session, identical on mobile, tablet, and desktop because green carries no reload-dependent settle state ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC6). A same-container reload or vault-tab return of an already-warm device shows `armed` (green) immediately (optimistic reload-skip) and opens on a single click. A session resumed after the container restarted briefly flickers green then has that optimistic green REVOKED once the advanced `lastStartedAt` polls in, dropping to 'available' so it re-initializes exactly like a fresh session ([AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)).
...
That ready signal requires local `sb_data_*`/`sb_files_*` proof, an active per-session service worker, a `space-sync-complete` signal from SilverBullet, SilverBullet's current object-index version complete with the index queue empty, and a local `/.fs/` listing containing the codeflare-authoritative files (`CONFIG.md`, `Index.md`, `STYLES.md`). The arming poll breathes the button green ('armed') the moment that prewarm proof holds (`pw === 'ready'`); it probes `/.vault-key` at the same time but only as a NON-BLOCKING diagnostic — the green never waits on it, because the key is armed for real at open time ([REQ-VAULT-019](../../sdd/spec/vault.md#req-vault-019-vault-key-recoverable-open-diagnostic)).

The second click then opens synchronously inside the gesture, directly via the bootstrap-hop with no readiness/key re-verify ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC1). The hidden prewarm shell is focus-inert, so the on-demand prewarm runs while the user types in the terminal without dismissing the mobile keyboard ([REQ-VAULT-020](../../sdd/spec/vault.md#req-vault-020-vault-prewarm-focus-safety)). Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
...
For AC1/AC2, the global graph should gain a `document` node for the file plus `concept` nodes for its visible titles, headings, and named entities, not skip it as opaque binary. For AC3, a citation edge should connect the PDF's document node to the wikilink concept so the two unify. For AC4, the corrupt PDF should emit only a bare document node while the healthy file still ingests and the high-water marker advances; one unreadable PDF does not block the batch.
...
| Vault button shows armed (green, one-click-to-open) immediately after a page reload or vault-tab return, with no prewarm iframe visible | This device completed a full prewarm for this session, so the persistent `vault-session-<sid>-prewarmed` marker plus a live local-readiness check let `Layout.tsx` arm green OPTIMISTICALLY and skip re-mounting the bootstrap iframe ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC2, [AD96](../decisions/README.md#ad96-vault-reload-skip-optimistic-arm-with-revocation-green-on-prewarm-proof-key-check-diagnostic)). | Expected, not stale: the skip needs a live `checkVaultLocalReadiness` proof (recorded `sb_data_*`/`sb_files_*` + active service worker). If the container was actually resumed, the polled-in `lastStartedAt` differs from the marker and the optimistic green is REVOKED — the button drops to 'available' and a click re-prepares the cache. If storage was evicted, the readiness probe fails and the skip is refused. |
...
| Vault button breathes `'preparing'` indefinitely on a perfectly stable, never-restarted container and never arms green | Older image: the on-demand prewarm effect read `sessionStore.sessions` directly (unmemoized) inside `Layout.tsx`; every batch-status poll tick (`lastActiveAt`/`ptyActive`/`startupStage` churn) re-ran the effect, whose `onCleanup` cancelled the in-flight prewarm AND cleared its `'prewarming'` status, so the effect re-mounted fresh on every tick, each restart pre-empting the last before it finished ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC2). | Redeploy. The container-start read is now firewalled behind a memo (`activeSessionStartedAt` in `Layout.tsx`) that only notifies when `lastStartedAt` actually changes, so poll churn no longer re-runs the prewarm effect. |
| Vault content stale in a long session; new notes only appear after a slow manual refresh | The SB client synced + re-indexed the whole space, most of it `Raw/Sessions/` machine logs the capture pipeline rewrites every ~15 user prompts, so IndexedDB churned constantly ([REQ-VAULT-015](../../sdd/spec/vault.md#req-vault-015-vault-idb-lifecycle-and-listing-filters) AC1). | Redeploy. `Raw/Sessions/` is now filtered from the `/.fs` listing (`isFilteredVaultListingName`), so the client no longer syncs/indexes it; the logs stay on disk for R2 + the memory graph. |
...
- [REQ-VAULT-019](../../sdd/spec/vault.md#req-vault-019-vault-key-recoverable-open-diagnostic) - Vault key-recoverable open gate
