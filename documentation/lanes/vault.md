...
The served worker is NOT the verbatim upstream bytes: `graftVaultKeyRecovery` (`src/routes/vault-native-sw.ts`) rewrites the native worker's `get-encryption-key` handler to re-fetch the key from `/.vault-key` when its in-memory key is empty (see the encryption section below). This graft is mandatory, not optional - the native worker flushes its key 5s after the last client disconnects and the browser can idle-terminate the worker at any time, so without recovery the key is gone before the shell boots and SB bounces to `.auth`; the first integration deploy reproduced exactly that on cold boot. The verbatim upstream bytes are stored separately (`VAULT_NATIVE_SW_VERBATIM`) and SHA-256 drift-guarded so a SilverBullet version bump is caught. The only AD69 item still gated on integration observation is the `/.client/*` precache-auth exemption (reserved [REQ-VAULT-013](../../sdd/spec/vault.md#req-vault-013-silverbullet-subpath-adapter) AC9), needed only if those precache fetches return 401.
...
- The service worker is SilverBullet's native worker (`VAULT_NATIVE_SERVICE_WORKER_JS`), the full SB sync engine plus its native `set-encryption-key` / `get-encryption-key` message handlers, with the codeflare `graftVaultKeyRecovery` patch applied to the `get-encryption-key` handler. The native worker stores the posted key in a module-local var and FLUSHES it 5s after the last client disconnects (and loses it on any idle-termination), with no upstream recovery - so the graft makes the handler, when it holds no key, first `fetch('/api/vault/<sid>/.vault-key', {credentials:'same-origin'})` (the SW fetch carries the session cookie), decode the returned key with SB's own decoder, then reply. This is the same fallback the old key-shim had, and it is what keeps cold boot AND idle-reopen from bouncing to `.auth`. The former key-shim (`VAULT_KEY_SHIM_SERVICE_WORKER_JS`) is retained in `vault-html.ts` as a one-line revert but is no longer served.
