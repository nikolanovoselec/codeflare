...
The custom key-shim service worker was previously used here, but has been removed. The real SilverBullet service worker is now fetched from the container and serves the site. To support incremental indexing while preserving the zero-UI encryption contract, `handleVaultRequest` injects the `codeflare_vault_bootstrap` cookie before registration and dynamically prepends key-recovery logic to the real SW as it passes through the proxy.
...
- The real SilverBullet service worker handles the `SyncEngine` and incremental indexing natively. Because the real SW forgets its in-memory encryption key on idle termination (which would prompt the user for a passphrase on reload), the proxy dynamically injects recovery logic into the SW script body during the `/service_worker.js` fetch. When `get-encryption-key` is received and no key is stored, the injected logic fetches `GET /api/vault/<sid>/.vault-key` (auth-gated, returns `{key}` JSON via the DO's `ensureVaultKey()`) and provides it to the native handler. This makes vault reopen seamless without re-running the bootstrap-hop.
...
| Browser console shows `Failed to register a ServiceWorker ... 401 ... fetching the script`; SilverBullet loads but appears unregistered as a PWA / offline mode never activates | Older image: SW registration GET at `/api/vault/<sid>/service_worker.js` ran the cookie-auth chain, but browsers may omit credentials on SW script fetches, so auth returned 401 and registration failed permanently | Redeploy. Codeflare now successfully injects the bootstrap cookie before registration so the real SW can auth its own `cache.addAll()` fetch. |
