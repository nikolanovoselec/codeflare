...
#### Not-ready sync guard ([REQ-VAULT-017](../../sdd/spec/vault.md#req-vault-017-silverbullet-native-service-worker) AC6, [REQ-VAULT-023](../../sdd/spec/vault.md#req-vault-023-bucket-stable-vault-store-persistence-and-content-bootstrap) AC2)
...
## Vault encryption and IDB lifecycle (REQ-VAULT-008, REQ-VAULT-015, REQ-VAULT-021, REQ-VAULT-023)
...
- `GET /api/vault/<token>/.vault-key` is an auth-gated endpoint that returns `{key}` JSON via `getVaultEncryptionKey` (the bucket-derived HKDF key). Used by the grafted native worker to recover the encryption key whenever its in-memory key is gone ([REQ-VAULT-008](../../sdd/spec/vault.md#req-vault-008-zero-ui-vault-encryption) AC9). Same auth chain as `.codeflare-bootstrap`; `Cache-Control: no-store`. (The dashboard's pre-open recoverability check fetches the session-keyed `/api/vault/<sid>/.vault-key`, which 302-redirects here.)
