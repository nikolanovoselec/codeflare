...
The button remains guarded until a same-origin/current-attempt bridge ready signal also proves current-device local readiness: recorded `sb_data_*`, recorded `sb_files_*`, and the active worker at the recorded current scope. During a native-worker update the registration's active slot may report `activating`; that state remains valid while the independent sync, index, and content proofs settle, whereas a missing or `redundant` worker fails readiness. Requiring only `activated` can keep the bridge guarded throughout that valid transition. The bridge holds this proof across `requiredReadyStreak` (2) consecutive polls before it arms the control, and a single not-ready poll resets the streak ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-control-gating-and-on-demand-prewarm-trigger) AC6, [REQ-VAULT-029](../../sdd/spec/vault.md#req-vault-029-canonical-browser-state-cutover-and-future-worker-safety) AC4).
