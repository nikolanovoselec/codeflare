...
On first browser open after a fresh vault, SilverBullet must build this browser's IndexedDB stores, complete its service-worker space sync, and build the object index. Codeflare does that work before the user can open the editor: the Vault button remains visible but guarded until the server probe succeeds and the hidden same-origin prewarm iframe emits the current-device bridge ready signal ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC1-AC9). That ready signal requires local `sb_data_*`/`sb_files_*` proof, an active per-session service worker, a `space-sync-complete` signal from SilverBullet, SilverBullet's current object-index version complete with the index queue empty, and a local `/.fs/` listing containing the codeflare-authoritative files (`CONFIG.md`, `Index.md`, `STYLES.md`). The hidden prewarm shell is focus-inert, so this work continues while the user types in the terminal without dismissing the mobile keyboard. Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
