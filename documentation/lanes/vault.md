...
The button surfaces this on-demand flow as a breathing affordance (`VaultButton.tsx`, the same breathing the "Return to Dashboard" icon uses). Server-ready is `available` (clickable, no breathing). The first click breathes the codeflare accent and auto-surfaces a focus-loss warning tooltip while indexing runs (`preparing`); when indexing completes the icon breathes green and auto-surfaces a "ready" tooltip that hides after 5s (the green breathing stays), and the second click opens the vault instantly (`armed`). A reload of an already-warm device shows `armed` immediately (one click to open). Once the open click fires, a per-session `vaultOpenedBySession` latch (set in `openVaultTab`) settles the warm control out of the green-breathing state to a plain neutral 'ready' icon (still clickable to reopen) instead of breathing green forever; the latch clears on session departure / page reload so a fresh visit re-arms green ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC11). `prefers-reduced-motion` keeps the state colours without the breathing animation ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC9).
...
| Always-overwrite (Codeflare-authoritative pages) | `Index.md`, `README.md`, `CONFIG.md`, `STYLES.md` | Copied from `/opt/silverbullet-preseed/`, gated so identical files are not rewritten. On every boot each page is additionally stamped with the immutable preseed source's mtime (`touch -r`), even when the content-equality skip left it untouched, so the in-container SB server reports a stable `lastModified` across sessions and the persistent client sync snapshot never sees a spurious "changed on secondary" ([REQ-VAULT-021](../../sdd/spec/vault.md#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC9 -- see [Deterministic preseed mtime](#deterministic-preseed-mtime-stops-the-2nd-session-preparing-loop)). User edits are silently reverted on next boot; these files are Codeflare-owned because they encode dashboard contract, SB `#meta` config, theme, and user guide. |
| Create-if-missing (user-editable pages) | `Raw/Graphs/Vault Graph.md`, `Notes.md`, `References.md` | Copied from `/opt/silverbullet-preseed/` only when absent. Never overwritten on subsequent boots -- user edits and deletions are preserved. `Vault Graph.md` seeds the `Raw/Graphs/` treeview folder on a fresh vault (treeview is page-driven; an empty directory is invisible); `Notes.md`/`References.md` resolve `Index.md`'s bare `[[Notes]]`/`[[References]]` wikilinks to real pages instead of broken/aspiring 404s ([REQ-VAULT-021](../../sdd/spec/vault.md#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC10). |
...
| On a 2nd session the Vault button breathes 'preparing' forever and never goes green; the Service Worker console shows `Index.md` repeatedly "changed on secondary" / index churn every few seconds | Older image: bisync/cp gave the byte-identical `Index.md` a fresh mtime each boot, which diverged from the REQ-VAULT-021 persistent client sync snapshot, so the SW re-enqueued an index op on every ~3s watch-poll and the prewarm readiness gate (index queue empty for consecutive polls) never settled. Cold start was fine (its snapshot is built fresh in-session). | Redeploy. `init_user_vault()` now stamps the four codeflare-authoritative pages with the immutable preseed source mtime (`touch -r`) on every boot ([REQ-VAULT-021](../../sdd/spec/vault.md#req-vault-021-bucket-stable-vault-url-and-bucket-derived-key) AC9), so the in-container SB server reports a stable `lastModified` and the spurious "changed on secondary" never fires. |
