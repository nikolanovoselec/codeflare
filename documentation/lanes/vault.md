...
| Vault content stale in a long session; new notes only appear after a slow manual refresh | The SB client synced + re-indexed the whole space, ~95% of which is `Raw/Sessions/` machine logs the capture pipeline rewrites every ~15 min, so IndexedDB churned constantly ([REQ-VAULT-015](../../sdd/spec/vault.md#req-vault-015-vault-idb-lifecycle-and-listing-filters) AC1). | Redeploy. `Raw/Sessions/` is now filtered from the `/.fs` listing (`isFilteredVaultListingName`), so the client no longer syncs/indexes it; the logs stay on disk for R2 + the memory graph. |
