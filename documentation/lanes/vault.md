...
On a successful full prewarm the bridge's complete ready proof (runtime ready + space sync + object index complete + `/.fs/` file listing) records a persistent per-browser marker, `vault-session-<sid>-prewarmed`, in `localStorage`, stamped with the container's current start timestamp (`lastStartedAt`). On a later page load, where the in-memory prewarm status has reset, `Layout.tsx` skips re-mounting the bootstrap iframe and marks the control armed (green) directly with no click — but only if the container has not restarted since the marker was recorded ([AD93](../decisions/README.md#ad93-key-the-vault-reload-skip-to-the-container-start-so-resumed-sessions-re-initialize-cleanly)).

That reload-skip happens only when the marker's `lastStartedAt` matches the container's current start and live local readiness still holds. `checkVaultLocalReadiness` verifies recorded `sb_data_*`/`sb_files_*` plus an active service worker, not evicted, with the liveness probe bounded by a short timeout. If the probe does not settle, the marker is absent, or the container has restarted since the marker was recorded (a resumed session), the control stays 'available' for an on-demand click and is never auto-mounted.
