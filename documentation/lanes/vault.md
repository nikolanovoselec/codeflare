...
On first browser open after a fresh vault, SilverBullet must build this browser's IndexedDB stores and object index. Codeflare now does that work before the user can open the editor: the Vault button remains visible but guarded until the server probe succeeds and the hidden same-origin prewarm iframe emits the current-device bridge ready signal ([REQ-VAULT-018](../../sdd/spec/vault.md#req-vault-018-vault-browser-prewarm-readiness-gating) AC1-AC7). Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
...
| Vault button opens during boot or while the first SilverBullet index is still running | UI is not gating on both readiness layers | The button should stay visible but `aria-disabled`/guarded until `Layout.tsx` receives a 200 from `HEAD /api/vault/:sid/` and `startVaultPrewarm()` receives a same-origin ready message with local `sb_data_*`/`sb_files_*`/service-worker proof from the hidden SilverBullet client. If it opens too early, check the server probe, the `codeflare-vault-prewarm` iframe message path, and the click-time local-readiness recheck. |
