...
- `GET /api/vault/<sid>/.vault-key` is an auth-gated endpoint that returns `{key}` JSON via the DO's `ensureVaultKey()`. Used by the key-shim SW to recover the encryption key after browser idle termination (REQ-VAULT-008 AC7). Same auth chain as `.codeflare-bootstrap`; `Cache-Control: no-store`.
