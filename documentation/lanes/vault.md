...
- **The user** edits notes via SilverBullet (or any other tool that touches files under `Notes/`, `References/`, `Inbox/`, or `Journal/`). Attachments dropped onto a note are written by SilverBullet next to the referencing note (so a Quick Note in `Inbox/2026-05-18/` collects PDFs and images in the same date folder); `Raw/Pasted/` remains as an optional hand-organised archive but SilverBullet never auto-routes there.
...
The Vault button in `Header.tsx` (`VaultButton`, between Bookmarks and Storage) opens the editor in a new tab via `window.open`. It only renders when an active session exists **and the session mode is `advanced`** -- `Layout.tsx` passes `onVaultOpen` only when both conditions hold (`sessionStore.activeSessionId && sessionStore.preferences.sessionMode === 'advanced'`). Default-mode sessions never see the button ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-readiness-gating) AC1, CF-060/CF-075).

Readiness has two layers. First, `Layout.tsx` issues `HEAD /api/vault/:sid/` and flips a per-session server latch on the first 200 response, retrying every 5s until success and then steady-probing every 60s ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-readiness-gating) AC4-AC5). That tests the actual vault proxy path and catches SB-crashed scenarios that a startup-stage flag check would miss. Second, once the server latch is set, `startVaultPrewarm()` mounts a hidden same-origin iframe at `/api/vault/:sid/.codeflare-bootstrap?codeflarePrewarm=1&prewarmId=...`. The bootstrap hop registers/configures the native service worker, preserves the prewarm query through the redirect, and the shell injection marks `window.sbRuntime.headless = true` without using SilverBullet's upstream `?headless` flag (which would disable the service worker). The shell carries a generic Codeflare prewarm bridge so the path also works when SilverBullet's service worker serves the precached shell instead of a fresh Worker response. The hidden client then uses SilverBullet's own `widgetsReady` path to post `codeflare-vault-prewarm` when IndexedDB, service-worker sync, full object-index completion, and page-list loading are done. The header button remains disabled until that browser prewarm message passes same-origin and prewarm-id validation. Timeout/error states stay disabled and retry in the background rather than opening a slow first-click vault.
...
On first browser open after a fresh vault, SilverBullet must build its browser IndexedDB stores and object index. Codeflare now does that work before the user can click: the Vault button remains disabled until the server probe succeeds and the hidden same-origin prewarm iframe reports SilverBullet's full client readiness ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-readiness-gating) AC4-AC6). Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
