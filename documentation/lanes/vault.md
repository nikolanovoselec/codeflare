...
6. **JSON output** - emits `{hookSpecificOutput:{...,additionalContext}}` with three launch constraints.
   ...
   - There is no blocking hook. An armed request relaunches once per user prompt except while a capture is still running ([REQ-MEM-020](../../sdd/spec/memory.md#req-mem-020-capture-requests-are-re-delivered-under-a-bound) AC3, AC5-AC6).
