...
`checkVaultLocalReadiness` verifies recorded `sb_data_*`/`sb_files_*` plus an active service worker, not evicted, with the liveness probe bounded by a short timeout. If the probe does not settle, the marker is absent, or a restart is positively detected (a resumed session, advanced `lastStartedAt`), the control stays 'available' for an on-demand click and is never auto-mounted.
