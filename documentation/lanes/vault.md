...
Readiness has two layers. First, `Layout.tsx` issues `HEAD /api/vault/:sid/` and flips a per-session server latch on the first 200 response, retrying every 5s until success and then steady-probing every 60s ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-readiness-gating) AC5-AC6). That tests the real vault proxy path and catches SB-crashed scenarios a startup-stage flag would miss.

Second, `startVaultPrewarm()` mounts a hidden same-origin iframe at `/api/vault/:sid/.codeflare-bootstrap?codeflarePrewarm=1&prewarmId=...`. The bootstrap hop registers/configures the native service worker and preserves the prewarm query through the redirect. `injectVaultPrewarmBridge()` then enables `window.sbRuntime.headless`, polls `window.sbRuntime.ready === true`, and posts `codeflare-vault-prewarm` to the parent. The button remains disabled until that bridge ready signal passes same-origin and prewarm-id validation. Timeout/error states stay disabled and retry in the background.
...
On first browser open after a fresh vault, SilverBullet must build its browser IndexedDB stores and object index. Codeflare now does that work before the user can click: the Vault button remains disabled until the server probe succeeds and the hidden same-origin prewarm iframe emits the bridge ready signal ([REQ-VAULT-012](../../sdd/spec/vault.md#req-vault-012-vault-button-render-and-readiness-gating) AC5-AC9). Subsequent user clicks open a tab against an already-prepared browser cache instead of showing the indexing state first.
