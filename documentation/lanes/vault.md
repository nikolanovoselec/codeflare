...
| Vault button shows armed (green, one-click-to-open) immediately after a page reload, with no prewarm iframe visible | This device completed a full prewarm earlier in the SAME container run, so the persistent `vault-session-<sid>-prewarmed` marker (stamped with the container's `lastStartedAt`) still matches and a live local-readiness check lets `Layout.tsx` skip re-mounting the bootstrap iframe ([REQ-VAULT-022](../../sdd/spec/vault.md#req-vault-022-vault-armed-state-open-flow-and-persistence) AC2, [AD93](../decisions/README.md#ad93-key-the-vault-reload-skip-to-the-container-start-so-resumed-sessions-re-initialize-cleanly)). | Expected, not stale: the skip requires both a live `checkVaultLocalReadiness` proof (recorded `sb_data_*`/`sb_files_*` + active service worker) AND a marker `lastStartedAt` matching the current container start. If origin storage was evicted, or the container restarted since the marker was recorded (a resumed session), the reload-skip is refused and the button stays 'available' for an on-demand click instead of opening stale/pre-stop content; clicking then re-prepares the cache. |
