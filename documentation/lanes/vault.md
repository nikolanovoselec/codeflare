...
- `init_user_vault()` at boot, republishing the vault under `user_vault` so a restored vault is queryable before any capture runs. The source is the cumulative `graphify-out/vault-graph.json`, never the empty `graph.json` scaffold that sits beside it ([REQ-MEM-009](../../sdd/spec/memory.md#req-mem-009-vault-graph-accumulates-monotonically-across-extractions) AC5). A first boot with no captures yet has no cumulative graph and publishes nothing.
...
1. **Repo resolution**: walk up from the candidate directory to the nearest checkout, identified by `.git` as a directory or as the file a worktree/submodule uses. Nothing else counts as a repo root ([REQ-VAULT-004](../../sdd/spec/vault.md#req-vault-004-unified-global-graph-merges-vault-and-active-repos) AC6).
...
4. **Reconcile ([REQ-VAULT-014](../../sdd/spec/vault.md#req-vault-014-graphify-active-repo-invariant-and-lock-serialisation) AC1/AC5)**: read the manifest's repo tags, remove every one that is neither `user_vault` nor the active checkout's tag, and add the active checkout's graph when its recorded `source_hash` differs or the tag is new. All of it runs inside one `flock -w 5 /tmp/graphify-global.lock` critical section.
5. **Sentinel advance**: both sentinels are rewritten and the path sentinel `touch`-bumped only after the whole reconciliation succeeded, so a lock timeout or a graphify failure leaves the prior state in place and the next tool call retries.
