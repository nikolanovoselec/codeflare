...
**Prerequisites:** Confirm the intended commit is `origin/main`, every required exact-head check is green, and the production environment owns the expected public configuration. Each deployment environment must define stable `VAPID_SUBJECT`, `VAPID_PUBLIC_KEY`, and `VAPID_PRIVATE_KEY` secrets; the subject and public key intentionally use the secret context so Actions masks step metadata, while only the private key is confidential at runtime. The workflow fails before Worker deployment when any value is missing, whitespace-padded, malformed, or when the unpadded-base64url P-256 key pair does not match. Keep the private key out of Wrangler configuration and logs. Rotating the key pair invalidates existing push subscriptions and requires users to re-enroll. For a manual retry, confirm the failed automatic run did not already promote the same tree.
