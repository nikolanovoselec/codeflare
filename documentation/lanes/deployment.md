...
**Prerequisites:** Confirm the intended commit is `origin/main`, every required exact-head check is green, and the production environment owns the expected public configuration. Each deployment environment must define stable `VAPID_SUBJECT` and `VAPID_PUBLIC_KEY` variables plus a secret-only `VAPID_PRIVATE_KEY`; the workflow fails before Worker deployment when any is missing. Keep the private key out of Wrangler configuration and logs. Rotating the key pair invalidates existing push subscriptions and requires users to re-enroll. For a manual retry, confirm the failed automatic run did not already promote the same tree.
