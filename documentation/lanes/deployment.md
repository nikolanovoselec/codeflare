...
Container resource usage scales per active session. Classic sessions may run up to six outer PTYs; opt-in Herdr sessions run one outer client plus inner pane processes in the same container. Idle sessions stop after `sleepAfter` (default 30m, configurable 15m-4h) of no user input. Durable files and Herdr's structural topology snapshot return from R2 on a later fresh container; arbitrary processes do not ([REQ-TERM-033](../../sdd/spec/terminal.md#req-term-033-durable-herdr-structural-session-recovery), [AD146](../decisions/README.md#ad146-terminal-mode-is-an-immutable-per-session-choice)).
