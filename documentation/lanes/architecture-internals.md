...
`LlmInterceptor` buffers model-routable requests so a complete REST 404 can be replayed through the compatibility transport before any stream starts. It strips container placeholder authorization and selects `dynamic/<route>` through the shared route-catalogue resolver. For Chat Completions it resolves that route's single active profile, removes its declared conflicting reasoning controls, applies the selected canonical Pi level (or scope default), and fails before provider I/O when the assignment or level is invalid. The gateway then chooses primary, conditional, percentage, limit, or fallback legs; stored per-leg profile references are evidence and never runtime selectors.
