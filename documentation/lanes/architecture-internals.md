...
Panel turns and editor-originated turns submitted through Code OSS's panel-open path share one IDE-owned Pi RPC process and one in-memory conversation, separate from terminal Pi. The panel submission occurs before inference because Code OSS 1.132 editor Inline Chat filters ordinary participant text and expects host-owned edit transactions, while Pi's unrestricted direct writes cannot be replayed safely as transactional edits. `NativePiRuntime` reserves owning panel requests FIFO because stream events carry no prompt ID, retains a reusable backend after normal completion, and retires it after cancellation or failure before replacement. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::NativePiRuntime -->
