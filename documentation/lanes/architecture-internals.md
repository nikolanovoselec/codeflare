...
Panel turns and editor proposal turns share one IDE-owned Pi RPC process and in-memory conversation, separate from terminal Pi. `NativePiRuntime` reserves both FIFO because stream events carry no prompt ID. Panel turns retain unrestricted tools; an editor turn temporarily exposes only one terminating proposal tool, then restores the exact prior set. The backend correlates its arguments, and the VS Code adapter validates the captured document version and bounded ranges before emitting native text edits. Normal completion retains the backend; cancellation, malformed proposals, or failure retire it before replacement. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::NativePiRuntime --> <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::registerInlineEditMode --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::parseInlineEditProposal -->
