...
| `src/lib/reasoning-profiles.ts` | Exact code-owned capability-profile catalog, bounded immutable custom-revision normalization, canonical hashes, protected-path validation, and scalar request translation. |
| `src/lib/reasoning-configuration.ts` | Atomic `setup:reasoning_configuration` parsing, reference integrity, revision immutability, legacy GLM/Kimi migration proposals, and activation warnings. |
| `src/lib/dynamic-route-inventory.ts` | One-pass active-version traversal, reachable conditional/fallback path summaries, and byte-identical common-mapping derivation. |
| `src/lib/reasoning-discovery.ts` | Deterministic Pi 0.84.4 streaming, tool-call, result-replay discovery and the shared REST/complete-404 compat attempt primitive. |
| `src/routes/admin/reasoning.ts` | Admin-only catalog, route inventory, and one-target non-activating discovery APIs using Worker-side gateway credentials. |
...
The route catalogue and HTTP outcomes live in [API Reference](api-reference.md). CORS sources live in [Configuration](configuration.md); the all-request `run_worker_first` boundary and failures where a Worker-owned route falls through to SPA HTML live in [Troubleshooting](troubleshooting.md).
...
`LlmInterceptor` buffers model-routable requests so a complete REST 404 can be replayed through the compatibility transport before any stream starts. It strips container placeholder authorization and selects `dynamic/<route>` through the shared route-catalogue resolver. For Chat Completions it resolves the route's active profile and maps reasoning to an exact executable level, otherwise the next higher, otherwise the highest lower. Absent or unrecognized hints use an applicable scope/preferred default; provider-default profiles discard reasoning controls. Unavailable assignments or profiles without executable mappings fail before provider I/O; discovery and verification remain strict. The gateway then chooses primary, conditional, percentage, limit, or fallback legs; stored per-leg profile references are evidence and never runtime selectors.

REST keeps the translated OpenAI fields. Only after a complete REST 404 does compat replay the same translation without `store` and `prompt_cache_key`. `/responses` remains route-pinned passthrough without profile translation, and `ensureStreamTerminator` repairs streamed Chat Completions outside native Bedrock, whose adapter owns completion. The same ordered configured user-group list controls first-match route restrictions and bounded metadata tags.

Provider-native Bedrock replay stores complete validated tool-turn content as encrypted Worker KV with a 30-day TTL. Adaptive thinking can legitimately omit normal thinking blocks, even at High; tool-only, text-and-tool, and redacted-only turns therefore retain authentic replay state too. Present thinking remains private to the Worker. Keys bind authenticated user, bound session, native target, and tool-call ID; restoration additionally matches each tool name and arguments. <!-- @impl: src/llm-interceptor.ts::nativeReplayStateKey --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor -->

The adapter validates AWS frame CRCs and typed routing headers, then decodes each chunk's JSON/base64 `bytes` envelope as UTF-8 Anthropic events. The JSON content-type header is optional. Its existing 2 MiB frame and 64 KiB replay bounds remain unchanged. Exceptions, invalid headers or payloads, and replay persistence failures emit a sanitized terminal OpenAI SSE error; generic terminator repair cannot append success afterward. Only message stop followed by clean EOF permits successful completion. See [AWS PayloadPart](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_PayloadPart.html), [REQ-ENTERPRISE-076](../../sdd/spec/enterprise-mode.md#req-enterprise-076-provider-native-bedrock-protocol-translation) AC3 and [REQ-ENTERPRISE-080](../../sdd/spec/enterprise-mode.md#req-enterprise-080-provider-native-bedrock-stream-completion). <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::decodeBedrockChunk --> <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::adaptBedrockAnthropicResponse -->

Administration fetches Dynamic Route names and active versions through Cloudflare's management API with the configured Worker-side token. Inventory expands every reachable element once and retains conditional/fallback path summaries; terminal `end`/`END` targets need no node. Custom-provider identity is administrator-owned provenance. A common level exists only when every leg has current Pi tool/replay evidence through `ai-gateway-chat-completions` and the normalized removal/write bytes match exactly. Discovery exercises one route/profile target with the pinned Pi 0.84.4 streaming wire shape and exact tool-result replay, records logical probes separately from HTTP attempts, and does not save or activate its draft. Credential containment and exact configuration remain in [Security](security.md) and [Configuration](configuration.md).
...
MultiView is a browser-local virtual workspace. It validates members against live sessions, allows two to four desktop members and exactly two tablet members, remains hidden on mobile, and is never sent to lifecycle, quota, storage, metrics, or terminal-route APIs. Focused-pane ownership prevents cleanup from an old pane clearing the current pane's detected URL. During overlapping reconnects under [REQ-TERM-016](../../sdd/spec/terminal.md#req-term-016-terminal-pane-reconnect-and-resize-authority), an unfocused successor records its fitted dimensions without stealing resize authority; if the old owner then disconnects, the host immediately applies those retained dimensions to the PTY and headless reconnect state. <!-- @impl: host/src/session.ts::resize --> <!-- @impl: host/src/session.ts::detach -->
...
The notification path does not call `recordInput()`, change `lastInputAt`, add a service-worker fetch/sync handler, or contact a stopped container. Final idle, quota, Stop, and Delete drains run before final R2 sync; terminal-convergence recovery skips them because its host transport is already unavailable. Canonical clicks stay on `/app/session/:sessionId`; the SPA selects only an ID found in the authenticated Session store and does not start a stopped session. Immutable mode-specific terminal ownership, configured-command startup, Herdr topology ownership, write batching, and mode-specific labeling are governed by [REQ-TERM-001](../../sdd/spec/terminal.md#req-term-001-terminal-surface-count-follows-session-mode), [REQ-TERM-005](../../sdd/spec/terminal.md#req-term-005-herdr-runtime-and-configured-agent-startup), [REQ-TERM-006](../../sdd/spec/terminal.md#req-term-006-herdr-owns-in-session-terminal-topology), [REQ-TERM-008](../../sdd/spec/terminal.md#req-term-008-write-batching-at-30fps), and [REQ-TERM-009](../../sdd/spec/terminal.md#req-term-009-process-label-ownership-follows-terminal-mode). Requirement status and remaining deployed evidence stay authoritative in [REQ-TERM-023](../../sdd/spec/terminal.md#req-term-023-away-only-agent-notification-delivery), [REQ-TERM-025](../../sdd/spec/terminal.md#req-term-025-per-device-notification-enrollment), [REQ-TERM-027](../../sdd/spec/terminal.md#req-term-027-service-worker-notification-display-and-navigation), [REQ-SEC-023](../../sdd/spec/security.md#req-sec-023-agent-notification-capability-boundaries), and [REQ-SEC-024](../../sdd/spec/security.md#req-sec-024-agent-notification-delivery-trust-boundaries).
