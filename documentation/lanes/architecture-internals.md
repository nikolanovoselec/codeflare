...
REST keeps the translated OpenAI fields. Only after a complete REST 404 does compat replay the same translation without `store` and `prompt_cache_key`. `/responses` remains route-pinned passthrough without profile translation, and `ensureStreamTerminator` remains the sole streamed Chat Completions repair. The same ordered configured user-group list controls first-match route restrictions and bounded metadata tags.

Provider-native Bedrock replay stores bounded signed-thinking blocks as encrypted Worker KV with a 30-day TTL. Keys bind authenticated user, bound session, native target, and tool-call ID; restoration additionally matches each tool name and arguments. The adapter emits a terminal OpenAI SSE error when eventstream decoding or replay persistence fails after streaming begins. <!-- @impl: src/llm-interceptor.ts::nativeReplayStateKey --> <!-- @impl: src/llm-interceptor.ts::LlmInterceptor --> <!-- @impl: src/lib/bedrock-anthropic-native-adapter.ts::adaptBedrockAnthropicResponse -->
