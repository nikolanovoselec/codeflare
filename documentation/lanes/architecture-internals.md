...
Panel turns and editor proposal turns share one IDE-owned Pi RPC process and in-memory conversation, separate from terminal Pi. `NativePiRuntime` reserves both FIFO because stream events carry no prompt ID. Panel turns retain unrestricted tools; an editor turn temporarily exposes only one terminating proposal tool, dispatches through Pi's `ExtensionAPI.sendUserMessage`, then restores the exact prior set. The backend correlates its arguments, and the VS Code adapter validates the captured document version and bounded ranges before emitting native text edits. Normal completion retains the backend; cancellation, malformed proposals, command-attributed or nested `<runtime>` dispatch errors, and transport failures retire it before replacement. <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/pi/native-chat.ts::NativePiRuntime --> <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::registerInlineEditMode --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::parseInlineEditProposal -->

For panel turns, the backend keeps answer and provider reasoning deltas separate. The participant forwards reasoning to Code OSS's native thinking presentation and coalesces tool starts into bounded, argument-free activity categories, so repeated reads and commands do not create an unbounded progress list. Editor proposal turns suppress final-answer markdown. The pinned Inline controller excludes the entire response when no confirmation is pending, and visibility is not durable after confirmation resolves. <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-027: a native Pi panel turn streams reasoning, bounds tool progress, and settles with its answer) -->

The correlated proposal carries one bounded what-and-why explanation in a native confirmation and a focus-independent Keep/Undo notification. Under [REQ-IDE-032](../../sdd/spec/browser-ide.md#req-ide-032-native-inline-review-correlation-safety), bounded correlation accepts only the current decision for each URI, so delayed superseded and duplicate actions are inert. Either accepted owned decision invokes the URI-scoped Chat Editing command and restores the ordinary text document. <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::registerInlineEditMode --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::resolveInlineReview --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-020 + REQ-IDE-026 + REQ-IDE-029 + REQ-IDE-031: inline-first renders one host-owned edit with native feedback) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-032: a delayed superseded review action cannot resolve a newer proposal for the same URI) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-032: pending review correlation evicts the oldest request beyond its bound) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-032: malformed and duplicate confirmation decisions invoke neither Pi nor review commands) -->
