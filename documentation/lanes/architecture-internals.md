...
For panel turns, the backend keeps answer and provider reasoning deltas separate. The participant forwards reasoning to Code OSS's native thinking presentation and coalesces tool starts into bounded, argument-free activity categories, so repeated reads and commands do not create an unbounded progress list.

Editor turns suppress unstructured final-answer markdown. Edit results retain their bounded explanation and count in result details; no-change results render their explanation without opening an editor transaction. Schema-invalid raw result starts may be corrected within three attempts; invalid-only settlement reports a bounded category. <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-027: a native Pi panel turn streams reasoning, bounds tool progress, and settles with its answer) --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-030: inline Pi accepts a valid retry after one invalid raw result) -->
