...
For panel turns, the backend keeps answer and provider reasoning deltas separate. The participant forwards reasoning to Code OSS's native thinking presentation and coalesces tool starts into bounded, argument-free activity categories, so repeated reads and commands do not create an unbounded progress list. Editor proposal turns suppress final-answer markdown and forward reasoning without treating the pinned host's filtered response rows as a durable transcript. The correlated proposal instead carries one bounded what-and-why explanation; the participant renders it through the native confirmation state admitted by the Inline controller and repeats it in a focus-independent Keep/Undo notification. A bounded registry accepts only the current decision for each URI, so delayed superseded and duplicate actions are inert. Either accepted owned decision invokes the URI-scoped Chat Editing command and restores the ordinary text document. <!-- @impl: preseed/agents/pi/extensions/inline-edit.ts::registerInlineEditMode --> <!-- @impl: openvscode/agent-sidebar/src/pi/node-rpc-backend.ts::PiRpcBackend --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::activate --> <!-- @impl: openvscode/agent-sidebar/src/extension.ts::resolveInlineReview --> <!-- @test: openvscode/agent-sidebar/test/backend-generation.test.ts (REQ-IDE-027: a native Pi panel turn streams reasoning, bounds tool progress, and settles with its answer) --> <!-- @test: openvscode/agent-sidebar/test/activation.test.ts (REQ-IDE-020 + REQ-IDE-026 + REQ-IDE-029: inline-first renders one host-owned edit with native feedback) -->
