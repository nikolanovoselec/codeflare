...
Provider-native Bedrock replay stores complete validated tool-turn content as encrypted Worker KV with a 30-day TTL. Adaptive thinking can legitimately omit normal thinking blocks, even at High; tool-only, text-and-tool, and redacted-only turns therefore retain authentic replay state too. Present thinking remains private to the Worker. Keys bind authenticated user, bound session, native target, and tool-call ID; restoration additionally matches each tool name and arguments.
