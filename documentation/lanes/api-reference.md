...
Discovery accepts `{ route, profileRef, maxCompletionTokens }` to revalidate an existing revision. Omitting `profileRef` tests bounded built-in protocol candidates and returns a draft only for one unambiguous compatible mapping. A maximal mapping may subsume a matching subset; divergent shared mappings remain ambiguous. Generated drafts return `profileDraft.classification: "Compatible, unverified"`, `toolCompatibility: { status: "unverified", levels: [] }`, and `validatedTransports: []`; sanitized evidence remains advisory, and activation requires warning confirmation. The completion ceiling is 32–16,384; results are sanitized and rate limited to 5/minute. REST runs first, and compat is permitted only after completely consuming a REST 404; only `store` and `prompt_cache_key` are removed on replay.
