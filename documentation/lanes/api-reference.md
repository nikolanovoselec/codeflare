...
| PATCH | `/api/sessions/:id` | Session cookie | [REQ-SESSION-010](../../sdd/spec/session-lifecycle.md#req-session-010-session-status-observable-from-dashboard), [REQ-SESSION-027](../../sdd/spec/session-lifecycle.md#req-session-027-user-can-rename-sessions) | Update session (rename or `tabConfig`); the accepted `name` is sanitized and returned |
...
| POST | `/api/sessions/:id/stop` | Session cookie | [REQ-SESSION-006](../../sdd/spec/session-lifecycle.md#req-session-006-user-can-stop-restart-and-delete-sessions), [REQ-SESSION-014](../../sdd/spec/session-lifecycle.md#req-session-014-user-configurable-auto-sleep-timeout-in-settings) | Claim a D1 termination intent, destroy the container, and confirm stopped only after exit evidence |
...
| GET | `/api/sessions/batch-status` | Session cookie | [REQ-SESSION-001](../../sdd/spec/session-lifecycle.md#req-session-001-session-creation-with-name-and-agent-type), [REQ-SESSION-035](../../sdd/spec/session-lifecycle.md#req-session-035-stale-stopping-records-reset-on-owner-status-read), [REQ-OPS-006](../../sdd/spec/operations.md#req-ops-006-idle-containers-stop-metered-container-resources), [REQ-IDE-048](../../sdd/spec/browser-ide.md#req-ide-048-default-workspace-and-dashboard-owned-vs-code-sessions) | Conditionally reset this owner's `stopping` rows older than three minutes, then return the owner-indexed lifecycle, readiness, activity, and metrics projection from D1; always `no-store` |
| GET | `/api/sessions/ancillary-status` | Session cookie | [REQ-AGENT-049](../../sdd/spec/agents.md#req-agent-049-auto-upgrade-preseed-on-release), [REQ-STOR-036](../../sdd/spec/storage.md#req-stor-036-managed-reconciliation-progress-reads), [REQ-ENTERPRISE-001](../../sdd/spec/enterprise-mode.md#req-enterprise-001-enterprise_mode-forces-unlimited-tier-and-pro-mode) AC6 | Slower usage, storage-cache, entitlement, managed-release, preseed, and governed-migration projection; always `no-store` |
...
The ancillary response may include a nonempty opaque `preseedUpgradeTarget` derived from the desired release or baked hash, sequence, mode, policy, and projection. This target stays stable across progress updates and supports page-local attempt suppression; it is neither applied state nor mutation authority. A hash mismatch sets `preseedNeedsUpgrade`, as does an Enterprise session whose stored mode is not yet Advanced. While a managed release is `upgrading`, a matching valid progress record may add `managedReleaseProgress: { phase, completed, total }`.
...
| POST | `/operator-webhook/v1/activities/claims/boundary` | Signed protected Action OIDC bearer and bounded JSON claim | [REQ-OPERATOR-054](../../sdd/spec/operator-registry.md#req-operator-054-protected-action-claim-and-stop-fence) | Claim the exact prepared Action start handoff once. |
| POST | `/operator-webhook/v1/activities/claims/publication` | Signed protected Action OIDC bearer and bounded JSON intent | [REQ-OPERATOR-055](../../sdd/spec/operator-registry.md#req-operator-055-pr-wide-publication-ordering) | Reconcile an opaque PR-wide effect receipt; no GitHub write or publication credential. |
...
| GET | `/operator-webhook/v1/activities/:activityId/status` | Activity read bearer capability | [REQ-OPERATOR-029](../../sdd/spec/operators.md#req-operator-029-capability-authenticated-webhook-edge), [REQ-OPERATOR-031](../../sdd/spec/operators.md#req-operator-031-non-consuming-webhook-observation) | Read bounded status and generation without consumption. |
| POST | `/operator-webhook/v1/activities/:activityId/continue` | Activity read bearer capability, JSON `{ "generation": 1 }` | [REQ-OPERATOR-053](../../sdd/spec/operator-registry.md#req-operator-053-enterprise-pr-boundary-review-handoff) | Claim one observed waiting generation for a fenced resume. |
...
### Protected Action boundary claim [REQ-OPERATOR-054](../../sdd/spec/operator-registry.md#req-operator-054-protected-action-claim-and-stop-fence) <!-- @impl: src/routes/operator-webhook.ts::app --> <!-- @impl: src/operators/review-boundary-claim.ts::claimVerifiedBoundaryAction -->

`POST /operator-webhook/v1/activities/claims/boundary` is Enterprise-only. It requires a signed GitHub `pull_request_target` OIDC bearer for the configured protected workflow and deployment audience, not a browser Access JWT. Its `application/json` body is at most 4096 bytes and contains exactly positive integers `repositoryId`, `pullRequest`, `runId`, `runAttempt` and lowercase 40-character SHA strings `head`, `base`, `mergeBase`; a stalled body is cancelled after two seconds. On verified current PR/run, human authority and session generation, `200` returns `{ activityId, origin, startCapability, repositoryId, pullRequest, head, base, mergeBase, workflowId, runId, runAttempt, generation, contextDigest }` once; `generation` is the session lifecycle generation, while `contextDigest` is the prepared context digest. Invalid bodies return `400`, absent Action bearer `401`, denied claims `403`, stale/absent handoffs `409`, and uncertain claims `503`; none return the capability. Responses are `no-store`. A lost successful response cannot redeem the consumed handoff again.

### Protected publication journal [REQ-OPERATOR-055](../../sdd/spec/operator-registry.md#req-operator-055-pr-wide-publication-ordering) <!-- @impl: src/routes/operator-webhook.ts::app --> <!-- @impl: src/operators/review-boundary-claim.ts::operateBoundaryPublication -->

`POST /operator-webhook/v1/activities/claims/publication` is Enterprise-only. Its signed protected `pull_request_target` OIDC bearer has a deployment-specific audience, distinct from the start-claim audience. Strict JSON (at most 4096 bytes; two-second stalled-body deadline) binds numeric repository/PR/workflow/run/attempt, exact head/base/merge-base, activity and session/terminal-drive generations, context and effect digests, one `artifact|comment|check` effect, and `begin|read|complete`. Only `complete` accepts a positive numeric `externalId`.

The parent rechecks protected workflow bytes and current GitHub PR, the consumed run handoff, and non-driving collected terminal Activity metadata before the Registry records an effect. `200` returns `new` once, then `pending` until a trusted publisher-reported numeric external ID is acknowledged; the journal binds only that ID as `published` metadata, not as GitHub provenance. No bearer, browser JWT, GitHub credential or result bytes are returned. Invalid JSON returns `400`, absent OIDC `401`, denied identity `403`, stale/conflicting context `409`, and uncertain I/O `503`; all responses are `no-store`.

A pending receipt never authorizes a blind second write. Execution expiry does not renew a start capability: existing receipts remain reconcilable, but new effects are denied. This journal does not itself prove independent GitHub publication, current check selection or required-check activation.
...
For activity-specific webhook operations, only continuation accepts a request body: `Content-Type: application/json` and at most 128 bytes of JSON with exactly one positive integer `generation`; a stalled body is cancelled after two seconds. Start, status and result reject bodies.

These activity-specific operations are throttled to 30 requests/minute per edge key. All webhook operations use `Cache-Control: no-store` and remain within the narrow managed Access bypass. Start success queues one admitted activity and returns its read capability once. Status is non-consuming and returns metadata including generation, never result bytes. Continuation atomically claims only the observed waiting generation and refuses a duplicate or stale generation. Result returns `202` while not ready and consumes one terminal redemption before delivery. <!-- @impl: src/routes/operator-webhook.ts --> <!-- @impl: src/operators/activity.ts --> <!-- @impl: src/routes/setup/access.ts::upsertOperatorWebhookBypassAccessApp -->
...
| POST | `/internal/operator/sync/inspect` | Exactly `{ paths }`, containing 1–128 unique canonical paths and at most 64 KiB of JSON | `200 { files }`, with each fixed-root `~/Operators` file represented by path, size and SHA-256; aggregate bytes are bounded to 8 MiB | `400 SYNC_REQUEST_INVALID`; `409 SYNC_OUTPUT_NOT_FOUND` or `SYNC_OUTPUT_MISMATCH`; `413 REQUEST_TOO_LARGE`; `500 SYNC_OPERATION_FAILED`; `503 SYNC_STATE_FAILED` |
...
Inspection and upload read declared files from the fixed `~/Operators` root without following symlinks. Inspection returns bounded size/digest identity without persisting or uploading; upload checks the declared identity and mirrors files beneath R2 `Operators/**`. The private parent-selected `.codeflare/operators/<activity>/<operation>/` prefix receives `manifest.json` only after every declared file upload. Exact completed repeats reconcile, changed repeats conflict, and a pre-existing nonterminal receipt is fenced as unknown rather than replayed. Restricted sessions use `/internal/bisync-trigger` for this scoped Sync now contract; only `/internal/final-sync` remains denied with `403 OPERATOR_BISYNC_DENIED`. <!-- @impl: host/src/operator-sync-http.ts::OperatorSyncHttpController --> <!-- @impl: host/src/operator-sync.ts::OperatorSyncService --> <!-- @impl: host/src/request-router.ts::createRequestHandler -->
...
| GET | `/api/admin/configuration` | Admin role | [REQ-SETUP-017](../../sdd/spec/setup.md#req-setup-017-mode-aware-administration-configuration-read), [REQ-SETUP-027](../../sdd/spec/setup.md#req-setup-027-native-target-configuration-projection) | Read mode-gated Environment values, revision, active run, sanitized latest summaries, and sanitized native targets. Absent or malformed native-target storage yields an empty projection; unreadable storage fails the request. |
...
| POST | `/api/admin/configuration-runs` | Admin role | [REQ-SETUP-018](../../sdd/spec/setup.md#req-setup-018-stateless-environment-preview-and-bounded-execution), [REQ-SETUP-024](../../sdd/spec/setup.md#req-setup-024-routine-oauth-identifier-persistence), [REQ-ENTERPRISE-069](../../sdd/spec/enterprise-mode.md#req-enterprise-069-dynamic-route-profile-persistence-before-access) | Start one bounded run and stream NDJSON snapshots. An empty authoritative `aiRouting` diff returns JSON `409 configuration_no_changes` before creating a run or writing configuration. Explicitly blank OAuth client IDs remove saved IDs; blank replacement secrets preserve saved secrets. |
...
| GET, POST | `/api/admin/reasoning/catalog` | Admin role | [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-provider-discovery-and-selection), [REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation), [REQ-ENTERPRISE-063](../../sdd/spec/enterprise-mode.md#req-enterprise-063-ai-gateway-management-url-compatibility) | Return built-in and custom profiles, compatibility notices, assignment usage, Dynamic Route names, and sanitized gateway-scoped provider presence. Dynamic Route and provider discovery report independent readiness. POST supports read-only gateway overlays or revision-checked saved-inventory reconciliation. |
| POST | `/api/admin/reasoning/native/profile-discovery` | Admin role, rate limited | [REQ-ENTERPRISE-047](../../sdd/spec/enterprise-mode.md#req-enterprise-047-native-ai-gateway-provider-discovery-and-selection), [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-native-provider-capability-catalog), [REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation) | Assess one configured native/custom provider model through bounded direct-compat probes. Returns exact matching profile revisions or a bounded custom-profile draft only from unambiguous compatible evidence. |
| POST | `/api/admin/reasoning/native/discover` | Admin role, rate limited | [REQ-ENTERPRISE-051](../../sdd/spec/enterprise-mode.md#req-enterprise-051-native-ai-gateway-provider-and-model-workspace), [REQ-ENTERPRISE-052](../../sdd/spec/enterprise-mode.md#req-enterprise-052-native-provider-verification-and-runtime-enforcement), [REQ-ENTERPRISE-053](../../sdd/spec/enterprise-mode.md#req-enterprise-053-native-target-identity-and-document), [REQ-ENTERPRISE-054](../../sdd/spec/enterprise-mode.md#req-enterprise-054-native-target-profile-and-lifecycle-administration), [REQ-ENTERPRISE-055](../../sdd/spec/enterprise-mode.md#req-enterprise-055-native-target-authority-and-save), [REQ-ENTERPRISE-060](../../sdd/spec/enterprise-mode.md#req-enterprise-060-native-target-input-validation), [REQ-ENTERPRISE-061](../../sdd/spec/enterprise-mode.md#req-enterprise-061-native-target-administration-projection) | Verify one exact native/custom provider target and selected immutable profile revision through bounded compat tool-call and replay, or record explicit administrator confirmation. Returns a temporary exact-identity Save receipt and no raw provider authority. |
| GET, POST | `/api/admin/reasoning/routes/:route/inventory` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation), [REQ-ENTERPRISE-063](../../sdd/spec/enterprise-mode.md#req-enterprise-063-ai-gateway-management-url-compatibility) | Return the active route version, every reachable model leg and conditional/fallback path, sanitized evidence, common levels/mapping, `inventoryDigest`, current saved `verification`, and warnings. POST accepts `gateway` and `backendDescriptions`. Missing routes return 404; malformed or unavailable inventory returns 502. |
| POST | `/api/admin/reasoning/discover` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection), [REQ-ENTERPRISE-037](../../sdd/spec/enterprise-mode.md#req-enterprise-037-enterprise-pi-custom-profile-generation), [REQ-ENTERPRISE-057](../../sdd/spec/enterprise-mode.md#req-enterprise-057-ai-gateway-connection-rotation), [REQ-ENTERPRISE-063](../../sdd/spec/enterprise-mode.md#req-enterprise-063-ai-gateway-management-url-compatibility) | Run bounded Pi 0.84.4 compatibility discovery for one route with saved Worker-side credentials. A supplied `profileRef` verifies a saved revision or exact `profileDraft`; with `administratorConfirmed: true`, it records administrator confirmation without model probes. Omitting `profileRef` requests named existing-profile matches or a non-persisted custom draft from compatible modes. |
...
Catalog POST accepts either a read-only transient `gateway` overlay or `{reconcileSaved:true,baseRevision}` for cleanup from complete current saved-Gateway inventories. The modes cannot be combined. The `reconciliation` object contains `status` (`unchanged` or `applied`), `removedDynamicRoutes`, `removedNativeTargetIds`, and the resulting `revision`; only persisted removals are reported as applied. GET and POST `{}` remain read-only. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: src/routes/admin/configuration-runs.ts::reconcileSavedAiRoutingConfiguration -->

Discovery and verification do not activate configuration. POST variants accept transient `gateway: {gatewayUrl, gatewayId?, replacementToken?}` overlays and otherwise reuse saved encrypted credentials. `gatewayId` is required for account API URLs and unnecessary for legacy full gateway URLs. No saved token is returned. Selected discovery also accepts `profileDraft` and `backendDescriptions`; drafts must canonicalize to the exact selected reference before provider I/O. Confirmed aiRouting Save remains the sole activation path and preserves explicit warning codes and baseRevision concurrency protection. Legacy ID-only saves preserve unchanged exact assignments and custom revisions. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes -->

Discovery accepts `{ route, profileRef, profileDraft?, gateway?, backendDescriptions?, maxCompletionTokens, administratorConfirmed? }` for a selected profile. Optional `administratorConfirmed` accepts only `true` and requires an exact canonical selected profile; it returns `classification: "Administrator-confirmed"`, `checkId`, and `verification.method: "administrator"` without model probes or live-check results. Omitting `profileRef` tests the finite built-in protocol bank and compares its compatible modes with enabled built-in and saved custom revisions. The route-only `outcome` is `existing-profile`, `custom-profile`, `ambiguous`, `inconclusive`, or `unsupported`. Every complete existing match returns a `matchedProfiles` entry containing `name`, exact `profileRef`, and `supportedLevels`, with no duplicate `profileDraft`. Different passing mappings remain available for administrator selection. Saved custom profiles do not expand the paid probe bank. Unknown provider properties are not inferred. <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes -->

When no existing profile fits, an unambiguous partial candidate may produce `profileDraft` containing only passed reasoning/tool/replay modes; failed modes, unproven off, and dangling aliases are removed. Complete existing matches take precedence over partial drafts. Only custom-draft construction requires a maximal mapping with identical shared mutations; divergent partial mappings remain ambiguous. Generated drafts retain `profileDraft.classification: "Compatible, unverified"`, `toolCompatibility: { status: "unverified", levels: [] }`, and `validatedTransports: []`; generated evidence remains advisory; activation requires either a successful selected live check or explicit administrator confirmation, followed by group assignment and confirmed configuration Save. ([REQ-ENTERPRISE-037](../../sdd/spec/enterprise-mode.md#req-enterprise-037-enterprise-pi-custom-profile-generation))

`diagnostics` contains bounded levels, stage, failure code, and optional HTTP status/transport. `candidateResults` retains these diagnostics and `verifiedLevels` for each tested protocol. A `completion_limit` at tool generation or replay means incomplete verification, not unsupported behavior. `requestedCompletionCeiling` echoes the chosen per-request limit: 32–16,384, default 4,096. The API range is unchanged; the UI always sends 4,096, exposes no token control, and never automatically retries or escalates. Results remain sanitized and rate limited to 5/minute. A rejected candidate stops without retry; authentication, rate-limit, server, transport, and malformed-stream errors stop further scan requests. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection))

HTTP 429 preserves previously completed matches as selectable recommendations, but suppresses custom drafts. Other fatal errors suppress both recommendations and drafts. REST runs first; compat is permitted only after completely consuming a REST 404, removing only `store` and `prompt_cache_key`. ([REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection)) <!-- @impl: src/routes/admin/reasoning.ts::reasoningRoutes --> <!-- @impl: src/lib/reasoning-discovery.ts::discoverPiCompatibility -->

Successful selected verification or administrator confirmation returns `checkId` and server-derived `verification`: schemaVersion, profileRef, routeVersion, inventoryDigest, connectionFingerprint, canaryVersion, supportedLevels, scope, checkedAt, and optional method. `method: "administrator"` identifies confirmation rather than a live check and produces the `administrator_confirmed` Save warning. Scope is `single-model` or `observed-path`; a live observed-path check produces the untested-backend warning. Missing custom-backend descriptions do not block live verification and limit its evidence to the observed path. `routeChecks` maps route names to receipt IDs or null (clear); omission preserves only unchanged current saved authority.

Dynamic and Native temporary Save receipts last 30 days, exceeding the one-hour minimum; saved authority does not expire with them. Save validates unchanged saved authority before temporary receipt lookup. A fresh valid receipt replaces observations; an unavailable or expired receipt may fall back only to that validated saved authority. Missing unsaved proof still rejects Save. Explicit null clears proof, and present mismatched receipts reject. Private replay retention is separate. <!-- @impl: src/lib/reasoning-verification.ts::issueRouteCheck --> <!-- @impl: src/lib/native-ai-targets.ts::issueNativeTargetCheck --> <!-- @impl: src/lib/admin-configuration.ts::validateConfigurationValues -->

For unavailable receipts, retry Save without rerunning the check. If recovery still fails, use Advanced to verify the selected profile or explicitly Mark as verified, then review and Save again. No check runs automatically. Live verification still requires complete supported-level tools/replay and applicable Off checks against unchanged inventory. Both authority methods retain exact identity validation and explicit warning acknowledgment at Save. <!-- @impl: src/lib/reasoning-verification.ts::readRouteCheck --> <!-- @impl: src/lib/admin-configuration.ts::normalizeAiReasoningConfiguration -->

aiRouting accepts `fallbackRouting: {enabled:false}` or `{enabled:true,routes,defaultRoute,reasoning}` and persists it in reasoningConfiguration. Disabled or absent fallback denies unmatched users. A checked connection and complete disabled provider drafts can be saved without routes or group policies, while only live-verified or administrator-confirmed targets become eligible for access. ([REQ-ENTERPRISE-044](../../sdd/spec/enterprise-mode.md#req-enterprise-044-enterprise-pi-minimum-save-and-access-policies), [REQ-ENTERPRISE-066](../../sdd/spec/enterprise-mode.md#req-enterprise-066-native-provider-draft-persistence-before-access)) <!-- @impl: src/lib/admin-configuration.ts::normalizeAiReasoningConfiguration -->

Without a global default, clients submit an empty route with `reasoning: "off"`; another reasoning value without a route returns a validation error. Empty configured group policies remain deny-only entries; first-match selection precedes eligibility filtering. Active dynamicRoutes are the group/fallback union. Client verification flags cannot authorize routes, and an empty runtime catalog denies inference before upstream I/O. ([REQ-ENTERPRISE-039](../../sdd/spec/enterprise-mode.md#req-enterprise-039-enterprise-pi-default-reasoning-controls), [REQ-ENTERPRISE-044](../../sdd/spec/enterprise-mode.md#req-enterprise-044-enterprise-pi-minimum-save-and-access-policies)) <!-- @impl: src/lib/admin-configuration.ts::normalizeAiReasoningConfiguration --> <!-- @impl: src/lib/access.ts::loadEnterpriseRouteConfig -->
...
| POST | `/api/github/clone` | Session cookie | [REQ-GITHUB-004](../../sdd/spec/github.md#req-github-004-clone-a-repository-into-a-session), [REQ-GITHUB-015](../../sdd/spec/github.md#req-github-015-workspace-repository-tracking) | Clone `{repo, ref?, sessionId}` into a running session. Outcomes include `200`, `404 SESSION_NOT_FOUND`, `409 CLONE_TARGET_EXISTS`, `502 CLONE_FAILED`, `503 NOT_RUNNING`, or `504`. A `200` persists the repository for resume (rate-limited 20/min) |
...
Under [REQ-GITHUB-015](../../sdd/spec/github.md#req-github-015-workspace-repository-tracking) the session also tracks the GitHub-hosted repositories present at the top of its workspace (up to 20, however they arrived). [REQ-GITHUB-016](../../sdd/spec/github.md#req-github-016-tracked-repository-restoration) restores absent tracked repositories without blocking readiness: no attempt starts once 180 seconds have elapsed, and each attempt that does start is separately capped at 120 seconds.
...
- `configure_ai_gateway` — persists the canonical AI Gateway URL and, for account API URLs, its gateway name (both plain), plus the token (encrypted at rest, no-clobber on blank). Legacy full gateway URLs remain supported. [REQ-ENTERPRISE-017](../../sdd/spec/enterprise-mode.md#req-enterprise-017-ai-gateway-configured-in-the-setup-wizard)
