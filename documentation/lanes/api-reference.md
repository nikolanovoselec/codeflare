...
| GET | `/api/github/repos?page=` | Session cookie | [REQ-GITHUB-002](../../sdd/spec/github.md#req-github-002-github-panel-and-repository-listing) | The user's accessible repos (owner + collaborator + org member), server-side proxy, 50/page (rate-limited 60/min); `401 NOT_CONNECTED` when no token |
...
| GET | `/auth/github/connect/callback` | Session cookie | [REQ-GITHUB-001](../../sdd/spec/github.md#req-github-001-github-token-capture-and-storage) | OAuth/App callback: exchanges the code and persists the token to the deploy-keys entry; does not mint a new session cookie |
