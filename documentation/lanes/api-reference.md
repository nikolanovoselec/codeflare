...
| GET | `/api/admin/reasoning/catalog` | Admin role | [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-route-administration) | Return the six built-ins, custom revisions, four nonassignable compatibility notices, assignment usage, and the auto-discovered Dynamic Route names. `routeCatalogStatus` is `unavailable` when Worker-side gateway credentials cannot read routes. |
| GET | `/api/admin/reasoning/routes/:route/inventory` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence) | Return the active route version, every reachable model leg and conditional/fallback path, sanitized evidence, common levels/mapping, and warnings. Missing routes return 404; malformed or unavailable inventory returns 502. |
| POST | `/api/admin/reasoning/discover` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence), [REQ-ENTERPRISE-035](../../sdd/spec/enterprise-mode.md#req-enterprise-035-enterprise-pi-protocol-match-selection), [REQ-ENTERPRISE-036](../../sdd/spec/enterprise-mode.md#req-enterprise-036-enterprise-pi-custom-profile-draft-lifecycle) | Run bounded Pi 0.84.4 compatibility discovery for one route with saved Worker-side credentials. A supplied `profileRef` revalidates a revision; omission requests named existing-profile matches or a non-persisted custom draft from compatible modes. |
...
The three `/api/admin/reasoning` routes are read/discovery APIs only. Custom revisions, route-wide assignments, route inventory/evidence references, and common mappings are saved through the existing `aiRouting` configuration preview and run/Apply APIs. Apply submits explicit warning codes with the same `baseRevision`; the server recomputes warnings, requires confirmation, and rejects a revision conflict. Catalog, discovery, and inventory all reuse the configured encrypted Worker-side AI Gateway URL/token, never browser credentials or a second profile-specific token.

Discovery accepts `{ route, profileRef, maxCompletionTokens }` to revalidate an existing revision. Omitting `profileRef` tests the finite built-in protocol bank and compares its compatible modes with enabled built-in and saved custom revisions. The route-only `outcome` is `existing-profile`, `custom-profile`, `ambiguous`, `inconclusive`, or `unsupported`. An existing match returns `matchedProfiles` entries containing `name`, exact `profileRef`, and `supportedLevels`, with no duplicate `profileDraft`. Saved custom profiles do not expand the paid probe bank. Unknown provider properties are not inferred.

When no existing profile fits, an unambiguous partial candidate may produce `profileDraft` containing only passed reasoning/tool/replay modes; failed modes, unproven off, and dangling aliases are removed. Complete existing matches take precedence over partial drafts. A maximal mapping may subsume matching subsets only with identical shared mutations; divergent mappings remain ambiguous. Generated drafts retain `profileDraft.classification: "Compatible, unverified"`, `toolCompatibility: { status: "unverified", levels: [] }`, and `validatedTransports: []`; evidence remains advisory and activation requires warning confirmation.

`diagnostics` contains bounded levels, stage, failure code, and optional HTTP status/transport. `candidateResults` retains these diagnostics and `verifiedLevels` for each tested protocol. A `completion_limit` at tool generation or replay means incomplete verification, not unsupported behavior. `requestedCompletionCeiling` echoes the chosen per-request limit: 32–16,384, default 32. The UI requires an explicit higher-budget run; no automatic retry or escalation occurs. Results remain sanitized and rate limited to 5/minute. A rejected candidate stops without retry, while authentication, rate-limit, server, transport, and malformed-stream errors stop the whole scan and suppress any recommendations/draft. REST runs first; compat is permitted only after completely consuming a REST 404, removing only `store` and `prompt_cache_key`.
