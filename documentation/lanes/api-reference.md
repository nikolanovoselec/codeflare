...
| POST | `/api/admin/reasoning/discover` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence) | Run deterministic Pi 0.84.4 streaming/tool/result-replay discovery for one route using the saved Worker-side AI Gateway connection. `{ route, profileRef, maxCompletionTokens }` revalidates an existing revision; omitting `profileRef` tests the bounded built-in protocol candidates and returns a custom-profile draft only for one unambiguous compatible mapping. The ceiling is 32–16,384; results are sanitized, non-persisted, and rate limited to 5/minute. |
