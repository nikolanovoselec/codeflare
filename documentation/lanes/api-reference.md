...
| POST | `/api/admin/reasoning/discover` | Admin role | [REQ-ENTERPRISE-033](../../sdd/spec/enterprise-mode.md#req-enterprise-033-enterprise-pi-discovery-and-multi-model-evidence) | Run the deterministic Pi 0.84.4 streaming/tool/result-replay canary for one `{ route, profileRef, maxCompletionTokens }` target. The ceiling is 32–16,384; results are sanitized and non-persisted. Rate limited to 5/minute. |
