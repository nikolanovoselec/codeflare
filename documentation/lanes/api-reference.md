...
| POST | `/api/admin/reasoning/native/discover` | Admin role, rate limited | [REQ-ENTERPRISE-048](../../sdd/spec/enterprise-mode.md#req-enterprise-048-bedrock-provider-default-capability-verification) | Verify one exact native Bedrock target through bounded compat tool-call and replay, or record explicit administrator confirmation. Returns a short-lived exact-identity Save receipt and no raw provider authority. |
