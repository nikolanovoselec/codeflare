...
| POST | `/api/github/clone` | Session cookie | [REQ-GITHUB-004](../../sdd/spec/github.md#req-github-004-clone-a-repository-into-a-session), [REQ-GITHUB-015](../../sdd/spec/github.md#req-github-015-workspace-repository-tracking) | Clone `{repo, ref?, sessionId}` into a **running** session's workspace; relays the container's outcome verbatim (`200` cloned / `409 CLONE_TARGET_EXISTS` / `502 CLONE_FAILED` / `504`); `404 SESSION_NOT_FOUND` when the session record is absent; `503 NOT_RUNNING` when the container is asleep (rate-limited 20/min). A `200` also records the repository on the session so a resume restores it |
