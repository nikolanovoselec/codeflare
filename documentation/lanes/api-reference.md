...
Selected-profile UI verification uses the existing discovery endpoint with an exact `profileRef`, bracketed by read-only inventory requests. Complete current Verified evidence with `toolReplay: true` and `ingress: "ai-gateway-chat-completions"` attaches only to one unchanged node/provider/model/backend/version in the route draft. Multi-model success is an observed path, not all-leg certification. Only explicit confirmed configuration Save persists evidence; GET restores it through unchanged KV machinery, and green UI status additionally requires matching fresh single-model inventory and exact profile identity. Warning confirmation and `baseRevision` checks remain mandatory. Compatibility does not prove increasing reasoning effort. ([REQ-ENTERPRISE-038](../../sdd/spec/enterprise-mode.md#req-enterprise-038-enterprise-pi-selected-profile-verification))
