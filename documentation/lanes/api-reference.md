...
| GET | `/api/admin/reasoning/catalog` | Admin role | [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-capability-profile-administration), [REQ-ENTERPRISE-034](../../sdd/spec/enterprise-mode.md#req-enterprise-034-enterprise-pi-administration-workflow) | Return the six built-ins, custom revisions, four nonassignable compatibility notices, assignment usage, and the auto-discovered Dynamic Route names. `routeCatalogStatus` is `unavailable` when Worker-side gateway credentials cannot read routes. |
