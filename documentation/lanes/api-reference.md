...
- `configure_model_routing` — persists the dynamic-route catalog, default route, per-route context windows, finite per-route reasoning profiles, and per-group routing. `routeReasoningProfiles` accepts `workers-ai-gpt-oss`, `workers-ai-glm-5.3`, or `workers-ai-kimi-k2.6`; when submitted, every catalog route must have both a profile and positive context window. Legacy routes without profiles remain readable but model requests fail closed until completed. [REQ-ENTERPRISE-012](../../sdd/spec/enterprise-mode.md#req-enterprise-012-setup-configured-dynamic-route-catalog-and-access-group-list), [REQ-ENTERPRISE-013](../../sdd/spec/enterprise-mode.md#req-enterprise-013-per-group-dynamic-routing), [REQ-ENTERPRISE-022](../../sdd/spec/enterprise-mode.md#req-enterprise-022-per-route-context-windows-for-dynamic-routes), [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-gateway-provider-compatibility)
...
Under `ENTERPRISE_MODE` the response additionally carries the stored enterprise configuration for wizard round-trip: Access groups, route catalog, `routeContextWindows`, `routeReasoningProfiles`, masked token flags, and toggles. Profile IDs are the finite set listed under `configure_model_routing`; a legacy numeric context-window entry appears without a profile until an administrator completes it. The response also includes `activeAgents` (the stored coding-agent selection intersected with installed CLIs, or every installed capable agent when absent/invalid) and `configurableAgents` (the build-installed subset of the governable `copilot`, `pi` universe); both agent fields are omitted outside enterprise mode ([REQ-ENTERPRISE-025](../../sdd/spec/enterprise-mode.md#req-enterprise-025-active-coding-agents-configured-in-the-setup-wizard), [REQ-ENTERPRISE-031](../../sdd/spec/enterprise-mode.md#req-enterprise-031-enterprise-pi-gateway-provider-compatibility), [REQ-AGENT-123](../../sdd/spec/agents.md#req-agent-123-installed-agent-runtime-availability)).
