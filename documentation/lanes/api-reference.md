...
Unique KV receipts expire after 15 minutes, but saved authority does not. Missing receipts fail closed with retry guidance and no automatic paid check. For live verification, complete supported-level tools/replay and applicable Off checks must pass against unchanged inventory. Both methods retain exact identity validation and explicit warning acknowledgment at Save. <!-- @impl: src/lib/reasoning-verification.ts::issueRouteCheck -->
