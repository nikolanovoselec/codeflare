...
| `mcp__graphify__*` tools not visible in Claude Code (REQ-AGENT-023) | Plugin manifest absent or `~/.claude.json` malformed | Check `~/.claude/plugins/graphify/.claude-plugin/plugin.json` exists (R2 bisync should have delivered it). Then `jq '.mcpServers.graphify' ~/.claude.json` - expect `{command:"python3",args:["-m","graphify.serve"]}`. If null, re-run `entrypoint.sh`. |
