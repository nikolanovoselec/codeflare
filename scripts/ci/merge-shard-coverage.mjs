...
export function mergeShardCoverage({ artifactRoot, outputDir, expectedReports, thresholds }) {
  ...
  for (const relativePath of reportPaths) {
    ...
    for (const [source, coverage] of Object.entries(report)) {
      if (!coverage || typeof coverage !== 'object' || coverage.path !== source) throw new Error(`invalid coverage record: ${source}`);
...
function main() {
  const [artifactRoot, outputDir, rawExpected, ...rawThresholds] = process.argv.slice(2);
  if (!artifactRoot || !outputDir || rawThresholds.length !== METRICS.length) {
    throw new Error('Usage: merge-shard-coverage.mjs <artifact-root> <output-dir> <expected-reports> <statements> <branches> <functions> <lines>');
  }
  ...
  if (!mergeShardCoverage({ artifactRoot, outputDir, expectedReports, thresholds })) process.exitCode = 1;
}
